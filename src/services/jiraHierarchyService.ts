...
function issueTypeToLevel(type: string): { level: number; name: string; color: string; colorText: string } {
  switch (type) {
    ...
    case 'Sub-task':
      return { level: 4, name: 'Sub-task', color: 'var(--cp-ink-3, #64748B)', colorText: '#475569' };
    case 'QA Bug':
    ...
    default:
      return { level: 3, name: type, color: 'var(--cp-ink-3, #64748B)', colorText: '#475569' };
  }
}
...
function statusCategoryToColors(category: string): { color: string; colorText: string; isTerminal: boolean } {
  switch (category) {
    ...
    case 'To Do':
    default:
      return { color: 'var(--cp-ink-3, #64748B)', colorText: '#475569', isTerminal: false };
  }
}

function priorityToObj(priority: string | null): WorkItem['priority'] | undefined {
  if (!priority) return undefined;
  return { name: priority, color: 'var(--cp-ink-3, #64748B)', colorText: '#475569' };
}
