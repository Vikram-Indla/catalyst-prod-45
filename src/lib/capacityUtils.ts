...
export const GOLDEN_HOUR = {
  expert: 'var(--ds-chart-teal-bold)',      // Teal - Full allocation, success
  advanced: 'var(--ds-background-warning-bold)',    // Amber - Warnings, underallocation
  intermediate: 'var(--ds-link)', // Blue - Brand accent
  beginner: 'var(--ds-text-disabled)',    // Gray - Light states
  none: 'var(--ds-border)',        // Grey - Neutral
} as const;
...
export function getStatus(percentage: number): AllocationStatusInfo {
  if (percentage > 100) {
    return { 
      status: 'over', 
      label: 'Overallocated', 
      colorClass: 'text-[var(--ds-background-warning-bold)] bg-[var(--ds-background-warning-bold)]/10' // Amber for over
    };
  }
  if (percentage >= 80) {
    return { 
      ...
      colorClass: 'text-[var(--ds-chart-teal-bold)] bg-[var(--ds-chart-teal-bold)]/10' // Teal for full
    };
  }
  return { 
    status: 'under', 
    label: 'Underallocated', 
    colorClass: 'text-[var(--ds-text-subtlest)] bg-[var(--ds-text-subtlest)]/10' // Gray for under
  };
}
