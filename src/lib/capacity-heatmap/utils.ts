...
export function getUtilizationColor(percentage: number, mode: 'standard' | 'thermal' = 'standard') {
  ...
  if (mode === 'thermal') {
    ...
    if (percentage <= 40) return { bg: 'var(--ds-background-success, #DFFCF0)', text: 'var(--ds-background-success-bold, var(--ds-background-success-bold, #1F845A))', pulse: false };
    if (percentage <= 70) return { bg: 'var(--ds-background-information, var(--ds-background-information, #E9F2FF))', text: '#0369a1', pulse: false };
    if (percentage <= 85) return { bg: 'var(--ds-background-information, var(--ds-background-information, #E9F2FF))', text: 'var(--ds-link-pressed, #1d4ed8)', pulse: false };
    if (percentage <= 100) return { bg: 'var(--ds-background-information-bold, var(--ds-link, #0C66E4))', text: 'var(--ds-link-pressed, #1e40af)', pulse: false };
