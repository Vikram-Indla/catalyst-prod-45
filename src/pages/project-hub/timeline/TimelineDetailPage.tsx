...
export default function TimelineDetailPage() {
  ...
  useEffect(() => {
    ...
    async function resolve() {
      try {
        ...
        setDebugInfo(`Query returned null. issue_key="${issueKey}" not found.`);
        setLoading(false);
      } catch (err: any) {
        if (!cancelled) {
  ...
  const handleOpenItem = (itemId: string) => {
    navigate(`/project-hub/${projectKey}/timeline/${itemId}`);
  };

  const handleClose = () => {
    navigate(`/project-hub/${projectKey}/timeline`);
  };
  ...
  if (!issue) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', fontFamily: 'var(--cp-font-body)', gap: 12 }}>
        ...
        <button
          onClick={handleClose}
          style={{ marginTop: 8, padding: '8px 16px', background: 'var(--ds-text-brand, #0052CC)', color: '#FFFFFF', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--cp-font-body)' }}
        >
          Back to timeline
        </button>
      </div>
