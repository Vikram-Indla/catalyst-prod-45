    ...
    <div style={{ fontFamily: 'var(--cp-font-body)', padding: '20px 24px 16px' }}>
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="relative">
          <Search size={14} color="var(--ds-text-subtlest, #94A3B8)" className="absolute left-2.5 top-1/2 -translate-y-1/2" />
          <input
            value={searchText}
        ...
        <button
          ...
          style={{
            ...
            color: hasActiveFilters ? 'var(--ds-text-brand, #2563EB)' : ('var(--cp-text-secondary)'),
            fontFamily: 'var(--cp-font-body)',
            transition: 'all 150ms ease',
          }}
        >
          ...
          {hasActiveFilters && (
            <span
              ...
              style={{ width: 16, height: 16, background: 'var(--ds-text-brand, #2563EB)', color: 'var(--ds-surface, #fff)', fontSize: 9, fontWeight: 700 }}
            >
      ...
      {activeView === 'timeline' && (
        ...
        >
          <GanttChart size={36} color="var(--ds-text-subtlest, #94A3B8)" strokeWidth={1.5} />
...
function FilterDropdown({ label, value, options, isOpen, onToggle, onChange }: {
  ...
  return (
    <div className="flex flex-col gap-1 relative" data-filter-dropdown>
      <label style={{ fontSize: 10, fontWeight: 600, color: 'var(--ds-text-subtlest, #94A3B8)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        {label}
      </label>
      <button
        onClick={onToggle}
      ...
      >
        ...
        <ChevronDown size={12} color="var(--ds-text-subtlest, #94A3B8)" />
      </button>
      {isOpen && (
        <div
        ...
        >
          {options.map(o => (
            ...
            >
              {o.label}
              {value === o.value && <span style={{ float: 'right', color: 'var(--ds-text-brand, #2563EB)', fontSize: 11 }}>✓</span>}
            </button>
          ))}
        </div>
      )}
