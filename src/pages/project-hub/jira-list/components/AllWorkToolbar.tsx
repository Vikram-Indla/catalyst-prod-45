...
export function itemPassesFilters(item: WorkItem, f: FilterState): boolean {
  ...
  if (f.status.length > 0 && !f.status.includes(toLabel(item.statusName)))
    return false;
  ...
  return (
    ...
    >
      ...
      {facet === "assignee" || facet === "reporter" ? (
        ...
        <span style={{ flex: 1 }}>
          <Lozenge
            appearance={statusToLozenge(opt.label, opt.meta?.statusCategory)}
          >
            {opt.label}
          </Lozenge>
        </span>
  ...
  return (
    <>
      ...
      >
        ...
        >
          <span style={{ color: isActive ? undefined : SUBTLE }}>
            {chipLabel}
          </span>
        </Button>
      </span>
            ...
            <div style={{ flex: 1, padding: 4, overflowY: "auto" }}>
              {filteredOptions.length === 0 ? (
                ...
                filteredOptions.map((opt) =>
                  renderFacetRow(facet, opt, selected.includes(opt.value), () =>
                    onToggle(opt.value),
                  ),
                )
              )}
...
export function AllWorkToolbar({
  ...
  const filterOpen =
    filterOpenProp !== undefined ? filterOpenProp : filterOpenLocal;
  ...
  const [openChipKey, setOpenChipKey] = useState<FilterFacet | "more" | null>(
    null,
  );
