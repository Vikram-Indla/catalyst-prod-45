...
export default function EvidenceToExecutionFull() {
  ...
  if (stage === 'br-view') {
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', minHeight: '100vh', backgroundColor: '#FFFFFF' }}>
        {/* Left Content */}
        <div style={{ borderRight: '1px solid #DFE1E6' }}>
          {/* Top Action Bar */}
          <div style={{ padding: '16px 32px', borderBottom: '1px solid #DFE1E6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '12px', color: '#626F86', marginBottom: '4px' }}>Senael BAU / BAU-104</div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button style={{ padding: '8px 16px', backgroundColor: '#F7F8F9', border: '1px solid #DFE1E6', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }}>
                Discuss
              </button>
            </div>
          </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {[
                    ...
                    <div key={i} style={{ padding: '12px', backgroundColor: '#F7F8F9', borderRadius: '4px' }}>
        ...
        <div style={{ padding: '32px 16px', backgroundColor: '#F7F8F9', overflowY: 'auto' }}>
          <h4 style={{ marginTop: 0, fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', color: '#626F86' }}>Details</h4>
          ...
          <div style={{ borderTop: '1px solid #DFE1E6', paddingTop: '16px' }}>
  ...
  if (stage === 'evidence-modal') {
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', minHeight: '100vh', backgroundColor: '#FFFFFF' }}>
        <div style={{ borderRight: '1px solid #DFE1E6', padding: '32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', minHeight: '100vh', backgroundColor: '#FFFFFF' }}>
        <div style={{ padding: '32px', borderRight: '1px solid #DFE1E6' }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ fontSize: '14px', color: '#626F86' }}>{currentStep}</div>
            <div style={{ width: '100%', height: '8px', backgroundColor: '#F0F0F0', borderRadius: '4px', overflow: 'hidden', marginTop: '8px' }}>
              <div style={{ height: '100%', backgroundColor: '#0052CC', width: `${progress}%`, transition: 'width 0.2s' }} />
            </div>
            <div style={{ fontSize: '12px', color: '#626F86', marginTop: '8px' }}>{progress}% complete</div>
          </div>
          {progress === 100 && (
            <div style={{ padding: '16px', backgroundColor: '#DFFCF0', borderRadius: '4px', marginTop: '24px' }}>
  ...
  if (stage === 'evidence-pack') {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', minHeight: '100vh', backgroundColor: '#FFFFFF' }}>
        <div style={{ borderRight: '1px solid #DFE1E6' }}>
          <div style={{ padding: '32px', borderBottom: '1px solid #DFE1E6' }}>
            <h1 style={{ margin: '0 0 8px 0' }}>Evidence Pack — BR-104</h1>
            <div style={{ fontSize: '14px', color: '#626F86' }}>Industrial License Renewal Enhancement</div>
          </div>

          <div style={{ borderBottom: '1px solid #DFE1E6', padding: '0 32px', display: 'flex', gap: '16px', overflowX: 'auto' }}>
            {tabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setSelectedTab(tab)}
          ...
          <div style={{ padding: '32px' }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
                  {[
                    ...
                    <div key={stat.label} style={{ padding: '12px', backgroundColor: '#F7F8F9', borderRadius: '4px', border: '1px solid #DFE1E6' }}>
                      <div style={{ fontSize: '12px', color: '#626F86', marginBottom: '4px' }}>{stat.label}</div>
                      <div style={{ fontSize: '18px', fontWeight: 600, color: '#172B4D' }}>{stat.value}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {selectedTab === 'Comments Intelligence' && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {[
                    { type: 'Decision', color: '#0052CC', text: 'Arabic + English support mandatory', author: 'Vikram' },
                    { type: 'Scope Change', color: '#974F0C', text: 'Add phone OTP verification', author: 'Abdullah' },
                    { type: 'Clarification', color: '#216E4E', text: 'Identity verification requirements?', author: 'Aisha' },
                    { type: 'Unresolved', color: '#974F0C', text: 'Who owns approval workflow?', author: 'Marcus' },
                    { type: 'Contradiction', color: '#AE2A19', text: '24 hours vs 3-5 business days', author: 'Priya' },
                    { type: 'Test', color: '#5E4DB2', text: 'Test in Arabic and English?', author: 'James' },
                  ].map((c, i) => (
                    <div
                      key={i}
                      style={{
                    ...
                    >
                      <div style={{ fontSize: '12px', marginBottom: '4px' }}>
                        <span style={{ backgroundColor: c.color, color: '#FFFFFF', padding: '2px 6px', borderRadius: '3px', fontSize: '11px', fontWeight: 500 }}>
                          {c.type}
                        </span>
                        <span style={{ marginLeft: '8px', color: '#626F86', fontSize: '11px' }}>{c.author}</span>
                      </div>
                      <div style={{ fontSize: '13px', color: '#172B4D' }}>{c.text}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {[
                    ...
                    <div key={i} style={{ padding: '12px', backgroundColor: '#F7F8F9', borderRadius: '4px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                      <div
                        style={{
                          backgroundColor: issue.severity === 'P0' ? '#AE2A19' : issue.severity === 'P1' ? '#974F0C' : '#5E4DB2',
                          color: '#FFFFFF',
                          padding: '2px 8px',
                          borderRadius: '3px',
                          fontSize: '11px',
                  ...
                  {[
                    ...
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: '4px',
                        border: `1px solid ${source.included ? '#4CE97' : '#DFE1E6'}`,
                        display: 'flex',
                        alignItems: 'center',
                        gap: '12px',
                      }}
                    >
                      ...
                      <span style={{ flex: 1, fontSize: '14px', color: '#172B4D' }}>{source.name}</span>
                      {source.lang && (
                        <span style={{ fontSize: '11px', backgroundColor: '#E3F2FD', color: '#0052CC', padding: '2px 6px', borderRadius: '3px', fontWeight: 500 }}>
                          {source.lang}
                        </span>
                      )}
                    </div>
            ...
            {selectedTab === 'Delta History' && (
              <div style={{ padding: '16px', backgroundColor: '#F7F8F9', borderRadius: '4px', border: '1px solid #DFE1E6' }}>
                <div style={{ marginBottom: '12px' }}>
                  <strong>Evidence Pack v1</strong>
                  <span style={{ fontSize: '12px', color: '#626F86', marginLeft: '8px' }}>(current)</span>
                </div>
                <div style={{ fontSize: '14px', color: '#626F86' }}>No previous version found. All sources processed for the first time.</div>
              </div>
            )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
                  {[
                    ...
                    >
                      ...
                      <p style={{ margin: 0, fontSize: '13px', color: '#626F86' }}>{option.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div style={{ padding: '32px 16px', backgroundColor: '#F7F8F9', overflowY: 'auto' }}>
          <h4 style={{ marginTop: 0, fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', color: '#626F86' }}>Summary</h4>
          <div style={{ fontSize: '12px', color: '#626F86', lineHeight: '1.8' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', minHeight: '100vh', backgroundColor: '#FFFFFF' }}>
        <div style={{ padding: '32px', borderRight: '1px solid #DFE1E6' }}>
          ...
          {generateStage === 'project-select' && (
            <div>
              <p style={{ color: '#626F86', marginBottom: '24px' }}>Select the destination project where these work items will be created.</p>
              <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ padding: '12px', backgroundColor: '#DFFCF0', borderRadius: '4px', marginBottom: '24px' }}>
                <strong>Expected output:</strong>
                <div style={{ fontSize: '13px', marginTop: '8px', color: '#172B4D' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    backgroundColor: selectedProject ? '#0052CC' : '#DFE1E6',
                    color: '#FFFFFF',
                    border: 'none',
                    borderRadius: '4px',
          ...
          {generateStage === 'preview' && (
            <div>
              ...
              <div style={{ fontSize: '13px', color: '#626F86', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '400px', overflowY: 'auto' }}>
                {generatedItems.map((item) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: '12px', color: '#626F86', marginLeft: '26px' }}>
          ...
          {generateStage === 'success' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: '#626F86', marginBottom: '20px' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center' }}>
                  ...
                  <span style={{ color: '#216E4E', fontWeight: 500 }}>In production, these items would be created as drafts in your project.</span>
                </div>
              </div>
              <p style={{ fontSize: '12px', color: '#626F86' }}>This is a prototype. Actual creation is mocked.</p>
              <button
                onClick={() => setStage('evidence-pack')}
                style={{
                  padding: '8px 16px',
