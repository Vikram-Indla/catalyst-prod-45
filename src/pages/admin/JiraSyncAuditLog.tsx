...
function Lozenge({ status }: { status: string }) {
  const map: Record<string, { bg: string; text: string }> = {
    ...
    error: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', text: 'var(--ds-text, #253858)' },
    failed: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', text: 'var(--ds-text, #253858)' },
    skipped: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', text: 'var(--ds-text, #253858)' },
    abandoned: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', text: 'var(--ds-text, #253858)' },
...
const thClass = 'text-left text-[10px] font-semibold uppercase text-[var(--ds-text-subtlest,var(--cp-ink-3, var(--cp-text-secondary, #64748B)))] dark:text-gray-400';
const thStyle = { padding: '10px 12px', height: 50, maxHeight: 50 } as const;
const tdClass = 'text-[var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))] dark:text-white';
...
function PaginationBar({ page, setPage, count }: { page: number; setPage: (p: number) => void; count: number }) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2 text-[var(--ds-text-subtlest,var(--cp-ink-3, var(--cp-text-secondary, #64748B)))] dark:text-gray-400" style={{ fontSize: 12 }}>
...
export function SyncEventsTab() {
  ...
  return (
      <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] rounded-md overflow-hidden">
        <div className="flex items-center gap-3 p-3 border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]">
          {(() => {
            const statusOpts = [
              { label: 'All', value: 'all' },
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]">
          ...
          {isLoading ? <SkeletonRows cols={7} /> : isError ? <ErrorRow cols={7} /> : (
            <tbody>
              {filteredLogs.map((log: any) => {
                ...
                return (
                  <tr key={log.id} className="border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]" style={{ height: 50, maxHeight: 50 }}>
          ...
          {isLoading ? <SkeletonRows cols={7} /> : isError ? <ErrorRow cols={7} /> : (
            <tbody>
              {(queueItems ?? []).map((q: any) => (
                <tr key={q.id} className="border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]" style={{ height: 50, maxHeight: 50 }}>
                  <td className={tdClass} style={tdStyle}>{q.ph_work_items?.title || q.ph_work_item_id}</td>
                  <td style={tdStyle}>
                    <span className="inline-block bg-[var(--ds-surface-sunken,var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))] text-[var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))] dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] dark:text-gray-300" style={{ fontSize: 11, fontWeight: 600, borderRadius: 4, padding: '2px 6px', textTransform: 'uppercase' }}>
                      {q.operation}
                    </span>
                  </td>
      ...
      <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] rounded-md overflow-hidden">
        {!isLoading && !isError && (!deletedItems || deletedItems.length === 0) ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2">
            ...
            <span style={{ fontSize: 13 }} className="text-[var(--ds-text-subtlest,var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))] dark:text-gray-400">No deleted items archived</span>
          </div>
        ) : (
          <>
            <table className="w-full border-collapse">
              ...
              {isLoading ? <SkeletonRows cols={4} /> : isError ? <ErrorRow cols={4} /> : (
                <tbody>
                  {(deletedItems ?? []).map((d: any) => {
                    ...
                    return (
                      <tr key={d.id} className="border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]" style={{ height: 50, maxHeight: 50 }}>
  ...
  return (
    <div className="p-6 space-y-6 dark:bg-[var(--ds-surface,#0A0A0A)] min-h-screen">
      <div className="flex items-start justify-between">
        <div>
          <h1 style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 20, fontWeight: 700 }} className="text-[var(--ds-text,var(--cp-ink-1, var(--cp-ink-1, #0F172A)))] dark:text-white">
            Jira Sync Audit Log
          </h1>
          <p style={{ fontSize: 13 }} className="text-[var(--ds-text-subtlest,var(--cp-ink-3, var(--cp-text-secondary, #64748B)))] dark:text-gray-400 mt-1">
