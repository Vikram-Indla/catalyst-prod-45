...
function Lozenge({ status }: { status: string }) {
  const map: Record<string, { bg: string; text: string }> = {
    success: { bg: 'rgba(74,222,128,0.10)', text: '#006644' },
    completed: { bg: 'rgba(74,222,128,0.10)', text: '#006644' },
    error: { bg: '#DFE1E6', text: '#253858' },
    failed: { bg: '#DFE1E6', text: '#253858' },
    skipped: { bg: '#DFE1E6', text: '#253858' },
    abandoned: { bg: '#DFE1E6', text: '#253858' },
    processing: { bg: 'rgba(59,130,246,0.10)', text: '#0747A6' },
    pending: { bg: 'rgba(59,130,246,0.10)', text: '#0747A6' },
    approved: { bg: 'rgba(59,130,246,0.10)', text: '#0747A6' },
  };
...
const thClass = 'text-left text-[10px] font-semibold uppercase text-[rgba(237,237,237,0.40)] dark:text-gray-400';
const thStyle = { padding: '10px 12px', height: 50, maxHeight: 50 } as const;
const tdClass = 'text-[rgba(237,237,237,0.53)] dark:text-white';
...
function PaginationBar({ page, setPage, count }: { page: number; setPage: (p: number) => void; count: number }) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2 text-[rgba(237,237,237,0.40)] dark:text-gray-400" style={{ fontSize: 12 }}>
...
export function SyncEventsTab() {
  ...
  return (
    <TooltipProvider>
      <div className="bg-white dark:bg-[#1A1A1A] border border-[var(--bd-default, rgba(255,255,255,0.10))] dark:border-[#1A1A1A] rounded-md overflow-hidden">
        <div className="flex items-center gap-3 p-3 border-b border-[var(--bd-default, rgba(255,255,255,0.10))] dark:border-[#1A1A1A]">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[var(--bd-default, rgba(255,255,255,0.10))] dark:border-[#1A1A1A]">
          ...
          {isLoading ? <SkeletonRows cols={7} /> : isError ? <ErrorRow cols={7} /> : (
            <tbody>
              {filteredLogs.map((log: any) => {
                ...
                return (
                  <tr key={log.id} className="border-b border-[var(--bd-default, rgba(255,255,255,0.10))] dark:border-[#1A1A1A]" style={{ height: 50, maxHeight: 50 }}>
          ...
          {isLoading ? <SkeletonRows cols={7} /> : isError ? <ErrorRow cols={7} /> : (
            <tbody>
              {(queueItems ?? []).map((q: any) => (
                <tr key={q.id} className="border-b border-[var(--bd-default, rgba(255,255,255,0.10))] dark:border-[#1A1A1A]" style={{ height: 50, maxHeight: 50 }}>
                  <td className={tdClass} style={tdStyle}>{q.ph_work_items?.title || q.ph_work_item_id}</td>
                  <td style={tdStyle}>
                    <span className="inline-block bg-[#1A1A1A] text-[rgba(237,237,237,0.53)] dark:bg-[#1A1A1A] dark:text-gray-300" style={{ fontSize: 11, fontWeight: 600, borderRadius: 4, padding: '2px 6px', textTransform: 'uppercase' }}>
                      {q.operation}
                    </span>
                  </td>
      ...
      <div className="bg-white dark:bg-[#1A1A1A] border border-[var(--bd-default, rgba(255,255,255,0.10))] dark:border-[#1A1A1A] rounded-md overflow-hidden">
        {!isLoading && !isError && (!deletedItems || deletedItems.length === 0) ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2">
            ...
            <span style={{ fontSize: 13 }} className="text-[rgba(237,237,237,0.40)] dark:text-gray-400">No deleted items archived</span>
          </div>
        ) : (
          <>
            <table className="w-full border-collapse">
              ...
              {isLoading ? <SkeletonRows cols={4} /> : isError ? <ErrorRow cols={4} /> : (
                <tbody>
                  {(deletedItems ?? []).map((d: any) => {
                    ...
                    return (
                      <tr key={d.id} className="border-b border-[var(--bd-default, rgba(255,255,255,0.10))] dark:border-[#1A1A1A]" style={{ height: 50, maxHeight: 50 }}>
  ...
  return (
    <div className="p-6 space-y-6 dark:bg-[#0A0A0A] min-h-screen">
      <div className="flex items-start justify-between">
        <div>
          <h1 style={{ fontFamily: 'Sora, sans-serif', fontSize: 20, fontWeight: 700 }} className="text-[rgba(237,237,237,0.93)] dark:text-white">
            Jira Sync Audit Log
          </h1>
          <p style={{ fontSize: 13 }} className="text-[rgba(237,237,237,0.40)] dark:text-gray-400 mt-1">
