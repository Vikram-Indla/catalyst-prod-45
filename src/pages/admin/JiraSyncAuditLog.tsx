            ...
            <tbody>
              {(queueItems ?? []).map((q: any) => (
                <tr key={q.id} className="border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]" style={{ height: 50, maxHeight: 50 }}>
                  ...
                  <td style={tdStyle}>
                    <span className="inline-block bg-[var(--ds-surface-sunken,var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))] text-[var(--cp-ink-2, var(--cp-ink-2, #334155))] dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] dark:text-gray-300" style={{ fontSize: 11, fontWeight: 600, borderRadius: 4, padding: '2px 6px', textTransform: 'uppercase' }}>
                      {q.operation}
                    </span>
                  </td>
