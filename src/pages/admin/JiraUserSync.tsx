...
const JiraUserSync: React.FC = () => {
  ...
  return (
    <div
      ...
      style={{ background: isDark ? '#0A0A0A' : '#1A1A1A', minHeight: '100vh' }}
    >
      ...
      <div className="jira-surface flex flex-col flex-1 overflow-hidden mx-5 my-4 rounded-lg"
          ...
          <div className="flex items-start justify-between pb-3">
            <div>
              <h1 className="jira-text-primary"
                style={{ fontFamily: "'Sora', sans-serif", fontSize: '17px', fontWeight: 700, letterSpacing: '-0.3px', margin: 0, lineHeight: 1.3, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>
                Jira User Sync
              </h1>
              <p className="jira-text-secondary"
                style={{ fontSize: '11px', margin: '2px 0 0', color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>
            ...
            >
              <div className="flex items-center gap-[5px] mb-[4px]">
                ...
                <span className="jira-stat-label"
                  style={{ fontSize: '11px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.03em', fontFamily: "'Inter', sans-serif", color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
                  {card.label}
                </span>
              </div>
              <div className="jira-stat-value"
                style={{ fontSize: '28px', fontWeight: 650, lineHeight: 1.1, fontFamily: "'Sora', sans-serif", color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>
                {getStatValue(card.key)}
              </div>
              <div className="jira-stat-sub"
                style={{ fontSize: '11px', marginTop: '4px', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
                {card.sub}
              </div>
            </div>
          ))}
        ...
        <div className="jira-toolbar shrink-0 flex items-center gap-[7px] flex-wrap"
          style={{ padding: '9px 18px', background: isDark ? '#0A0A0A' : '#FFFFFF', borderBottom: `0.75px solid ${isDark ? 'rgba(255,255,255,0.10)' : 'rgba(15,23,42,0.06)'}` }}>
          <div className="relative w-[220px]">
            <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }} />
            <input
              ...
              <button
                ...
                style={{
                  ...
                  background: filter === f.value
                    ? (isDark ? 'rgba(37,99,235,0.12)' : 'rgba(59,130,246,0.06)')
                    : (isDark ? '#1A1A1A' : '#FFFFFF'),
                  color: filter === f.value
                    ? (isDark ? '#93C5FD' : '#2563EB')
                    : (isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)'),
        ...
        {selected.size > 0 && (
          <div className="jira-selection-bar shrink-0 flex items-center gap-[9px]"
            style={{
              padding: '7px 18px',
              background: isDark ? 'rgba(37,99,235,0.12)' : 'rgba(59,130,246,0.06)',
            ...
            <span style={{ fontSize: '11px', color: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(237,237,237,0.40)' }}>|</span>
            ...
            <Popover open={assignPopoverOpen} onOpenChange={setAssignPopoverOpen}>
              ...
              >
                <div style={{ padding: '12px 14px 8px' }}>
                  <div className="jira-text-primary" style={{ fontSize: '13px', fontWeight: 600, marginBottom: '2px', color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>
                    Assign to Jira Space
                  </div>
                  <div className="jira-text-secondary" style={{ fontSize: '11px', marginBottom: '8px', color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>
                  ...
                  <div className="flex items-center gap-1 mb-2">
                    <span style={{ fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', marginRight: '4px', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>Level:</span>
                ...
                <div style={{ maxHeight: '200px', overflowY: 'auto', borderTop: `0.75px solid ${isDark ? 'rgba(255,255,255,0.10)' : 'rgba(15,23,42,0.06)'}` }}>
                      ...
                      >
                        <span className="jira-chip"
                          style={{
                            ...
                            background: isDark ? 'rgba(255,255,255,0.10)' : '#1A1A1A',
                            color: isDark ? '#A1A1A1' : '#374151',
                          }}>
                          {proj.project_key}
                        </span>
                        <span style={{ fontSize: '11px', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)' }}>
                  ...
                  {(jiraProjects || []).filter(p => !assignSearch || p.project_key.toLowerCase().includes(assignSearch.toLowerCase())).length === 0 && (
                    <div style={{ padding: '16px', textAlign: 'center', fontSize: '11px', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
                      No projects found
                    </div>
                  )}
                </div>
            ...
            <button onClick={clearAll}
              className="ml-auto"
              style={{ padding: '2px 6px', borderRadius: '4px', fontSize: '11px', fontWeight: 500, background: 'transparent', border: 'none', cursor: 'pointer', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
              Clear selection
            </button>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto overflow-x-auto">
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1000px' }}>
            <thead>
              <tr className="jira-table-header" style={{ background: isDark ? '#1A1A1A' : '#1A1A1A', borderBottom: `0.75px solid ${isDark ? 'rgba(255,255,255,0.10)' : 'rgba(15,23,42,0.06)'}` }}>
              ...
              {usersLoading ? (
                Array.from({ length: 10 }).map((_, i) => (
                  <tr key={`skel-${i}`} className="jira-row"
                    ...
                    <td style={{ padding: '8px 12px' }}><div className="jira-skeleton animate-pulse" style={{ width: 14, height: 14, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} /></td>
                    <td style={{ padding: '8px 12px' }}>
                      <div className="flex items-center gap-2">
                        <div className="jira-skeleton animate-pulse" style={{ width: 28, height: 28, borderRadius: '50%', background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} />
                        <div>
                          <div className="jira-skeleton animate-pulse" style={{ width: 120, height: 10, borderRadius: 4, marginBottom: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} />
                          <div className="jira-skeleton animate-pulse" style={{ width: 160, height: 8, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : '#1A1A1A' }} />
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '8px 12px' }}><div className="jira-skeleton animate-pulse" style={{ width: 70, height: 16, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} /></td>
                    <td style={{ padding: '8px 12px' }}><div className="jira-skeleton animate-pulse" style={{ width: 100, height: 16, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} /></td>
                    <td style={{ padding: '8px 12px' }}><div className="jira-skeleton animate-pulse" style={{ width: 80, height: 10, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} /></td>
                    <td style={{ padding: '8px 12px' }}><div className="jira-skeleton animate-pulse" style={{ width: 60, height: 10, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} /></td>
                    <td style={{ padding: '8px 12px' }}><div className="jira-skeleton animate-pulse" style={{ width: 60, height: 10, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} /></td>
                    <td style={{ padding: '8px 12px' }}><div className="jira-skeleton animate-pulse" style={{ width: 50, height: 18, borderRadius: 4, background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' }} /></td>
                    <td />
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={9} style={{ textAlign: 'center', padding: '60px 20px' }}>
                    {debouncedSearch ? (
                      <>
                        <Search size={24} style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: '0 auto 10px', display: 'block' }} />
                        <div className="jira-text-primary" style={{ fontSize: '14px', fontWeight: 500, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.53)' }}>No users match '{debouncedSearch}'</div>
                        <button onClick={() => setSearch('')}
                          style={{ marginTop: 8, fontSize: '12px', background: 'none', border: 'none', cursor: 'pointer', color: '#2563EB' }}>Clear search</button>
                      </>
                    ) : filter !== 'all' ? (
                      <>
                        <FolderSearch size={24} style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: '0 auto 10px', display: 'block' }} />
                        <div className="jira-text-primary" style={{ fontSize: '14px', fontWeight: 500, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.53)' }}>
                      ...
                      <>
                        <Users2 size={32} style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: '0 auto 10px', display: 'block' }} />
                        <div className="jira-text-primary" style={{ fontSize: '14px', fontWeight: 500, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.53)' }}>No synced users yet</div>
                  ...
                  <tr key={user.id}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div style={{ minWidth: 0, maxWidth: '220px' }}>
                          <div className="flex items-center gap-[5px]">
                            <span className="jira-text-primary" style={{ fontSize: '12px', fontWeight: 500, textDecoration: isInactive ? 'line-through' : 'none', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{user.display_name}</span>
                    ...
                    <td style={{ padding: '8px 12px' }}>
                      {user.auth_mode === 'jira_proxy' ? (
                        <span className="jira-badge-jira" style={{
                          ...
                          background: isDark ? 'rgba(37,99,235,0.18)' : 'rgba(59,130,246,0.06)',
                    ...
                    <td style={{ padding: '8px 12px', maxWidth: '200px' }}>
                      <div className="flex gap-1 flex-nowrap overflow-hidden items-center">
                        ...
                        {perms.length > 2 && <span style={{ fontSize: '10px', fontWeight: 500, whiteSpace: 'nowrap', color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>+{perms.length - 2} more</span>}
                        {perms.length === 0 && <span style={{ fontSize: '10px', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>—</span>}
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '8px 12px' }}>
                      {isCatalystOnly ? (
                        <span style={{ fontSize: '11px', fontStyle: 'italic', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>Not synced</span>
                      ) : (
                        <div className="flex items-center gap-1">
                          <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#16A34A', flexShrink: 0 }} />
                          <span style={{ fontSize: '11px', fontFamily: "'JetBrains Mono', monospace", whiteSpace: 'nowrap', color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>{formatSyncDate(user.last_synced_at)}</span>
                        </div>
                      )}
                    </td>

                    {/* Last Jira Login */}
                    <td style={{ padding: '8px 12px', fontSize: '11px', fontFamily: "'JetBrains Mono', monospace", color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>{relativeTime(user.last_jira_login_at)}</td>

                    {/* Last in Catalyst */}
                    <td style={{ padding: '8px 12px', fontSize: '11px', fontFamily: "'JetBrains Mono', monospace", color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>{relativeTime(user.last_catalyst_login_at)}</td>

                    {/* Status lozenge */}
                    <td style={{ padding: '8px 12px' }}>
                      {hasConflicts ? (
                        <span className="jira-lozenge-conflict"
                          style={{ display: 'inline-block', padding: '0 7px', borderRadius: '4px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', height: '20px', lineHeight: '20px', letterSpacing: '0.03em', background: isDark ? '#451A03' : 'rgba(251,191,36,0.10)', color: isDark ? '#FCD34D' : '#FBBF24' }}>CONFLICT</span>
                      ) : isInactive ? (
                        <span className="jira-lozenge-inactive"
                          style={{ display: 'inline-block', padding: '0 7px', borderRadius: '4px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', height: '20px', lineHeight: '20px', letterSpacing: '0.03em', background: isDark ? '#450A0A' : 'rgba(248,113,113,0.10)', color: isDark ? '#FCA5A5' : '#F87171' }}>INACTIVE</span>
                      ) : (
                        <span className="jira-lozenge-active"
                          style={{ display: 'inline-block', padding: '0 7px', borderRadius: '4px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', height: '20px', lineHeight: '20px', letterSpacing: '0.03em', background: isDark ? '#064E3B' : 'rgba(74,222,128,0.10)', color: isDark ? '#6EE7B7' : '#006644' }}>ACTIVE</span>
                      )}
                    </td>
        ...
        <div className="jira-pagination-bg shrink-0 flex items-center justify-between"
          style={{ padding: '9px 16px', borderTop: `0.75px solid ${isDark ? 'rgba(255,255,255,0.10)' : 'rgba(15,23,42,0.06)'}`, background: isDark ? '#0A0A0A' : '#1A1A1A' }}>
          <span style={{ fontSize: '11px', color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>
          ...
          <div className="flex gap-1 items-center">
            ...
            {getPageNumbers(page, totalPages).map((p, i) =>
              p === '...' ? (
                <span key={`ell-${i}`} style={{ width: '28px', textAlign: 'center', fontSize: '11px', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>…</span>
              ) : (
                <button
                  key={p}
                  ...
                  style={{
                    ...
                    color: page === p ? '#FFFFFF' : (isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)'),
