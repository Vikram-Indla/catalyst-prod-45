...
const CATEGORY_STYLE: Record<string, { bg: string; border: string; text: string }> = {
  todo:        { bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', border: '#A5ADBA', text: 'var(--ds-text, var(--ds-text, #253858))' },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-sm font-semibold text-[var(--ds-text,var(--ds-text, #EDEDED))] font-['Sora']">{schemeName}</span>
        <span className="text-[11px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]">
          ...
          return (
            ...
            >
              <div className="mb-3 flex items-center gap-2">
                ...
                <span className="text-[10px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]">({items.length})</span>
              </div>
      ...
      {/* Transition list */}
      <div className="space-y-2">
        <h4 className="text-xs font-semibold text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))] uppercase tracking-wider mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {globalTransitions.map(t => {
            ...
            return (
              <div
                key={t.id}
                className="flex items-center gap-2 px-3 py-2 rounded border border-[var(--ds-border,var(--ds-border, #2E2E2E))] bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))]"
              >
                <Zap size={12} className="text-amber-400 shrink-0" />
                <span className="text-xs text-amber-400 font-medium">Any</span>
                <ArrowRight size={12} className="text-[var(--ds-border-bold,var(--ds-border-bold, #454545))] shrink-0" />
                <span className="text-xs text-[var(--ds-text,var(--ds-text, #EDEDED))]">{toStatus?.name || '?'}</span>
              </div>
            );
          })}
          {directTransitions.map(t => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs text-[var(--ds-text,var(--ds-text, #EDEDED))] truncate">{fromStatus?.name || '?'}</span>
                <ArrowRight size={12} className="text-[var(--ds-border-bold,var(--ds-border-bold, #454545))] shrink-0" />
                <StatusDot category={toStatus?.category || 'todo'} />
                <span className="text-xs text-[var(--ds-text,var(--ds-text, #EDEDED))] truncate">{toStatus?.name || '?'}</span>
              </div>
            );
          })}
        </div>
...
function StatusNode({ status, style }: { status: WorkflowStatus; style: typeof CATEGORY_STYLE.todo }) {
  return (
    ...
    >
      {status.is_initial && (
        <span className="text-[9px] font-bold text-[var(--ds-text,var(--ds-text, #EDEDED))] bg-[var(--ds-border,var(--ds-border, #292929))] px-1 py-0.5 rounded">
          START
        </span>
      )}
      ...
      {status.is_final && (
        <span className="text-[9px] font-bold text-[var(--ds-text,var(--ds-text, #EDEDED))] bg-[var(--ds-border,var(--ds-border, #292929))] px-1 py-0.5 rounded ml-auto">
          END
        </span>
      )}
    </div>
...
function StatusDot({ category }: { category: string }) {
  const bg = CATEGORY_STYLE[category]?.bg || 'var(--ds-border, var(--ds-border, #DFE1E6))';
