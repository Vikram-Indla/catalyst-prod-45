...
export default function WorkflowAdminPage() {
  ...
  return (
    <div className="space-y-0 bg-white min-h-screen text-[#0F172A]">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#E2E8F0]">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-[#F1F5F9] flex items-center justify-center border border-[#E2E8F0]">
            <GitBranch size={18} className="text-[#0F172A]" />
          </div>
          <div>
            <h1 className="text-lg font-semibold text-[#0F172A] font-['Sora']">
              Workflows
            </h1>
        ...
        {/* View toggle */}
        <div className="flex items-center gap-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-md p-0.5">
          <button
            ...
            {ISSUE_TYPES.map(t => (
              <TabsTrigger
                ...
                className={cn(
                  ...
                  'data-[state=active]:border-[#2563EB] data-[state=active]:text-[#0F172A]',
                  'data-[state=inactive]:border-transparent data-[state=inactive]:text-[#64748B] data-[state=inactive]:hover:text-[#0F172A]',
                  'bg-transparent'
                )}
              >
                {t.label}
  ...
  if (workflow.isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="animate-spin w-5 h-5 border-2 border-[#64748B] border-t-transparent rounded-full" />
      </div>
    );
  }
