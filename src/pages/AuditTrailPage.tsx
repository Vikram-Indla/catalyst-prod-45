...
function restoreStatus(entry: AuditEntry): { label: string; bg: string; color: string; border: string } {
  ...
  if (deadline < new Date()) {
    return { label: 'EXPIRED', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', border: 'var(--ds-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))' };
  }
