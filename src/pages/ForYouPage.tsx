...
export default function ForYouPage() {
  ...
  const filterCategories = React.useMemo<FilterCategory[]>(() => {
    ...
    const HUB_ICONS: Record<string, React.ReactNode> = {
      ...
      Project: <Folder size={14} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" strokeWidth={2} />,
      Task: <CheckSquare size={14} color="#4BADE8" strokeWidth={2} />,
      Plan: <BookOpen size={14} color="#0D9488" strokeWidth={2} />,
      Strategy: <Zap size={14} color="var(--ds-text-warning, var(--ds-text-warning, #D97706))" strokeWidth={2} />,
    };
    ...
    return [
      {
        ...
        options: projectOptions.map(p => ({
          id: p, label: p,
          iconNode: <Folder size={14} color="var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))" strokeWidth={1.5} />,
        })),
      },
      {
        ...
        options: hubOptions.map(h => ({
          id: h, label: h + ' Hub',
          iconNode: HUB_ICONS[h] || <LayoutGrid size={14} color="var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))" strokeWidth={2} />,
        })),
      },
      {
          ...
          <StatusSummaryBar
            ...
            filterSlot={
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <GroupByPopover<ForYouGroupByKey> value={fyGroupBy} onChange={setFyGroupBy} options={FY_GROUP_OPTIONS} noneKey="none" />
              </div>
            }
          />
        </div>
