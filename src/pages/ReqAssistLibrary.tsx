    ...
    <div style={{ background: 'var(--cp-bg-page, var(--ds-surface-sunken))', minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '24px 28px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          >
            <Download size={14} color="var(--ds-text-subtle)" /> Import from Jira
          </button>
          <button
      ...
      <div style={{ padding: '20px 28px 28px' }}>
        {!isLoading && totalCount === 0 && !isFiltering ? (
          ...
          <div style={{
            ...
            <p style={{ fontSize: 'var(--ds-font-size-500)', fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, var(--ds-text))))', margin: '0 0 6px', fontFamily: 'var(--cp-font-heading)' }}>No documents yet</p>
          ...
          <div style={{
            ...
            {isFiltering && (
              <div style={{
                ...
                padding: '8px 28px', fontSize: 'var(--ds-font-size-200)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, var(--ds-text-subtlest))))',
                background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))', borderBottom: `0.75px solid ${'var(--cp-border-subtle, rgba(15,23,42,0.08))'}`,
                fontFamily: 'var(--cp-font-body)',
              }}>
                Showing {documents?.length ?? 0} of {totalCount} documents
                <span style={{ color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, var(--ds-text-disabled))))' }}>·</span>
                Filtered by: <strong style={{ color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, var(--ds-text-subtle))))', fontWeight: 600 }}>{tab !== 'all' ? tab.charAt(0).toUpperCase() + tab.slice(1) : search}</strong>
                <button
                  onClick={() => { setSearch(''); setTab('all'); }}
                  style={{
                    border: 'none', background: 'transparent', cursor: 'pointer',
                    fontSize: 'var(--ds-font-size-200)', color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', fontWeight: 500, padding: 0,
                    fontFamily: 'var(--cp-font-body)',
                  }}
                >
                  Clear filter ×
                ...
                {isLoading ? (
                    ...
                    <tr key={i} style={{ height: 50 }}>
                      {Array.from({ length: 8 }).map((_, j) => (
                        <td key={j} style={{ padding: '8px 12px' }}>
                          <div style={{ width: j === 1 ? '80%' : '60%', height: 12, background: 'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, var(--ds-border))))', borderRadius: 4, animation: 'ra-pulse 1.5s ease-in-out infinite' }} />
                        </td>
                      ))}
                    </tr>
                  ))
                      ...
                      >
                          ...
                          {parentKeys[doc.id] ? (
                              ...
                              <div style={{ paddingLeft: 8 }}>
                                {doc.jira_ticket_url ? (
                                  <a href={doc.jira_ticket_url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
                                    style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', textDecoration: 'none' }}
                                  ...
                                  <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' }}>
                                    {doc.jira_ticket_key}
                                  </span>
                                )}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              {doc.jira_ticket_url ? (
                                ...
                                <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 'var(--ds-font-size-200)', fontWeight: 500, color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' }}>
                                  {doc.jira_ticket_key}
                                </span>
                              )}
                              <TicketTypeBadge type={ticketTypes[doc.id]} />
                        ...
                        <td style={{ padding: '8px 12px', overflow: 'hidden', maxWidth: 0 }}>
                          <span title={doc.title} style={{
                            display: 'block', fontSize: 'var(--ds-font-size-300)', fontWeight: 400, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, var(--ds-text))))',
                            fontFamily: 'var(--cp-font-body)',
                        ...
                        <td style={{ padding: '8px 12px', overflow: 'hidden' }}>
                          {(() => {
                            ...
                            return domain ? (
                              <span style={{
                                fontSize: 'var(--ds-font-size-200)', color: 'var(--cp-text-secondary, var(--ds-text-subtle))', fontFamily: 'var(--cp-font-body)',
                        ...
                        <td data-col="pdf" style={{ padding: '8px 12px', overflow: 'hidden', textAlign: 'center', width: 80 }}>
                          {doc.pdf_url ? (
                            <button
                              ...
                              onMouseLeave={e => (e.currentTarget.style.textDecoration = 'none')}
                            >
                              <FileText size={14} color="var(--ds-text-brand, var(--cp-workstream-catalyst-primary))" />
                              PDF
                            </button>
                          ) : (
                        ...
                        {/* Imported */}
                        <td style={{ padding: '8px 12px', overflow: 'hidden' }}>
                          <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 'var(--ds-font-size-200)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, var(--ds-text-subtlest))))' }}>
                  ...
                  <tr>
                    <td colSpan={8} style={{ padding: '48px 0', textAlign: 'center' }}>
                      <FileSearch size={24} color="var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light)))" style={{ margin: '0 auto 8px', display: 'block' }} />
                      <p style={{ fontSize: 'var(--ds-font-size-400)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, var(--ds-text-subtlest))))', margin: '0 0 8px', fontFamily: 'var(--cp-font-body)' }}>
                        No documents match "{search || 'your search term'}"
                      </p>
                      <button onClick={() => { setSearch(''); setTab('all'); }}
                        style={{ fontSize: 'var(--ds-font-size-200)', color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', fontWeight: 600, background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'var(--cp-font-body)' }}>
                        Clear search
                      </button>
                    </td>
                  </tr>
            ...
            {!isLoading && documents && (
              <div style={{
                ...
                <span style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 400, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, var(--ds-text-subtlest))))', fontFamily: 'var(--cp-font-body)' }}>
      ...
      {regenConfirm && (
        <>
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 'var(--ds-font-size-500)', fontWeight: 650, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, var(--ds-text))))', margin: '0 0 8px', fontFamily: 'var(--cp-font-heading)' }}>
              Epics Already Exist
            </h3>
            <p style={{ fontSize: 'var(--ds-font-size-400)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, var(--ds-text-subtlest))))', margin: '0 0 20px', lineHeight: 1.5 }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setRegenConfirm(null)} style={{
                ...
                border: 'none', background: 'transparent', color: 'var(--cp-text-secondary, var(--ds-text-subtle))', cursor: 'pointer',
              }}>Cancel</button>
              <button onClick={() => {
                ...
                padding: '8px 16px', fontSize: 'var(--ds-font-size-300)', fontWeight: 500, borderRadius: 6,
                border: `0.75px solid ${'var(--cp-border-strong, rgba(15,23,42,0.15))'}`, background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))', color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, var(--ds-text-subtle))))', cursor: 'pointer',
              }}>View Drafts</button>
              <button onClick={() => { const d = regenConfirm.doc; setRegenConfirm(null); setBgModal({ type: 'epics', doc: d }); }} style={{
                ...
                border: 'none', background: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))', cursor: 'pointer',
              }}>Regenerate</button>
            </div>
          </div>
        </>
...
function PdfUploadCell({ brdId: initialBrdId, jiraKey }: { brdId: string | null; jiraKey: string }) {
  ...
  return (
    <>
      ...
      >
        {uploading ? (
          <Loader2 size={14} color="var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light)))" style={{ animation: 'ra-pulse 1s linear infinite' }} />
        ) : (
          <FileUp size={14} color="var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light)))" />
        )}
      </button>
      <input
        ref={fileRef}
...
function StatusBadge({ status, epicCount, pipelineStage }: { status: string; epicCount: number; pipelineStage: string | null }) {
  ...
  if (ps === 'failed') {
    bg = 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral)))'; color = 'var(--ds-text-subtle)'; label = 'FAILED';
  } else if (epicCount > 0) {
    // Having epics = document is usable regardless of pipeline_stage
    bg = 'var(--cp-lozenge-green-bg, var(--ds-background-success-bold))'; color = 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))'; label = 'READY';
  } else if (ps === 'complete') {
    // pipeline complete but no epics yet → still READY
    bg = 'var(--cp-lozenge-green-bg, var(--ds-background-success-bold))'; color = 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))'; label = 'READY';
  } else if (['extract', 'process', 'validate', 'distribute', 'processing'].includes(ps)) {
    bg = 'var(--ds-link)'; color = 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))'; label = 'PROCESSING';
  } else {
    // intake, pending, or unknown
    bg = 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral)))'; color = 'var(--ds-text-subtle)'; label = 'PENDING';
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <button
          ...
          style={{
            ...
            borderRadius: 6, border: '0.75px solid var(--ds-background-information)',
            background: 'var(--cp-primary-light, var(--ds-background-information))', color: 'var(--ds-background-brand-bold-hovered)', cursor: 'pointer',
            fontFamily: 'var(--cp-font-body)', whiteSpace: 'nowrap',
            transition: 'background 80ms ease',
          }}
          onMouseEnter={e => { e.currentTarget.style.background = 'var(--ds-background-information)'; e.currentTarget.style.borderColor = 'var(--ds-background-information-bold)'; }}
          onMouseLeave={e => { e.currentTarget.style.background = 'var(--ds-background-selected)'; e.currentTarget.style.borderColor = 'var(--ds-background-information)'; }}
        >
          View Drafts
        </button>
        <span style={{
          ...
          background: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))',
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{
          ...
          fontSize: 'var(--ds-font-size-100)', fontWeight: 700, textTransform: 'uppercase',
          letterSpacing: '0.03em', whiteSpace: 'nowrap',
          background: 'var(--cp-lozenge-green-bg, var(--ds-background-success-bold))', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))',
          fontFamily: 'var(--cp-font-body)',
        }}>AI INDEXED</span>
        <button
    ...
    return (
      <button
        ...
        style={{
          display: 'inline-flex', alignItems: 'center', gap: 4,
          height: 28, padding: '0 10px', fontSize: 'var(--ds-font-size-200)', fontWeight: 500,
          borderRadius: 6, border: '0.75px solid var(--cp-danger, var(--ds-background-danger-bold))',
          background: 'transparent', color: 'var(--ds-text-danger, var(--cp-danger))', cursor: 'pointer',
          fontFamily: 'var(--cp-font-body)', whiteSpace: 'nowrap',
        }}
      >
  ...
  if (isProcessing) {
    return (
      <button disabled style={{
        ...
        background: 'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, var(--ds-border))))', color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, var(--ds-text-disabled))))',
        fontFamily: 'var(--cp-font-body)', whiteSpace: 'nowrap',
      }}>
        Generate
      </button>
...
const TICKET_TYPE_STYLES: Record<string, { bg: string; color: string; label: string }> = {
  subtask: { bg: 'var(--ds-background-warning)', color: 'var(--ds-text-warning)', label: 'SUBTASK' },
  story:   { bg: 'var(--ds-background-selected)', color: 'var(--ds-background-brand-bold-hovered)', label: 'STORY' },
  epic:    { bg: 'var(--ds-background-discovery)', color: 'var(--ds-background-discovery-bold)', label: 'EPIC' },
  task:    { bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken)))', color: 'var(--ds-text-subtle)', label: 'TASK' },
};
