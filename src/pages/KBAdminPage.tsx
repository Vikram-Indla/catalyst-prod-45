...
/* ═══════════════════════════════════════════
   SHARED STYLES
   ═══════════════════════════════════════════ */
const card = 'bg-white dark:bg-[#232019] border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg p-5 shadow-sm';
    ...
    <div className="space-y-5">
      ...
      <div className={`${overallBg} border rounded-xl p-6 flex items-center justify-between`}>
        ...
        <button
          onClick={runChecks}
          disabled={loading}
          className="flex items-center gap-1.5 px-4 py-2 bg-white dark:bg-[#232019] border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg text-xs font-semibold text-zinc-700 dark:text-[#A09890] hover:bg-zinc-50 dark:hover:bg-[#2C2823] transition-colors disabled:opacity-50"
        >
                    ...
                    <tr>
                      <td colSpan={12} className="px-0 py-0">
                        <div className="bg-blue-50/60 border-t border-b border-blue-100 px-6 py-4 space-y-3">
                          ...
                          <div className="bg-white dark:bg-[#232019] rounded-lg border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] p-3">
          ...
          <tbody className="divide-y divide-zinc-100">
            {roles.map((role) => (
              <tr key={role} className="hover:bg-zinc-50/50">
                <td className="px-3 py-2 font-medium text-zinc-700 dark:text-[#F5F3F0] sticky left-0 bg-white dark:bg-[#232019] z-10 whitespace-nowrap">{role}</td>
                {modules.map((mod) => {
      ...
      <div className={card}>
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <input
            ...
            className="flex-1 min-w-[200px] px-3 py-1.5 text-xs border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded bg-white dark:bg-[#232019] dark:text-[#F5F3F0] focus:outline-none focus:ring-1 focus:ring-blue-400"
          />
          <select value={catFilter} onChange={(e) => setCatFilter(e.target.value)} className="px-3 py-1.5 text-xs border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded bg-white dark:bg-[#232019] dark:text-[#F5F3F0]">
            <option value="all">All Categories ({questions.length})</option>
            {categories.map(c => (
              <option key={c.category} value={c.category}>{c.category} ({c.count})</option>
            ))}
          </select>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="px-3 py-1.5 text-xs border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded bg-white dark:bg-[#232019] dark:text-[#F5F3F0]">
            <option value="all">All Status</option>
...
export default function KBAdminPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50" style={{ paddingTop: 48 }}>
      {/* Page header */}
      <div className="bg-white dark:bg-[#232019] border-b border-zinc-200 dark:border-[rgba(255,255,255,0.08)] px-6 py-4">
        <h1 className="text-lg font-bold text-zinc-900 dark:text-[#F5F3F0]">Knowledge Base — Admin</h1>
        <p className="text-xs text-zinc-500 mt-0.5">Manage training, sources, access control, and pipeline configuration</p>
      </div>

      {/* Tabs */}
      <div className="bg-white dark:bg-[#232019] border-b border-zinc-200 dark:border-[rgba(255,255,255,0.08)] px-6">
