 ...
 *
 * JiraTable-based All Work view for a product's Business Requests.
 * Cloned from ProductNativeBacklogPage (JiraTable pattern) — product and
 * project modules look IDENTICAL on components, varied only by data source,
 * statuses, and fields.
 *
 * Data source:   business_requests (MDT project — NOT ph_issues)
 * Statuses:      useCatalystWorkflow('Business Request') from /admin/workflows
 *                + demand_process_steps as fallback label source
 * Work item icon: JiraIssueTypeIcon type='Business Request'
 * Fields (image-2 spec): English name, Arabic name, Status, Description
 *   + extended: Type, Priority, Delivery manager, Product owner, Target date
 */
...
// ─── ToolbarMenuButton (portal pattern — @atlaskit/dropdown-menu empty-portal bug) ──
...
function buildColumns(
  ...
  return [
    ...
    {
      id: 'title', label: 'Summary', width: 24, sortable: true, alwaysVisible: true, defaultVisible: true,
      accessor: r => r.title ?? '',
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return s === null
          ? <span style={{ color: token('color.text.subtlest', '#6B778C'), fontSize: 12 }}>—</span>
          : <span style={{ fontSize: 12, fontWeight: 500, color: token('color.text', '#172B4D'), fontFamily: 'var(--cp-font-mono)' }}>{s}</span>;
      },
    },
    {
      id: '__actions', label: '', width: 4, sortable: false, alwaysVisible: true, defaultVisible: true,
      accessor: () => '',
      cell: makeRowMenuCell({ onOpen, onDelete }),
    },
  ];
}
...
function buildGroups(
  ...
  rows.forEach(r => {
    const k = groupBy === 'status' ? ((r as any).process_step ?? '')
...
export default function ProductNativeAllWorkPage() {
  ...
  // Also load the Catalyst workflow for status filter options (from /admin/workflows)
  ...
  // Status options from workflow (admin/workflows Business Request)
  const statusFilterOptions = useMemo<StatusFilterOption[]>(() => {
    if (workflowStatuses.length > 0) {
      return workflowStatuses.map(s => ({
        value: s.slug,
        label: s.name,
        appearance: (s.category === 'done' ? 'success' : s.category === 'in_progress' ? 'inprogress' : 'default') as LozengeAppearance,
      }));
    }
    // Fallback to demand_process_steps
    return processSteps.map(s => ({ value: s.value, label: s.label, appearance: stepToLozengeAppearance(s) as LozengeAppearance }));
  }, [workflowStatuses, processSteps]);
  ...
  return (
    ...
    >
      {/* Toolbar — two-cluster layout identical to BacklogPage */}
      <div style={{
        display: 'flex', alignItems: 'center', gap: 8,
        padding: '8px 24px',
          ...
          groups={[
            ...
            { items: [
              { id: 'export', label: 'Export to CSV', icon: <AkDownloadIcon label="" size="small" />,
                onClick: () => {
                  const headers = ['Key','Summary','Type','Status','Priority','Delivery manager','Product owner','Target date'];
                  ...
                  a.download = `${key}-allwork.csv`; a.click();
                },
              },
