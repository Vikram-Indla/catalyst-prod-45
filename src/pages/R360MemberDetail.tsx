    ...
    return (
      <div ref={canvasRef} className="r3-ring-canvas" style={{ marginTop: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          {/* Name + Role */}
          <div style={{ fontSize: '16px', fontWeight: 650, color: '#0F172A', fontFamily: "'Sora', sans-serif" }}>{name}</div>
          <div style={{ fontSize: '13px', color: '#64748B', marginBottom: '16px' }}>{role}</div>
          ...
          {/* Divider */}
          <div style={{ height: '1px', background: '#F1F5F9', margin: '20px 0', width: '100%' }} />

          {/* Open Items Section */}
          {nonDone.length > 0 && (
            <div style={{ textAlign: 'left' as const, marginBottom: '16px' }}>
              <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748B', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: '10px' }}>Open Items</div>
              {nonDone.map(item => {
                ...
                return (
                  <div key={item.id} onClick={() => onSelect(item)} style={{
                    ...
                    marginBottom: '8px', cursor: 'pointer', background: '#FFFFFF',
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '5px' }}>
                      ...
                      <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: '3px', background: '#F1F5F9', color: '#64748B' }}>{item.project_key}</span>
                      <span style={{ marginLeft: 'auto', fontSize: '10px', fontWeight: 600, padding: '1px 6px', borderRadius: '4px', background: '#F8FAFC', color: item.age_days > 30 ? '#D97706' : '#64748B', fontFamily: "'JetBrains Mono', monospace" }}>{item.age_days}d</span>
                    </div>
                    {/* Row 3 — full title */}
                ...
                return (
                  <div key={item.id} onClick={() => onSelect(item)} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '8px', cursor: 'pointer' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '12px', color: '#0F172A', marginTop: '2px' }}>{item.title}</div>
                      <div style={{ fontSize: '11px', color: '#94A3B8', marginTop: '2px' }}>Resolved{resolvedLabel ? ` · ${resolvedLabel}` : ''}</div>
                    </div>
                  </div>
                );
              })}
          ...
          {nonDone.length === 0 && doneItems.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '20px 0' }}>
              ...
              <span style={{ fontSize: '13px', color: '#94A3B8', fontStyle: 'italic' }}>No activity recorded this week</span>
            </div>
          )}
        </div>
      ...
      {visible.map((item, i) => {
        ...
        return (
          <div key={item.id} style={{ position: 'absolute', left: slotPos.left, top: slotPos.top }}>
            ...
            <div style={{ textAlign: 'center', marginTop: '6px', fontSize: '11px', color: '#94A3B8', width: `${CARD_W}px`, pointerEvents: 'none' }}>
