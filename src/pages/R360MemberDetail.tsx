  ...
  return (
    <div style={{ padding: '10px 0', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'}` }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' as const }}>
        ...
        <span style={{ fontSize: '13px', fontWeight: 500, color: isDark ? '#888888' : '#334155' }}>{period.range}</span>
        ...
        <button style={{ width: '28px', height: '28px', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: '4px', background: isDark ? '#111111' : '#FFF', cursor: 'pointer', fontSize: '13px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', transition: 'background var(--cp-duration-fast, 0.15s) ease' }} onClick={() => onNavigatePeriod(-1)}
          onMouseEnter={e => { e.currentTarget.style.background = isDark ? 'rgba(255,255,255,0.04)' : 'rgba(15,23,42,0.04)'; }}
          onMouseLeave={e => { e.currentTarget.style.background = isDark ? '#111111' : '#FFF'; }}
        >‹</button>
        <button style={{ width: '28px', height: '28px', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: '4px', background: isDark ? '#111111' : '#FFF', cursor: 'pointer', fontSize: '13px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', transition: 'background var(--cp-duration-fast, 0.15s) ease' }} onClick={() => onNavigatePeriod(1)}
          onMouseEnter={e => { e.currentTarget.style.background = isDark ? 'rgba(255,255,255,0.04)' : 'rgba(15,23,42,0.04)'; }}
          onMouseLeave={e => { e.currentTarget.style.background = isDark ? '#111111' : '#FFF'; }}
        ...
        <span style={{ marginLeft: 'auto', fontSize: '12.5px', color: isDark ? '#666666' : '#64748B', fontFamily: "'JetBrains Mono', monospace" }}>{weekItems.length} items</span>
      </div>
  ...
  return (
    <>
      <div id="r360-root" data-r360-page-content style={{ position: 'relative', width: '100%', minWidth: 0, overflow: 'hidden' }}>
        <div className="r3-page" style={{ background: isDark ? '#111111' : '#FFFFFF', height: '100%', overflow: 'auto' }}>
          {/* ── Sticky Header: Profile + Week Nav ── */}
          <div style={{ position: 'sticky', top: 0, zIndex: 10, background: isDark ? '#111111' : '#FFFFFF' }}>
            {/* ── Profile Header ── */}
            <div className="r3-profile">
              ...
              <div className="r3-tabs">
                ...
                <button
                  ...
                  style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'transparent', border: 'none', color: isDark ? '#666666' : '#64748B', fontSize: '13px', fontWeight: 500, cursor: 'pointer', padding: '4px 8px' }}
          ...
          {itemsLoading ? (
            ...
            <div>
              ...
              {workItems.length > 0 && lastActivityDate && (
                <div style={{ margin: '16px auto', maxWidth: 560, padding: '16px 24px', borderRadius: '8px', border: '1px solid rgba(15,23,42,0.12)', background: isDark ? '#111111' : '#FFFFFF', textAlign: 'center' }}>
                  <div style={{ fontSize: '13px', color: isDark ? '#888888' : '#334155', marginBottom: '10px' }}>
                  ...
                  <div style={{ fontSize: '12.5px', color: isDark ? '#666666' : '#64748B', marginBottom: '12px' }}>
                    Last activity: <strong style={{ color: isDark ? '#888888' : '#334155' }}>{lastActivityDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</strong>
                  </div>
                  <button
...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  return <span style={{ fontSize: '10.5px', fontWeight: 500, color: isDark ? '#666666' : '#64748B' }}>{priority}</span>;
}
        ...
        >
          ...
          <div style={{ padding: '12px 16px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={(e) => { e.stopPropagation(); setShowDone(false); }}
              style={{ width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: '4px', background: isDark ? '#111111' : '#FFF', cursor: 'pointer', color: isDark ? '#666666' : '#64748B', fontSize: '14px' }}
              aria-label="Close completed panel"
            >✕</button>
          </div>
          {/* Throughput */}
          <div style={{ padding: '8px 16px', fontSize: '12px', color: isDark ? '#666666' : '#64748B', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'}` }}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: '3px', background: isDark ? '#111111' : '#F1F5F9', color: isDark ? '#666666' : '#64748B' }}>{item.project_key}</span>
                    ...
                    <div style={{ fontSize: '11px', color: isDark ? '#666666' : '#64748B', marginTop: '2px', fontStyle: 'italic' }}>Resolved{resolvedLabel ? ` · ${resolvedLabel}` : ''}</div>
                  </div>
                </div>
              );
            })}
          ...
          {/* Footer */}
          <div style={{ padding: '10px 16px', borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'}`, fontSize: '11px', color: isDark ? '#666666' : '#94A3B8', textAlign: 'center', fontStyle: 'italic' }}>
    ...
    return (
      <div ref={canvasRef} className="r3-ring-canvas" style={{ marginTop: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          background: isDark ? '#111111' : '#FFFFFF', border: '1px solid rgba(15,23,42,0.08)',
          ...
          <div style={{ fontSize: '13px', color: isDark ? '#666666' : '#64748B', marginBottom: '16px' }}>{role}</div>

          {/* Week Stats Row */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', fontSize: '13px' }}>
            <span style={{ fontWeight: 650, color: isDark ? '#888888' : '#334155' }}>{nonDone.length} open</span>
          ...
          {/* Divider */}
          <div style={{ height: '1px', background: isDark ? '#111111' : '#F1F5F9', margin: '20px 0', width: '100%' }} />

          {/* Open Items Section */}
          {nonDone.length > 0 && (
            <div style={{ textAlign: 'left' as const, marginBottom: '16px' }}>
              <div style={{ fontSize: '11px', fontWeight: 700, color: isDark ? '#666666' : '#64748B', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: '10px' }}>Open Items</div>
              {nonDone.map(item => {
                ...
                return (
                  <div key={item.id} onClick={() => onSelect(item)} style={{
                    ...
                    marginBottom: '8px', cursor: 'pointer', background: isDark ? '#111111' : '#FFFFFF',
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        ...
                        <span style={{ fontSize: '11px', fontWeight: 650, textTransform: 'uppercase' as const, color: isDark ? '#888888' : '#334155' }}>{item.item_type}</span>
                      </div>
                      <span style={{ fontSize: '11px', color: isDark ? '#666666' : '#64748B' }}>{item.priority}</span>
                    </div>
                    {/* Row 2 */}
                ...
                return (
                  <div key={item.id} onClick={() => onSelect(item)} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '8px', cursor: 'pointer' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '11px', color: isDark ? '#666666' : '#94A3B8', marginTop: '2px' }}>Resolved{resolvedLabel ? ` · ${resolvedLabel}` : ''}</div>
                    </div>
                  </div>
                );
              })}
              <div style={{ fontSize: '12px', color: isDark ? '#666666' : '#64748B', marginTop: '8px' }}>
                {doneCount} of {totalItems} total resolved ({totalItems > 0 ? Math.round((doneCount / totalItems) * 100) : 0}%)
              </div>
            </div>
          )}
          ...
          {nonDone.length === 0 && doneItems.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '20px 0' }}>
              ...
              <span style={{ fontSize: '13px', color: isDark ? '#666666' : '#94A3B8', fontStyle: 'italic' }}>No activity recorded this week</span>
            </div>
          )}
        </div>
    ...
    <div ref={canvasRef} className="r3-ring-canvas" style={{ marginTop: '8px', overflow: 'visible', position: 'relative' }}>
          ...
          <div key={item.id} style={{ position: 'absolute', left: slotPos.left, top: slotPos.top }}>
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginTop: 'auto', flexShrink: 0, height: '24px' }}>
                ...
                {isContributor && (
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '10px', fontWeight: 500, color: isDark ? '#666666' : '#64748B' }}>→ <MiniAvatar name={item.assignee_name} size={16} /> {item.assignee_name}</span>
                )}
              </div>
            </div>
            {/* Updated Xd ago label below card */}
            <div style={{ textAlign: 'center', marginTop: '6px', fontSize: '11px', color: isDark ? '#666666' : '#94A3B8', width: `${CARD_W}px`, pointerEvents: 'none' }}>
      ...
      {nonDone.length > PAGE_SIZE && (
        <div style={{
          ...
          fontSize: '11px', fontWeight: 600, color: isDark ? '#666666' : '#64748B', background: isDark ? '#0A0A0A' : '#F8FAFC',
      ...
      {/* EMPTY STATE */}
      {items.length === 0 && (
        <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center', color: isDark ? '#666666' : '#64748B', fontSize: '14px' }}>
      ...
      <div key={item.id} className="r3-chrono-card" onClick={() => onSelect(item)}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {item.role_on_item === 'Contributor' && (
              <span style={{ fontSize: 12.5, color: isDark ? '#888888' : '#334155', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                <span style={{ color: isDark ? '#666666' : '#64748B' }}>→</span> <MiniAvatar name={item.assignee_name} size={18} /> {item.assignee_name}
              </span>
            )}
            <span
              style={{
      ...
      <div data-testid="r360-chrono-today" ref={el => { groupRefs.current[todayStr] = el; }} className="r3-date-group">
        ...
        {!collapsed.has('__today__') && (
          todayGroup ? (
            ...
            <div style={{ paddingLeft: '28px', fontSize: '12px', fontStyle: 'italic', color: isDark ? '#666666' : '#94A3B8', padding: '8px 0 8px 28px' }}>
      ...
      {carryoverItems.length > 0 && (
        <div data-testid="r360-chrono-carryover" ref={carryoverRef} className="r3-date-group">
          <div className="r3-date-header" onClick={() => setCollapsed(prev => { const n = new Set(prev); n.has('__carryover__') ? n.delete('__carryover__') : n.add('__carryover__'); return n; })}>
            ...
            <span style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '3px', background: isDark ? '#111111' : '#F1F5F9', color: isDark ? '#666666' : '#64748B' }}>No activity this week</span>
                  ...
                  <div key={item.id} className="r3-board-card" onClick={() => onSelect(item)}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 500, color: isDark ? '#888888' : '#334155' }}>{item.priority}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <StatusLozenge status={item.status} statusCategory={item.status_category} />
                        {item.carried_from_label && (
      ...
      <div className="r3-panel r3-panel--open">
        ...
        <div className="r3-panel-header">
          ...
          <div className="r3-panel-pills">
            ...
            <span style={{ fontSize: 10.5, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: isDark ? '#111111' : '#F1F5F9', color: isDark ? '#888888' : '#334155' }}>{item.priority}</span>
        ...
        <div className="r3-panel-body">
          ...
          <div className="r3-meta">
            ...
            <div className="r3-meta-cell">
              ...
              <div style={{ fontSize: 11, color: isDark ? '#888888' : '#334155' }}>{formatDate(item.created_at)}</div>
            </div>
            <div className="r3-meta-cell">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ width: 60, height: 4, borderRadius: 2, background: isDark ? '#111111' : '#F1F5F9', overflow: 'hidden' }}>
          ...
          {item.parent_key && (
            <div className="r3-hierarchy">
              ...
              <div className="r3-hier-item" style={{ padding: '6px 8px' }}>
                ...
                <span style={{ fontSize: 12, color: isDark ? '#888888' : '#334155', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.parent_title}</span>
              </div>
    ...
    <>
      ...
      <div className="r3-panel r3-panel--open" style={{ maxWidth: 480 }}>
        ...
        <div className="r3-panel-header" style={{ borderBottom: '1px solid rgba(15,23,42,0.08)', paddingBottom: 12 }}>
          ...
          <div style={{ fontSize: 12, color: isDark ? '#666666' : '#64748B' }}>
        ...
        <div className="r3-panel-body" style={{ padding: 0 }}>
          {items.length === 0 ? (
            <div style={{ padding: 24, textAlign: 'center', color: isDark ? '#666666' : '#94A3B8', fontSize: 13 }}>
              No {mode} items
            </div>
          ) : (
            items.map(item => (
