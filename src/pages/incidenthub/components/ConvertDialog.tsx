    ...
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-[640px]" style={{ borderRadius: 8, padding: 0 }}>
        <DialogHeader className="px-6 pt-5 pb-3" style={{ borderBottom: '0.75px solid rgba(15,23,42,0.06)' }}>
          <DialogTitle style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 16, fontWeight: 700 }}>Convert Incident</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-3 gap-3">
            {TYPES.map(t => (
              ...
              >
                ...
                <div style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 650, color: 'var(--fg-1, #0F172A)', marginBottom: 2 }}>{t.label}</div>
                <div style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: '#64748B' }}>{t.desc}</div>
              </button>
            ))}
          </div>
        </div>
