    ...
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-[760px]" style={{ borderRadius: 8, padding: 0 }}>
        <DialogHeader className="px-6 pt-5 pb-3" style={{ borderBottom: '0.75px solid rgba(15,23,42,0.06)' }}>
          <DialogTitle style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 16, fontWeight: 700 }}>Escalation Committee</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex items-center gap-1 px-6" style={{ borderBottom: '0.75px solid rgba(15,23,42,0.06)' }}>
          {TABS.map((tab, i) => (
            <button
              ...
              className="px-3 py-2"
              style={{
                fontFamily: 'var(--ds-font-family-body)', fontSize: 12,
        ...
        <div className="px-6 py-4" style={{ minHeight: 300, maxHeight: 440, overflowY: 'auto' }}>
          {activeTab === 0 && (
            <div className="space-y-3">
              {members.length === 0 && (
                <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: '#94A3B8' }}>No committee members assigned.</p>
              )}
              {members.map((m: any) => {
                const vote = m.vote;
                  ...
                  <div key={m.id} className="flex items-center gap-3 p-2" style={{ border: `1px solid ${isDark ? '#2E2E2E' : 'rgba(15,23,42,0.08)'}`, borderRadius: 4 }}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, fontWeight: 650, color: 'var(--fg-1, #0F172A)' }}>
                      ...
                      <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: '#64748B' }}>{m.role || 'Member'}</span>
                    </div>
          ...
          {activeTab === 1 && (
            <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: '#94A3B8' }}>Approver management coming soon.</p>
          )}

          {activeTab === 2 && (
            <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: '#94A3B8' }}>Activity log coming soon.</p>
          )}
        </div>
