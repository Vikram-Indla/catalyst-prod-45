      ...
      <DialogContent className="sm:max-w-[760px]" style={{ borderRadius: 8, padding: 0 }}>
        ...
        <div className="px-6 py-4" style={{ minHeight: 300, maxHeight: 440, overflowY: 'auto' }}>
          {activeTab === 0 && (
            <div className="space-y-3">
              {members.length === 0 && (
                <p style={{ fontFamily: 'var(--cp-font-body)', fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>No committee members assigned.</p>
              )}
              {members.map((m: any) => {
                const vote = m.vote;
                ...
                return (
                  <div key={m.id} className="flex items-center gap-3 p-2" style={{ border: `1px solid ${'var(--cp-border-subtle, rgba(15,23,42,0.08))'}`, borderRadius: 4 }}>
                    ...
                    <div className="flex-1">
                      ...
                      <span style={{ fontFamily: 'var(--cp-font-body)', fontSize: 11, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>{m.role || 'Member'}</span>
                    </div>
                        ...
                        <span
                          ...
                          style={{
                            ...
                            backgroundColor: voteStatus === v
                              ? (v === 'approved' ? '#1B7F37' : v === 'rejected' ? '#FEE2E2' : 'var(--ds-border, var(--ds-border, #DFE1E6))')
                              : 'transparent',
                            color: voteStatus === v
                              ? (v === 'approved' ? 'var(--ds-text-inverse, #FFFFFF)' : v === 'rejected' ? 'var(--ds-text-danger, var(--ds-text-danger, #991B1B))' : '#42526E')
          ...
          {activeTab === 1 && (
            <p style={{ fontFamily: 'var(--cp-font-body)', fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>Approver management coming soon.</p>
          )}

          {activeTab === 2 && (
            <p style={{ fontFamily: 'var(--cp-font-body)', fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>Activity log coming soon.</p>
          )}
        </div>
        ...
        <DialogFooter className="px-6 py-3" style={{ borderTop: '0.75px solid rgba(15,23,42,0.06)' }}>
          ...
          <Button
            ...
            style={{
              backgroundColor: quorumMet ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : undefined,
              borderRadius: 6,
