  ...
  if (!incident) {
    return (
      <div className="flex-1 flex items-center justify-center" style={{ backgroundColor: isDark ? '#0A0A0A' : '#FFFFFF' }}>
        <p style={{ fontFamily: 'var(--ds-font-family-body)', color: isDark ? '#878787' : '#94A3B8' }}>Incident not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ backgroundColor: isDark ? '#0A0A0A' : '#FFFFFF' }}>
      ...
      <div className="flex items-center justify-between px-6 shrink-0" style={{
        ...
        <div className="flex items-center gap-1" style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: isDark ? '#878787' : '#64748B' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto" style={{ padding: '28px 32px 40px' }}>
          ...
          <div className="flex items-start gap-3 mb-4">
            ...
            <h1 style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 20, fontWeight: 700, color: isDark ? '#EDEDED' : '#0F172A', letterSpacing: '-0.02em' }}>
              {incident.title}
            </h1>
          </div>
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 12, color: '#2563EB' }}>
              {incident.incident_key || incident.jira_key}
            </span>
            <span style={{ color: isDark ? '#878787' : '#CBD5E1' }}>&middot;</span>
            <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: isDark ? '#878787' : '#64748B' }}>
          ...
          {(slaBreached || slaWarning) && (
            <div className="flex items-center gap-3 p-3 mb-4" style={{
              ...
              <div>
                <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: slaBreached ? '#DC2626' : '#D97706', fontWeight: 650 }}>
                  {slaBreached ? 'SLA BREACHED' : 'SLA breach in '}
                </span>
                <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 14, fontWeight: 700, color: slaBreached ? '#DC2626' : '#D97706' }}>
                  {slaCountdown}
                </span>
              </div>
            </div>
          ...
          {/* Description */}
          <div className="mb-6">
            <h3 style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 700, color: isDark ? '#EDEDED' : '#0F172A', marginBottom: 8 }}>Description</h3>
            <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 14, lineHeight: 1.75, color: isDark ? '#A1A1A1' : '#334155' }}>
          ...
          {incident.labels && Array.isArray(incident.labels) && incident.labels.length > 0 && (
            <div className="mb-6">
              <h3 style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 700, color: isDark ? '#EDEDED' : '#0F172A', marginBottom: 8 }}>Labels</h3>
          ...
          {/* Activity Section */}
          <div>
            <h3 style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 700, color: isDark ? '#EDEDED' : '#0F172A', marginBottom: 8 }}>Activity</h3>
              ...
              <div>
                ...
                <div className="mb-4">
                  <Textarea
                    ...
                    style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, minHeight: 60, borderRadius: 4 }}
                  />
                ...
                {(!incident.comments || incident.comments.length === 0) && (
                  <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: isDark ? '#878787' : '#94A3B8' }}>No comments yet. Be the first to comment.</p>
                )}
                {incident.comments?.map((c: any) => (
                  <div key={c.id} className="mb-3 pb-3" style={{ borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(15,23,42,0.06)' }}>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, fontWeight: 650, color: isDark ? '#EDEDED' : '#0F172A' }}>
                        {c.author?.full_name || c.author_name || 'User'}
                      </span>
                      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, color: isDark ? '#878787' : '#94A3B8' }}>
                        {c.created_at ? formatDistanceToNow(new Date(c.created_at), { addSuffix: true }) : ''}
                      </span>
                    </div>
                    <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, color: isDark ? '#A1A1A1' : '#334155', lineHeight: 1.6 }}>{c.content}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {activeTab === 'history' && (
              <div>
                {(!incident.history || incident.history.length === 0) && (
                  <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: isDark ? '#878787' : '#94A3B8' }}>No history entries.</p>
                )}
                {incident.history?.map((h: any) => (
                  <div key={h.id} className="flex items-start gap-3 mb-3 pb-3" style={{ borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(15,23,42,0.06)' }}>
                    ...
                    <div>
                      <div style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, fontWeight: 650, color: isDark ? '#EDEDED' : '#0F172A' }}>
                        {h.field_name} changed
                      </div>
                      <div className="flex items-center gap-1 mt-0.5">
                        {h.old_value && (h.field_name === 'status' ? <StatusLozenge status={h.old_value} /> : <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, color: isDark ? '#878787' : '#94A3B8' }}>{h.old_value}</span>)}
                        <span style={{ color: isDark ? '#878787' : '#94A3B8', fontSize: 11 }}>&rarr;</span>
                        {h.new_value && (h.field_name === 'status' ? <StatusLozenge status={h.new_value} /> : <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, color: isDark ? '#EDEDED' : '#0F172A' }}>{h.new_value}</span>)}
                      </div>
                      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, color: isDark ? '#878787' : '#94A3B8' }}>
        ...
        <div className="shrink-0 overflow-y-auto" style={{
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={row.label} className="flex items-center gap-2">
                <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: isDark ? '#878787' : '#64748B', width: 80, flexShrink: 0 }}>{row.label}</span>
                <div style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: isDark ? '#EDEDED' : '#0F172A' }}>
          ...
          <div className="mt-4 pt-4" style={{ borderTop: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(15,23,42,0.06)' }}>
            <div className="flex items-center gap-2 mb-2">
              <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: isDark ? '#878787' : '#64748B', width: 80 }}>Created</span>
              <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 12, color: isDark ? '#A1A1A1' : '#64748B' }}>
                {incident.created_at ? new Date(incident.created_at).toLocaleString() : '\u2014'}
              </span>
            </div>
            <div className="flex items-center gap-2 mb-2">
              <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: isDark ? '#878787' : '#64748B', width: 80 }}>Updated</span>
              <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 12, color: isDark ? '#A1A1A1' : '#64748B' }}>
