...
export default function IncidentDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 p-6" style={{ backgroundColor: isDark ? '#1A1714' : '#FFFFFF' }}>
  ...
  if (!incident) {
    return (
      <div className="flex-1 flex items-center justify-center" style={{ backgroundColor: isDark ? '#1A1714' : '#FFFFFF' }}>
        <p style={{ fontFamily: 'Inter, sans-serif', color: isDark ? '#6B6560' : '#94A3B8' }}>Incident not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ backgroundColor: isDark ? '#1A1714' : '#FFFFFF' }}>
      {/* Breadcrumb */}
      <div className="flex items-center justify-between px-6 shrink-0" style={{
        ...
        <div className="flex items-center gap-1" style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: isDark ? '#6B6560' : '#64748B' }}>
          <span className="cursor-pointer hover:underline" onClick={() => navigate('/incident-hub')}>Incident List</span>
          <ChevronRight size={12} />
          <span style={{ color: isDark ? '#F5F3F0' : '#0F172A', fontWeight: 650 }}>{incident.incident_key || incident.jira_key || 'INC'}</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto" style={{ padding: '28px 32px 40px' }}>
          ...
          <div className="flex items-start gap-3 mb-4">
            ...
            <h1 style={{ fontFamily: 'Sora, sans-serif', fontSize: 20, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', letterSpacing: '-0.02em' }}>
              {incident.title}
            </h1>
          </div>
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <span style={{ color: isDark ? '#6B6560' : '#CBD5E1' }}>&middot;</span>
            <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: '#2563EB' }}>
              {incident.incident_key || incident.jira_key}
            </span>
            <span style={{ color: isDark ? '#6B6560' : '#CBD5E1' }}>&middot;</span>
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: isDark ? '#6B6560' : '#64748B' }}>
          ...
          {/* Description */}
          <div className="mb-6">
            <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 8 }}>Description</h3>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, lineHeight: 1.75, color: isDark ? '#A09890' : '#334155' }}>
          ...
          {incident.labels && Array.isArray(incident.labels) && incident.labels.length > 0 && (
            <div className="mb-6">
              <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 8 }}>Labels</h3>
              <div className="flex flex-wrap gap-1">
                {(incident.labels as string[]).map((label: string) => (
                  <span key={label} className="px-2 py-0.5" style={{ fontSize: 11, backgroundColor: isDark ? '#232019' : '#F1F5F9', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(15,23,42,0.08)', borderRadius: 3, color: isDark ? '#A09890' : '#475569' }}>{label}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {/* Activity Section */}
          <div>
            <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 8 }}>Activity</h3>
              ...
              {(['comments', 'history'] as const).map(tab => (
                <button
                  ...
                  style={{
                    ...
                    color: activeTab === tab ? '#2563EB' : (isDark ? '#6B6560' : '#64748B'),
            ...
            {activeTab === 'comments' && (
              <div>
                ...
                {(!incident.comments || incident.comments.length === 0) && (
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: isDark ? '#6B6560' : '#94A3B8' }}>No comments yet. Be the first to comment.</p>
                )}
                {incident.comments?.map((c: any) => (
                  <div key={c.id} className="mb-3 pb-3" style={{ borderBottom: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.06)' }}>
                    <div className="flex items-center gap-2 mb-1">
                      <div className="rounded-full flex items-center justify-center" style={{ width: 24, height: 24, backgroundColor: isDark ? '#2C2823' : '#E2E8F0', fontSize: 10, fontWeight: 650, color: isDark ? '#A09890' : '#475569' }}>
                        {(c.author?.full_name || c.author_name || 'U').charAt(0).toUpperCase()}
                      </div>
                      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A' }}>
                        {c.author?.full_name || c.author_name || 'User'}
                      </span>
                      <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: isDark ? '#6B6560' : '#94A3B8' }}>
                        {c.created_at ? formatDistanceToNow(new Date(c.created_at), { addSuffix: true }) : ''}
                      </span>
                    </div>
                    <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: isDark ? '#A09890' : '#334155', lineHeight: 1.6 }}>{c.content}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {activeTab === 'history' && (
              <div>
                {(!incident.history || incident.history.length === 0) && (
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: isDark ? '#6B6560' : '#94A3B8' }}>No history entries.</p>
                )}
                {incident.history?.map((h: any) => (
                  <div key={h.id} className="flex items-start gap-3 mb-3 pb-3" style={{ borderBottom: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.06)' }}>
                    <div className="shrink-0 mt-1" style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: '#2563EB', border: isDark ? '2px solid #1A1714' : '2px solid #FFFFFF', boxShadow: '0 0 0 1px #2563EB' }} />
                    <div>
                      <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A' }}>
                        {h.field_name} changed
                      </div>
                      <div className="flex items-center gap-1 mt-0.5">
                        {h.old_value && (h.field_name === 'status' ? <StatusLozenge status={h.old_value} /> : <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: isDark ? '#6B6560' : '#94A3B8' }}>{h.old_value}</span>)}
                        <span style={{ color: isDark ? '#6B6560' : '#94A3B8', fontSize: 11 }}>&rarr;</span>
                        {h.new_value && (h.field_name === 'status' ? <StatusLozenge status={h.new_value} /> : <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: isDark ? '#F5F3F0' : '#0F172A' }}>{h.new_value}</span>)}
                      </div>
                      <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: isDark ? '#6B6560' : '#94A3B8' }}>
        ...
        <div className="shrink-0 overflow-y-auto" style={{
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={row.label} className="flex items-center gap-2">
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: isDark ? '#6B6560' : '#64748B', width: 80, flexShrink: 0 }}>{row.label}</span>
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: isDark ? '#F5F3F0' : '#0F172A' }}>
          ...
          <div className="mt-4 pt-4" style={{ borderTop: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.06)' }}>
            <div className="flex items-center gap-2 mb-2">
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: isDark ? '#6B6560' : '#64748B', width: 80 }}>Created</span>
              <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: isDark ? '#A09890' : '#64748B' }}>
                {incident.created_at ? new Date(incident.created_at).toLocaleString() : '\u2014'}
              </span>
            </div>
            <div className="flex items-center gap-2 mb-2">
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: isDark ? '#6B6560' : '#64748B', width: 80 }}>Updated</span>
              <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: isDark ? '#A09890' : '#64748B' }}>
