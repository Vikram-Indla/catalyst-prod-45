  ...
  if (!incident) {
    return (
      <div className="flex-1 flex items-center justify-center" style={{ backgroundColor: '#FFFFFF' }}>
        <p style={{ fontFamily: 'Inter, sans-serif', color: '#94A3B8' }}>Incident not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ backgroundColor: '#FFFFFF' }}>
      ...
      <div className="flex items-center justify-between px-6 shrink-0" style={{
        ...
        <div className="flex items-center gap-1" style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#64748B' }}>
          <span className="cursor-pointer hover:underline" onClick={() => navigate('/incident-hub')}>Incident List</span>
          <ChevronRight size={12} />
          <span style={{ color: '#0F172A', fontWeight: 650 }}>{incident.incident_key || incident.jira_key || 'INC'}</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto" style={{ padding: '28px 32px 40px' }}>
          ...
          <div className="flex items-start gap-3 mb-4">
            ...
            <h1 style={{ fontFamily: 'Sora, sans-serif', fontSize: 20, fontWeight: 700, color: '#0F172A', letterSpacing: '-0.02em' }}>
              {incident.title}
            </h1>
          </div>
          ...
          {/* Description */}
          <div className="mb-6">
            <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 8 }}>Description</h3>
          ...
          {/* Activity Section */}
          <div>
            <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 8 }}>Activity</h3>
            ...
            {activeTab === 'comments' && (
              <div>
                ...
                {(!incident.comments || incident.comments.length === 0) && (
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#94A3B8' }}>No comments yet. Be the first to comment.</p>
                )}
                {incident.comments?.map((c: any) => (
                  <div key={c.id} className="mb-3 pb-3" style={{ borderBottom: '0.75px solid rgba(15,23,42,0.06)' }}>
                    <div className="flex items-center gap-2 mb-1">
                      <div className="rounded-full flex items-center justify-center" style={{ width: 24, height: 24, backgroundColor: '#E2E8F0', fontSize: 10, fontWeight: 650, color: '#475569' }}>
            ...
            {activeTab === 'history' && (
              <div>
                {(!incident.history || incident.history.length === 0) && (
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#94A3B8' }}>No history entries.</p>
                )}
                {incident.history?.map((h: any) => (
                  <div key={h.id} className="flex items-start gap-3 mb-3 pb-3" style={{ borderBottom: '0.75px solid rgba(15,23,42,0.06)' }}>
                    ...
                    <div>
                      <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 650, color: '#0F172A' }}>
                        {h.field_name} changed
                      </div>
                      <div className="flex items-center gap-1 mt-0.5">
                        {h.old_value && (h.field_name === 'status' ? <StatusLozenge status={h.old_value} /> : <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#94A3B8' }}>{h.old_value}</span>)}
                        <span style={{ color: '#94A3B8', fontSize: 11 }}>&rarr;</span>
                        {h.new_value && (h.field_name === 'status' ? <StatusLozenge status={h.new_value} /> : <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#0F172A' }}>{h.new_value}</span>)}
                      </div>
                      <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#94A3B8' }}>
