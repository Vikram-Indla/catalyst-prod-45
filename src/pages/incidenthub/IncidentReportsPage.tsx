  ...
  return (
    <div className="flex-1 overflow-auto" style={{ backgroundColor: 'var(--bg-app, #FFFFFF)' }}>
      <div className="px-6 pt-6 pb-4">
          ...
          <div>
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="p-3" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: 'var(--fg-3, #64748B)', marginBottom: 4 }}>Total Breaches</div>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 22, fontWeight: 700, color: '#DC2626' }}>{breachedCount}</div>
              </div>
              <div className="p-3" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: 'var(--fg-3, #64748B)', marginBottom: 4 }}>Breach Rate</div>
              ...
              <div className="p-3" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: 'var(--fg-3, #64748B)', marginBottom: 4 }}>Avg Age (days)</div>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 22, fontWeight: 700, color: 'var(--fg-1, #0F172A)' }}>{avgAge}</div>
              </div>
            </div>
            <div className="p-8 flex items-center justify-center" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6, backgroundColor: 'var(--surface-muted, #F1F5F9)' }}>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: 'var(--fg-3, #64748B)' }}>[Chart] SLA Breach trend over time</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 1 && (
          <div className="p-8 flex items-center justify-center" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6, backgroundColor: 'var(--surface-muted, #F1F5F9)' }}>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: 'var(--fg-3, #64748B)' }}>[Chart] Incident aging distribution by severity</p>
          </div>
        )}
            ...
            <div className="flex items-center gap-2 mb-4">
              {[
                ...
                <div key={s.label} className="flex items-center gap-2 p-3 flex-1" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
                  ...
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: 'var(--fg-3, #64748B)' }}>{s.label}</span>
        ...
        {activeTab === 3 && (
          <div className="p-8 flex items-center justify-center" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6, backgroundColor: 'var(--surface-muted, #F1F5F9)' }}>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: 'var(--fg-3, #64748B)' }}>[Chart] Severity vs Priority heatmap</p>
          </div>
        )}
      </div>
    </div>
