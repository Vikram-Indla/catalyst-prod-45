    ...
    <div className="flex-1 overflow-auto" style={{ backgroundColor: isDark ? '#0A0A0A' : '#FFFFFF' }}>
      <div className="px-6 pt-6 pb-4">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 18, fontWeight: 700, color: '#0F172A' }}>Analytics</h1>
            <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: '#64748B' }}>Incident management metrics and trends</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[
            ...
            <div key={s.label} className="p-3" style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFFFFF', border: isDark ? '1px solid #2E2E2E' : '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
              <div style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: '#64748B', marginBottom: 4 }}>{s.label}</div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="p-4" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
            <h3 style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 12 }}>Incidents by Severity</h3>
            <div className="space-y-2">
              {analytics.bySeverity.map(([sev, count]) => (
                <div key={sev} className="flex items-center gap-2">
                  <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, color: '#64748B', width: 40 }}>{sev}</span>
                  ...
                  <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, color: '#0F172A', width: 24, textAlign: 'right' }}>{count}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* By Status */}
          <div className="p-4" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
            <h3 style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 12 }}>Status Distribution</h3>
            <div className="space-y-2">
              {analytics.byStatus.map(([status, count]) => (
                <div key={status} className="flex items-center gap-2">
                  <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: '#64748B', width: 80, textTransform: 'capitalize' }}>{status.replace(/_/g, ' ')}</span>
                  <div className="flex-1" style={{ height: 16, backgroundColor: '#F1F5F9', borderRadius: 4, overflow: 'hidden' }}>
          ...
          {/* By Assignee */}
          <div className="p-4" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6 }}>
            <h3 style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 12 }}>Assignee Workload</h3>
            <div className="space-y-2">
              {analytics.byAssignee.slice(0, 8).map(([name, count]) => (
                <div key={name} className="flex items-center gap-2">
                  <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 11, color: '#64748B', width: 100, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</span>
                  <div className="flex-1" style={{ height: 16, backgroundColor: '#F1F5F9', borderRadius: 4, overflow: 'hidden' }}>
          ...
          <div className="p-4 flex items-center justify-center" style={{ border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6, minHeight: 180 }}>
            <div className="text-center" style={{ backgroundColor: '#F1F5F9', borderRadius: 6, padding: '24px 32px', border: '1px solid rgba(15,23,42,0.08)' }}>
              <p style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: '#64748B' }}>[Chart] Resolution Trend over time</p>
            </div>
          </div>
        </div>
      </div>
