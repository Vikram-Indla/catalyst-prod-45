...
export default function IdeaDrawer({ ideaKey, onClose, onConvert }: Props) {
  ...
  if (isLoading) return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '560px', background: isDark ? '#232019' : '#FFFFFF', zIndex: 201, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    <>
      ...
      <div style={{
        ...
        {canEdit ? (
          ...
          <div style={{
            padding: '12px 20px', borderTop: `0.75px solid ${dk.border}`,
            backgroundColor: isDark ? '#232019' : '#FFFFFF', display: 'flex', justifyContent: 'flex-end', flexShrink: 0,
          }}>
...
function FieldBlock({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <div>
      <div style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: isDark ? '#6B6560' : '#64748B', marginBottom: '6px' }}>{label}</div>
      {children}
    </div>
  );
}
