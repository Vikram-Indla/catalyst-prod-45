...
const FUNNEL_ORDER: { key: string; label: string; color: string }[] = [
  { key: 'draft', label: 'Draft', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' },
  { key: 'submitted', label: 'Submitted', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' },
  { key: 'under_review', label: 'Under Review', color: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' },
  { key: 'approved', label: 'Approved', color: 'var(--ds-text-success, var(--ds-text-success, #16A34A))' },
  { key: 'converted', label: 'Converted', color: '#0D9488' },
  { key: 'rejected', label: 'Rejected', color: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))' },
];

const DEPT_COLORS = ['var(--ds-text-brand, var(--ds-text-brand, #2563EB))', '#0D9488', 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', '#7C3AED', 'var(--ds-text-success, var(--ds-text-success, #16A34A))', 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))', 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', '#0F766E', '#6366F1', 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))'];
...
export default function IdeationAnalyticsView({ ideas }: Props) {
  ...
  const contributors = useMemo(() => {
    ...
    const colors = ['#0D9488', 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', '#7C3AED', 'var(--ds-text-success, var(--ds-text-success, #16A34A))'];
  ...
  // ── Metric cards ──
  const METRICS = [
    { label: 'TOTAL SUBMISSIONS', value: String(stats.total), color: dk.t1, sub: `${stats.total} ideas in backlog`, subColor: dk.t3, icon: ClipboardList, iconBg: 'var(--cp-primary-light, #EFF6FF)', iconColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' },
    { label: 'AVG IMPACT SCORE', value: stats.avgImpact.toFixed(2), color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', sub: 'across all ideas', subColor: dk.t3, icon: BarChart3, iconBg: 'var(--cp-primary-light, #EFF6FF)', iconColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' },
    { label: 'CONVERSION RATE', value: `${stats.convRate.toFixed(1)}%`, color: dk.greenText, sub: `${stats.converted} ideas → requests`, subColor: dk.greenText, icon: RefreshCw, iconBg: 'var(--cp-success-light, #F0FDFA)', iconColor: '#0D9488' },
    { label: 'AI COVERAGE', value: `${stats.aiPct}%`, color: 'var(--ds-text-brand, var(--ds-text-brand, #3B82F6))', sub: `${stats.aiReady} of ${stats.total} enriched`, subColor: dk.t3, icon: Sparkles, iconBg: 'var(--cp-primary-light, #EFF6FF)', iconColor: 'var(--ds-text-brand, var(--ds-text-brand, #3B82F6))' },
    { label: 'PIPELINE VALUE', value: String(stats.pipeline), color: dk.greenText, sub: 'Active ideas in pipeline', subColor: dk.greenText, icon: Rocket, iconBg: 'var(--cp-success-light, #F0FDF4)', iconColor: 'var(--ds-text-success, var(--ds-text-success, #16A34A))' },
  ];
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: '8px', height: '160px' }}>
            {weeks.map(w => (
              <div key={w.w} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                ...
                <div style={{
                  width: '32px', height: `${Math.max((w.c / maxWeek) * 120, 4)}px`, background: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))',
          ...
          {contributors.length > 0 ? contributors.map(c => (
            <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: '10px', height: '40px' }}>
              ...
              <div style={{ flex: 1, height: '6px', background: barTrack, borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{ width: `${c.pct}%`, height: '100%', background: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', borderRadius: '4px' }} />
              </div>
            </div>
          )) : (
              ...
              {traces.map((t, i) => (
                <div key={t.idea} style={{
                  ...
                  <span style={{
                    ...
                    <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--ds-text-success, var(--ds-text-success, #16A34A))' }} />
                    {t.status}
                  </span>
                </div>
              ))}
