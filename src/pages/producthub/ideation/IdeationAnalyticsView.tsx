...
const FUNNEL_ORDER: { key: string; label: string; color: string }[] = [
  { key: 'draft', label: 'Draft', color: 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' },
...
const DEPT_COLORS = ['var(--ds-text-brand, #2563EB)', 'var(--cp-teal-60, #0D9488)', 'var(--ds-text-warning, var(--cp-warning, #D97706))', 'var(--cp-purple-60, #7C3AED)', 'var(--ds-text-success, var(--cp-success, #16A34A))', 'var(--ds-text-danger, #EF4444)', 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))', '#0F766E', '#6366F1', 'var(--ds-text-danger, var(--cp-danger, #DC2626))'];
...
export default function IdeationAnalyticsView({ ideas }: Props) {
  ...
  const contributors = useMemo(() => {
    ...
    const colors = ['var(--cp-teal-60, #0D9488)', 'var(--ds-text-brand, #2563EB)', 'var(--ds-text-warning, var(--cp-warning, #D97706))', 'var(--cp-purple-60, #7C3AED)', 'var(--ds-text-success, var(--cp-success, #16A34A))'];
  ...
  // ── Metric cards ──
  const METRICS = [
    { label: 'TOTAL SUBMISSIONS', value: String(stats.total), color: dk.t1, sub: `${stats.total} ideas in backlog`, subColor: dk.t3, icon: ClipboardList, iconBg: 'var(--cp-primary-light, #EFF6FF)', iconColor: 'var(--ds-text-brand, #2563EB)' },
            ...
            <div key={f.label} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
              ...
              <div style={{ flex: 1, height: '50px', background: barTrack, borderRadius: '6px', overflow: 'hidden' }}>
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', paddingLeft: '12px', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', fontSize: '12px', fontWeight: 700,
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: '8px', height: '160px' }}>
            {weeks.map(w => (
              <div key={w.w} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                ...
                <div style={{
                  width: '32px', height: `${Math.max((w.c / maxWeek) * 120, 4)}px`, background: 'var(--ds-text-brand, #2563EB)',
          ...
          {contributors.length > 0 ? contributors.map(c => (
            <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: '10px', height: '40px' }}>
              <div style={{
                ...
                color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', fontSize: '9px', fontWeight: 700,
              }}>{c.initials}</div>
              ...
              <div style={{ flex: 1, height: '6px', background: barTrack, borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{ width: `${c.pct}%`, height: '100%', background: 'var(--ds-text-brand, #2563EB)', borderRadius: '4px' }} />
              </div>
            </div>
          )) : (
