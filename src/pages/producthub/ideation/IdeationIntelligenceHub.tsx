...
function computeThemes(ideas: Idea[]) {
  ...
  const colors = ['var(--ds-text-brand, #2563EB)', 'var(--cp-purple-60, #7C3AED)', 'var(--ds-text-warning, var(--cp-warning, #D97706))', 'var(--ds-text-success, var(--cp-success, #16A34A))', 'var(--cp-teal-60, #0D9488)', 'var(--ds-text-danger, #EF4444)', '#EC4899', '#6366F1'];
      ...
      <div style={{ padding: '0 32px 32px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        ...
        <ContentCard title="Team Distribution" badge={`${teamStats.length} teams`} badgeColor={C.success}>
          {teamStats.map(([team, count]) => {
            ...
            const barColor = team.includes('BAU') ? 'var(--ds-text-brand, #2563EB)' : team.includes('Integration') ? 'var(--cp-teal-60, #0D9488)' : team.includes('Mobile') ? 'var(--cp-purple-60, #7C3AED)' : 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))';
            return (
