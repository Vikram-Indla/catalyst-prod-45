  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--divider, #E2E8F0)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Sparkles size={18} color="#2563EB" strokeWidth={2} />
              <span style={{ fontSize: '16px', fontWeight: 600, color: 'var(--fg-1, #0F172A)' }}>AI Intelligence — Triage Results</span>
            </div>
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--fg-4, #94A3B8)', padding: '4px' }}><X size={18} /></button>
          </div>
          <div style={{ fontSize: '13px', fontWeight: 500, color: 'var(--fg-3, #64748B)', marginTop: '4px' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', scrollbarWidth: 'thin' }}>
          ...
          {totalRecommendations === 0 && (
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--fg-4, #94A3B8)', fontSize: '14px' }}>
              No triage recommendations at this time.
            </div>
          )}

          <div style={{ marginTop: '20px', borderTop: '1px solid var(--divider, #E2E8F0)', paddingTop: '16px' }}>
            <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--fg-3, #64748B)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>Team Distribution</div>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '16px' }}>
              {teamStats.map(([team, count]) => (
                <span key={team} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '11px', fontWeight: 600, background: 'var(--surface-muted, #F1F5F9)', border: '1px solid var(--divider, #E2E8F0)', color: 'var(--fg-2, #475569)', padding: '4px 10px', borderRadius: '4px' }}>
                  {team} <span style={{ fontWeight: 700, color: 'var(--fg-1, #0F172A)' }}>{count}</span>
                </span>
              ))}
            </div>
            <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--fg-3, #64748B)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>Top Themes</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {themeStats.map(([theme, count]) => (
                <div key={theme} style={{ background: 'var(--surface-subtle, #F8FAFC)', border: '1px solid var(--divider, #E2E8F0)', borderRadius: '8px', padding: '8px', textAlign: 'center' }}>
                  <div style={{ fontSize: '16px', fontWeight: 800, color: 'var(--fg-1, #0F172A)' }}>{count}</div>
                  <div style={{ fontSize: '9px', fontWeight: 600, color: 'var(--fg-3, #64748B)', marginTop: '2px', direction: /[\u0600-\u06FF]/.test(theme) ? 'rtl' : 'ltr' }}>{theme}</div>
                </div>
              ))}
            </div>
          </div>
...
function CategoryHeader({ label, sub }: { label: string; sub: string }) {
  ...
  return (
    <div style={{ marginBottom: '12px', marginTop: '8px' }}>
      <div style={{ borderTop: '1px solid var(--divider, #E2E8F0)', marginBottom: '16px' }} />
      <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', color, textTransform: 'uppercase' }}>{label}</div>
      <div style={{ fontSize: '12px', color: 'var(--fg-3, #64748B)', marginTop: '2px' }}>{sub}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: isDark ? 'transparent' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.10)' : '#E2E8F0'}`, borderRadius: '10px', padding: '16px', marginBottom: '10px', boxShadow: isDark ? 'none' : '0 1px 2px rgba(0,0,0,0.04)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'var(--surface-muted, #F1F5F9)', color: 'var(--fg-2, #475569)', padding: '3px 10px', borderRadius: '4px', fontSize: '11px', fontWeight: 600, letterSpacing: '0.02em', textTransform: 'uppercase' }}>
          <span style={{ width: '4px', height: '4px', borderRadius: '50%', background: dotColor, flexShrink: 0 }} />
          {badge}
        </span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', fontWeight: 600, color: 'var(--fg-3, #64748B)' }}>{ideaKey}</span>
      </div>
      <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--fg-1, #0F172A)', marginTop: '8px', lineHeight: 1.4 }}>{title}</div>
      <div style={{ fontSize: '13px', fontWeight: 400, color: 'var(--fg-3, #64748B)', lineHeight: 1.5, marginTop: '4px' }}>{body}</div>
      {aiSuggestion && (
        <div style={{ background: 'var(--surface-subtle, #F8FAFC)', borderLeft: '2px solid var(--divider, #CBD5E1)', borderRadius: '0 6px 6px 0', padding: '8px 12px', marginTop: '10px', fontSize: '12px', fontWeight: 500, color: 'var(--fg-2, #475569)', lineHeight: 1.4 }}>
          {aiSuggestion}
        </div>
      )}
      <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
        ...
        <button onClick={secondary.onClick} style={{ background: 'var(--bg-app, #FFFFFF)', color: 'var(--fg-2, #475569)', border: '1.5px solid var(--divider, #E2E8F0)', borderRadius: '6px', padding: '7px 16px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '6px' }}
