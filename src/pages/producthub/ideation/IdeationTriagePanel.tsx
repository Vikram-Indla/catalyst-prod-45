  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}` }}>
          ...
          <div style={{ marginTop: '20px', borderTop: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, paddingTop: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '16px' }}>
              {teamStats.map(([team, count]) => (
                <span key={team} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '11px', fontWeight: 600, background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9))', border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, color: 'var(--cp-text-secondary, #475569)', padding: '4px 10px', borderRadius: '4px' }}>
                  {team} <span style={{ fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, #0F172A))' }}>{count}</span>
                </span>
              ))}
            </div>
            <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>Top Themes</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {themeStats.map(([theme, count]) => (
                <div key={theme} style={{ background: 'var(--cp-bg-page, #F8FAFC)', border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, borderRadius: '8px', padding: '8px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '9px', fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', marginTop: '2px', direction: /[\u0600-\u06FF]/.test(theme) ? 'rtl' : 'ltr' }}>{theme}</div>
                </div>
              ))}
            </div>
          </div>
...
function CategoryHeader({ label, sub }: { label: string; sub: string }) {
  const { isDark } = useTheme();
  const color = CATEGORY_TEXT_COLORS[label] || 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))';
  return (
    <div style={{ marginBottom: '12px', marginTop: '8px' }}>
      ...
      <div style={{ fontSize: '12px', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', marginTop: '2px' }}>{sub}</div>
    </div>
  );
}
...
function TriageCard({ badge, ideaKey, title, body, aiSuggestion, primary, secondary }: {
  ...
  return (
    <div style={{ background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, borderRadius: '12px', padding: '16px', marginBottom: '10px', boxShadow: isDark ? 'none' : '0 1px 2px rgba(0,0,0,0.04)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
        ...
        <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: '12px', fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>{ideaKey}</span>
      </div>
      ...
      <div style={{ fontSize: '13px', fontWeight: 400, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', lineHeight: 1.5, marginTop: '4px' }}>{body}</div>
      {aiSuggestion && (
