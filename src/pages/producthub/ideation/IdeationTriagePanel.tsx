...
const DOT_COLORS: Record<string, string> = {
  'Fast-Track': 'var(--ds-text-success, var(--cp-success, #16A34A))', 'Merge': 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))', 'Investigate': 'var(--ds-text-warning, var(--cp-warning, #D97706))', 'Defer': 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))',
};
...
const CATEGORY_TEXT_COLORS: Record<string, string> = {
  ...
  'NEEDS INVESTIGATION': 'var(--ds-text-warning, var(--cp-warning, #D97706))', 'RECOMMENDED TO DEFER': 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))',
};
...
function CategoryHeader({ label, sub }: { label: string; sub: string }) {
  const { isDark } = useTheme();
  const color = CATEGORY_TEXT_COLORS[label] || 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))';
  return (
...
function TriageCard({ badge, ideaKey, title, body, aiSuggestion, primary, secondary }: {
  ...
  const { isDark } = useTheme();
  const dotColor = DOT_COLORS[badge] || 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))';
  return (
