...
const DOT_COLORS: Record<string, string> = {
  'Fast-Track': 'var(--ds-text-success, var(--ds-text-success, #16A34A))', 'Merge': 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', 'Investigate': 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', 'Defer': 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))',
};

const CATEGORY_TEXT_COLORS: Record<string, string> = {
  'FAST-TRACK RECOMMENDED': 'var(--ds-text-success, var(--ds-text-success, #16A34A))', 'MERGE CANDIDATES': 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))',
  'NEEDS INVESTIGATION': 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', 'RECOMMENDED TO DEFER': 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))',
};
        ...
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${'var(--cp-border, #E2E8F0)'}` }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Sparkles size={18} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" strokeWidth={2} />
...
function CategoryHeader({ label, sub }: { label: string; sub: string }) {
  const { isDark } = useTheme();
  const color = CATEGORY_TEXT_COLORS[label] || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))';
  return (
...
function TriageCard({ badge, ideaKey, title, body, aiSuggestion, primary, secondary }: {
  ...
  const { isDark } = useTheme();
  const dotColor = DOT_COLORS[badge] || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))';
  return (
    <div style={{ background: 'var(--cp-bg-elevated, #FFFFFF)', border: `1px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: '12px', padding: '16px', marginBottom: '10px', boxShadow: isDark ? 'none' : '0 1px 2px rgba(0,0,0,0.04)' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
        <button onClick={primary.onClick} style={{ background: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', color: 'var(--ds-text-inverse, #FFFFFF)', border: 'none', borderRadius: '6px', padding: '7px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '6px' }}
          onMouseEnter={e => (e.currentTarget.style.background = 'var(--ds-background-brand-bold-hovered, var(--ds-background-brand-bold-hovered, #1D4ED8))')} onMouseLeave={e => (e.currentTarget.style.background = 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))')}>
