...
const ROADMAP_COLS = [
  { key: null, label: 'NO QUARTER', borderColor: 'transparent', textColor: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' },
  { key: 'Q1', label: 'Q1 2026', borderColor: 'var(--ds-text-danger, var(--ds-text-danger, #991B1B))', textColor: 'var(--ds-text-danger, var(--ds-text-danger, #991B1B))' },
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: dk.t2, cursor: 'pointer' }}>
              <input type="checkbox" checked={committedOnly} onChange={e => setCommittedOnly(e.target.checked)} style={{ accentColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} />
              Committed only
            </label>
          </div>
        </div>
      ...
      <div style={{ background: dk.pageBg, borderBottom: `1px solid ${dk.border}`, padding: '10px 28px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        {TEAMS.map(t => (
          <button key={t} onClick={() => setTeamFilter(t)} style={{
            background: teamFilter === t ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : ('var(--cp-bg-elevated, #FFFFFF)'), color: teamFilter === t ? 'var(--ds-text-inverse, #FFFFFF)' : dk.t2,
            border: `1px solid ${teamFilter === t ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : dk.border}`,
...
function RoadmapCard({ idea, onClick, isDark, dk }: { idea: IdeaRow; onClick: () => void; isDark: boolean; dk: typeof DK }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      border: `1px solid ${isDark ? 'var(--ds-border-bold, var(--ds-border-bold, #454545))' : dk.border}`,
      ...
      onMouseEnter={e => {
        ...
        e.currentTarget.style.borderColor = isDark ? 'var(--ds-border-bold, var(--ds-border-bold, #454545))' : 'rgba(37,99,235,0.3)';
      }}
      onMouseLeave={e => {
        if (!isDark) {
