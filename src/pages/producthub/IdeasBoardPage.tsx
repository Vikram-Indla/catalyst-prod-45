            ...
            return (
              <div key={col.status} style={{ minWidth: '260px', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', padding: '0 4px', height: 36 }}>
                  ...
                  <span style={{ fontSize: '10px', fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, background: isDark ? 'rgba(255,255,255,0.06)' : col.countBg, borderRadius: '10px', padding: '0 6px', height: 18, display: 'inline-flex', alignItems: 'center', color: isDark ? dk.t3 : col.countText }}>{colIdeas.length}</span>
                </div>
                {colIdeas.length === 0 && (
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
        <span style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9', color: dk.t2, padding: '1px 6px', borderRadius: '3px', fontSize: '11px', fontWeight: 500, border: isDark ? `1px solid ${dk.border}` : 'none' }}>{idea.idea_type || 'Feature'}</span>
