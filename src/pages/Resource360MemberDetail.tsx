...
const CardsSkeleton = () => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
    {[1, 2, 3, 4].map(i => (
      <div key={i} style={{ background: '#FFF', border: '1px solid var(--bd-default, rgba(255,255,255,0.10))', borderRadius: 8, padding: '14px 16px', height: 64 }}>
  ...
  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
    {[1, 2, 3].map(col => (
      <div key={col}>
        ...
        {[1, 2].map(card => (
          <div key={card} style={{ background: '#FFF', border: '1px solid var(--bd-default, rgba(255,255,255,0.10))', borderRadius: 8, padding: 12, marginBottom: 8 }}>
...
/** Ring skeleton */
const RingSkeleton = () => (
  <div style={{ position: 'relative', minHeight: 640, background: 'radial-gradient(circle, #FFF, #1A1A1A)', borderRadius: 12, border: '1px solid var(--bd-default, rgba(255,255,255,0.10))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div id="r360-root">
      <div style={{ maxWidth: 1440, margin: '0 auto', padding: '16px 20px' }}>
            ...
            {(allMembers as any[]).map((m: any) => (
              <Link
                ...
                style={{
                  ...
                  background: m.id === memberId ? '#2563EB' : '#1A1A1A',
                  color: m.id === memberId ? '#FFFFFF' : 'rgba(237,237,237,0.53)',
        ...
        {hasError && (
          <div style={{ textAlign: 'center', padding: 40, background: 'rgba(248,113,113,0.06)', border: '1px solid #FECACA', borderRadius: 12, marginBottom: 12 }} role="alert">
            <div style={{ fontSize: 14, fontWeight: 600, color: '#F87171', marginBottom: 4 }}>Failed to load data</div>
            <div style={{ fontSize: 12, color: '#B91C1C', marginBottom: 10 }}>{(memberError || chronoError)?.message}</div>
            <button
              onClick={() => window.location.reload()}
              style={{ fontSize: 12, padding: '6px 16px', borderRadius: 6, border: '1px solid #FECACA', background: '#FFF', color: '#F87171', cursor: 'pointer' }}
            >
              Retry
            </button>
          </div>
