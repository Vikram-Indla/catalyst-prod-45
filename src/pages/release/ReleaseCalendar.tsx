                    ...
                    <div className="space-y-0.5">
                      {day.releases.slice(0, 2).map((release) => (
                        <Tooltip key={release.id}>
                          ...
                          <TooltipContent side="top" className="bg-card border shadow-lg p-2 max-w-[200px]">
                            <div className="space-y-1">
                              <div className="font-semibold text-xs text-foreground">{release.name}</div>
                              <div className="text-[10px] text-muted-foreground">
                                {format(new Date(release.releaseDate), 'MMM dd, yyyy')}
                              </div>
                              <div className="flex items-center gap-1.5">
                                <span className={cn(
                                  "px-1.5 py-0.5 rounded text-[9px] font-medium text-white",
                                  getReleaseBarColor(release)
                                )}>
