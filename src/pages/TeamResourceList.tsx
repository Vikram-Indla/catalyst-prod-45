    ...
    <div style={{ padding: 24, fontFamily: 'var(--cp-font-body)' }}>
      {resources.map((r) => (
        <div
          ...
          style={{ cursor: 'pointer', padding: '8px 0', borderBottom: '1px solid var(--ds-border, var(--cp-border, var(--cp-bg-sunken)))' }}
        >
          <span style={{ fontSize: 'var(--ds-font-size-400)', color: 'var(--ds-text, var(--cp-text-primary, var(--cp-text-inverse)))' }}>{r.displayName}</span>
        </div>
      ))}
    </div>
  );
