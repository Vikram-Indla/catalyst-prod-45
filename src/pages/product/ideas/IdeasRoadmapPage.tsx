...
function SkeletonCard({ isDark }: { isDark?: boolean }) {
  return <div style={{ height: 80, background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken)))', borderRadius: 8, animation: 'shimmer 1.5s infinite' }} />;
}
    ...
    <div style={{ display: 'flex', gap: 12, padding: 16, flex: 1 }}>
      {[1, 2, 3, 4, 5].map(col => (
        <div key={col} style={{
          ...
          borderRadius: 8, border: isDark ? '1px solid var(--ds-background-neutral)' : '1px solid var(--cp-border, var(--cp-bg-sunken))', padding: 12,
        }}>
          <div style={{ height: 20, width: 100, background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken)))', borderRadius: 4, marginBottom: 8 }} />
  ...
  if (isError) {
    return (
      <div style={{
        ...
        justifyContent: 'center', gap: 12, background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', fontFamily: 'var(--cp-font-body)',
      }}>
        <div style={{ fontSize: 'var(--ds-font-size-400)', fontWeight: 600, color: 'var(--ds-text-danger, var(--cp-danger))' }}>Failed to load roadmap</div>
        <button
          onClick={() => refetch()}
          style={{
            height: 50, padding: '0 16px', borderRadius: 6, border: 'none',
            background: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', fontSize: 'var(--ds-font-size-300)', fontWeight: 600,
  ...
  if (!isLoading && ideas.length === 0) {
    return (
      <div style={{
        ...
        background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', fontFamily: 'var(--cp-font-body)',
      }}>
        ...
        <div style={{
          ...
          justifyContent: 'center', gap: 12, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary)))',
        }}>
          <div style={{ fontSize: 40, opacity: 0.3 }}>💡</div>
          <div style={{ fontSize: 'var(--ds-font-size-400)', fontWeight: 600, color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2)))' }}>No ideas in backlog yet</div>
          <div style={{ fontSize: 'var(--ds-font-size-300)', color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light)))' }}>Add ideas from the Ideas list to get started.</div>
          <a href="/producthub/ideation" style={{
            marginTop: 8, fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', textDecoration: 'none',
          }}>Go to Ideas →</a>
        </div>
      </div>
    );
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        {isLoading ? (
          ...
          <div style={{
            ...
            <div style={{ fontSize: 'var(--ds-font-size-400)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary)))', maxWidth: 340, lineHeight: 1.5 }}>
              No committed ideas yet. Toggle an idea's committed switch to include it on the roadmap.
            </div>
            <button
              onClick={() => setCommittedOnly(false)}
              style={{
                fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', background: 'none',
          ...
          <div style={{
            ...
            <div style={{ fontSize: 'var(--ds-font-size-400)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary)))' }}>No ideas match the current filters</div>
            <button
      ...
      <AlertDialog open={!!convertTarget} onOpenChange={open => { if (!open) setConvertTarget(null); }}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirmConvert} style={{ background: 'var(--cp-teal-60)', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))' }}>
              Convert
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
