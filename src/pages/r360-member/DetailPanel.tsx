        ...
        <div className="r3-panel-header">
          ...
          <div className="r3-panel-pills">
            ...
            {item.role_on_item === 'Contributor' && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: 'var(--ds-font-size-50)', fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: 'var(--cp-purple-5, #F5F3FF)', color: 'var(--cp-purple-60, #7C3AED)', border: `1px solid ${isDark ? 'var(--ds-background-discovery-bold, rgba(124,58,237,0.25))' : 'var(--ds-background-discovery, #F3F0FF)'}` }}>
        ...
        <div className="r3-panel-body">
          ...
          <div className="r3-meta">
            ...
            <div className="r3-meta-cell">
              ...
              <div style={{ fontSize: 'var(--ds-font-size-100)', color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))' }}>{formatDate(item.created_at)}</div>
            </div>
            <div className="r3-meta-cell">
              <div className="r3-meta-label">Days Sitting</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span className={`r3-age r3-age--${item.age_class}`} style={{ fontSize: 'var(--ds-font-size-300)', fontWeight: 600 }}>{item.age_days}</span>
            ...
            <div className="r3-meta-cell">
              ...
              <div className="r3-meta-value">
                {item.fix_version ? item.fix_version
                  : item.item_type === 'Sub-task' && item.parent_key ? <span style={{ color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))', fontSize: 'var(--ds-font-size-200)' }}>Inherited from {item.parent_key}</span>
                  : '\u2014'}
              </div>
            </div>
            <div className="r3-meta-cell">
          ...
          {item.parent_key && (
            <div className="r3-hierarchy">
              ...
              <div className="r3-hier-item" style={{ padding: '6px 8px' }}>
                ...
                <span style={{ fontSize: 'var(--ds-font-size-200)', color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.parent_title}</span>
              </div>
              ...
              <div className="r3-hier-item r3-hier-item--current">
                ...
                <span style={{ fontSize: 'var(--ds-font-size-200)', color: 'var(--cp-text-primary, #020617)', fontWeight: 500 }}>{item.title}</span>
              </div>
            </div>
          )}
            ...
            <div className="r3-siblings">
              ...
              {siblings.map((s: any) => (
                ...
                >
                  ...
                  <span style={{ fontSize: 'var(--ds-font-size-200)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{s.title}</span>
