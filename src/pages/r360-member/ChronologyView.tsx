...
export function ChronologyView({ items, onSelect, weekStart, weekEnd }: { items: R360WorkItem[]; onSelect: (i: R360WorkItem) => void; weekStart: Date; weekEnd: Date }) {
  ...
  const accentColor = (cat: string) => cat === 'in_progress' ? 'var(--ds-text-brand, #2563EB)' : cat === 'in_qa' ? 'var(--cp-teal-60, #0D9488)' : cat === 'blocked' ? 'var(--ds-text-danger, #EF4444)' : cat === 'done' ? 'var(--ds-text-success, var(--cp-success, #16A34A))' : 'var(--ds-text-warning, #D97706)';
