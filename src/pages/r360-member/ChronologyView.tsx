...
export function ChronologyView({ items, onSelect, weekStart, weekEnd }: { items: R360WorkItem[]; onSelect: (i: R360WorkItem) => void; weekStart: Date; weekEnd: Date }) {
  ...
  const accentColor = (cat: string) => cat === 'in_progress' ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : cat === 'in_qa' ? '#0D9488' : cat === 'blocked' ? 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))' : cat === 'done' ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : 'var(--ds-text-warning, var(--ds-text-warning, #D97706))';
      ...
      <div key={dateKey} ref={el => { groupRefs.current[dateKey] = el; }} className="r3-date-group">
        <div className="r3-date-header" onClick={() => setCollapsed(prev => { const n = new Set(prev); n.has(dateKey) ? n.delete(dateKey) : n.add(dateKey); return n; })}>
          ...
          <div className="r3-minibar">
            {statusDist.done > 0 && <div style={{ width: `${statusDist.done / total * 100}%`, background: 'var(--ds-text-success, var(--ds-text-success, #16A34A))' }} />}
            {statusDist.in_progress > 0 && <div style={{ width: `${statusDist.in_progress / total * 100}%`, background: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} />}
            {statusDist.to_do > 0 && <div style={{ width: `${statusDist.to_do / total * 100}%`, background: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' }} />}
            {statusDist.blocked > 0 && <div style={{ width: `${statusDist.blocked / total * 100}%`, background: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))' }} />}
          </div>
      ...
      <div data-testid="r360-chrono-today" ref={el => { groupRefs.current[todayStr] = el; }} className="r3-date-group">
        <div className="r3-date-header" onClick={() => setCollapsed(prev => { const n = new Set(prev); n.has('__today__') ? n.delete('__today__') : n.add('__today__'); return n; })}>
          ...
          {todayGroup && (() => {
            ...
            return (
              <div className="r3-minibar">
                {sd.done > 0 && <div style={{ width: `${sd.done / total * 100}%`, background: 'var(--ds-text-success, var(--ds-text-success, #16A34A))' }} />}
                {sd.in_progress > 0 && <div style={{ width: `${sd.in_progress / total * 100}%`, background: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} />}
                {sd.to_do > 0 && <div style={{ width: `${sd.to_do / total * 100}%`, background: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' }} />}
                {sd.blocked > 0 && <div style={{ width: `${sd.blocked / total * 100}%`, background: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))' }} />}
              </div>
            );
          })()}
