...
export function StatusLozenge({ status, statusCategory }: { status: string; statusCategory?: string }) {
  ...
  return <LozengeSpan bg={token('color.background.neutral', 'rgba(9,30,66,0.06)')} color={token('color.text.subtle', '#44546F')} label={label} />;
}
...
export function getChronologyStatusLozengeColors(status: string, statusCategory?: string): { background: string; color: string } {
  ...
  if (cat === 'new' || cat === 'todo') return { background: token('color.background.neutral', 'rgba(9,30,66,0.06)'), color: token('color.text.subtle', '#44546F') };
  ...
  return { background: token('color.background.neutral', 'rgba(9,30,66,0.06)'), color: token('color.text.subtle', '#44546F') };
}
