        ...
        <div className="r3-panel-header" style={{ borderBottom: '1px solid rgba(15,23,42,0.08)', paddingBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, #0F172A))' }}>{title}</span>
            </div>
          ...
          {items.length === 0 ? (
            ...
            items.map(item => (
              ...
              >
                ...
                {/* Title */}
                <span style={{ fontSize: 12, color: 'var(--cp-text-primary, var(--cp-ink-1, #0F172A))', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {item.title}
                </span>
                {/* Status */}
