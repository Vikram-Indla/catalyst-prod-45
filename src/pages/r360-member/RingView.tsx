...
import { categoryToAppearance } from '@/components/catalyst-detail-views/shared/sections/statusPalette';
...
const T = {
  surface:           () => token('elevation.surface', 'var(--ds-surface)'),
  surfaceRaised:     () => token('elevation.surface.raised', 'var(--ds-surface)'),
  bgNeutral:         () => token('color.background.neutral', 'var(--ds-background-neutral, var(--ds-background-neutral))'),
  bgNeutralSubtle:   () => token('color.background.neutral.subtle', 'var(--ds-surface-sunken, var(--ds-background-neutral-subtle))'),
  bgNeutralHovered:  () => token('color.background.neutral.hovered', '#E9EAEB'),
  bgSuccessBold:     () => token('color.background.success.bold', 'var(--ds-background-success-bold, var(--ds-background-success-bold))'),
  bgInfoBold:        () => token('color.background.information.bold', 'var(--cp-primary-60, var(--ds-link))'),
  text:              () => token('color.text', 'var(--cp-text-primary, var(--cp-text-inverse))'),
  textSubtle:        () => token('color.text.subtle', 'var(--ds-icon-subtle, var(--ds-text-subtlest))'),
  textSubtlest:      () => token('color.text.subtlest', 'var(--ds-text-disabled, var(--ds-border-bold))'),
  textDisabled:      () => token('color.text.disabled', 'var(--ds-text-disabled, var(--ds-border-bold))'),
  textInverse:       () => token('color.text.inverse', 'var(--ds-surface)'),
  textInfo:          () => token('color.text.information', 'var(--ds-link, var(--ds-link))'),
  textWarning:       () => token('color.text.warning', 'var(--ds-text-warning, var(--ds-text-warning))'),
  textDanger:        () => token('color.text.danger', 'var(--ds-text-danger, var(--ds-text-danger))'),
  textSuccess:       () => token('color.text.success', 'var(--ds-text-success, var(--ds-chart-green-bold))'),
  iconSuccess:       () => token('color.icon.success', 'var(--ds-background-success-bold, var(--ds-background-success-bold))'),
        ...
        >
          ...
          <div style={{ padding: '12px 16px', borderBottom: `1px solid ${T.border()}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 700, color: T.textInverse(), background: T.bgSuccessBold(), padding: '0px 8px', borderRadius: 12 }}>{doneCount}</span>
            </div>
            ...
            {doneItems.map(item => {
              ...
              return (
                <div key={item.id}
                  ...
                  style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '8px 16px', cursor: 'pointer', borderBottom: `1px solid ${T.border()}`, transition: 'background 80ms' }}
                  onMouseEnter={e => (e.currentTarget.style.background = T.bgNeutral())}
                  onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                >
                  <div style={{ flexShrink: 0, marginTop: 0 }}>{getJiraIcon(item.item_type)}</div>
          ...
          {doneCount > 0 && (
            <div style={{ padding: '8px 16px', borderTop: `1px solid ${T.border()}`, fontSize: 'var(--ds-font-size-100)', color: T.textSubtlest(), textAlign: 'center', fontStyle: 'italic' }}>
    ...
    return (
      <div ref={canvasRef} className="r3-ring-canvas" style={{ marginTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          {/* Divider */}
          <div style={{ height: 1, background: T.border(), margin: '16px 0', width: '100%' }} />

          {/* Open Items */}
          {nonDone.length > 0 && (
            <div style={{ textAlign: 'left' as const, marginBottom: 16 }}>
              <div style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 600, color: T.textSubtle(), letterSpacing: '0.04em', marginBottom: 8 }}>Open items</div>
              {nonDone.map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 4, minWidth: 0 }}>
                      ...
                      <span style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 600, color: T.textInfo(), fontFamily: MONO, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.item_key}</span>
                      <span style={{ fontSize: 'var(--ds-font-size-50)', fontWeight: 600, padding: '0px 5px', borderRadius: 3, background: T.bgNeutral(), color: T.textSubtle(), flexShrink: 0 }}>{item.project_key}</span>
                      <span style={{ marginLeft: 'auto', fontSize: 'var(--ds-font-size-50)', fontWeight: 600, color: item.age_days > 30 ? T.textWarning() : T.textSubtlest(), fontFamily: MONO, flexShrink: 0 }}>{item.age_days}d</span>
                    </div>
                    {/* Title */}
          ...
          {doneItems.length > 0 && (
            <div style={{ textAlign: 'left' as const, borderInlineStart: `3px solid ${T.borderSuccess()}`, paddingInlineStart: 12 }}>
              <div style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 600, color: T.textSuccess(), letterSpacing: '0.04em', marginBottom: 8 }}>Completed</div>
              {doneItems.map(item => {
                const closedDate = item.resolved_at || item.updated_at;
                const resolvedLabel = closedDate ? new Date(closedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '';
                return (
                  <div key={item.id} onClick={() => onSelect(item)} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 8, cursor: 'pointer' }}>
                    <div style={{ width: 16, height: 16, borderRadius: '50%', background: T.bgSuccessBold(), border: `1px solid ${T.borderSuccess()}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 0 }}>
  ...
  return (
    <div ref={canvasRef} className="r3-ring-canvas" style={{ marginTop: 8, overflow: 'visible', position: 'relative' }}>
          ...
          <div key={item.id} style={{ position: 'absolute', left: `${slotPos.left}px`, top: `${slotPos.top}px` }}>
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', marginTop: 'auto', flexShrink: 0, minHeight: 24 }}>
                {item.status && (
                  <Lozenge appearance={categoryToAppearance(item.status_category)} isBold={false} maxWidth={140}>{item.status}</Lozenge>
                )}
                {item.carried_from_label && (
      ...
      {nonDone.length > PAGE_SIZE && (
        <div style={{
          position: 'absolute', bottom: 8, left: '48%', transform: 'translateX(-50%)',
      ...
      {/* EMPTY STATE */}
      {items.length === 0 && (
        <div style={{ position: 'absolute', left: '48%', top: '48%', transform: 'translate(-50%,-50%)', textAlign: 'center', color: T.textSubtle(), fontSize: 'var(--ds-font-size-400)' }}>
