  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", padding: "0 24px", color: "var(--fg-1, #e5e5e5)" }}>
      ...
      <SectionCard title="2 · Embeddings (Content Chunks)" loading={embeddings.loading} error={embeddings.error}>
        {embeddings.data && (
          <>
            ...
            <p style={{ marginTop: 8, fontSize: 12, color: "var(--fg-2, #888)" }}>Total embeddings: <strong style={{ color: embeddingsTotal === 0 ? "var(--ds-text-danger, #EF4444)" : "var(--fg-1)" }}>{embeddingsTotal}</strong></p>
          </>
        )}
      </SectionCard>
      ...
      <SectionCard title="6 · Live Test" loading={false} error={null}>
        ...
        {liveTest.error && <p style={{ color: "var(--ds-text-danger, #EF4444)", fontSize: 13 }}>{liveTest.error}</p>}
        {liveTest.data && (
