              ...
              {filtered.map((so: any) => {
                ...
                return (
                  <tr key={so.id}
                    ...
                    className="cursor-pointer hover:bg-[rgba(15,23,42,0.04)] group"
                    style={{ height: 36, maxHeight: 36, borderBottom: '0.75px solid rgba(15,23,42,0.06)' }}
                  >
                    <td className="px-3" style={{ fontFamily: RH.fontMono, color: '#2563EB', fontWeight: 650 }}>{so.rh_changes?.chg_number || '—'}</td>
                    <td className="px-3 truncate max-w-[240px]" style={{ color: RH.ink2 }}>{so.rh_changes?.title || '—'}</td>
                    <td className="px-3" style={{ color: RH.ink2 }}>{so.signoff_role || so.stage || '—'}</td>
        ...
        <DialogContent className="sm:max-w-[520px]" style={{ background: 'var(--bg-app, #FFFFFF)' }}>
          ...
          {actionModal && (
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[12px] font-semibold text-[#475569] mb-1">Comment {actionModal.action === 'reject' && '*'}</label>
                <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Add a comment..."
                  className="w-full h-24 px-3 py-2 rounded-[4px] text-[13px] placeholder:text-[#94A3B8] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 resize-none"
                  style={{ border: '0.75px solid rgba(15,23,42,0.12)' }} />
              </div>
            </div>
          )}
          <DialogFooter>
            <button onClick={() => { setActionModal(null); setComment(''); }} className="h-9 px-4 rounded-[6px] text-[13px] font-medium text-[#475569] hover:bg-[#F1F5F9]" style={{ border: '0.75px solid rgba(15,23,42,0.12)' }}>Cancel</button>
