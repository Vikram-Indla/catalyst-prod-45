...
function SleekGadget({ title, subtitle, icon: Icon, collapsedContent, expandedContent, defaultOpen = false }: SleekGadgetProps) {
  ...
  return (
    <div className={cn("bg-brand-dark rounded-xl sm:rounded-2xl overflow-hidden shadow-lg border border-brand-primary/20", isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1))] border-[var(--ds-border,var(--cp-ink-1))]")}>
      <button
...
export default function DemandSummaryPage() {
  ...
  return (
    <div className={cn("min-h-screen bg-background", isArabic && "rtl", isDark && "bg-[var(--ds-surface)]")} style={{ fontFamily: 'var(--cp-font-body)' }} dir={isArabic ? 'rtl' : 'ltr'}>
      {/* Header */}
      <header className={cn("h-[72px] bg-card border-b border-border sticky top-0 z-50 flex-shrink-0", isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1))] border-[var(--ds-border,var(--cp-ink-1))]")}>
        <div className="h-full px-4 sm:px-6 flex items-center">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <button 
              ...
              className={cn("flex items-center gap-1.5 px-3 sm:px-3.5 py-2 bg-muted border-2 border-brand-primary rounded-lg text-xs font-medium text-foreground hover:bg-brand-primary/10 transition-colors", isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1))] text-[var(--ds-text,var(--cp-bg-neutral))]")}
            >
              <Globe className="w-4 h-4 text-brand-primary" />
              <span>{isArabic ? 'EN | عربي' : 'عربي | EN'}</span>
            </button>
            <div className={cn("flex bg-muted rounded-lg p-0.5", isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1))]")}>
              {periodOptions.map((p) => (
                <button key={p.key} onClick={() => setActivePeriod(p.key)} className={cn('px-2 sm:px-3 py-1.5 rounded-md text-xs font-medium transition-all', activePeriod === p.key ? (isDark ? 'bg-[var(--ds-surface)] text-[var(--ds-text,var(--cp-bg-neutral, var(--ds-background-neutral)))]' : 'bg-brand-dark text-white') : (isDark ? 'text-[var(--ds-text-subtlest)] hover:bg-[var(--ds-border,var(--cp-ink-1))]' : 'text-muted-foreground hover:bg-border'))}>{p.label}</button>
              ))}
            </div>
          </div>
          </div>
      ...
      {chatOpen && (
        <div className={cn(
          ...
          isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1))]",
