...
function SleekGadget({ title, subtitle, icon: Icon, collapsedContent, expandedContent, defaultOpen = false }: SleekGadgetProps) {
  ...
  return (
    <div className={cn("bg-brand-dark rounded-xl sm:rounded-2xl overflow-hidden shadow-lg border border-brand-primary/20", isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))]")}>
      <button
...
export default function DemandSummaryPage() {
  ...
  return (
    <div className={cn("min-h-screen bg-background", isArabic && "rtl", isDark && "bg-[var(--ds-surface,#0A0A0A)]")} style={{ fontFamily: 'var(--cp-font-body)' }} dir={isArabic ? 'rtl' : 'ltr'}>
      {/* Header */}
      <header className={cn("h-[72px] bg-card border-b border-border sticky top-0 z-50 flex-shrink-0", isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))]")}>
        <div className="h-full px-4 sm:px-6 flex items-center">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <button 
              ...
              className={cn("flex items-center gap-1.5 px-3 sm:px-3.5 py-2 bg-muted border-2 border-brand-primary rounded-lg text-xs font-medium text-foreground hover:bg-brand-primary/10 transition-colors", isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] text-[var(--ds-text,var(--cp-bg-neutral, #EDEDED))]")}
            >
      ...
      {chatOpen && (
        <div className={cn(
          ...
          isDark && "bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]",
