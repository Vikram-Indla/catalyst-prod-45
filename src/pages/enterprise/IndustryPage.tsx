...
export default function IndustryPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-muted text-foreground">
        {step.label}
      </span>;
  };
  const getBusinessScoreBadge = (score: number | null | undefined) => {
    ...
    if (score >= 90) colorClass = 'bg-green-100 text-green-700';else if (score >= 75) colorClass = 'bg-emerald-100 text-emerald-700';else if (score >= 60) colorClass = 'bg-amber-100 text-amber-700';else if (score >= 40) colorClass = 'bg-orange-100 text-orange-700';
    return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${colorClass}`}>
        {score}
      </span>;
  };
  ...
  const handleExport = () => {
    if (requests && requests.length > 0) {
      ...
      toast({
        title: 'Requests exported successfully'
      });
    }
  };
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      await Promise.all(affectedItems.map(item => supabase.from('business_requests').update({
        rank: item.rank
      }).eq('id', item.id)));
  ...
  return <div className="h-full flex flex-col bg-background">
      ...
      <div className="flex items-center justify-between gap-4 px-4 sm:px-6 py-3 border-b bg-card">
        ...
        <div className="flex items-center gap-2">
          {activeFilterCount > 0 && <span className="text-xs text-muted-foreground px-2 py-1 bg-muted rounded">
              {activeFilterCount} filter{activeFilterCount > 1 ? 's' : ''} applied
            </span>}

          {totalPages > 1 && <div className="flex items-center gap-1 border-l border-r px-3 border-border">
