...
export default function BusinessRequests() {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Business Requests imported successfully' });
    },
    onError: () => {
      toast({ title: 'Failed to import business requests', variant: 'destructive' });
    }
  });

  const handleExport = () => {
    if (items && items.length > 0) {
      exportToCSV(items, 'business-requests', ['name', 'description', 'status', 'health', 'start_date', 'end_date']);
      toast({ title: 'Business Requests exported successfully' });
    }
  };
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      <div className="border-b bg-card px-3 sm:px-6 py-3 sm:py-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold">Business Requests</h1>
            <p className="text-xs sm:text-sm text-muted-foreground">Enterprise-level business requests and initiatives</p>
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No business requests found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
      ...
      <ImportDialog
        ...
        title="Import Business Requests"
        requiredFields={['name']}
      />
    </div>
  );
