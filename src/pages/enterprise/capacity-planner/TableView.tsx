  ...
  const columns: CatalystColumn<ResourceMetric>[] = useMemo(() => [
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="font-semibold text-[14px] text-[var(--ds-text,var(--ds-text, #0f172a))] dark:text-foreground truncate">{value}</span>
                {/* Online indicator */}
                {isOnline && (
    ...
    },
    {
      ...
      render: (_: any, row: ResourceMetric) => {
        ...
        if (!vendor) {
          return <span className="text-[13px] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">-</span>;
        }
        return (
          <span className="text-[13px] font-medium text-[var(--ds-text-subtle,var(--ds-text-subtle, #334155))]">{vendor}</span>
        );
      },
    },
    {
      ...
      render: (_: any, row: ResourceMetric) => {
        ...
        if (resourceAllocations.length === 0) {
          return (
            <span className="text-[13px] font-medium text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">No assignments this month</span>
          );
        }
        ...
        return (
          <div className="flex flex-col gap-1.5">
            {resourceAllocations.slice(0, 3).map((alloc, idx) => {
              ...
              const accentColor = isCommitted ? 'var(--ds-text-brand, var(--ds-text-brand, #2563eb))' : 'var(--ds-text-warning, var(--ds-text-warning, #f59e0b))'; // Blue for committed, Amber for forecast
              const pctColor = isCommitted ? 'var(--ds-text-brand, var(--ds-text-brand, #2563eb))' : '#92400e';

              return (
                <span
                  key={idx}
                  className="inline-flex items-center gap-2.5 px-3 py-1.5 rounded bg-white text-[13px] font-medium text-[var(--ds-text-subtle,var(--ds-text-subtle, #334155))]"
                  style={{
            ...
            {resourceAllocations.length > 3 && (
              <span className="text-[11px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748b))]">+{resourceAllocations.length - 3} more</span>
            )}
          </div>
        );
      },
    },
    {
      ...
      sortable: true,
      render: (value: string) => (
        <span className="text-[13px] font-medium text-[var(--ds-text-subtle,var(--ds-text-subtle, #334155))] dark:text-slate-300">{value || '-'}</span>
      ),
    },
    {
      id: 'department',
      ...
      render: (value: string) => {
        ...
        const style = deptStyles[deptUpper] || { bg: 'rgba(100,116,139,0.12)', text: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))' };
    ...
    },
    {
      ...
      render: (_: any, row: ResourceMetric) => {
        ...
        if (!endDate) {
          return <span className="text-[13px] text-[var(--ds-text-subtle,var(--ds-text-subtle, #334155))]">Permanent</span>;
        }
        ...
        const textColors: Record<string, string> = {
          critical: '#b91c1c',
          warning: '#92400e',
          safe: 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))',
          expired: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748b))',
        };
      ...
      render: (_: any, row: ResourceMetric) => {
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button
            ...
            className="w-[30px] h-[30px] rounded-md flex items-center justify-center text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))] hover:text-[var(--ds-text,var(--ds-text, #0f172a))] hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #f1f5f9))] transition-colors"
            title="Edit resource"
          >
