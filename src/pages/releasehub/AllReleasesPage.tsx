...
export default function AllReleasesPage() {
  ...
  const accentColor = (status: string) => {
    ...
    if (s === 'in_progress') return 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))';
    if (s === 'released') return 'var(--ds-text-success, var(--ds-text-success, #16A34A))';
  ...
  return (
    <div style={{ background: 'var(--cp-bg-elevated, #FFFFFF)', minHeight: '100%', padding: '24px' }}>
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-[22px] font-extrabold" style={{ fontFamily: RH.fontDisplay, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : RH.ink1 }}>All Releases</h1>
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleImport} disabled={importing}
            ...
            style={{ border: `1px solid ${'var(--cp-border, #E2E8F0)'}`, background: 'var(--cp-bg-elevated, #FFFFFF)', color: 'var(--cp-text-secondary, #475569)' }}>
      ...
      {freezeConflicts.length > 0 && (
        <div style={{
          ...
          <AlertTriangle size={16} color="var(--ds-text-warning, var(--ds-text-warning, #D97706))" />
      ...
      <div className="flex items-center gap-2 mb-4">
        {[
          ...
          <button key={s.key} onClick={() => setFilter(s.key)}
            ...
            style={filter === s.key
              ? { borderColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', background: 'var(--cp-primary-light, #EFF6FF)', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="relative">
          ...
          <input type="text" placeholder="Search releases..." value={search} onChange={e => setSearch(e.target.value)}
            className="h-9 w-64 pl-9 pr-3 rounded text-[13px] focus:outline-none focus:ring-2 focus:ring-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]/20 focus:border-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]"
        ...
        <div className="flex items-center gap-1 rounded-md p-0.5" style={{ border: `1px solid ${'var(--cp-border-default, rgba(15,23,42,0.12))'}`, background: 'var(--cp-bg-elevated, #FFFFFF)' }}>
          <button onClick={() => setView('cards')} className="h-7 w-7 rounded flex items-center justify-center" style={view === 'cards' ? { background: 'var(--cp-primary-light, #EFF6FF)', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' } : { color: 'var(--cp-text-muted, #94A3B8)' }}><LayoutGrid size={14} /></button>
          <button onClick={() => setView('table')} className="h-7 w-7 rounded flex items-center justify-center" style={view === 'table' ? { background: 'var(--cp-primary-light, #EFF6FF)', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' } : { color: 'var(--cp-text-muted, #94A3B8)' }}><List size={14} /></button>
        </div>
      </div>
            ...
            return (
              <button key={r.id} onClick={() => setSelectedRelease(r)}
                ...
                <div className="p-4 pl-5">
                  <h3 className="text-[15px] font-bold mb-1" style={{ fontFamily: RH.fontDisplay, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : RH.ink1, fontWeight: 650 }}>{r.name}</h3>
                  ...
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    ...
                    {(r.source === 'jira') && relativeTime(r.synced_at || r.updated_at) && (
                      ...
                      >
                        <span className="inline-flex items-center gap-1 text-[11px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] cursor-default">
                          <Clock size={12} />
              ...
              {filtered.map((r: any) => {
                ...
                return (
                  <tr key={r.id} onClick={() => setSelectedRelease(r)}
                    ...
                    <td className="px-3 py-0 font-medium" style={{ color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : RH.ink1 }}>
                      <div className="flex items-center gap-2">
                        {r.name}
                        {r.source === 'jira' && relativeTime(r.synced_at || r.updated_at) && (
                          <Tooltip
                          ...
                          >
                            <span className="inline-flex items-center gap-0.5 text-[11px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] cursor-default">
                              <Clock size={12} />
                            </span>
                          </Tooltip>
                        )}
