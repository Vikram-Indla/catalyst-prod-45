    ...
    <div style={{ background: isDark ? '#0A0A0A' : '#FFFFFF', minHeight: '100%', padding: '24px' }}>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-[13px]" style={{ fontFamily: RH.fontBody, color: isDark ? '#666666' : '#64748B' }}>Manage and track all releases</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleImport} disabled={importing}
            ...
            style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, background: isDark ? '#111111' : '#FFFFFF', color: isDark ? '#888888' : '#475569' }}>
        ...
        {[
          ...
          <button key={s.key} onClick={() => setFilter(s.key)}
            ...
            style={filter === s.key
              ...
              : { borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.12)', background: isDark ? '#111111' : '#FFFFFF', color: isDark ? '#888888' : '#475569' }
            }>
            {s.label}
            <span className="inline-flex items-center justify-center min-w-[18px] h-[18px] rounded-full text-[10px] font-bold"
              style={{ background: isDark ? '#1A1A1A' : '#F1F5F9', color: isDark ? '#888888' : '#475569' }}>{s.count}</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="relative">
          ...
          <input type="text" placeholder="Search releases..." value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.12)'}`, background: isDark ? '#111111' : '#FFFFFF', color: isDark ? '#EDEDED' : '#0F172A' }} />
        </div>
        <div className="flex items-center gap-1 rounded-md p-0.5" style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.12)'}`, background: isDark ? '#111111' : '#FFFFFF' }}>
          <button onClick={() => setView('cards')} className="h-7 w-7 rounded flex items-center justify-center" style={view === 'cards' ? { background: isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF', color: '#2563EB' } : { color: isDark ? '#666666' : '#94A3B8' }}><LayoutGrid size={14} /></button>
          <button onClick={() => setView('table')} className="h-7 w-7 rounded flex items-center justify-center" style={view === 'table' ? { background: isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF', color: '#2563EB' } : { color: isDark ? '#666666' : '#94A3B8' }}><List size={14} /></button>
        </div>
      </div>
      ...
      {isLoading ? (
          ...
          {filtered.map((r: any) => {
            ...
            return (
              <button key={r.id} onClick={() => setSelectedRelease(r)}
                ...
                style={{ borderRadius: 6, background: isDark ? '#111111' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.12)'}` }}>
                ...
                <div className="p-4 pl-5">
                  ...
                  {r.jira_key && <span className="text-[11px] block mb-2" style={{ fontFamily: RH.fontMono, color: isDark ? '#666666' : '#94A3B8' }}>{r.jira_key}</span>}
                  ...
                  <div className="flex items-center gap-3 text-[12px] mb-3" style={{ color: isDark ? '#666666' : '#64748B' }}>
                  ...
                  {progress.empty ? (
                    <p className="text-[11px]" style={{ fontFamily: RH.fontBody, color: isDark ? '#666666' : '#94A3B8' }}>No changes yet</p>
                  ) : (
                    <>
                      <div className="w-full h-1 rounded-full overflow-hidden" style={{ background: isDark ? '#1A1A1A' : '#F1F5F9' }}>
                        <div className="h-full rounded-full transition-all" style={{
                          width: progress.pct + '%',
                          background: accentColor(r.status)
                        }} />
                      </div>
                      <p className="text-[11px] mt-1" style={{ fontFamily: RH.fontBody, color: isDark ? '#666666' : '#94A3B8' }}>
        ...
        <div className="rounded overflow-hidden" style={{ background: isDark ? '#111111' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.12)'}` }}>
            ...
            <thead>
              <tr style={{ background: isDark ? '#1A1A1A' : '#F1F5F9' }}>
                {['RELEASE', 'SOURCE', 'STATUS', 'TARGET DATE', 'CHANGES', 'PROGRESS'].map(h => (
                  <th key={h} className="px-3 py-0 h-[36px] text-left text-[11px] font-semibold uppercase tracking-[0.06em]" style={{ color: isDark ? '#666666' : '#64748B' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((r: any) => {
                ...
                return (
                  <tr key={r.id} onClick={() => setSelectedRelease(r)}
                    ...
                    style={{ height: 36, background: isDark ? '#111111' : '#FFFFFF', transition: 'background 120ms', borderBottom: `0.75px solid ${isDark ? 'rgba(255,255,255,0.05)' : 'rgba(15,23,42,0.06)'}` }}
                    ...
                    <td className="px-3 py-0" style={{ fontFamily: RH.fontMono, fontSize: 12, color: isDark ? '#888888' : '#475569' }}>
                    ...
                    <td className="px-3 py-0">
                      {progress.empty ? (
                        <span className="text-[11px]" style={{ color: isDark ? '#666666' : '#94A3B8' }}>No changes yet</span>
                      ) : (
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1 rounded-full overflow-hidden" style={{ background: isDark ? '#1A1A1A' : '#F1F5F9' }}>
                            <div className="h-full rounded-full" style={{ width: progress.pct + '%', background: accentColor(r.status) }} />
                          </div>
                          <span className="text-[11px]" style={{ fontFamily: RH.fontMono, color: isDark ? '#666666' : '#64748B' }}>{progress.pct}%</span>
                        </div>
                      )}
                    </td>
                  </tr>
