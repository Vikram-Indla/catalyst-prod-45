...
export default function SignOffQueuePage() {
  ...
  return (
    <div className="p-6" style={{ background: 'var(--cp-bg-elevated, #FFFFFF)' }}>
      <div className="mb-5">
        <h1 className="text-[24px]" style={{ fontFamily: RH.fontDisplay, fontWeight: 650, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : RH.ink1 }}>Signoff Queue</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative">
          ...
          <input type="text" placeholder="Search changes or approvers..." value={search} onChange={e => setSearch(e.target.value)}
            className="h-9 w-72 pl-9 pr-3 rounded-[4px] text-[13px] focus:outline-none focus:ring-2 focus:ring-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]/20"
            style={{ border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(15,23,42,0.12)', fontFamily: RH.fontBody, background: 'var(--cp-bg-elevated, #FFFFFF)', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined }} />
        </div>
      </div>
              ...
              {filtered.map((so: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 truncate max-w-[240px]" style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : RH.ink2 }}>{so.rh_changes?.title || '—'}</td>
                    <td className="px-3" style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : RH.ink2 }}>{so.signoff_role || so.stage || '—'}</td>
                      ...
                      {isWaiting ? (
                        <div className="flex items-center gap-1.5">
                          <button onClick={e => { e.stopPropagation(); setActionModal({ signoff: so, action: 'approve' }); }}
                            className="h-7 px-2.5 rounded-[4px] bg-[var(--ds-text-success,var(--ds-text-success, #16A34A))] text-white text-[11px] font-bold hover:bg-[#15803D]">Approve</button>
                          <button onClick={e => { e.stopPropagation(); setActionModal({ signoff: so, action: 'reject' }); }}
                            className="h-7 px-2.5 rounded-[4px] text-[var(--ds-text-danger,var(--ds-text-danger, #DC2626))] text-[11px] font-bold hover:bg-[var(--ds-background-danger,var(--ds-background-danger, #FEF2F2))]" style={{ border: '0.75px solid #FCA5A5' }}>Reject</button>
                        </div>
                      ) : isPending ? (
                        <span className="text-[11px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))]" title="Requires previous gate approval">Locked</span>
                      ) : null}
                    </td>
                  </tr>
                );
          ...
          {actionModal && (
            <div className="space-y-4">
              <div className="rounded-[6px] p-3" style={{ background: 'var(--cp-bg-sunken, #F1F5F9)' }}>
                ...
                <p className="text-[12px]" style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>Change: <span style={{ fontFamily: RH.fontMono, fontWeight: 650, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>{actionModal.signoff.rh_changes?.chg_number}</span> — {actionModal.signoff.rh_changes?.title}</p>
              ...
              <div>
                ...
                <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Add a comment..."
                  className="w-full h-24 px-3 py-2 rounded-[4px] text-[13px] focus:outline-none focus:ring-2 focus:ring-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]/20 resize-none"
                  style={{ border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(15,23,42,0.12)', background: isDark ? 'var(--cp-bg-surface, #242528)' : undefined, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined }} />
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <button onClick={handleAction} disabled={approveSignoff.isPending || rejectSignoff.isPending}
              className={`h-9 px-4 rounded-[6px] text-[13px] font-semibold text-white disabled:opacity-50 ${actionModal?.action === 'approve' ? 'bg-[var(--ds-text-success,var(--ds-text-success, #16A34A))] hover:bg-[#15803D]' : 'bg-[var(--ds-text-danger,var(--ds-text-danger, #DC2626))] hover:bg-[#B91C1C]'}`}>
