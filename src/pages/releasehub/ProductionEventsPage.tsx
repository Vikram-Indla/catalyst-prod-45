...
const EVENT_TYPE_LOZENGE: Record<string, { bg: string; color: string; label: string }> = {
  ...
  HOTFIX: { bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', color: '#42526E', label: 'HOTFIX' },
  ROLLBACK: { bg: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))', color: 'var(--ds-text-danger, var(--ds-text-danger, #991B1B))', label: 'ROLLBACK' },
};
...
const RESULT_BADGE: Record<string, { bg: string; color: string; label: string; icon?: boolean }> = {
  ...
  ROLLED_BACK: { bg: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))', color: 'var(--ds-text-danger, var(--ds-text-danger, #991B1B))', label: 'ROLLED BACK' },
...
function getDotStyle(event: any) {
  ...
  let borderColor = 'var(--ds-text-success, var(--ds-text-success, #16A34A))';
  let size = 16;

  if (type === 'ROLLBACK' || result === 'ROLLED_BACK') {
    borderColor = 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))';
    size = 14;
  } else if (type === 'HOTFIX') {
    borderColor = 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))';
    size = 12;
  }
      ...
      <div className="flex items-center gap-2 mb-5">
        {filterChips.map(chip => (
          <button key={chip.key} onClick={() => setResultFilter(chip.key)}
            ...
            style={{
              fontWeight: 600,
              border: `0.75px solid ${resultFilter === chip.key ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : 'var(--cp-border-default, rgba(15,23,42,0.12))'}`,
              background: resultFilter === chip.key ? ('var(--cp-primary-light, #EFF6FF)') : ('var(--cp-bg-elevated, #FFFFFF)'),
              color: resultFilter === chip.key ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : ('var(--cp-text-tertiary, #64748B)'),
            }}
          >
            {chip.label}
          </button>
            ...
            return (
              <div key={ev.id} className="relative" style={{ paddingBottom: idx === filtered.length - 1 ? 0 : 24 }}>
                ...
                >
                  ...
                  {/* Notes (if any) */}
                  {ev.notes && (
                    <p style={{ fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', marginTop: 6 }}>{ev.notes}</p>
                  )}
                </div>
              </div>
            );
