...
export default function ProductionEventsPage() {
  ...
  return (
    <div style={{ background: isDark ? '#1A1714' : '#FFFFFF', minHeight: '100%', padding: 24 }}>
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p style={{ fontSize: 13, color: isDark ? '#6B6560' : '#64748B', marginTop: 2 }}>
            Post-deployment monitoring & event log
          </p>
        </div>
        <button
          className="h-9 px-4 rounded-md text-[13px] font-semibold flex items-center gap-1.5"
          style={{ border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.12)', background: isDark ? '#232019' : '#FFFFFF', color: isDark ? '#A09890' : '#475569' }}
          onClick={() => {
        ...
        {filterChips.map(chip => (
          <button key={chip.key} onClick={() => setResultFilter(chip.key)}
            ...
            style={{
              ...
              background: resultFilter === chip.key ? (isDark ? 'rgba(37,99,235,0.15)' : '#EFF6FF') : (isDark ? '#232019' : '#FFFFFF'),
              color: resultFilter === chip.key ? '#2563EB' : (isDark ? '#A09890' : '#64748B'),
            }}
          >
            {chip.label}
          </button>
            ...
            return (
              <div key={ev.id} className="relative" style={{ paddingBottom: idx === filtered.length - 1 ? 0 : 24 }}>
                ...
                >
                  ...
                  {/* Meta row */}
                  <div className="flex flex-wrap" style={{ gap: 16, fontSize: 12, color: isDark ? '#6B6560' : '#64748B' }}>
