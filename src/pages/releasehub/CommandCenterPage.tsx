  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[12px] font-medium uppercase tracking-[0.04em]" style={{ fontFamily: RH.fontBody, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>{label}</p>
          {loading ? (
          ...
          {delta && !loading && (
            <p className="text-[12px] mt-1" style={{ fontWeight: 600, color: deltaLabel === 'neutral' ? ('var(--cp-text-tertiary, var(--cp-ink-3, #64748B))') : delta.startsWith('+') ? 'var(--ds-text-danger, var(--cp-danger, #DC2626))' : 'var(--ds-text-success, var(--cp-success, #16A34A))' }}>
              {delta}
            </p>
          )}
        </div>
...
export default function CommandCenterPage() {
  ...
  // Pipeline columns
  const pipelineCols = [
    { key: 'new', label: 'NEW', loz: { bg: 'var(--cp-border, var(--cp-lozenge-grey-bg, #DFE1E6))', text: 'var(--cp-text-secondary, #253858)' } },
    ...
    <div className="p-6" style={{ background: 'var(--cp-bg-elevated, #FFFFFF)' }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[13px] mt-1" style={{ fontFamily: RH.fontBody, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>Release operations overview — real-time</p>
        </div>
      </div>
          ...
          {latestDeployed ? (
            <div className="cursor-pointer" onClick={() => setSelectedChange(latestDeployed)}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><p className="text-[11px] uppercase mb-0.5" style={{ color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>Release</p><p className="text-[12px]" style={{ color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : RH.ink2 }}>{latestDeployed.release_name || '—'}</p></div>
                <div><p className="text-[11px] uppercase mb-0.5" style={{ color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>Deployed</p><p className="text-[12px]" style={{ color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : RH.ink2 }}>{latestDeployed.deployment_date ? format(new Date(latestDeployed.deployment_date), 'MMM d, yyyy') : '—'}</p></div>
              </div>
            </div>
          ) : (
              ...
              <React.Fragment key={col.key}>
                {i > 0 && (
                  <svg width="16" height="24" viewBox="0 0 16 24" className="shrink-0 -mx-1">
                    <path d="M0 0 L12 12 L0 24" fill="none" stroke="var(--ds-text-disabled, #CBD5E1)" strokeWidth="1.5" />
                  </svg>
                )}
                <button
        ...
        <div className="rounded-[6px] p-5" style={{ background: 'var(--cp-bg-elevated, #FFFFFF)', border: `0.75px solid ${'var(--cp-border-default, rgba(15,23,42,0.12))'}` }}>
          ...
          {!coreDataReady ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[1,2,3,4].map(i => (
                  <div key={i} className="rounded-[6px] p-3" style={{ border: `0.75px solid ${'var(--cp-border, var(--cp-lozenge-grey-bg, #DFE1E6))'}`, borderLeft: `4px solid ${'var(--cp-border, var(--cp-lozenge-grey-bg, #DFE1E6))'}` }}>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                {computed.gates.map(gate => {
                  ...
                  const borderColor = gate.noData ? 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))' : pass ? 'var(--ds-text-success, var(--cp-success, #16A34A))' : 'var(--ds-text-danger, var(--cp-danger, #DC2626))';
                  const GateIcon = gate.noData ? Minus : pass ? CheckCircle2 : XCircle;
                  const iconColor = gate.noData ? 'var(--ds-text-subtlest, #94A3B8)' : pass ? 'var(--ds-text-success, var(--cp-success, #16A34A))' : 'var(--ds-text-danger, var(--cp-danger, #DC2626))';
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[11px]" style={{ color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>{gate.noData ? 'No data yet' : gate.detail}</p>
                    </div>
                  );
                })}
              </div>
              ...
              {prodEvents.slice(0, 4).map((ev: any) => (
                <div key={ev.id} className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{
                    border: `2px solid ${ev.deployment_result === 'SUCCESS' ? 'var(--ds-text-success, var(--cp-success, #16A34A))' : ev.deployment_result === 'ROLLED_BACK' ? 'var(--ds-text-danger, var(--cp-danger, #DC2626))' : 'var(--ds-text-subtlest, #94A3B8)'}`,
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px]" style={{ color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>{ev.deployed_at ? format(new Date(ev.deployed_at), 'MMM d, HH:mm') : '—'} · {ev.deployed_by}</p>
                  </div>
