...
import { useReleases, useChanges, useCommandCenterMappings, useApproveSignoff } from '@/hooks/useReleaseHub';
...
export default function CommandCenterPage() {
  ...
  // Real-time subscription on signoffs
  useEffect(() => {
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { queryClient.invalidateQueries({ queryKey: ['releasehub'] }); }}
            className="h-9 px-3 rounded-md border border-[#E2E8F0] bg-white text-[13px] font-medium text-[#475569] flex items-center gap-1.5 hover:bg-[#F8FAFC]">
              ...
              <div className="bg-white rounded-lg border border-[#E2E8F0]">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-[13px]" style={{ fontFamily: RH.fontBody }}>
                    ...
                    <tbody>
                      {mapLoading ? (
                        <tr><td colSpan={5} className="px-4 py-8 text-center text-[#94A3B8]">Loading...</td></tr>
                      ) : mappings.length === 0 ? (
                        ...
                        <tr key={i} className="border-b border-[#F1F5F9] hover:bg-[#FAFBFC]">
                          ...
                          <td className="px-4 py-2.5 text-[#94A3B8]">{m.tm_test_cycles?.pass_count ?? '—'} / {m.tm_test_cycles?.fail_count ?? '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                        ...
                        <div key={c.id} className="px-4 py-3 hover:bg-[#FAFBFC]">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => {
                                  // We need the signoff ID — for now approve based on change context
                                  toast.info('Open CHG drawer to approve specific sign-offs');
                                }}
