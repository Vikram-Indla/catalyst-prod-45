...
import { RiskLozenge } from '@/components/releasehub/shared/ReleaseOpsLozenges';
      ...
      <Section title={`Changes (${data.changes.length})`} empty="No linked changes — this release has a deployment gap.">
        {data.changes.map((c) => (
          <button key={c.id} onClick={() => navigate(`/release-hub/changes/${c.slug ?? c.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', textAlign: 'left', background: T.card, border: `1px solid ${T.border}`, borderRadius: 6, padding: '4px 10px', marginBottom: 6, cursor: 'pointer' }}>
            ...
            {c.isEmergency && <Lozenge appearance="moved">Emergency</Lozenge>}
            {c.risk && <RiskLozenge risk={c.risk} />}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 8px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {r.productName ? <span style={{ fontFamily: RH.fontBody, fontSize: 'var(--ds-font-size-100)', color: T.subtle }}>{r.productName}</span> : <Lozenge appearance="moved">No product</Lozenge>}
            {r.targetEnv && <span style={{ fontFamily: RH.fontBody, fontSize: 'var(--ds-font-size-50)', color: T.subtle }}>{r.targetEnv}</span>}
            {r.hasEmergency && <Lozenge appearance="moved">Emergency</Lozenge>}
            {r.freezeConflict && <Lozenge appearance="removed">Freeze</Lozenge>}
            {r.isEmptyScope && <Lozenge appearance="moved">Empty scope</Lozenge>}
            {r.hasDeploymentGap && <Lozenge appearance="removed">No change</Lozenge>}
          </div>
  ...
  return (
    <div style={{ padding: 24, background: T.surface, minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', flexWrap: 'wrap' }}>
        <div role="heading" aria-level={1} style={{ fontFamily: RH.fontDisplay, fontSize: 'var(--ds-font-size-500)', fontWeight: 600, color: T.text, margin: 0 }}>Release timeline</div>
        ...
        <div style={{ position: 'relative', marginLeft: 'auto' }}>
          <Search size={14} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: T.subtlest }} />
