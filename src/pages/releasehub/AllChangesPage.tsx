  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="h-9 px-3 rounded-md text-[13px] font-medium flex items-center gap-1.5 min-w-[140px]"
        style={{ border: `1px solid ${isDark ? '#2E2E2E' : 'rgba(15,23,42,0.12)'}`, background: isDark ? '#1A1A1A' : '#FFFFFF', color: isDark ? '#A1A1A1' : '#475569' }}>
      ...
      {open && (
        <>
          ...
          <div className="absolute top-full left-0 mt-1 w-48 rounded-md shadow-lg z-50 py-1 max-h-60 overflow-y-auto"
            style={{ background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? '#2E2E2E' : 'rgba(15,23,42,0.12)'}` }}>
            {options.map(o => (
