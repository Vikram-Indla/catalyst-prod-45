  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="h-9 px-3 rounded-md text-[13px] font-medium flex items-center gap-1.5 min-w-[140px]"
        style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.12)'}`, background: isDark ? '#111111' : '#FFFFFF', color: isDark ? '#888888' : '#475569' }}>
          ...
          <div className="absolute top-full left-0 mt-1 w-48 rounded-md shadow-lg z-50 py-1 max-h-60 overflow-y-auto"
            ...
            {options.map(o => (
              <button key={o.value} onClick={() => { onChange(o.value); setOpen(false); }}
                ...
                style={{ color: value === o.value ? '#2563EB' : (isDark ? '#888888' : '#475569') }}
                onMouseEnter={e => (e.currentTarget.style.background = isDark ? '#222222' : '#F8FAFC')}
    ...
    <div style={{ background: isDark ? '#0A0A0A' : '#FFFFFF', minHeight: '100%', padding: '24px' }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-[13px]" style={{ fontFamily: RH.fontBody, color: isDark ? '#666666' : '#64748B' }}>Every deployment change — past, present & future</p>
        </div>
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1 rounded-md p-0.5" style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.12)'}`, background: isDark ? '#111111' : '#FFFFFF' }}>
          <button onClick={() => setParam('view', 'list')}
            ...
            style={view === 'list' ? { background: isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF', color: '#2563EB' } : { color: isDark ? '#666666' : '#94A3B8' }}>
            <List size={12} /> List
          </button>
          <button onClick={() => setParam('view', 'kanban')}
            className="h-7 px-2.5 rounded flex items-center gap-1 text-[11px] font-medium"
            style={view === 'kanban' ? { background: isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF', color: '#2563EB' } : { color: isDark ? '#666666' : '#94A3B8' }}>
              ...
              {filtered.map((c: any) => {
                ...
                return (
                  <tr key={c.id} onClick={() => setSelectedChg(c)}
                    ...
                    <td className="px-3 py-0">
                      {relName ? <span className="text-[12px] font-medium text-[#2563EB]">{relName}</span> : <span style={{ color: isDark ? '#666666' : '#94A3B8' }}>—</span>}
                    </td>
                    <td className="px-3 py-0"><SourceBadge source={c.source} /></td>
                    <td className="px-3 py-0">
                      <span className="text-[12px]" style={{ fontFamily: RH.fontMono, color: isDark ? '#666666' : '#64748B' }}>
  ...
  return (
    <div className="grid grid-cols-4 gap-3">
      {columns.map(col => {
        ...
        return (
          <div key={col.key} className="rounded-lg" style={{ background: isDark ? '#111111' : '#F1F5F9', minHeight: 200 }}>
            <div className="px-3 py-2 flex items-center gap-2">
              <span className="text-[11px] font-bold uppercase tracking-[0.06em]" style={{ color: isDark ? '#666666' : '#64748B' }}>{col.label}</span>
              <span className="text-[10px] font-bold rounded-full px-1.5" style={{ color: isDark ? '#666666' : '#94A3B8', background: isDark ? '#1A1A1A' : '#FFFFFF' }}>{items.length}</span>
            </div>
