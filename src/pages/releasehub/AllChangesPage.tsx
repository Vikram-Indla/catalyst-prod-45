      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1 rounded-md p-0.5" style={{ border: `1px solid ${isDark ? '#2E2E2E' : 'rgba(15,23,42,0.12)'}`, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF' }}>
          ...
          <button onClick={() => setParam('view', 'kanban')}
            className="h-7 px-2.5 rounded flex items-center gap-1 text-[11px] font-medium"
            style={view === 'kanban' ? { background: isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF', color: '#2563EB' } : { color: 'var(--cp-text-muted, #94A3B8)' }}>
  ...
  return (
    <div className="grid grid-cols-4 gap-3">
      {columns.map(col => {
        ...
        return (
          <div key={col.key} className="rounded-lg" style={{ background: isDark ? 'var(--cp-bg-surface, #242528)' : '#F1F5F9', minHeight: 200 }}>
            <div className="px-3 py-2 flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold rounded-full px-1.5" style={{ color: 'var(--cp-text-muted, #94A3B8)', background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF' }}>{items.length}</span>
            </div>
