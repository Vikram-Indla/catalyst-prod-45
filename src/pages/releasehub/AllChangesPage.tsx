          ...
          <div className="absolute top-full left-0 mt-1 w-48 rounded-md shadow-lg z-50 py-1 max-h-60 overflow-y-auto"
            ...
            {options.map(o => (
              <button key={o.value} onClick={() => { onChange(o.value); setOpen(false); }}
                ...
                style={{ color: value === o.value ? 'var(--ds-text-brand, #2563EB)' : ('var(--cp-text-secondary, #475569)') }}
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input type="text" placeholder="Search changes..." value={search} onChange={e => setSearch(e.target.value)}
              ...
              style={{ border: `1px solid ${'var(--cp-border-default, rgba(15,23,42,0.12))'}`, background: 'var(--cp-bg-elevated, #FFFFFF)', color: 'var(--cp-text-primary, var(--cp-ink-1, #0F172A))' }} />
          </div>
        ...
        <div className="flex items-center gap-1 rounded-md p-0.5" style={{ border: `1px solid ${'var(--cp-border-default, rgba(15,23,42,0.12))'}`, background: 'var(--cp-bg-elevated, #FFFFFF)' }}>
          <button onClick={() => setParam('view', 'list')}
            ...
            style={view === 'list' ? { background: 'var(--cp-primary-light, #EFF6FF)', color: 'var(--ds-text-brand, #2563EB)' } : { color: 'var(--cp-text-muted, var(--cp-ink-4, #94A3B8))' }}>
            <List size={12} /> List
          </button>
          <button onClick={() => setParam('view', 'kanban')}
            className="h-7 px-2.5 rounded flex items-center gap-1 text-[11px] font-medium"
            style={view === 'kanban' ? { background: 'var(--cp-primary-light, #EFF6FF)', color: 'var(--ds-text-brand, #2563EB)' } : { color: 'var(--cp-text-muted, var(--cp-ink-4, #94A3B8))' }}>
                ...
                return (
                  <tr key={c.id} onClick={() => setSelectedChg(c)}
                    ...
                    <td className="px-3 py-0 max-w-[300px]">
                      <span className="text-[13px] font-medium truncate block" style={{ color: 'var(--cp-text-primary, var(--cp-ink-1, #0F172A))' }}>{c.title}</span>
                    </td>
                    <td className="px-3 py-0"><StatusLozenge status={c.status} /></td>
                    <td className="px-3 py-0"><RiskBadge risk={mapRisk(c.risk_level)} /></td>
                    <td className="px-3 py-0">
                      {relName ? <span className="text-[12px] font-medium text-[var(--ds-text-brand,#2563EB)]">{relName}</span> : <span style={{ color: 'var(--cp-text-muted, var(--cp-ink-4, #94A3B8))' }}>—</span>}
                    </td>
        ...
        return (
          <div key={col.key} className="rounded-lg" style={{ background: 'var(--cp-bg-sunken, #F1F5F9)', minHeight: 200 }}>
            <div className="px-3 py-2 flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold rounded-full px-1.5" style={{ color: 'var(--cp-text-muted, var(--cp-ink-4, #94A3B8))', background: 'var(--cp-bg-elevated, #FFFFFF)' }}>{items.length}</span>
            </div>
            <div className="px-2 pb-2 space-y-2">
              {items.map((c: any) => (
                <button key={c.id} onClick={() => onSelect(c)}
                  ...
                  <span className="text-[13px] font-medium block truncate" style={{ color: 'var(--cp-text-primary, var(--cp-ink-1, #0F172A))' }}>{c.title}</span>
