  ...
  return (
    <div style={{ background: isDark ? '#0A0A0A' : '#FFFFFF', minHeight: '100%', padding: '24px' }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-[22px]" style={{ fontFamily: RH.fontDisplay, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', fontWeight: 650 }}>Freeze Windows</h1>
          <p className="text-[13px]" style={{ fontFamily: RH.fontBody, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
      ...
      {/* Calendar Strip */}
      <div className="mb-6 rounded-lg p-4" style={{ background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}` }}>
        <div className="flex items-center justify-between mb-3">
          <button onClick={() => setCalMonth(new Date(calMonth.getFullYear(), calMonth.getMonth() - 1))}
            className="h-7 w-7 rounded flex items-center justify-center" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
            <ChevronLeft size={16} />
          </button>
          <span className="text-[14px] font-semibold" style={{ fontFamily: RH.fontDisplay, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{monthLabel}</span>
          <button onClick={() => setCalMonth(new Date(calMonth.getFullYear(), calMonth.getMonth() + 1))}
            className="h-7 w-7 rounded flex items-center justify-center" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
        ...
        <div className="grid grid-cols-7 gap-0.5">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
            <div key={d} className="text-center text-[10px] font-semibold uppercase tracking-[0.06em] pb-1" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>{d}</div>
          ))}
              ...
              <TooltipProvider key={i}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    `} style={{
                      color: !cd.inMonth ? (isDark ? '#292929' : 'rgba(237,237,237,0.53)') : (isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)'),
                      ...(fw ? { background: isDark ? 'rgba(217,119,6,0.15)' : 'rgba(251,191,36,0.10)', border: `1px solid ${isDark ? 'rgba(217,119,6,0.3)' : '#FCD34D'}` } : {}),
                    }}>
                      {cd.date.getDate()}
                    </div>
                  </TooltipTrigger>
      ...
      {isLoading ? (
        <div className="space-y-1">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-9 rounded animate-pulse" style={{ background: isDark ? '#1A1A1A' : '#1A1A1A' }} />
          ))}
        </div>
      ) : windows.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: isDark ? '#1A1A1A' : '#1A1A1A' }}>
            <CalendarOff className="w-8 h-8" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }} />
          </div>
          <h3 className="font-semibold text-lg mb-1" style={{ color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>No freeze windows defined</h3>
          <p className="text-[13px] max-w-md" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>Add one to protect critical periods.</p>
        </div>
      ) : (
        <div className="rounded-lg overflow-hidden" style={{ background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}` }}>
            ...
            <thead>
              <tr style={{ background: isDark ? '#1A1A1A' : '#1A1A1A' }}>
                {['NAME', 'START DATE', 'END DATE', 'DURATION', 'REASON', 'ACTIONS'].map(h => (
                  <th key={h} className="px-3 py-0 h-9 text-left text-[11px] font-semibold uppercase tracking-[0.06em]" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {windows.map((fw: any) => (
                <tr key={fw.id} className="group"
                  style={{ height: 50, transition: 'background 120ms', borderBottom: `0.75px solid ${isDark ? 'rgba(255,255,255,0.05)' : 'rgba(15,23,42,0.06)'}` }}
                  ...
                  <td className="px-3 py-0 font-medium" style={{ color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', fontWeight: 650 }}>{fw.name}</td>
                  ...
                  <td className="px-3 py-0 max-w-[240px] truncate" style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }} title={fw.reason || ''}>{fw.reason ? (fw.reason.length > 40 ? fw.reason.slice(0, 40) + '…' : fw.reason) : '—'}</td>
                  <td className="px-3 py-0">
                    <button onClick={() => handleDelete(fw.id, fw.name)}
                      className="opacity-0 group-hover:opacity-100 transition-opacity h-7 w-7 rounded flex items-center justify-center hover:bg-[rgba(248,113,113,0.10)] text-[rgba(237,237,237,0.40)] hover:text-[#DC2626]">
                      <Trash2 size={14} />
                    </button>
                  </td>
                </tr>
        ...
        <DialogContent className="sm:max-w-[440px]">
          ...
          <div className="space-y-3 py-2">
            <div>
              ...
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                ...
                className="h-9 w-full px-3 rounded border border-[rgba(255,255,255,0.10)] dark:border-[rgba(255,255,255,0.08)] bg-white dark:bg-[#1A1A1A] dark:text-[#EDEDED] text-[13px] placeholder:text-[rgba(237,237,237,0.40)] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[12px] font-semibold text-[#475569] block mb-1">Start Date *</label>
                <input type="date" value={form.start_date} onChange={e => setForm(f => ({ ...f, start_date: e.target.value }))}
                  className="h-9 w-full px-3 rounded border border-[rgba(255,255,255,0.10)] dark:border-[rgba(255,255,255,0.08)] bg-white dark:bg-[#1A1A1A] dark:text-[#EDEDED] text-[13px] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                ...
                className="w-full px-3 py-2 rounded border border-[rgba(255,255,255,0.10)] dark:border-[rgba(255,255,255,0.08)] bg-white dark:bg-[#1A1A1A] dark:text-[#EDEDED] text-[13px] placeholder:text-[rgba(237,237,237,0.40)] resize-none focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]" />
            </div>
          ...
          <DialogFooter>
            <button onClick={() => { setShowModal(false); setForm({ name: '', start_date: '', end_date: '', reason: '' }); setFormError(''); }}
              className="h-9 px-4 rounded-md text-[13px] font-medium text-[#475569] hover:bg-[#1A1A1A] transition-colors">
              Cancel
            </button>
