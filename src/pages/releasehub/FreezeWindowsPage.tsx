  ...
  return (
    <div style={{ background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', minHeight: '100%', padding: '24px' }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-[22px]" style={{ fontFamily: RH.fontDisplay, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', fontWeight: 650 }}>Freeze Windows</h1>
      ...
      <div className="mb-6 rounded-lg p-4" style={{ background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}` }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[14px] font-semibold" style={{ fontFamily: RH.fontDisplay, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))' }}>{monthLabel}</span>
          <button onClick={() => setCalMonth(new Date(calMonth.getFullYear(), calMonth.getMonth() + 1))}
            className="h-7 w-7 rounded flex items-center justify-center" style={{ color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))' }}>
        ...
        <div className="grid grid-cols-7 gap-0.5">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
            <div key={d} className="text-center text-[10px] font-semibold uppercase tracking-[0.06em] pb-1" style={{ color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))' }}>{d}</div>
          ))}
      ...
      {isLoading ? (
        <div className="space-y-1">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-9 rounded animate-pulse" style={{ background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))' }} />
          ))}
        </div>
      ) : windows.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))' }}>
            <CalendarOff className="w-8 h-8" style={{ color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))' }} />
          </div>
          <h3 className="font-semibold text-lg mb-1" style={{ color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))' }}>No freeze windows defined</h3>
            ...
            <tbody>
              {windows.map((fw: any) => (
                <tr key={fw.id} className="group"
                  ...
                  <td className="px-3 py-0 font-medium" style={{ color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', fontWeight: 650 }}>{fw.name}</td>
                  ...
                  <td className="px-3 py-0">
                    <button onClick={() => handleDelete(fw.id, fw.name)}
                      className="opacity-0 group-hover:opacity-100 transition-opacity h-7 w-7 rounded flex items-center justify-center hover:bg-[#FEE2E2] text-[var(--ds-text-subtlest,var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))] hover:text-[var(--ds-text-danger,var(--cp-danger, #DC2626))]">
                      <Trash2 size={14} />
                    </button>
                  </td>
                </tr>
        ...
        <DialogContent className="sm:max-w-[440px]">
          ...
          <div className="space-y-3 py-2">
            ...
            <div>
              ...
              <textarea value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                ...
                className="w-full px-3 py-2 rounded border border-[var(--ds-border,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] dark:text-[var(--ds-text,var(--cp-bg-neutral, #EDEDED))] text-[13px] placeholder:text-[var(--ds-text-subtlest,var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))] resize-none focus:outline-none focus:ring-2 focus:ring-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563EB))]/20 focus:border-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563EB))]" />
            </div>
          ...
          <DialogFooter>
            <button onClick={() => { setShowModal(false); setForm({ name: '', start_date: '', end_date: '', reason: '' }); setFormError(''); }}
              className="h-9 px-4 rounded-md text-[13px] font-medium text-[var(--ds-text-subtle,#475569)] hover:bg-[var(--ds-surface-sunken,var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))] transition-colors">
              Cancel
            </button>
