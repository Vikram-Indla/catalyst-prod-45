...
export default function FreezeWindowsPage() {
  ...
  return (
    <div style={{ background: isDark ? 'var(--cp-bg-page, #1F1F21)' : '#FFFFFF', minHeight: '100%', padding: '24px' }}>
      ...
      {/* Calendar Strip */}
      <div className="mb-6 rounded-lg p-4" style={{ background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: `1px solid ${'var(--cp-border, #E2E8F0)'}` }}>
          ...
          {calendarDays.map((cd, i) => {
            ...
            return (
              <Tooltip key={i} position="top" content={fw ? (fw as any).name : null}>
                ...
                `} style={{
                  color: !cd.inMonth ? (isDark ? '#292929' : '#CBD5E1') : ('var(--cp-text-secondary, #334155)'),
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg overflow-hidden" style={{ background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: `1px solid ${'var(--cp-border, #E2E8F0)'}` }}>
