  ...
  return (
    <div style={{ background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', minHeight: '100%', padding: '24px' }}>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] rounded border border-[var(--ds-shadow-overlay, rgba(15,23,42,0.12))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] p-10 flex flex-col items-center gap-2">
          <CheckCircle size={24} className="text-[var(--ds-background-success-bold, #1F845A)]" />
          <span className="text-[14px] font-bold text-[var(--ds-background-success-bold, #1F845A)]" style={{ fontFamily: RH.fontDisplay }}>No unlinked work items — pipeline is clean ✓</span>
        </div>
      ) : (
        <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] rounded border border-[var(--ds-shadow-overlay, rgba(15,23,42,0.12))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] overflow-hidden">
          <table className="w-full text-[13px]" style={{ fontFamily: RH.fontBody }} role="table">
            <thead>
              <tr style={{ background: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))' }}>
            ...
            <tbody>
              {unlinked.map((c: any) => {
                ...
                return (
                  <tr key={c.id} className="border-b border-[var(--ds-shadow-overlay, rgba(15,23,42,0.06))]" style={{ background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))' }}>
                    ...
                    <td className="px-3 py-2 max-w-[220px]">
                      <span className="text-[13px] text-[var(--ds-text,var(--cp-ink-1, var(--cp-ink-1, #0F172A)))] truncate block">{c.title}</span>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      {rec ? (
                        <div className="bg-[var(--ds-background-selected,#EFF6FF)] border border-[var(--ds-background-information, #E9F2FF)] rounded-md px-2.5 py-2 flex items-start gap-2 max-w-[260px]">
                          ...
                          <div className="text-[12px] leading-relaxed">
                            ...
                            <span className="text-[var(--ds-text,var(--cp-ink-1, var(--cp-ink-1, #0F172A)))]">{rec.releaseName}</span>
                            <br />
                            <span className="text-[var(--ds-text-subtlest,var(--cp-ink-3, var(--cp-text-secondary, #64748B)))]">{rec.reason}</span>
                          </div>
                        </div>
                      ) : (
                        <span className="text-[var(--ds-text-subtlest,var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))] text-[12px]">No suggestion</span>
                      )}
                    </td>
                    <td className="px-3 py-2">
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)}
        className="h-7 px-3 rounded text-[11px] font-semibold border border-[var(--ds-shadow-overlay, rgba(15,23,42,0.12))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563EB))] flex items-center gap-1">
        Link Release <ChevronDown size={10} />
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
          <div className="absolute top-full right-0 mt-1 w-48 bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] rounded-md shadow-lg border border-[var(--ds-shadow-overlay, rgba(15,23,42,0.12))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] z-50 py-1 max-h-48 overflow-y-auto">
            {releases.length === 0 ? (
              <div className="px-3 py-2 text-[12px] text-[var(--ds-text-subtlest,var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))]">No releases available</div>
            ) : releases.map((r: any) => (
