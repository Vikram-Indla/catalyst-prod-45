...
const RADAR_COLORS = ['var(--ds-text-brand, var(--ds-text-brand, #2563EB))', '#0D9488', 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))'];
...
export default function ReleaseComparePage() {
  ...
  const comparisonRows: { label: string; values: any[]; render: (v: any, r?: any) => React.ReactNode; bestIdx: number }[] = useMemo(() => {
    ...
    return [
      ...
      { label: 'Days Remaining', values: selectedReleases.map((r: any) => r.days_remaining ?? 0), render: (v: any, r: any) => r?.is_overdue ? <span className="text-[var(--ds-text-danger,var(--ds-text-danger, #DC2626))] font-bold">{Math.abs(v)}d OVERDUE</span> : <span>{v}d</span>, bestIdx: getBestIdx(selectedReleases.map((r: any) => r.days_remaining ?? 0), true) },
      ...
      { label: 'Sign-offs Pending', values: selectedReleases.map((r: any) => r.pending_signoffs || 0), render: (v: any) => <span className={v > 0 ? 'text-[var(--ds-text-danger,var(--ds-text-danger, #DC2626))] font-bold' : ''}>{v}</span>, bestIdx: getBestIdx(selectedReleases.map((r: any) => r.pending_signoffs || 0), false) },
    ];
  }, [selectedReleases]);
  ...
  return (
    <div className="rh-page">
      <div className="mb-5">
        ...
        <p className="text-[13px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748B))]" style={{ fontFamily: RH.fontBody }}>Side-by-side release comparison (select up to 4)</p>
      </div>
        ...
        <>
            ...
            {releases.slice(0, 8).map((r: any) => {
              ...
              return (
                <button key={r.id} onClick={() => toggleSelect(r.id)}
                  className={`bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] rounded-lg border p-3 text-left transition-all ${isSelected ? 'border-[#0D9488] border-2 ring-2 ring-[#0D9488]/20' : 'border-[var(--bd-default, #E2E8F0)] dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] hover:border-[#C9D3E0]'}`}
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    {r.version && <span className="text-[10px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748B))]" style={{ fontFamily: RH.fontMono }}>{r.version}</span>}
                  </div>
                </button>
              );
            })}
          ...
          {selectedReleases.length >= 2 ? (
            <div className="space-y-4">
              <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] rounded-lg border border-[var(--bd-default, #E2E8F0)] dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] overflow-hidden">
                <table className="w-full text-[13px]" style={{ fontFamily: RH.fontBody }} role="table">
                  <thead>
                    <tr className="bg-[#F4F7FA] border-b border-[var(--bd-default, #E2E8F0)]">
                      <th scope="col" className="px-3 py-0 h-9 text-left text-[11px] font-extrabold uppercase tracking-[0.04em] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))] w-[160px]">METRIC</th>
                      {selectedReleases.map((r: any) => (
                        <th key={r.id} scope="col" className="px-3 py-0 h-9 text-left text-[11px] font-extrabold uppercase tracking-[0.04em] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">{r.name}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {comparisonRows.map(row => (
                      <tr key={row.label} className="border-b border-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F1F5F9))] h-9">
                        <td className="px-3 py-0 font-semibold text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748B))]">{row.label}</td>
              ...
              {radarData.length > 0 && (
                <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] rounded-lg border border-[var(--bd-default, #E2E8F0)] dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] p-4">
                  ...
                  <ResponsiveContainer width="100%" height={320}>
                    <RadarChart data={radarData}>
                      ...
                      <PolarAngleAxis dataKey="subject" tick={{ fontSize: 11, fill: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))' }} />
            ...
            <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] rounded-lg border border-[var(--bd-default, #E2E8F0)] dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] p-8 text-center text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] text-[13px]">
