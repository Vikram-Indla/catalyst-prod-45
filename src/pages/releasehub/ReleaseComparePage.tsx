      ...
      {isLoading ? (
        ...
        <>
            ...
            {releases.slice(0, 8).map((r: any) => {
              ...
              return (
                <button key={r.id} onClick={() => toggleSelect(r.id)}
                  className={`bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-lg border p-3 text-left transition-all ${isSelected ? 'border-[var(--cp-teal-60, #0D9488)] border-2 ring-2 ring-[var(--cp-teal-60, #0D9488)]/20' : 'border-[var(--bd-default,var(--cp-border, #E2E8F0))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] hover:border-[#C9D3E0]'}`}
          ...
          {selectedReleases.length >= 2 ? (
            <div className="space-y-4">
              <div className="bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-lg border border-[var(--bd-default,var(--cp-border, #E2E8F0))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] overflow-hidden">
                <table className="w-full text-[13px]" style={{ fontFamily: RH.fontBody }} role="table">
                  <thead>
                    <tr className="bg-[#F4F7FA] border-b border-[var(--bd-default,var(--cp-border, #E2E8F0))]">
              ...
              {radarData.length > 0 && (
                <div className="bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-lg border border-[var(--bd-default,var(--cp-border, #E2E8F0))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] p-4">
                  <h3 className="text-[14px] font-bold mb-3" style={{ fontFamily: RH.fontDisplay, color: RH.ink1 }}>Radar Comparison</h3>
                  <ResponsiveContainer width="100%" height={320}>
                    <RadarChart data={radarData}>
                      <PolarGrid stroke="var(--bd-default, var(--cp-border, #E2E8F0))" />
            ...
            <div className="bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-lg border border-[var(--bd-default,var(--cp-border, #E2E8F0))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] p-8 text-center text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))] text-[13px]">
