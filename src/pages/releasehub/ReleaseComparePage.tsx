        ...
        <>
            ...
            {releases.slice(0, 8).map((r: any) => {
              ...
              return (
                <button key={r.id} onClick={() => toggleSelect(r.id)}
                  className={`bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-lg border p-3 text-left transition-all ${isSelected ? 'border-[var(--cp-teal-60, #0D9488)] border-2 ring-2 ring-[var(--cp-teal-60, #0D9488)]/20' : 'border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] hover:border-[#C9D3E0]'}`}
          ...
          {selectedReleases.length >= 2 ? (
            <div className="space-y-4">
              ...
              {radarData.length > 0 && (
                <div className="bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-lg border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] p-4">
