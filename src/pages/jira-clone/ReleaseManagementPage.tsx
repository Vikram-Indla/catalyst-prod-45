    ...
    <div style={{ padding: '24px', maxWidth: '1400px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', alignItems: 'center', flexWrap: 'wrap' }}>
        <input
          ...
          style={{ width: '180px', padding: '6px 12px', border: '1px solid var(--ds-border, #DFE1E6)', borderRadius: '3px', fontSize: 'var(--ds-font-size-400)' }}
        />

        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as ReleaseStatus)}
          style={{ padding: '6px 12px', border: '1px solid var(--ds-border, #DFE1E6)', borderRadius: '3px', fontSize: 'var(--ds-font-size-400)', cursor: 'pointer' }}
        >
        ...
        <span style={{ color: 'var(--ds-text-subtlest, #626F86)', fontSize: 'var(--ds-font-size-300)' }}>This space has 47 releases</span>

        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
          <button
            onClick={() => { }}
            style={{ padding: '6px 16px', background: 'none', border: '1px solid var(--ds-border, #DFE1E6)', borderRadius: '3px', cursor: 'pointer', fontSize: 'var(--ds-font-size-400)' }}
          >
            Give feedback
          </button>
          <button
            onClick={() => setIsCreateDialogOpen(true)}
            style={{ padding: '6px 16px', background: 'var(--ds-link, #0052CC)', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: 'var(--ds-font-size-400)', fontWeight: 500 }}
          >
            Create release
          </button>
        </div>
        ...
        <tbody>
          {filteredReleases.map((release) => (
            <tr key={release.id} style={{ borderBottom: '1px solid var(--ds-background-neutral, #EBECF0)', background: 'var(--ds-surface, #FFFFFF)', hover: { background: 'var(--ds-surface-sunken, #F7F8F9)' } as any }}>
              ...
              <td style={{ padding: '12px 16px' }}>
                <span style={{ display: 'inline-block', padding: '2px 8px', background: 'var(--ds-surface, #FFFFFF)', border: '1px solid var(--ds-text-subtlest, #626F86)', borderRadius: '3px', fontSize: 'var(--ds-font-size-200)', fontWeight: 600 }}>
                  {release.status}
                </span>
              </td>
              <td style={{ padding: '12px 16px' }}>
                {release.workItemsCount === 0 ? (
                  <span style={{ fontStyle: 'italic', color: 'var(--ds-text-subtlest, #626F86)', fontSize: 'var(--ds-font-size-300)' }}>No work items</span>
                ) : (
              ...
              <td style={{ padding: '12px 16px' }}>
                {release.description === 'Release' && (
                  <button
                    ...
                    style={{ padding: '4px 12px', background: 'var(--ds-link, #0052CC)', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: 'var(--ds-font-size-200)', fontWeight: 500 }}
                  >
                    Release
                  </button>
                )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(9, 30, 66, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1001 }}> // ads-scanner:ignore-line — Atlassian elevation shadow rgba(9,30,66,*), no ds-shadow token for arbitrary alpha
          <div style={{ background: 'white', borderRadius: '3px', padding: '24px', maxWidth: '500px', width: '90%', boxShadow: '0 20px 32px rgba(9, 30, 66, 0.25)' }}> // ads-scanner:ignore-line — Atlassian elevation shadow rgba(9,30,66,*), no ds-shadow token for arbitrary alpha
            ...
            <input
              ...
              style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--ds-border, #DFE1E6)', borderRadius: '3px', fontSize: 'var(--ds-font-size-400)', marginBottom: '24px', boxSizing: 'border-box' }}
            />
          ...
          <div style={{ background: 'white', borderRadius: '3px', padding: '24px', maxWidth: '500px', width: '90%', boxShadow: '0 20px 32px rgba(9, 30, 66, 0.25)' }}> // ads-scanner:ignore-line — Atlassian elevation shadow rgba(9,30,66,*), no ds-shadow token for arbitrary alpha
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{ padding: '6px 16px', background: 'var(--ds-text-danger, #AE2A19)', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer', fontSize: 'var(--ds-font-size-400)', fontWeight: 500 }}
              >
                Release
              </button>
            </div>
