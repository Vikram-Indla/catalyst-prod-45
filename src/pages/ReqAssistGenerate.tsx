    ...
    <div style={{ background: isDark ? '#0A0A0A' : '#F8FAFC', minHeight: '100%' }}>
      ...
      <div style={{ padding: '24px 28px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <button onClick={() => navigate('/product/req-assist')} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 14, color: '#2563EB', fontWeight: 500, marginBottom: 16, padding: 0, fontFamily: 'var(--ds-font-family-body)' }}>
            <ArrowLeft size={14} /> Back to Library
          </button>
          <h2 style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 24, fontWeight: 700, color: isDark ? '#EDEDED' : '#111827', margin: '0 0 6px' }}>Generate BRD from Text</h2>
          <p style={{ fontSize: 14, color: isDark ? '#A1A1A1' : '#374151', margin: 0, lineHeight: 1.5, fontFamily: 'var(--ds-font-family-body)' }}>
        ...
        <div style={{ background: isDark ? '#1A1A1A' : '#FFFFFF', border: `0.75px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, borderRadius: 6, padding: 24, marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 650, color: isDark ? '#EDEDED' : '#111827', fontFamily: 'var(--ds-font-family-body)' }}>Requirements Input</span>
            <span style={{ fontSize: 13, color: isDark ? '#878787' : '#6B7280', fontFamily: 'var(--ds-font-family-body)' }}>· Paste text from meetings, briefs, email threads, or scope documents</span>
          </div>
          ...
          <textarea
            ...
            style={{ width: '100%', minHeight: 200, padding: 14, fontSize: 14, lineHeight: 1.65, border: `0.75px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, borderRadius: 4, outline: 'none', resize: 'vertical', fontFamily: 'var(--ds-font-family-body)', color: isDark ? '#EDEDED' : '#111827', transition: 'border-color 150ms, box-shadow 150ms' }}
          ...
          />
          <p style={{ fontSize: 12, color: isDark ? '#878787' : '#6B7280', margin: '6px 0 0', fontFamily: 'var(--ds-font-family-body)' }}>The AI will first qualify whether this text contains enough structured requirements.</p>
          ...
          {qualifyResult && !qualifyResult.qualified && (
            <div style={{ marginTop: 12, padding: '14px 16px', background: '#FEF2F2', border: '0.75px solid rgba(220,38,38,0.12)', borderRadius: 6, display: 'flex', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#DC2626', fontFamily: 'var(--ds-font-family-body)' }}>✗ Not Qualified</div>
                {qualifyResult.reasons.map((r, i) => <p key={i} style={{ fontSize: 12, color: '#991B1B', margin: '4px 0 0', lineHeight: 1.5, fontFamily: 'var(--ds-font-family-body)' }}>• {r}</p>)}
              </div>
            </div>
          )}
          ...
          {qualifyResult && qualifyResult.qualified && (
            <div style={{ marginTop: 12, padding: '14px 16px', background: '#F0FDF4', border: '0.75px solid #DCFCE7', borderRadius: 6, display: 'flex', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#16A34A', fontFamily: 'var(--ds-font-family-body)' }}>✓ Qualified — Score {qualifyResult.score}/100</div>
                {qualifyResult.reasons.map((r, i) => <p key={i} style={{ fontSize: 12, color: '#166534', margin: '4px 0 0', lineHeight: 1.5, fontFamily: 'var(--ds-font-family-body)' }}>• {r}</p>)}
                <p style={{ fontSize: 11, color: isDark ? '#878787' : '#64748B', margin: '6px 0 0', fontFamily: 'var(--ds-font-family-body)' }}>
          ...
          {genError && (
            <div style={{ marginTop: 12, padding: '14px 16px', background: '#FEF2F2', border: '0.75px solid rgba(220,38,38,0.12)', borderRadius: 6 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#DC2626', fontFamily: 'var(--ds-font-family-body)' }}>Generation Error</div>
              <p style={{ fontSize: 12, color: '#991B1B', margin: '4px 0 0', fontFamily: 'var(--ds-font-family-body)' }}>{sanitiseError(genError)}</p>
            </div>
          )}
        ...
        {generating && !genResult && (
          <div style={{ background: isDark ? '#1A1A1A' : '#FFFFFF', border: `0.75px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, borderRadius: 6, padding: '40px 24px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 500, color: isDark ? '#A1A1A1' : '#374151', margin: '0 0 4px', fontFamily: 'var(--ds-font-family-body)' }}>Generating BRD sections from your input…</p>
            <p style={{ fontSize: 12, color: isDark ? '#878787' : '#6B7280', margin: 0, fontFamily: 'var(--ds-font-family-body)' }}>This typically takes 15–30 seconds</p>
          </div>
        )}
        ...
        {genResult && genResult.sections.length > 0 && (
          <div style={{ border: `0.75px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, borderRadius: 6, overflow: 'hidden', background: isDark ? '#1A1A1A' : '#FFFFFF' }}>
            ...
            <div style={{ padding: '12px 16px', borderBottom: `0.75px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 14, fontWeight: 650, color: isDark ? '#EDEDED' : '#0F172A', fontFamily: 'var(--ds-font-family-heading)' }}>Generated BRD</span>
              ...
              <span style={{ fontSize: 11, color: isDark ? '#878787' : '#64748B', marginLeft: 'auto', fontFamily: 'var(--ds-font-family-monospaced)' }}>
            ...
            <div style={{ padding: 20 }}>
              {genResult.sections.map((section, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: isDark ? '#878787' : '#94A3B8', textTransform: 'uppercase' as const, letterSpacing: '0.06em', fontFamily: 'var(--ds-font-family-body)' }}>SECTION {section.sectionNumber}</span>
                  <h4 style={{ fontSize: 15, fontWeight: 650, color: isDark ? '#EDEDED' : '#0F172A', margin: '4px 0 8px', fontFamily: 'var(--ds-font-family-heading)' }}>{section.title}</h4>
                  <div className="ra-brd-markdown">
                    <ReactMarkdown components={{
                      p: ({ children }) => <p style={{ fontSize: 14, color: isDark ? '#A1A1A1' : '#374151', lineHeight: 1.6, marginBottom: 12, fontFamily: 'var(--ds-font-family-body)' }}>{children}</p>,
                      ...
                      li: ({ children }) => <li style={{ fontSize: 14, color: isDark ? '#A1A1A1' : '#374151', lineHeight: 1.6, marginBottom: 4, listStyleType: 'disc', fontFamily: 'var(--ds-font-family-body)' }}>{children}</li>,
                      h3: ({ children }) => <h3 style={{ fontSize: 13, fontFamily: 'var(--ds-font-family-heading)', fontWeight: 650, color: isDark ? '#878787' : '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8, marginTop: 16 }}>{children}</h3>,
                      h4: ({ children }) => <h4 style={{ fontSize: 13, fontFamily: 'var(--ds-font-family-heading)', fontWeight: 650, color: isDark ? '#878787' : '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8, marginTop: 16 }}>{children}</h4>,
            ...
            {brdState === 'generated' && (
              <div style={{ margin: '0 16px 12px', padding: '10px 14px', background: '#FEF2F2', border: '0.75px solid #FECACA', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 13, color: '#DC2626', fontFamily: 'var(--ds-font-family-body)' }}>
            ...
            {showSavedBanner && brdState === 'saved' && (
              <div style={{ margin: '0 16px 12px', padding: '10px 14px', background: '#F0FDF4', border: '0.75px solid #BBF7D0', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 13, color: '#166534', fontFamily: 'var(--ds-font-family-body)' }}>Saved to Library. Ready to generate Epics.</span>
              </div>
            )}
            ...
            <div style={{ padding: '12px 16px', background: isDark ? '#1A1A1A' : '#FFFFFF', borderTop: `0.75px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              {brdState === 'saved' && (
                <>
                  ...
                  {wikiState === 'success' && (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '8px 16px', borderRadius: 6, background: '#1B7F37', color: '#FFFFFF', fontSize: 14, fontWeight: 600, fontFamily: 'var(--ds-font-family-body)' }}>
      ...
      {dupModalOpen && duplicateDoc && (
        <ModalOverlay>
          <div style={{ width: 480, background: isDark ? '#1A1A1A' : '#FFFFFF', borderRadius: 6, padding: 24, fontFamily: 'var(--ds-font-family-body)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <AlertTriangle size={20} color="#64748B" />
              <span style={{ fontSize: 16, fontWeight: 650, color: isDark ? '#EDEDED' : '#111827', fontFamily: 'var(--ds-font-family-heading)' }}>Document Already Exists</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => setOverwriteConfirmOpen(true)} style={{
                ...
                border: '0.75px solid #DC2626', borderRadius: 6, background: 'transparent', color: '#DC2626', cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)',
              }}>Overwrite</button>
      ...
      {overwriteConfirmOpen && (
        <ModalOverlay>
          <div style={{ width: 440, background: isDark ? '#1A1A1A' : '#FFFFFF', borderRadius: 6, padding: 24, fontFamily: 'var(--ds-font-family-body)' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={async () => {
                ...
                border: 'none', borderRadius: 6, background: '#DC2626', color: '#FFFFFF', cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)',
...
function BtnOutline({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button {...props} style={{
      ...
      background: isDark ? '#1A1A1A' : '#FFFFFF', color: isDark ? '#A1A1A1' : '#374151', fontFamily: 'var(--ds-font-family-body)',
...
function BtnGhost({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button {...props} style={{
      ...
      background: 'transparent', color: isDark ? '#878787' : '#64748B', fontFamily: 'var(--ds-font-family-body)',
      ...props.style,
    }}>{children}</button>
  );
}
...
function BtnDanger({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button {...props} style={{
      ...
      background: 'transparent', color: '#DC2626', fontFamily: 'var(--ds-font-family-body)',
      ...props.style,
    }}>{children}</button>
  );
}
...
function CatalystTopNav() {
  ...
  return (
    <nav style={{ height: 48, display: 'flex', alignItems: 'center', gap: 0, background: isDark ? '#1A1A1A' : '#FFFFFF', borderBottom: `0.75px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, padding: '0 24px', fontFamily: 'var(--ds-font-family-body)' }}>
      {HUBS.map(h => {
        ...
        return (
          <button key={h.label} onClick={() => navigate(h.path)} style={{
            ...
            borderBottom: isActive ? '3px solid #2563EB' : '3px solid transparent', transition: 'color 120ms', fontFamily: 'var(--ds-font-family-body)',
          }}>{h.label}</button>
        );
      })}
    </nav>
