...
export default function ReqAssistGenerate() {
  ...
  return (
    <div style={{ background: isDark ? '#1A1714' : '#F8FAFC', minHeight: '100%' }}>
      <CatalystTopNav />
      ...
      <div style={{ padding: '24px 28px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <h2 style={{ fontFamily: "'Sora', sans-serif", fontSize: 24, fontWeight: 700, color: isDark ? '#F5F3F0' : '#111827', margin: '0 0 6px' }}>Generate BRD from Text</h2>
          <p style={{ fontSize: 14, color: isDark ? '#A09890' : '#374151', margin: 0, lineHeight: 1.5, fontFamily: "'Inter', sans-serif" }}>
        ...
        {/* INPUT CARD */}
        <div style={{ background: isDark ? '#232019' : '#FFFFFF', border: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 6, padding: 24, marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <FileText size={15} color="#7C3AED" />
            <span style={{ fontSize: 13, fontWeight: 650, color: isDark ? '#F5F3F0' : '#111827', fontFamily: "'Inter', sans-serif" }}>Requirements Input</span>
            <span style={{ fontSize: 13, color: isDark ? '#6B6560' : '#6B7280', fontFamily: "'Inter', sans-serif" }}>· Paste text from meetings, briefs, email threads, or scope documents</span>
          </div>
          ...
          <textarea
            ...
            style={{ width: '100%', minHeight: 200, padding: 14, fontSize: 14, lineHeight: 1.65, border: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 4, outline: 'none', resize: 'vertical', fontFamily: "'Inter', sans-serif", color: isDark ? '#F5F3F0' : '#111827', transition: 'border-color 150ms, box-shadow 150ms' }}
          ...
          />
          <p style={{ fontSize: 12, color: isDark ? '#6B6560' : '#6B7280', margin: '6px 0 0', fontFamily: "'Inter', sans-serif" }}>The AI will first qualify whether this text contains enough structured requirements.</p>
          ...
          {qualifyResult && qualifyResult.qualified && (
            <div style={{ marginTop: 12, padding: '14px 16px', background: '#F0FDF4', border: '0.75px solid #DCFCE7', borderRadius: 6, display: 'flex', gap: 10 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: isDark ? '#6B6560' : '#64748B', margin: '6px 0 0', fontFamily: "'Inter', sans-serif" }}>
        ...
        {generating && !genResult && (
          <div style={{ background: isDark ? '#232019' : '#FFFFFF', border: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 6, padding: '40px 24px', textAlign: 'center' }}>
            <Loader2 size={24} color="#7C3AED" style={{ animation: 'ra-spin 1s linear infinite', margin: '0 auto 12px', display: 'block' }} />
            <p style={{ fontSize: 14, fontWeight: 500, color: isDark ? '#A09890' : '#374151', margin: '0 0 4px', fontFamily: "'Inter', sans-serif" }}>Generating BRD sections from your input…</p>
            <p style={{ fontSize: 12, color: isDark ? '#6B6560' : '#6B7280', margin: 0, fontFamily: "'Inter', sans-serif" }}>This typically takes 15–30 seconds</p>
          </div>
        )}

        {/* GENERATED BRD */}
        {genResult && genResult.sections.length > 0 && (
          <div style={{ border: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 6, overflow: 'hidden', background: isDark ? '#232019' : '#FFFFFF' }}>
            {/* Header */}
            <div style={{ padding: '12px 16px', borderBottom: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 14, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A', fontFamily: "'Sora', sans-serif" }}>Generated BRD</span>
              ...
              <span style={{ fontSize: 11, color: isDark ? '#6B6560' : '#64748B', marginLeft: 'auto', fontFamily: "'JetBrains Mono', monospace" }}>
            ...
            <div style={{ padding: 20 }}>
              {genResult.sections.map((section, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: isDark ? '#6B6560' : '#94A3B8', textTransform: 'uppercase' as const, letterSpacing: '0.06em', fontFamily: "'Inter', sans-serif" }}>SECTION {section.sectionNumber}</span>
                  <h4 style={{ fontSize: 15, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A', margin: '4px 0 8px', fontFamily: "'Sora', sans-serif" }}>{section.title}</h4>
                  <div className="ra-brd-markdown">
                    <ReactMarkdown components={{
                      p: ({ children }) => <p style={{ fontSize: 14, color: isDark ? '#A09890' : '#374151', lineHeight: 1.6, marginBottom: 12, fontFamily: "'Inter', sans-serif" }}>{children}</p>,
                      strong: ({ children }) => <strong style={{ fontWeight: 650, color: isDark ? '#F5F3F0' : '#111827' }}>{children}</strong>,
                      ul: ({ children }) => <ul style={{ paddingLeft: 20, marginBottom: 12 }}>{children}</ul>,
                      ol: ({ children }) => <ol style={{ paddingLeft: 20, marginBottom: 12 }}>{children}</ol>,
                      li: ({ children }) => <li style={{ fontSize: 14, color: isDark ? '#A09890' : '#374151', lineHeight: 1.6, marginBottom: 4, listStyleType: 'disc', fontFamily: "'Inter', sans-serif" }}>{children}</li>,
                      h3: ({ children }) => <h3 style={{ fontSize: 13, fontFamily: "'Sora', sans-serif", fontWeight: 650, color: isDark ? '#6B6560' : '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8, marginTop: 16 }}>{children}</h3>,
                      h4: ({ children }) => <h4 style={{ fontSize: 13, fontFamily: "'Sora', sans-serif", fontWeight: 650, color: isDark ? '#6B6560' : '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8, marginTop: 16 }}>{children}</h4>,
            ...
            {/* ACTION BAR */}
            <div style={{ padding: '12px 16px', background: isDark ? '#232019' : '#FFFFFF', borderTop: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, display: 'flex', gap: 8, alignItems: 'center' }}>
              {brdState === 'generated' && (
                <>
      ...
      {dupModalOpen && duplicateDoc && (
        <ModalOverlay>
          <div style={{ width: 480, background: isDark ? '#232019' : '#FFFFFF', borderRadius: 6, padding: 24, fontFamily: "'Inter', sans-serif" }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <AlertTriangle size={20} color="#64748B" />
              <span style={{ fontSize: 16, fontWeight: 650, color: isDark ? '#F5F3F0' : '#111827', fontFamily: "'Sora', sans-serif" }}>Document Already Exists</span>
            </div>
            <p style={{ fontSize: 14, color: isDark ? '#A09890' : '#374151', lineHeight: 1.6, margin: '0 0 20px' }}>
              A BRD with identical content already exists in your library: <strong style={{ fontWeight: 650, color: isDark ? '#F5F3F0' : '#111827' }}>"{duplicateDoc.title}"</strong>. What would you like to do?
            </p>
      ...
      {overwriteConfirmOpen && (
        <ModalOverlay>
          <div style={{ width: 440, background: isDark ? '#232019' : '#FFFFFF', borderRadius: 6, padding: 24, fontFamily: "'Inter', sans-serif" }}>
            <p style={{ fontSize: 14, color: isDark ? '#A09890' : '#374151', lineHeight: 1.6, margin: '0 0 16px' }}>
...
function BtnOutline({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button {...props} style={{
      ...
      background: isDark ? '#232019' : '#FFFFFF', color: isDark ? '#A09890' : '#374151', fontFamily: "'Inter', sans-serif",
...
function BtnGhost({ children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button {...props} style={{
      ...
      background: 'transparent', color: isDark ? '#6B6560' : '#64748B', fontFamily: "'Inter', sans-serif",
      ...props.style,
    }}>{children}</button>
  );
}
...
function CatalystTopNav() {
  ...
  return (
    <nav style={{ height: 48, display: 'flex', alignItems: 'center', gap: 0, background: isDark ? '#232019' : '#FFFFFF', borderBottom: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, padding: '0 24px', fontFamily: "'Inter', sans-serif" }}>
      {HUBS.map(h => {
