    ...
    <div className="th-page-content" style={{ flex: 1, padding: '24px 32px', overflow: 'auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--fg-3, #64748B)', margin: '4px 0 0' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative', flex: '0 1 280px' }}>
          <Search style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', width: 16, height: 16, color: 'var(--fg-4, #94A3B8)' }} />
          <input
            ...
            style={{
              ...
              border: '1px solid var(--divider, #E2E8F0)', borderRadius: 8, fontSize: 13, outline: 'none',
            }}
          />
          {filters.search && (
            <button onClick={() => setFilters(f => ({ ...f, search: '' }))} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--fg-4, #94A3B8)' }}>
        ...
        <select
          ...
          style={{ height: 38, padding: '0 12px', border: '1px solid var(--divider, #E2E8F0)', borderRadius: 8, fontSize: 13, color: '#334155', background: '#fff', cursor: 'pointer' }}
        >
        ...
        {/* View toggle */}
        <div style={{ display: 'flex', border: '1px solid var(--divider, #E2E8F0)', borderRadius: 8, overflow: 'hidden' }}>
          <button
      ...
      {/* Content */}
      {isLoading ? (
        <div style={{ display: 'flex', justifyContent: 'center', padding: 60, color: 'var(--fg-4, #94A3B8)' }}>
          Loading releases...
        </div>
      ) : releases.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60, color: 'var(--fg-4, #94A3B8)' }}>
      ...
      <div style={{ marginTop: 16, fontSize: 13, color: 'var(--fg-4, #94A3B8)' }}>
        ...
        <tbody>
          {releases.map(r => {
            ...
            return (
              ...
              >
                <td style={tdStyle}>
                  <span style={{ fontFamily: 'monospace', fontSize: 12, color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>{r.version}</span>
                </td>
                <td style={tdStyle}>
                  <div style={{ fontWeight: 600, color: 'var(--fg-1, #0F172A)' }}>{r.name}</div>
                  {r.vehicle && (
                    <span style={{ fontSize: 11, color: 'var(--fg-3, #64748B)' }}>{r.vehicle.name}</span>
                  )}
                </td>
                <td style={tdStyle}>
                  <span style={{
      ...
      {releases.map(r => {
        ...
        return (
          <div
            ...
            onClick={() => navigate(`/testhub/releases/${r.id}`)}
            style={{
              background: '#fff', border: '1px solid var(--divider, #E2E8F0)', borderRadius: 12,
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <div>
                <span style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--fg-4, #94A3B8)', fontWeight: 600 }}>{r.version}</span>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              ...
              {r.vehicle && (
                <span style={{ padding: '2px 8px', borderRadius: 5, backgroundColor: 'var(--surface-muted, #F1F5F9)', color: 'var(--fg-3, #64748B)', fontSize: 11, fontWeight: 500 }}>
                  {r.vehicle.name}
                </span>
              )}
            </div>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                <span style={{ color: 'var(--fg-3, #64748B)' }}>Test Progress</span>
                <span style={{ fontWeight: 600, color: 'var(--fg-1, #0F172A)' }}>{execPct}%</span>
              </div>
              <div style={{ height: 6, backgroundColor: 'var(--surface-muted, #F1F5F9)', borderRadius: 3 }}>
            ...
            {/* Date */}
            <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--fg-4, #94A3B8)' }}>
