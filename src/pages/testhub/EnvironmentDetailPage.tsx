...
const TYPE_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  other: { label: 'Other', color: 'var(--fg-3, #64748B)', bg: '#F1F5F9' },
};

const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  active: { label: 'Active', color: '#059669', bg: '#ECFDF5' },
  maintenance: { label: 'Maintenance', color: '#D97706', bg: '#FFFBEB' },
  inactive: { label: 'Inactive', color: 'var(--fg-3, #64748B)', bg: '#F1F5F9' },
...
const HEALTH_CONFIG: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  ...
  unknown: { label: 'Unknown', color: 'var(--fg-3, #64748B)', bg: '#F1F5F9', icon: HelpCircle },
};
...
export default function EnvironmentDetailPage() {
  ...
  if (!environment) {
    return <div style={{ padding: 24, textAlign: 'center', color: 'var(--fg-3, #64748B)' }}>Environment not found</div>;
  }
    ...
    <div style={{ padding: 24, backgroundColor: 'var(--surface-subtle, #F8FAFC)', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{environment.name}</h1>
          {environment.owner && (
            <p style={{ fontSize: 13, color: 'var(--fg-3, #64748B)', margin: '8px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ backgroundColor: health.bg, borderRadius: 12, padding: 20, border: `1px solid ${health.color}30`, marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--fg-3, #64748B)', margin: '4px 0 0' }}>Last checked: {formatDate(environment.last_health_check)}</p>
          </div>
        </div>
        <select value={environment.health_status} onChange={(e) => updateHealthStatus(e.target.value)}
          style={{ height: 36, padding: '0 12px', border: '1px solid var(--divider, #E2E8F0)', borderRadius: 6, fontSize: 13, backgroundColor: '#FFF', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div>
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 24, border: '1px solid var(--divider, #E2E8F0)', marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {environment.url && (
                <div>
                  <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Globe size={12} /> Application URL</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <ExternalLink size={14} style={{ color: 'var(--fg-4, #94A3B8)' }} />
                  </div>
                </div>
              )}
              {environment.api_url && (
                <div>
                  <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Link2 size={12} /> API URL</p>
                  <p style={{ fontSize: 14, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{environment.api_url}</p>
                </div>
              )}
              {environment.database_info && (
                <div>
                  <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Database size={12} /> Database</p>
                  <p style={{ fontSize: 14, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{environment.database_info}</p>
                </div>
              )}
              {!environment.url && !environment.api_url && !environment.database_info && (
                <p style={{ fontSize: 13, color: 'var(--fg-4, #94A3B8)', textAlign: 'center', padding: 20 }}>No connection info configured</p>
              )}
            </div>
          </div>
          ...
          {environment.description && (
            <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 24, border: '1px solid var(--divider, #E2E8F0)' }}>
              ...
              <p style={{ fontSize: 14, color: '#334155', margin: 0, whiteSpace: 'pre-wrap' }}>{environment.description}</p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 24, border: '1px solid var(--divider, #E2E8F0)', marginBottom: 24 }}>
            ...
            {variables.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--fg-4, #94A3B8)', textAlign: 'center', padding: 20 }}>No environment variables configured</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {variables.map((v) => (
                  <div key={v.id} style={{ padding: 12, backgroundColor: 'var(--surface-subtle, #F8FAFC)', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{v.key}</p>
                      <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: '2px 0 0', fontFamily: 'monospace' }}>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      {v.is_secret && (
                        <button onClick={() => toggleSecretVisibility(v.id)}
                          style={{ width: 28, height: 28, border: 'none', borderRadius: 4, backgroundColor: 'transparent', color: 'var(--fg-3, #64748B)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 24, border: '1px solid var(--divider, #E2E8F0)' }}>
            ...
            {linkedCycles.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--fg-4, #94A3B8)', textAlign: 'center', padding: 20 }}>No test cycles using this environment</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {linkedCycles.map((cycle) => (
                  <div key={cycle.cycle_id} onClick={() => navigate(`/testhub/cycles/${cycle.cycle_id}`)}
                    style={{ padding: 12, backgroundColor: 'var(--surface-subtle, #F8FAFC)', borderRadius: 8, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <span style={{ fontSize: 13, color: 'var(--fg-1, #0F172A)' }}>{cycle.name}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 12, color: 'var(--fg-3, #64748B)' }}>{cycle.progress_percent}%</span>
                      <ChevronRight size={16} style={{ color: 'var(--fg-4, #94A3B8)' }} />
                    </div>
                  </div>
                ))}
              </div>
