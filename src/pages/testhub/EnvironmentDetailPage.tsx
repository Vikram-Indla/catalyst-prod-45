...
export default function EnvironmentDetailPage() {
  ...
  if (!environment) {
    return <div style={{ padding: 24, textAlign: 'center', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>Environment not found</div>;
  }
  ...
  return (
    <div style={{ padding: 24, backgroundColor: 'var(--cp-bg-page, #F8FAFC)', minHeight: '100vh' }}>
      <button onClick={() => navigate('/testhub/environments')}
        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', border: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 8, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, #FFF)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', fontSize: 13, cursor: 'pointer', marginBottom: 16 }}>
      ...
      <div style={{ backgroundColor: health.bg, borderRadius: 12, padding: 20, border: `1px solid ${health.color}30`, marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', margin: '4px 0 0' }}>Last checked: {formatDate(environment.last_health_check)}</p>
          </div>
        </div>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      {v.is_secret && (
                        <button onClick={() => toggleSecretVisibility(v.id)}
                          style={{ width: 28, height: 28, border: 'none', borderRadius: 4, backgroundColor: 'transparent', color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
