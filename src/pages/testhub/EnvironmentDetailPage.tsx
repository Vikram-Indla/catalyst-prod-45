...
export default function EnvironmentDetailPage() {
  ...
  if (!environment) {
    return <div style={{ padding: 24, textAlign: 'center', color: isDark ? '#888888' : '#64748B' }}>Environment not found</div>;
  }
  ...
  return (
    <div style={{ padding: 24, backgroundColor: isDark ? '#0A0A0A' : '#F8FAFC', minHeight: '100vh' }}>
      <button onClick={() => navigate('/testhub/environments')}
        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 8, backgroundColor: isDark ? '#111111' : '#FFF', color: isDark ? '#888888' : '#64748B', fontSize: 13, cursor: 'pointer', marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          {environment.owner && (
            <p style={{ fontSize: 13, color: isDark ? '#888888' : '#64748B', margin: '8px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <select value={environment.status} onChange={(e) => updateStatus(e.target.value)}
            style={{ height: 40, padding: '0 14px', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 8, fontSize: 13, backgroundColor: isDark ? '#111111' : '#FFF', color: isDark ? '#EDEDED' : undefined, cursor: 'pointer' }}>
      ...
      <div style={{ backgroundColor: health.bg, borderRadius: 12, padding: 20, border: `1px solid ${health.color}30`, marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: isDark ? '#888888' : '#64748B', margin: '4px 0 0' }}>Last checked: {formatDate(environment.last_health_check)}</p>
          </div>
        </div>
        <select value={environment.health_status} onChange={(e) => updateHealthStatus(e.target.value)}
          style={{ height: 36, padding: '0 12px', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 6, fontSize: 13, backgroundColor: isDark ? '#111111' : '#FFF', color: isDark ? '#EDEDED' : undefined, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        {/* Left Column */}
        <div>
          <div style={{ backgroundColor: isDark ? '#111111' : '#FFF', borderRadius: 12, padding: 24, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {environment.url && (
                <div>
                  <p style={{ fontSize: 12, color: isDark ? '#888888' : '#64748B', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Globe size={12} /> Application URL</p>
              ...
              {environment.api_url && (
                <div>
                  <p style={{ fontSize: 12, color: isDark ? '#888888' : '#64748B', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Link2 size={12} /> API URL</p>
                  <p style={{ fontSize: 14, color: isDark ? '#EDEDED' : '#0F172A', margin: 0 }}>{environment.api_url}</p>
                </div>
              )}
              {environment.database_info && (
                <div>
                  <p style={{ fontSize: 12, color: isDark ? '#888888' : '#64748B', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Database size={12} /> Database</p>
              ...
              {!environment.url && !environment.api_url && !environment.database_info && (
                <p style={{ fontSize: 13, color: isDark ? '#666666' : '#94A3B8', textAlign: 'center', padding: 20 }}>No connection info configured</p>
              )}
            </div>
          </div>

          {environment.description && (
            <div style={{ backgroundColor: isDark ? '#111111' : '#FFF', borderRadius: 12, padding: 24, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
              ...
              <p style={{ fontSize: 14, color: isDark ? '#888888' : '#334155', margin: 0, whiteSpace: 'pre-wrap' }}>{environment.description}</p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ backgroundColor: isDark ? '#111111' : '#FFF', borderRadius: 12, padding: 24, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, marginBottom: 24 }}>
            ...
            {variables.length === 0 ? (
              <p style={{ fontSize: 13, color: isDark ? '#666666' : '#94A3B8', textAlign: 'center', padding: 20 }}>No environment variables configured</p>
            ) : (
                ...
                {variables.map((v) => (
                  <div key={v.id} style={{ padding: 12, backgroundColor: isDark ? '#1A1A1A' : '#F8FAFC', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <p style={{ fontSize: 12, color: isDark ? '#888888' : '#64748B', margin: '2px 0 0', fontFamily: 'monospace' }}>
          ...
          <div style={{ backgroundColor: isDark ? '#111111' : '#FFF', borderRadius: 12, padding: 24, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
            ...
            {linkedCycles.length === 0 ? (
              <p style={{ fontSize: 13, color: isDark ? '#666666' : '#94A3B8', textAlign: 'center', padding: 20 }}>No test cycles using this environment</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {linkedCycles.map((cycle) => (
                  <div key={cycle.cycle_id} onClick={() => navigate(`/testhub/cycles/${cycle.cycle_id}`)}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 12, color: isDark ? '#888888' : '#64748B' }}>{cycle.progress_percent}%</span>
