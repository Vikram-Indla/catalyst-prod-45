...
export default function EnvironmentDetailPage() {
  ...
  if (!environment) {
    return <div style={{ padding: 24, textAlign: 'center', color: '#64748B' }}>Environment not found</div>;
  }
    ...
    <div style={{ padding: 24, backgroundColor: '#F8FAFC', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#0F172A', margin: 0 }}>{environment.name}</h1>
          {environment.owner && (
            <p style={{ fontSize: 13, color: '#64748B', margin: '8px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ backgroundColor: health.bg, borderRadius: 12, padding: 20, border: `1px solid ${health.color}30`, marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#64748B', margin: '4px 0 0' }}>Last checked: {formatDate(environment.last_health_check)}</p>
          </div>
        </div>
        <select value={environment.health_status} onChange={(e) => updateHealthStatus(e.target.value)}
          style={{ height: 36, padding: '0 12px', border: '1px solid #E2E8F0', borderRadius: 6, fontSize: 13, backgroundColor: '#FFF', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div>
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 24, border: '1px solid #E2E8F0', marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {environment.url && (
                <div>
                  <p style={{ fontSize: 12, color: '#64748B', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Globe size={12} /> Application URL</p>
              ...
              {environment.api_url && (
                <div>
                  <p style={{ fontSize: 12, color: '#64748B', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Link2 size={12} /> API URL</p>
                  <p style={{ fontSize: 14, color: '#0F172A', margin: 0 }}>{environment.api_url}</p>
                </div>
              )}
              {environment.database_info && (
                <div>
                  <p style={{ fontSize: 12, color: '#64748B', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Database size={12} /> Database</p>
                  <p style={{ fontSize: 14, color: '#0F172A', margin: 0 }}>{environment.database_info}</p>
                </div>
              )}
              {!environment.url && !environment.api_url && !environment.database_info && (
                <p style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: 20 }}>No connection info configured</p>
              )}
            </div>
          </div>
        ...
        <div>
          ...
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 24, border: '1px solid #E2E8F0', marginBottom: 24 }}>
            ...
            {variables.length === 0 ? (
              <p style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: 20 }}>No environment variables configured</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {variables.map((v) => (
                  <div key={v.id} style={{ padding: 12, backgroundColor: '#F8FAFC', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#0F172A', margin: 0 }}>{v.key}</p>
          ...
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 24, border: '1px solid #E2E8F0' }}>
            ...
            {linkedCycles.length === 0 ? (
              <p style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', padding: 20 }}>No test cycles using this environment</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {linkedCycles.map((cycle) => (
                  <div key={cycle.cycle_id} onClick={() => navigate(`/testhub/cycles/${cycle.cycle_id}`)}
                    style={{ padding: 12, backgroundColor: '#F8FAFC', borderRadius: 8, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <span style={{ fontSize: 13, color: '#0F172A' }}>{cycle.name}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 12, color: '#64748B' }}>{cycle.progress_percent}%</span>
