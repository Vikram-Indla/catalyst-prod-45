...
const ACTION_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
  ...
  view: { label: 'Viewed', color: 'var(--fg-3, #64748B)', icon: Activity },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--surface-subtle, #F8FAFC)' }}>
      <TestHubPageHeader title="Activity Feed" subtitle="Track all changes and actions across TestHub">
        <button onClick={fetchActivities}
          style={{ display: 'flex', alignItems: 'center', gap: 8, height: 40, padding: '0 20px', border: '1px solid var(--divider, #E2E8F0)', borderRadius: 8, backgroundColor: 'var(--bg-app, #FFFFFF)', color: '#334155', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 16, border: '1px solid var(--divider, #E2E8F0)' }}>
            <p style={{ fontSize: 11, color: 'var(--fg-3, #64748B)', margin: 0, textTransform: 'uppercase' }}>Total (30d)</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: '4px 0 0' }}>{stats.total_events}</p>
          </div>
          ...
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 16, border: '1px solid var(--divider, #E2E8F0)' }}>
            <p style={{ fontSize: 11, color: 'var(--fg-3, #64748B)', margin: 0, textTransform: 'uppercase' }}>Active Users</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: '4px 0 0' }}>{stats.active_users}</p>
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div style={{ backgroundColor: 'var(--bg-app, #FFFFFF)', borderRadius: 12, padding: 60, textAlign: 'center', border: '1px solid var(--divider, #E2E8F0)' }}>
          ...
          <p style={{ fontSize: 16, color: 'var(--fg-3, #64748B)', margin: 0 }}>No activity found</p>
          <p style={{ fontSize: 14, color: 'var(--fg-4, #94A3B8)', margin: '8px 0 0' }}>Activity will appear here as users make changes</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {Object.entries(groupedActivities).map(([date, items]) => (
            <div key={date}>
              <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-3, #64748B)', margin: '0 0 12px', textTransform: 'uppercase' }}>{date}</h3>
              <div style={{ backgroundColor: '#FFF', borderRadius: 12, border: '1px solid var(--divider, #E2E8F0)', overflow: 'hidden' }}>
                {items.map((activity, index) => {
                  ...
                  const entity = ENTITY_CONFIG[activity.entity_type] || { label: activity.entity_type, color: 'var(--fg-3, #64748B)', icon: FileText };
                    ...
                    <div key={activity.id}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg-1, #0F172A)' }}>{activity.user_name || 'Unknown user'}</span>
                          <span style={{ fontSize: 14, color: 'var(--fg-3, #64748B)' }}>{action.label.toLowerCase()}</span>
                      ...
                      <span style={{ fontSize: 12, color: 'var(--fg-4, #94A3B8)', whiteSpace: 'nowrap' }}>{formatTime(activity.created_at)}</span>
                    </div>
                  );
                })}
              </div>
