    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: isDark ? '#1A1714' : '#F8FAFC' }}>
      <TestHubPageHeader title="Activity Feed" subtitle="Track all changes and actions across TestHub">
        <button onClick={fetchActivities}
          style={{ display: 'flex', alignItems: 'center', gap: 8, height: 40, padding: '0 20px', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 8, backgroundColor: isDark ? '#232019' : '#FFFFFF', color: isDark ? '#A09890' : '#334155', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
          <div style={{ backgroundColor: isDark ? '#232019' : '#FFF', borderRadius: 12, padding: 16, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
            <p style={{ fontSize: 11, color: isDark ? '#A09890' : '#64748B', margin: 0, textTransform: 'uppercase' }}>Total (30d)</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: '4px 0 0' }}>{stats.total_events}</p>
          </div>
          ...
          <div style={{ backgroundColor: isDark ? '#232019' : '#FFF', borderRadius: 12, padding: 16, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
            <p style={{ fontSize: 11, color: isDark ? '#A09890' : '#64748B', margin: 0, textTransform: 'uppercase' }}>Active Users</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: '4px 0 0' }}>{stats.active_users}</p>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 300px', maxWidth: 400 }}>
          ...
          <input type="text" placeholder="Search activity..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            style={{ width: '100%', height: 44, padding: '0 14px 0 44px', border: `1.5px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 10, fontSize: 14, backgroundColor: isDark ? '#232019' : '#FFFFFF', color: isDark ? '#F5F3F0' : undefined }} />
        </div>
        <select value={entityFilter} onChange={(e) => setEntityFilter(e.target.value)}
          style={{ height: 44, padding: '0 36px 0 14px', border: `1.5px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 10, fontSize: 14, backgroundColor: isDark ? '#232019' : '#FFF', color: isDark ? '#F5F3F0' : undefined, cursor: 'pointer' }}>
      ...
      {isLoading ? (
        ...
        <div style={{ backgroundColor: isDark ? '#232019' : '#FFFFFF', borderRadius: 12, padding: 60, textAlign: 'center', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
          <Activity size={48} style={{ color: isDark ? '#6B6560' : '#CBD5E1', marginBottom: 16 }} />
          <p style={{ fontSize: 16, color: isDark ? '#A09890' : '#64748B', margin: 0 }}>No activity found</p>
          <p style={{ fontSize: 14, color: isDark ? '#6B6560' : '#94A3B8', margin: '8px 0 0' }}>Activity will appear here as users make changes</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {Object.entries(groupedActivities).map(([date, items]) => (
            <div key={date}>
              <h3 style={{ fontSize: 13, fontWeight: 600, color: isDark ? '#A09890' : '#64748B', margin: '0 0 12px', textTransform: 'uppercase' }}>{date}</h3>
              <div style={{ backgroundColor: isDark ? '#232019' : '#FFF', borderRadius: 12, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, overflow: 'hidden' }}>
                    ...
                    <div key={activity.id}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A' }}>{activity.user_name || 'Unknown user'}</span>
                          <span style={{ fontSize: 14, color: isDark ? '#A09890' : '#64748B' }}>{action.label.toLowerCase()}</span>
                        ...
                        <p style={{ fontSize: 13, color: isDark ? '#A09890' : '#334155', margin: '4px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      ...
                      <span style={{ fontSize: 12, color: isDark ? '#6B6560' : '#94A3B8', whiteSpace: 'nowrap' }}>{formatTime(activity.created_at)}</span>
                    </div>
                  );
                })}
              </div>
