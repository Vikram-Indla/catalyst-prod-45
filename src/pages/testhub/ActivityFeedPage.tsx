...
const ACTION_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
  ...
  update: { label: 'Updated', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', icon: Edit2 },
  delete: { label: 'Deleted', color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', icon: Trash2 },
  execute: { label: 'Executed', color: '#7C3AED', icon: Play },
  assign: { label: 'Assigned', color: '#0891B2', icon: UserPlus },
  status_change: { label: 'Status Changed', color: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', icon: ArrowRight },
  view: { label: 'Viewed', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', icon: Activity },
...
const ENTITY_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
  test_case: { label: 'Test Case', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', icon: FileText },
  defect: { label: 'Defect', color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', icon: Bug },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--cp-bg-page, #F8FAFC)' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
          <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', borderRadius: 12, padding: 16, border: `1px solid ${'var(--cp-border, #E2E8F0)'}` }}>
          ...
          <div style={{ backgroundColor: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', borderRadius: 12, padding: 16, border: '1px solid #BFDBFE' }}>
            <p style={{ fontSize: 11, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', margin: 0, textTransform: 'uppercase' }}>Updated</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', margin: '4px 0 0' }}>{stats.updates}</p>
          </div>
          <div style={{ backgroundColor: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))', borderRadius: 12, padding: 16, border: '1px solid #FECACA' }}>
            <p style={{ fontSize: 11, color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', margin: 0, textTransform: 'uppercase' }}>Deleted</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', margin: '4px 0 0' }}>{stats.deletes}</p>
          </div>
          <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', borderRadius: 12, padding: 16, border: `1px solid ${'var(--cp-border, #E2E8F0)'}` }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 300px', maxWidth: 400 }}>
          ...
          <input type="text" placeholder="Search activity..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            style={{ width: '100%', height: 44, padding: '0 14px 0 44px', border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 12, fontSize: 14, backgroundColor: 'var(--cp-bg-elevated, #FFFFFF)', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined }} />
        </div>
        <select value={entityFilter} onChange={(e) => setEntityFilter(e.target.value)}
          style={{ height: 44, padding: '0 36px 0 14px', border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 12, fontSize: 14, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined, cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {Object.entries(groupedActivities).map(([date, items]) => (
            <div key={date}>
              ...
              <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', borderRadius: 12, border: `1px solid ${'var(--cp-border, #E2E8F0)'}`, overflow: 'hidden' }}>
                {items.map((activity, index) => {
                  const action = ACTION_CONFIG[activity.action] || ACTION_CONFIG.view;
                  const entity = ENTITY_CONFIG[activity.entity_type] || { label: activity.entity_type, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', icon: FileText };
                    ...
                    <div key={activity.id}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ fontSize: 13, color: 'var(--cp-text-secondary, #334155)', margin: '4px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {activity.entity_key && <span style={{ fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>{activity.entity_key}</span>}
