    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: isDark ? '#0A0A0A' : '#F8FAFC' }}>
      <TestHubPageHeader title="Shared Steps Library" subtitle="Reusable test steps for consistency across test cases">
            <button onClick={handleRefresh} title="Refresh" style={{
              ...
              backgroundColor: isDark ? '#111111' : '#FFFFFF', color: isDark ? '#888888' : '#64748B', cursor: 'pointer',
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        {/* Category Sidebar */}
        <div style={{
          width: 260, backgroundColor: isDark ? '#111111' : '#FFFFFF', borderRight: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`,
          ...
          <div style={{
            ...
            <h2 style={{
              fontSize: 11, fontWeight: 600, color: isDark ? '#666666' : '#64748B', textTransform: 'uppercase',
            ...
            <button onClick={() => setIsCreateCategoryModalOpen(true)} title="Add category" style={{
              ...
              backgroundColor: isDark ? '#1A1A1A' : '#FFFFFF', color: isDark ? '#888888' : '#64748B', cursor: 'pointer',
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 8 }}>
            ...
            <CategorySidebarItem
              label="All Steps"
              icon={<Folder size={16} style={{ color: !selectedCategoryId ? '#2563EB' : (isDark ? '#888888' : '#64748B') }} />}
              count={totalCount}
            ...
            {categories.map(cat => (
              <CategorySidebarItem
                key={cat.id}
                label={cat.name}
                icon={getCategoryIcon(cat.icon, selectedCategoryId === cat.id ? cat.color : (isDark ? '#888888' : '#64748B'))}
          ...
          {/* Sidebar Footer */}
          <div style={{ padding: '12px 16px', borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'}`, fontSize: 12, color: isDark ? '#666666' : '#94A3B8' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {/* Toolbar */}
          <div style={{
            padding: '16px 24px', backgroundColor: isDark ? '#111111' : '#FFFFFF', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`,
            display: 'flex', alignItems: 'center', gap: 12,
          }}>
            <div style={{ position: 'relative', flex: 1, maxWidth: 400 }}>
              <Search size={16} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: isDark ? '#666666' : '#94A3B8' }} />
              <input
            ...
            <div style={{ position: 'relative' }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  backgroundColor: activeFilters.length > 0 ? (isDark ? 'rgba(37,99,235,0.15)' : '#EFF6FF') : (isDark ? '#111111' : '#FFFFFF'),
                  color: activeFilters.length > 0 ? '#2563EB' : (isDark ? '#888888' : '#334155'),
              ...
              {isFilterOpen && (
                ...
                >
                  <div style={{
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#888888' : '#64748B', fontFamily: 'Inter' }}>Filter by Category</span>
                    {activeFilters.length > 0 && (
                    ...
                    {categories.map((cat) => {
                      ...
                      return (
                        <label key={cat.id} style={{
                          ...
                          borderRadius: 8, cursor: 'pointer', fontSize: 14, color: isDark ? '#EDEDED' : '#334155', fontFamily: 'Inter',
                          ...
                          <span style={{ fontSize: 12, color: isDark ? '#666666' : '#94A3B8' }}>{cat.step_count || 0}</span>
                        </label>
                      );
                    })}
                  </div>
            ...
            <div style={{ position: 'relative' }}>
              <button
                ...
                style={{
                  ...
                  backgroundColor: isDark ? '#111111' : '#FFFFFF', color: isDark ? '#888888' : '#334155', fontSize: 14, fontWeight: 500,
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          color: isActive ? '#2563EB' : (isDark ? '#888888' : '#334155'), fontSize: 14,
            ...
            <div style={{ display: 'flex', border: `1.5px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 8, overflow: 'hidden' }}>
              <button onClick={() => setViewMode('list')} title="List view" style={{
                ...
                backgroundColor: viewMode === 'list' ? (isDark ? 'rgba(37,99,235,0.15)' : '#EFF6FF') : (isDark ? '#111111' : '#FFFFFF'),
                color: viewMode === 'list' ? '#2563EB' : (isDark ? '#888888' : '#64748B'), cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                <List size={18} />
              </button>
              <button onClick={() => setViewMode('card')} title="Card view" style={{
                ...
                backgroundColor: viewMode === 'card' ? (isDark ? 'rgba(37,99,235,0.15)' : '#EFF6FF') : (isDark ? '#111111' : '#FFFFFF'),
                color: viewMode === 'card' ? '#2563EB' : (isDark ? '#888888' : '#64748B'), cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                <LayoutGrid size={18} />
              </button>
            ...
            <span style={{ fontSize: 13, color: isDark ? '#666666' : '#64748B', fontFamily: 'Inter, sans-serif' }}>
          ...
          <div style={{ flex: 1, padding: 24, overflowY: 'auto' }}>
            {loading ? (
              ...
              <div style={{
                ...
                justifyContent: 'center', height: '100%', color: isDark ? '#666666' : '#94A3B8',
              }}>
              ...
              <div style={{ backgroundColor: isDark ? '#111111' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 12, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Inter' }}>
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, backgroundColor: isDark ? '#1A1A1A' : '#F8FAFC' }}>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: isDark ? '#666666' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Name</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: isDark ? '#666666' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Category</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 11, fontWeight: 600, color: isDark ? '#666666' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', width: 80 }}>Variables</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 11, fontWeight: 600, color: isDark ? '#666666' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', width: 70 }}>Used</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: 11, fontWeight: 600, color: isDark ? '#666666' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', width: 80 }}>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        ...
                        >
                          <td style={{ padding: '12px 16px' }}>
                            ...
                            {step.description && (
                              <div style={{ fontSize: 12, color: isDark ? '#666666' : '#64748B', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 400 }}>
                                {step.description}
                              </div>
                            )}
                          </td>
                          <td style={{ padding: '12px 16px' }}>
                            {step.category ? (
                              <span style={{
                                ...
                                fontWeight: 600, color: isDark ? '#888888' : '#374151',
                              }}>
                                {step.category.name}
                              </span>
                            ) : (
                              <span style={{ color: isDark ? '#666666' : '#94A3B8', fontSize: 13 }}>—</span>
                            )}
                          </td>
                          ...
                          <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                            <span style={{
                              ...
                              color: step.usage_count > 0 ? '#059669' : (isDark ? '#666666' : '#64748B'),
                            }}>
                              {step.usage_count}x
                            </span>
                          </td>
                          <td style={{ padding: '12px 16px', textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
                            <button onClick={() => setEditingStep(step)} title="Edit" style={{
                              ...
                              backgroundColor: 'transparent', color: isDark ? '#888888' : '#64748B', cursor: 'pointer',
                              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                            }}>
                              <Pencil size={14} />
                            </button>
  ...
  return (
    ...
    >
      ...
      <span style={{
        ...
        color: isSelected ? selectedColor : (isDark ? '#888888' : '#334155'), fontFamily: 'Inter',
      ...
      <span style={{
        ...
        color: isSelected ? selectedColor : (isDark ? '#666666' : '#94A3B8'),
...
function SharedStepCard({ step, onView, onEdit, onDuplicate, onDelete, isDark = false }: {
  ...
  return (
    <div
      style={{
        backgroundColor: isDark ? '#111111' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 12,
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        {cat && (
            <span style={{
              display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px',
              backgroundColor: isDark ? '#1A1A1A' : '#F1F5F9', borderRadius: 4, fontSize: 11,
              fontWeight: 600, color: isDark ? '#888888' : '#374151', fontFamily: 'Inter',
            }}>
              {cat.name}
            </span>
          )}
        ...
        <div ref={menuRef} style={{ position: 'relative' }}>
          <button
            ...
            style={{
              width: 32, height: 32, padding: 0, border: 'none', borderRadius: 6,
              backgroundColor: 'transparent', color: isDark ? '#666666' : '#94A3B8', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}
          >
      ...
      {/* Name */}
      <h3 style={{ fontFamily: 'Inter', fontSize: 15, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A', margin: '0 0 4px' }}>
        {step.name}
      </h3>
      ...
      {step.description && (
        <p style={{
          fontFamily: 'Inter', fontSize: 13, color: isDark ? '#666666' : '#64748B', margin: '0 0 12px',
      ...
      <div style={{ marginBottom: 12 }}>
        <span style={{ fontFamily: 'Inter', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: isDark ? '#666666' : '#64748B' }}>
          Action
        </span>
        <p style={{
          fontFamily: 'Inter', fontSize: 13, color: isDark ? '#888888' : '#334155', margin: '4px 0 0',
          display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden',
          lineHeight: 1.6,
        }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, paddingTop: 12, borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9'}` }}>
          {variables.map((v, i) => (
            <span key={i} style={{
              ...
              fontFamily: 'monospace', fontSize: 11, color: isDark ? '#666666' : '#64748B',
            }}>
              {`{{${v.name}}}`}
            </span>
          ))}
