    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: pageBg }}>
      <CatalystPageHeader title="Settings" actions={
        hasChanges ? (
          <button onClick={savePreferences} disabled={isSaving}
            style={{ display: 'flex', alignItems: 'center', gap: 8, height: 40, padding: '0 20px', border: 'none', borderRadius: 8, background: 'linear-gradient(135deg, var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB)) 0%, var(--ds-background-brand-bold-hovered, #1D4ED8) 100%)', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', fontSize: 14, fontWeight: 600, cursor: isSaving ? 'not-allowed' : 'pointer', opacity: isSaving ? 0.7 : 1 }}>
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        ...
        <div style={{ width: 220, flexShrink: 0 }}>
          <div style={{ backgroundColor: surfaceBg, borderRadius: 12, border: `1px solid ${borderColor}`, overflow: 'hidden' }}>
            {TABS.map((tab) => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  ...
                  <TabIcon size={18} style={{ color: isActive ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : textSecondary }} />
        ...
        <div style={{ flex: 1 }}>
          {preferences && (
            <>
              {/* Display Tab */}
              {activeTab === 'display' && (
                <div style={{ backgroundColor: surfaceBg, borderRadius: 12, padding: 24, border: `1px solid ${borderColor}` }}>
                  <h2 style={{ fontSize: 18, fontWeight: 600, color: textPrimary, margin: '0 0 24px' }}>Display Preferences</h2>

                  {/* Theme */}
                  <div style={{ marginBottom: 28 }}>
                    <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: textPrimary, marginBottom: 10 }}>Theme</label>
                      ...
                      {THEME_OPTIONS.map((opt) => {
                        ...
                        return (
                          <button key={opt.value} onClick={() => updatePreference('theme', opt.value)}
                            style={{ flex: 1, padding: 16, borderRadius: 12, border: `2px solid ${isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : borderColor}`, backgroundColor: isSelected ? ('var(--cp-primary-light, #EFF6FF)') : surfaceBg, cursor: 'pointer', textAlign: 'center' }}>
                            <OptIcon size={24} style={{ color: isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : textSecondary, marginBottom: 8 }} />
                            <p style={{ fontSize: 14, fontWeight: 500, color: isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : textPrimary, margin: 0 }}>{opt.label}</p>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {/* Density */}
                  <div style={{ marginBottom: 28 }}>
                    <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: textPrimary, marginBottom: 10 }}>Display Density</label>
                      ...
                      {DENSITY_OPTIONS.map((opt) => {
                        ...
                        return (
                          <button key={opt.value} onClick={() => updatePreference('density', opt.value)}
                            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 14, borderRadius: 8, border: `1.5px solid ${isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : borderColor}`, backgroundColor: isSelected ? ('var(--cp-primary-light, #EFF6FF)') : surfaceBg, cursor: 'pointer', textAlign: 'left' }}>
                            <div>
                            ...
                            {isSelected && <Check size={18} style={{ color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' }} />}
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {/* Default Page Size */}
                  <div style={{ marginBottom: 28 }}>
                    <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: textPrimary, marginBottom: 10 }}>Default Page Size</label>
                      ...
                      {PAGE_SIZE_OPTIONS.map((size) => {
                        ...
                        return (
                          <button key={size} onClick={() => updatePreference('default_page_size', size)}
                            style={{ padding: '10px 20px', borderRadius: 8, border: `1.5px solid ${isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : borderColor}`, backgroundColor: isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : surfaceBg, color: isSelected ? 'var(--ds-surface, #FFF)' : textPrimary, fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
                            {size}
                          </button>
                        );
                      })}
                  ...
                  <div>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                      <input type="checkbox" checked={preferences.show_archived} onChange={(e) => updatePreference('show_archived', e.target.checked)} style={{ width: 20, height: 20, accentColor: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' }} />
                      <div>
              ...
              {activeTab === 'execution' && (
                <div style={{ backgroundColor: surfaceBg, borderRadius: 12, padding: 24, border: `1px solid ${borderColor}` }}>
                  <h2 style={{ fontSize: 18, fontWeight: 600, color: textPrimary, margin: '0 0 24px' }}>Test Execution Preferences</h2>
                  <div style={{ marginBottom: 28 }}>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                      <input type="checkbox" checked={preferences.auto_advance_on_status} onChange={(e) => updatePreference('auto_advance_on_status', e.target.checked)} style={{ width: 20, height: 20, accentColor: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' }} />
                      <div>
                  ...
                  <div>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                      <input type="checkbox" checked={preferences.confirm_status_change} onChange={(e) => updatePreference('confirm_status_change', e.target.checked)} style={{ width: 20, height: 20, accentColor: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' }} />
                      <div>
              ...
              {activeTab === 'notifications' && (
                <div style={{ backgroundColor: surfaceBg, borderRadius: 12, padding: 24, border: `1px solid ${borderColor}` }}>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                      <input type="checkbox" checked={preferences.email_on_assignment} onChange={(e) => updatePreference('email_on_assignment', e.target.checked)} style={{ width: 20, height: 20, accentColor: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' }} />
                      <div>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                      <input type="checkbox" checked={preferences.email_on_cycle_complete} onChange={(e) => updatePreference('email_on_cycle_complete', e.target.checked)} style={{ width: 20, height: 20, accentColor: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' }} />
                      <div>
                  ...
                  <div style={{ marginBottom: 28 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                      <input type="checkbox" checked={preferences.email_on_defect_update} onChange={(e) => updatePreference('email_on_defect_update', e.target.checked)} style={{ width: 20, height: 20, accentColor: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' }} />
                      <div>
              ...
              {activeTab === 'datetime' && (
                <div style={{ backgroundColor: surfaceBg, borderRadius: 12, padding: 24, border: `1px solid ${borderColor}` }}>
                  <h2 style={{ fontSize: 18, fontWeight: 600, color: textPrimary, margin: '0 0 24px' }}>Date & Time Preferences</h2>
                  <div style={{ marginBottom: 28 }}>
                  ...
                  <div style={{ marginBottom: 28 }}>
                    <label style={{ display: 'block', fontSize: 14, fontWeight: 600, color: textPrimary, marginBottom: 10 }}>Time Format</label>
                    <div style={{ display: 'flex', gap: 12 }}>
                      {['12h', '24h'].map((fmt) => {
                        const isSelected = preferences.time_format === fmt;
                        return (
                          <button key={fmt} onClick={() => updatePreference('time_format', fmt)}
                            style={{ padding: '12px 24px', borderRadius: 8, border: `1.5px solid ${isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : borderColor}`, backgroundColor: isSelected ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : surfaceBg, color: isSelected ? 'var(--ds-surface, #FFF)' : textPrimary, fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
