      ...
      <CatalystPageHeader title="Settings" actions={
        hasChanges ? (
          <button onClick={savePreferences} disabled={isSaving}
            style={{ display: 'flex', alignItems: 'center', gap: 8, height: 40, padding: '0 20px', border: 'none', borderRadius: 8, background: 'linear-gradient(135deg, #2563EB 0%, #1D4ED8 100%)', color: 'var(--ds-text-inverse, #FFFFFF)', fontSize: 14, fontWeight: 600, cursor: isSaving ? 'not-allowed' : 'pointer', opacity: isSaving ? 0.7 : 1 }}>
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  ...
                  <TabIcon size={18} style={{ color: isActive ? '#2563EB' : textSecondary }} />
                      ...
                      {PAGE_SIZE_OPTIONS.map((size) => {
                        ...
                        return (
                          <button key={size} onClick={() => updatePreference('default_page_size', size)}
                            style={{ padding: '10px 20px', borderRadius: 8, border: `1.5px solid ${isSelected ? '#2563EB' : borderColor}`, backgroundColor: isSelected ? '#2563EB' : surfaceBg, color: isSelected ? '#FFF' : textPrimary, fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
                            {size}
                          </button>
                        );
                      })}
                      ...
                      {['12h', '24h'].map((fmt) => {
                        ...
                        return (
                          <button key={fmt} onClick={() => updatePreference('time_format', fmt)}
                            style={{ padding: '12px 24px', borderRadius: 8, border: `1.5px solid ${isSelected ? '#2563EB' : borderColor}`, backgroundColor: isSelected ? '#2563EB' : surfaceBg, color: isSelected ? '#FFF' : textPrimary, fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>
