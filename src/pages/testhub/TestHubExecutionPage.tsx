...
const statusConfigDark: Record<string, { icon: any; color: string; bg: string; label: string }> = {
  not_run: { icon: Clock, color: '#6B6560', bg: '#232019', label: 'Not Run' },
  ...
  skipped: { icon: SkipForward, color: '#6B6560', bg: '#2C2823', label: 'Skipped' },
};
  ...
  const updateStepStatus = useCallback((status: 'passed' | 'failed' | 'blocked' | 'skipped', comment?: string) => {
    ...
    if (element) {
      const colors: Record<string, string> = isDark
        ? { passed: 'rgba(34,197,94,0.12)', failed: 'rgba(248,113,113,0.12)', blocked: 'rgba(251,191,36,0.12)', skipped: '#2C2823' }
                      ...
                      {executionHistory.step_results.length === 0 ? (
                        ...
                        executionHistory.step_results.map((step, i) => {
                          const stepColors: Record<string, { text: string; bg: string; border: string }> = isDark ? {
                            ...
                            skipped: { text: '#6B6560', bg: '#2C2823', border: 'rgba(255,255,255,0.08)' },
                            not_run: { text: '#6B6560', bg: '#232019', border: 'rgba(255,255,255,0.08)' },
                          } : {
                          ...
                          return (
                            <div key={i} style={{
                              padding: '12px 16px', backgroundColor: isDark ? '#232019' : '#FFFFFF',
                              ...
                              {step.notes ? (
                                <div style={{ marginTop: 8, padding: '6px 10px', backgroundColor: isDark ? '#2C2823' : '#F8FAFC', borderRadius: 4 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                         ...
                         <div style={{ padding: 20 }}>
                           ...
                           {currentStep.expected_result && (
                             <div style={{ marginTop: 16, padding: 14, backgroundColor: isDark ? 'rgba(34,197,94,0.12)' : '#ECFDF5', borderRadius: 8, borderLeft: '3px solid #10B981' }}>
                               ...
                               <p style={{ fontSize: 14, color: isDark ? '#A09890' : '#065F46', margin: '4px 0 0', lineHeight: 1.5 }}>
                ...
                <div style={{
                  ...
                  {[
                    ...
                    { key: 'skipped', label: 'Skip', shortcut: 'S', icon: SkipForward, onClick: handleSkip, color: isDark ? '#6B6560' : '#64748B', bg: 'hsl(var(--muted) / 0.3)', activeBg: 'linear-gradient(135deg, #64748B, #475569)' },
                  ].map(btn => {
                    const Icon = btn.icon;
                  ...
                  <div style={{ position: 'absolute', right: 20, display: 'flex', gap: 8 }}>
                    {steps.length > 0 && !fastTrackMode && (() => {
                      ...
                      const statusColors: Record<string, { bg: string; text: string }> = {
                        ...
                        not_run: { bg: isDark ? '#2C2823' : '#E2E8F0', text: isDark ? '#6B6560' : '#64748B' },
                      };
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 6, backgroundColor: isDisabled ? (isDark ? '#2C2823' : '#F8FAFC') : colors.bg,
                            color: isDisabled ? (isDark ? '#6B6560' : '#94A3B8') : colors.text, fontSize: 12, fontWeight: 700,
