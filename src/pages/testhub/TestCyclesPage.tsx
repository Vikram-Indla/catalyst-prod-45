      ...
      <div style={{ padding: '16px 32px', backgroundColor: surfaceBg, borderBottom: `1px solid ${borderColor}`, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {isFilterOpen && (
            <div onClick={(e) => e.stopPropagation()} style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, width: 200, backgroundColor: surfaceBg, border: `1px solid ${borderColor}`, borderRadius: 12, boxShadow: isDark ? '0 10px 40px rgba(0,0,0,0.4)' : '0 10px 40px rgba(0,0,0,0.12)', zIndex: 200, padding: 8 }}>
              {Object.entries(statusConfig).map(([key, config]) => {
                ...
                return (
                  <label key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 8, cursor: 'pointer', backgroundColor: isChecked ? ('var(--cp-primary-light, #EFF6FF)') : 'transparent' }}>
                    <input type="checkbox" checked={isChecked} onChange={() => setStatusFilter(prev => isChecked ? prev.filter(s => s !== key) : [...prev, key])} style={{ width: 16, height: 16, accentColor: 'var(--ds-text-brand, #2563EB)' }} />
              ...
              {statusFilter.length > 0 && (
                <button onClick={() => setStatusFilter([])} style={{ width: '100%', marginTop: 8, padding: '8px 12px', border: 'none', backgroundColor: 'transparent', color: 'var(--ds-text-brand, #2563EB)', fontSize: 13, fontWeight: 500, cursor: 'pointer', textAlign: 'center' }}>Clear filters</button>
              )}
            </div>
          )}
        </div>
        ...
        <div style={{ position: 'relative' }}>
          <button onClick={(e) => { e.stopPropagation(); setIsDateFilterOpen(!isDateFilterOpen); setIsFilterOpen(false); setIsSortOpen(false); }}
            style={{ height: 40, padding: '0 14px', border: `1.5px solid ${(dateFrom || dateTo) ? 'var(--ds-text-brand, #2563EB)' : borderColor}`, borderRadius: 8, backgroundColor: (dateFrom || dateTo) ? ('var(--cp-primary-light, #EFF6FF)') : surfaceBg, color: (dateFrom || dateTo) ? 'var(--ds-text-brand, #2563EB)' : textBody, fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            <Calendar size={16} /> Date
            {(dateFrom || dateTo) && <span style={{ minWidth: 8, height: 8, backgroundColor: 'var(--ds-text-brand, #2563EB)', borderRadius: '50%', display: 'inline-block' }} />}
            <ChevronDown size={14} />
          </button>
          {isDateFilterOpen && (
            <div onClick={(e) => e.stopPropagation()} style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, width: 260, backgroundColor: surfaceBg, border: `1px solid ${borderColor}`, borderRadius: 12, boxShadow: isDark ? '0 10px 40px rgba(0,0,0,0.4)' : '0 10px 40px rgba(0,0,0,0.12)', zIndex: 200, padding: 16 }}>
              ...
              {(dateFrom || dateTo) && (
                <button onClick={() => { setDateFrom(''); setDateTo(''); }} style={{ width: '100%', padding: '8px 12px', border: 'none', backgroundColor: 'transparent', color: 'var(--ds-text-brand, #2563EB)', fontSize: 13, fontWeight: 500, cursor: 'pointer', textAlign: 'center' }}>Clear dates</button>
              )}
            </div>
          )}
        </div>
              ...
              {[
                ...
                return (
                  <button key={option.field} onClick={() => {
                    ...
                    setIsSortOpen(false);
                  }} style={{ width: '100%', height: 40, padding: '8px 12px', border: 'none', borderRadius: 8, backgroundColor: isActive ? ('var(--cp-primary-light, #EFF6FF)') : 'transparent', color: isActive ? 'var(--ds-text-brand, #2563EB)' : textBody, fontSize: 14, fontWeight: isActive ? 600 : 400, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', textAlign: 'left' }}>
        ...
        {isLoading ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, color: textSecondary }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: 32, height: 32, border: `3px solid ${borderColor}`, borderTopColor: 'var(--ds-text-brand, #2563EB)', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 12px' }} />
              Loading test cycles...
            </div>
          </div>
        ) : cycles.length === 0 ? (
