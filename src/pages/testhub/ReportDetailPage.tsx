...
const TYPE_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  custom: { label: 'Custom', color: 'var(--fg-3, #64748B)' },
};
...
export default function ReportDetailPage() {
  ...
  if (!report) {
    return <div style={{ padding: 24, textAlign: 'center', color: 'var(--fg-3, #64748B)' }}>Report not found</div>;
  }
  ...
  return (
    <div style={{ padding: 24, backgroundColor: 'var(--surface-subtle, #F8FAFC)', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 600, color: '#F59E0B', backgroundColor: '#FFFBEB', padding: '6px 14px', borderRadius: 8 }}>{report.report_key}</span>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{report.name}</h1>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={exportToCSV} style={{ display: 'flex', alignItems: 'center', gap: 6, height: 40, padding: '0 16px', border: '1px solid var(--divider, #E2E8F0)', borderRadius: 8, backgroundColor: '#FFF', color: '#334155', fontSize: 13, cursor: 'pointer' }}>
            <Download size={16} /> CSV
          </button>
          <button onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: 6, height: 40, padding: '0 14px', border: '1px solid var(--divider, #E2E8F0)', borderRadius: 8, backgroundColor: '#FFF', color: '#334155', fontSize: 13, cursor: 'pointer' }}>
            <Printer size={16} />
          </button>
          <button onClick={deleteReport} style={{ display: 'flex', alignItems: 'center', gap: 6, height: 40, padding: '0 14px', border: '1px solid #FECACA', borderRadius: 8, backgroundColor: '#FEF2F2', color: '#DC2626', fontSize: 13, cursor: 'pointer' }}>
            <Trash2 size={16} />
          </button>
        </div>
      </div>
      ...
      {report.type === 'cycle_summary' && data.summary && (
        <>
          {data.cycle && (
            <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)', marginBottom: 24 }}>
              <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg-3, #64748B)', margin: '0 0 8px', textTransform: 'uppercase' }}>Test Cycle</h3>
              <p style={{ fontSize: 18, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{data.cycle.cycle_key} - {data.cycle.name}</p>
              <p style={{ fontSize: 13, color: 'var(--fg-3, #64748B)', margin: '4px 0 0' }}>Status: {data.cycle.status} | {data.cycle.start_date} to {data.cycle.end_date}</p>
            </div>
          )}

          {/* Stats */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
            <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)', textAlign: 'center' }}>
              <BarChart3 size={24} style={{ color: '#2563EB', marginBottom: 8 }} />
              <p style={{ fontSize: 28, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{data.summary.total_cases}</p>
              <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: '4px 0 0' }}>Total Tests</p>
            </div>
            <div style={{ backgroundColor: '#ECFDF5', borderRadius: 12, padding: 20, border: '1px solid #A7F3D0', textAlign: 'center' }}>
            ...
            <div style={{ backgroundColor: '#FEF2F2', borderRadius: 12, padding: 20, border: '1px solid #FECACA', textAlign: 'center' }}>
            ...
            <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: '4px 0 0' }}>Pass Rate</p>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)', marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg-1, #0F172A)' }}>Execution Progress</span>
              <span style={{ fontSize: 14, fontWeight: 700, color: '#2563EB' }}>{data.summary.progress_percent}%</span>
            </div>
            <div style={{ height: 16, backgroundColor: '#E2E8F0', borderRadius: 8, overflow: 'hidden', display: 'flex' }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 12, fontSize: 12, color: 'var(--fg-3, #64748B)' }}>
          ...
          {data.assignee_breakdown && data.assignee_breakdown.length > 0 && (
            <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)', marginBottom: 24 }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                <Users size={18} style={{ color: '#F59E0B' }} /> By Assignee
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
                {data.assignee_breakdown.map((a: any, i: number) => (
                  <div key={i} style={{ padding: 14, backgroundColor: 'var(--surface-subtle, #F8FAFC)', borderRadius: 8 }}>
                    <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: '0 0 8px' }}>{a.assignee_name}</p>
          ...
          {data.test_cases && data.test_cases.length > 0 && (
            <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)' }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: '0 0 16px' }}>Test Case Results ({data.test_cases.length})</h3>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--divider, #E2E8F0)' }}>
                      <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Case Key</th>
                      <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Title</th>
                      <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Priority</th>
                      <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Status</th>
                      <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Executed By</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.test_cases.map((tc: any, i: number) => (
                      <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                        <td style={{ padding: '12px 8px' }}>
                          <span style={{ fontSize: 12, fontWeight: 600, color: '#2563EB', backgroundColor: '#EFF6FF', padding: '2px 6px', borderRadius: 4 }}>{tc.case_key}</span>
                        </td>
                        <td style={{ padding: '12px 8px', color: 'var(--fg-1, #0F172A)' }}>{tc.title}</td>
                        <td style={{ padding: '12px 8px', textTransform: 'capitalize' }}>{tc.priority}</td>
                        <td style={{ padding: '12px 8px' }}>
                          <span style={{
                            fontSize: 11, fontWeight: 500, padding: '3px 8px', borderRadius: 4,
                            backgroundColor: tc.status === 'passed' ? '#ECFDF5' : tc.status === 'failed' ? '#FEF2F2' : '#F1F5F9',
                            color: tc.status === 'passed' ? '#059669' : tc.status === 'failed' ? '#DC2626' : '#64748B',
                          }}>
                            {tc.status || 'Not Run'}
                          </span>
                        </td>
                        <td style={{ padding: '12px 8px', color: 'var(--fg-3, #64748B)' }}>{tc.executed_by || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {report.type === 'defect' && data.defects && (
        <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)' }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: '0 0 16px' }}>Defects ({data.total_defects})</h3>
          ...
          {data.severity_breakdown && (
            <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
              {Object.entries(data.severity_breakdown).map(([sev, count]) => (
                <div key={sev} style={{ padding: '8px 16px', backgroundColor: 'var(--surface-subtle, #F8FAFC)', borderRadius: 8, fontSize: 13 }}>
          ...
          {Array.isArray(data.defects) && data.defects.length > 0 && (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--divider, #E2E8F0)' }}>
                  <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Key</th>
                  <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Title</th>
                  <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Severity</th>
                  <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Status</th>
                  <th style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Reporter</th>
                </tr>
              </thead>
              <tbody>
                {data.defects.map((d: any, i: number) => (
                  <tr key={i} style={{ borderBottom: '1px solid #F1F5F9' }}>
                    <td style={{ padding: '12px 8px' }}><span style={{ fontSize: 12, fontWeight: 600, color: '#DC2626', backgroundColor: '#FEF2F2', padding: '2px 6px', borderRadius: 4 }}>{d.defect_key}</span></td>
                    <td style={{ padding: '12px 8px', color: 'var(--fg-1, #0F172A)' }}>{d.title}</td>
                    <td style={{ padding: '12px 8px', textTransform: 'capitalize' }}>{d.severity}</td>
                    <td style={{ padding: '12px 8px', textTransform: 'capitalize' }}>{d.status}</td>
                    <td style={{ padding: '12px 8px', color: 'var(--fg-3, #64748B)' }}>{d.reporter || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {!['cycle_summary', 'defect'].includes(report.type) && (
        <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 40, border: '1px solid var(--divider, #E2E8F0)', textAlign: 'center' }}>
          <FileBarChart size={48} style={{ color: '#CBD5E1', marginBottom: 16 }} />
          <p style={{ fontSize: 16, color: 'var(--fg-3, #64748B)', margin: 0 }}>Report data visualization coming soon</p>
          <p style={{ fontSize: 13, color: 'var(--fg-4, #94A3B8)', margin: '8px 0 0' }}>Raw data has been captured and stored</p>
        </div>
      )}
