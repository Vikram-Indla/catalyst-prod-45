...
export default function SharedStepDetailPage() {
  ...
  if (!step) {
    return (
      <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#878787' : '#64748B' }}>
      ...
      <div style={{ flex: 1, padding: 32, overflowY: 'auto' }}>
        ...
        <div style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, borderRadius: 12, padding: 24, marginBottom: 24 }}>
          ...
          {step.description && (
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#878787' : '#94A3B8', display: 'block', marginBottom: 4 }}>Description</label>
              <p style={{ fontSize: 14, color: isDark ? '#EDEDED' : '#334155', margin: 0 }}>{step.description}</p>
            </div>
          )}
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#878787' : '#94A3B8', display: 'block', marginBottom: 4 }}>Action</label>
            <p style={{ fontSize: 14, color: isDark ? '#EDEDED' : '#334155', margin: 0, whiteSpace: 'pre-wrap' }}>{step.action}</p>
          </div>
          {step.expected_result && (
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#878787' : '#94A3B8', display: 'block', marginBottom: 4 }}>Expected Result</label>
              <p style={{ fontSize: 14, color: isDark ? '#EDEDED' : '#334155', margin: 0, whiteSpace: 'pre-wrap' }}>{step.expected_result}</p>
            </div>
          )}
        </div>
        ...
        <div style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? '#2E2E2E' : '#E2E8F0'}`, borderRadius: 12, overflow: 'hidden' }}>
          ...
          {linkedCases.length === 0 ? (
            <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#878787' : '#94A3B8' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: isDark ? '#111111' : '#F8FAFC' }}>
                  <th style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: isDark ? '#878787' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Case Key</th>
                  <th style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: isDark ? '#878787' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Title</th>
                </tr>
              </thead>
              <tbody>
                {linkedCases.map((tc) => (
                  <tr
                    ...
                    style={{ height: 36, maxHeight: 36, borderBottom: `0.75px solid ${isDark ? '#292929' : '#F1F5F9'}`, cursor: 'pointer' }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = isDark ? '#1F1F1F' : '#F8FAFC'}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                  >
                    <td style={{ padding: '0 16px' }}>
                      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 13, fontWeight: 500, color: '#2563EB' }}>{tc.case_key}</span>
                    </td>
