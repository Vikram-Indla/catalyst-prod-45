    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--cp-bg-page, #F8FAFC)' }}>
      ...
      {isLoading ? (
        ...
        <div style={{ backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', borderRadius: 12, padding: 60, textAlign: 'center', border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))' }}>
          ...
          <p style={{ fontSize: 16, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: 0 }}>No requirements found</p>
          <p style={{ fontSize: 14, color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', margin: '8px 0 0' }}>Add requirements to track test coverage</p>
        </div>
      ) : (
        <div style={{ backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', borderRadius: 8, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))', overflow: 'hidden' }}>
              ...
              <tr>
                {['KEY', 'TITLE', 'TYPE', 'PRIORITY', 'STATUS', 'LINKED TESTS'].map(h => (
                  <th key={h} style={{
                    ...
                    fontSize: 10.5, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', textTransform: 'uppercase',
                    borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))', backgroundColor: 'var(--cp-bg-page, #F8FAFC)',
                  }}>{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {filteredRequirements.map((req) => {
                ...
                return (
                  <tr key={req.id} onClick={() => navigate(`/testhub/requirements/${req.id}`)}
                    style={{ cursor: 'pointer', height: 50, maxHeight: 50, borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))' }}
                    ...
                    <td style={{ padding: '8px 12px' }}>
                      <span style={{ fontSize: 11, fontWeight: 600, color: '#374151', backgroundColor: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))', padding: '2px 8px', borderRadius: 4 }}>{priority.label}</span>
                    </td>
                    <td style={{ padding: '8px 12px' }}>
                      <span style={{
                    ...
                    <td style={{ padding: '8px 12px' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))' }}>
