...
const COVERAGE_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  'Skipped': { label: 'SKIPPED', color: 'var(--ds-text-subtlest, #6B778C)', bg: 'var(--ds-border, #DFE1E6)' },
  'Not Run': { label: 'NOT RUN', color: 'var(--ds-text, #253858)', bg: 'var(--ds-border, #DFE1E6)' },
};
...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  deprecated:   { label: 'DEPRECATED',   color: 'var(--ds-text, #253858)', bg: 'var(--ds-border, #DFE1E6)' },
};
...
const RequirementDefectsPanel = ({ requirementId }: { requirementId?: string }) => {
  ...
  if (isLoading) {
    return <div style={{ padding: 24, color: 'var(--ds-text-subtlest, #94A3B8)', textAlign: 'center' }}>Loading defects...</div>;
  }

  if (defects.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: 40, color: 'var(--ds-text-subtlest, #94A3B8)' }}>
...
export default function RequirementDetailPage() {
  ...
  if (!requirement) {
    return <div style={{ padding: 24, textAlign: 'center', color: 'var(--ds-text-subtlest, #64748B)' }}>Requirement not found</div>;
  }
  ...
  return (
    <div style={{ padding: 24, backgroundColor: 'var(--cp-bg-page, #F8FAFC)', minHeight: '100vh' }}>
      <button onClick={() => navigate('/testhub/requirements')}
        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, #E2E8F0)', borderRadius: 8, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, #FFF)', color: 'var(--cp-text-tertiary, #64748B)', fontSize: 13, cursor: 'pointer', marginBottom: 16 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            ...
            {requirement.external_id && (
              <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--ds-text-subtlest, #64748B)', backgroundColor: 'var(--ds-surface-sunken, #F1F5F9)', padding: '4px 10px', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 13, color: 'var(--ds-text-subtlest, #64748B)' }}>
              ...
              <div key={test.link_id}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--ds-text-subtlest, #64748B)' }}>{test.priority}</span>
                    {(() => {
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => navigate(`/testhub/repository?view=${test.test_case_id}`)}
                    style={{ display: 'flex', alignItems: 'center', gap: 4, height: 32, padding: '8px 12px', border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, #E2E8F0)', borderRadius: 6, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, #FFF)', color: 'var(--cp-text-secondary, #334155)', fontSize: 12, cursor: 'pointer' }}>
                    View <ChevronRight size={14} />
                  </button>
                  <button onClick={() => unlinkTest(test.link_id)}
                    style={{ width: 32, height: 32, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, #E2E8F0)', borderRadius: 6, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, #FFF)', color: 'var(--cp-text-muted, #94A3B8)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Unlink size={14} />
                  </button>
                </div>
              </div>
