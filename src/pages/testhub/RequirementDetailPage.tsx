  ...
  return (
    <div style={{ padding: 24, backgroundColor: isDark ? '#1A1714' : '#F8FAFC', minHeight: '100vh' }}>
      <button onClick={() => navigate('/testhub/requirements')}
        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0', borderRadius: 8, backgroundColor: isDark ? '#232019' : '#FFF', color: isDark ? '#A09890' : '#64748B', fontSize: 13, cursor: 'pointer', marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: 0 }}>{requirement.title}</h1>
      ...
      {requirement.description && (
        <div style={{ backgroundColor: isDark ? '#232019' : '#FFF', borderRadius: 12, padding: 24, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0', marginBottom: 24 }}>
      ...
      {/* Linked Tests */}
      <div style={{ backgroundColor: isDark ? '#232019' : '#FFF', borderRadius: 12, padding: 24, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {linkedTests.map((test) => (
              <div key={test.link_id}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 14, backgroundColor: isDark ? '#2C2823' : '#F8FAFC', borderRadius: 10, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0' }}>
                <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => navigate(`/testhub/repository?view=${test.test_case_id}`)}
                    style={{ display: 'flex', alignItems: 'center', gap: 4, height: 32, padding: '0 12px', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0', borderRadius: 6, backgroundColor: isDark ? '#232019' : '#FFF', color: isDark ? '#A09890' : '#334155', fontSize: 12, cursor: 'pointer' }}>
                    View <ChevronRight size={14} />
                  </button>
                  <button onClick={() => unlinkTest(test.link_id)}
                    style={{ width: 32, height: 32, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0', borderRadius: 6, backgroundColor: isDark ? '#232019' : '#FFF', color: isDark ? '#6B6560' : '#94A3B8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Unlink size={14} />
                  </button>
                </div>
              </div>
