...
const executionStatusConfig: Record<string, { label: string; color: string; bg: string; Icon: any }> = {
  not_run: { label: 'Not Run', color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', Icon: Clock },
...
const CycleDefectsPanel = ({ cycleId, isDark }: { cycleId?: string; isDark: boolean }) => {
  ...
  if (isLoading) {
    return <p style={{ padding: '20px 32px', fontSize: 14, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>Loading defects...</p>;
  }
  ...
  return (
    <div style={{ backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 12, overflow: 'hidden' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ backgroundColor: 'var(--cp-bg-page, #F8FAFC)' }}>
            {['Defect Key', 'Title', 'Status', 'Severity', 'Source'].map(h => (
              <th key={h} style={{ padding: '10px 12px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase' as const, letterSpacing: '0.05em', borderBottom: `0.75px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}` }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {defects.map((d: any) => {
            ...
            return (
              <tr key={d.id} style={{ height: 36, maxHeight: 36, borderBottom: `0.75px solid ${'var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9))'}` }}>
                ...
                <td style={{ padding: '0 12px', fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>{d.title}</td>
                <td style={{ padding: '0 12px' }}>
                ...
                <td style={{ padding: '0 12px' }}>
                  {d.link_source === 'auto_execution' && (
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 6px', borderRadius: 3, backgroundColor: 'var(--cp-bg-page, var(--cp-bg-sunken, #F1F5F9))', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>Auto</span>
                  )}
...
export default function TestCycleDetailPage() {
  ...
  if (isLoading || !cycle) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', backgroundColor: 'var(--cp-bg-page, #F8FAFC)' }}>
        <div style={{ textAlign: 'center', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>
          <div style={{ width: 32, height: 32, border: `3px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderTopColor: 'var(--ds-text-brand, #2563EB)', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 12px' }} />
          Loading cycle...
        </div>
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--cp-bg-page, #F8FAFC)' }}>
      {/* Header */}
      <div style={{ padding: '20px 32px', backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}` }}>
        <button onClick={() => navigate(fromPlanId ? `/testhub/test-plans/${fromPlanId}` : '/testhub/cycles')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: 0, border: 'none', backgroundColor: 'transparent', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', fontSize: 13, fontWeight: 500, cursor: 'pointer', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => navigate(`/testhub/cycles/${cycleId}/report`)}
              style={{ height: 40, padding: '0 16px', border: `1.5px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 8, backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              <BarChart3 size={16} /> Report
            </button>
            <button onClick={handleExportCSV} title="Export CSV" disabled={testCases.length === 0}
              style={{ width: 40, height: 40, padding: 0, border: `1.5px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 8, backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', color: testCases.length > 0 ? ('var(--cp-text-tertiary, var(--cp-ink-3, #64748B))') : ('var(--cp-text-muted, #CBD5E1)'), cursor: testCases.length > 0 ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Download size={18} />
            </button>
            <button onClick={() => { setIsLoading(true); fetchCycle(); fetchTestCases(); catalystToast.success('Refreshed'); }} title="Refresh"
              style={{ width: 40, height: 40, padding: 0, border: `1.5px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 8, backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <RefreshCw size={18} />
            </button>
            ...
            {cycle.status === 'draft' && (
              ...
              }} style={{ height: 40, padding: '0 16px', background: 'linear-gradient(135deg, var(--ds-text-brand, #2563EB) 0%, var(--ds-background-brand-bold-hovered, #1D4ED8) 100%)', border: 'none', borderRadius: 8, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ padding: '24px 32px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {/* Progress Panel */}
        <div style={{ backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 12, padding: 24, textAlign: 'center' }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 16px' }}>Progress</p>
          <div style={{ width: 100, height: 100, margin: '0 auto 16px', position: 'relative' }}>
            <svg viewBox="0 0 36 36" style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)' }}>
              <circle cx="18" cy="18" r="15.9155" fill="none" stroke={'var(--cp-border, var(--cp-border, #E2E8F0))'} strokeWidth="3" />
          ...
          <p style={{ fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))', margin: 0, fontWeight: 500 }}>{executedCount}/{totalCount} executed</p>
        </div>

        {/* By Status Panel */}
        <div style={{ backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 12, padding: 24 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 16px' }}>By Status</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <button onClick={() => setStatusFilter('passed')} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', border: 'none', borderRadius: 8, backgroundColor: statusFilter === 'passed' ? ('var(--cp-success-light, #ECFDF5)') : 'transparent', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
              ...
              <span style={{ flex: 1, fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>Passed</span>
            ...
            <button onClick={() => setStatusFilter('failed')} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', border: 'none', borderRadius: 8, backgroundColor: statusFilter === 'failed' ? ('var(--cp-danger-light, #FEF2F2)') : 'transparent', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
              ...
              <span style={{ flex: 1, fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>Failed</span>
            ...
            <button onClick={() => setStatusFilter('blocked')} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', border: 'none', borderRadius: 8, backgroundColor: statusFilter === 'blocked' ? ('var(--cp-warning-light, #FFFBEB)') : 'transparent', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
              ...
              <span style={{ flex: 1, fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>Blocked</span>
            ...
            <button onClick={() => setStatusFilter('not_run')} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', border: 'none', borderRadius: 8, backgroundColor: statusFilter === 'not_run' ? ('var(--cp-bg-page, #F8FAFC)') : 'transparent', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
              <Clock size={18} style={{ color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }} />
              <span style={{ flex: 1, fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>Not Run</span>
              <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>{notRunCount}</span>
            </button>
            {statusFilter !== 'all' && (
        ...
        {/* By Tester Panel */}
        <div style={{ backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 12, padding: 24 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                return (
                  <div key={i}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>{ts.name}</span>
                      <span style={{ fontSize: 12, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>{ts.executed}/{ts.total}</span>
                    </div>
                    <div style={{ height: 6, backgroundColor: 'var(--cp-border, var(--cp-border, #E2E8F0))', borderRadius: 4, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, padding: '0 32px 32px', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {selectedTestCaseIds.size > 0 ? (
                  <>
                    ...
                    <button onClick={deselectAllTestCases} style={{ padding: '4px 8px', border: 'none', backgroundColor: 'transparent', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
                      Clear
                    </button>
                  </>
                ) : (
                  <button onClick={selectAllTestCases} style={{ padding: '4px 8px', border: 'none', backgroundColor: 'transparent', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
                    Select all
                  </button>
                )}
              </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
              style={{ height: 50, padding: '8px 12px', border: `1.5px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 8, fontSize: 13, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))', backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))' }}>
            ...
            {canEdit && (
              <button onClick={() => setIsAddModalOpen(true)} style={{ height: 50, padding: '0 14px', background: 'linear-gradient(135deg, var(--ds-text-brand, #2563EB) 0%, var(--ds-background-brand-bold-hovered, #1D4ED8) 100%)', border: 'none', borderRadius: 8, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ flex: 1, backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          {testCases.length === 0 ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--cp-text-muted, var(--cp-ink-4, #94A3B8))', padding: 40 }}>
              ...
              <p style={{ fontSize: 16, fontWeight: 500, margin: '0 0 8px', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>No test cases added yet</p>
              <p style={{ fontSize: 14, margin: '0 0 16px' }}>Add test cases from the Test Repository to start planning</p>
              {canEdit && (
                <button onClick={() => setIsAddModalOpen(true)} style={{ height: 40, padding: '0 20px', background: 'linear-gradient(135deg, var(--ds-text-brand, #2563EB) 0%, var(--ds-background-brand-bold-hovered, #1D4ED8) 100%)', border: 'none', borderRadius: 8, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ backgroundColor: 'var(--cp-bg-page, #F8FAFC)' }}>
                    {/* Checkbox Header */}
                    {canEdit && (
                      <th style={{ padding: '14px 16px', textAlign: 'center', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, width: 50 }}>
                        <input
                          type="checkbox"
                    ...
                    <th style={{ padding: '14px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}` }}>Test Case</th>
                    <th style={{ padding: '14px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, width: 100 }}>Priority</th>
                    <th style={{ padding: '14px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, width: 140 }}>Assigned To</th>
                    <th style={{ padding: '14px 16px', textAlign: 'center', fontSize: 11, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, width: 120 }}>Status</th>
                    <th style={{ padding: '14px 16px', textAlign: 'center', fontSize: 11, fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, width: 100 }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={ctc.id} style={{ borderBottom: index < filteredTestCases.length - 1 ? `1px solid ${'var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9))'}` : 'none', backgroundColor: isSelected ? ('var(--cp-primary-light, #EFF6FF)') : 'transparent' }}>
                        ...
                        <td style={{ padding: '14px 16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <span style={{ fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>{ctc.test_case?.title || 'Unknown'}</span>
                          </div>
                        </td>
                        <td style={{ padding: '14px 16px' }}>
      ...
      {isRemoveConfirmOpen && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              padding: '20px 24px', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`,
            ...
            <div style={{ padding: 24 }}>
              <p style={{ fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))', margin: 0 }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))', margin: '12px 0 0' }}>
                The test cases will remain in the Test Repository. Any execution data for these test cases in this cycle will be lost.
              </p>
            </div>
            <div style={{
              padding: '16px 24px', borderTop: `1px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`,
              display: 'flex', justifyContent: 'flex-end', gap: 12,
            }}>
              <button
                onClick={() => setIsRemoveConfirmOpen(false)}
                style={{ height: 40, padding: '0 20px', backgroundColor: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: `1.5px solid ${'var(--cp-border, var(--cp-border, #E2E8F0))'}`, borderRadius: 8, fontSize: 14, fontWeight: 500, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))', cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button
