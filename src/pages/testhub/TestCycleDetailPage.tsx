...
const executionStatusConfig: Record<string, { label: string; color: string; bg: string; Icon: any }> = {
  not_run: { label: 'Not Run', color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', bg: 'var(--ds-surface-sunken, #F1F5F9)', Icon: Clock },
              ...
              <tr key={d.id} style={{ height: 36, maxHeight: 36, borderBottom: `0.75px solid ${'var(--cp-bg-sunken, #F1F5F9)'}` }}>
                ...
                <td style={{ padding: '0 12px' }}>
                  {d.link_source === 'auto_execution' && (
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 6px', borderRadius: 3, backgroundColor: 'var(--cp-bg-page, #F1F5F9)', color: 'var(--cp-text-tertiary, var(--cp-ink-3, #64748B))' }}>Auto</span>
                  )}
