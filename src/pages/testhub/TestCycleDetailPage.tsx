            ...
            return (
              <tr key={d.id} style={{ height: 36, maxHeight: 36, borderBottom: `0.75px solid ${'var(--cp-bg-sunken, #F1F5F9)'}` }}>
                <td style={{ padding: '0 12px' }}>
                  <span onClick={() => navigate(`/testhub/defects/${d.id}`)} style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 13, fontWeight: 500, color: '#2563EB', cursor: 'pointer' }}>{d.defect_key}</span>
                </td>
...
export default function TestCycleDetailPage() {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', backgroundColor: isDark ? 'var(--cp-bg-page, #1F1F21)' : '#F8FAFC' }}>
        <div style={{ textAlign: 'center', color: 'var(--cp-text-tertiary, #64748B)' }}>
          <div style={{ width: 32, height: 32, border: `3px solid ${'var(--cp-border, #E2E8F0)'}`, borderTopColor: '#2563EB', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 12px' }} />
          Loading cycle...
        </div>
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: isDark ? 'var(--cp-bg-page, #1F1F21)' : '#F8FAFC' }}>
      {/* Header */}
      <div style={{ padding: '20px 32px', backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', borderBottom: `1px solid ${'var(--cp-border, #E2E8F0)'}` }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => navigate(`/testhub/cycles/${cycleId}/report`)}
              style={{ height: 40, padding: '0 16px', border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 8, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', color: 'var(--cp-text-secondary, #334155)', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              <BarChart3 size={16} /> Report
            </button>
            <button onClick={handleExportCSV} title="Export CSV" disabled={testCases.length === 0}
              style={{ width: 40, height: 40, padding: 0, border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 8, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', color: testCases.length > 0 ? ('var(--cp-text-tertiary, #64748B)') : ('var(--cp-text-muted, #CBD5E1)'), cursor: testCases.length > 0 ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Download size={18} />
            </button>
            <button onClick={() => { setIsLoading(true); fetchCycle(); fetchTestCases(); catalystToast.success('Refreshed'); }} title="Refresh"
              style={{ width: 40, height: 40, padding: 0, border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 8, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', color: 'var(--cp-text-tertiary, #64748B)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <RefreshCw size={18} />
            </button>
      ...
      <div style={{ padding: '24px 32px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {/* Progress Panel */}
        <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: `1px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 12, padding: 24, textAlign: 'center' }}>
        ...
        {/* By Status Panel */}
        <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: `1px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 12, padding: 24 }}>
      ...
      {blockedTestCases.length > 0 && (
        <div style={{ padding: '0 32px 16px' }}>
          <div style={{ padding: '14px 20px', backgroundColor: isDark ? 'rgba(217,119,6,0.1)' : '#FFFBEB', border: `1px solid ${isDark ? 'rgba(217,119,6,0.2)' : '#FDE68A'}`, borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button onClick={() => setStatusFilter('blocked')} style={{ marginLeft: 'auto', padding: '4px 12px', border: `1px solid ${isDark ? 'rgba(217,119,6,0.3)' : '#FDE68A'}`, borderRadius: 6, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', color: '#D97706', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
              View Blocked
            </button>
          </div>
        </div>
      ...
      <div style={{ flex: 1, padding: '0 32px 32px', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
              style={{ height: 50, padding: '8px 12px', border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 8, fontSize: 13, color: 'var(--cp-text-secondary, #334155)', backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF' }}>
        ...
        <div style={{ flex: 1, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: `1px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          {testCases.length === 0 ? (
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#F8FAFC' }}>
                    {/* Checkbox Header */}
                    {canEdit && (
                ...
                <tbody>
                  {filteredTestCases.map((ctc, index) => {
                    ...
                    return (
                      <tr key={ctc.id} style={{ borderBottom: index < filteredTestCases.length - 1 ? `1px solid ${'var(--cp-bg-sunken, #F1F5F9)'}` : 'none', backgroundColor: isSelected ? (isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF') : 'transparent' }}>
                        {/* Checkbox Cell */}
                        {canEdit && (
      ...
      {isRemoveConfirmOpen && (
        <div style={{
          ...
          <div style={{
            width: 420, backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', borderRadius: 12,
            ...
            <div style={{
              ...
              <button
                ...
                style={{ height: 40, padding: '0 20px', backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 8, fontSize: 14, fontWeight: 500, color: 'var(--cp-text-secondary, #334155)', cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button
