...
const TYPE_CONFIG: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  ...
  custom: { label: 'Custom', color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', bg: 'var(--ds-surface-sunken, #F1F5F9)', icon: FileBarChart },
};

const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  draft: { label: 'Draft', color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', bg: 'var(--ds-surface-sunken, #F1F5F9)' },
          ...
          {filteredReports.map((report) => {
            ...
            return (
              <div key={report.id} onClick={() => navigate(`/testhub/reports/${report.id}`)}
                style={{ backgroundColor: 'var(--cp-bg-elevated, #FFFFFF)', borderRadius: 12, padding: 20, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, #E2E8F0)', cursor: 'pointer', transition: 'all 0.15s' }}
