  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: isDark ? '#1A1714' : '#F8FAFC' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
          <div style={{ backgroundColor: isDark ? '#232019' : '#FFF', borderRadius: 12, padding: 20, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <p style={{ fontSize: 12, color: isDark ? '#6B6560' : '#64748B', margin: 0, textTransform: 'uppercase' }}>Total Reports</p>
                <p style={{ fontSize: 28, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: '8px 0 0' }}>{stats.total_reports}</p>
              </div>
          ...
          <div style={{ backgroundColor: isDark ? '#232019' : '#FFF', borderRadius: 12, padding: 20, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <p style={{ fontSize: 12, color: isDark ? '#6B6560' : '#64748B', margin: 0, textTransform: 'uppercase' }}>This Month</p>
                <p style={{ fontSize: 28, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: '8px 0 0' }}>{stats.this_month}</p>
              </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} style={{ height: 44, padding: '0 36px 0 14px', border: isDark ? '1.5px solid rgba(255,255,255,0.08)' : '1.5px solid #E2E8F0', borderRadius: 10, fontSize: 14, backgroundColor: isDark ? '#232019' : '#FFF', color: isDark ? '#F5F3F0' : undefined, cursor: 'pointer' }}>
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ height: 44, padding: '0 36px 0 14px', border: isDark ? '1.5px solid rgba(255,255,255,0.08)' : '1.5px solid #E2E8F0', borderRadius: 10, fontSize: 14, backgroundColor: isDark ? '#232019' : '#FFF', color: isDark ? '#F5F3F0' : undefined, cursor: 'pointer' }}>
      ...
      {isLoading ? (
        ...
        <div style={{ backgroundColor: isDark ? '#232019' : '#FFFFFF', borderRadius: 12, padding: 60, textAlign: 'center', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0' }}>
          ...
          <p style={{ fontSize: 16, color: isDark ? '#A09890' : '#64748B', margin: 0 }}>No reports found</p>
          <p style={{ fontSize: 14, color: isDark ? '#6B6560' : '#94A3B8', margin: '8px 0 0' }}>Generate a new report to get started</p>
        </div>
      ) : (
          ...
          {filteredReports.map((report) => {
            ...
            return (
              <div key={report.id} onClick={() => navigate(`/testhub/reports/${report.id}`)}
                style={{ backgroundColor: isDark ? '#232019' : '#FFFFFF', borderRadius: 12, padding: 20, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0', cursor: 'pointer', transition: 'all 0.15s' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ display: 'flex', gap: 16, flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <h3 style={{ fontSize: 16, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A', margin: '0 0 4px' }}>{report.name}</h3>
                      <div style={{ display: 'flex', gap: 12, fontSize: 13, color: isDark ? '#6B6560' : '#64748B' }}>
