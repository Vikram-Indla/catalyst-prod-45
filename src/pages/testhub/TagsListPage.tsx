    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--surface-subtle, #F8FAFC)' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)' }}>
            <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: 0, textTransform: 'uppercase' }}>Total Tags</p>
            <p style={{ fontSize: 28, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: '8px 0 0' }}>{stats.total_tags}</p>
          </div>
          ...
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)' }}>
            <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: 0, textTransform: 'uppercase' }}>Categories</p>
            <p style={{ fontSize: 28, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: '8px 0 0' }}>{stats.categories}</p>
          </div>
          <div style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 20, border: '1px solid var(--divider, #E2E8F0)' }}>
            <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: 0, textTransform: 'uppercase' }}>Most Used</p>
            <p style={{ fontSize: 18, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: '8px 0 0' }}>{stats.most_used_tag || '—'}</p>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: '1 1 300px', maxWidth: 400 }}>
          ...
          <input type="text" placeholder="Search tags..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            style={{ width: '100%', height: 44, padding: '0 14px 0 44px', border: '1.5px solid var(--divider, #E2E8F0)', borderRadius: 10, fontSize: 14, backgroundColor: 'var(--bg-app, #FFFFFF)' }} />
        </div>
        <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} style={{ height: 44, padding: '0 36px 0 14px', border: '1.5px solid var(--divider, #E2E8F0)', borderRadius: 10, fontSize: 14, backgroundColor: '#FFF', cursor: 'pointer' }}>
          <option value="all">All Categories</option>
          {categories.map((cat) => <option key={cat} value={cat!}>{cat}</option>)}
        </select>
        {hasActiveFilters && (
          <button onClick={clearFilters} style={{ display: 'flex', alignItems: 'center', gap: 6, height: 44, padding: '0 16px', border: '1.5px solid var(--divider, #E2E8F0)', borderRadius: 10, backgroundColor: '#FFF', color: 'var(--fg-3, #64748B)', fontSize: 14, cursor: 'pointer' }}>
            <X size={16} /> Clear
          </button>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <div style={{ backgroundColor: 'var(--bg-app, #FFFFFF)', borderRadius: 12, padding: 60, textAlign: 'center', border: '1px solid var(--divider, #E2E8F0)' }}>
          ...
          <p style={{ fontSize: 16, color: 'var(--fg-3, #64748B)', margin: 0 }}>No tags found</p>
          <p style={{ fontSize: 14, color: 'var(--fg-4, #94A3B8)', margin: '8px 0 0' }}>Create tags to organize your test assets</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {Object.entries(groupedTags).map(([category, categoryTags]) => (
            <div key={category}>
              <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg-3, #64748B)', margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {category} ({categoryTags.length})
              </h3>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                {categoryTags.map((tag) => (
                  <div key={tag.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', backgroundColor: 'var(--bg-app, #FFFFFF)', borderRadius: 10, border: '1px solid var(--divider, #E2E8F0)', cursor: 'pointer' }}
                    ...
                    <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--fg-1, #0F172A)' }}>{tag.name}</span>
                    <span style={{ fontSize: 11, color: 'var(--fg-4, #94A3B8)', backgroundColor: 'var(--surface-muted, #F1F5F9)', padding: '2px 6px', borderRadius: 4 }}>{tag.usage_count}</span>
                    <div style={{ display: 'flex', gap: 4, marginLeft: 4 }}>
                      <button onClick={(e) => { e.stopPropagation(); setEditingTag(tag); setShowCreateModal(true); }}
                        style={{ width: 24, height: 24, border: 'none', borderRadius: 4, backgroundColor: 'transparent', color: 'var(--fg-3, #64748B)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Edit2 size={12} />
                      </button>
