  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: isDark ? '#0A0A0A' : '#1A1A1A' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
          <div style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFF', borderRadius: 12, padding: 20, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(255,255,255,0.10)' }}>
            <p style={{ fontSize: 12, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: 0, textTransform: 'uppercase' }}>Total Tags</p>
            <p style={{ fontSize: 28, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', margin: '8px 0 0' }}>{stats.total_tags}</p>
          </div>
          ...
          <div style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFF', borderRadius: 12, padding: 20, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(255,255,255,0.10)' }}>
            <p style={{ fontSize: 12, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: 0, textTransform: 'uppercase' }}>Categories</p>
            <p style={{ fontSize: 28, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', margin: '8px 0 0' }}>{stats.categories}</p>
          </div>
          <div style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFF', borderRadius: 12, padding: 20, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(255,255,255,0.10)' }}>
            <p style={{ fontSize: 12, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: 0, textTransform: 'uppercase' }}>Most Used</p>
            <p style={{ fontSize: 18, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', margin: '8px 0 0' }}>{stats.most_used_tag || '—'}</p>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} style={{ height: 44, padding: '0 36px 0 14px', border: isDark ? '1.5px solid rgba(255,255,255,0.08)' : '1.5px solid rgba(255,255,255,0.10)', borderRadius: 12, fontSize: 14, backgroundColor: isDark ? '#1A1A1A' : '#FFF', color: isDark ? '#EDEDED' : undefined, cursor: 'pointer' }}>
      ...
      {isLoading ? (
        ...
        <div style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFFFFF', borderRadius: 12, padding: 60, textAlign: 'center', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(255,255,255,0.10)' }}>
          <Tags size={48} style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.53)', marginBottom: 16 }} />
          <p style={{ fontSize: 16, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)', margin: 0 }}>No tags found</p>
          <p style={{ fontSize: 14, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: '8px 0 0' }}>Create tags to organize your test assets</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {Object.entries(groupedTags).map(([category, categoryTags]) => (
            <div key={category}>
              <h3 style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {category} ({categoryTags.length})
              </h3>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                {categoryTags.map((tag) => (
                  <div key={tag.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', backgroundColor: isDark ? '#1A1A1A' : '#FFFFFF', borderRadius: 12, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(255,255,255,0.10)', cursor: 'pointer' }}
                    ...
                    <span style={{ fontSize: 14, fontWeight: 500, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{tag.name}</span>
                    <span style={{ fontSize: 11, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', backgroundColor: isDark ? '#1A1A1A' : '#1A1A1A', padding: '2px 6px', borderRadius: 4 }}>{tag.usage_count}</span>
