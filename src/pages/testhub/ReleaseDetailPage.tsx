...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  ...
  archived: { label: 'Archived', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F8FAFC))', icon: Archive },
};
...
export default function ReleaseDetailPage() {
  ...
  return (
    <div className="th-page-content" style={{ flex: 1, overflow: 'auto' }}>
      {/* Header */}
      <div style={{ padding: '20px 32px', borderBottom: `1px solid ${'var(--cp-border, #E2E8F0)'}`, background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 }}>
          <StatCard label="Days Left" value={daysLeft !== null ? String(daysLeft) : '—'} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" isDark={isDark} />
          <StatCard label="Total Tests" value={String(release.test_cases_total || 0)} color={'var(--cp-text-secondary, #334155)'} isDark={isDark} />
          <StatCard label="Execution" value={`${execRate}%`} color={execRate >= 80 ? '#059669' : 'var(--ds-text-warning, var(--ds-text-warning, #D97706))'} isDark={isDark} />
          <StatCard label="Pass Rate" value={`${passRate}%`} color={passRate >= 80 ? '#059669' : 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))'} isDark={isDark} />
          <StatCard label="Open Defects" value={String(release.defects_open || 0)} color={release.defects_open > 0 ? 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' : '#059669'} isDark={isDark} />
          <StatCard label="Critical" value={String(release.critical_defects || 0)} color={release.critical_defects > 0 ? 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' : '#059669'} isDark={isDark} />
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${'var(--cp-border, #E2E8F0)'}`, padding: '0 32px', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' }}>
        {tabs.map(tab => {
          ...
          return (
            <button
              ...
              style={{
                ...
                color: active ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : ('var(--cp-text-tertiary, #64748B)'), background: 'none', cursor: 'pointer',
              }}
            >
        ...
        <div style={{ height: 10, backgroundColor: 'var(--cp-bg-sunken, #F1F5F9)', borderRadius: 6, overflow: 'hidden', marginBottom: 8 }}>
          {release.test_cases_total > 0 && (
            <div style={{
              ...
              <div style={{ width: `${(release.test_cases_passed / release.test_cases_total) * 100}%`, backgroundColor: 'var(--ds-text-success, var(--ds-text-success, #22C55E))' }} />
              <div style={{ width: `${((release.test_cases_failed || 0) / release.test_cases_total) * 100}%`, backgroundColor: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))' }} />
              <div style={{ width: `${((release.test_cases_blocked || 0) / release.test_cases_total) * 100}%`, backgroundColor: 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' }} />
            </div>
          )}
        </div>
...
function CyclesTab({ cycles, isLoading, navigate, isDark }: { cycles: any[]; isLoading: boolean; navigate: any; isDark: boolean }) {
  ...
  return (
    <div style={{ border: `1px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 12, overflow: 'hidden', background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' }}>
          ...
          {cycles.map(c => {
            ...
            return (
              ...
              >
                ...
                <td style={tdStyle}><span style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', fontWeight: 600 }}>{cycle.failed_count || 0}</span></td>
                <td style={tdStyle}><ChevronRight style={{ width: 14, height: 14, color: 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))' }} /></td>
              </tr>
            );
          })}
        </tbody>
    ...
    <div style={{ textAlign: 'center', padding: 48, color: 'var(--cp-text-muted, #94A3B8)' }}>
      ...
      <div style={{ display: 'flex', gap: 24, justifyContent: 'center', marginTop: 16 }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>{release.defects_open || 0}</div>
          <div style={{ fontSize: 12, color: 'var(--cp-text-tertiary, #64748B)' }}>Open</div>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>{release.critical_defects || 0}</div>
