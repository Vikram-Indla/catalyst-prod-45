...
export default function ReleaseDetailPage() {
  ...
  if (isLoading) {
    return <div style={{ padding: 60, textAlign: 'center', color: isDark ? '#6B6560' : '#94A3B8' }}>Loading release...</div>;
  }

  if (!release) {
    return <div style={{ padding: 60, textAlign: 'center', color: isDark ? '#6B6560' : '#94A3B8' }}>Release not found</div>;
  }
  ...
  return (
    <div className="th-page-content" style={{ flex: 1, overflow: 'auto' }}>
      {/* Header */}
      <div style={{ padding: '20px 32px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, background: isDark ? '#1A1714' : '#fff' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          <button
            onClick={() => navigate('/testhub/releases')}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#A09890' : '#64748B', display: 'flex', alignItems: 'center', gap: 6 }}
          >
            <ArrowLeft style={{ width: 18, height: 18 }} />
          </button>
          <span style={{ fontFamily: 'monospace', fontSize: 13, color: isDark ? '#6B6560' : '#94A3B8', fontWeight: 600 }}>{release.version}</span>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: 0, flex: 1 }}>{release.name}</h1>
          <span style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 }}>
          ...
          <StatCard label="Total Tests" value={String(release.test_cases_total || 0)} color={isDark ? '#A09890' : '#334155'} isDark={isDark} />
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, padding: '0 32px', background: isDark ? '#1A1714' : '#fff' }}>
        {tabs.map(tab => {
          ...
          return (
            <button
              ...
              style={{
                ...
                color: active ? '#2563EB' : (isDark ? '#A09890' : '#64748B'), background: 'none', cursor: 'pointer',
              }}
            >
...
function StatCard({ label, value, color, isDark }: { label: string; value: string; color: string; isDark: boolean }) {
  return (
    <div style={{ padding: '14px 16px', background: isDark ? '#232019' : '#F8FAFC', borderRadius: 10, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
      <div style={{ fontSize: 22, fontWeight: 700, color, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
      <div style={{ fontSize: 12, color: isDark ? '#A09890' : '#64748B', marginTop: 2 }}>{label}</div>
    </div>
  );
}
...
function OverviewTab({ release, isDark }: { release: any; isDark: boolean }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24 }}>
      <div>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 12 }}>Description</h3>
        <p style={{ fontSize: 14, color: isDark ? '#A09890' : '#334155', lineHeight: 1.7 }}>
          {release.description || 'No description provided.'}
        </p>

        <h3 style={{ fontSize: 15, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: '24px 0 12px' }}>Test Progress</h3>
        <div style={{ height: 10, backgroundColor: isDark ? '#232019' : '#F1F5F9', borderRadius: 5, overflow: 'hidden', marginBottom: 8 }}>
          {release.test_cases_total > 0 && (
            <div style={{
              display: 'flex', height: '100%',
            }}>
              <div style={{ width: `${(release.test_cases_passed / release.test_cases_total) * 100}%`, backgroundColor: '#22C55E' }} />
              <div style={{ width: `${((release.test_cases_failed || 0) / release.test_cases_total) * 100}%`, backgroundColor: '#EF4444' }} />
              <div style={{ width: `${((release.test_cases_blocked || 0) / release.test_cases_total) * 100}%`, backgroundColor: '#F59E0B' }} />
            </div>
          )}
        </div>
        <div style={{ display: 'flex', gap: 16, fontSize: 12, color: isDark ? '#A09890' : '#64748B' }}>
      ...
      {/* Details panel */}
      <div style={{ background: isDark ? '#232019' : '#F8FAFC', borderRadius: 10, padding: 20, border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 16 }}>Details</h3>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      <dt style={{ fontSize: 13, color: isDark ? '#A09890' : '#64748B' }}>{label}</dt>
      <dd style={{ fontSize: 13, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A', margin: 0 }}>{value}</dd>
    </div>
  );
}

function CyclesTab({ cycles, isLoading, navigate, isDark }: { cycles: any[]; isLoading: boolean; navigate: any; isDark: boolean }) {
  if (isLoading) return <div style={{ padding: 40, textAlign: 'center', color: isDark ? '#6B6560' : '#94A3B8' }}>Loading cycles...</div>;

  if (cycles.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: 48, color: isDark ? '#6B6560' : '#94A3B8' }}>
  ...
  return (
    <div style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}`, borderRadius: 10, overflow: 'hidden', background: isDark ? '#232019' : '#fff' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ backgroundColor: isDark ? '#2C2823' : '#F8FAFC', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : '#E2E8F0'}` }}>
        ...
        <tbody>
          {cycles.map(c => {
            ...
            return (
              ...
              >
                <td style={tdStyle}><span style={{ fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A' }}>{cycle.name}</span></td>
    ...
    <div style={{ textAlign: 'center', padding: 48, color: isDark ? '#6B6560' : '#94A3B8' }}>
      ...
      <div style={{ display: 'flex', gap: 24, justifyContent: 'center', marginTop: 16 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: isDark ? '#A09890' : '#64748B' }}>Open</div>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#DC2626' }}>{release.critical_defects || 0}</div>
          <div style={{ fontSize: 12, color: isDark ? '#A09890' : '#64748B' }}>Critical</div>
        </div>
      </div>
    </div>
  );
...
const getThStyle = (isDark: boolean): React.CSSProperties => ({
  ...
  color: isDark ? '#A09890' : '#64748B', textTransform: 'uppercase', letterSpacing: '0.04em',
});
