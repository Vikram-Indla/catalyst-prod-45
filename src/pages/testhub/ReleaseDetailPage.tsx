...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  planning: { label: 'Planning', color: 'var(--fg-3, #64748B)', bg: '#F1F5F9', icon: Clock },
  planned: { label: 'Planned', color: 'var(--fg-3, #64748B)', bg: '#F1F5F9', icon: Clock },
  ...
  archived: { label: 'Archived', color: 'var(--fg-4, #94A3B8)', bg: '#F8FAFC', icon: Archive },
};
...
const HEALTH_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  ...
  none: { label: '—', color: 'var(--fg-4, #94A3B8)', dot: '#CBD5E1' },
};
...
export default function ReleaseDetailPage() {
  ...
  if (isLoading) {
    return <div style={{ padding: 60, textAlign: 'center', color: 'var(--fg-4, #94A3B8)' }}>Loading release...</div>;
  }

  if (!release) {
    return <div style={{ padding: 60, textAlign: 'center', color: 'var(--fg-4, #94A3B8)' }}>Release not found</div>;
  }
  ...
  return (
    <div className="th-page-content" style={{ flex: 1, overflow: 'auto' }}>
      {/* Header */}
      <div style={{ padding: '20px 32px', borderBottom: '1px solid var(--divider, #E2E8F0)', background: '#fff' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          <button
            onClick={() => navigate('/testhub/releases')}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--fg-3, #64748B)', display: 'flex', alignItems: 'center', gap: 6 }}
          >
            <ArrowLeft style={{ width: 18, height: 18 }} />
          </button>
          <span style={{ fontFamily: 'monospace', fontSize: 13, color: 'var(--fg-4, #94A3B8)', fontWeight: 600 }}>{release.version}</span>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: 0, flex: 1 }}>{release.name}</h1>
          <span style={{
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--divider, #E2E8F0)', padding: '0 32px', background: '#fff' }}>
        {tabs.map(tab => {
...
function StatCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div style={{ padding: '14px 16px', background: 'var(--surface-subtle, #F8FAFC)', borderRadius: 10, border: '1px solid var(--divider, #E2E8F0)' }}>
      <div style={{ fontSize: 22, fontWeight: 700, color, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
      <div style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', marginTop: 2 }}>{label}</div>
    </div>
  );
}
...
function OverviewTab({ release }: { release: any }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24 }}>
      <div>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--fg-1, #0F172A)', marginBottom: 12 }}>Description</h3>
        <p style={{ fontSize: 14, color: '#334155', lineHeight: 1.7 }}>
          {release.description || 'No description provided.'}
        </p>

        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: '24px 0 12px' }}>Test Progress</h3>
        <div style={{ height: 10, backgroundColor: 'var(--surface-muted, #F1F5F9)', borderRadius: 5, overflow: 'hidden', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 16, fontSize: 12, color: 'var(--fg-3, #64748B)' }}>
      ...
      {/* Details panel */}
      <div style={{ background: 'var(--surface-subtle, #F8FAFC)', borderRadius: 10, padding: 20, border: '1px solid var(--divider, #E2E8F0)' }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--fg-1, #0F172A)', marginBottom: 16 }}>Details</h3>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      <dt style={{ fontSize: 13, color: 'var(--fg-3, #64748B)' }}>{label}</dt>
      <dd style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-1, #0F172A)', margin: 0 }}>{value}</dd>
    </div>
  );
}

function CyclesTab({ cycles, isLoading, navigate }: { cycles: any[]; isLoading: boolean; navigate: any }) {
  if (isLoading) return <div style={{ padding: 40, textAlign: 'center', color: 'var(--fg-4, #94A3B8)' }}>Loading cycles...</div>;

  if (cycles.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: 48, color: 'var(--fg-4, #94A3B8)' }}>
  ...
  return (
    <div style={{ border: '1px solid var(--divider, #E2E8F0)', borderRadius: 10, overflow: 'hidden', background: '#fff' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ backgroundColor: 'var(--surface-subtle, #F8FAFC)', borderBottom: '1px solid var(--divider, #E2E8F0)' }}>
        ...
        <tbody>
          {cycles.map(c => {
            ...
            return (
              ...
              >
                <td style={tdStyle}><span style={{ fontWeight: 600, color: 'var(--fg-1, #0F172A)' }}>{cycle.name}</span></td>
    ...
    <div style={{ textAlign: 'center', padding: 48, color: 'var(--fg-4, #94A3B8)' }}>
      ...
      <div style={{ display: 'flex', gap: 24, justifyContent: 'center', marginTop: 16 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--fg-3, #64748B)' }}>Open</div>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#DC2626' }}>{release.critical_defects || 0}</div>
          <div style={{ fontSize: 12, color: 'var(--fg-3, #64748B)' }}>Critical</div>
        </div>
      </div>
    </div>
  );
...
const thStyle: React.CSSProperties = {
  ...
  color: 'var(--fg-3, #64748B)', textTransform: 'uppercase', letterSpacing: '0.04em',
};
