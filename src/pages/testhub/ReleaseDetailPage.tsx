...
export default function ReleaseDetailPage() {
  ...
  return (
    <div className="th-page-content" style={{ flex: 1, overflow: 'auto' }}>
      {/* Header */}
      <div style={{ padding: '20px 32px', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface, #fff)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          ...
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0, flex: 1 }}>{release.name}</h1>
          <span style={{
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 0, borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, padding: '0 32px', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface, #fff)' }}>
        {tabs.map(tab => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24 }}>
      <div>
        ...
        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: '24px 0 12px' }}>Test Progress</h3>
      ...
      {/* Details panel */}
      <div style={{ background: 'var(--cp-bg-page, #F8FAFC)', borderRadius: 12, padding: 20, border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}` }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', marginBottom: 16 }}>Details</h3>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <dd style={{ fontSize: 13, fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0 }}>{value}</dd>
    </div>
  );
}
        ...
        <tbody>
          {cycles.map(c => {
            ...
            return (
              ...
              >
                <td style={tdStyle}><span style={{ fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))' }}>{cycle.name}</span></td>
