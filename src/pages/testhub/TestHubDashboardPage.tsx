...
export default function TestHubDashboardPage() {
  ...
  if (isLoading) {
    return (
      <div style={{ height: 'calc(100vh - 44px)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: isDark ? '#0A0A0A' : '#1A1A1A', padding: '12px 16px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, flexShrink: 0, marginBottom: 12 }}>
          {[...Array(5)].map((_, i) => (
            <div key={i} className="animate-pulse" style={{ height: 130, background: isDark ? '#1A1A1A' : 'rgba(255,255,255,0.10)', borderRadius: 8 }} />
          ))}
        </div>
        <div className="animate-pulse" style={{ height: 48, background: isDark ? '#1A1A1A' : 'rgba(255,255,255,0.10)', borderRadius: 8, marginBottom: 12, flexShrink: 0 }} />
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 320px', gap: 12, flex: 1, minHeight: 0 }}>
          {[...Array(3)].map((_, i) => (
            <div key={i} className="animate-pulse" style={{ height: '100%', background: isDark ? '#1A1A1A' : 'rgba(255,255,255,0.10)', borderRadius: 8 }} />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      background: isDark ? '#0A0A0A' : '#1A1A1A', fontFamily: 'Inter, system-ui, sans-serif',
    }}>

      {/* ═══ PAGE HEADER — 64px ═══ */}
      <TestHubPageHeader title="TestHub Dashboard" subtitle="Test execution metrics and activity">
          <span style={{ fontSize: 12, fontWeight: 400, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
            Updated {formatLastUpdated()}
          </span>
          <button onClick={fetchDashboardData} style={{
            display: 'inline-flex', alignItems: 'center', gap: 4,
            height: 32, padding: '8px 12px', fontSize: 13, fontWeight: 500,
            color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}`,
      ...
      <div style={{ flex: 1, minHeight: 0, padding: '12px 16px', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{
          background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}`, borderRadius: 8,
          padding: '10px 16px', flexShrink: 0,
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)' }}>Execution status</span>
            <span style={{ fontSize: 14, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{totalAll} total</span>
          </div>
          <div style={{ display: 'flex', height: 8, borderRadius: 4, background: isDark ? '#1A1A1A' : '#1A1A1A', gap: 1, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 6 }}>
            {execSegments.map(seg => (
              <span key={seg.label} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 500, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)' }}>{seg.label}</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{seg.count}</span>
              </span>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 320px', gap: 12, flex: 1, minHeight: 0 }}>
          ...
          <Card title="Active Cycles" badge={String(activeCycles.length)} onViewAll={() => navigate('/testhub/cycles')}>
                   ...
                   return (
                     <div key={cycle.id} onClick={() => navigate(`/testhub/cycles/${cycle.id}`)}
                       ...
                       style={{
                         ...
                         cursor: 'pointer', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}`,
                       }}>
                       <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, fontWeight: 600, color: '#2563EB' }}>
                         {cycle.cycle_key}
                       </span>
                       <span style={{ fontSize: 14, fontWeight: 500, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                         {cycle.name}
                       </span>
                       <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                         <div style={{ flex: 1, height: 4, borderRadius: 4, background: isDark ? '#1A1A1A' : '#1A1A1A', overflow: 'hidden' }}>
                           <div style={{ width: `${pct}%`, height: '100%', background: barColor, borderRadius: 4, transition: 'width 400ms ease-out' }} />
                         </div>
                         <span style={{ fontSize: 13, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', minWidth: 32, textAlign: 'right' }}>{pct}%</span>
                       </div>
           ...
           <div style={{
             ...
             background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}`, borderRadius: 8, overflow: 'hidden',
           }}>
             {/* Header: Top Failing Tests */}
             <div style={{
               padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
               borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}`, flexShrink: 0,
             }}>
               <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                 <span style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>Top Failing Tests</span>
                 <span style={{ fontSize: 12, fontWeight: 600, color: '#DC2626', background: isDark ? 'rgba(248,113,113,0.12)' : 'rgba(248,113,113,0.06)', padding: '2px 8px', borderRadius: 10 }}>
                   {failingTests.length}
                 </span>
               </div>
                 ...
                 <>
                   {failingTests.slice(0, 10).map(test => {
                     const sevColor = test.priority?.toLowerCase() === 'high' || test.priority?.toLowerCase() === 'critical'
                       ? { bg: isDark ? 'rgba(248,113,113,0.12)' : 'rgba(248,113,113,0.06)', color: '#DC2626' }
             ...
             <div style={{ borderTop: `1px dashed ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}`, flexShrink: 0 }} />
             ...
             <div style={{ flexShrink: 0 }}>
               <div style={{
                 ...
                 <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                   <span style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>Defects</span>
                   <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', background: isDark ? '#1A1A1A' : '#1A1A1A', padding: '2px 8px', borderRadius: 10 }}>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
                 {[
                   ...
                   <div key={d.label} style={{
                     ...
                     borderRight: i < 3 ? `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}` : 'none',
                   }}>
                     <div style={{ fontSize: 20, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', lineHeight: 1 }}>{d.value}</div>
                     <div style={{ fontSize: 12, fontWeight: 500, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', marginTop: 4 }}>{d.label}</div>
                   </div>
                 ))}
               </div>
             </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minHeight: 0 }}>
            ...
            <div style={{
              ...
              <div style={{ padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>Needs attention</span>
                <span style={{
                  fontSize: 12, fontWeight: 600, color: '#DC2626', background: isDark ? 'rgba(248,113,113,0.12)' : 'rgba(248,113,113,0.06)',
              ...
              {attentionItems.length === 0 ? (
                <div style={{ padding: '16px', textAlign: 'center', fontSize: 13, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
                  All clear — no issues
                </div>
              ) : (
                attentionItems.map((item, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderTop: i > 0 ? `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}` : 'none' }}>
                    <div style={{
                      width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      background: item.danger ? (isDark ? 'rgba(248,113,113,0.12)' : 'rgba(248,113,113,0.06)') : (isDark ? 'rgba(251,191,36,0.12)' : '#FFFBEB'),
                    }}>
                      <AlertTriangle size={12} color={item.danger ? '#DC2626' : '#D97706'} />
                    </div>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 500, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{item.title}</div>
                      <div style={{ fontSize: 12, fontWeight: 400, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>{item.meta}</div>
                    </div>
                  </div>
                ))
              )}
            ...
            <div style={{
              ...
              <div style={{ padding: '12px 16px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}`, flexShrink: 0 }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>Activity</span>
              </div>
               <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                 {recentActivity.length === 0 ? (
                   <div style={{ padding: '24px 16px', textAlign: 'center', fontSize: 13, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
                   ...
                   <>
                     {recentActivity.slice(0, 10).map((a, i) => {
                       ...
                       return (
                         <div key={a.id} style={{
                           ...
                           padding: '8px 16px', borderTop: i > 0 ? `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}` : 'none',
                         }}>
                           <span style={{ width: 7, height: 7, borderRadius: '50%', background: dotColor, marginTop: 6, flexShrink: 0 }} />
                           <div style={{ flex: 1, minWidth: 0 }}>
                             <div style={{ fontSize: 13, fontWeight: 400, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', lineHeight: 1.43 }}>
                             ...
                             <div style={{ fontSize: 11, fontWeight: 400, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
                     ...
                     {recentActivity.length < 3 && (
                       <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px', fontSize: 12, fontWeight: 400, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
               ...
               <div style={{ display: 'flex', gap: 8, padding: '10px 16px', paddingRight: '80px', borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}`, flexShrink: 0 }}>
      ...
      <style>{`
        .c10-row:hover { background: ${isDark ? '#1A1A1A' : '#1A1A1A'}; }
        .c10-row-danger:hover { background: ${isDark ? 'rgba(248,113,113,0.12)' : 'rgba(248,113,113,0.06)'}; }
        .c10-kpi:hover { border-color: ${isDark ? 'rgba(255,255,255,0.15)' : 'rgba(237,237,237,0.53)'} !important; box-shadow: 0 4px 16px ${isDark ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.08)'} !important; }
        .c10-qbtn:hover { border-color: #2563EB !important; color: #2563EB !important; background: ${isDark ? 'rgba(37,99,235,0.12)' : 'rgba(59,130,246,0.06)'} !important; }
      `}</style>
    </div>
  );
}
...
function KPICard({ label, value, accent, trend, subtitle, sparkData, valueColor, isDanger }: {
  ...
  return (
    <div className="c10-kpi" style={{
      ...
      border: isDanger ? `1px solid ${isDark ? 'rgba(248,113,113,0.2)' : '#FECACA'}` : `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}`,
      borderRadius: 8,
      borderLeft: `3px solid ${accent}`,
      padding: 16, cursor: 'pointer',
      transition: 'border-color 150ms, box-shadow 150ms',
    }}>
      <div style={{ fontSize: 13, fontWeight: 500, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)', marginBottom: 4 }}>{label}</div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <span style={{ fontSize: 36, fontWeight: 700, color: valueColor || (isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)'), letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>
          {value}
        </span>
      ...
      <div style={{ fontSize: 12, fontWeight: 400, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', marginTop: 4 }}>{subtitle}</div>
    </div>
  );
}
...
function Card({ title, badge, badgeDanger, badgeColor, onViewAll, children, style }: {
  ...
  const { isDark } = useTheme();
  const bg = badgeDanger ? (isDark ? 'rgba(248,113,113,0.12)' : 'rgba(248,113,113,0.06)') : (isDark ? '#1A1A1A' : '#1A1A1A');
  const color = badgeDanger ? '#DC2626' : badgeColor || (isDark ? '#878787' : 'rgba(237,237,237,0.40)');
  return (
    <div style={{ background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}`, borderRadius: 8, overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: 0, ...style }}>
      <div style={{ padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : '#1A1A1A'}`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{title}</span>
          {badge && (
...
function EmptyMini({ icon, text }: { icon: React.ReactNode; text: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 0', gap: 6 }}>
      {icon}
      <span style={{ fontSize: 13, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>{text}</span>
    </div>
  );
}
...
function QuickBtn({ label, onClick }: { label: string; onClick: () => void }) {
  ...
  return (
    <button className="c10-qbtn" onClick={onClick} style={{
      ...
      color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)', background: isDark ? '#1A1A1A' : '#FFFFFF', border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)'}`,
