...
export default function TestHubVerifyPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        {lastRun && (
          <span style={{ fontSize: 13, color: 'var(--ds-text-subtlest, #94A3B8)', fontFamily: 'var(--cp-font-body)' }}>
            Last run: {lastRun}
          </span>
        )}
        <button
                  ...
                  <>
                    ...
                    >
                      ...
                      <td style={{ padding: '8px 12px', fontSize: 13, color: '#1E293B' }}>
                        <span style={{ color: 'var(--ds-text-subtlest, #94A3B8)', fontFamily: 'var(--cp-font-mono)', fontSize: 11, marginRight: 8 }}>{check.id}</span>
                        {check.label}
                      </td>
                      <td style={{ padding: '8px 12px', fontSize: 12, color: 'var(--ds-text-subtlest, #64748B)', fontFamily: 'var(--cp-font-mono)' }}>
                        {check.expected}
                      </td>
  ...
  return (
    <>
        ...
        {MODULE_DEFS.map(mod => {
          ...
          return (
            <div key={mod.key} style={{
              ...
              <div style={{ fontSize: 12, color: 'var(--ds-text-subtlest, #94A3B8)', marginBottom: 8 }}>
              ...
              {mod.key === 'testAssets' && (
                <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {['A1', 'A2', 'A3'].map(id => {
                    const c = checks.find(x => x.id === id);
                    return (
                      <div key={id} style={{ fontSize: 11, color: 'var(--ds-text-subtlest, #64748B)', fontFamily: 'var(--cp-font-mono)' }}>
                        {c?.label}: {c?.actual ?? '—'}
                      </div>
                    );
                  })}
              ...
              {mod.note && s.pct < 100 && (
                <div style={{ marginTop: 6, fontSize: 11, fontStyle: 'italic', color: 'var(--ds-text-subtlest, #94A3B8)' }}>
                  {mod.note}
                </div>
              )}
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 32, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 12, color: 'var(--ds-text-subtlest, #94A3B8)', marginTop: 8 }}>Overall score</span>
        <span style={{ fontSize: 11, color: 'var(--ds-text-subtlest, #94A3B8)' }}>{overallPassed}/20 checks passed</span>
      </div>
    </>
  );
}
