    ...
    <div style={{ fontFamily: 'Inter, sans-serif', color: isDark ? '#F5F3F0' : '#0F172A', background: isDark ? '#1A1714' : '#F8FAFC', minHeight: '100%', padding: '24px 40px 48px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 12 }}>
        {isLoading ? Array.from({ length: 3 }).map((_, i) => (
          <div key={i} style={{ padding: 24, borderRadius: 8, background: isDark ? '#232019' : '#FFFFFF', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(0,0,0,0.06)', height: 160 }} />
        )) : (paths ?? []).length === 0 ? (
          ...
          return (
            <div key={p.id} style={{ padding: 24, borderRadius: 8, background: isDark ? '#232019' : '#FFFFFF', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(0,0,0,0.06)', transition: 'border-color 120ms' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: 'Sora, sans-serif', fontSize: 14, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A' }}>{p.title}</div>
              ...
              <div style={{ height: 4, borderRadius: 2, background: isDark ? '#3A3530' : '#E2E8F0', marginBottom: 8 }}>
