  ...
  return (
    <div style={{ fontFamily: 'var(--cp-font-body)', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--ds-text, var(--ds-text, #0F172A))', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F8FAFC))', minHeight: '100%', padding: '24px 40px 48px' }}>
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', fontWeight: 600 }}>All Articles</span>
      </nav>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        {selected.size > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>{selected.size} selected</span>
            {[
              ...
              <button key={a.key} onClick={() => { setBulkAction(a.key); if (a.key === 'archive') setShowConfirm(true); }} style={{
                ...
                border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.12)', background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))',
                color: a.key === 'archive' ? 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' : (isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))'), cursor: 'pointer',
      ...
      {bulkAction && bulkAction !== 'archive' && (
        <div style={{
          marginBottom: 16, padding: '12px 16px', borderRadius: 8, background: isDark ? 'rgba(59,130,246,0.12)' : 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))',
          border: '1px solid rgba(37,99,235,0.2)', display: 'flex', alignItems: 'center', gap: 12,
        }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>
          ...
          {bulkAction === 'verification' ? (
            <select value={bulkValue} onChange={e => setBulkValue(e.target.value)} style={{
              ...
              background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--ds-text, var(--ds-text, #0F172A))',
            }}>
            ...
            <input value={bulkValue} onChange={e => setBulkValue(e.target.value)} placeholder={bulkAction === 'domain' ? 'e.g. D1' : 'tag1, tag2'}
              style={{ fontSize: 12, padding: '4px 8px', borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid #CBD5E1', width: 200, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--ds-text, var(--ds-text, #0F172A))' }} />
          )}
          <button onClick={() => setShowConfirm(true)} disabled={!bulkValue} style={{
            ...
            background: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', cursor: 'pointer', opacity: bulkValue ? 1 : 0.5,
          ...
          <button onClick={() => { setBulkAction(null); setBulkValue(''); }} style={{
            ...
            background: 'transparent', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', cursor: 'pointer',
          }}><X size={14} /></button>
        </div>
      )}
      ...
      {showConfirm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative', background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', borderRadius: 12, padding: 24, width: 400, boxShadow: '0 12px 40px rgba(0,0,0,0.15)' }}>
            <h3 style={{ fontFamily: F.sora, fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Confirm Bulk Action</h3>
            <p style={{ fontSize: 13, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => setShowConfirm(false)} style={{
                ...
                border: isDark ? '1px solid #2E2E2E' : '1px solid #E2E8F0', background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))', cursor: 'pointer',
              }}>Cancel</button>
              <button onClick={executeBulk} style={{
                fontSize: 12, fontWeight: 600, padding: '8px 16px', borderRadius: 6, border: 'none',
                background: bulkAction === 'archive' ? 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' : 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', cursor: 'pointer',
              }}>Confirm</button>
            </div>
          </div>
        </div>
      ...
      {/* Table */}
      <div style={{ borderRadius: 8, border: `0.75px solid ${borderColor}`, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', overflow: 'hidden' }}>
        <div style={{
          display: 'grid', gridTemplateColumns: '32px 3% 1fr 80px 100px 80px 80px 100px 50px',
          background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))', padding: '8px 12px', height: 50, alignItems: 'center',
          fontFamily: F.sora, fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const,
          color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', letterSpacing: '0.05em', borderBottom: `0.75px solid ${borderColor}`,
        }}>
          <span>
            <input type="checkbox" checked={articles?.length ? selected.size === articles.length : false} onChange={toggleAll}
              style={{ width: 14, height: 14, cursor: 'pointer', accentColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} />
          </span>
        ...
        {isLoading ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', fontSize: 12 }}>Loading...</div> :
          (articles ?? []).length === 0 ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', fontSize: 12 }}>No articles found.</div> :
          (articles ?? []).map((a: any) => {
            ...
            const verBadge = verStatus === 'verified' ? { bg: 'rgba(22,163,74,0.08)', color: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', label: 'Verified' }
              : verStatus === 'needs_review' ? { bg: 'rgba(217,119,6,0.08)', color: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', label: 'Review' }
              : { bg: 'rgba(100,116,139,0.08)', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', label: 'Unverified' };
            ...
            return (
              <div key={a.id} style={{
              ...
              }} onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'rgba(15,23,42,0.04)'; }}
                ...
                <span onClick={() => navigate(`/wiki/${a.slug}`)}>
                  {a.format === 'pdf' ? <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 4px', borderRadius: 4, background: isDark ? 'rgba(220,38,38,0.12)' : '#FEE2E2', color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>PDF</span> : <FileText size={14} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }} />}
                </span>
                ...
                <span style={{ fontSize: 10, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>{a.domain_code}</span>
                ...
                <span style={{ fontFamily: F.mono, fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>{a.view_count ?? 0}</span>
                <span style={{ fontFamily: F.mono, fontSize: 10, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>{new Date(a.updated_at).toLocaleDateString()}</span>
                <span style={{ fontFamily: F.mono, fontSize: 10, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>v{a.version ?? 1}</span>
              </div>
            );
          })}
      </div>
