  ...
  return (
    <div style={{ fontFamily: 'Geist, -apple-system, sans-serif', color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', background: isDark ? '#0A0A0A' : '#1A1A1A', minHeight: '100%', padding: '24px 40px 48px' }}>
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)', fontWeight: 600 }}>All Articles</span>
      </nav>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {[
              ...
              <button key={a.key} onClick={() => { setBulkAction(a.key); if (a.key === 'archive') setShowConfirm(true); }} style={{
                ...
                color: a.key === 'archive' ? '#DC2626' : (isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)'), cursor: 'pointer',
      ...
      {bulkAction && bulkAction !== 'archive' && (
        <div style={{
          marginBottom: 16, padding: '12px 16px', borderRadius: 8, background: isDark ? 'rgba(59,130,246,0.12)' : 'rgba(59,130,246,0.06)',
          ...
          {bulkAction === 'verification' ? (
            <select value={bulkValue} onChange={e => setBulkValue(e.target.value)} style={{
              fontSize: 12, padding: '4px 8px', borderRadius: 4, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(237,237,237,0.53)',
              background: isDark ? '#1A1A1A' : '#FFFFFF', color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)',
            }}>
            ...
            <input value={bulkValue} onChange={e => setBulkValue(e.target.value)} placeholder={bulkAction === 'domain' ? 'e.g. D1' : 'tag1, tag2'}
              style={{ fontSize: 12, padding: '4px 8px', borderRadius: 4, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(237,237,237,0.53)', width: 200, background: isDark ? '#1A1A1A' : '#FFFFFF', color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }} />
          )}
          ...
          <button onClick={() => { setBulkAction(null); setBulkValue(''); }} style={{
            ...
            background: 'transparent', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', cursor: 'pointer',
          }}><X size={14} /></button>
        </div>
      )}
      ...
      {showConfirm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative', background: isDark ? '#1A1A1A' : '#FFFFFF', borderRadius: 12, padding: 24, width: 400, boxShadow: '0 12px 40px rgba(0,0,0,0.15)' }}>
            ...
            <p style={{ fontSize: 13, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => setShowConfirm(false)} style={{
                ...
                border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(255,255,255,0.10)', background: isDark ? '#1A1A1A' : '#FFFFFF', color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)', cursor: 'pointer',
              }}>Cancel</button>
      ...
      <div style={{ borderRadius: 8, border: `0.75px solid ${borderColor}`, background: isDark ? '#1A1A1A' : '#FFFFFF', overflow: 'hidden' }}>
        <div style={{
          ...
          background: isDark ? '#1A1A1A' : '#1A1A1A', padding: '8px 12px', height: 50, alignItems: 'center',
          fontFamily: F.sora, fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const,
          color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', letterSpacing: '0.05em', borderBottom: `0.75px solid ${borderColor}`,
        }}>
          <span>
        ...
        {isLoading ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', fontSize: 12 }}>Loading...</div> :
          (articles ?? []).length === 0 ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', fontSize: 12 }}>No articles found.</div> :
          (articles ?? []).map((a: any) => {
            ...
            const verBadge = verStatus === 'verified' ? { bg: 'rgba(22,163,74,0.08)', color: '#16A34A', label: 'Verified' }
              ...
              : { bg: 'rgba(100,116,139,0.08)', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', label: 'Unverified' };
            ...
            return (
              <div key={a.id} style={{
                ...
                <span onClick={() => navigate(`/wiki/${a.slug}`)}>
                  {a.format === 'pdf' ? <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 4px', borderRadius: 4, background: isDark ? 'rgba(220,38,38,0.12)' : 'rgba(248,113,113,0.10)', color: '#DC2626' }}>PDF</span> : <FileText size={14} style={{ color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }} />}
                </span>
                ...
                <span style={{ fontFamily: F.mono, fontSize: 11, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>{a.view_count ?? 0}</span>
                <span style={{ fontFamily: F.mono, fontSize: 10, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>{new Date(a.updated_at).toLocaleDateString()}</span>
                <span style={{ fontFamily: F.mono, fontSize: 10, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.40)' }}>v{a.version ?? 1}</span>
              </div>
            );
          })}
      </div>
