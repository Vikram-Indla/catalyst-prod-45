...
const SectionHeader = React.memo(({ title, count, rightLabel, rightAction, isDark }: {
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
    ...
    <span style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{title}</span>
    {count !== undefined && (
      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: isDark ? '#1A1A1A' : '#F1F5F9', color: isDark ? '#878787' : '#64748B' }}>{count}</span>
    )}
    {rightLabel && (
    ...
    <div style={{ fontFamily: 'var(--ds-font-family-body)', color: isDark ? '#EDEDED' : '#0F172A', background: isDark ? '#0A0A0A' : '#F8FAFC', minHeight: '100%' }}>
      ...
      <div style={{ background: isDark ? '#1A1A1A' : '#FFFFFF', position: 'relative', overflow: 'hidden', borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(15,23,42,0.08)' }}>
        ...
        <div style={{ padding: '48px 40px 40px', display: 'flex', gap: 40, alignItems: 'flex-start', position: 'relative', zIndex: 1, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 320, maxWidth: 640 }}>
            <h1 style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 18, fontWeight: 700, margin: '0 0 4px', color: isDark ? '#EDEDED' : '#0F172A' }}>WikiHub</h1>
      ...
      <div style={{ padding: '24px 40px 48px' }}>
        ...
        >
          ...
          <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF', color: '#2563EB' }}>
        ...
        <div style={{ borderRadius: 8, border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', background: isDark ? '#1A1A1A' : '#FFFFFF', overflow: 'hidden', marginBottom: 40 }}>
          <div style={{
            ...
            fontFamily: 'var(--ds-font-family-heading)', fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const, color: isDark ? '#878787' : '#64748B', letterSpacing: '0.05em',
          }}>
...
const StatCard = React.memo(({ label, value, valueColor, isDark }: { label: string; value: string | number; valueColor?: string; isDark?: boolean }) => (
  ...
  >
    <div style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 28, fontWeight: 700, color: valueColor || (isDark ? '#EDEDED' : '#0F172A') }}>{value}</div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 12, fontSize: 10 }}>
        <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', color: '#2563EB', fontWeight: 500 }}>{qr.steps} steps</span>
        <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', color: isDark ? '#878787' : '#64748B', fontWeight: 500 }}>{formatK(qr.view_count ?? qr.views ?? 0)} views</span>
      </div>
    </div>
  );
});
    ...
    >
      ...
      <div style={{ flex: 1, padding: '16px 16px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 10, fontWeight: 700, color: isDark ? '#878787' : '#64748B', background: isDark ? '#1A1A1A' : '#F1F5F9', padding: '1px 5px', borderRadius: 3 }}>{d.domain_code}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 12.5, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{d.name}</div>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginBottom: 10 }}>
          {[{ v: d.article_count, l: 'Articles' }, { v: formatK(d.view_count), l: 'Views' }, { v: d.knowledge_gaps, l: 'Gaps' }].map(s => (
            <div key={s.l} style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 13, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{s.v}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 10, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{d.coverage_percent}%</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 12.5, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{p.title}</div>
        </div>
...
const ArticleRow = React.memo(({ a, navigate, bookmarked, onToggleBookmark, isDark }: { a: any; navigate: any; bookmarked: boolean; onToggleBookmark: () => void; isDark?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, fontWeight: 500, color: confColor }}>{conf}%</span>

      {/* Helpful */}
      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11, color: isDark ? '#878787' : '#64748B' }}>{Math.round(a.helpfulness_score ?? 0)}%</span>
      ...
      {/* Version */}
      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 10, color: isDark ? '#878787' : '#64748B' }}>v{a.version ?? 1}</span>
