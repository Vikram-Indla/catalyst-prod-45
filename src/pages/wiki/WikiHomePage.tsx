...
const Skeleton = ({ w, h, style, isDark }: { w: string | number; h: number; style?: React.CSSProperties; isDark?: boolean }) => (
  <div style={{ width: w, height: h, borderRadius: 4, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#E2E8F0', animation: 'pulse 1.5s ease-in-out infinite', ...style }} />
);
...
const SectionHeader = React.memo(({ title, count, rightLabel, rightAction, isDark }: {
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
    ...
    <span style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{title}</span>
    {count !== undefined && (
      <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#F1F5F9', color: isDark ? '#878787' : '#64748B' }}>{count}</span>
    )}
    {rightLabel && (
...
export default function WikiHomePage() {
  ...
  return (
    <div style={{ fontFamily: 'var(--cp-font-body)', color: isDark ? '#EDEDED' : '#0F172A', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : '#F8FAFC', minHeight: '100%' }}>
      ...
      {/* ═══ HERO ═══ */}
      <div style={{ background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', position: 'relative', overflow: 'hidden', borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(15,23,42,0.08)' }}>
        ...
        <div style={{ padding: '48px 40px 40px', display: 'flex', gap: 40, alignItems: 'flex-start', position: 'relative', zIndex: 1, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 320, maxWidth: 640 }}>
            <h1 style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 18, fontWeight: 700, margin: '0 0 4px', color: isDark ? '#EDEDED' : '#0F172A' }}>WikiHub</h1>
            ...
            <div onClick={() => setCmdOpen(true)} tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') setCmdOpen(true); }} style={{
              ...
              background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', transition: 'border-color 150ms, box-shadow 150ms', marginBottom: 12, height: 40,
            }}
              ...
              {chips.map(c => {
                ...
                return (
                  <button key={c.label} onClick={() => setActiveChip(c.label)} style={{
                    ...
                    border: active ? '1.5px solid #2563EB' : isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', background: active ? (isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF') : (isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF'), color: active ? '#2563EB' : (isDark ? '#878787' : '#64748B'), transition: 'all 120ms',
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setUploadOpen(true)} style={{ fontSize: 12, fontWeight: 650, padding: '8px 16px', borderRadius: 6, cursor: 'pointer', border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', color: isDark ? '#A1A1A1' : '#334155', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, minWidth: 260 }}>
            {statsLoading ? Array.from({ length: 4 }).map((_, i) => (
              <div key={i} style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#F8FAFC', border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ padding: '24px 40px 48px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 8, marginBottom: 40, scrollSnapType: 'x mandatory' }}>
          {qrLoading ? Array.from({ length: 6 }).map((_, i) => (
            <div key={i} style={{ minWidth: 190, maxWidth: 190, padding: 16, borderRadius: 8, border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF' }}>
        ...
        <div onClick={() => setKrFormOpen(true)} style={{
          ...
          background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: '0.75px solid rgba(37,99,235,0.2)', marginBottom: 40, flexWrap: 'wrap',
          cursor: 'pointer', transition: 'border-color 150ms, background 150ms',
        }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = '#2563EB'; e.currentTarget.style.background = isDark ? 'var(--cp-bg-surface, #242528)' : '#F0F5FF'; }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(37,99,235,0.2)'; e.currentTarget.style.background = isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF'; }}
        >
          ...
          <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF', color: '#2563EB' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 10, marginBottom: 40 }}>
          {domainsLoading ? Array.from({ length: 6 }).map((_, i) => (
            <div key={i} style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ borderRadius: 8, border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', overflow: 'hidden', marginBottom: 40 }}>
          <div style={{
            display: 'grid', gridTemplateColumns: '3% 34% 8% 12% 8% 8% 14% 5% 3%',
            background: isDark ? 'var(--cp-bg-surface, #242528)' : '#F1F5F9', padding: '8px 12px', height: 50, alignItems: 'center', borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)',
            fontFamily: 'var(--cp-font-heading)', fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const, color: isDark ? '#878787' : '#64748B', letterSpacing: '0.05em',
          }}>
...
const StatCard = React.memo(({ label, value, valueColor, isDark }: { label: string; value: string | number; valueColor?: string; isDark?: boolean }) => (
  <div style={{
    padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#F8FAFC', textAlign: 'center',
    ...
    onMouseLeave={e => { e.currentTarget.style.borderColor = isDark ? '#2E2E2E' : 'rgba(0,0,0,0.06)'; e.currentTarget.style.background = isDark ? 'var(--cp-bg-surface, #242528)' : '#F8FAFC'; }}
  >
    <div style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 28, fontWeight: 700, color: valueColor || (isDark ? '#EDEDED' : '#0F172A') }}>{value}</div>
...
const QuickRefCard = React.memo(({ qr, onClick, isDark }: { qr: any; onClick: () => void; isDark?: boolean }) => {
  ...
  return (
    <div onClick={onClick} style={{
      minWidth: 190, maxWidth: 190, padding: 16, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF',
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 12, fontSize: 10 }}>
        <span style={{ fontFamily: 'var(--cp-font-mono)', color: '#2563EB', fontWeight: 500 }}>{qr.steps} steps</span>
        <span style={{ fontFamily: 'var(--cp-font-mono)', color: isDark ? '#878787' : '#64748B', fontWeight: 500 }}>{formatK(qr.view_count ?? qr.views ?? 0)} views</span>
      </div>
    </div>
  );
});
...
const DomainCard = React.memo(({ d, Icon, navigate, isDark }: { d: any; Icon: React.ComponentType<any>; navigate: any; isDark?: boolean }) => {
  ...
  return (
    <div onClick={() => navigate(`/wiki/domains/${d.domain_code}`)} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display: 'flex', borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', cursor: 'pointer', border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', overflow: 'hidden', transition: 'border-color 120ms, box-shadow 120ms', borderColor: hovered ? '#2563EB' : (isDark ? '#2E2E2E' : 'rgba(0,0,0,0.06)'), boxShadow: hovered ? '0 2px 8px rgba(37,99,235,0.08)' : 'none' }}
    >
      ...
      <div style={{ flex: 1, padding: '16px 16px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 10, fontWeight: 700, color: isDark ? '#878787' : '#64748B', background: isDark ? 'var(--cp-bg-surface, #242528)' : '#F1F5F9', padding: '1px 5px', borderRadius: 3 }}>{d.domain_code}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 12.5, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{d.name}</div>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginBottom: 10 }}>
          {[{ v: d.article_count, l: 'Articles' }, { v: formatK(d.view_count), l: 'Views' }, { v: d.knowledge_gaps, l: 'Gaps' }].map(s => (
            <div key={s.l} style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 13, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{s.v}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ flex: 1, height: 3, borderRadius: 4, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#E2E8F0' }}>
            <div style={{ height: '100%', borderRadius: 4, background: coverageColor, width: `${d.coverage_percent}%`, transition: 'width 600ms' }} />
          </div>
          <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 10, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{d.coverage_percent}%</span>
        </div>
      </div>
    </div>
  );
...
const LearningPathCard = React.memo(({ p, navigate, isDark }: { p: any; navigate: any; isDark?: boolean }) => {
  ...
  return (
    <div onClick={() => navigate(`/wiki/learning-paths/${p.id}`)} style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#FFFFFF', border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', transition: 'border-color 120ms, box-shadow 120ms', cursor: 'pointer' }}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 12.5, fontWeight: 600, color: isDark ? '#EDEDED' : '#0F172A' }}>{p.title}</div>
        </div>
      ...
      <div style={{ height: 4, borderRadius: 4, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#E2E8F0', marginBottom: 8 }}>
...
const ArticleRow = React.memo(({ a, navigate, bookmarked, onToggleBookmark, isDark }: { a: any; navigate: any; bookmarked: boolean; onToggleBookmark: () => void; isDark?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      {/* Helpful */}
      <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 11, color: isDark ? '#878787' : '#64748B' }}>{Math.round(a.helpfulness_score ?? 0)}%</span>

      {/* Tags — grey pills */}
      <div style={{ display: 'flex', gap: 3, overflow: 'hidden' }}>
        {tags.map((t: string) => (
          <span key={t} style={{ fontSize: 9.5, fontWeight: 500, padding: '1px 6px', borderRadius: 4, background: isDark ? 'var(--cp-bg-surface, #242528)' : '#F1F5F9', color: isDark ? '#878787' : '#64748B', border: isDark ? '0.75px solid #292929' : '0.75px solid rgba(0,0,0,0.06)', whiteSpace: 'nowrap' }}>{t}</span>
        ))}
      </div>

      {/* Version */}
      <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 10, color: isDark ? '#878787' : '#64748B' }}>v{a.version ?? 1}</span>
