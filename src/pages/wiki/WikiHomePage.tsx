      ...
      <div style={{ textAlign: 'center', padding: '48px 24px 32px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
          <button onClick={() => setUploadOpen(true)} style={{
            ...
            background: 'var(--cp-primary-60)', color: '#fff', display: 'flex', alignItems: 'center', gap: 6,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12 }}>
          {(domains || []).map((d: any) => (
            ...
            >
              ...
              <div style={{ fontSize: 14, fontWeight: 650, color: 'var(--cp-text-primary)', marginBottom: 4 }}>{d.name}</div>
              <div style={{ fontSize: 12, color: 'var(--cp-text-tertiary)', lineHeight: 1.4, marginBottom: 8, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                {d.description}
              </div>
          ...
          {isLoading && Array.from({ length: 8 }).map((_, i) => (
            <div key={i} style={{ padding: 20, borderRadius: 6, border: '1px solid var(--cp-border-default)', height: 140, background: 'var(--cp-bg-sunken)', animation: 'pulse 1.5s infinite' }} />
          ))}
        </div>
