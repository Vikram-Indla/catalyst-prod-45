...
const Sk = ({ w, h, style, isDark }: { w: string | number; h: number; style?: React.CSSProperties; isDark?: boolean }) => (
  <div style={{
    width: w, height: h, borderRadius: 4, background: isDark ? '#2C2823' : '#E2E8F0',
...
function VersionHistoryPanel({ versions, onRestore, onClose }: {
  ...
  return (
    <div style={{
      ...
      background: isDark ? '#232019' : '#FFFFFF', borderLeft: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(15,23,42,0.12)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'Sora, sans-serif', fontSize: 14, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A' }}>Version History</span>
          <span style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0' }}>
        {versions.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center' }}>
            <History size={32} style={{ color: isDark ? '#3A3530' : '#E2E8F0', marginBottom: 12 }} />
            <div style={{ fontSize: 13, color: isDark ? '#6B6560' : '#94A3B8' }}>No version history yet</div>
          </div>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{
                  ...
                  color: i === 0 ? '#2563EB' : isDark ? '#A09890' : '#64748B',
                  padding: '2px 6px', borderRadius: 3,
                  background: i === 0 ? (isDark ? 'rgba(37,99,235,0.12)' : '#EFF6FF') : (isDark ? '#2C2823' : '#F1F5F9'),
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 4 }}>{v.title || 'Untitled'}</div>
            {v.change_summary && (
              <div style={{ fontSize: 11, color: isDark ? '#A09890' : '#64748B', marginBottom: 4, lineHeight: 1.5 }}>{v.change_summary}</div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 10, color: isDark ? '#6B6560' : '#94A3B8' }}>
...
function ExportDropdown({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{
      ...
      background: isDark ? '#2C2823' : '#FFFFFF', borderRadius: 6, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(15,23,42,0.12)',
      ...
      <button onClick={handlePdf} style={{
        ...
        color: isDark ? '#F5F3F0' : '#0F172A', background: 'transparent', border: 'none', cursor: 'pointer',
      ...
      <button onClick={handlePrint} style={{
        ...
        onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
      >
        <Printer size={13} style={{ color: isDark ? '#A09890' : '#64748B' }} /> Print
      </button>
    </div>
  );
}
...
function CrossModuleLinks({ links }: { links: any[] }) {
  ...
  return (
    <div style={{
      ...
      border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(15,23,42,0.12)', background: isDark ? '#232019' : '#FFFFFF',
    }}>
      <div style={{
        fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const,
        color: isDark ? '#A09890' : '#64748B', marginBottom: 10, letterSpacing: '0.04em',
...
export default function WikiArticlePage() {
  ...
  if (error || !page) {
    return (
      <div style={{ fontFamily: 'Inter, sans-serif', padding: 80, textAlign: 'center', background: isDark ? '#1A1714' : undefined, minHeight: '100%' }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>📄</div>
        <div style={{ fontFamily: 'Sora, sans-serif', fontSize: 18, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 8 }}>Article not found</div>
        <div style={{ fontSize: 13, color: isDark ? '#A09890' : '#64748B', marginBottom: 16 }}>The article you're looking for doesn't exist or has been removed.</div>
  ...
  const infoRows = [
    ...
    { label: 'Stories', value: info.totalStories ? (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ width: 40, height: 3, borderRadius: 2, background: isDark ? '#3A3530' : '#E2E8F0' }}>
    ...
    { label: 'Open Defects', value: info.openDefects != null ? (
      <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: info.openDefects > 0 ? '#DC2626' : (isDark ? '#A09890' : '#64748B') }}>{info.openDefects}</span>
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', color: isDark ? '#F5F3F0' : '#0F172A', background: isDark ? '#1A1714' : '#F8FAFC', minHeight: '100%' }}>
      ...
      <div style={{ padding: '20px 40px 48px' }}>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
          ...
          <ChevronRight size={12} style={{ color: isDark ? '#6B6560' : '#94A3B8' }} />
          <span style={{ fontSize: 13, color: isDark ? '#A09890' : '#64748B', fontWeight: 600 }}>{title}</span>
        </nav>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 32, alignItems: 'start' }}>
          ...
          <article>
            {/* Title */}
            <h1 style={{ fontFamily: 'Sora, sans-serif', fontSize: 28, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', margin: '0 0 12px' }}>{title}</h1>

            {/* Metadata row */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 8 }}>
              <span style={{ fontSize: 12, color: isDark ? '#A09890' : '#64748B' }}>{page.updated_at ? timeAgo(page.updated_at) : '—'}</span>
              {conf > 0 && (
                <span style={{
              ...
              {formatIcon}
              <span style={{ fontSize: 11, color: isDark ? '#A09890' : '#64748B', display: 'inline-flex', alignItems: 'center', gap: 3, fontFamily: 'JetBrains Mono, monospace' }}>
                <GitBranch size={11} /> v{page.version ?? 1}
              </span>
              {readTime && (
                <span style={{ fontSize: 11, color: isDark ? '#A09890' : '#64748B', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
              ...
              <button onClick={handleBookmark} style={{
                ...
                color: bookmarked ? '#D97706' : (isDark ? '#A09890' : '#64748B'), display: 'flex', alignItems: 'center', gap: 4,
              }}>
              ...
              <button onClick={() => setShowHistory(prev => !prev)} style={{
                ...
                color: showHistory ? '#2563EB' : (isDark ? '#A09890' : '#64748B'), display: 'flex', alignItems: 'center', gap: 4,
              }}>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={(e) => { e.stopPropagation(); setShowExport(prev => !prev); }} style={{
                  ...
                  color: showExport ? '#2563EB' : (isDark ? '#A09890' : '#64748B'), display: 'flex', alignItems: 'center', gap: 4,
                }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              {authorName ? (
                <>
                  <div style={{
                    width: 24, height: 24, borderRadius: '50%', background: isDark ? '#2C2823' : '#E2E8F0',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 10, fontWeight: 700, color: isDark ? '#A09890' : '#64748B',
                  }}>{authorName.charAt(0).toUpperCase()}</div>
                  <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#A09890' : '#334155' }}>{authorName}</span>
                </>
              ) : (
                <span style={{ fontSize: 12, color: isDark ? '#6B6560' : '#94A3B8' }}>No author</span>
              )}
              <span style={{
                fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 3,
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 20 }}>
                {tags.map(t => (
                  <span key={t} style={{
                    ...
                    background: isDark ? '#2C2823' : '#F1F5F9', color: isDark ? '#A09890' : '#64748B', fontWeight: 500,
                  }}>{t}</span>
                ))}
              </div>
            )}
            ...
            <div style={{
              ...
              {tldr ? (
                <div style={{ fontSize: 13, color: isDark ? '#A09890' : '#334155', lineHeight: 1.7, marginTop: 8 }}>{tldr}</div>
              ) : (
                <div style={{ fontSize: 12, color: isDark ? '#6B6560' : '#94A3B8', marginTop: 8, fontStyle: 'italic' }}>AI summary not yet generated</div>
              )}
            </div>
            ...
            {page.lead_content && (
              <div style={{
                ...
                color: isDark ? '#A09890' : '#334155', marginBottom: 24, paddingBottom: 20,
            ...
            {sections.length > 0 && (
              <nav style={{
                background: isDark ? '#232019' : '#F8FAFC', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(15,23,42,0.12)',
                borderRadius: 6, padding: 16, marginBottom: 28,
              }}>
                <div style={{
                  fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const,
                  color: isDark ? '#A09890' : '#64748B', marginBottom: 8, letterSpacing: '0.04em',
                }}>Contents</div>
                {sections.map((s: any, i: number) => (
                  <a key={s.id} href={`#section-${i}`} style={{
                    ...
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: isDark ? '#6B6560' : '#94A3B8', marginRight: 8 }}>{i + 1}.</span>
                    {s.title}
                  </a>
                ))}
              </nav>
            ...
            {sections.map((s: any, i: number) => (
              <div key={s.id} id={`section-${i}`} style={{ marginBottom: 32 }}>
                <h2 style={{
                  fontFamily: 'Sora, sans-serif', fontSize: 18, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A',
                  margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: 8,
                }}>
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 14, color: isDark ? '#6B6560' : '#94A3B8' }}>{i + 1}.</span>
                  {s.title}
                  {s.is_live_data && (
                    <span style={{
                ...
                {s.section_type === 'delivery_status' ? (
                  ...
                  <div style={{
                    fontSize: 15, lineHeight: 1.7, color: isDark ? '#A09890' : '#334155',
            ...
            {sections.length === 0 && (
              <div style={{ padding: 48, textAlign: 'center', color: isDark ? '#6B6560' : '#94A3B8', fontSize: 13 }}>
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 4 }}>Was this helpful?</div>
                <div style={{ fontSize: 11, color: isDark ? '#A09890' : '#64748B' }}>
            ...
            {(related ?? []).length > 0 && (
              <div style={{ marginTop: 40 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  ...
                  <span style={{ fontFamily: 'Sora, sans-serif', fontSize: 16, fontWeight: 650, color: isDark ? '#F5F3F0' : '#0F172A' }}>Related Articles</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
                  {(related ?? []).map((r: any) => (
                    <div key={r.id} onClick={() => navigate(`/wiki/${r.slug}`)} style={{
                      padding: 16, borderRadius: 6, background: isDark ? '#232019' : '#FFFFFF',
                    ...
                    >
                      <div style={{ fontSize: 13, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 6 }}>{r.title}</div>
                      <div style={{ fontSize: 11, color: isDark ? '#A09890' : '#64748B' }}>
                        {r.domain_code}
          ...
          <aside style={{
            ...
            {infoRows.map(row => (
              <div key={row.label} style={{
                ...
                <span style={{ color: isDark ? '#A09890' : '#64748B', fontSize: 11 }}>{row.label}</span>
                <span style={{ fontSize: 11, color: isDark ? '#F5F3F0' : '#0F172A', fontWeight: 500 }}>{row.value}</span>
              </div>
            ))}
          </aside>
        </div>
...
function DeliveryStatusSection({ content }: { content: string | null }) {
  const { isDark } = useTheme();
  if (!content) return <div style={{ fontSize: 13, color: isDark ? '#6B6560' : '#94A3B8' }}>No delivery data available.</div>;
  return (
    <div style={{
      borderRadius: 6, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(15,23,42,0.12)', overflow: 'hidden',
    }}>
      <div style={{ fontSize: 13, color: isDark ? '#A09890' : '#334155', padding: 16, lineHeight: 1.7 }}>{content}</div>
    </div>
  );
}
...
function ReferencesSection({ refs }: { refs: any[] }) {
  const { isDark } = useTheme();
  if (refs.length === 0) return <div style={{ fontSize: 12, color: isDark ? '#6B6560' : '#94A3B8', padding: 8 }}>No references.</div>;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {refs.map((r: any) => (
        <div key={r.ref_number ?? r.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0' }}>
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: isDark ? '#6B6560' : '#94A3B8', minWidth: 24 }}>[{r.ref_number}]</span>
          {r.source_type === 'jira' ? (
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color: isDark ? '#A09890' : '#334155' }}>{r.source_key}</span>
          )}
          <span style={{ fontSize: 12, color: isDark ? '#A09890' : '#64748B' }}>{r.description || '—'}</span>
        </div>
      ))}
    </div>
  );
...
function ArticleSkeleton() {
  const { isDark } = useTheme();
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', background: isDark ? '#1A1714' : '#F8FAFC', minHeight: '100%', padding: '20px 40px 48px' }}>
