...
function VersionHistoryPanel({ versions, onRestore, onClose }: {
  ...
  return (
    <div style={{
      ...
      background: 'var(--bg-app, #FFFFFF)', borderLeft: '1px solid rgba(15,23,42,0.12)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'Sora, sans-serif', fontSize: 14, fontWeight: 700, color: 'var(--fg-1, #0F172A)' }}>Version History</span>
          <span style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0' }}>
        {versions.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--fg-4, #94A3B8)' }}>No version history yet</div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--fg-1, #0F172A)', marginBottom: 4 }}>{v.title || 'Untitled'}</div>
            {v.change_summary && (
              <div style={{ fontSize: 11, color: 'var(--fg-3, #64748B)', marginBottom: 4, lineHeight: 1.5 }}>{v.change_summary}</div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 10, color: 'var(--fg-4, #94A3B8)' }}>
...
function ExportDropdown({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--bg-app, #FFFFFF)', borderRadius: 6, border: '1px solid rgba(15,23,42,0.12)',
      ...
      <button onClick={handlePdf} style={{
        ...
        color: 'var(--fg-1, #0F172A)', background: 'transparent', border: 'none', cursor: 'pointer',
      ...
      <button onClick={handlePrint} style={{
        ...
        onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
      >
        <Printer size={13} style={{ color: 'var(--fg-3, #64748B)' }} /> Print
      </button>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: 'var(--fg-3, #64748B)', marginBottom: 10, letterSpacing: '0.04em',
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {links.map((link: any) => {
          const mod = MODULE_COLORS[link.target_module] || { bg: '#F1F5F9', color: 'var(--fg-3, #64748B)', label: link.target_module };
          return (
  ...
  if (error || !page) {
    return (
      <div style={{ fontFamily: 'Inter, sans-serif', padding: 80, textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: 'Sora, sans-serif', fontSize: 18, fontWeight: 700, color: 'var(--fg-1, #0F172A)', marginBottom: 8 }}>Article not found</div>
        <div style={{ fontSize: 13, color: 'var(--fg-3, #64748B)', marginBottom: 16 }}>The article you're looking for doesn't exist or has been removed.</div>
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', color: 'var(--fg-1, #0F172A)', background: 'var(--surface-subtle, #F8FAFC)', minHeight: '100%' }}>
      ...
      <div style={{ padding: '20px 40px 48px' }}>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
          ...
          <ChevronRight size={12} style={{ color: 'var(--fg-4, #94A3B8)' }} />
          <span style={{ fontSize: 13, color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>{title}</span>
        </nav>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 32, alignItems: 'start' }}>
          ...
          <article>
            {/* Title */}
            <h1 style={{ fontFamily: 'Sora, sans-serif', fontSize: 28, fontWeight: 700, color: 'var(--fg-1, #0F172A)', margin: '0 0 12px' }}>{title}</h1>

            {/* Metadata row */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 8 }}>
              <span style={{ fontSize: 12, color: 'var(--fg-3, #64748B)' }}>{page.updated_at ? timeAgo(page.updated_at) : '—'}</span>
              {conf > 0 && (
                <span style={{
              ...
              {formatIcon}
              <span style={{ fontSize: 11, color: 'var(--fg-3, #64748B)', display: 'inline-flex', alignItems: 'center', gap: 3, fontFamily: 'JetBrains Mono, monospace' }}>
              ...
              {authorName ? (
                <>
                  <div style={{
                    ...
                    fontSize: 10, fontWeight: 700, color: 'var(--fg-3, #64748B)',
                  }}>{authorName.charAt(0).toUpperCase()}</div>
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#334155' }}>{authorName}</span>
                </>
              ) : (
                <span style={{ fontSize: 12, color: 'var(--fg-4, #94A3B8)' }}>No author</span>
              )}
              <span style={{
                fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 3,
            ...
            <div style={{
              ...
              {tldr ? (
                ...
                <div style={{ fontSize: 12, color: 'var(--fg-4, #94A3B8)', marginTop: 8, fontStyle: 'italic' }}>AI summary not yet generated</div>
              )}
            </div>
            ...
            {sections.length > 0 && (
              <nav style={{
                background: 'var(--surface-subtle, #F8FAFC)', border: '1px solid rgba(15,23,42,0.12)',
                borderRadius: 6, padding: 16, marginBottom: 28,
              }}>
                <div style={{
                  fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const,
                  color: 'var(--fg-3, #64748B)', marginBottom: 8, letterSpacing: '0.04em',
                }}>Contents</div>
                {sections.map((s: any, i: number) => (
                  <a key={s.id} href={`#section-${i}`} style={{
                    ...
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'var(--fg-4, #94A3B8)', marginRight: 8 }}>{i + 1}.</span>
                    {s.title}
                  </a>
                ))}
              </nav>
            ...
            {sections.map((s: any, i: number) => (
              <div key={s.id} id={`section-${i}`} style={{ marginBottom: 32 }}>
                <h2 style={{
                  fontFamily: 'Sora, sans-serif', fontSize: 18, fontWeight: 650, color: 'var(--fg-1, #0F172A)',
                  margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: 8,
                }}>
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 14, color: 'var(--fg-4, #94A3B8)' }}>{i + 1}.</span>
                  {s.title}
                  {s.is_live_data && (
                    <span style={{
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 650, color: 'var(--fg-1, #0F172A)', marginBottom: 4 }}>Was this helpful?</div>
                <div style={{ fontSize: 11, color: 'var(--fg-3, #64748B)' }}>
            ...
            {(related ?? []).length > 0 && (
              <div style={{ marginTop: 40 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  ...
                  <span style={{ fontFamily: 'Sora, sans-serif', fontSize: 16, fontWeight: 650, color: 'var(--fg-1, #0F172A)' }}>Related Articles</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
                  {(related ?? []).map((r: any) => (
                    <div key={r.id} onClick={() => navigate(`/wiki/${r.slug}`)} style={{
                      padding: 16, borderRadius: 6, background: 'var(--bg-app, #FFFFFF)',
                    ...
                    >
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-1, #0F172A)', marginBottom: 6 }}>{r.title}</div>
                      <div style={{ fontSize: 11, color: 'var(--fg-3, #64748B)' }}>
                        {r.domain_code}
          ...
          <aside style={{
            ...
            {infoRows.map(row => (
              <div key={row.label} style={{
                ...
                <span style={{ color: 'var(--fg-3, #64748B)', fontSize: 11 }}>{row.label}</span>
                <span style={{ fontSize: 11, color: 'var(--fg-1, #0F172A)', fontWeight: 500 }}>{row.value}</span>
              </div>
            ))}
          </aside>
        </div>
...
function DeliveryStatusSection({ content }: { content: string | null }) {
  if (!content) return <div style={{ fontSize: 13, color: 'var(--fg-4, #94A3B8)' }}>No delivery data available.</div>;
  return (
    <div style={{
...
function ReferencesSection({ refs }: { refs: any[] }) {
  if (refs.length === 0) return <div style={{ fontSize: 12, color: 'var(--fg-4, #94A3B8)', padding: 8 }}>No references.</div>;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {refs.map((r: any) => (
        <div key={r.ref_number ?? r.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0' }}>
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'var(--fg-4, #94A3B8)', minWidth: 24 }}>[{r.ref_number}]</span>
          {r.source_type === 'jira' ? (
          ...
          <span style={{ fontSize: 12, color: 'var(--fg-3, #64748B)' }}>{r.description || '—'}</span>
        </div>
      ))}
    </div>
  );
...
function ArticleSkeleton() {
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', background: 'var(--surface-subtle, #F8FAFC)', minHeight: '100%', padding: '20px 40px 48px' }}>
