      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0' }}>
        {versions.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }}>No version history yet</div>
          </div>
          ...
          <article>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              {authorName ? (
                ...
                <span style={{ fontSize: 12, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }}>No author</span>
              )}
              <span style={{
            ...
            <div style={{
              ...
              {tldr ? (
                ...
                <div style={{ fontSize: 12, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))', marginTop: 8, fontStyle: 'italic' }}>AI summary not yet generated</div>
              )}
            </div>
            ...
            {sections.map((s: any, i: number) => (
              <div key={s.id} id={`section-${i}`} style={{ marginBottom: 32 }}>
                <h2 style={{
                  ...
                  <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 14, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }}>{i + 1}.</span>
                  {s.title}
                  {s.is_live_data && (
                    <span style={{
            ...
            {sections.length === 0 && (
              <div style={{ padding: 48, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))', fontSize: 13 }}>
...
function DeliveryStatusSection({ content }: { content: string | null }) {
  const { isDark } = useTheme();
  if (!content) return <div style={{ fontSize: 13, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }}>No delivery data available.</div>;
  return (
    <div style={{
...
function ReferencesSection({ refs }: { refs: any[] }) {
  const { isDark } = useTheme();
  if (refs.length === 0) return <div style={{ fontSize: 12, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))', padding: 8 }}>No references.</div>;
  return (
