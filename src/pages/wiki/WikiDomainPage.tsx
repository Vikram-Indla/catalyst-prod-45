  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', color: isDark ? '#EDEDED' : '#0F172A', background: isDark ? '#0A0A0A' : '#F8FAFC', minHeight: '100%', padding: '24px 40px 48px' }}>
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <ChevronRight size={12} style={{ color: isDark ? '#666666' : '#94A3B8' }} />
        <span style={{ fontSize: 13, color: isDark ? '#888888' : '#64748B', fontWeight: 600 }}>{domain?.name || code}</span>
      </nav>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: isDark ? '#888888' : '#64748B', margin: '4px 0 0' }}>{domain?.description || ''}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 24, marginBottom: 32 }}>
        {[{ l: 'Articles', v: domain?.article_count ?? 0 }, { l: 'Views', v: domain?.view_count ?? 0 }, { l: 'Coverage', v: `${domain?.coverage_percent ?? 0}%` }, { l: 'Gaps', v: domain?.knowledge_gaps ?? 0 }].map(s => (
          <div key={s.l} style={{ padding: '16px 24px', borderRadius: 8, background: isDark ? '#111111' : '#FFFFFF', border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(0,0,0,0.06)', textAlign: 'center', minWidth: 100 }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 20, fontWeight: 700, color: isDark ? '#EDEDED' : '#0F172A' }}>{s.v}</div>
            <div style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', color: isDark ? '#666666' : '#94A3B8', marginTop: 4 }}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ borderRadius: 8, border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(0,0,0,0.06)', background: isDark ? '#111111' : '#FFFFFF', overflow: 'hidden' }}>
        {isLoading ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#666666' : '#64748B', fontSize: 12 }}>Loading...</div> :
          (articles ?? []).length === 0 ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#666666' : '#64748B', fontSize: 12 }}>No articles in this domain yet.</div> :
          (articles ?? []).map((a: any) => (
            <div key={a.id} onClick={() => navigate(`/wiki/${a.slug}`)} style={{
              ...
              <ArrowRight size={12} style={{ color: isDark ? '#666666' : '#94A3B8' }} />
            </div>
          ))}
      </div>
    </div>
