  ...
  return (
    <div style={{ fontFamily: F.inter, color: isDark ? '#F5F3F0' : '#0F172A', background: isDark ? '#1A1714' : '#F8FAFC', minHeight: '100%', padding: '24px 40px 60px' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: isDark ? '#A09890' : '#64748B', fontWeight: 600 }}>Subscriptions</span>
      </nav>

      <h1 style={{ fontFamily: F.sora, fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Subscriptions</h1>
      <p style={{ fontSize: 12, color: isDark ? '#A09890' : '#64748B', marginBottom: 32 }}>Get notified when content you follow is updated.</p>
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: 8, background: isDark ? '#232019' : '#FFFFFF', border: `0.75px solid ${active ? '#2563EB' : border}`,
                cursor: 'pointer', transition: 'all 120ms', textAlign: 'left',
              }}
            >
              <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 3, background: active ? (isDark ? 'rgba(37,99,235,0.15)' : '#DBEAFE') : (isDark ? '#2C2823' : '#F1F5F9'), color: active ? (isDark ? '#93C5FD' : '#1E40AF') : (isDark ? '#6B6560' : '#64748B') }}>{d.code}</span>
              <span style={{ fontSize: 12.5, fontWeight: 500, color: isDark ? '#F5F3F0' : '#0F172A', flex: 1 }}>{d.name}</span>
              <div style={{
                width: 36, height: 20, borderRadius: 10,
                background: active ? '#2563EB' : (isDark ? '#3A3530' : '#E2E8F0'),
                position: 'relative', transition: 'background 150ms',
              }}>
                <div style={{
                  width: 16, height: 16, borderRadius: '50%', background: isDark ? '#F5F3F0' : '#FFFFFF',
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
        {tagSubs.map((tag: string) => (
          <span key={tag} style={{
            ...
            borderRadius: 4, background: isDark ? '#2C2823' : '#F1F5F9', border: `0.75px solid ${border}`,
            fontSize: 11, fontWeight: 500, color: isDark ? '#A09890' : '#334155',
          }}>
            {tag}
            <X size={11} style={{ color: isDark ? '#6B6560' : '#94A3B8', cursor: 'pointer' }} onClick={() => toggleSub.mutate({ entityType: 'tag', entityId: tag })} />
          </span>
        ))}
        {tagSubs.length === 0 && <span style={{ fontSize: 11, color: isDark ? '#6B6560' : '#94A3B8' }}>No tag subscriptions yet.</span>}
      </div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 32 }}>
        <input
          ...
          style={{
            ...
            border: `0.75px solid ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, background: isDark ? '#232019' : '#FFFFFF',
            outline: 'none', width: 180, color: isDark ? '#F5F3F0' : undefined,
          }}
        />
      ...
      <div style={{ borderRadius: 8, border: `0.75px solid ${border}`, background: isDark ? '#232019' : '#FFFFFF', overflow: 'hidden' }}>
        {subArticles.length > 0 ? subArticles.map((a: any) => (
          ...
          >
            ...
            <span onClick={() => navigate(`/wiki/${a.slug}`)} style={{ fontSize: 12.5, fontWeight: 500, color: isDark ? '#F5F3F0' : '#0F172A', cursor: 'pointer', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.title}</span>
            <span style={{ fontSize: 9, fontWeight: 650, padding: '1px 5px', borderRadius: 3, background: isDark ? '#2C2823' : '#F1F5F9', color: isDark ? '#6B6560' : '#64748B' }}>{a.domain_code}</span>
            <button
          ...
          <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#6B6560' : '#64748B', fontSize: 12 }}>
...
function SectionLabel({ icon, label, isDark }: { icon: React.ReactNode; label: string; isDark?: boolean }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
      {icon}
      <span style={{ fontFamily: "'Sora', sans-serif", fontSize: 14, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A' }}>{label}</span>
    </div>
  );
}
