...
function SimpleDateInput({ label, value, onChange, helperText }: {
  ...
  return (
    <div style={{ flex: 1, minWidth: 200 }}>
      <label style={{
        fontSize: 11, fontWeight: 700, color: 'var(--fg-3, #64748B)', marginBottom: 4,
      ...
      <div style={{ position: 'relative' }}>
        <input
          ...
          style={{
            ...
            border: '0.75px solid rgba(15,23,42,0.12)', background: 'var(--bg-app, #FFFFFF)',
            color: 'var(--fg-1, #0F172A)', fontFamily: 'Inter, sans-serif',
          }}
        />
      </div>
      <div style={{ fontSize: 10, color: 'var(--fg-4, #94A3B8)', marginTop: 4 }}>{helperText}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', color: 'var(--fg-1, #0F172A)', background: 'var(--surface-subtle, #F8FAFC)', minHeight: '100%', padding: '24px 40px 48px' }}>
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Templates</span>
      </nav>

      <h1 style={{ fontFamily: "'Sora', sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 8 }}>Article Templates</h1>
      <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', marginBottom: 16 }}>Pre-built structures for common article types. Click "Use Template" to create a pre-filled article.</p>
      ...
      <div style={{
        ...
        background: 'var(--bg-app, #FFFFFF)', border: '0.75px solid rgba(15,23,42,0.08)',
      }}>
        <button onClick={() => setShowScheduling(!showScheduling)} style={{
          fontSize: 12, fontWeight: 650, color: 'var(--fg-1, #0F172A)', background: 'transparent',
          border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, width: '100%',
        }}>
          <CalendarIcon size={14} style={{ color: '#2563EB' }} />
          Content Scheduling
          <span style={{ fontSize: 10, color: 'var(--fg-4, #94A3B8)', fontWeight: 500, marginLeft: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
        {isLoading ? Array.from({ length: 5 }).map((_, i) => (
          <div key={i} style={{ padding: 20, borderRadius: 8, background: 'var(--bg-app, #FFFFFF)', border: '0.75px solid rgba(0,0,0,0.06)', height: 140 }} />
        )) : (templates ?? []).map((t: any) => {
          const sections = (t.template_sections as any[]) || [];
          return (
            <div key={t.id} style={{ padding: 20, borderRadius: 8, background: 'var(--bg-app, #FFFFFF)', border: '0.75px solid rgba(0,0,0,0.06)', transition: 'border-color 120ms' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <div style={{ width: 32, height: 32, borderRadius: 6, background: 'var(--surface-muted, #F1F5F9)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <FileText size={16} style={{ color: 'var(--fg-3, #64748B)' }} />
                </div>
                <div>
                  <div style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--fg-1, #0F172A)' }}>{t.name}</div>
                  <div style={{ fontSize: 11, color: 'var(--fg-3, #64748B)' }}>{t.description}</div>
                </div>
              </div>
              <div style={{ fontSize: 11, color: 'var(--fg-3, #64748B)', marginBottom: 12 }}>{sections.length} sections</div>
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 12 }}>
                {sections.slice(0, 4).map((s: any, i: number) => (
                  <span key={i} style={{ fontSize: 9, padding: '2px 6px', borderRadius: 3, background: 'var(--surface-muted, #F1F5F9)', color: 'var(--fg-3, #64748B)' }}>{s.title}</span>
                ))}
                {sections.length > 4 && <span style={{ fontSize: 9, color: 'var(--fg-4, #94A3B8)' }}>+{sections.length - 4} more</span>}
              </div>
              <button
