...
export default function WikiLearningPathDetailPage() {
  ...
  if (!path) {
    return (
      <div style={{ fontFamily: F.inter, padding: '60px 40px', textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, #64748B)', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : undefined, minHeight: '100%' }}>
        Learning path not found.
      </div>
    );
  }

  const diffColor = DIFF_COLORS[path.difficulty] || 'var(--ds-text-subtlest, #64748B)';
  ...
  return (
    <div style={{ fontFamily: F.inter, color: isDark ? 'var(--ds-text, #EDEDED)' : 'var(--ds-text, #0F172A)', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface-sunken, #F8FAFC)', minHeight: '100%', padding: '24px 40px 60px' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <ChevronRight size={12} style={{ color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, #94A3B8)' }} />
        <span style={{ fontSize: 13, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, #64748B)', fontWeight: 600 }}>{path.title}</span>
      </nav>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 32 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, #64748B)' }}>{path.estimated_hours}h estimated</span>
            <span style={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, #64748B)' }}>{totalCount} articles</span>
          </div>
      ...
      <div style={{ borderRadius: 8, border: `0.75px solid ${border}`, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, #FFFFFF)', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          fontFamily: F.sora, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, #64748B)', letterSpacing: '0.05em',
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <span style={{ fontFamily: F.mono, fontSize: 10, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, #64748B)' }}>{a.read_time_minutes ?? '?'}m</span>
              </div>
              <button
        ...
        {articles.length === 0 && !pathLoading && (
          <div style={{ padding: 40, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, #64748B)', fontSize: 12 }}>
