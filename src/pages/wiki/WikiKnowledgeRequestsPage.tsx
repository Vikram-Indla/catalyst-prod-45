...
const PRIORITY_STYLES: Record<string, { bg: string; color: string }> = {
  ...
  low: { bg: '#F1F5F9', color: 'var(--fg-3, #64748B)' },
};
...
const STATUS_STYLES: Record<string, { bg: string; color: string; label: string }> = {
  ...
  closed: { bg: '#F1F5F9', color: 'var(--fg-3, #64748B)', label: 'CLOSED' },
};
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', color: 'var(--fg-1, #0F172A)', background: 'var(--surface-subtle, #F8FAFC)', minHeight: '100%', padding: '24px 40px 48px' }}>
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: 'var(--fg-3, #64748B)', fontWeight: 600 }}>Knowledge Requests</span>
      </nav>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: 'Sora, sans-serif', fontSize: 18, fontWeight: 700, margin: 0 }}>Knowledge Requests</h1>
          <p style={{ fontSize: 12, color: 'var(--fg-3, #64748B)', margin: '4px 0 0' }}>Track content gaps and route to domain experts.</p>
        </div>
        <button style={{
      ...
      <div style={{ borderRadius: 8, border: '0.75px solid rgba(0,0,0,0.06)', background: 'var(--bg-app, #FFFFFF)', overflow: 'hidden' }}>
        ...
        {isLoading ? <div style={{ padding: 32, textAlign: 'center', color: 'var(--fg-3, #64748B)', fontSize: 12 }}>Loading...</div> :
          (requests ?? []).length === 0 ? <div style={{ padding: 32, textAlign: 'center', color: 'var(--fg-3, #64748B)', fontSize: 12 }}>No knowledge requests yet.</div> :
          (requests ?? []).map((r: any) => {
            ...
            return (
              <div key={r.id} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontWeight: 500, color: 'var(--fg-1, #0F172A)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.title}</div>
                  {r.description && <div style={{ fontSize: 10, color: 'var(--fg-4, #94A3B8)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.description}</div>}
                </div>
                <span style={{ fontSize: 10, color: 'var(--fg-3, #64748B)' }}>{r.domain_code || '—'}</span>
                ...
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'var(--fg-3, #64748B)' }}>{new Date(r.created_at).toLocaleDateString()}</span>
              </div>
            );
          })}
      </div>
