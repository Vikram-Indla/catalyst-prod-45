...
const PRIORITY_STYLES_DARK: Record<string, { bg: string; color: string }> = {
  ...
  low: { bg: 'rgba(255,255,255,0.06)', color: '#666666' },
};
...
const STATUS_STYLES_DARK: Record<string, { bg: string; color: string; label: string }> = {
  ...
  closed: { bg: 'rgba(255,255,255,0.06)', color: '#666666', label: 'CLOSED' },
};
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', color: isDark ? '#EDEDED' : '#0F172A', background: isDark ? '#0A0A0A' : '#F8FAFC', minHeight: '100%', padding: '24px 40px 48px' }}>
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: isDark ? '#888888' : '#64748B', fontWeight: 600 }}>Knowledge Requests</span>
      </nav>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: 'Sora, sans-serif', fontSize: 18, fontWeight: 700, margin: 0 }}>Knowledge Requests</h1>
          <p style={{ fontSize: 12, color: isDark ? '#888888' : '#64748B', margin: '4px 0 0' }}>Track content gaps and route to domain experts.</p>
        </div>
        <button style={{
      ...
      <div style={{ borderRadius: 8, border: `0.75px solid ${border}`, background: isDark ? '#111111' : '#FFFFFF', overflow: 'hidden' }}>
        ...
        {isLoading ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#666666' : '#64748B', fontSize: 12 }}>Loading...</div> :
          (requests ?? []).length === 0 ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? '#666666' : '#64748B', fontSize: 12 }}>No knowledge requests yet.</div> :
          (requests ?? []).map((r: any) => {
            ...
            return (
              <div key={r.id} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {r.description && <div style={{ fontSize: 10, color: isDark ? '#666666' : '#94A3B8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.description}</div>}
                </div>
                <span style={{ fontSize: 10, color: isDark ? '#888888' : '#64748B' }}>{r.domain_code || '—'}</span>
                ...
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: isDark ? '#888888' : '#64748B' }}>{new Date(r.created_at).toLocaleDateString()}</span>
              </div>
            );
          })}
      </div>
