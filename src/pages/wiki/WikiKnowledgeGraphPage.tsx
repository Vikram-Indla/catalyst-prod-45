...
export default function WikiKnowledgeGraphPage() {
  ...
  useEffect(() => {
    ...
    const node = g.append('g')
      ...
      .attr('stroke', isDark ? '#1A1714' : '#FFFFFF')
    ...
    const labels = g.append('g')
      ...
      .attr('fill', isDark ? '#A09890' : '#334155')
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', color: isDark ? '#F5F3F0' : '#0F172A', background: isDark ? '#1A1714' : '#F8FAFC', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '20px 40px 0' }}>
        <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 13, color: isDark ? '#A09890' : '#64748B', fontWeight: 600 }}>Knowledge Graph</span>
        </nav>
        <h1 style={{ fontFamily: "'Sora', sans-serif", fontSize: 18, fontWeight: 700, margin: '0 0 8px' }}>Knowledge Graph</h1>
        <p style={{ fontSize: 12, color: isDark ? '#A09890' : '#64748B', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
          {Object.entries(DOMAIN_COLORS).map(([code, color]) => (
            <div key={code} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ fontSize: 10, color: isDark ? '#A09890' : '#64748B', fontWeight: 500 }}>{code}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, position: 'relative', borderTop: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(0,0,0,0.06)' }}>
        <svg ref={svgRef} style={{ width: '100%', height: '100%', background: isDark ? '#232019' : '#FFFFFF' }} />
        {tooltip && (
          <div style={{
            position: 'absolute', left: tooltip.x + 12, top: tooltip.y - 8,
            background: isDark ? '#2C2823' : '#FFFFFF', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0', borderRadius: 6,
            ...
            <div style={{ fontWeight: 600, marginBottom: 2, color: isDark ? '#F5F3F0' : undefined }}>{tooltip.title}</div>
            <div style={{ fontSize: 10, color: isDark ? '#A09890' : '#64748B' }}>{tooltip.domain} · {tooltip.views} views</div>
          </div>
        )}
      </div>
    </div>
