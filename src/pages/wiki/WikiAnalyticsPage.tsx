...
export default function WikiAnalyticsPage() {
  ...
  const { data: verificationPie } = useQuery({
    ...
    queryFn: async () => {
      ...
      return [
        { name: 'Verified', value: counts.verified, fill: 'var(--ds-text-success, var(--ds-text-success, #16A34A))' },
        { name: 'Needs Review', value: counts.needs_review, fill: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' },
        { name: 'Unverified', value: counts.unverified, fill: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' },
      ];
    },
  });
  ...
  return (
    <div style={{ fontFamily: 'var(--cp-font-body)', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--ds-text, var(--ds-text, #0F172A))', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F8FAFC))', minHeight: '100%', padding: '24px 40px 48px' }}>
      <nav style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', fontWeight: 600 }}>Analytics</span>
      </nav>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 32 }}>
        {isLoading ? Array.from({ length: 8 }).map((_, i) => (
          <div key={i} style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', border: `0.75px solid ${border}`, height: 80 }} />
        )) : statCards.map(s => (
          <div key={s.label} style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', border: `0.75px solid ${border}`, textAlign: 'center' }}>
            <div style={{ fontFamily: F.sora, fontSize: 24, fontWeight: 700, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--ds-text, var(--ds-text, #0F172A))' }}>{s.value}</div>
            <div style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', marginTop: 4, letterSpacing: '0.05em' }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 32 }}>
        {/* Articles per Domain */}
        <div style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', border: `0.75px solid ${border}` }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={domainDistribution ?? []}>
              ...
              <XAxis dataKey="domain" tick={{ fontSize: 10, fill: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }} />
              <YAxis tick={{ fontSize: 10, fill: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }} />
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 6, border: `1px solid ${isDark ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--ds-border, var(--ds-border, #E2E8F0))'}`, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined }} />
              <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                {(domainDistribution ?? []).map((d: any) => (
                  <Cell key={d.domain} fill={DOMAIN_COLORS[d.domain] || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', border: `0.75px solid ${border}` }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 6, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined, border: `1px solid ${isDark ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--ds-border, var(--ds-border, #E2E8F0))'}` }} />
              <Legend wrapperStyle={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : undefined }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div>
          ...
          <div style={{ borderRadius: 8, border: `0.75px solid ${border}`, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', overflow: 'hidden' }}>
            <div style={{
              ...
              background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))', padding: '0 14px', height: 32, alignItems: 'center',
              fontFamily: F.sora, fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const,
              color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', letterSpacing: '0.05em', borderBottom: `0.75px solid ${border}`,
            }}>
            ...
            {(topArticles ?? []).map((a: any) => {
              ...
              return (
                <div key={a.id} onClick={() => navigate(`/wiki/${a.slug}`)} style={{
                  ...
                  <span style={{ fontFamily: F.mono, fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>{Math.round(a.helpfulness_score ?? 0)}%</span>
        ...
        <div>
          ...
          <div style={{ borderRadius: 8, border: `0.75px solid ${border}`, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--ds-surface, var(--ds-surface, #FFFFFF))', overflow: 'hidden' }}>
            ...
            {(leastHelpful ?? []).length === 0 ? (
              <div style={{ padding: 24, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', fontSize: 12 }}>No feedback data yet</div>
              ...
              <div key={a.id} onClick={() => navigate(`/wiki/${a.slug}`)} style={{
                ...
                fontSize: 12, cursor: 'pointer', transition: 'background 80ms',
                 ...
                 onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                <span style={{ fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.title}</span>
                <span style={{ fontFamily: F.mono, fontSize: 11, color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>{Math.round(a.helpfulness_score ?? 0)}%</span>
                <span style={{ fontFamily: F.mono, fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>{a.helpfulness_votes}</span>
              </div>
            ))}
          </div>
        </div>
