    ...
    <div style={{ fontFamily: 'var(--cp-font-body)', color: isDark ? 'var(--ds-text, #EDEDED)' : 'var(--ds-text, var(--cp-ink-1, #0F172A))', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--ds-surface-sunken, #F8FAFC)', minHeight: '100%' }}>
      <div style={{ maxWidth: 840, marginInline: 'auto', padding: '24px 28px 48px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          <div style={{
            ...
            <button onClick={() => { setMode('ai'); setPage(0); }} style={{
              ...
              background: mode === 'ai' ? 'var(--cp-purple-60, #7C3AED)' : (isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))'),
              color: mode === 'ai' ? 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))' : (isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))'),
              transition: 'all 120ms',
        ...
        {debouncedQuery.length >= 2 && !isLoading && (
          <div style={{ fontSize: 13, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', marginBottom: 16 }}>
            {mode === 'keyword'
          ...
          <div>
            ...
            {aiAnswer && (
              <div style={{
                ...
                <div style={{ fontSize: 14, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--cp-ink-2, var(--cp-ink-2, #334155))', lineHeight: 1.75, marginTop: 10 }}>
                  {aiAnswer}
                </div>
                {aiSources.length > 0 && (
                  <div style={{ marginTop: 14, paddingTop: 12, borderTop: `0.75px solid ${borderColor}` }}>
                    <div style={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {aiSources.map((s: any, i: number) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{
                            fontFamily: 'var(--cp-font-mono)', fontSize: 10, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))',
                          ...
                          <span style={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }}>{s.entity_type || 'wiki'}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {!aiAnswer && aiSources.length === 0 && (
              <div style={{ padding: 40, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))', fontSize: 13 }}>
                ...
                {paginatedResults.map((r: any) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={e => e.currentTarget.style.background = isDark ? 'var(--ds-surface-overlay, #1F1F1F)' : 'rgba(37,99,235,0.03)'}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, #878787)' : 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }}>{r.updated_at ? timeAgo(r.updated_at) : '—'}</span>
                      </div>

                      {/* Snippet */}
                      {r.lead_content && (
                        <div style={{
                          fontSize: 12, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', lineHeight: 1.6,
                  ...
                  <div style={{
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: page === 0 ? (isDark ? 'var(--ds-border, #292929)' : 'var(--ds-text-disabled, #CBD5E1)') : (isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--cp-ink-2, var(--cp-ink-2, #334155))'), cursor: page === 0 ? 'default' : 'pointer',
                      }}
                    >← Previous</button>
                    <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))' }}>
                      {page + 1} / {totalPages}
                    </span>
                    <button
                      ...
                      style={{
                        fontSize: 12, fontWeight: 650, padding: '6px 14px', borderRadius: 4,
                        border: `1px solid ${borderColor}`, background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))',
                        color: page >= totalPages - 1 ? (isDark ? 'var(--ds-border, #292929)' : 'var(--ds-text-disabled, #CBD5E1)') : (isDark ? 'var(--ds-text-subtlest, #A1A1A1)' : 'var(--cp-ink-2, var(--cp-ink-2, #334155))'), cursor: page >= totalPages - 1 ? 'default' : 'pointer',
                      }}
                    >Next →</button>
                  </div>
                )}
