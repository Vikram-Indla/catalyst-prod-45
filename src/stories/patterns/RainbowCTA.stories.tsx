...
function RainbowCTAPattern() {
  return (
    <div style={{ maxWidth: 600, fontFamily: 'Atlassian Sans, -apple-system, sans-serif' }}>
      <h2 style={{ fontSize: 'var(--ds-font-size-500)', fontWeight: 653, color: 'var(--ds-text, #172B4D)', margin: '0 0 8px' }}>Rainbow CTA pattern</h2>
      <p style={{ fontSize: 'var(--ds-font-size-300)', color: 'var(--ds-text-subtle, #42526E)', margin: '0 0 24px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {[
          ...
          <div key={v.ctx} style={{ padding: 16, border: '1px solid var(--ds-border, #DFE1E6)', borderRadius: 8 }}>
            <div style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-subtlest, #6B778C)', marginBlockEnd: 8 }}>{v.ctx}</div>
            <CatyRainbowCTA label={v.label} onClick={() => {}} align="left" />
          </div>
        ))}
        <div style={{ padding: 16, border: '1px solid var(--ds-border, #DFE1E6)', borderRadius: 8 }}>
          <div style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-subtlest, #6B778C)', marginBlockEnd: 8 }}>Loading state</div>
