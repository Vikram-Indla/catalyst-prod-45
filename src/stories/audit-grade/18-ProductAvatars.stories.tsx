      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        {STOCK_PLACE_IDS.map((id) => (
          <div key={id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, width: 96 }}>
            ...
            <span style={{ fontSize: 'var(--ds-font-size-50)', color: 'var(--ds-text-subtle, #42526E)', textAlign: 'center', wordBreak: 'break-all' }}>{id}</span>
          </div>
        ))}
      </div>
    </Frame>
  ...
  render: () => (
    <Frame width={400}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <div style={{ fontSize: 'var(--ds-font-size-400)', fontWeight: 500, color: 'var(--ds-text, #172B4D)' }}>Investor Journey Product</div>
          <div style={{ fontSize: 'var(--ds-font-size-200)', color: 'var(--ds-text-subtle, #42526E)' }}>INV → Elephant Rock</div>
        </div>
      </div>
    </Frame>
  ),
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
        {['ALPHA', 'BETA', 'GAMMA', 'DELTA', 'OMEGA', 'ZETA', 'THETA', 'KAPPA'].map((code) => (
          <div key={code} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 'var(--ds-font-size-100)', color: 'var(--ds-text-subtle, #42526E)' }}>{code}</span>
          </div>
        ))}
      </div>
    </Frame>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16 }}>
        {([16, 24, 32, 40, 48, 64] as const).map((s) => (
          <div key={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 'var(--ds-font-size-50)', color: 'var(--ds-text-subtle, #42526E)' }}>{s}px</span>
          </div>
        ))}
      </div>
    </Frame>
