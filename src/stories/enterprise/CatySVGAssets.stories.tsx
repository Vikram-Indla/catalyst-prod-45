...
function AssetRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 32 }}>
      <p style={{ margin: '0 0 12px', fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
        {label}
      </p>
...
function SizeChip({ size, bg, src, alt }: { size: number; bg: string; src: string; alt: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      ...
      <span style={{ fontSize: 'var(--ds-font-size-100)', color: 'var(--ds-text-subtlest)' }}>{size}px</span>
    </div>
  );
}
  ...
  render: () => (
    <div>
      <AssetRow label="Ask Caty — Gradient (on light bg)">
        {sizes.map(s => <SizeChip key={s} size={s} bg="var(--ds-surface)" src={askCatyGradient} alt="Ask Caty gradient" />)}
      </AssetRow>
      <AssetRow label="Ask Caty — Dark (on light bg)">
        {sizes.map(s => <SizeChip key={s} size={s} bg="var(--ds-surface)" src={askCatyLight} alt="Ask Caty dark" />)}
      </AssetRow>
  ...
  render: () => (
    <div>
      <AssetRow label="Caty — Gradient (on light bg)">
        {sizes.map(s => <SizeChip key={s} size={s} bg="var(--ds-surface)" src={catyGradient} alt="Caty gradient" />)}
      </AssetRow>
      <AssetRow label="Caty — Dark (on light bg)">
        {sizes.map(s => <SizeChip key={s} size={s} bg="var(--ds-surface)" src={catyLight} alt="Caty dark" />)}
      </AssetRow>
...
export const AllVariants: StoryObj = {
  ...
  render: () => (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40 }}>
      <div>
        <h3 style={{ margin: '0 0 16px', fontSize: 'var(--ds-font-size-400)', fontWeight: 600, color: 'var(--ds-text)' }}>Ask Caty</h3>
        {[
          { label: 'gradient', src: askCatyGradient, bg: 'var(--ds-surface)', textColor: 'var(--ds-text-subtle)' },
          { label: 'dark (light bg)', src: askCatyLight, bg: 'var(--ds-surface)', textColor: 'var(--ds-text-subtle)' },
      ...
      <div>
        <h3 style={{ margin: '0 0 16px', fontSize: 'var(--ds-font-size-400)', fontWeight: 600, color: 'var(--ds-text)' }}>Caty</h3>
        {[
          { label: 'gradient', src: catyGradient, bg: 'var(--ds-surface)', textColor: 'var(--ds-text-subtle)' },
          { label: 'dark (light bg)', src: catyLight, bg: 'var(--ds-surface)', textColor: 'var(--ds-text-subtle)' },
  ...
  render: () => (
    <div>
      ...
      <AssetRow label="Caty AI — no background (transparent)">
        {sizes.map(s => <SizeChip key={s} size={s} bg="var(--ds-surface-sunken)" src={catyAI} alt="Caty AI" />)}
        {/* ads-scanner:ignore-line — Storybook design demo, intentional dark bg */}
        {sizes.map(s => <SizeChip key={s} size={s} bg="#2A2832" src={catyAI} alt="Caty AI on dark" />)}
      </AssetRow>
      <AssetRow label="Logo Mark — Light (on light bg)">
        {sizes.map(s => <SizeChip key={s} size={s} bg="var(--ds-surface)" src={logoMarkLight} alt="Logo mark light" />)}
      </AssetRow>
  ...
  render: () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24 }}>
      {[
        ...
        { label: 'Caty AI', src: catyAI, bg: 'var(--ds-surface-sunken)', textColor: 'var(--ds-text-subtle)' },
        { label: 'Logo Mark Light', src: logoMarkLight, bg: 'var(--ds-surface)', textColor: 'var(--ds-text-subtle)' },
        // ads-scanner:ignore-next-line — Storybook design demo, intentional dark bg
...
export const CatyFabIconStory: StoryObj = {
  ...
  render: () => (
    <div>
      <p style={{ margin: '0 0 16px', fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
        CatyFabIcon — inline SVG with CSS animation states
      </p>
      <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        {([16, 24, 32, 40, 48, 56, 64, 80, 96] as const).map(s => (
          <div key={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div style={{ background: 'var(--ds-surface)', borderRadius: 8, padding: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 4px var(--ds-background-neutral-subtle-pressed, rgba(9,30,66,0.12))' }}>
              <CatyFabIcon size={s} />
            </div>
            <span style={{ fontSize: 'var(--ds-font-size-100)', color: 'var(--ds-text-subtlest)' }}>{s}px</span>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 32, display: 'flex', gap: 32, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <span style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-subtle)' }}>Default (breathing)</span>
          <div style={{ background: 'var(--ds-surface)', borderRadius: 12, padding: 16, boxShadow: '0 1px 4px var(--ds-background-neutral-subtle-pressed, rgba(9,30,66,0.12))' }}>
            <CatyFabIcon size={56} />
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <span style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-subtle)' }}>On dark bg</span>
          {/* ads-scanner:ignore-line — Storybook design demo, intentional dark bg */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <span style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text-subtle)' }}>Awake state (hover)</span>
          <div style={{ background: 'var(--ds-surface)', borderRadius: 12, padding: 16, boxShadow: '0 1px 4px var(--ds-background-neutral-subtle-pressed, rgba(9,30,66,0.12))' }}>
              ...
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width={56} height={56} className="is-awake" role="img" aria-label="Ask Caty awake">
                <defs>
                  <linearGradient id="askdg-sb" x1="256" y1="40" x2="256" y2="470" gradientUnits="userSpaceOnUse">
                    <stop stopColor="#F79357" /><stop offset=".5" stopColor="var(--ds-background-danger-bold)" />
                ...
                <g className="cf-sb">
                  ...
                  <text x="350" y="293" textAnchor="middle" fontFamily="'Atlassian Sans', system-ui, sans-serif" fontSize="84" fontWeight="800" fontStyle="italic" fill="var(--caty-fg, var(--ds-text))">ask</text>
                  <g className="cf-ears-sb">
                    ...
                    <path d="M288 96 Q301 56 312 94" fill="none" stroke="var(--caty-fg, var(--ds-text))" strokeWidth="9" strokeLinecap="round" opacity=".55" />
                    <path d="M374 96 Q387 54 398 94" fill="none" stroke="var(--caty-fg, var(--ds-text))" strokeWidth="9" strokeLinecap="round" opacity=".55" />
                  </g>
                  <g stroke="var(--caty-fg, var(--ds-text))" strokeWidth="9.5" strokeLinecap="round" fill="none">
                  ...
                  <path d="M340 176 L356 176 Q348 188 340 176 Z" fill="var(--caty-fg, var(--ds-text))" />
                  <g className="cf-eyes-sb" fill="none" stroke="var(--caty-fg, var(--ds-text))" strokeWidth="13" strokeLinecap="round">
                  ...
                  <g className="cf-awake-sb">
                    ...
                    <circle cx="324" cy="153" r="8.5" fill="var(--caty-fg, var(--ds-text))" /><circle cx="376" cy="153" r="8.5" fill="var(--caty-fg, var(--ds-text))" />
      ...
      <p style={{ marginTop: 16, fontSize: 'var(--ds-font-size-200)', color: 'var(--ds-text-subtlest)' }}>
...
export const InteractivePreview: StoryObj = {
  ...
  render: () => (
    <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
      {[
        { label: 'Ask Caty Gradient', src: askCatyGradient, bg: 'var(--ds-surface)' },
        { label: 'Caty Gradient', src: catyGradient, bg: 'var(--ds-surface)' },
        { label: 'Ask Caty Dark', src: askCatyLight, bg: 'var(--ds-surface)' },
        ...
        <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 'var(--ds-font-size-200)', color: 'var(--ds-text-subtle)' }}>{label}</span>
          <span style={{ fontSize: 'var(--ds-font-size-100)', color: 'var(--ds-text-subtlest)' }}>SVG hover animations active</span>
        </div>
      ))}
    </div>
  ),
