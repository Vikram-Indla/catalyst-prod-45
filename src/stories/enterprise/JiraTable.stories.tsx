    ...
    return (
      <Frame>
        <div>
          ...
          />
          {sel.size > 0 && (
            <div style={{ padding: '8px 16px', background: 'var(--ds-surface-sunken, #F7F8F9)', borderTop: '1px solid var(--ds-border, #DFE1E6)', display: 'flex', alignItems: 'center', gap: 16, fontSize: 13, color: 'var(--ds-text-subtle, #42526E)' }}>
              <strong style={{ color: 'var(--ds-text, #172B4D)' }}>{sel.size} selected</strong>
              <span>·</span>
              <button onClick={() => setSel(new Set(SEED.map((r) => r.id)))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ds-link, #0052CC)', fontSize: 13 }}>
                Select all {SEED.length}
              </button>
              <span>·</span>
              <button onClick={() => setSel(new Set())} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ds-text-subtle, #42526E)', fontSize: 13 }}>
                Clear
              </button>
            </div>
          )}
  ...
  render: () => (
    <Frame>
      <JiraTable<BacklogRow>
        ...
        getRowId={(r) => r.id}
        emptyView={
          <div style={{ textAlign: 'center', padding: '48px 24px', color: 'var(--ds-text-subtle, #42526E)', fontSize: 14 }}>
