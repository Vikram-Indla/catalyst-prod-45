...
export function T10SidePanel({ item, isOpen, onClose, onUpdate, onDelete, isReadOnly = false }: T10SidePanelProps) {
  ...
  const getRankStyles = () => {
    if (rankTier === 'top') {
      return { background: 'linear-gradient(135deg, var(--ds-text-brand, var(--cp-workstream-catalyst-primary)) 0%, var(--ds-link-pressed) 100%)', color: 'white' };
    } else if (rankTier === 'mid') {
      return { background: 'var(--ds-text-subtlest)', color: 'white' };
    }
    return { background: 'transparent', border: '2px dashed var(--ds-border)', color: 'var(--ds-text-subtlest)' };
  };
      ...
      <div className={`t10-panel ${isOpen ? 'open' : ''}`}>
        ...
        <div className="t10-panel-header">
          <div className="t10-panel-title-group">
            ...
            <span className="t10-panel-title-text">Priority</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            {hasChanges && (
              <span style={{ fontSize: 'var(--ds-font-size-200)', color: 'var(--ds-text-subtlest)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <Save size={14} /> Saving...
              </span>
            )}
            ...
            <>
              ...
              <div className="t10-field">
                ...
                >
                  ...
                  <span style={{ fontSize: 'var(--ds-font-size-400)', color: isCompleted ? 'var(--ds-background-success-bold)' : 'var(--ds-text-subtlest)', fontWeight: isCompleted ? 600 : 400 }}>
