...
export const PRIORITY_CONFIG: Record<TaskPriority, { label: string; color: string; bgColor: string }> = {
  critical: { label: 'Critical', color: 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))', bgColor: 'var(--ds-background-danger, var(--ds-background-danger, #fef2f2))' },
  high: { label: 'High', color: 'var(--ds-text-warning, var(--ds-text-warning, #f59e0b))', bgColor: '#fffbeb' },
  medium: { label: 'Medium', color: '#8b5cf6', bgColor: '#f5f3ff' },
  low: { label: 'Low', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))', bgColor: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #f8fafc))' },
};

// Time section configuration
export const TIME_SECTION_CONFIG: Record<TimeSection, { label: string; icon: string; color: string }> = {
  overdue: { label: 'Overdue', icon: 'alert-circle', color: 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))' },
  today: { label: 'Today', icon: 'calendar', color: 'var(--ds-text-warning, var(--ds-text-warning, #f59e0b))' },
  this_week: { label: 'This Week', icon: 'calendar-days', color: 'var(--ds-text-brand, var(--ds-text-brand, #3b82f6))' },
  upcoming: { label: 'Upcoming', icon: 'calendar-plus', color: '#8b5cf6' },
  someday: { label: 'Someday', icon: 'cloud', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))' },
...
export const INVOLVEMENT_CONFIG: Record<InvolvementType, { label: string; icon: string; color: string }> = {
  assignee: { label: 'Assigned to you', icon: 'user', color: 'var(--ds-text-brand, var(--ds-text-brand, #3b82f6))' },
  creator: { label: 'Created by you', icon: 'plus', color: '#8b5cf6' },
  reporter: { label: 'Reported by you', icon: 'clipboard', color: '#06b6d4' },
  reviewer: { label: 'Needs your review', icon: 'check-square', color: 'var(--ds-text-warning, var(--ds-text-warning, #f59e0b))' },
  watcher: { label: "You're watching", icon: 'eye', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748b))' },
