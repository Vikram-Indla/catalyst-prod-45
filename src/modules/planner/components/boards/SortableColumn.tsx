...
export function SortableColumn({ 
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'boards-column dark:text-[var(--ds-text,var(--ds-text, #EDEDED))]',
    ...
    >
      ...
      <div className="boards-column__header group">
        <div className="boards-column__header-left">
          ...
          <div
            className="boards-column__drag-handle dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))] dark:hover:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))]"
            {...attributes}
            {...listeners}
          ...
          {/* Title */}
          <h3 className="boards-column__title dark:text-[var(--ds-text,var(--ds-text, #EDEDED))]">{column.name}</h3>

          {/* Task Count */}
          <span className="boards-column__count dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))]">{column.task_count}</span>
        </div>
      ...
      >
        ...
        {tasks.length === 0 && (
          <div className="boards-column__empty dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]">
            No tasks
          </div>
        )}

        {/* Add Task Button at bottom */}
        <button className="boards-add-task dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))] dark:hover:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] dark:hover:text-[var(--ds-text,var(--ds-text, #EDEDED))] dark:hover:border-[var(--ds-border-bold,var(--ds-border-bold, #454545))]" onClick={() => onAddTask?.(column.id)}>
          <Plus className="w-4 h-4" />
          Add Task
        </button>
      </div>
