                  ...
                  >
                    <motion.div
                      ...
                      style={{
                        ...
                        height: '64px',
                        borderRadius: '50%',
                        backgroundColor: 'var(--ds-background-success, var(--ds-background-success, #dcfce7))',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        marginBottom: '16px',
                    ...
                    >
                      <Check style={{ width: '32px', height: '32px', color: 'var(--ds-text-success, var(--ds-text-success, #16a34a))' }} />
                    </motion.div>
                    <h3 style={{
                      fontSize: '18px',
                      fontWeight: 600,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      color: COLORS.textPrimary,
                      backgroundColor: COLORS.surfaceCard,
                      border: `1px solid ${errors.title ? 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))' : COLORS.borderDefault}`,
                      borderRadius: '12px',
                      outline: 'none',
                    ...
                    onBlur={(e) => {
                      e.currentTarget.style.borderColor = errors.title ? 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))' : COLORS.borderDefault;
                      e.currentTarget.style.boxShadow = 'none';
                    }}
                  />
                  {errors.title && (
                    <span style={{ fontSize: '12px', color: 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))' }}>{errors.title}</span>
                  )}
                </div>
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (!isPending && title.trim()) {
                      e.currentTarget.style.backgroundColor = 'var(--ds-background-brand-bold-hovered, var(--ds-background-brand-bold-hovered, #1d4ed8))';
...
function PriorityDropdown({ value, onChange }: PriorityDropdownProps) {
  ...
  const currentColor = PRIORITY_COLORS[displayValue] || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))';
...
function WorkstreamDropdown({ value, onChange, workstreams, error }: WorkstreamDropdownProps) {
  ...
  const displayColor = selectedWorkstream?.color || WORKSTREAM_COLORS[displayName] || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))';
  ...
  return (
    <div ref={dropdownRef} style={{ display: 'flex', flexDirection: 'column', gap: '6px', position: 'relative' }}>
      <label style={{
        ...
        Workstream <span style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #dc2626))' }}>*</span>
      </label>
      ...
      <div
        ...
        style={{
          ...
          padding: '10px 14px',
          backgroundColor: COLORS.surfaceCard,
          border: `1px solid ${error ? 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))' : isOpen ? COLORS.borderFocus : (isHovered ? COLORS.borderDefault : COLORS.borderLight)}`,
          borderRadius: '12px',
          cursor: 'pointer',
          transition: 'all 0.15s ease',
          boxShadow: isOpen ? '0 0 0 3px rgba(59, 130, 246, 0.15)' : 'none'
      ...
      {error && <span style={{ fontSize: '12px', color: 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))' }}>{error}</span>}
      ...
      {isOpen && (
        ...
        >
          {workstreams.map((ws) => {
            const color = ws.color || WORKSTREAM_COLORS[ws.name] || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))';
            return (
              <DropdownItem
                key={ws.id}
                value={ws.name}
...
function AssigneeDropdown({ value, onChange, users }: AssigneeDropdownProps) {
  ...
  const getColorFromName = (name: string): string => {
    const colors = ['var(--ds-text-brand, var(--ds-text-brand, #3b82f6))', '#8b5cf6', '#ec4899', '#14b8a6', 'var(--ds-text-warning, var(--ds-text-warning, #f59e0b))', '#6366f1'];
    let hash = 0;
  ...
  const displayColor = selectedUser ? getColorFromName(selectedUser.name) : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))';
  ...
  return (
    <div ref={dropdownRef} style={{ display: 'flex', flexDirection: 'column', gap: '8px', position: 'relative' }}>
      <label style={{
        ...
        {label} {required && <span style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #dc2626))' }}>*</span>}
      </label>
      ...
      <div
        ...
        style={{
          ...
          padding: '10px 14px',
          backgroundColor: COLORS.surfaceCard,
          border: `1px solid ${error ? 'var(--ds-text-danger, var(--ds-text-danger, #ef4444))' : isOpen ? COLORS.borderFocus : (isHovered ? COLORS.borderFocus : COLORS.borderDefault)}`,
          borderRadius: '12px',
          cursor: 'pointer',
          transition: 'all 0.15s ease',
          boxShadow: isOpen ? '0 0 0 3px rgba(59, 130, 246, 0.15)' : 'none'
