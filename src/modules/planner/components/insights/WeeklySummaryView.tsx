...
function EmptyState({ title, icon: Icon }: { title: string; icon: any }) {
  return (
    <div className="flex flex-col items-center justify-center py-8 text-slate-400 dark:text-[#878787]">
...
export function WeeklySummaryView() {
  ...
  if (error || !data) {
    return (
      <div className="p-6 flex flex-col items-center justify-center h-full text-slate-500 dark:text-[#878787] dark:text-[#A1A1A1]">
  ...
  const kpiStrip = [
    { label: 'Stories', value: stories.length, colorClass: 'text-slate-800 dark:text-[#EDEDED]' },
    { label: 'Releases', value: releases.length, colorClass: 'text-emerald-600', highlight: releases.length > 0 },
    { label: 'Incidents', value: incidents.length, colorClass: incidents.length > 0 ? 'text-amber-600' : 'text-slate-800 dark:text-[#EDEDED]' },
    { label: 'Defects', value: defects.length, colorClass: defects.length > 0 ? 'text-red-600' : 'text-slate-800 dark:text-[#EDEDED]' },
  ];

  return (
    <ScrollArea className="h-full">
      <div className="p-6 bg-slate-100 dark:bg-[#0A0A0A] min-h-full">
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          {kpiStrip.map(kpi => (
            <div 
              key={kpi.label}
              className={cn(
                "bg-white dark:bg-[#1A1A1A] rounded-lg p-4 border border-slate-200 dark:border-[#2E2E2E] text-center transition-all hover:-translate-y-0.5 hover:shadow-md",
                kpi.highlight && "border-emerald-400 bg-emerald-50"
              )}
            >
              <div className={cn("text-3xl font-extrabold", kpi.colorClass)}>{kpi.value}</div>
              <div className="text-[10px] font-semibold text-slate-500 dark:text-[#878787] uppercase mt-1.5">{kpi.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-[#2E2E2E] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-[#2E2E2E]">
              ...
              <span className="text-xs text-slate-500 dark:text-[#878787]">{releases.length} this week</span>
            </div>
            <div className="p-4 space-y-3">
              {releases.length === 0 ? (
                ...
                releases.slice(0, 5).map((release: any) => (
                  <div key={release.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-[#111111] rounded-lg hover:bg-slate-100 dark:hover:bg-[#1F1F1F] transition-colors">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-blue-600 text-white font-bold text-xs">
                      REL
                    </div>
                    <div className="flex-1">
                      <div className="text-sm font-semibold text-slate-800 dark:text-[#EDEDED]">
                        {release.name || release.version}
                      </div>
                      <div className="text-xs text-slate-500 dark:text-[#878787] mt-0.5">
          ...
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-[#2E2E2E] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-[#2E2E2E]">
              ...
              <span className="text-xs text-slate-500 dark:text-[#878787]">{incidents.length} this week</span>
            </div>
            <div className="p-4 space-y-2">
              {incidents.length === 0 ? (
                ...
                incidents.slice(0, 5).map((incident: any) => (
                  <div key={incident.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-[#111111] rounded-lg hover:bg-slate-100 dark:hover:bg-[#1F1F1F] transition-colors">
                    <div className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-slate-800 dark:text-[#EDEDED] truncate">
                        {incident.title}
                      </div>
                      <div className="text-xs text-slate-500 dark:text-[#878787] mt-0.5">
          ...
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-[#2E2E2E] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-[#2E2E2E]">
              ...
              <span className="text-xs text-slate-500 dark:text-[#878787]">{defects.length} this week</span>
            </div>
            <div className="p-4 space-y-2">
              {defects.length === 0 ? (
                ...
                defects.slice(0, 5).map((defect: any) => (
                  <div key={defect.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-[#111111] rounded-lg hover:bg-slate-100 dark:hover:bg-[#1F1F1F] transition-colors">
                    <div className={cn(
                      "w-2.5 h-2.5 rounded-full flex-shrink-0",
          ...
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-[#2E2E2E] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-[#2E2E2E]">
              ...
              <span className="text-xs text-slate-500 dark:text-[#878787]">{stories.length} this week</span>
            </div>
            <div className="p-4 space-y-2">
              {stories.length === 0 ? (
                ...
                stories.slice(0, 5).map((story: any) => (
                  <div key={story.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-[#111111] rounded-lg hover:bg-slate-100 dark:hover:bg-[#1F1F1F] transition-colors">
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-slate-800 dark:text-[#EDEDED] truncate">
                        {story.title}
                      </div>
