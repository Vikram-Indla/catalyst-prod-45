...
export function WorkstreamDrawer({ workstream, isOpen, onClose }: WorkstreamDrawerProps) {
  ...
  const navigateToTasks = () => {
    navigate(`/task/task-list?workstream=${encodeURIComponent(workstream.slug || workstream.id)}`);
    onClose();
  };

  const navigateToBoard = () => {
    navigate(`/task/boards?workstream=${encodeURIComponent(workstream.slug || workstream.id)}`);
    onClose();
  };

  const navigateToCalendar = () => {
    navigate(`/task/calendar?workstream=${encodeURIComponent(workstream.slug || workstream.id)}`);
    onClose();
  };
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingBottom: '80px' }}>
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '16px' }}>
              ...
              <button
                onClick={() => {
                  navigate(`/task/task-list?workstream=${encodeURIComponent(workstream.slug || workstream.id)}&overdue=true`);
                  onClose();
                }}
                style={{ ...statCardStyle, cursor: 'pointer', transition: 'all 0.15s ease' }}
