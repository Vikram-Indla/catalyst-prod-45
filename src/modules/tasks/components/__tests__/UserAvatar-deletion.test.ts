 ...
 * (grep -rn 'import.*UserAvatar' src/modules/tasks/ returned nothing).
...
const TARGET = join(ROOT, 'src/modules/tasks/components/UserAvatar.tsx');
...
describe('planner/UserAvatar — dead code deletion', () => {
  it('src/modules/tasks/components/UserAvatar.tsx no longer exists', () => {
  ...
  it('no source file imports from the deleted planner UserAvatar', () => {
    ...
    for (const file of walk(join(ROOT, 'src'))) {
      ...
      if (!file.includes('/modules/tasks/')) continue;
