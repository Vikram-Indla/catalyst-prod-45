...
function SectionHeader({ icon: Icon, title, color }: { icon: any; title: string; color: string }) {
  return (
    <div className="flex items-center gap-3 mb-4">
      ...
      <h2 className="text-lg font-bold text-slate-800 dark:text-[var(--ds-text,var(--cp-bg-neutral))]">{title}</h2>
    </div>
  );
}
...
function EmptyState({ title }: { title: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-8 text-slate-400 dark:text-[var(--ds-text-subtlest,var(--cp-text-secondary))] bg-slate-50 dark:bg-[var(--ds-surface-overlay,var(--cp-ink-1))] rounded-lg">
...
function ItemCard({ 
  ...
  return (
    <div className={cn(
      "flex items-center gap-3.5 p-4 bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] border border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] rounded-lg transition-all hover:translate-x-1 hover:shadow-sm",
      "border-l-4",
      ...
      <div className="flex-1">
        <div className="text-[15px] font-semibold text-slate-800 dark:text-[var(--ds-text,var(--cp-bg-neutral))]">
          {id && <span className="text-blue-600 cursor-pointer hover:underline mr-2">{id}</span>}
          {title}
        </div>
        <div className="text-xs text-slate-500 dark:text-[var(--ds-text-subtlest,var(--cp-text-secondary))] mt-0.5">{meta}</div>
      </div>
    </div>
  );
}
...
export function MonthlyChronicleView() {
  ...
  if (error || !data) {
    return (
      <div className="p-6 flex flex-col items-center justify-center h-full text-slate-500 dark:text-[var(--ds-text-subtlest,var(--cp-text-secondary))]">
  ...
  return (
    <ScrollArea className="h-full">
      <div className="min-h-full">
        {/* Main Card */}
        <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] rounded-xl shadow-md border border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] overflow-hidden m-6">
          ...
          {/* Chronicle Footer */}
          <div className="flex items-center justify-between px-8 py-5 border-t border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] bg-slate-50 dark:bg-[var(--ds-surface-overlay,var(--cp-ink-1))]">
