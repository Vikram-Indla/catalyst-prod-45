...
const STATUS_OPTIONS = [
  { value: 'backlog', label: 'Backlog', color: 'var(--ds-text-subtlest)' },
  { value: 'planned', label: 'Planned', color: 'var(--ds-text-brand)' },
  { value: 'progress', label: 'In Progress', color: 'var(--ds-text-warning)' },
  { value: 'review', label: 'Review', color: 'var(--ds-background-discovery-bold)' },
  { value: 'done', label: 'Done', color: 'var(--ds-text-success)' },
];
...
function WorkstreamDropdown({ 
  ...
  const displayColor = selectedWorkstream?.color || 'var(--ds-text-subtlest)';
  ...
  return (
    <div ref={dropdownRef} style={{ position: 'relative' }}>
      ...
      >
        <Layers size={14} style={{ color: selectedId ? displayColor : 'var(--ds-text-subtlest)' }} />
        <span style={{ 
          flex: 1, 
          fontSize: 'var(--ds-font-size-300)', 
          fontWeight: 500, 
          color: selectedId ? COLORS.textPrimary : 'var(--ds-text-subtlest)',
        }}>
          {displayName}
        </span>
        <ChevronDown 
        ...
        >
          ...
          {workstreams.map((ws) => (
            <DropdownItem
              key={ws.id}
              value={ws.name}
              color={ws.color || 'var(--ds-text-brand)'}
...
function StatusDropdown({ 
  ...
  const displayColor = selected?.color || 'var(--ds-text-subtlest)';
    ...
    <div ref={dropdownRef} style={{ position: 'relative' }}>
      ...
      >
        ...
        <span style={{ 
          ...
          fontSize: 'var(--ds-font-size-300)', 
          fontWeight: 500, 
          color: selectedStatus ? COLORS.textPrimary : 'var(--ds-text-subtlest)',
        }}>
          {displayName}
        </span>
        <ChevronDown 
