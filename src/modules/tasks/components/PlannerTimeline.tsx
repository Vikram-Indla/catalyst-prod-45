...
// ============================================================
const STATUS_LEGEND = [
  { key: 'backlog', label: 'Backlog', color: 'var(--ds-text-subtlest)' },  // F2
  { key: 'planned', label: 'Planned', color: 'var(--ds-text-brand)' },  // F3
  { key: 'progress', label: 'In Progress', color: 'var(--ds-text-warning)' },  // F4
  { key: 'review', label: 'Review', color: 'var(--ds-background-discovery-bold)' },  // F5
  { key: 'done', label: 'Done', color: 'var(--ds-text-success)' },  // F6
];
    ...
    queryFn: async (): Promise<TimelineTask[]> => {
      ...
      return (tasks || []).map((t: any): TimelineTask => {
        ...
        return {
          ...
          workstream_color: t.workstream?.color || 'var(--ds-text-subtlest)',
...
export function PlannerTimeline({ onTaskClick }: PlannerTimelineProps) {
  ...
  return (
    <>
      {/* M4: Page background var(--ds-surface-sunken) */}
      <div className="timeline-enterprise h-full flex flex-col bg-[var(--ds-surface-sunken)] dark:bg-slate-950">
        {/* ============================================================
            TOP BAR - Title, Today, Date Nav, View Mode Toggle
            ============================================================ */}
        {/* M5: Container background var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface)))) */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--ds-border,var(--cp-bg-sunken))] dark:border-slate-800 bg-white dark:bg-slate-950 flex-shrink-0">
          {/* Left: Title */}
          <div>
            <h1 className="text-xl font-bold text-[var(--ds-text)] dark:text-slate-100">Timeline</h1>
            <p className="text-sm text-[var(--ds-text-subtlest)] dark:text-slate-400">
          ...
          <div className="flex items-center gap-3">
            {/* M3: Primary button color var(--ds-text-brand, var(--cp-workstream-catalyst-primary)) */}
            <Button 
              onClick={() => setIsCreateOpen(true)}
              className="bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))] hover:bg-[var(--ds-background-brand-bold-hovered)] text-white shadow-md"
            >
            ...
            {/* Today Button */}
            <Button variant="outline" size="sm" onClick={goToToday} className="gap-2 h-9 border-[var(--ds-border,var(--cp-bg-sunken))]">
            ...
            <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg">
              ...
              <span className="px-4 text-sm font-medium text-[var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, var(--ds-text-subtle))))] dark:text-slate-300 min-w-[160px] text-center">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    viewMode === mode
                      ...
                      : "text-[var(--ds-text-subtlest)] dark:text-slate-400 hover:text-[var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, var(--ds-text-subtle))))] dark:hover:text-slate-300"
                  )}
                >
        ...
        {/* ============================================================
            ...
            ============================================================ */}
        <div className="flex items-center gap-3 px-6 py-3 border-b border-[var(--ds-border,var(--cp-bg-sunken))] dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex-shrink-0">
          {/* Search */}
          <div className="relative w-64">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--ds-text-subtlest)]" />
            <Input
              ref={searchInputRef}
              type="search"
              ...
              className="pl-9 h-9 border-[var(--ds-border,var(--cp-bg-sunken))]"
            />
            {filters.search && (
              <button
                onClick={() => clearFilter('search')}
                className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--ds-text-subtlest)] hover:text-[var(--ds-text-subtlest)]"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                size="sm"
                className={cn(
                  "h-9 border-[var(--ds-border,var(--cp-bg-sunken))] filter-btn",
          ...
          {/* Clear Filters */}
          {hasActiveFilters && (
            <Button variant="ghost" size="sm" onClick={clearAllFilters} className="h-9 text-[var(--ds-text-subtlest)]">
          ...
          <div className="status-legend ml-auto flex items-center gap-4 text-xs text-[var(--ds-text-subtlest)]">
            {STATUS_LEGEND.map(s => (
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          {/* Column headers */}
          <div className="flex flex-shrink-0 border-b border-[var(--ds-border,var(--cp-bg-sunken))] dark:border-slate-800 bg-white dark:bg-slate-950">
            {/* Left header */}
            <div
              className="flex items-center px-4 border-r border-[var(--ds-border,var(--cp-bg-sunken))] dark:border-slate-800 bg-slate-50 dark:bg-slate-900 font-medium text-xs text-[var(--ds-text-subtlest)] dark:text-slate-400 uppercase tracking-wide"
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          // J5: Weekend columns — Subtle gray bg var(--ds-surface-sunken)
                          isWeekend && !isCurrentDay && "weekend bg-[var(--ds-surface-sunken)] dark:bg-slate-900/50",
                          // J3: Today column highlight Blue-50 var(--ds-background-selected)
                          isCurrentDay && "today bg-[var(--ds-background-selected)]"
                        )}
                        style={{ width: columnConfig.width }}
                      >
                        {isCurrentDay ? (
                          // J4: Today number style — Blue circle badge
                          <div className="today-badge flex flex-col items-center justify-center w-9 h-9 rounded-full bg-[var(--ds-text-brand)] text-white">
                          ...
                          <>
                            {/* J1: Day name 10-11px uppercase */}
                            <span className="day-name text-[10px] font-medium text-[var(--ds-text-subtlest)] dark:text-slate-500 uppercase">
                              {format(date, 'EEE')}
                            </span>
                            {/* J2: Day number 13-14px */}
                            <span className="day-number text-[13px] font-semibold text-[var(--ds-text-subtle)] dark:text-slate-300">
                              {format(date, 'd')}
                            </span>
                          </>
                        )}
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="relative flex">
              ...
              <div
                className="workstream-panel flex-shrink-0 bg-white dark:bg-slate-950 border-r border-[var(--ds-border,var(--cp-bg-sunken))] dark:border-slate-800"
                ...
                {swimlanes.map((lane) => {
                  ...
                  return (
                    <div key={lane.id} data-workstream={lane.name}>
                      {/* Workstream Header */}
                      <div
                        className="workstream-row w-full flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-900/70 border-b border-[var(--ds-border,var(--cp-bg-sunken))] dark:border-slate-700 hover:bg-[var(--ds-surface-sunken)] dark:hover:bg-slate-800 transition-colors text-left overflow-hidden"
                      ...
                      >
                        ...
                        >
                          <ChevronRight className="chevron w-4 h-4 text-[var(--ds-text-subtlest)]" />
                        </button>
                        ...
                        <div className="workstream-info flex-1 min-w-0">
                          {/* I2-I3: Workstream name font-weight 500, color var(--ds-text) */}
                          <button
                            onClick={() => wsForDrawer && openWorkstreamDrawer(wsForDrawer)}
                            className="workstream-name font-medium text-sm text-[var(--ds-text)] dark:text-slate-200 hover:text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))] dark:hover:text-blue-400 transition-colors truncate text-left w-full"
                            title={lane.name}
                          >
                            {lane.name}
                          </button>

                          <div className="workstream-lead flex items-center gap-1.5 text-xs text-[var(--ds-text-subtle)] dark:text-slate-400 mt-0.5 min-w-0">
                            {lane.lead ? (
                              <>
                                <div
                              ...
                              // I4: "No lead assigned" color Gray var(--ds-text-subtlest)
                              <span className="no-lead text-[var(--ds-text-subtlest)]">No lead assigned</span>
                            )}
                          </div>

                          {/* I5: Task stats color Gray var(--ds-text-subtlest) */}
                          <div className="workstream-stats text-[11px] text-[var(--ds-text-subtlest)] dark:text-slate-500 mt-1 truncate">
                            {lane.taskCount} tasks
                            {lane.inProgressCount > 0 &&
                              ` · ${lane.inProgressCount} in progress`}
                            {lane.overdueCount > 0 && (
                              // D3: Overdue text red var(--ds-text-danger)
                              <span className="overdue-count text-[var(--ds-text-danger)] font-semibold">
                  ...
                  <div style={{ minWidth: columnConfig.days * columnConfig.width }}>
                    <div className="relative">
                      ...
                      {showTodayLine && (
                        ...
                        >
                          {/* K3: TODAY label — Blue bg var(--ds-text-brand), white text */}
                          <div className="today-label absolute -top-1 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-[var(--ds-text-brand)] text-white text-[10px] font-semibold rounded uppercase whitespace-nowrap z-30">
                            TODAY
                          </div>
                          <div className="absolute top-4 left-1/2 -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-6 border-l-transparent border-r-transparent border-t-[var(--ds-text-brand)]" />
                          {/* K1-K2: Today line — Blue var(--ds-text-brand), 2px width */}
                          <div className="w-0.5 h-full bg-[var(--ds-text-brand)] mt-7" />
                        </div>
                      )}
                      ...
                      {swimlanes.map(lane => {
                        ...
                        return (
                          <div key={lane.id} data-workstream={lane.name}>
                            ...
                            <div
                              className="relative flex bg-slate-50 dark:bg-slate-900/70 border-b border-[var(--ds-border,var(--cp-bg-sunken))] dark:border-slate-700"
                              style={{ height: SWIMLANE_HEADER_HEIGHT }}
                            >
                              {/* J5: Weekend shading */}
                                ...
                                <div className="absolute inset-0 flex pointer-events-none">
                                  {dateColumns.map((date, i) => {
                                    ...
                                    return (
                                      <div
                                        key={i}
                                        className={cn("flex-shrink-0", isWeekend && "bg-[var(--ds-surface-sunken)]/50 dark:bg-slate-800/20")}
                                        style={{ width: columnConfig.width }}
                                      />
                                    );
                                  })}
                                      ...
                                      content={
                                        <div className="space-y-2 max-w-xs p-3">
                                          ...
                                          <div className="text-xs space-y-1 text-slate-600 dark:text-slate-400">
                                            ...
                                            {task.is_overdue && (
                                              <div className="text-[var(--ds-text-danger)] font-medium mt-1 flex items-center gap-1">
                                    ...
                                    >
                                      <div
                                        ...
                                        style={{
                                          ...
                                          // D1: OVERDUE — Red left border 3px solid var(--ds-text-danger)
                                      ...
                                      >
                                        ...
                                        <span
                                          ...
                                          style={{ color: wsColors.textPrimary || 'var(--ds-text-subtle)' }}
                                        >
                                          {task.key}
                                        </span>
                                        ...
                                        {showTitle && (
                                          <span
                                            ...
                                            style={{ color: wsColors.textDark || 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, var(--ds-text-subtle))))' }}
                                          >
                                            {task.title}
                                          </span>
                                        )}
                                        ...
                                        {task.is_overdue && (
                                          <span className="overdue-dot absolute -top-1 -right-1 w-2 h-2 bg-[var(--ds-text-danger)] rounded-full border-2 border-white" />
                                        )}
                                      </div>
                                    </Tooltip>
                                  );
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-slate-950/80 z-40">
            <div className="text-center">
              <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center mx-auto mb-4">
                <CalendarDays className="w-8 h-8 text-[var(--ds-text-subtlest)] dark:text-slate-500" />
              </div>
              <h3 className="text-lg font-medium text-[var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, var(--ds-text-subtle))))] dark:text-slate-300 mb-1">
        ...
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-slate-950/80 z-40">
            <div className="flex flex-col items-center gap-3">
              <div className="w-8 h-8 border-2 border-[var(--ds-text-brand)] border-t-transparent rounded-full animate-spin" />
              <span className="text-sm text-[var(--ds-text-subtlest)]">Loading timeline...</span>
            </div>
          </div>
        )}
