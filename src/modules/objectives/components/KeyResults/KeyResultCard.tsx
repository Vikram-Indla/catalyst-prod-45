...
import { KeyResultCheckIn } from './KeyResultCheckIn';
import { formatMetricValue } from '@/modules/objectives/utils/scoreCalculations';
      ...
      <Card className={cn('p-[var(--s4)]', className)}>
        ...
        <div className="space-y-[var(--s2)] mb-[var(--s3)]">
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>{formatMetricValue(startValue, keyResult.metric_type)}</span>
            <span>
              {formatMetricValue(currentValue, keyResult.metric_type)} / {formatMetricValue(targetValue, keyResult.metric_type)}
            </span>
          </div>
        </div>
                ...
                {keyResult.checkins.map((checkin: any) => (
                  <div key={checkin.id} className="text-xs space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="font-medium">{formatMetricValue(checkin.value, keyResult.metric_type)}</span>
