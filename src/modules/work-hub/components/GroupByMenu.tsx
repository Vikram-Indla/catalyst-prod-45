      ...
      <DropdownMenuTrigger asChild>
        <Button 
          ...
          className={cn(
            ...
            value !== 'none' ? "text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))] bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))]/10 dark:text-[var(--ds-text-brand)] dark:bg-[var(--ds-text-brand)]/10" : "text-muted-foreground"
          )}
        >
          Group
        ...
        {groupOptions.map((option) => (
          <DropdownMenuItem
            ...
            className={cn(
              ...
              value === option.id && "bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))]/10 dark:bg-[var(--ds-text-brand)]/10"
            )}
            onClick={() => onChange(value === option.id ? 'none' : option.id)}
          >
            {option.icon}
            <span className="flex-1">{option.label}</span>
            {value === option.id && <Check className="h-4 w-4 text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))] dark:text-[var(--ds-text-brand)]" />}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
