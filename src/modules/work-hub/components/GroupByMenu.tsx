      ...
      <DropdownMenuTrigger asChild>
        <Button 
          ...
          className={cn(
            ...
            value !== 'none' ? "text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))] bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))]/10 dark:text-[var(--ds-text-brand,#60a5fa)] dark:bg-[var(--ds-text-brand,#3b82f6)]/10" : "text-muted-foreground"
          )}
        >
          Group
        ...
        {groupOptions.map((option) => (
          <DropdownMenuItem
            ...
            className={cn(
              ...
              value === option.id && "bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))]/10 dark:bg-[var(--ds-text-brand,#3b82f6)]/10"
            )}
            onClick={() => onChange(value === option.id ? 'none' : option.id)}
          >
            {option.icon}
            <span className="flex-1">{option.label}</span>
            {value === option.id && <Check className="h-4 w-4 text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))] dark:text-[var(--ds-text-brand,#60a5fa)]" />}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
