...
const statusStyles: Record<string, { bg: string; text: string; label: string }> = {
  'Backlog': { bg: 'bg-[var(--ds-border,var(--ds-border, #DFE1E6))]', text: 'text-[#42526E]', label: 'BACKLOG' },
  'To Do': { bg: 'bg-[var(--ds-border,var(--ds-border, #DFE1E6))]', text: 'text-[#42526E]', label: 'TO DO' },
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ borderBottom: `1px solid ${'var(--cp-border, #DFE1E6)'}` }}
      >
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" className={cn("h-7 w-7", isDark ? "hover:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))]" : "hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F4F5F7))]")}>
          ...
          <Button variant="ghost" size="icon" className={cn("h-7 w-7", isDark ? "hover:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))]" : "hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F4F5F7))]")} onClick={onClose}>
      ...
      {item.parent && (
        ...
        >
          <div className="flex items-center gap-2 text-[12px]">
            <span style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>Parent:</span>
            <button
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {/* Title Section */}
        <div className="px-4 py-4" style={{ borderBottom: `1px solid ${'var(--cp-border-lt, #F1F5F9)'}` }}>
          <h1 className="text-[18px] font-semibold leading-tight mb-3" style={{ color: 'var(--cp-text-primary, #172B4D)' }}>
            {item.summary}
          </h1>
        ...
        <Tabs defaultValue="details" className="flex-1 flex flex-col overflow-hidden">
          ...
          <TabsContent value="details" className="flex-1 overflow-auto mt-0 p-4">
            {/* Description */}
            <div className="mb-6">
              <div className="text-[11px] font-semibold uppercase mb-2" style={{ color: 'var(--cp-text-tertiary, #6B778C)' }}>Description</div>
              <div
              ...
              >
                {item.description || (
                  <span style={{ color: 'var(--cp-text-muted, #94A3B8)', fontStyle: 'italic' }}>Click to add description...</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="text-[11px] font-semibold uppercase" style={{ color: 'var(--cp-text-tertiary, #6B778C)' }}>Details</div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {item.assignee ? (
                      <>
                        ...
                        <span className="text-[13px]" style={{ color: 'var(--cp-text-primary, #172B4D)' }}>{item.assignee}</span>
                      </>
                    ) : (
                      <span className="text-[13px]" style={{ color: 'var(--cp-text-muted, #94A3B8)' }}>Unassigned</span>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px]" style={{ color: 'var(--cp-text-primary, #172B4D)' }}>{item.reporter}</span>
                  </div>
                </div>

                {/* Type */}
                <div>
                  <div className="text-[11px] mb-1" style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>Type</div>
                  <div className="flex items-center gap-2">
                    <JiraIssueTypeIcon type={item.type} size={16} />
                    <span className="text-[13px]" style={{ color: 'var(--cp-text-primary, #172B4D)' }}>{item.type}</span>
                  </div>
                </div>

                {/* Created */}
                <div>
                  <div className="text-[11px] mb-1" style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>Created</div>
                  <span className="text-[13px]" style={{ color: 'var(--cp-text-primary, #172B4D)' }}>{item.created}</span>
                </div>
              </div>

              {/* Links Section */}
              <div className="pt-4" style={{ borderTop: `1px solid ${'var(--cp-border-lt, #F1F5F9)'}` }}>
                <div className="flex items-center justify-between mb-2">
                  <div className="text-[11px] font-semibold uppercase" style={{ color: 'var(--cp-text-tertiary, #6B778C)' }}>Links</div>
                ...
                {item.parent ? (
                  ...
                  >
                    <div className="text-[10px] uppercase mb-1" style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>is child of</div>
                    <button
                      onClick={() => onNavigateToParent?.(item.parent!, item.parentType || 'Feature')}
                    ...
                    >
                      ...
                      <span className="text-[12px]" style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>
                        {item.parentType === 'Epic' ? '(Epic)' : '(Feature)'}
                      </span>
                    </button>
                  </div>
                ) : (
                  <div className="text-[12px] italic" style={{ color: 'var(--cp-text-muted, #94A3B8)' }}>No links</div>
                )}
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="comments" className="flex-1 overflow-auto mt-0 p-4">
            <div className="text-[13px] text-center py-8" style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>
              No comments yet
            </div>
          </TabsContent>
