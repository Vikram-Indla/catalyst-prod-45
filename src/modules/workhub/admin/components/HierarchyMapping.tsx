...
export function HierarchyMapping() {
  ...
  if (isLoading) {
    return <div style={{ padding: 40, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', fontFamily: 'var(--cp-font-body)' }}>Loading hierarchy config...</div>
  }
    ...
    <div style={{ maxWidth: 900, fontFamily: 'var(--cp-font-body)' }}>
      ...
      {isDirty && (
        <div style={{
          ...
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))', flexShrink: 0 }} />
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          ...
          <span style={{
            fontSize: 9, background: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', padding: '2px 8px',
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {levels.map((l, idx) => (
            ...
            >
              <span style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', fontSize: 16, cursor: 'grab', userSelect: 'none' }}>⠿</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-1, #0F172A)', width: 100 }}>
                {l.level}. {l.name}
              </span>
              <span style={{ fontSize: 11, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', flex: 1 }}>
        ...
        {levels.map(l => (
          <div key={l.name} style={{ marginBottom: 16 }}>
            <label style={{
              ...
              color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', letterSpacing: '.3px', fontWeight: 600, display: 'block', marginBottom: 6,
            }}>
              Level: {l.name}
            </label>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: isOn ? 'var(--ds-background-brand-bold-hovered, var(--ds-background-brand-bold-hovered, #1D4ED8))' : 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))',
                    }}
                  >
                    {chip}
                  </button>
