  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
        <label style={{ fontSize: '11px', fontWeight: 600, color: '#64748B', textTransform: 'uppercase', letterSpacing: '.4px', fontFamily: 'var(--cp-font-body)' }}>
          {label}
        </label>
        {selected.length > 0 && (
          <button
      ...
      <div ref={containerRef} style={{ position: 'relative' }}>
        ...
        >
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', flex: 1, alignItems: 'center' }}>
            {selected.length === 0 ? (
              <span style={{ fontSize: '12px', color: '#94A3B8', fontFamily: 'var(--cp-font-body)' }}>{placeholder}</span>
            ) : selected.length <= 3 ? (
              selected.map(v => {
        ...
        {open && (
          <div style={{
            ...
            <div style={{ padding: '8px 10px', borderBottom: '1px solid #F1F5F9', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <input
                ...
                style={{
                  ...
                  fontSize: '12px', fontFamily: 'var(--cp-font-body)', color: '#334155',
            ...
            <div style={{ padding: '6px 10px', borderBottom: '1px solid #F1F5F9', display: 'flex', gap: '12px' }}>
              ...
              <button
                onClick={() => onChange([])}
                style={{ fontSize: '10px', color: '#64748B', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--cp-font-body)', fontWeight: 600 }}
              >
                Deselect all
              </button>
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <span style={{ fontSize: '12px', color: '#334155', fontFamily: 'var(--cp-font-body)', fontWeight: isSelected ? 600 : 400 }}>
                          {opt.label}
                        </span>
                        {opt.sublabel && (
                          <span style={{ fontSize: '10px', color: '#94A3B8', marginLeft: '6px', fontFamily: 'var(--cp-font-body)' }}>
                            {opt.sublabel}
                          </span>
                        )}
                      </div>
