...
export function SyncLogs() {
  ...
  return (
    <div className="wh-module space-y-6">
      {/* Page Header */}
      <div>
        <h1 style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: '18px', fontWeight: 700, color: 'var(--fg-1, #0F172A)' }}>
          Sync & Logs
        </h1>
      ...
      {syncStatus === 'error' && lastError && (
        <div style={{ background: '#FEF2F2', border: '1px solid #FCA5A5', borderRadius: '8px', padding: '16px 20px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          ...
          <div>
            <div style={{ fontFamily: 'var(--ds-font-family-heading)', fontWeight: 600, fontSize: '13px', color: '#EF4444' }}>Sync Failed</div>
      ...
      <div style={{ background: 'var(--bg-1, #F8FAFC)', border: '1px solid var(--bd-default, #E2E8F0)', borderRadius: '8px', overflow: 'visible' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: '13px', fontWeight: 700, color: 'var(--fg-1, #0F172A)' }}>
              Sync Filters
            </span>
            {hasFilters && (
              <span style={{ fontSize: '10px', padding: '1px 8px', borderRadius: '12px', background: '#DBEAFE', color: '#2563EB', fontWeight: 600, fontFamily: 'var(--ds-font-family-body)' }}>
                {activeFilterCount} active
              </span>
            )}
          </div>
          {filtersOpen ? <ChevronDown size={14} style={{ color: '#94A3B8' }} /> : <ChevronRight size={14} style={{ color: '#94A3B8' }} />}
        </button>
        ...
        {filtersOpen && (
          <div style={{ padding: '0 20px 20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {/* Date Range */}
            <div>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: '#64748B', textTransform: 'uppercase', letterSpacing: '.4px', marginBottom: '8px', fontFamily: 'var(--ds-font-family-body)' }}>
                Date Range (Lookback)
              </label>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: '4px' }}>
              <button
                ...
                style={{
                  ...
                  cursor: saveFilters.isPending ? 'not-allowed' : 'pointer', fontFamily: 'var(--ds-font-family-body)',
                }}
              >
                <Save size={13} />
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {hasFilters && !isSyncing && (
          <span style={{ fontSize: '11px', color: '#64748B', fontFamily: 'var(--ds-font-family-body)' }}>
        ...
        {!hasFilters && !isSyncing && (
          <span style={{ fontSize: '11px', color: '#EF4444', fontFamily: 'var(--ds-font-family-body)' }}>
          ...
          {confirmFullSync ? (
            ...
            <button
              ...
              style={{
                padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--bd-default, #E2E8F0)',
                background: 'var(--bg-app, #fff)', color: '#64748B', fontSize: '11px', fontWeight: 600,
                cursor: isSyncing ? 'not-allowed' : 'pointer', fontFamily: 'var(--ds-font-family-body)',
              }}
            >
        ...
        {syncStatus === 'error' && (
          <button
            ...
            style={{
              ...
              fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)',
            }}
          >
            <Trash2 size={14} />
            Clear Error & Retry
      ...
      <div style={{ background: 'var(--bg-1, #F8FAFC)', border: '1px solid var(--bd-default, #E2E8F0)', borderRadius: '8px', padding: '20px' }}>
        <div style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: '13px', fontWeight: 700, color: 'var(--fg-1, #0F172A)', marginBottom: '16px' }}>
          Sync Schedule
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <label style={{ width: '180px', fontSize: '12px', fontWeight: 500, color: '#334155', fontFamily: 'var(--ds-font-family-body)' }}>Incremental sync every:</label>
            <select value={intervalMin} onChange={(e) => setIntervalMin(Number(e.target.value))} style={{ height: '50px', padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--bd-default, #E2E8F0)', fontSize: '12px', color: '#334155', fontFamily: 'var(--ds-font-family-body)', background: 'var(--bg-app, #fff)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <label style={{ width: '180px', fontSize: '12px', fontWeight: 500, color: '#334155', fontFamily: 'var(--ds-font-family-body)' }}>Full sync daily at:</label>
            <select value={fullSyncTime} onChange={(e) => setFullSyncTime(e.target.value)} style={{ height: '50px', padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--bd-default, #E2E8F0)', fontSize: '12px', color: '#334155', fontFamily: 'var(--ds-font-family-body)', background: 'var(--bg-app, #fff)' }}>
          ...
          <div style={{ marginTop: '8px' }}>
            <button onClick={handleSaveSchedule} disabled={updateSchedule.isPending} style={{ padding: '7px 16px', borderRadius: '6px', border: '1px solid var(--bd-default, #E2E8F0)', background: 'var(--bg-app, #fff)', color: '#334155', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)' }}>
      ...
      <div style={{ background: 'var(--bg-app, #fff)', border: '1px solid var(--bd-default, #E2E8F0)', borderRadius: '8px', overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--bd-default, #E2E8F0)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: '13px', fontWeight: 700, color: 'var(--fg-1, #0F172A)' }}>Sync Log</span>
        ...
        <div style={{ maxHeight: '340px', overflowY: 'auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '140px 90px 80px 1fr 100px 70px', padding: '8px 20px', background: 'var(--bg-1, #F8FAFC)', borderBottom: '1px solid var(--bd-default, #E2E8F0)', position: 'sticky', top: 0, zIndex: 1 }}>
            {['TIMESTAMP', 'TYPE', 'STATUS', 'DETAILS', 'PROJECTS', 'DURATION'].map(h => (
              <span key={h} style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: '10px', fontWeight: 600, color: '#94A3B8', letterSpacing: '.5px', textTransform: 'uppercase', textAlign: h === 'DURATION' ? 'right' : 'left' }}>{h}</span>
            ))}
          </div>
          {logsLoading ? (
            <div style={{ padding: '40px', textAlign: 'center' }}><Loader2 size={20} className="animate-spin" style={{ color: '#94A3B8', margin: '0 auto' }} /></div>
          ) : !logs || logs.length === 0 ? (
  ...
  return (
    <div style={{ background: 'var(--bg-1, #F8FAFC)', border: '1px solid var(--bd-default, #E2E8F0)', borderRadius: '6px', padding: '14px', textAlign: 'center' }}>
      {loading ? (
        ...
        <div style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: '18px', fontWeight: 700, color: valueColor, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
          {spinning && <Loader2 size={14} className="animate-spin" />}
          {value}
        </div>
      )}
      <div style={{ fontSize: '9px', textTransform: 'uppercase', color: '#94A3B8', letterSpacing: '.3px', marginTop: '4px', fontFamily: 'var(--ds-font-family-body)', fontWeight: 600 }}>{label}</div>
    </div>
  )
}
...
function LogRow({ log, formatDuration }: { log: SyncLogEntry; formatDuration: (ms: number) => string }) {
  ...
  return (
    ...
    >
      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: '11px', color: '#64748B' }}>{timestamp}</span>
      <span><span style={{ fontSize: '9px', padding: '2px 8px', borderRadius: '4px', background: tc.bg, color: tc.text, fontWeight: 600, textTransform: 'capitalize', fontFamily: 'var(--ds-font-family-body)' }}>{log.sync_type}</span></span>
      <span><span style={{ fontSize: '10px', padding: '2px 10px', borderRadius: '12px', background: sc.bg, color: sc.text, fontWeight: 600, textTransform: 'capitalize', fontFamily: 'var(--ds-font-family-body)' }}>{log.status}</span></span>
      <span style={{ color: '#334155', fontSize: '12px', fontFamily: 'var(--ds-font-family-body)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{details}</span>
      <span style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>
        {projectChips ? (
          projectChips.length <= 3 ? (
            projectChips.map(p => (
              <span key={p} style={{ fontSize: '9px', padding: '1px 6px', borderRadius: '4px', background: '#E0F2FE', color: '#0369A1', fontWeight: 600, fontFamily: 'var(--ds-font-family-body)' }}>{p}</span>
            ))
          ) : (
            <span style={{ fontSize: '9px', padding: '1px 6px', borderRadius: '4px', background: '#E0F2FE', color: '#0369A1', fontWeight: 600, fontFamily: 'var(--ds-font-family-body)' }}>{projectChips.length} projects</span>
          )
        ) : (
          <span style={{ fontSize: '9px', color: '#CBD5E1' }}>All</span>
        )}
      </span>
      <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: '11px', color: '#94A3B8', textAlign: 'right' }}>{log.duration_ms ? formatDuration(log.duration_ms) : '—'}</span>
    </div>
  )
}