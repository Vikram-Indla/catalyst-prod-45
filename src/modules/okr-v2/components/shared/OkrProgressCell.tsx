...
function getProgressBarColor(actual?: number | null): string {
  ...
  if (actual >= 80) return 'bg-[var(--ds-text-success,var(--ds-text-success, #22c55e))]';  // Success - On Track
  if (actual >= 50) return 'bg-[var(--ds-text-warning,var(--ds-text-warning, #f59e0b))]';  // Warning - At Risk
  return 'bg-[var(--ds-text-danger,var(--ds-text-danger, #ef4444))]';                     // Danger - Off Track
}
...
function TrendArrow({ trend }: { trend: TrendCode }) {
  ...
  if (trend === 'ahead' || trend === 'on-track') {
    return (
      <TrendingUp className="h-3.5 w-3.5 text-[var(--ds-text-success,var(--ds-text-success, #22c55e))] flex-shrink-0" />
    );
  }
  ...
  if (trend === 'at-risk') {
    return (
      <TrendingDown className="h-3.5 w-3.5 text-[var(--ds-text-warning,var(--ds-text-warning, #f59e0b))] flex-shrink-0" />
    );
  }
  ...
  if (trend === 'off-track') {
    return (
      <TrendingDown className="h-3.5 w-3.5 text-[var(--ds-text-danger,var(--ds-text-danger, #ef4444))] flex-shrink-0" />
    );
  }
  ...
  return (
    <div className="flex items-center justify-start gap-3 w-full">
      ...
      <div className={cn(
        ...
        isDark ? 'bg-[var(--ds-border,var(--ds-border, #292929))]' : 'bg-[#e5e5e5]',
        compact ? 'w-20' : 'w-28'
      )}>
        <div
      ...
      {/* Percentage */}
      <span className={cn("text-[14px] font-semibold flex-shrink-0 min-w-[36px] text-left tabular-nums", isDark ? 'text-[var(--ds-text,var(--ds-text, #EDEDED))]' : 'text-[#171717]')}>
        {Math.round(actual)}%
      </span>
