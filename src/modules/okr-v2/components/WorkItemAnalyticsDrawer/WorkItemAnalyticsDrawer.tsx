// ═══════════════════════════════════════════════════════════════════════════════
// CATALYST WORK ITEM (DELIVERY ITEM) ANALYTICS DRAWER
// Executive-ready analytics panel for Epics, Features, Stories
// Shows Performance, Risks, Alignment, Insights
// ═══════════════════════════════════════════════════════════════════════════════
...
import { useWorkItemAnalytics } from '../../hooks/useWorkItemAnalytics';
import type { WorkItemAnalyticsData } from '../../lib/workItemAnalytics';
import type { TrendDirection, InsightSeverity } from '../../lib/sharedAnalyticsTypes';
import type { AnalyticsRiskSummary } from '../../lib/okrRiskTypes';

interface WorkItemAnalyticsDrawerProps {
...
function TypeBadge({ type }: { type: string }) {
  return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-semibold uppercase tracking-wider bg-secondary-bronze/10 text-secondary-bronze border border-secondary-bronze/20">
...
// ─────────────────────────────────────────────────────────────────────────────────
// WORK ITEM SPECIFIC SECTIONS
// ─────────────────────────────────────────────────────────────────────────────────

function PerformanceCard({ analytics }: { analytics: WorkItemAnalyticsData }) {
  const { baseline, progress } = analytics;
...
function RisksSection({ risks }: { risks: AnalyticsRiskSummary }) {
  const hasNoRisks = risks.totals.high === 0 && risks.totals.medium === 0 && risks.blockedItems === 0 && risks.delayedItems === 0;
...
function DrawerContent({ analytics }: { analytics: WorkItemAnalyticsData }) {
  const { risks, insights, alignment, workItemType } = analytics;
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <CollapsibleSection title="Alignment & Context">
          <div className="p-4 bg-muted/50 rounded-lg border border-border space-y-2">
            ...
            {alignment.objectiveName && (
              <div className="flex items-center gap-3 text-sm pl-5">
                <ArrowRight className="h-4 w-4 text-muted-foreground" />
                <span className="text-muted-foreground">{alignment.objectiveName}</span>
              </div>
            )}
            {alignment.krName && (
              <div className="flex items-center gap-3 text-sm pl-10">
                <ArrowRight className="h-4 w-4 text-muted-foreground" />
                <span className="text-muted-foreground truncate">{alignment.krName}</span>
              </div>
            )}
          </div>
        </CollapsibleSection>
...
export function WorkItemAnalyticsDrawer({ workItemId, open, onClose, snapshotId }: WorkItemAnalyticsDrawerProps) {
  const { analytics, isLoading, error } = useWorkItemAnalytics(workItemId, snapshotId);
    ...
    <Sheet open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <SheetContent className="w-screen sm:w-[540px] sm:max-w-[540px] p-0 border-l border-border overflow-hidden" aria-describedby={undefined}>
        <SheetHeader className="sr-only">
          <SheetTitle>Delivery Item Analytics</SheetTitle>
        </SheetHeader>
        ...
        {isLoading ? (
          ...
          <div className="p-6 text-center text-muted-foreground">Delivery item not found</div>
        )}
      </SheetContent>
    </Sheet>
  );
