    ...
    <>
      ...
      {showTooltip && createPortal(
        <div style={{
          ...
          <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--fg-1, var(--cp-ink-1, #0F172A))', marginBottom: 6 }}>
            {item.request_key}: {item.title}
          </div>
          <div className="flex items-center gap-1.5" style={{ fontSize: 12, color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', marginBottom: 4 }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))' }}>
              {(item as any).initiative_type_label || typeKey}
            </span>
            {item.progress > 0 && (
              <div className="flex items-center gap-1 ml-auto">
                <div style={{ width: 60, height: 4, background: 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', borderRadius: 999, overflow: 'hidden' }}>
                  <div style={{ width: `${item.progress}%`, height: '100%', background: finalColor, borderRadius: 999 }} />
                </div>
                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--fg-1, var(--cp-ink-1, #0F172A))' }}>{item.progress}%</span>
              </div>
            )}
          </div>
        </div>,
