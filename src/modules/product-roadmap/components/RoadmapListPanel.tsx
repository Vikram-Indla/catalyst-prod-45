...
export function RoadmapListPanel({
  ...
  const renderHeader = () => (
    <div
      ...
      style={{ height: 44, borderBottom: '1px solid var(--bd-default, #E2E8F0)', background: 'var(--ds-surface-sunken, #FAFBFC)' }}
    >
      ...
      <ArrowUpDown className="w-3.5 h-3.5" style={{ color: 'var(--ds-text-subtlest, #94A3B8)' }} />
    </div>
  );
  ...
  const renderAddRow = () => (
    <button
      ...
      style={{ color: 'var(--ds-text-brand, #2563EB)', borderTop: '1px solid #F1F5F9' }}
    >
        ...
        <ScrollArea className="flex-1">
          <div role="table">
            {groups.map(group => {
              ...
              const color = TYPE_COLORS[typeKey] || group.color || 'var(--ds-text-subtlest, #94A3B8)';
              return (
                <div key={group.key} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  ...
                  >
                    {group.isExpanded ? <ChevronDown className="w-3.5 h-3.5" style={{ color: 'var(--ds-text-subtlest, #94A3B8)' }} /> : <ChevronRight className="w-3.5 h-3.5" style={{ color: 'var(--ds-text-subtlest, #94A3B8)' }} />}
                    <div className="w-3 h-3 rounded-sm flex-shrink-0" style={{ background: color }} />
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ds-text-subtle, #334155)' }}>{group.label}</span>
                    <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--ds-text-subtlest, #94A3B8)', marginLeft: 'auto' }}>{group.items.length}</span>
                  </button>
