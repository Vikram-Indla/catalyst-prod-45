  ...
  const renderHeader = () => (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span
          style={{
            fontSize: 10, fontWeight: 600, color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', background: 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))',
      ...
      <ArrowUpDown className="w-3.5 h-3.5" style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }} />
    </div>
  );
        ...
        <ScrollArea className="flex-1">
          <div role="table">
            {groups.map(group => {
              ...
              const color = TYPE_COLORS[typeKey] || group.color || 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))';
              return (
                <div key={group.key} style={{ borderBottom: '1px solid var(--cp-bg-sunken, #F1F5F9)' }}>
                  ...
                  >
                    {group.isExpanded ? <ChevronDown className="w-3.5 h-3.5" style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }} /> : <ChevronRight className="w-3.5 h-3.5" style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))' }} />}
                    <div className="w-3 h-3 rounded-sm flex-shrink-0" style={{ background: color }} />
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ds-text-subtle, #334155)' }}>{group.label}</span>
                    <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))', marginLeft: 'auto' }}>{group.items.length}</span>
                  </button>
