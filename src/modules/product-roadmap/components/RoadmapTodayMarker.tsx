...
export function RoadmapTodayMarker({ position }: RoadmapTodayMarkerProps) {
  ...
  return (
    <div className="absolute top-0 bottom-0 z-20 pointer-events-none" style={{ left: `${position}%` }}>
      <div className="absolute -top-0.5 -translate-x-1/2 px-1.5 py-0.5 rounded" style={{ background: 'var(--ds-text-danger, #EF4444)', fontSize: 9, fontWeight: 700, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', whiteSpace: 'nowrap' }}>
        Today
      </div>
