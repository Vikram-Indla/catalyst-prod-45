...
export function useResourceAllocations() {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          ...
          updated_at,
          created_by,
          resource_inventory(id, name, profile_id, role_name, department_name),
      ...
      return (data || []).map((row: any) => {
        ...
        return {
          ...
          department_name: row.resource_inventory?.department_name || null,
        };
  ...
  // Handles merged allocations by tracking all original IDs
