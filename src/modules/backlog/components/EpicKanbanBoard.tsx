...
const COLOR_MAP: Record<string, string> = {
  info: 'var(--ds-text-brand)',      // Blue
  warning: 'var(--ds-text-warning)',   // Amber
  forest: 'var(--ds-chart-teal-bold)',    // Teal (was Green)
  stone: 'var(--ds-text-subtlest)',     // Gray
  success: 'var(--ds-chart-teal-bold)',   // Teal (was Green)
  danger: 'var(--ds-text-danger)',    // Red
  primary: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))',   // Blue (was Olive)
};

// Fallback states if fetch fails
const FALLBACK_STATES = [
  { id: 'proposed', label: 'New Epic', color: 'var(--ds-text-brand)' },
  { id: 'analyzing', label: 'Analysis', color: 'var(--ds-text-brand)' },
  { id: 'approved', label: 'Ready for Implementation', color: 'var(--ds-chart-teal-bold)' },
  { id: 'in_progress', label: 'In Implementation', color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' },
  { id: 'done', label: 'Done', color: 'var(--ds-chart-teal-bold)' },
  { id: 'cancelled', label: 'Cancelled', color: 'var(--ds-text-subtlest)' },
];
  ...
  const states = useMemo(() => {
    if (epicStatuses && epicStatuses.length > 0) {
      return epicStatuses.map((status: any) => ({
        id: status.value,
        label: status.label,
        color: COLOR_MAP[status.color] || status.color || 'var(--ds-text-subtlest)',
      }));
    }
