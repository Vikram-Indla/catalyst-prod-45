...
const LOZENGE_STYLES: Record<LozengeAppearance, { bg: string; color: string }> = {
  default:    { bg: 'var(--ds-background-neutral, var(--cp-lozenge-grey-bg, var(--cp-border-neutral)))', color: 'var(--ds-text)' },
  inprogress: { bg: 'var(--ds-background-information)', color: 'var(--ds-link-pressed)' },
  success:    { bg: 'var(--ds-background-success)', color: 'var(--ds-text-success)' },
  removed:    { bg: 'var(--ds-background-neutral, var(--cp-lozenge-grey-bg, var(--cp-border-neutral)))', color: 'var(--ds-text)' },
  moved:      { bg: 'var(--ds-background-neutral, var(--cp-lozenge-grey-bg, var(--cp-border-neutral)))', color: 'var(--ds-text)' },
  new:        { bg: 'var(--ds-background-neutral, var(--cp-lozenge-grey-bg, var(--cp-border-neutral)))', color: 'var(--ds-text)' },
};
              ...
              {testCases.map(tc => (
                <div key={tc.id} className="sdm-child-row">
                  <span className="sdm-type-icon">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--ds-background-success-bold)" strokeWidth="2"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
                  </span>
            ...
            <div className="sdm-child-list" role="list">
              {executions.map(ex => (
                <div key={ex.id ?? ex.test_case_id} className="sdm-child-row">
                  ...
                  <span className="sdm-child-key" style={{ color: 'var(--ds-text-subtle)' }}>{ex.case_key ?? '—'}</span>
