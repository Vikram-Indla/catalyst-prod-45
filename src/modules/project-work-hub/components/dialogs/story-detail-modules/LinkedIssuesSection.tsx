  ...
  return (
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        borderRadius: 3, fontSize: 14, fontFamily: 'inherit', background: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))', cursor: 'pointer',
        display: 'flex', alignItems: 'center', gap: 6, color: 'var(--ds-text, var(--ds-text, #172B4D))', minWidth: 160,
      ...
      {open && (
        <div style={{
          ...
          background: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))', border: '1px solid #DFE1E6', borderRadius: 4,
          ...
          {JIRA_LINK_TYPES.map(opt => (
            <div key={opt} onClick={() => { onChange(opt); setOpen(false); }}
              style={{
                ...
                cursor: 'pointer', fontSize: 14, color: 'var(--ds-text, var(--ds-text, #172B4D))',
                background: opt === value ? '#DEEBFF' : 'transparent',
              }}
              onMouseEnter={e => { if (opt !== value) (e.currentTarget as HTMLElement).style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; }}
  ...
  return (
    <div style={{ padding: '12px 0', borderTop: '1px solid #DFE1E6' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
        ...
        <div
          ...
          style={{
            ...
            border: '2px solid #4C9AFF', borderRadius: 3, background: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))',
        ...
        >
          {selectedItems.map(item => {
            ...
            return (
              <span key={item.item_key} style={{
                ...
                padding: '0 6px', background: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))', borderRadius: 3, border: '1px solid #DFE1E6',
                fontSize: 12, fontWeight: 500, color: 'var(--ds-text, var(--ds-text, #172B4D))', whiteSpace: 'nowrap',
              }}>
                ...
                <button onClick={e => { e.stopPropagation(); removeSelected(item.item_key); }}
                  style={{ display: 'flex', alignItems: 'center', border: 'none', background: 'none', cursor: 'pointer', padding: 0, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', fontSize: 14, lineHeight: 1 }}
                >×</button>
              </span>
            );
          })}
          <input
            ...
            style={{
              ...
              fontSize: 14, fontFamily: 'inherit', color: 'var(--ds-text, var(--ds-text, #172B4D))', background: 'transparent',
            }}
          />
          {selectedItems.length > 0 && (
            <button onClick={() => setSelectedItems([])}
              style={{ display: 'flex', alignItems: 'center', border: 'none', background: 'none', cursor: 'pointer', padding: 0, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', flexShrink: 0 }}
              title="Clear all"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="var(--ds-border, var(--ds-border, #DFE1E6))"/><path d="M8 8l8 8M16 8l-8 8" stroke="var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))" strokeWidth="2" strokeLinecap="round"/></svg>
            </button>
          )}
        </div>
      </div>
      ...
      {showDropdown && filteredResults.length > 0 && (
        <div ref={dropdownRef} style={{ maxHeight: 320, overflowY: 'auto', border: '1px solid #DFE1E6', borderRadius: 3, background: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))', marginBottom: 8, boxShadow: '0 4px 8px rgba(9,30,66,.13)' }}>
          <div style={{ padding: '8px 12px 4px', fontSize: 11, fontWeight: 700, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            ...
            return (
              <div key={r.issue_key} onClick={() => handleSelect(r)}
                style={{
                  ...
                  cursor: 'pointer', fontSize: 13, color: 'var(--ds-text, var(--ds-text, #172B4D))', borderLeft: '3px solid transparent',
                  transition: 'background 0.1s, border-color 0.1s',
                }}
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; e.currentTarget.style.borderLeftColor = '#4C9AFF'; }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <button
          ...
          onMouseEnter={e => { e.currentTarget.style.textDecoration = 'underline'; e.currentTarget.style.color = 'var(--ds-text, var(--ds-text, #172B4D))'; }}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            style={{
              ...
              background: selectedItems.length ? '#0052CC' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))',
              color: selectedItems.length ? 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' : '#A5ADBA', fontSize: 14, fontWeight: 500,
          ...
          <button onClick={onClose} style={{
            height: 32, padding: '0 16px', border: 'none', borderRadius: 3,
            background: 'transparent', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', fontSize: 14,
    ...
    <SectionBlock title="Linked work items" count={links.length} defaultExpanded={links.length > 0} headerRight={
      ...
      {!isLoading && Object.entries(grouped).map(([type, typeLinks]) => (
        <div key={type} style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', padding: '6px 0 4px', textTransform: 'lowercase' }}>{type}</div>
            ...
            {(typeLinks as any[]).map((link: any) => {
              ...
              return (
                <div key={link.id} style={{
                  ...
                  onMouseEnter={e => (e.currentTarget.style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #FAFBFC))')}
                ...
                >
                  ...
                  <span style={{ flex: 1, fontSize: 13, color: 'var(--ds-text, var(--ds-text, #172B4D))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{target.summary}</span>
                  ...
                  {target.assignee_display_name ? (
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 11, fontWeight: 700, color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))', flexShrink: 0,
                  ...
                  <button onClick={e => { e.stopPropagation(); removeMutation.mutate(link.id); }}
                    ...
                    style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      width: 24, height: 24, border: 'none', borderRadius: 3, background: 'transparent',
                      cursor: 'pointer', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', flexShrink: 0, transition: 'color 0.15s',
                    }}
                    onMouseEnter={e => (e.currentTarget.style.color = 'var(--ds-text, var(--ds-text, #172B4D))')}
                    onMouseLeave={e => (e.currentTarget.style.color = 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))')}
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
