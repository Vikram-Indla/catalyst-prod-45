      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, width: 200, background: 'var(--ds-surface)', border: '1px solid var(--ds-shadow-raised, rgba(9,30,66,.24))', borderRadius: 6, boxShadow: '0 6px 16px var(--ds-shadow-raised, rgba(9,30,66,.15))', zIndex: 60, overflow: 'hidden', paddingBottom: 4 }}>
          ...
          {COLS.map(col => (
            ...
            >
              <div style={{ width: 14, height: 14, borderRadius: 3, border: `1.5px solid ${columns[col.key] ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' : 'var(--ds-shadow-raised, rgba(9,30,66,.24))'}`, background: columns[col.key] ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' : 'var(--ds-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background .12s, border-color .12s' }}>
