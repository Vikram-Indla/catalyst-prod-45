      ...
      <Select<AssigneeOption>
        ...
        formatOptionLabel={(opt: AssigneeOption) => (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            ...
            <span style={{
              fontSize: 14,
              color: opt.value === UNASSIGNED_VALUE ? 'var(--ds-text-subtlest, #6B6E76)' : 'var(--ds-text, var(--ds-text, #172B4D))',
              fontWeight: 400,
              overflow: 'hidden',
      ...
      <Select<{ label: string; value: string }>
        ...
        formatOptionLabel={(opt) => (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 14, color: 'var(--ds-text, var(--ds-text, #172B4D))', fontWeight: 400 }}>
              {opt.label}
            </span>
          </span>
        )}
...
export function EditableStoryPoints({ issueId, currentPoints, onUpdate }: {
  ...
  return (
    <div ref={ref} style={{ flex: 1, position: 'relative' }}>
      ...
      >
        <span style={{ fontSize: 14, color: currentPoints != null ? 'var(--ds-text, var(--ds-text, #172B4D))' : '#97A0AF', fontWeight: 400 }}>
        ...
        <div style={{ ...ATLASSIAN_DROPDOWN, position: 'absolute', top: 'calc(100% + 4px)', left: 0, width: 160, overflow: 'hidden' }}>
          ...
          <div onClick={() => updateMutation.mutate(null)}
            style={{
              ...
              cursor: 'pointer', fontSize: 14, fontWeight: 400, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))',
              background: currentPoints == null ? '#DEEBFF' : 'transparent',
              borderBottom: '1px solid #F4F5F7',
            }}
            onMouseEnter={e => { if (currentPoints != null) (e.currentTarget as HTMLElement).style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; }}
          ...
          {FIBONACCI_POINTS.map(p => (
            <div key={p} onClick={() => updateMutation.mutate(p)}
              style={{
                height: 36, padding: '0 12px', display: 'flex', alignItems: 'center',
                cursor: 'pointer', fontSize: 14, fontWeight: 400, color: 'var(--ds-text, var(--ds-text, #172B4D))',
                background: p === currentPoints ? '#DEEBFF' : 'transparent',
              }}
              onMouseEnter={e => { if (p !== currentPoints) (e.currentTarget as HTMLElement).style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; }}
...
const EpicIconInline = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 16 16" style={{ flexShrink: 0 }}>
    ...
    <path fill="var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))" d="M8.39 2L4.5 9h3.11v5L11.5 7H8.39V2z"/>
  </svg>
);
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', flex: 1 }}>
      ...
      <div onClick={() => setOpen(o => !o)} style={{
        ...
        onMouseEnter={e => { e.currentTarget.style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; setHovered(true); }}
        onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; setHovered(false); }}
      >
        {parentKey && currentParent ? (
          <>
            <EpicIconInline />
            <span style={{ flex: 1, fontSize: 14, color: 'var(--ds-text, var(--ds-text, #172B4D))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            ...
            <button onClick={e => { e.stopPropagation(); handleSelect(null); }} style={{
              ...
              background: 'var(--ds-border, var(--ds-border, #DFE1E6))', cursor: 'pointer', color: '#42526E', flexShrink: 0,
              opacity: hovered ? 1 : 0, transition: 'opacity 0.15s',
            }}
              onMouseEnter={e => (e.currentTarget.style.background = '#C1C7D0')}
              onMouseLeave={e => (e.currentTarget.style.background = 'var(--ds-border, var(--ds-border, #DFE1E6))')}
            >
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))" strokeWidth="2" style={{ flexShrink: 0, opacity: hovered ? 1 : 0, transition: 'opacity 0.15s' }}><path d="M6 9l6 6 6-6"/></svg>
          </>
        ) : (
          <>
            <span style={{ flex: 1, fontSize: 14, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))' }}>None</span>
          </>
        )}
      </div>
          ...
          <div style={{
            ...
            <div style={{ padding: '8px 8px 4px' }}>
              <input ref={searchInputRef} value={search} onChange={e => setSearch(e.target.value)}
                ...
                style={{
                  ...
                  fontSize: 14, fontFamily: 'inherit', outline: 'none', color: 'var(--ds-text, var(--ds-text, #172B4D))',
                }} />
            </div>

            {/* Show done checkbox */}
            <div style={{ padding: '6px 12px', borderBottom: '1px solid #F4F5F7' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: 'var(--ds-text, var(--ds-text, #172B4D))' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                      <EpicIconInline />
                      <span style={{ fontFamily: 'var(--cp-font-mono)', fontWeight: 600, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', fontSize: 12 }}>{result.issue_key}</span>
                    </div>
                    {/* Line 2: summary */}
                    <div style={{ fontSize: 14, color: 'var(--ds-text, var(--ds-text, #172B4D))', paddingLeft: 22, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {result.summary}
                    </div>
                  </div>
                );
              ...
              {searchResults.length === 0 && search && (
                <div style={{ padding: 16, fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textAlign: 'center' }}>No epics found for "{search}"</div>
              )}
              {searchResults.length === 0 && !search && (
                <div style={{ padding: 16, fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textAlign: 'center' }}>No epics available</div>
              )}
            </div>
          </div>
        );
