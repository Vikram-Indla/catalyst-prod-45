      ...
      content={() => (
        <div className="sp-pop" style={{ width: 280, padding: 0 }} onClick={(e) => e.stopPropagation()}>
          ...
          <div className="sp-pop-list">
            ...
            >
              <span className="sp-avatar-fallback" style={{ background: 'var(--ds-border, var(--ds-border, #DFE1E6))', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', width: 24, height: 24 }}>
                <UserX size={12} />
              </span>
              <span style={{ fontSize: 13, color: 'var(--ds-text, var(--ds-text, #172B4D))' }}>Unassigned</span>
              {showActive && !currentAccountId && <Check size={14} color="#0052CC" style={{ marginLeft: 'auto' }} />}
            </button>

            {isLoading && (
              <div style={{ padding: '12px', fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))' }}>Loading…</div>
            )}
            {!isLoading && filtered.length === 0 && (
              <div style={{ padding: '12px', fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))' }}>No matches</div>
            )}
