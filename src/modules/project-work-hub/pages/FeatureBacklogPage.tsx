...
export default function FeatureBacklogPage({ projectId: propProjectId }: { projectId?: string }) {
  ...
  if (error) return <div className="h-full flex items-center justify-center" style={{ background: 'var(--bg-app, #FFFFFF)', color: '#DC2626' }}>Error loading features</div>;
    ...
    <div className="h-full flex flex-col" style={{ background: 'var(--bg-app, #FFFFFF)' }}>
      <div className="flex items-center justify-between px-6 py-3 border-b" style={{ borderColor: 'var(--divider, #E2E8F0)' }}>
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-semibold" style={{ color: 'var(--fg-1, #0F172A)', fontWeight: 650 }}>Feature Backlog</h1>
          <span className="text-xs" style={{ color: 'var(--fg-3, #64748B)' }}>{total} features across {groups.length} groups</span>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        {total === 0 ? (
          ...
          <div style={{ minWidth: 1320 }}>
            <div className="flex items-center h-[32px] px-2 border-b" style={{ borderColor: 'var(--divider, #E2E8F0)', background: 'var(--surface-subtle, #FAFBFC)' }}>
              <div style={{ width: 38, flexShrink: 0 }} />
              <div style={{ width: 26, flexShrink: 0 }} />
              <div style={{ width: 38, flexShrink: 0 }} />
              <div style={{ width: 110, flexShrink: 0, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--fg-3, #64748B)' }}>KEY</div>
              <div style={{ flex: 1, minWidth: 0, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--fg-3, #64748B)' }}>SUMMARY</div>
              <div style={{ width: 138, flexShrink: 0, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--fg-3, #64748B)' }}>STATUS</div>
              <div style={{ width: 158, flexShrink: 0, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--fg-3, #64748B)' }}>ASSIGNEE</div>
              <div style={{ width: 96, flexShrink: 0, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--fg-3, #64748B)' }}>DUE DATE</div>
              <div style={{ width: 88, flexShrink: 0, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--fg-3, #64748B)' }}>PRIORITY</div>
            </div>
                ...
                {!collapsed[group.status] && group.items.map((feat) => {
                  ...
                  return (
                    <div key={feat.id} className="group flex items-center h-[36px] px-2 border-b" style={{ borderColor: '#F1F5F9', maxHeight: 36, transition: 'background 120ms' }}
                      ...
                      <div style={{ flex: 1, minWidth: 0, fontSize: 13, fontWeight: 500, color: 'var(--fg-1, #0F172A)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{feat.name}</div>
                      ...
                      <div style={{ width: 158, flexShrink: 0, fontSize: 12, color: '#9CA3AF', display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: 'var(--fg-3, #64748B)', flexShrink: 0 }}>{getInitials(null)}</div>
                      ...
                      <div style={{ width: 88, flexShrink: 0, fontSize: 12, position: 'relative' }}>
                        ...
                        <div className="absolute right-0 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-1" style={{ background: 'rgba(255,255,255,0.95)' }}>
                          <button onClick={() => setEditFeatureId(feat.id)} className="p-1 rounded hover:bg-gray-100" title="Edit"><Pencil className="h-3.5 w-3.5" style={{ color: 'var(--fg-3, #64748B)' }} /></button>
