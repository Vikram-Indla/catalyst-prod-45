...
const BG_SOLIDS: Array<{ name: string; value: string }> = [
  ...
  { name: 'Stone',   value: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))' },
...
export function BacklogPage({ projectId, projectKey, assigneeIds, displayName, baseUrl }: { projectId: string; projectKey: string; assigneeIds?: string[]; displayName?: string; baseUrl?: string }) {
    ...
    return keys.map((k) => {
      ...
      if (groupBy === 'status') {
        ...
        const PRIORITY_RANK: Record<string, { level: number; color: string }> = {
          ...
          lowest:   { level: 0, color: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))' },
        };
        const rank = PRIORITY_RANK[p] || { level: 0, color: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))' };
        labelNode = (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }} title={k}>
            <span style={{ display: 'inline-flex', gap: 2 }}>
              {[1, 2, 3, 4].map((i) => (
                <span key={i} style={{ width: 4, height: 12, borderRadius: 1, background: i <= rank.level ? rank.color : 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))' }} />
              ))}
            </span>
            <span>{k}</span>
          </span>
  ...
  return (
    ...
    <AtlaskitPageShell
      ...
      cardBorder="1px solid var(--cp-lozenge-grey-bg, #DFE1E6)"
                ...
                {[1, 2].map((s) => (
                  <React.Fragment key={s}>
                    <span style={{
                      ...
                      background: bulkWizardStep >= s ? '#0C66E4' : 'var(--cp-lozenge-grey-bg, #DFE1E6)',
                    ...
                    {s < 2 && <span style={{ flex: 1, height: 1, background: 'var(--cp-lozenge-grey-bg, #DFE1E6)' }} />}
                  </React.Fragment>
                ))}
              </div>
                  ...
                  {([
                    ...
                    <label
                      ...
                      style={{
                        ...
                        border: `2px solid ${bulkWizardAction === opt.id ? '#0C66E4' : 'var(--cp-lozenge-grey-bg, #DFE1E6)'}`,
        ...
        >
          {[
            ...
            if ((item as any).divider) {
              return <div key={item.id} style={{ height: 1, background: token('color.border', 'var(--cp-lozenge-grey-bg, #DFE1E6)'), margin: '6px 0' }} />;
            }
            return (
              <button
                key={item.id}
        ...
        >
          ...
          <div style={{ padding: '0 24px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              ...
              >
                Type <code style={{ background: token('color.background.neutral', 'var(--cp-lozenge-grey-bg, #DFE1E6)'), padding: '0 4px', borderRadius: 3, fontFamily: 'inherit' }}>{projectKey}</code> to confirm
              </label>
              <Textfield
    ...
    <>
      ...
      <button
        ...
        style={{
          ...
          padding: '0 12px',
          borderRadius: 3,
          border: `1px solid ${(isOpen || value !== 'none') ? token('color.border.selected', '#0C66E4') : token('color.border', 'var(--cp-lozenge-grey-bg, #DFE1E6)')}`,
            ...
            <React.Fragment key={`g${gi}`}>
              ...
              {gi < groups.length - 1 && (
                <div style={{
                  height: 1,
                  margin: '4px 0',
                  background: token('color.border', 'var(--cp-lozenge-grey-bg, #DFE1E6)'),
                }} />
              )}
            </React.Fragment>
          ))}
...
function ColumnFilterMultiSelect({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 220 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', paddingTop: 6 }}>
        <button
          type="button"
          onClick={onClose}
          style={{
            border: '1px solid var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', background: 'transparent',
    ...
    return (
      <button
        ...
        onMouseEnter={(e) => {
          ...
          (e.currentTarget as HTMLElement).style.borderColor = token('color.border', 'var(--cp-lozenge-grey-bg, #DFE1E6)');
        }}
        onMouseLeave={(e) => {
          (e.currentTarget as HTMLElement).style.background = 'transparent';
