...
export default function StoryBacklogPage({ projectId: propProjectId, projectKey }: { projectId?: string; projectKey?: string }) {
  ...
  const renderCell = (colKey: string, story: BacklogStory, isSelected: boolean, isFocused: boolean) => {
    ...
    switch (colKey) {
          ...
          <td key={colKey} style={{ width: columnWidths.star, overflow: 'visible', textOverflow: 'clip' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              >
                <Star
                  size={14}
                  fill={isStarred ? 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' : 'none'}
                  stroke={isStarred ? 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))'}
      ...
      case 'key':
        return (
          <td key={colKey} style={{ width: columnWidths.key }}>
            <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 13, fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>
              {story.story_key || '—'}
            </span>
          </td>
        );
        ...
        return (
          <td key={colKey} style={{ width: columnWidths.parent, overflow: 'hidden' }}>
            {story.feature?.epic ? (
              ...
              <span style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', fontSize: 12 }}>—</span>
            )}
          </td>
        );
      case 'assignee':
        return (
          <td key={colKey} style={{ width: columnWidths.assignee }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, overflow: 'hidden' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          ...
          <td key={colKey} style={{ width: columnWidths.priority }} title={getPriorityLabel(story.priority)}>
            <div style={{ display: 'flex', gap: 2 }}>
              {[1, 2, 3, 4].map(i => {
                ...
                const fillColor = level >= 4 ? '#E5484D' : level >= 3 ? 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' : 'var(--ds-text-success, var(--ds-text-success, #22C55E))';
                return <div key={i} style={{ width: 4, height: 14, borderRadius: 1, background: filled ? fillColor : 'var(--ds-border, var(--ds-border, #E2E8F0))' }} />;
              })}
            </div>
          </td>
        );
      case 'updated':
        return (
          <td key={colKey} style={{ width: columnWidths.updated, fontSize: 12, fontWeight: 500, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>
  ...
  const v2Columns = useMemo<DynamicTableColumn<BacklogStory>[]>(() => [
    {
      ...
      header: () => (
        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
          <Star size={13} stroke="var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))" fill="none" />
        </span>
      ),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>
  ...
  if (error) return <div className="h-full flex items-center justify-center" style={{ background: tk.pageBg, color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>Error loading stories</div>;

  const v2EmptyState = (
    <div className="flex flex-col items-center justify-center" style={{ padding: '64px 0' }}>
      <BookOpen className="h-12 w-12 mb-4" style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }} />
      <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--ds-text, var(--ds-text, #0F172A))', marginBottom: 4 }}>No stories found</p>
      <p style={{ fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', marginBottom: 16 }}>Try adjusting your filters or search</p>
    </div>
  );
  ...
  const renderPagination = () => (
    totalPages > 1 ? (
      <div style={{
        ...
        fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', fontFamily: 'var(--cp-font-body)',
      }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            ...
            .map((p, i) =>
              p === 'ellipsis' ? (
                <span key={`e${i}`} style={{ padding: '0 4px', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>…</span>
              ) : (
                <button
                  key={p}
                  ...
                  style={{
                    ...
                    color: page === p ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))',
                    ...
                    <tr>
                      <td
                        ...
                        style={{
                          ...
                          fontSize: 11, fontWeight: 700, color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))',
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          {collapsed[group.label]
                            ? <ChevronRight size={14} style={{ color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))' }} />
                            : <ChevronDown size={14} style={{ color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))' }} />
                          }
                          ...
                          <span style={{
                            ...
                            background: 'var(--ds-border, var(--ds-border, #DFE1E6))', color: 'var(--ds-text, var(--ds-text, #253858))',
                            fontSize: 10, fontWeight: 700,
                          }}>
                            {group.items.length}
                          </span>
    ...
    <div ref={containerRef} className="h-full flex flex-col" style={{ background: tk.pageBg }}>
      ...
      <div className="flex items-center gap-3 px-6 py-2.5" style={{ borderColor: tk.border }}>
        ...
        <div className="relative w-80">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4" style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }} />
          <Input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search by title, key, Jira ID..."
            className="h-9 pl-9 pr-3 text-sm"
            style={{ borderColor: 'var(--ds-border, var(--ds-border, #E2E8F0))', background: tk.pageBg, color: tk.t1 }}
          />
        </div>
        ...
        {/* Total count */}
        <span style={{ fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>{total} stories</span>
      ...
      {panelMode && detailItemId ? (
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
          ...
          <div
            ...
            onMouseEnter={e => (e.currentTarget.style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))')}
            onMouseLeave={e => { if (!isDraggingPanel.current) e.currentTarget.style.background = 'transparent'; }}
          >
            <div style={{ width: 1.5, height: 40, borderRadius: 1, background: 'var(--ds-border, var(--ds-border, #E2E8F0))' }} />
          </div>
