...
export default function StoryBacklogPage({ projectId: propProjectId }: { projectId?: string }) {
  ...
  if (error) return <div className="h-full flex items-center justify-center" style={{ background: 'var(--bg-app, #FFFFFF)', color: '#DC2626' }}>Error loading stories</div>;
    ...
    <div className="h-full flex flex-col" style={{ background: 'var(--bg-app, #FFFFFF)' }}>
      <div className="flex items-center justify-between px-6 py-3 border-b" style={{ borderColor: 'var(--divider, #E2E8F0)' }}>
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-semibold" style={{ color: 'var(--fg-1, #0F172A)', fontWeight: 650 }}>Story Backlog</h1>
          <span className="text-xs" style={{ color: 'var(--fg-3, #64748B)' }}>{total} stories across {groups.length} groups</span>
        </div>
                ...
                {!collapsed[group.status] && group.items.map((story) => {
                  ...
                  return (
                    <div key={story.id} className="group flex items-center h-[36px] px-2 border-b cursor-pointer" style={{ borderColor: '#F1F5F9', maxHeight: 36, transition: 'background 120ms' }}
                      ...
                      <div style={{ flex: 1, minWidth: 0, fontSize: 13, fontWeight: 400, color: 'var(--fg-1, #0F172A)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{story.title}</div>
                      ...
                      <div style={{ width: 240, flexShrink: 0, overflow: 'hidden' }}>
                        {story.feature?.epic ? (
                          ...
                          <span style={{ color: 'var(--fg-4, #94A3B8)', fontSize: 12 }}>—</span>
                        )}
                      </div>
                      ...
                      <div style={{ width: 160, flexShrink: 0, fontSize: 13, color: story.assignee_name ? '#0F172A' : '#94A3B8', fontStyle: story.assignee_name ? 'normal' : 'italic', display: 'flex', alignItems: 'center', gap: 6, overflow: 'hidden' }}>
                        {avatarUrl ? (
                          ...
                          <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: 'var(--fg-3, #64748B)', flexShrink: 0 }}>{getInitials(story.assignee_name || null)}</div>
                        )}
                      ...
                      <div style={{ width: 78, flexShrink: 0, fontSize: 12, position: 'relative' }}>
                        ...
                        <div className="absolute right-0 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-1" style={{ background: 'rgba(255,255,255,0.95)' }}>
                          <button onClick={(e) => { e.stopPropagation(); setEditStoryId(story.id); }} className="p-1 rounded hover:bg-gray-100" title="Edit"><Pencil className="h-3.5 w-3.5" style={{ color: 'var(--fg-3, #64748B)' }} /></button>
