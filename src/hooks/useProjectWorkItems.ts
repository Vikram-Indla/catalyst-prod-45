    ...
    queryFn: async (): Promise<WorkItemRow[]> => {
      ...
      return (data || []).map((row: any) => {
        ...
        return {
          ...
          type_color: row.ph_work_types?.color ?? 'var(--cp-ink-4, var(--cp-border-neutral-light))',
          ...
          status_color: row.ph_workflow_statuses?.color ?? 'var(--cp-ink-4, var(--cp-border-neutral-light))',
