    ...
    queryFn: async (): Promise<KanbanCard[]> => {
      ...
      return ranks.map((r: any) => {
        ...
        const priorityColors: Record<string, string> = {
          ...
          Medium: 'var(--cp-warning, #D97706)', Low: 'var(--cp-ink-4, #94A3B8)', Lowest: 'var(--cp-ink-4, #94A3B8)',
        };
        const pName = item.priority ?? '';
        const priorityObj = pName ? {
          id: pName.toLowerCase(),
          name: pName.toLowerCase() as any,
          color: priorityColors[pName] ?? 'var(--cp-ink-4, #94A3B8)',
        } : null;
          ...
          try {
            ...
            if (Array.isArray(parsed)) {
              labelsList = parsed.map((l: any) => ({
                ...
                color: typeof l === 'object' ? l.color ?? 'var(--cp-ink-4, #94A3B8)' : 'var(--cp-ink-4, #94A3B8)',
              }));
            }
          } catch {}
        }
