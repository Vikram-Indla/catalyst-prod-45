  ...
  return useQuery({
    ...
    queryFn: async (): Promise<WorkItemDetail | null> => {
      ...
      const children: ChildItem[] = (childrenRaw || []).map((c: any) => ({
        ...
        type_color: c.ph_work_types?.color ?? 'var(--cp-ink-4, #94A3B8)',
      ...
      if (linkedItemIds.length > 0) {
        ...
        linked_items = (linkedRaw || []).map((li: any) => {
          ...
          return {
            ...
            type_color: li.ph_work_types?.color ?? 'var(--cp-ink-4, #94A3B8)',
      ...
      return {
        ...
        type_color: (data as any).ph_work_types?.color ?? 'var(--cp-ink-4, #94A3B8)',
        ...
        status_color: (data as any).ph_workflow_statuses?.color ?? 'var(--cp-ink-4, #94A3B8)',
