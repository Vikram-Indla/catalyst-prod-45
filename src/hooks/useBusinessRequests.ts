...
export function useDuplicateBusinessRequest() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      // Create new request with only title copied, status reset to new_demand (New Demand), all scoring/optional fields cleared
      const { data: newRequest, error: insertError } = await supabase
        .from('business_requests')
        .insert({
          // request_key is auto-generated by the backend
