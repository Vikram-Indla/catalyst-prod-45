...
describe('computeProjectAllocations', () => {
  it('always includes a Buffer entry at exactly 20%', () => {
    const result = computeProjectAllocations([
      { projectId: 'p1', name: 'Senaei BAU', key: 'BAU', color: 'var(--cp-primary-60)', role: 'lead' },
      { projectId: 'p2', name: 'Data Analytics', key: 'DA', color: '#00B8D9', role: 'member' }, // ads-scanner:ignore-line — semi-transparent overlay, no ADS token for alpha variant
    ]);
  ...
  it('all percentages sum to exactly 100', () => {
    const input = [
      { projectId: 'p1', name: 'A', key: 'A', color: 'var(--ds-text)', role: 'lead' },
      { projectId: 'p2', name: 'B', key: 'B', color: 'var(--ds-text)', role: 'member' },
      { projectId: 'p3', name: 'C', key: 'C', color: 'var(--ds-text)', role: 'reviewer' },
    ];
  ...
  it('lead role gets strictly more allocation than member', () => {
    const result = computeProjectAllocations([
      { projectId: 'p1', name: 'Lead project', key: 'LP', color: 'var(--ds-text)', role: 'lead' },
      { projectId: 'p2', name: 'Member project', key: 'MP', color: 'var(--ds-text)', role: 'member' },
    ]);
  ...
  it('single project gets the full 80% project pool', () => {
    const result = computeProjectAllocations([
      { projectId: 'p1', name: 'Solo', key: 'S', color: 'var(--ds-text)', role: 'lead' },
    ]);
  ...
  it('reviewer gets less allocation than member', () => {
    const result = computeProjectAllocations([
      { projectId: 'p1', name: 'A', key: 'A', color: 'var(--ds-text)', role: 'member' },
      { projectId: 'p2', name: 'B', key: 'B', color: 'var(--ds-text)', role: 'reviewer' },
    ]);
    const member   = result.find((p) => p.role === 'member');
  ...
  it('Buffer entry has isBuffer=true; all others have isBuffer=false', () => {
    const result = computeProjectAllocations([
      { projectId: 'p1', name: 'X', key: 'X', color: 'var(--ds-text)', role: 'lead' },
    ]);
