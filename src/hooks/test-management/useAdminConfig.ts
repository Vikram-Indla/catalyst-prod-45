  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map(row => ({
        ...
        name: row.name,
        color: row.color || 'var(--ds-text-subtlest)',
  ...
  return useMutation({
    mutationFn: async (projectId: string) => {
      ...
      const defaultPriorities = [
        { project_id: projectId, name: 'Critical', color: 'var(--ds-background-danger-bold)', sort_order: 1, is_default: false },
        { project_id: projectId, name: 'High', color: 'var(--ds-background-warning-bold)', sort_order: 2, is_default: false },
        { project_id: projectId, name: 'Medium', color: 'var(--ds-background-warning-bold)', sort_order: 3, is_default: true },
        { project_id: projectId, name: 'Low', color: 'var(--ds-background-success-bold)', sort_order: 4, is_default: false },
      ];
