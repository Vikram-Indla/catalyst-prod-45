    ...
    <div data-goals-page="" style={{ padding: '16px 24px 24px', minHeight: '100%', backgroundColor: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={exportCSV} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '8px 14px', fontSize: 'var(--ds-font-size-300)', fontWeight: 500, color: isDark ? DK.t2 : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))', background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', border: `1px solid ${isDark ? DK.border : 'var(--ds-border, var(--cp-border, var(--cp-bg-sunken)))'}`, borderRadius: 6, cursor: 'pointer', boxShadow: isDark ? 'none' : '0 1px 2px var(--ds-shadow-raised, rgba(0,0,0,0.04))' }}>
            <Download size={14} /> Export
          </button>
          <button onClick={() => setShowCreateModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '8px 16px', fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', background: 'linear-gradient(135deg, var(--ds-text-brand, var(--cp-workstream-catalyst-primary)), var(--ds-background-brand-bold-hovered))', border: 'none', borderRadius: 6, cursor: 'pointer', boxShadow: '0 2px 8px var(--ds-background-information, rgba(37,99,235,0.18))' }}>
