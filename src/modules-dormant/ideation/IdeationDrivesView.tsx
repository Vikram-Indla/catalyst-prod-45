...
export default function IdeationDrivesView() {
  ...
  return (
    <div style={{ padding: '16px 28px 32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
            <h2 style={{ fontSize: 'var(--ds-font-size-700)', fontWeight: 800, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', letterSpacing: '-0.5px', margin: 0, fontFamily: 'var(--cp-font-heading)' }}>Ideas Themes</h2>
            <span style={{
              ...
              fontFamily: MONO, color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))',
            }}>{drives.length}</span>
          </div>
          <p style={{ fontSize: 'var(--ds-font-size-300)', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: 0 }}>
      ...
      {isLoading && (
        <div style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', fontSize: 'var(--ds-font-size-300)', padding: 20 }}>Loading drives...</div>
      )}

      {error && (
        <div style={{ color: 'var(--ds-text-danger, #EF4444)', fontSize: 'var(--ds-font-size-300)', padding: 20 }}>Failed to load drives.</div>
      )}
        ...
        return (
          <div key={drive.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: 'var(--ds-font-size-500)', fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', flex: 1 }}>{drive.title}</span>
              {/* V12 3-color lozenge for status */}
              <span style={{
                display: 'inline-flex', alignItems: 'center',
                background: isActive
                  ? ('var(--cp-primary-light, #DEEBFF)')
                  : ('var(--cp-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))'),
                color: isActive
            ...
            {/* Description */}
            <p style={{ fontSize: 'var(--ds-font-size-300)', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '0 0 12px', lineHeight: 1.5 }}>{drive.description}</p>

            {/* Stats row */}
            <div style={{ display: 'flex', gap: '24px', fontSize: 'var(--ds-font-size-200)', color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))', fontWeight: 500, marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
              <div style={{
                ...
                background: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))', overflow: 'hidden',
              ...
              <span style={{ fontFamily: MONO, fontSize: 'var(--ds-font-size-100)', fontWeight: 600, color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', minWidth: 32, textAlign: 'right' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: 'var(--ds-font-size-100)', color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', fontWeight: 600, marginRight: '4px' }}>Ideas:</span>
              {drive.ideas.length === 0 && (
                <span style={{ fontSize: 'var(--ds-font-size-100)', color: 'var(--ds-text-disabled, #CBD5E1)', fontStyle: 'italic' }}>No ideas linked yet</span>
              )}
              {drive.ideas.map(idea => (
                <span key={idea.idea_key} style={{
                  fontFamily: MONO, fontSize: 'var(--ds-font-size-100)', fontWeight: 600, color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))',
