...
const DOT_COLORS: Record<string, string> = {
  'Fast-Track': 'var(--ds-text-success, var(--cp-success))', 'Merge': 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', 'Investigate': 'var(--ds-text-warning, var(--cp-warning))', 'Defer': 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light)))',
};

const CATEGORY_TEXT_COLORS: Record<string, string> = {
  'FAST-TRACK RECOMMENDED': 'var(--ds-text-success, var(--cp-success))', 'MERGE CANDIDATES': 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))',
  'NEEDS INVESTIGATION': 'var(--ds-text-warning, var(--cp-warning))', 'RECOMMENDED TO DEFER': 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))',
};
...
export default function IdeationTriagePanel({ open, onClose, onMerge, ideas = [] }: Props) {
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'var(--ds-shadow-raised)', zIndex: 250 }} />
      <div style={{
        position: 'fixed', top: 0, right: 0, bottom: 0, width: '480px', background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', zIndex: 251,
        boxShadow: isDark ? 'none' : '-8px 0 32px var(--ds-shadow-raised)', display: 'flex', flexDirection: 'column',
        animation: 'slideInRight 0.25s ease forwards',
      }}>
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken)))'}` }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Sparkles size={18} color="var(--ds-text-brand, var(--cp-workstream-catalyst-primary))" strokeWidth={2} />
              <span style={{ fontSize: 'var(--ds-font-size-500)', fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1)))' }}>AI Intelligence — Triage Results</span>
            </div>
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light)))', padding: '4px' }}><X size={18} /></button>
          </div>
          <div style={{ fontSize: 'var(--ds-font-size-300)', fontWeight: 500, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary)))', marginTop: '4px' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', scrollbarWidth: 'thin' }}>
            ...
            <>
              ...
              {visibleFastTrack.map(item => (
                <TriageCard key={item.ideaKey} badge="Fast-Track" ideaKey={item.ideaKey} title={item.title}
                  ...
                  primary={{ label: 'Fast-Track to Approved', icon: <Zap size={14} color="var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))" />, onClick: () => handleFastTrack(item.ideaKey) }}
            ...
            <>
              ...
              {visibleMerge.map(item => (
                <TriageCard key={item.ideaKey} badge="Merge" ideaKey={item.ideaKey} title={item.title}
                  body={item.body} aiSuggestion={item.aiSuggestion}
                  primary={{ label: 'Merge & Consolidate', icon: <GitMerge size={14} color="var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))" />, onClick: () => handleMerge(item.ideaKey, (item as any).mergeKey) }}
            ...
            <>
              ...
              {visibleInvestigate.map(item => (
                <TriageCard key={item.ideaKey} badge="Investigate" ideaKey={item.ideaKey} title={item.title}
                  body={item.body} aiSuggestion={item.aiSuggestion}
                  primary={{ label: 'Request Business Case', icon: <FileSearch size={14} color="var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))" />, onClick: () => dismiss(item.ideaKey) }}
            ...
            <>
              ...
              {visibleDefer.map(item => (
                <TriageCard key={item.ideaKey} badge="Defer" ideaKey={item.ideaKey} title={item.title}
                  body={item.body} aiSuggestion={item.aiSuggestion}
                  primary={{ label: 'Defer to Next Cycle', icon: <Clock size={14} color="var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))" />, onClick: () => dismiss(item.ideaKey) }}
          ...
          {totalRecommendations === 0 && (
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light)))', fontSize: 'var(--ds-font-size-400)' }}>
...
function CategoryHeader({ label, sub }: { label: string; sub: string }) {
  const { isDark } = useTheme();
  const color = CATEGORY_TEXT_COLORS[label] || 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))';
  return (
...
function TriageCard({ badge, ideaKey, title, body, aiSuggestion, primary, secondary }: {
  ...
  return (
    <div style={{ background: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken)))'}`, borderRadius: '12px', padding: '16px', marginBottom: '8px', boxShadow: isDark ? 'none' : '0 1px 2px var(--ds-shadow-raised)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
        ...
        <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary)))' }}>{ideaKey}</span>
      </div>
