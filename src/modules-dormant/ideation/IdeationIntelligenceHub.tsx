...
export default function IdeationIntelligenceHub({ open, onClose, onMerge, ideas = [] }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: C.surface, zIndex: 400, overflowY: 'auto' }}>
      {/* ══ Header ══ */}
      <div style={{
        background: C.bg, padding: '16px 32px', borderBottom: `1px solid ${C.border}`,
      ...
      <div style={{ padding: '16px 32px', display: 'flex', gap: '16px' }}>
        {[
          ...
          <div key={s.title} style={{
            ...
            <div style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 500, color: C.textTertiary, marginTop: '0px' }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '0 32px 32px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        ...
        <ContentCard title="Duplicate Detection" badge={`${duplicates.length} signals found`} badgeColor={C.warning}>
          {duplicates.length === 0 ? (
            <div style={{ fontSize: 'var(--ds-font-size-300)', color: C.textTertiary, padding: '16px 0', textAlign: 'center' }}>
        ...
        <ContentCard title="Theme Discovery" badge={`${themes.length} themes`} badgeColor={C.primary}>
          {themes.map(t => (
            <div key={t.name} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
          ...
          {themes.length > 0 && (
            <div style={{
              ...
              <div style={{ fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: C.insightText, marginBottom: '0px' }}>✦ Emerging Insight</div>
        ...
        <ContentCard title="Team Distribution" badge={`${teamStats.length} teams`} badgeColor={C.success}>
          {teamStats.map(([team, count]) => {
            ...
            return (
              <div key={team} style={{ marginBottom: '12px', paddingBottom: '12px', borderBottom: `1px solid ${C.surfaceAlt}` }}>
        ...
        <ContentCard title="Status Breakdown" badge={`${total} ideas`} badgeColor={C.success}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '16px' }}>
            {[
              ...
              <div key={p.name} style={{
                ...
                <div style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: C.textSecondary, marginTop: '4px' }}>{p.name}</div>
                <div style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 500, color: C.textTertiary, marginTop: '0px' }}>{total > 0 ? Math.round((p.count / total) * 100) : 0}%</div>
              </div>
            ))}
          </div>
          ...
          {draft > 0 && (
            <div style={{
              ...
              <div style={{ fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: C.gapText, marginBottom: '0px' }}>⚠ Attention Needed</div>
    ...
    <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: '12px', overflow: 'hidden' }}>
      <div style={{
        ...
        <span style={{
          ...
          fontSize: 'var(--ds-font-size-200)', fontWeight: 500, padding: '0px 10px', borderRadius: '12px',
        }}>{badge}</span>
      </div>
...
function DuplicatePair({ match, idea1, idea2, signals, onMerge }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 'var(--ds-font-size-300)', marginBottom: '0px' }}>
        <span style={{ fontFamily: MONO, fontWeight: 600, color: C.textPrimary }}>{idea1.key}</span>
        <span style={{ color: C.textSecondary, marginLeft: '4px', fontWeight: 400 }}>{idea1.title}</span>
      </div>
      <div style={{ fontSize: 'var(--ds-font-size-300)', marginBottom: '8px' }}>
        ...
        <span style={{ color: C.textSecondary, marginLeft: '4px', fontWeight: 400 }}>{idea2.title}</span>
      </div>
      <div style={{ fontSize: 'var(--ds-font-size-200)', color: C.textTertiary, lineHeight: 1.5, marginBottom: '8px' }}>
        Shared signals: {signals}
      </div>
      <div style={{ display: 'flex', gap: '8px' }}>
        <button onClick={() => { if (onMerge) onMerge(); else toast.success('Merge initiated'); }}
          style={{ background: C.primary, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', border: 'none', borderRadius: '6px', padding: '4px 14px', fontSize: 'var(--ds-font-size-200)', fontWeight: 600, cursor: 'pointer' }}>
          Merge Ideas
        </button>
        <button onClick={() => toast('Kept separate')}
          style={{ background: 'transparent', color: C.textSecondary, border: `1px solid ${C.border}`, borderRadius: '6px', padding: '4px 14px', fontSize: 'var(--ds-font-size-200)', fontWeight: 600, cursor: 'pointer' }}>
          Dismiss
        </button>
      </div>
    </div>
