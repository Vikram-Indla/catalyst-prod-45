...
const DOMAIN_COLORS: Record<string, string> = {
  D1: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', D2: 'var(--cp-teal-60)', D3: 'var(--ds-text-warning, var(--cp-warning))', D4: 'var(--ds-text-success, var(--cp-success))',
...
export default function WikiKnowledgeGraphPage() {
  ...
  useEffect(() => {
    ...
    const link = g.append('g')
      ...
      .data(links)
      .join('line')
      .attr('stroke', isDark ? 'var(--ds-border-bold)' : 'var(--ds-text-disabled)')
      .attr('stroke-width', 1)
    ...
    const node = g.append('g')
      ...
      .attr('fill', d => DOMAIN_COLORS[d.domain_code] || 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))')
      .attr('stroke', isDark ? 'var(--cp-bg-page)' : 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))')
    ...
    const labels = g.append('g')
      ...
      .attr('fill', isDark ? 'var(--ds-text-subtlest)' : 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2)))')
  ...
  return (
    <div style={{ fontFamily: 'var(--cp-font-body)', color: isDark ? 'var(--ds-text, var(--cp-bg-neutral))' : 'var(--ds-text, var(--cp-ink-1, var(--cp-ink-1)))', background: isDark ? 'var(--cp-bg-page)' : 'var(--ds-surface-sunken)', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '16px 40px 0' }}>
        ...
        <h1 style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 'var(--ds-font-size-600)', fontWeight: 700, margin: '0 0 8px' }}>Knowledge Graph</h1>
        <p style={{ fontSize: 'var(--ds-font-size-200)', color: isDark ? 'var(--ds-text-subtlest)' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
          {Object.entries(DOMAIN_COLORS).map(([code, color]) => (
            <div key={code} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ fontSize: 'var(--ds-font-size-50)', color: isDark ? 'var(--ds-text-subtlest)' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))', fontWeight: 500 }}>{code}</span>
            </div>
          ))}
        </div>
      </div>

      <div style={{ flex: 1, position: 'relative', borderTop: isDark ? '0.75px solid var(--ds-background-neutral)' : '0.75px solid var(--ds-shadow-raised, rgba(0,0,0,0.06))' }}>
        <svg ref={svgRef} style={{ width: '100%', height: '100%', background: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1))' : 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))' }} />
        {tooltip && (
          <div style={{
            position: 'absolute', left: tooltip.x + 12, top: tooltip.y - 8,
            background: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1))' : 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', border: isDark ? '1px solid var(--ds-text)' : '1px solid var(--cp-border, var(--cp-bg-sunken))', borderRadius: 6,
