...
const SectionHeader = React.memo(({ title, count, rightLabel, rightAction, isDark }: {
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
    ...
    {rightLabel && (
      <span onClick={rightAction} style={{ marginLeft: 'auto', fontSize: 'var(--ds-font-size-200)', fontWeight: 650, color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 2 }}>
  ...
  return (
    <div style={{ fontFamily: 'var(--cp-font-body)', color: isDark ? 'var(--ds-text, var(--cp-bg-neutral))' : 'var(--ds-text, var(--cp-ink-1, var(--cp-ink-1)))', background: isDark ? 'var(--cp-bg-page, var(--ds-surface))' : 'var(--ds-surface-sunken)', minHeight: '100%' }}>
      ...
      {pendingAcks.length > 0 && (
        <div style={{ margin: '0 28px', padding: '10px 16px', background: isDark ? 'var(--ds-background-warning-bold, rgba(251,191,36,0.12))' : 'var(--ds-background-warning, var(--ds-background-warning))', border: isDark ? '0.75px solid var(--ds-background-warning-bold, rgba(251,191,36,0.2))' : '0.75px solid var(--ds-background-warning, var(--ds-background-warning))', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 10, fontSize: 'var(--ds-font-size-200)', color: isDark ? 'var(--ds-background-warning-bold, var(--ds-background-warning-bold))' : 'var(--ds-text-warning, var(--ds-text-warning))', fontWeight: 500 }}>
          <ShieldCheck size={14} />
        ...
        <div style={{ padding: '48px 40px 40px', display: 'flex', gap: 40, alignItems: 'flex-start', position: 'relative', zIndex: 1, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 320, maxWidth: 640 }}>
            ...
            <div onClick={() => setCmdOpen(true)} tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') setCmdOpen(true); }} style={{
              display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderRadius: 8, border: isDark ? '0.75px solid var(--ds-background-neutral)' : '0.75px solid var(--ds-shadow-raised, rgba(0,0,0,0.06))', cursor: 'pointer',
            ...
            >
              ...
              <span style={{ fontSize: 'var(--ds-font-size-50)', fontWeight: 500, padding: '2px 8px', borderRadius: 9999, background: 'var(--ds-background-discovery-bold, rgba(124,58,237,0.06))', color: 'var(--cp-purple-60, var(--ds-background-discovery-bold))' }}>AI-powered</span>
              <button style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 650, padding: '6px 14px', borderRadius: 6, border: 'none', background: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))', cursor: 'pointer' }}>Search</button>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, minWidth: 260 }}>
            {statsLoading ? Array.from({ length: 4 }).map((_, i) => (
              <div key={i} style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, var(--ds-surface)))' : 'var(--ds-surface-sunken)', border: isDark ? '0.75px solid var(--ds-background-neutral)' : '0.75px solid var(--ds-shadow-raised, rgba(0,0,0,0.06))' }}>
      ...
      <div style={{ padding: '24px 40px 48px' }}>
        ...
        >
          ...
          <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 'var(--ds-font-size-50)', fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: isDark ? 'var(--ds-background-information, rgba(37,99,235,0.12))' : 'var(--ds-background-selected)', color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 10, marginBottom: 40 }}>
          {domainsLoading ? Array.from({ length: 6 }).map((_, i) => (
            <div key={i} style={{ padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, var(--ds-surface)))' : 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, var(--ds-surface))))', border: isDark ? '0.75px solid var(--ds-background-neutral)' : '0.75px solid var(--ds-shadow-raised, rgba(0,0,0,0.06))' }}>
...
const StatCard = React.memo(({ label, value, valueColor, isDark }: { label: string; value: string | number; valueColor?: string; isDark?: boolean }) => (
  <div style={{
    padding: 20, borderRadius: 8, background: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, var(--ds-surface)))' : 'var(--ds-surface-sunken)', textAlign: 'center',
    ...
    >
      ...
      <div style={{ flex: 1, padding: '16px 16px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 700, textTransform: 'uppercase', padding: '2px 6px', borderRadius: 4, background: isDark ? 'rgba(55,48,163,0.15)' : tagStyle.bg, color: isDark ? 'var(--ds-text-subtlest)' : tagStyle.color, letterSpacing: '0.04em' }}>{d.tag}</span>
        </div>
...
const ArticleRow = React.memo(({ a, navigate, bookmarked, onToggleBookmark, isDark }: { a: any; navigate: any; bookmarked: boolean; onToggleBookmark: () => void; isDark?: boolean }) => {
  const formatIcon = a.format === 'pdf'
    ? <span style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 700, padding: '1px 4px', borderRadius: 4, background: isDark ? 'var(--ds-background-danger-bold, rgba(220,38,38,0.15))' : 'var(--ds-background-danger, var(--ds-background-danger))', color: 'var(--ds-text-danger, var(--cp-danger))' }}>PDF</span>
