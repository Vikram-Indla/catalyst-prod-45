        ...
        {isLoading ? <div style={{ padding: 32, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, var(--cp-text-secondary))' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))', fontSize: 'var(--ds-font-size-200)' }}>Loading...</div> :
          (queue ?? []).length === 0 ? (
            <div style={{ padding: 48, textAlign: 'center' }}>
              <ShieldCheck size={32} style={{ color: 'var(--ds-text-success, var(--cp-success))', margin: '0 auto 12px' }} />
              <div style={{ fontSize: 'var(--ds-font-size-400)', fontWeight: 600, color: isDark ? 'var(--ds-text, var(--cp-bg-neutral))' : 'var(--ds-text, var(--cp-ink-1, var(--cp-ink-1)))' }}>All clear!</div>
              <div style={{ fontSize: 'var(--ds-font-size-200)', color: isDark ? 'var(--ds-text-subtlest, var(--cp-text-secondary))' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))', marginTop: 4 }}>No articles pending review.</div>
            </div>
          ) : (queue ?? []).map((a: any) => {
            ...
            return (
              <div key={a.id} style={{
                ...
                <span style={{ fontSize: 'var(--ds-font-size-100)', color: isDark ? 'var(--ds-text-subtlest)' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))' }}>{a.author_name || '—'}</span>
                <span style={{
                  fontFamily: 'var(--cp-font-mono)', fontSize: 'var(--ds-font-size-100)', fontWeight: 500,
                  color: stale ? 'var(--ds-text-warning, var(--cp-warning))' : fresh >= 80 ? 'var(--ds-text-success, var(--cp-success))' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))',
                }}>{fresh}%</span>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <button onClick={() => handleRequestChanges(a.id)} style={{
                    ...
                    border: `0.75px solid ${isDark ? 'var(--ds-border-bold)' : 'var(--ds-shadow-raised, rgba(0,0,0,0.12))'}`, background: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1))' : 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))', color: 'var(--ds-text-warning, var(--cp-warning))',
                    cursor: 'pointer',
