    ...
    <div className="th-page-content" style={{ flex: 1, padding: '24px 32px', overflow: 'auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '4px 0 0' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative', flex: '0 1 280px' }}>
          ...
          <input
            ...
            style={{
              ...
              border: '1px solid var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))', borderRadius: 8, fontSize: 13, outline: 'none',
            }}
          />
          {filters.search && (
        ...
        <select
          ...
          style={{ height: 38, padding: '8px 12px', border: '1px solid var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))', borderRadius: 8, fontSize: 13, color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))', background: 'var(--bg-app, #fff)', cursor: 'pointer' }}
        >
        ...
        {/* View toggle */}
        <div style={{ display: 'flex', border: '1px solid var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))', borderRadius: 8, overflow: 'hidden' }}>
          <button
            ...
            style={{
              ...
              color: viewMode === 'table' ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))',
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              color: viewMode === 'card' ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563EB))' : 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))',
              borderLeft: '1px solid var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))',
            }}
          >
...
function TableView({ releases, navigate }: { releases: Release[]; navigate: any }) {
  return (
    <div style={{ border: '1px solid var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))', borderRadius: 12, overflow: 'hidden', background: 'var(--bg-app, #fff)' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ backgroundColor: 'var(--bg-1, #F8FAFC)', borderBottom: '1px solid var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))' }}>
          ...
          {releases.map(r => {
            ...
            return (
              <tr
                ...
                style={{ borderBottom: '1px solid var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9))', cursor: 'pointer', transition: 'background 0.15s' }}
              ...
              >
                ...
                <td style={tdStyle}>
                  <span style={{ fontSize: 12, color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))' }}>
                    {r.test_cases_passed}/{r.test_cases_total || 0}
                  </span>
                </td>
                <td style={tdStyle}>
                  <span style={{ fontSize: 12, color: r.critical_defects > 0 ? 'var(--ds-text-danger, var(--cp-danger, #DC2626))' : 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))', fontWeight: r.critical_defects > 0 ? 600 : 400 }}>
      ...
      {releases.map(r => {
        ...
        return (
          <div
            ...
            onClick={() => navigate(`/testhub/releases/${r.id}`)}
            style={{
              background: 'var(--bg-app, #fff)', border: '1px solid var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))', borderRadius: 12,
            ...
            onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'; e.currentTarget.style.boxShadow = '0 1px 3px rgba(0,0,0,0.04)'; }}
          >
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              ...
              {r.vehicle && (
                <span style={{ padding: '2px 8px', borderRadius: 6, backgroundColor: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', fontSize: 11, fontWeight: 500 }}>
                  {r.vehicle.name}
                </span>
              )}
            </div>
            ...
            {/* Stats row */}
            <div style={{ display: 'flex', gap: 16, fontSize: 12, color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))' }}>
...
const thStyle: React.CSSProperties = {
  ...
  color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', textTransform: 'uppercase', letterSpacing: '0.04em',
};
