...
const TYPE_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  non_functional: { label: 'Non-Functional', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))' },
  user_story: { label: 'User Story', color: '#0891B2', bg: '#ECFEFF' },
  epic: { label: 'Epic', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))' },
        ...
        <thead>
          <tr style={{ height: 36, backgroundColor: '#111111', textAlign: 'left' }}>
            {['Defect Key', 'Title', 'Status', 'Severity', 'Source'].map(h => (
              <th key={h} style={{ padding: '8px 12px', fontWeight: 600, color: 'var(--ds-text-subtlest, #A1A1A1)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.03em', borderBottom: '1px solid #2E2E2E' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {defects.map((d: any) => {
            ...
            return (
              <tr key={d.id} style={{ height: 36, backgroundColor: 'var(--ds-surface-raised, var(--cp-ink-1, #1A1A1A))', borderBottom: '0.75px solid #2E2E2E' }}>
                ...
                <td style={{ padding: '8px 12px', color: 'var(--ds-text-subtlest, #A1A1A1)' }}>{d.severity ?? '—'}</td>
                <td style={{ padding: '8px 12px' }}>
                  {d.link_source === 'auto_execution' && (
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--ds-text-subtlest, #A1A1A1)', backgroundColor: 'var(--ds-surface-overlay, #1F1F1F)', padding: '2px 6px', borderRadius: 4 }}>Auto</span>
                  )}
                  {d.link_source === 'auto_jira' && (
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--ds-text-subtlest, #A1A1A1)', backgroundColor: 'var(--ds-surface-overlay, #1F1F1F)', padding: '2px 6px', borderRadius: 4 }}>Jira</span>
                  )}
                </td>
              </tr>
            );
  ...
  return (
    <div style={{ padding: 24, backgroundColor: 'var(--cp-bg-page, #F8FAFC)', minHeight: '100vh' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            ...
            {requirement.external_id && (
              <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', backgroundColor: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))', padding: '4px 10px', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 4 }}>
                <ExternalLink size={12} /> {requirement.external_id}
              </span>
            )}
          </div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0 }}>{requirement.title}</h1>
      ...
      {requirement.description && (
        <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', borderRadius: 12, padding: 24, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))', marginBottom: 24 }}>
      ...
      <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', borderRadius: 12, padding: 24, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {linkedTests.map((test) => (
              <div key={test.link_id}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 14, backgroundColor: 'var(--cp-bg-page, #F8FAFC)', borderRadius: 12, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))' }}>
                <div style={{ flex: 1 }}>
      ...
      {/* Linked Defects */}
      <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', borderRadius: 12, padding: 24, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, var(--cp-bg-sunken, #E2E8F0))', marginTop: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
