...
const TYPE_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  other: { label: 'Other', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))' },
};
...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  inactive: { label: 'Inactive', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))' },
  deprecated: { label: 'Deprecated', color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', bg: 'var(--ds-surface-sunken, #F8FAFC)' },
};
...
export default function EnvironmentDetailPage() {
  ...
  const fetchEnvironment = async () => {
    ...
    try {
      ...
      const { data: varsData } = await typedQuery('th_environment_variables')
        .select('*')
        .eq('environment_id', environmentId)
        .order('key');
      if (varsData) setVariables(varsData);

      const { data: cyclesData } = await typedRpc('get_environment_cycles', { p_environment_id: environmentId });
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      const { error } = await typedQuery('tm_environments').update({ status: newStatus }).eq('id', environment.id);
  ...
  const updateHealthStatus = async (newHealth: string) => {
    if (!environment) return;
    try {
      const { error } = await typedQuery('tm_environments').update({ health_status: newHealth, last_health_check: new Date().toISOString() }).eq('id', environment.id);
      if (error) throw error;
  ...
  if (!environment) {
    return <div style={{ padding: 24, textAlign: 'center', color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))' }}>Environment not found</div>;
  }
    ...
    <div style={{ padding: 24, backgroundColor: 'var(--cp-bg-page, #F8FAFC)', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0 }}>{environment.name}</h1>
          {environment.owner && (
            <p style={{ fontSize: 13, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '8px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <select value={environment.status} onChange={(e) => updateStatus(e.target.value)}
            style={{ height: 40, padding: '0 14px', border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, borderRadius: 8, fontSize: 13, backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', color: isDark ? 'var(--ds-text, var(--cp-bg-neutral, #EDEDED))' : undefined, cursor: 'pointer' }}>
      ...
      <div style={{ backgroundColor: health.bg, borderRadius: 12, padding: 20, border: `1px solid ${health.color}30`, marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '4px 0 0' }}>Last checked: {formatDate(environment.last_health_check)}</p>
          </div>
        </div>
        <select value={environment.health_status} onChange={(e) => updateHealthStatus(e.target.value)}
          style={{ height: 50, padding: '8px 12px', border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, borderRadius: 6, fontSize: 13, backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', color: isDark ? 'var(--ds-text, var(--cp-bg-neutral, #EDEDED))' : undefined, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        {/* Left Column */}
        <div>
          <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', borderRadius: 12, padding: 24, border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, marginBottom: 24 }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
              <Settings size={18} style={{ color: '#6366F1' }} /> Connection Info
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {environment.url && (
                <div>
                  <p style={{ fontSize: 12, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Globe size={12} /> Application URL</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <ExternalLink size={14} style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))' }} />
                  </div>
                </div>
              )}
              {environment.api_url && (
                <div>
                  <p style={{ fontSize: 12, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Link2 size={12} /> API URL</p>
                  <p style={{ fontSize: 14, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0 }}>{environment.api_url}</p>
                </div>
              )}
              {environment.database_info && (
                <div>
                  <p style={{ fontSize: 12, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 4 }}><Database size={12} /> Database</p>
                  <p style={{ fontSize: 14, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0 }}>{environment.database_info}</p>
                </div>
              )}
              {!environment.url && !environment.api_url && !environment.database_info && (
                <p style={{ fontSize: 13, color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', textAlign: 'center', padding: 20 }}>No connection info configured</p>
              )}
            </div>
          </div>

          {environment.description && (
            <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', borderRadius: 12, padding: 24, border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}` }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))', margin: 0, whiteSpace: 'pre-wrap' }}>{environment.description}</p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', borderRadius: 12, padding: 24, border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}`, marginBottom: 24 }}>
            ...
            {variables.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', textAlign: 'center', padding: 20 }}>No environment variables configured</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {variables.map((v) => (
                  <div key={v.id} style={{ padding: 12, backgroundColor: 'var(--cp-bg-page, #F8FAFC)', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))', margin: 0 }}>{v.key}</p>
                      <p style={{ fontSize: 12, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', margin: '2px 0 0', fontFamily: 'monospace' }}>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      {v.is_secret && (
                        <button onClick={() => toggleSecretVisibility(v.id)}
                          style={{ width: 28, height: 28, border: 'none', borderRadius: 4, backgroundColor: 'transparent', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ backgroundColor: isDark ? 'var(--cp-bg-surface, var(--cp-ink-1, #242528))' : 'var(--ds-surface, #FFF)', borderRadius: 12, padding: 24, border: `1px solid ${'var(--cp-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))'}` }}>
            ...
            {linkedCycles.length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--cp-text-muted, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', textAlign: 'center', padding: 20 }}>No test cycles using this environment</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {linkedCycles.map((cycle) => (
                  <div key={cycle.cycle_id} onClick={() => navigate(`/testhub/cycles/${cycle.cycle_id}`)}
                    ...
                    <div>
                      ...
                      <span style={{ fontSize: 13, color: 'var(--cp-text-primary, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))' }}>{cycle.name}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 12, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))' }}>{cycle.progress_percent}%</span>
                      <ChevronRight size={16} style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))' }} />
                    </div>
                  </div>
                ))}
              </div>
