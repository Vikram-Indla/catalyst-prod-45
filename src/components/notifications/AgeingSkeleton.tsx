...
function ShimmerBar({ width, height, borderRadius = 4, isDark }: {
  ...
  return (
    <div style={{
      ...
      background: isDark
        ? 'linear-gradient(90deg, var(--ds-surface-raised, var(--cp-ink-1, #1A1A1A)) 25%, var(--ds-border, var(--cp-ink-1, #292929)) 50%, var(--ds-surface-raised, var(--cp-ink-1, #1A1A1A)) 75%)'
      ...
      <tr>
        <td colSpan={5} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{
              fontSize: 'var(--ds-font-size-100)', fontWeight: 700, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))',
