...
const normaliseStatus = (
...
): { label: string; type: "gray" | "blue" | "green" } | null => {
  if (!raw) return null;
  ...
  if (s === "ready for development")
    return { label: "IN REVIEW", type: "blue" };
...
function getActionText(type: string, actorName: string | null): string {
  switch (type) {
    ...
    case "tester_assigned":
      return actorName
        ? `${actorName} assigned you to`
        : "You have been assigned to";
    case "unassigned":
      return actorName
        ? `${actorName} changed the assignee on`
        : "Assignee changed on";
    case "status_changed":
...
function NotificationItemInner({
  ...
  const isDueDate = DUE_DATE_TYPES.some(
    (t) => t === notification.notification_type
  );
  const isComment = COMMENT_PREVIEW_TYPES.some(
    (t) => t === notification.notification_type
  );
  ...
  const metaActorName =
    (notification.metadata as any)?.actor_display_name || null;
  const metaActorAvatar =
    (notification.metadata as any)?.actor_avatar_url || null;
  ...
  const renderAvatar = () => {
    ...
    const bg =
      colours[
        (notification.actor_user_id || "").charCodeAt(0) % colours.length
      ];
    return (
      <div
        style={{
  ...
  const daysUntilDue = notification.metadata?.due_date
    ? Math.ceil(
        (new Date(notification.metadata.due_date).getTime() - Date.now()) /
          (1000 * 60 * 60 * 24)
      )
    : null;
    ...
    <div
      ...
      style={{
        ...
        boxShadow: isHovered
          ...
          : "var(--ds-shadow-overlay, 0 1px 3px rgba(9, 30, 66, 0.08))",
        ...
        <div style={{ flex: 1, minWidth: 0, opacity: textOpacity }}>
          ...
          >
            ...
            >
              ...
              <span style={{ fontWeight: hasActor ? 400 : 600 }}>
                {actionText}
              </span>
            </span>
          ...
          >
            ...
            <span style={{ flexShrink: 0, display: "inline-flex" }}>
              <WorkItemIcon
                type={notification.entity_icon_type || notification.entity_type}
              />
            </span>
            <span
              style={{
            ...
            {!isDeleted && notification.entity_key && (
              ...
              >
                <WorkItemStarButton
                  ...
                  itemType={workItemStarType(
                    notification.entity_icon_type || notification.entity_type
                  )}
                  size="sm"
                  showTooltip
