...
const StatCard = ({ label, value, color, isDark }: { label: string; value: number | string; color: string; isDark: boolean }) => (
  <div
    ...
    style={{
      background: isDark ? '#111111' : 'var(--r360-surface, #FFFFFF)', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--r360-border, #C5BDB3)',
...
const ResourceBanner: React.FC<ResourceBannerProps> = ({ resource, summary }) => {
  ...
  return (
    <header style={{
      ...
      padding: '16px 20px', background: isDark ? '#111111' : 'var(--r360-surface, #FFFFFF)',
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {metaPills.map((label, i) => (
            <span key={i} style={{
              fontSize: 11.5, color: isDark ? '#888888' : 'var(--r360-text-2, #1A1A2E)',
