...
function getStatusStyleFallback(statusName: string, statusCategory?: string): StatusStyle {
  ...
  return { dot: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))', text: 'var(--cp-ink-2, var(--cp-ink-2, #334155))', category: 'unstarted' };
}
