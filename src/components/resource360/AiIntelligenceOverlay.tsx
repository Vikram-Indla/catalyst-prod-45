  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: 300, background: isDark ? '#1A1714' : '#FFFFFF', overflow: 'auto',
      ...
      <div style={{ padding: '24px', maxWidth: 1000, margin: '0 auto' }}>
        ...
        {generating && (
          <div style={{ padding: 48, textAlign: 'center' }}>
            ...
            <div style={{ marginTop: 20 }}>
              <div style={{ width: 200, height: 4, background: isDark ? '#2C2823' : '#DBEAFE', borderRadius: 2, margin: '0 auto', overflow: 'hidden' }}>
        ...
        {hasInsufficientData && !generating && !refreshing && (
          <div style={{ padding: 48, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A', marginBottom: 4 }}>
              No data synced yet
            </div>
            <div style={{ fontSize: 12, color: isDark ? '#6B6560' : '#64748B', marginBottom: 16 }}>
          ...
          <>
            ...
            <Section title="Resource Profile" isDark={isDark}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A' }}>{resource?.full_name}</div>
                  <div style={{ fontSize: 13, color: isDark ? '#A09890' : '#334155' }}>{resource?.job_role}</div>
                  <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                    {[resource?.r360_departments?.name, resource?.r360_vendors?.name, resource?.country].filter(Boolean).map((p, i) => (
                      <span key={i} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: isDark ? '#232019' : '#F1F5F9', color: isDark ? '#A09890' : '#475569' }}>{p}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {profile.resource_pattern && (
              <Section title="Resource Pattern" isDark={isDark}>
                <p style={{ fontSize: 13, lineHeight: 1.7, color: isDark ? '#A09890' : '#334155', margin: 0 }}>{profile.resource_pattern}</p>
              </Section>
            )}
                ...
                {(hubDist || []).map((h: any) => {
                  ...
                  return (
                    <div key={h.source_hub} style={{
                      ...
                      borderLeft: `4px solid ${hubColor}`, background: isDark ? '#232019' : '#F8FAFC',
                    }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: isDark ? '#6B6560' : '#64748B', textTransform: 'uppercase' }}>
                        {HUB_SHORT[h.source_hub] || h.source_hub}
                      </div>
                      <div style={{ fontSize: 18, fontWeight: 900, color: isDark ? '#F5F3F0' : '#0F172A' }}>{h.hub_pct?.toFixed(0) ?? 0}%</div>
                      <div style={{ fontSize: 10, color: isDark ? '#6B6560' : '#64748B' }}>{h.hub_item_count ?? 0} items · {h.hub_closure_pct?.toFixed(0) ?? 0}% closed</div>
                    </div>
                  );
                })}
              </div>
            ...
            <Section title="Delivery Pattern" isDark={isDark}>
              ...
              {weeklyHistory.length > 0 && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 10.5, fontWeight: 700, color: isDark ? '#6B6560' : '#64748B', textTransform: 'uppercase', marginBottom: 8 }}>Weekly Closures</div>
            ...
            {roleExp?.expected && (
              <Section title="Role Expectation vs Actual" isDark={isDark}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: isDark ? '#6B6560' : '#64748B', textTransform: 'uppercase', marginBottom: 8 }}>Expected Duties</div>
                    <ul style={{ margin: 0, paddingLeft: 16, fontSize: 12, color: isDark ? '#A09890' : '#334155', lineHeight: 1.8 }}>
                      {(roleExp.expected || []).map((e: string, i: number) => <li key={i}>{e}</li>)}
                    </ul>
                  </div>
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: isDark ? '#6B6560' : '#64748B', textTransform: 'uppercase', marginBottom: 8 }}>Actual Distribution</div>
                    {(roleExp.actual_distribution || []).map((a: any, i: number) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        <span style={{ fontSize: 11, color: isDark ? '#A09890' : '#334155', minWidth: 80 }}>{a.label}</span>
                        <div style={{ flex: 1, height: 8, background: isDark ? '#232019' : '#F1F5F9', borderRadius: 4 }}>
                          <div style={{ width: `${a.pct}%`, height: '100%', background: '#2563EB', borderRadius: 4 }} />
                        </div>
                        <span style={{ fontSize: 11, fontWeight: 700, color: isDark ? '#F5F3F0' : '#0F172A', minWidth: 36, textAlign: 'right' }}>{a.pct}%</span>
                      </div>
                    ))}
                ...
                <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                  {patterns.map((p: any) => (
                    <li key={p.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
                      ...
                      <div>
                        <span style={{ fontSize: 13, color: isDark ? '#A09890' : '#334155', lineHeight: 1.6 }}>{p.pattern_text}</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 20 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <svg width={120} height={120} viewBox="0 0 120 120" aria-label={`Completion: ${completionPct}%`}>
                      <circle cx={60} cy={60} r={50} fill="none" stroke={isDark ? '#232019' : '#F1F5F9'} strokeWidth={12} />
                      {completionPct > 0 && (
                      ...
                      <text x={60} y={completionPct === 100 ? 55 : 60} textAnchor="middle" dominantBaseline="central"
                        style={{ fontSize: 22, fontWeight: 900, fill: isDark ? '#F5F3F0' : '#0F172A' }}>
                        {completionPct}%
                      </text>
                      {completionPct === 100 && (
                    ...
                    {standing.confidence_score != null && (
                      <div style={{ marginTop: 4, fontSize: 10, color: isDark ? '#6B6560' : '#64748B' }}>
                      ...
                      <div>
                        {(standing.project_standings as any[]).map((ps: any, i: number) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#F5F3F0' : '#0F172A', minWidth: 100 }}>{ps.project}</span>
                            <div style={{ flex: 1, height: 6, background: isDark ? '#232019' : '#F1F5F9', borderRadius: 3 }}>
                              <div style={{ width: `${ps.items ? (ps.done / ps.items) * 100 : 0}%`, height: '100%', background: '#059669', borderRadius: 3 }} />
                            </div>
                            <span style={{ fontSize: 10, color: isDark ? '#6B6560' : '#64748B' }}>{ps.done}/{ps.items}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div style={{
              ...
              background: isDark ? '#232019' : '#F8FAFC', border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid #E2E8F0',
              display: 'flex', alignItems: 'center', gap: 12,
              fontSize: 11, color: isDark ? '#6B6560' : '#64748B',
            }}>
...
const MetricCard = ({ label, value, isDark }: { label: string; value: string; isDark?: boolean }) => (
  <div style={{ padding: '10px 12px', borderRadius: 8, background: isDark ? '#232019' : '#F8FAFC', textAlign: 'center' }}>
    <div style={{ fontSize: 20, fontWeight: 900, color: isDark ? '#F5F3F0' : '#0F172A' }}>{value}</div>
    <div style={{ fontSize: 9.5, fontWeight: 700, textTransform: 'uppercase', color: isDark ? '#6B6560' : '#64748B', marginTop: 4 }}>{label}</div>
  </div>
);

const MiniStat = ({ label, value, color, isDark }: { label: string; value: number; color: string; isDark?: boolean }) => (
  <div style={{ textAlign: 'center', padding: '8px', borderRadius: 6, background: isDark ? '#232019' : '#F8FAFC' }}>
    <div style={{ fontSize: 18, fontWeight: 900, color }}>{value}</div>
    <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', color: isDark ? '#6B6560' : '#64748B' }}>{label}</div>
  </div>
);
