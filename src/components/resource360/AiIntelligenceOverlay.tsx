      ...
      <div style={{ padding: '24px', maxWidth: 1000, margin: '0 auto' }}>
        ...
        {hasInsufficientData && !generating && !refreshing && (
          <div style={{ padding: 48, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', marginBottom: 4 }}>
              No data synced yet
            </div>
            <div style={{ fontSize: 12, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', marginBottom: 16 }}>
          ...
          <>
            ...
            <Section title="Resource Profile" isDark={isDark}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{resource?.full_name}</div>
                  <div style={{ fontSize: 13, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)' }}>{resource?.job_role}</div>
                  <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                    {[resource?.r360_departments?.name, resource?.r360_vendors?.name, resource?.country].filter(Boolean).map((p, i) => (
                      <span key={i} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: isDark ? '#1A1A1A' : '#1A1A1A', color: isDark ? '#A1A1A1' : '#475569' }}>{p}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {profile.resource_pattern && (
              <Section title="Resource Pattern" isDark={isDark}>
                <p style={{ fontSize: 13, lineHeight: 1.7, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)', margin: 0 }}>{profile.resource_pattern}</p>
              </Section>
            )}
                ...
                {(hubDist || []).map((h: any) => {
                  ...
                  return (
                    <div key={h.source_hub} style={{
                      ...
                      borderLeft: `4px solid ${hubColor}`, background: isDark ? '#1A1A1A' : '#1A1A1A',
                    }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', textTransform: 'uppercase' }}>
                        {HUB_SHORT[h.source_hub] || h.source_hub}
                      </div>
                      <div style={{ fontSize: 18, fontWeight: 900, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{h.hub_pct?.toFixed(0) ?? 0}%</div>
                      <div style={{ fontSize: 10, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>{h.hub_item_count ?? 0} items · {h.hub_closure_pct?.toFixed(0) ?? 0}% closed</div>
                    </div>
                  );
                })}
              </div>
            ...
            <Section title="Delivery Pattern" isDark={isDark}>
              ...
              {weeklyHistory.length > 0 && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 10.5, fontWeight: 700, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', textTransform: 'uppercase', marginBottom: 8 }}>Weekly Closures</div>
            ...
            {roleExp?.expected && (
              <Section title="Role Expectation vs Actual" isDark={isDark}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', textTransform: 'uppercase', marginBottom: 8 }}>Expected Duties</div>
                    <ul style={{ margin: 0, paddingLeft: 16, fontSize: 12, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)', lineHeight: 1.8 }}>
                      {(roleExp.expected || []).map((e: string, i: number) => <li key={i}>{e}</li>)}
                    </ul>
                  </div>
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 700, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', textTransform: 'uppercase', marginBottom: 8 }}>Actual Distribution</div>
                    {(roleExp.actual_distribution || []).map((a: any, i: number) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        <span style={{ fontSize: 11, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)', minWidth: 80 }}>{a.label}</span>
                        <div style={{ flex: 1, height: 8, background: isDark ? '#1A1A1A' : '#1A1A1A', borderRadius: 4 }}>
                          <div style={{ width: `${a.pct}%`, height: '100%', background: '#2563EB', borderRadius: 4 }} />
                        </div>
                        <span style={{ fontSize: 11, fontWeight: 700, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', minWidth: 36, textAlign: 'right' }}>{a.pct}%</span>
                      </div>
                    ))}
                ...
                <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                  {patterns.map((p: any) => (
                    <li key={p.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
                      ...
                      <div>
                        <span style={{ fontSize: 13, color: isDark ? '#A1A1A1' : 'rgba(237,237,237,0.53)', lineHeight: 1.6 }}>{p.pattern_text}</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 20 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <svg width={120} height={120} viewBox="0 0 120 120" aria-label={`Completion: ${completionPct}%`}>
                      <circle cx={60} cy={60} r={50} fill="none" stroke={isDark ? '#1A1A1A' : '#1A1A1A'} strokeWidth={12} />
                      {completionPct > 0 && (
                      ...
                      <text x={60} y={completionPct === 100 ? 55 : 60} textAnchor="middle" dominantBaseline="central"
                        style={{ fontSize: 22, fontWeight: 900, fill: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>
                        {completionPct}%
                      </text>
                      {completionPct === 100 && (
                    ...
                    {standing.confidence_score != null && (
                      <div style={{ marginTop: 4, fontSize: 10, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>
                      ...
                      <div>
                        {(standing.project_standings as any[]).map((ps: any, i: number) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', minWidth: 100 }}>{ps.project}</span>
                            <div style={{ flex: 1, height: 6, background: isDark ? '#1A1A1A' : '#1A1A1A', borderRadius: 3 }}>
                              <div style={{ width: `${ps.items ? (ps.done / ps.items) * 100 : 0}%`, height: '100%', background: '#059669', borderRadius: 3 }} />
                            </div>
                            <span style={{ fontSize: 10, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>{ps.done}/{ps.items}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div style={{
              ...
              fontSize: 11, color: isDark ? '#878787' : 'rgba(237,237,237,0.40)',
            }}>
...
const MetricCard = ({ label, value, isDark }: { label: string; value: string; isDark?: boolean }) => (
  <div style={{ padding: '10px 12px', borderRadius: 8, background: isDark ? '#1A1A1A' : '#1A1A1A', textAlign: 'center' }}>
    <div style={{ fontSize: 20, fontWeight: 900, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)' }}>{value}</div>
    <div style={{ fontSize: 9.5, fontWeight: 700, textTransform: 'uppercase', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)', marginTop: 4 }}>{label}</div>
  </div>
);

const MiniStat = ({ label, value, color, isDark }: { label: string; value: number; color: string; isDark?: boolean }) => (
  <div style={{ textAlign: 'center', padding: '8px', borderRadius: 6, background: isDark ? '#1A1A1A' : '#1A1A1A' }}>
    <div style={{ fontSize: 18, fontWeight: 900, color }}>{value}</div>
    <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', color: isDark ? '#878787' : 'rgba(237,237,237,0.40)' }}>{label}</div>
  </div>
);
