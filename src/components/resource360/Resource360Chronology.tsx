    ...
    <div style={{ fontFamily: 'var(--cp-font-body)' }}>
      ...
      <div style={{ padding: '8px 16px', borderBottom: `1px solid ${T.border}`, background: T.surface }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setFiltersExpanded(!filtersExpanded)} style={{ fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 6, cursor: 'pointer', background: filtersExpanded ? T.text1 : T.surface, color: filtersExpanded ? 'var(--ds-surface, #fff)' : T.text3, border: `1px solid ${T.borderStrong}` }}>
          ...
          return (
            <div key={date} style={{ marginBottom: 16 }}>
              ...
              {dateItems.map((item, idx) => {
                ...
                const hc = WH_HUB_COLORS[item.hub] ?? 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))';
                  ...
                  <React.Fragment key={item.work_item_id}>
                    <div onClick={() => onItemClick(item)} style={{
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, minWidth: 64 }}>
                        ...
                        <span style={{ fontSize: 8, fontWeight: 800, color: 'var(--ds-surface, #fff)', padding: '1px 5px', borderRadius: 4, background: hc }}>{hs}</span>
                      </div>
                      {/* Title + parent */}
                      ...
                      {/* Status pill — SOLID */}
                      <span style={{ fontSize: 9, fontWeight: 700, color: 'var(--ds-surface, #fff)', background: sc, padding: '2px 7px', borderRadius: 4, whiteSpace: 'nowrap' }}>
