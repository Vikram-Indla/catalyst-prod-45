...
function getLozengeStyle(status: string, statusCategory?: string): { bg: string; color: string; label: string } {
  ...
  if (cat === 'new' || cat === 'todo')
    return { bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', color: '#42526E', label: (status || 'TO DO').toUpperCase() };
            ...
            {!isCollapsed && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {groupItems.map((item: any) => {
                  ...
                  const accentDot = cat === 'done' ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : cat === 'progress' ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : cat === 'blocked' ? 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))' : 'var(--ds-text-warning, var(--ds-text-warning, #D97706))';
