...
export function InlineExpansionPanel({ item, onOpenDetail }: { item: Resource360Item; onOpenDetail: () => void }) {
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 8, fontSize: 'var(--ds-font-size-100)' }}>
        {[
          ...
          <div key={label}>
            <p style={{ color: 'var(--fg-3)', fontWeight: 700, fontSize: 'var(--ds-font-size-100)', margin: '0 0 2px', textTransform: 'uppercase' }}>{label}</p>
            <p style={{ color: 'var(--fg-1)', fontWeight: 600, margin: 0 }}>{value}</p>
          </div>
        ))}
        {item.parent_key && (
          <div>
            <p style={{ color: 'var(--fg-3)', fontWeight: 700, fontSize: 'var(--ds-font-size-100)', margin: '0 0 2px', textTransform: 'uppercase' }}>Parent</p>
            <p style={{ color: 'var(--fg-1)', fontWeight: 600, margin: 0 }}>{item.parent_key} — {item.parent_title ?? ''}</p>
          </div>
        )}
        {item.project_name && (
          <div>
            <p style={{ color: 'var(--fg-3)', fontWeight: 700, fontSize: 'var(--ds-font-size-100)', margin: '0 0 2px', textTransform: 'uppercase' }}>Project</p>
            <p style={{ color: 'var(--fg-1)', fontWeight: 600, margin: 0 }}>{item.project_name}</p>
          </div>
        )}
        {item.release_name && (
          <div>
            <p style={{ color: 'var(--fg-3)', fontWeight: 700, fontSize: 'var(--ds-font-size-100)', margin: '0 0 2px', textTransform: 'uppercase' }}>Release</p>
