      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Calendar className="h-5 w-5 text-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))]" />
            Reschedule Tests
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          <RadioGroup value={mode} onValueChange={(v) => setMode(v as 'shift' | 'range')}>
            ...
            <div
              className={cn(
                ...
                mode === 'shift' ? 'border-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))] bg-[var(--ds-background-selected,var(--ds-background-selected, #eff6ff))]' : 'border-[var(--ds-border,var(--ds-border, #e2e8f0))]'
              )}
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748b))] mt-1">
                  ...
                  {mode === 'shift' && (
                    <div className="flex items-center gap-4 mt-4">
                      <Button
                        ...
                        className="h-10 w-10 border-[var(--ds-border,var(--ds-border, #e2e8f0))]"
                      >
                        <Minus className="h-4 w-4" />
                      </Button>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-[var(--ds-text,var(--ds-text, #0f172a))]">
                          {shiftDays > 0 && '+'}{shiftDays}
                        </div>
                        <div className="text-xs text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748b))]">days</div>
                      </div>
                      <Button
                        type="button"
                        variant="outline"
            ...
            <div
              className={cn(
                'p-4 rounded-lg border-2 cursor-pointer',
                mode === 'range' ? 'border-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))] bg-[var(--ds-background-selected,var(--ds-background-selected, #eff6ff))]' : 'border-[var(--ds-border,var(--ds-border, #e2e8f0))]'
              )}
                ...
                <div className="flex-1">
                  ...
                  {mode === 'range' && (
                    <div className="mt-4 space-y-3">
                      <div>
                        <Label className="text-xs text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748b))]">From date</Label>
                        <Input
                          type="date"
                          value={fromDate}
                      ...
                      <div className="flex justify-center">
                        <ArrowRight className="h-4 w-4 text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94a3b8))]" />
                      </div>
                      <div>
                        <Label className="text-xs text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748b))]">To date</Label>
                        <Input
                          type="date"
                          value={toDate}
          ...
          {isExtendingPastCycle && (
            <div className="flex items-start gap-2 p-3 bg-[#fef3c7] rounded-lg">
              <AlertTriangle className="h-4 w-4 text-[var(--ds-text-warning,var(--ds-text-warning, #d97706))] shrink-0 mt-0.5" />
          ...
          {mode === 'shift' && shiftDays !== 0 && (
            <div className="p-3 bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #f1f5f9))] rounded-lg">
              <div className="text-xs font-medium text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748b))] mb-1">Preview</div>
              <div className="text-sm text-[var(--ds-text-subtle,var(--ds-text-subtle, #334155))]">
