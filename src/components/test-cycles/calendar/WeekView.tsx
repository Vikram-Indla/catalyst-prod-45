...
export function WeekView({
  ...
  return (
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={cn(
                'flex-1 px-2 py-3 text-center border-r border-[#e2e8f0] last:border-r-0',
                isInRange && 'bg-[#eff6ff]',
                isCurrentDay && 'border-t-2 border-t-[#2563eb]'
              )}
            >
              ...
              <div
                className={cn(
                  ...
                  isCurrentDay ? 'text-[#2563eb]' : 'text-[#0f172a]'
                )}
              >
                {format(day, 'd')}
              </div>
      ...
      {/* All-day events row */}
      <div className="flex border-b border-[#e2e8f0] min-h-[60px]">
        <div className="w-16 shrink-0 px-2 py-2 text-xs font-medium text-[#64748b] border-r border-[#e2e8f0]">
          All day
        </div>
        {days.map((day) => {
          ...
          return (
            <div
              key={dateKey}
              className={cn(
                'flex-1 p-1 border-r border-[#e2e8f0] last:border-r-0 space-y-1 overflow-hidden',
                isInRange && 'bg-[#eff6ff]/50'
              )}
            >
              ...
              {events.length > 3 && (
                <div className="text-xs text-[#2563eb] font-medium px-2">
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={cn(
                'flex-1 border-r border-[#e2e8f0] last:border-r-0 relative',
                isInRange && 'bg-[#eff6ff]/30'
              )}
            >
              {HOURS.map((hour) => (
                <div
