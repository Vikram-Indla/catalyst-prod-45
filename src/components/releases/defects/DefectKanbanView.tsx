    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <Popover open={showColumnConfig} onOpenChange={setShowColumnConfig}>
          ...
          <PopoverContent className="w-72 p-3 bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)]" align="end">
...
function KanbanColumn({ column, defects, headerColorClass }: KanbanColumnProps) {
  ...
  return (
    <div className={cn(
      ...
      {/* Column Header */}
      <div className="p-3 border-b border-gray-200 dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] bg-white/50 dark:bg-[var(--ds-surface-raised,#1A1A1A)]/50">
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {statuses.map(status => {
              ...
              return (
                <span
                  key={status.id}
                  className="text-[10px] text-muted-foreground bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] px-1.5 py-0.5 rounded"
                >
                  {status.name}: {count}
                </span>
              );
...
function DefectKanbanCard({ defect }: DefectKanbanCardProps) {
  ...
  return (
    <div 
      className="bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] border border-gray-200 dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] rounded-lg p-3 hover:shadow-md cursor-pointer transition-all"
