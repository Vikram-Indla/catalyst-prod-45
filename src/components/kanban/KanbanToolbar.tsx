              ...
              return (
                <CheckRow key={e.key} checked={isSel} onClick={() => onChange(isSel ? selected.filter(k => k !== e.key) : [...selected, e.key])} tk={tk}>
                  <div style={{ flex: 1, overflow: 'hidden', minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: tk.textMuted, fontFamily: 'var(--ds-font-family-monospaced)' }}>{e.key}</div>
                  </div>
                </CheckRow>
              );
            })}
            ...
            {types.map(t => {
              ...
              return (
                <CheckRow key={t.type} checked={isSel} onClick={() => onChange(isSel ? selected.filter(k => k !== t.type) : [...selected, t.type])} tk={tk}>
                  ...
                  <span style={{ fontSize: 11, color: tk.textMuted, fontWeight: 600, fontFamily: 'var(--ds-font-family-monospaced)' }}>{t.count}</span>
                </CheckRow>
              );
            })}
          </div>
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(p => !p)}
        ...
        style={{
          ...
          fontSize: 13.5, fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)',
          transition: 'all 120ms ease', outline: 'none',
        }}>
        <Filter size={14} />
        Quick{active && <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 20, height: 20, borderRadius: 10, background: tk.selectedAccent, color: '#FFFFFF', fontSize: 11, fontWeight: 700 }}>{selected.size}</span>}
      </button>
      {open && (
        <DropdownPanel width={240} tk={tk}>
    ...
    <div className="flex" style={{ borderRadius: 6, border: `1px solid ${tk.border}`, overflow: 'hidden' }}>
      {opts.map(o => (
        <button key={o.key} onClick={() => onChange(o.key)} style={{
          ...
          border: 'none', cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)',
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(p => !p)}
        ...
        style={{
          ...
          fontFamily: 'var(--ds-font-family-body)', transition: 'all 120ms ease', outline: 'none',
        }}>
    ...
    <button onClick={onClick}
      ...
      {count > 0 && (
        <span style={{
          ...
          background: tk.selectedAccent, color: '#FFFFFF', fontSize: 11, fontWeight: 700,
        }}>{count}</span>
      )}
      <ChevronDown size={13} />
    </button>
