...
export function GenerateButton() {
  ...
  return (
    <div className="p-4 border-t border-slate-100 flex-shrink-0 bg-gradient-to-r from-slate-50/50 to-white">
      {isGenerating ? (
        // During generation: show generating state with cancel button
        <div className="flex gap-2">
          <button
            disabled
            className="flex-1 h-12 flex items-center justify-center gap-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg"
          >
          ...
          <button
            ...
            className="h-12 px-4 flex items-center justify-center gap-2 rounded-xl text-sm font-medium border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-colors"
          >
            <X className="w-4 h-4" />
            Cancel
          </button>
