...
export function CatyWorkloadRisk({ teamMembers }: CatyWorkloadRiskProps) {
  ...
  const trendArrow = (trend: MemberSignal['closureTrend'], rate: number) => {
    if (trend === 'up') return <span style={{ color: token('color.text.success', 'var(--ds-text-success, var(--ds-chart-green-bold))') }}>↑ {rate}%</span>;
    if (trend === 'down') return <span style={{ color: token('color.text.danger', 'var(--ds-text-danger, var(--ds-text-danger))') }}>↓ {rate}%</span>;
    return <span style={{ color: token('color.text.subtlest', 'var(--ds-text-subtlest)') }}>→ {rate}%</span>;
  };
  ...
  return (
    <CatyInsightCard title="Workload signals" onRefresh={analyzeWorkload} onDismiss={() => { setData(null); }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <p style={{ margin: 0, color: token('color.text', 'var(--ds-text)') }}>{data.summary}</p>
        {data.members.map((m) => (
          <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 8, borderRadius: 4, background: token('color.background.neutral.subtle', 'var(--ds-surface-sunken)') }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBlockEnd: 4 }}>
                <span style={{ font: `500 13px/16px var(--ds-font-family-body, "Atlassian Sans")`, color: token('color.text', 'var(--ds-text)') }}>{m.name}</span>
                <span style={{ font: `400 12px/16px var(--ds-font-family-body, "Atlassian Sans")`, color: token('color.text.subtlest', 'var(--ds-text-subtlest)') }}>{m.openItems} open (avg: {m.roleAvg})</span>
                {trendArrow(m.closureTrend, m.closureRate)}
                {m.staleCount > 0 && <span style={{ font: `400 12px/16px var(--ds-font-family-body, "Atlassian Sans")`, color: token('color.text.warning', 'var(--ds-text-warning)') }}>{m.staleCount} stale</span>}
              </div>
