  ...
  if (resolvedGroups.length === 0) {
    return (
      <div className="fy-empty" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px 0', border: '0.555556px solid #E2E8F0', borderRadius: 8, background: 'var(--ds-text-inverse, #FFFFFF)' }}>
        ...
        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--ds-text, var(--ds-text, #0F172A))', marginBottom: 4 }}>No work items found</p>
        <p style={{ fontSize: 11, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>Try adjusting your filters or search</p>
      </div>
    );
  }
        ...
        {resolvedGroups.map(group => {
          ...
          return (
            <React.Fragment key={group.key}>
              ...
              >
                {isCollapsed
                  ? <ChevronRight size={14} style={{ color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', flexShrink: 0 }} />
                  : <ChevronDown size={14} style={{ color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', flexShrink: 0 }} />}
                {group.label}
                <span style={{
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 13, fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>{item.key}</span>
                      {(item.attachmentCount ?? 0) > 0 && (
                        <Paperclip size={12} style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', transform: 'rotate(-45deg)' }} />
                      )}
                      <div style={{ flex: 1 }} />
                      <button
                      ...
                      >
                        <Star size={16} fill={item.starred ? '#FACC15' : 'none'} stroke={item.starred ? '#FACC15' : 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))'} strokeWidth={2} />
                      </button>
                    </div>
                    ...
                    <div
                      style={{
                        fontSize: 14, fontWeight: 600, color: 'var(--ds-text, var(--ds-text, #0F172A))', lineHeight: 1.3,
                        display: '-webkit-box',
                        WebkitLineClamp: 2,
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }}>
                      {avatarUrl ? (
                      ...
                      <span style={{ fontWeight: 500, color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.project}</span>
                      <div style={{ flex: 1 }} />
          ...
          <td key={colKey} style={{ width: columnWidths.star, overflow: 'visible', textOverflow: 'clip' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <button
                ...
                title={item.starred ? 'Unstar' : 'Star'}
              >
                <Star size={14} fill={item.starred ? '#FACC15' : 'none'} stroke={item.starred ? '#FACC15' : 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))'} strokeWidth={2} />
              </button>
            </div>
          </td>
        );
        ...
        return (
          <td key={colKey} style={{ width: columnWidths.key }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              {(item.attachmentCount ?? 0) > 0 && (
                <span title="Attachments"><Paperclip size={12} style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', flexShrink: 0, transform: 'rotate(-45deg)' }} /></span>
              )}
              <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 13, fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>{item.key}</span>
            </div>
          </td>
      ...
      case 'project':
        return (
          <td key={colKey} style={{ width: columnWidths.project, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            <span title={item.project} style={{ fontSize: 13, fontWeight: 500, color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))' }}>{item.project}</span>
          </td>
        );
      case 'hub':
        return (
        ...
        return (
          <td key={colKey} style={{ width: columnWidths.reporter }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.reporter || item.assignee.name}</span>
            </div>
          </td>
        );
      default:
