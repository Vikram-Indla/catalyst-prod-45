...
function MultiSelectDropdown<T extends string>({ 
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1 bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] border shadow-lg rounded-md max-h-[200px] overflow-y-auto z-[100]" align="start">
        {options.map(option => (
          <div
            key={option.value}
            className={cn(
...
export function IncidentsFiltersDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={handleCancel}>
      <DialogContent className="max-w-[480px] w-[95vw] p-0 bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] border shadow-xl rounded-lg overflow-hidden gap-0 [&>button.absolute]:hidden">
        {/* Header */}
