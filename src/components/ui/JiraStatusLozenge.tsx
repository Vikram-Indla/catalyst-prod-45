...
const STATUS_MAP: Record<string, { bg: string; text: string; label: string }> = {
  backlog:         { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', text: 'var(--ds-text, #253858)', label: 'Backlog' },
  ...
  in_requirements: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', text: 'var(--ds-text, #253858)', label: 'In Requirements' },
...
export function JiraStatusLozenge({ status, interactive = false, onStatusChange }: Props) {
  ...
  const config = STATUS_MAP[status] ?? {
    bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))',
