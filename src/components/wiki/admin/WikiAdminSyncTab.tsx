...
function ElapsedTimer({ startedAt }: { startedAt: string }) {
  ...
  return (
    <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 13, fontWeight: 600 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 24, fontFamily: 'var(--ds-font-family-body)', fontSize: 13 }}>
      ...
      {isLatestRunning && latestRun && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            ...
            <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 600, color: 'var(--cp-text-primary, #0F172A)' }}>
              Sync in progress
            </span>
            <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 12, color: 'var(--cp-blue)', fontWeight: 700 }}>
              {progressPct}%
            </span>
            <div style={{ flex: 1 }} />
          ...
          {/* Active step info */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontFamily: 'var(--ds-font-family-body)', fontSize: 12 }}>
            <span style={{ color: 'var(--cp-text-tertiary, #64748B)' }}>
      ...
      {isFailed && (
        <div style={{
          ...
          fontFamily: 'var(--ds-font-family-body)', fontSize: 13, color: 'var(--sem-danger)',
        }}>
      ...
      {latestRun && (
        <div style={{
          ...
          <div style={{
            ...
            fontFamily: 'var(--ds-font-family-body)', fontSize: 10, fontWeight: 700,
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <span style={{
                    fontFamily: 'var(--ds-font-family-body)', fontSize: 13, fontWeight: 500,
                    color: 'var(--cp-text-primary, #0F172A)',
                  }}>{step.name}</span>
                  {isActive && stepMeta && (
                    <div style={{
                      fontFamily: 'var(--ds-font-family-body)', fontSize: 11,
                ...
                <span style={{
                  minWidth: 140, fontFamily: 'var(--ds-font-family-body)', fontSize: 11,
                }}>
                  {isActive && (
                    <span style={{
                ...
                <span style={{
                  fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11,
      ...
      {runs.length > 1 && (
        <div>
          <div style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 14, fontWeight: 600, color: 'var(--cp-text-primary, #0F172A)', marginBottom: 8 }}>
            Recent Runs
          </div>
              ...
              <tbody>
                {runs.slice(1).map(r => (
                  <tr key={r.id} style={{ borderTop: '0.75px solid var(--cp-border-default, rgba(15,23,42,0.08))', height: 50 }}>
                    <td style={{ padding: '8px 12px', fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11 }}>{format(new Date(r.started_at), 'MMM d HH:mm')}</td>
                    <td style={{ padding: '8px 12px' }}><StatusLoz status={r.status} /></td>
                    <td style={{ padding: '8px 12px', fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11 }}>{r.total_duration_ms ? `${(r.total_duration_ms / 1000).toFixed(0)}s` : '—'}</td>
                    <td style={{ padding: '8px 12px', fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11 }}>{r.total_items_processed ?? 0}</td>
                    <td style={{ padding: '8px 12px', fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11 }}>{r.new_pages ?? 0}</td>
                    <td style={{ padding: '8px 12px', fontFamily: 'var(--ds-font-family-monospaced)', fontSize: 11 }}>{r.updated_pages ?? 0}</td>
...
export function EmptyState({ icon, message, sub }: { icon: React.ReactNode; message: string; sub: string }) {
  return (
    <div style={{
      ...
      {icon}
      <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 14, fontWeight: 600, color: 'var(--cp-text-primary, #0F172A)' }}>{message}</span>
      <span style={{ fontFamily: 'var(--ds-font-family-body)', fontSize: 12, color: 'var(--cp-text-tertiary, #64748B)', textAlign: 'center', maxWidth: 320 }}>{sub}</span>
    </div>
  );
}
