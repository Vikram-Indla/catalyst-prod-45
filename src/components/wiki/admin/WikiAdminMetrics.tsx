...
const deltaStyle = (positive: boolean): React.CSSProperties => ({
  ...
  fontSize: 11,
  fontWeight: 500,
  color: positive ? 'var(--cp-success-text, #0D7331)' : 'var(--cp-text-tertiary, #64748B)',
  marginTop: 4,
});
...
export function WikiAdminMetrics() {
  ...
  const cacheRate = stats ? Math.round((stats.cache_entries / Math.max(stats.total_chunks, 1)) * 100) : 0;
  ...
  const cards = [
    ...
    { label: 'Cache Entries', value: stats?.cache_entries ?? 0, delta: `~${cacheRate}% coverage`, positive: true },
    { label: 'Avg Confidence', value: `${confidence}%`, delta: '— stable', positive: false },
  ];
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12 }}>
      {cards.map((c) => (
        <div key={c.label} style={cardStyle}>
          ...
          <div style={deltaStyle(c.positive)}>{c.delta}</div>
        </div>
      ))}
    </div>
  );
