...
export function WikiAdminPagesTab() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              fontFamily: 'Geist, -apple-system, sans-serif', fontSize: 12, width: '100%',
        ...
        {STATUS_OPTIONS.map(s => (
          <button key={s} onClick={() => { setStatusFilter(s); setPage(0); }}
            style={{
              ...
              color: statusFilter === s ? '#fff' : 'var(--cp-text-secondary, rgba(237,237,237,0.53))',
      ...
      {pages.length === 0 ? (
        <EmptyState
          icon={<FileText style={{ width: 28, height: 28, color: 'var(--cp-text-tertiary, rgba(237,237,237,0.40))' }} />}
        ...
        <>
              ...
              <thead>
                <tr style={{ background: 'var(--cp-bg-sunken, #1A1A1A)' }}>
                  {['Title', 'Domain', 'Status', 'Confidence', 'Coverage', 'Ver', 'Reads', 'Updated', 'Actions'].map(h => (
                    <th key={h} style={{ padding: '8px 12px', textAlign: 'start', fontWeight: 650, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.03em', color: 'var(--cp-text-tertiary, rgba(237,237,237,0.40))', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '8px 12px' }}>
                        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, padding: '2px 6px', borderRadius: 4, background: 'var(--cp-bg-sunken, #1A1A1A)', color: 'var(--cp-text-secondary, rgba(237,237,237,0.53))' }}>{p.domain_code ?? '—'}</span>
                      </td>
                      <td style={{ padding: '8px 12px' }}><PageStatusLoz status={p.status} /></td>
                      <td style={{ padding: '8px 12px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: isLowConf ? 'var(--sem-danger)' : 'var(--cp-text-primary, rgba(237,237,237,0.93))', fontWeight: isLowConf ? 700 : 400 }}>
                      ...
                      <td style={{ padding: '8px 12px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11 }}>{p.read_count ?? 0}</td>
                      <td style={{ padding: '8px 12px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11 }}>{(p.days_since_update ?? 0) > 0 ? `${p.days_since_update}d` : '—'}</td>
                      <td style={{ padding: '8px 12px', display: 'flex', gap: 4, alignItems: 'center' }}>
                        ...
                        <select
                          ...
                          style={{
                            ...
                            fontFamily: 'Geist, -apple-system, sans-serif', color: 'var(--cp-text-secondary, rgba(237,237,237,0.53))',
                            outline: 'none',
                          }}
                          onFocus={(e) => { e.currentTarget.style.boxShadow = '0 0 0 2px var(--cp-primary-60, #2563EB)'; }}
                          onBlur={(e) => { e.currentTarget.style.boxShadow = 'none'; }}
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end', fontFamily: 'Geist, -apple-system, sans-serif', fontSize: 12 }}>
              <span style={{ color: 'var(--cp-text-tertiary, rgba(237,237,237,0.40))', marginInlineEnd: 8 }}>{total} pages</span>
...
function ActionBtn({ icon, title, onClick }: { icon: React.ReactNode; title: string; onClick: () => void }) {
  return (
    <button onClick={onClick} title={title} aria-label={title} style={{
      ...
      color: 'var(--cp-text-tertiary, rgba(237,237,237,0.40))', outline: 'none',
    }}
      onFocus={(e) => { e.currentTarget.style.boxShadow = '0 0 0 2px var(--cp-primary-60, #2563EB)'; }}
      onBlur={(e) => { e.currentTarget.style.boxShadow = 'none'; }}
