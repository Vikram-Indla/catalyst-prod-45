...
export function WikiAdminPagesTab() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {STATUS_OPTIONS.map(s => (
          <button key={s} onClick={() => { setStatusFilter(s); setPage(0); }}
            style={{
              ...
              border: '1px solid var(--cp-border-default, rgba(15,23,42,0.12))',
              background: statusFilter === s ? 'var(--cp-primary-60, var(--cp-workstream-catalyst-primary, #2563EB))' : 'transparent',
              color: statusFilter === s ? 'var(--ds-surface, #fff)' : 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))',
      ...
      {pages.length === 0 ? (
        <EmptyState
          icon={<FileText style={{ width: 28, height: 28, color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))' }} />}
        ...
        <>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '8px 12px' }}>
                        <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 11, padding: '2px 6px', borderRadius: 4, background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))', color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))' }}>{p.domain_code ?? '—'}</span>
                      </td>
                      ...
                      <td style={{ padding: '8px 12px', fontFamily: 'var(--cp-font-mono)', fontSize: 11 }}>{p.version ?? '—'}</td>
                      <td style={{ padding: '8px 12px', fontFamily: 'var(--cp-font-mono)', fontSize: 11 }}>{p.read_count ?? 0}</td>
                      <td style={{ padding: '8px 12px', fontFamily: 'var(--cp-font-mono)', fontSize: 11 }}>{(p.days_since_update ?? 0) > 0 ? `${p.days_since_update}d` : '—'}</td>
                      <td style={{ padding: '8px 12px', display: 'flex', gap: 4, alignItems: 'center' }}>
                        ...
                        <select
                          ...
                          style={{
                            ...
                            fontFamily: 'var(--cp-font-body)', color: 'var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))',
                            outline: 'none',
                          }}
                          onFocus={(e) => { e.currentTarget.style.boxShadow = '0 0 0 2px var(--cp-primary-60, var(--cp-workstream-catalyst-primary, #2563EB))'; }}
                          onBlur={(e) => { e.currentTarget.style.boxShadow = 'none'; }}
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end', fontFamily: 'var(--cp-font-body)', fontSize: 12 }}>
              <span style={{ color: 'var(--cp-text-tertiary, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))', marginInlineEnd: 8 }}>{total} pages</span>
...
function PageStatusLoz({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string }> = {
    ...
    draft: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', color: 'var(--cp-text-secondary, var(--cp-text-secondary, #44546F))' },
    review: { bg: '#0C66E4', color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))' },
    archived: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', color: 'var(--cp-text-secondary, var(--cp-text-secondary, #44546F))' },
  };
