...
function HealthLoz({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string }> = {
    ...
    warning: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', color: 'var(--cp-text-secondary, #44546F)' },
    critical: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', color: 'var(--cp-text-secondary, #44546F)' },
  };
