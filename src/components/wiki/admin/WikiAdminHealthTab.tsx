              ...
              <tbody>
                {items.map(c => (
                  ...
                  >
                    ...
                    <td style={{ padding: '8px 12px', fontFamily: 'var(--cp-font-mono)', fontSize: 11 }}>{c.value ?? '—'}</td>
                    <td style={{ padding: '8px 12px', fontFamily: 'var(--cp-font-mono)', fontSize: 11 }}>{c.threshold ?? '—'}</td>
...
function HealthLoz({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string }> = {
    ...
    warning: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', color: 'var(--cp-text-secondary, #44546F)' },
    critical: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', color: 'var(--cp-text-secondary, #44546F)' },
  };
