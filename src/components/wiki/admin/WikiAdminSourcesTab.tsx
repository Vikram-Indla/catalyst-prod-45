...
function DocStatusLoz({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string }> = {
    ...
    failed: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', color: 'var(--cp-text-secondary, #44546F)' },
    deleted: { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', color: 'var(--cp-text-secondary, #44546F)' },
  };
