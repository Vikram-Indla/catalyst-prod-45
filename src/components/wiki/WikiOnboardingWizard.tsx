  ...
  const steps = [
    ...
    <div key="step1">
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        ...
        <h2 style={{ fontFamily: F.sora, fontSize: 18, fontWeight: 700, marginBottom: 6, color: isDark ? '#F5F3F0' : undefined }}>Welcome to WikiHub</h2>
        <p style={{ fontSize: 13, color: isDark ? '#6B6560' : 'var(--fg-3)', lineHeight: 1.5 }}>Let's personalize your experience. What's your role?</p>
      </div>
      <div style={{ display: 'grid', gap: 6 }}>
        {ROLES.map(r => (
          <button key={r} onClick={() => setRole(r)} style={{
            padding: '12px 16px', borderRadius: 8, background: isDark ? '#1A1714' : 'var(--bg-app)',
            border: `0.75px solid ${role === r ? 'var(--cp-blue)' : (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)')}`,
            cursor: 'pointer', textAlign: 'left', fontSize: 13, fontWeight: role === r ? 600 : 400,
            color: role === r ? 'var(--cp-blue)' : (isDark ? '#A09890' : 'var(--fg-2)'), transition: 'all 80ms',
    ...
    // Step 2: Domains
    <div key="step2">
      <h2 style={{ fontFamily: F.sora, fontSize: 16, fontWeight: 700, marginBottom: 6, textAlign: 'center', color: isDark ? '#F5F3F0' : undefined }}>Choose Your Domains</h2>
      <p style={{ fontSize: 12, color: isDark ? '#6B6560' : 'var(--fg-3)', marginBottom: 20, textAlign: 'center' }}>Select domains you want to follow.</p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        {DOMAINS.map(d => {
          const active = selectedDomains.includes(d.code);
          return (
            <button key={d.code} onClick={() => toggleDomain(d.code)} style={{
              padding: '10px 12px', borderRadius: 8, background: isDark ? '#1A1714' : 'var(--bg-app)',
              border: `0.75px solid ${active ? 'var(--cp-blue)' : (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)')}`,
              cursor: 'pointer', textAlign: 'left', fontSize: 12, transition: 'all 80ms',
              display: 'flex', alignItems: 'center', gap: 8,
            }}>
              <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 3, background: active ? 'var(--cp-primary-20)' : (isDark ? '#2C2823' : 'var(--cp-bd-zone)'), color: active ? '#1E40AF' : (isDark ? '#6B6560' : 'var(--fg-3)') }}>{d.code}</span>
              <span style={{ fontWeight: active ? 600 : 400, color: active ? (isDark ? '#F5F3F0' : 'var(--fg-1)') : (isDark ? '#A09890' : 'var(--fg-2)'), flex: 1, fontSize: 11.5 }}>{d.name}</span>
    ...
    <div key="step3">
      <h2 style={{ fontFamily: F.sora, fontSize: 16, fontWeight: 700, marginBottom: 6, textAlign: 'center', color: isDark ? '#F5F3F0' : undefined }}>Start a Learning Path</h2>
      <p style={{ fontSize: 12, color: isDark ? '#6B6560' : 'var(--fg-3)', marginBottom: 20, textAlign: 'center' }}>Pick one to begin structured learning.</p>
      <div style={{ display: 'grid', gap: 8 }}>
        {(paths as any[]).slice(0, 3).map((p: any) => {
          const diffColor = DIFF_COLORS[p.difficulty] || '#64748B';
          return (
            <div key={p.id} style={{
              padding: 16, borderRadius: 8, background: isDark ? '#1A1714' : 'var(--bg-app)',
              border: `0.75px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'}`, cursor: 'pointer',
              transition: 'border-color 120ms',
            }}
              onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--cp-blue)'}
              onMouseLeave={e => e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <GraduationCap size={16} style={{ color: 'var(--cp-blue)' }} />
                <span style={{ fontFamily: F.sora, fontSize: 12.5, fontWeight: 600, flex: 1, color: isDark ? '#F5F3F0' : undefined }}>{p.title}</span>
                <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', color: diffColor }}>{p.difficulty}</span>
              </div>
              <p style={{ fontSize: 11, color: isDark ? '#6B6560' : 'var(--fg-3)', lineHeight: 1.4 }}>{p.description}</p>
              <div style={{ fontSize: 10, color: isDark ? '#6B6560' : 'var(--fg-4)', marginTop: 6 }}>{p.estimated_hours}h · {p.article_count} articles</div>
            </div>
          );
        })}
        {paths.length === 0 && (
          <p style={{ fontSize: 12, color: isDark ? '#6B6560' : 'var(--fg-4)', textAlign: 'center', padding: 20 }}>No learning paths available yet.</p>
        )}
      </div>
    </div>,
  ];
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: isDark ? '#232019' : 'var(--cp-float)', borderRadius: 12, padding: 32,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 28 }}>
          {step > 0 ? (
            <button onClick={() => setStep(s => s - 1)} style={{ background: 'none', border: 'none', fontSize: 12, color: isDark ? '#6B6560' : 'var(--fg-3)', cursor: 'pointer', fontWeight: 500 }}>Back</button>
          ) : <span />}
          {step < 2 ? (
