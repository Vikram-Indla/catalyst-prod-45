    ...
    <div onClick={onClose} role="dialog" aria-modal="true" aria-label="Wiki search" style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <Search size={16} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', flexShrink: 0 }} />
          <input
            ...
            style={{
              ...
              color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)',
            }}
          />
        </div>
        {/* Results */}
        <div role="listbox" style={{ flex: 1, overflowY: 'auto', padding: 8 }}>
          {items.length === 0 && (
            <div style={{ padding: 24, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', fontSize: 13 }}>
              {query.length >= 2 ? (
          ...
          {items.map((item, idx) => (
            ...
            >
              {item.type === 'category' ? <Folder size={14} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }} /> : <FileText size={14} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }} />}
              <span style={{ flex: 1, fontSize: 13, fontWeight: 500, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)' }}>{item.label}</span>
              <span dir="ltr" style={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }}>{item.meta}</span>
              {idx === selectedIdx && <ArrowRight size={12} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }} />}
            </div>
          ))}
        </div>
        {/* Footer */}
        <div style={{
          ...
          display: 'flex', gap: 16, fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)',
        }}>
          <span><kbd style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 10, padding: '1px 4px', borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--cp-bg-sunken)' }}>↑↓</kbd> Navigate</span>
          <span><kbd style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 10, padding: '1px 4px', borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--cp-bg-sunken)' }}>↵</kbd> Open</span>
          <span><kbd style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 10, padding: '1px 4px', borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--cp-bg-sunken)' }}>ESC</kbd> Close</span>
        </div>
      </div>
    </div>
  );
