...
export function WikiKnowledgeRequestForm({ open, onClose }: Props) {
  ...
  const labelStyle: React.CSSProperties = {
    fontSize: 11, fontWeight: 600, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--fg-2)', marginBottom: 4, display: 'block',
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center' }}>
          <h2 style={{ fontFamily: 'var(--cp-font-heading)', fontSize: 14, fontWeight: 700, margin: 0, flex: 1, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined }}>New Knowledge Request</h2>
        ...
        <div style={{ padding: '16px 24px 20px', borderTop: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.06)', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button onClick={onClose} style={{
            ...
            border: isDark ? '0.75px solid #2E2E2E' : '0.75px solid rgba(0,0,0,0.10)', background: isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--cp-float)', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--fg-2)',
          }}>Cancel</button>
          <button onClick={handleSubmit} disabled={!canSubmit || submitting} style={{
            ...
            border: 'none', background: canSubmit ? 'var(--cp-blue)' : (isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--fg-4)'), color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))',
