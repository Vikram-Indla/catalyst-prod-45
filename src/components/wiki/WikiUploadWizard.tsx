...
export function WikiUploadWizard({ open, onClose }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: 640, maxHeight: '85vh', background: 'var(--cp-bg-elevated)',
        ...
        <div style={{ padding: '16px 20px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <span style={{ fontSize: 16, fontWeight: 650, fontFamily: 'var(--cp-font-body)', color: 'var(--cp-text-primary)' }}>Upload Documents</span>
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--cp-text-muted)', padding: 4 }}><X size={16} /></button>
          </div>
          <div style={{ height: 3, background: 'var(--cp-bg-sunken)', borderRadius: 2, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 8, marginBottom: 12 }}>
            {['Select Files', 'Classify', 'Review', 'Processing'].map((label, i) => (
              <span key={i} style={{
                ...
                color: step > i ? 'var(--cp-primary-60)' : step === i + 1 ? 'var(--cp-text-primary)' : 'var(--cp-text-muted)',
              }}>{label}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
          {step === 1 && (
            <>
              <div
                ...
                style={{
                  border: '2px dashed var(--cp-border-default)', borderRadius: 6,
                  padding: 40, textAlign: 'center', cursor: 'pointer',
                  background: 'var(--cp-bg-surface)', marginBottom: 16,
                }}
                onClick={() => document.getElementById('wiki-file-input')?.click()}
              >
                <Upload size={28} style={{ color: 'var(--cp-text-muted)', margin: '0 auto 8px' }} />
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--cp-text-secondary)' }}>Drop files here or click to browse</div>
                <div style={{ fontSize: 11, color: 'var(--cp-text-muted)', marginTop: 4 }}>PDF, DOCX, XLSX, PPTX, MD, TXT, CSV · Max 50MB · Up to 5 files</div>
                <input id="wiki-file-input" type="file" multiple accept={ACCEPT} onChange={handleFileInput} style={{ display: 'none' }} />
              </div>
              {files.map((f, i) => (
                <div key={i} style={{
                  display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px',
                  borderRadius: 4, border: '1px solid var(--cp-border-subtle)', marginBottom: 4,
                }}>
                  <FileText size={14} style={{ color: 'var(--cp-text-muted)' }} />
                  <span style={{ flex: 1, fontSize: 12, color: 'var(--cp-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.file.name}</span>
                  <span style={{ fontSize: 10, fontFamily: 'var(--cp-font-mono)', color: 'var(--cp-text-muted)' }}>{(f.file.size / 1024 / 1024).toFixed(1)} MB</span>
                  <button onClick={() => removeFile(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--cp-text-muted)' }}><X size={12} /></button>
                </div>
              ))}
            </>
          )}
                ...
                {DOMAINS.map(d => (
                  <button key={d.code} onClick={() => updateFile(idx, 'domain', d.code)}
                    style={{
                      ...
                      border: f.domain === d.code ? '1.5px solid var(--cp-primary-60)' : '1px solid var(--cp-border-default)',
                      background: f.domain === d.code ? 'var(--cp-primary-5)' : 'transparent',
                      color: f.domain === d.code ? 'var(--cp-primary-60)' : 'var(--cp-text-secondary)',
                    }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 }}>
                {DOC_TYPES.map(t => (
                  <button key={t} onClick={() => updateFile(idx, 'docType', t)}
                    style={{
                      fontSize: 11, fontWeight: 500, padding: '4px 8px', borderRadius: 4, cursor: 'pointer',
                      border: f.docType === t ? '1.5px solid var(--cp-primary-60)' : '1px solid var(--cp-border-default)',
                      background: f.docType === t ? 'var(--cp-primary-5)' : 'transparent',
                      color: f.docType === t ? 'var(--cp-primary-60)' : 'var(--cp-text-secondary)',
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input value={f.version} onChange={e => updateFile(idx, 'version', e.target.value)}
                    style={{ width: '100%', fontSize: 12, padding: '6px 8px', borderRadius: 4, border: '1px solid var(--cp-border-default)', background: 'transparent', color: 'var(--cp-text-primary)', fontFamily: 'var(--cp-font-body)', outline: 'none' }} />
                </div>
                <div style={{ flex: 1 }}>
          ...
          {step === 3 && (
            <>
              <div style={{ fontSize: 12, color: 'var(--cp-text-secondary)', marginBottom: 16 }}>
                Review your uploads before processing:
              </div>
              <div style={{ border: '1px solid var(--cp-border-default)', borderRadius: 6, overflow: 'hidden' }}>
                {files.map((f, i) => (
                  <div key={i} style={{
                    display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px',
                    borderBottom: i < files.length - 1 ? '0.75px solid var(--cp-border-subtle)' : 'none',
                  }}>
                    <FileText size={14} style={{ color: 'var(--cp-text-muted)' }} />
                    <span style={{ flex: 1, fontSize: 12, fontWeight: 500, color: 'var(--cp-text-primary)' }}>{f.file.name}</span>
                    <DomainBadge code={f.domain} />
                    <span style={{ fontSize: 10, color: 'var(--cp-text-muted)', textTransform: 'capitalize' }}>{f.docType.replace(/_/g, ' ')}</span>
                  </div>
                ))}
              </div>
        ...
        {/* Footer */}
        {step < 4 && (
          <div style={{ padding: '12px 20px', borderTop: '1px solid var(--cp-border-default)', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            {step > 1 && <button onClick={() => setStep(s => s - 1)} style={{ fontSize: 12, fontWeight: 600, padding: '6px 16px', borderRadius: 4, border: '1px solid var(--cp-border-default)', background: 'transparent', color: 'var(--cp-text-secondary)', cursor: 'pointer' }}>Back</button>}
            <button
