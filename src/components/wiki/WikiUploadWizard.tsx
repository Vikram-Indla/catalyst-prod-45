  ...
  return (
    <div style={{ marginBottom: 16, padding: 12, borderRadius: 6, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)' }}>
      ...
      {isFailed ? (
        ...
        steps.map((s, si) => (
          <div key={si} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontSize: 11, color: s.done ? 'var(--cp-success-60)' : (isDark ? '#666666' : 'var(--cp-text-muted)') }}>
      ...
      {isDone && status && (
        <div style={{ fontSize: 10, color: isDark ? '#666666' : 'var(--cp-text-muted)', marginTop: 6, fontFamily: 'var(--cp-font-mono)' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#666666' : 'var(--cp-text-muted)', padding: 4 }}><X size={16} /></button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 8, marginBottom: 12 }}>
            {['Select Files', 'Classify', 'Review', 'Processing'].map((label, i) => (
              <span key={i} style={{
                ...
                color: step > i ? 'var(--cp-primary-60)' : step === i + 1 ? (isDark ? '#EDEDED' : 'var(--cp-text-primary)') : (isDark ? '#666666' : 'var(--cp-text-muted)'),
              }}>{label}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
            ...
            <>
              <div
                ...
                style={{
                  ...
                  background: isDark ? '#111111' : 'var(--cp-bg-surface)', marginBottom: 16,
                }}
                onClick={() => document.getElementById('wiki-file-input')?.click()}
              >
                <Upload size={28} style={{ color: isDark ? '#666666' : 'var(--cp-text-muted)', margin: '0 auto 8px' }} />
                <div style={{ fontSize: 13, fontWeight: 600, color: isDark ? '#888888' : 'var(--cp-text-secondary)' }}>Drop files here or click to browse</div>
                <div style={{ fontSize: 11, color: isDark ? '#666666' : 'var(--cp-text-muted)', marginTop: 4 }}>PDF, DOCX, XLSX, PPTX, MD, TXT, CSV · Max 50MB · Up to 5 files</div>
              ...
              {files.map((f, i) => (
                <div key={i} style={{
                  ...
                  <FileText size={14} style={{ color: isDark ? '#666666' : 'var(--cp-text-muted)' }} />
                  ...
                  <span style={{ fontSize: 10, fontFamily: 'var(--cp-font-mono)', color: isDark ? '#666666' : 'var(--cp-text-muted)' }}>{(f.file.size / 1024 / 1024).toFixed(1)} MB</span>
                  <button onClick={() => removeFile(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#666666' : 'var(--cp-text-muted)' }}><X size={12} /></button>
                </div>
              ))}
            </>
          )}
          ...
          {step === 2 && files.map((f, idx) => (
            <div key={idx} style={{ marginBottom: 20, padding: 12, borderRadius: 6, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)' }}>
              ...
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? '#888888' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Domain</label>
                ...
                {DOMAINS.map(d => (
                  <button key={d.code} onClick={() => updateFile(idx, 'domain', d.code)}
                    style={{
                      ...
                      color: f.domain === d.code ? 'var(--cp-primary-60)' : (isDark ? '#888888' : 'var(--cp-text-secondary)'),
                    }}>
                    <span style={{ fontFamily: 'var(--cp-font-mono)', marginInlineEnd: 4 }}>{d.code}</span>{d.name}
                  </button>
                ))}
              </div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? '#888888' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Document Type</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 }}>
                {DOC_TYPES.map(t => (
                  <button key={t} onClick={() => updateFile(idx, 'docType', t)}
                    style={{
                      ...
                      color: f.docType === t ? 'var(--cp-primary-60)' : (isDark ? '#888888' : 'var(--cp-text-secondary)'),
                      textTransform: 'capitalize',
                    }}>{t.replace(/_/g, ' ')}</button>
                ))}
              </div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? '#888888' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Purpose</label>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? '#888888' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Version</label>
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? '#888888' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Language</label>
          ...
          {step === 3 && (
            <>
              <div style={{ fontSize: 12, color: isDark ? '#888888' : 'var(--cp-text-secondary)', marginBottom: 16 }}>
              ...
              <div style={{ border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)', borderRadius: 6, overflow: 'hidden' }}>
                {files.map((f, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: 10, color: isDark ? '#666666' : 'var(--cp-text-muted)', textTransform: 'capitalize' }}>{f.docType.replace(/_/g, ' ')}</span>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 16, padding: 12, borderRadius: 6, background: isDark ? 'rgba(37,99,235,0.08)' : 'var(--cp-primary-5)', fontSize: 12, color: isDark ? '#888888' : 'var(--cp-text-secondary)' }}>
          ...
          {step === 4 && (
            <div>
              {uploading && (
                <div style={{ padding: 24, textAlign: 'center', color: isDark ? '#666666' : 'var(--cp-text-muted)', fontSize: 12 }}>
              ...
              {!uploading && uploadedDocs.length === files.length && uploadedDocs.length > 0 && (
                <div style={{ padding: 16, borderRadius: 6, background: 'var(--cp-lozenge-green-bg)', textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                    ...
                    <button onClick={handleReset} style={{ fontSize: 12, fontWeight: 600, padding: '6px 16px', borderRadius: 4, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)', background: 'transparent', color: isDark ? '#888888' : 'var(--cp-text-secondary)', cursor: 'pointer' }}>Upload More</button>
                  </div>
                </div>
              )}
            </div>
        ...
        {step < 4 && (
          <div style={{ padding: '12px 20px', borderTop: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            {step > 1 && <button onClick={() => setStep(s => s - 1)} style={{ fontSize: 12, fontWeight: 600, padding: '6px 16px', borderRadius: 4, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)', background: 'transparent', color: isDark ? '#888888' : 'var(--cp-text-secondary)', cursor: 'pointer' }}>Back</button>}
            <button
