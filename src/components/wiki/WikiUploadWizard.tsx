...
function DocStatusPoller({ docId, fileName }: { docId: string; fileName: string }) {
  ...
  return (
    <div style={{ marginBottom: 16, padding: 12, borderRadius: 6, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)' }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)', marginBottom: 8 }}>{fileName}</div>
      {isFailed ? (
        ...
        steps.map((s, si) => (
          <div key={si} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', fontSize: 11, color: s.done ? 'var(--cp-success-60)' : (isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)') }}>
            {s.done ? <Check size={12} /> : <span style={{ width: 8, height: 8, borderRadius: '50%', background: si === 0 && !isDone ? 'var(--cp-primary-60)' : (isDark ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--cp-border-default)'), animation: si === 0 && !isDone ? 'wiki-pulse 1.5s ease-in-out infinite' : 'none' }} />}
            {s.label}
          </div>
        ))
      )}
      {isDone && status && (
        <div style={{ fontSize: 10, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', marginTop: 6, fontFamily: 'var(--cp-font-mono)' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <span style={{ fontSize: 16, fontWeight: 650, fontFamily: 'var(--cp-font-body)', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)' }}>Upload Documents</span>
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', padding: 4 }}><X size={16} /></button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 8, marginBottom: 12 }}>
            {['Select Files', 'Classify', 'Review', 'Processing'].map((label, i) => (
              <span key={i} style={{
                ...
                color: step > i ? 'var(--cp-primary-60)' : step === i + 1 ? (isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)') : (isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)'),
              }}>{label}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
          {step === 1 && (
            <>
              ...
              >
                <Upload size={28} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', margin: '0 auto 8px' }} />
                <div style={{ fontSize: 13, fontWeight: 600, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)' }}>Drop files here or click to browse</div>
                <div style={{ fontSize: 11, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', marginTop: 4 }}>PDF, DOCX, XLSX, PPTX, MD, TXT, CSV · Max 50MB · Up to 5 files</div>
              ...
              {files.map((f, i) => (
                <div key={i} style={{
                  ...
                  <FileText size={14} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }} />
                  <span style={{ flex: 1, fontSize: 12, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.file.name}</span>
                  <span style={{ fontSize: 10, fontFamily: 'var(--cp-font-mono)', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }}>{(f.file.size / 1024 / 1024).toFixed(1)} MB</span>
                  <button onClick={() => removeFile(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }}><X size={12} /></button>
                </div>
              ))}
            </>
          )}
          ...
          {step === 2 && files.map((f, idx) => (
            <div key={idx} style={{ marginBottom: 20, padding: 12, borderRadius: 6, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)' }}>
              <div style={{ fontSize: 13, fontWeight: 650, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)', marginBottom: 12 }}>{f.file.name}</div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Domain</label>
                ...
                {DOMAINS.map(d => (
                  <button key={d.code} onClick={() => updateFile(idx, 'domain', d.code)}
                    style={{
                      ...
                      color: f.domain === d.code ? 'var(--cp-primary-60)' : (isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)'),
                    }}>
                    <span style={{ fontFamily: 'var(--cp-font-mono)', marginInlineEnd: 4 }}>{d.code}</span>{d.name}
                  </button>
                ))}
              </div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Document Type</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 }}>
                {DOC_TYPES.map(t => (
                  <button key={t} onClick={() => updateFile(idx, 'docType', t)}
                    style={{
                      ...
                      color: f.docType === t ? 'var(--cp-primary-60)' : (isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)'),
                      textTransform: 'capitalize',
                    }}>{t.replace(/_/g, ' ')}</button>
                ))}
              </div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Purpose</label>
              <textarea value={f.purpose} onChange={e => updateFile(idx, 'purpose', e.target.value)} rows={2}
                style={{ width: '100%', fontSize: 12, padding: 8, borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', background: 'transparent', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)', fontFamily: 'var(--cp-font-body)', resize: 'vertical', outline: 'none', marginBottom: 8 }}
                placeholder="Brief description of this document's purpose..." />
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Version</label>
                  <input value={f.version} onChange={e => updateFile(idx, 'version', e.target.value)}
                    style={{ width: '100%', fontSize: 12, padding: '6px 8px', borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', background: 'transparent', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)', fontFamily: 'var(--cp-font-body)', outline: 'none' }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Language</label>
          ...
          {step === 3 && (
            <>
              <div style={{ fontSize: 12, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', marginBottom: 16 }}>
              ...
              <div style={{ border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', borderRadius: 6, overflow: 'hidden' }}>
                {files.map((f, i) => (
                  <div key={i} style={{
                    ...
                    <FileText size={14} style={{ color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)' }} />
                    <span style={{ flex: 1, fontSize: 12, fontWeight: 500, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : 'var(--cp-text-primary)' }}>{f.file.name}</span>
                    <DomainBadge code={f.domain} />
                    <span style={{ fontSize: 10, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', textTransform: 'capitalize' }}>{f.docType.replace(/_/g, ' ')}</span>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 16, padding: 12, borderRadius: 6, background: isDark ? 'rgba(37,99,235,0.08)' : 'var(--cp-primary-5)', fontSize: 12, color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)' }}>
          ...
          {step === 4 && (
            <div>
              {uploading && (
                <div style={{ padding: 24, textAlign: 'center', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--cp-text-muted)', fontSize: 12 }}>
              ...
              {!uploading && uploadedDocs.length === files.length && uploadedDocs.length > 0 && (
                <div style={{ padding: 16, borderRadius: 6, background: 'var(--cp-lozenge-green-bg)', textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                    ...
                    <button onClick={handleReset} style={{ fontSize: 12, fontWeight: 600, padding: '6px 16px', borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', background: 'transparent', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', cursor: 'pointer' }}>Upload More</button>
                  </div>
                </div>
              )}
            </div>
        ...
        {step < 4 && (
          <div style={{ padding: '12px 20px', borderTop: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            {step > 1 && <button onClick={() => setStep(s => s - 1)} style={{ fontSize: 12, fontWeight: 600, padding: '6px 16px', borderRadius: 4, border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border-default)', background: 'transparent', color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--cp-text-secondary)', cursor: 'pointer' }}>Back</button>}
            <button
              ...
              style={{
                ...
                background: files.length === 0 && step === 1 ? (isDark ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--cp-border-default)') : 'var(--cp-primary-60)', color: 'var(--cp-on-primary)',
