...
function DocStatusPoller({ docId, fileName }: { docId: string; fileName: string }) {
  ...
  return (
    <div style={{ marginBottom: 16, padding: 12, borderRadius: 6, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)' }}>
      ...
      {isDone && status && (
        <div style={{ fontSize: 10, color: isDark ? '#878787' : 'var(--cp-text-muted)', marginTop: 6, fontFamily: 'var(--cp-font-mono)' }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 8, marginBottom: 12 }}>
            {['Select Files', 'Classify', 'Review', 'Processing'].map((label, i) => (
              <span key={i} style={{
                ...
                color: step > i ? 'var(--cp-primary-60)' : step === i + 1 ? (isDark ? '#EDEDED' : 'var(--cp-text-primary)') : (isDark ? '#878787' : 'var(--cp-text-muted)'),
              }}>{label}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
          {step === 1 && (
            <>
              ...
              >
                <Upload size={28} style={{ color: isDark ? '#878787' : 'var(--cp-text-muted)', margin: '0 auto 8px' }} />
                <div style={{ fontSize: 13, fontWeight: 600, color: isDark ? '#A1A1A1' : 'var(--cp-text-secondary)' }}>Drop files here or click to browse</div>
                <div style={{ fontSize: 11, color: isDark ? '#878787' : 'var(--cp-text-muted)', marginTop: 4 }}>PDF, DOCX, XLSX, PPTX, MD, TXT, CSV · Max 50MB · Up to 5 files</div>
              ...
              {files.map((f, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 10, fontFamily: 'var(--cp-font-mono)', color: isDark ? '#878787' : 'var(--cp-text-muted)' }}>{(f.file.size / 1024 / 1024).toFixed(1)} MB</span>
                  <button onClick={() => removeFile(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: isDark ? '#878787' : 'var(--cp-text-muted)' }}><X size={12} /></button>
                </div>
              ))}
            </>
          )}
          ...
          {step === 2 && files.map((f, idx) => (
            <div key={idx} style={{ marginBottom: 20, padding: 12, borderRadius: 6, border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)' }}>
              ...
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: isDark ? '#A1A1A1' : 'var(--cp-text-secondary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Document Type</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 }}>
                {DOC_TYPES.map(t => (
              ...
              <div style={{ border: isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid var(--cp-border-default)', borderRadius: 6, overflow: 'hidden' }}>
                {files.map((f, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: 10, color: isDark ? '#878787' : 'var(--cp-text-muted)', textTransform: 'capitalize' }}>{f.docType.replace(/_/g, ' ')}</span>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 16, padding: 12, borderRadius: 6, background: isDark ? 'rgba(37,99,235,0.08)' : 'var(--cp-primary-5)', fontSize: 12, color: isDark ? '#A1A1A1' : 'var(--cp-text-secondary)' }}>
          ...
          {step === 4 && (
            <div>
              {uploading && (
                <div style={{ padding: 24, textAlign: 'center', color: isDark ? '#878787' : 'var(--cp-text-muted)', fontSize: 12 }}>
