...
export function WikiChatPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const border = 'var(--cp-border-subtle, rgba(0,0,0,0.06))';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ width: 28, height: 28, borderRadius: 6, background: 'var(--cp-blue)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <MessageCircle size={14} style={{ color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))' }} />
        </div>
        <div style={{ flex: 1 }}>
        ...
        {messages.map(m => (
          <div key={m.id} style={{
            ...
            <div style={{
              ...
              color: m.role === 'user' ? 'var(--ds-surface, var(--ds-surface, #FFFFFF))' : 'var(--fg-1)',
            }}>
              {m.loading ? (
      ...
      <div style={{
        ...
        <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
          ...
          style={{
            ...
            background: isDark ? 'var(--cp-bg-page, #1F1F21)' : 'var(--bg-1)', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined,
          }}
        ...
        <button onClick={handleSend} disabled={sending || !input.trim()} style={{
          ...
          background: 'var(--cp-blue)', color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', display: 'flex', alignItems: 'center', justifyContent: 'center',
