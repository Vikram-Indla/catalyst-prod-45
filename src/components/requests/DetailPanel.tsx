...
import { RequestMilestonesTab } from '@/components/producthub/RequestMilestonesTab';
import { RequestLinksTab } from '@/components/producthub/RequestLinksTab';
...
import { useDepartmentOptions, useProfileOptions } from '@/hooks/useRequestLookups';
...
function getV5AvatarColor(name: string): string {
  const colors = ['var(--ds-text-brand, var(--ds-text-brand, #2563eb))', '#0d9488', '#0369a1', 'var(--ds-text-warning, var(--ds-text-warning, #d97706))', '#0891b2', '#1e40af', '#b45309', '#0f766e', 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))'];
  let hash = 0;
...
/* ════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════ */
export function DetailPanel({ request, isOpen, onClose, onStatusChange, onScoreSave }: DetailPanelProps) {
  ...
  useEffect(() => {
    if (request) {
      setScores({ sa: request.score_strategic_alignment ?? 3.0, bi: request.score_business_impact ?? 3.0, tu: request.score_time_urgency ?? 3.0, rf: request.score_resource_feasibility ?? 3.0 });
      setBudgetAllocated((request as any).budget_allocated ?? 0);
  ...
  const handleQuickEdit = useCallback(async (field: string, value: any, label?: string) => {
    ...
    try {
      const { error } = await typedQuery('ph_requests').update({ [field]: sanitized, updated_at: new Date().toISOString() }).eq('id', request.id);
  ...
  const handleScoreSave = useCallback(async () => {
    if (!request) return;
    if (!isNativeInitiative(request.id)) { catalystToast.error('Jira-sourced items cannot be scored here'); return; }
    try {
      const { data: { user } } = await supabase.auth.getUser();
      await typedQuery('ph_request_scores').upsert({
        request_id: request.id, strategic_alignment: scores.sa, business_impact: scores.bi,
      ...
      await typedQuery('ph_requests').update({
  ...
  }, [request, scores, computedScore, queryClient]);
  ...
  const handleClone = async () => {
    try {
      ...
      const prefix = request.initiative_key?.replace(/-\d+$/, '') || 'MIM';
      const nextKey = `${prefix}-${String(maxNum + 1).padStart(3, '0')}`;
      await typedQuery('ph_requests').insert({ title: `${request.title} (Copy)`, initiative_key: nextKey, description: request.description, status: 'backlog', progress: 0, department_id: request.department_id, assignee_id: request.assignee_id });
      invalidateAllInitiatives(queryClient);
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        for (const file of Array.from(input.files)) {
          const path = `requests/${request.id}/${Date.now()}_${file.name}`;
          const { error: uploadErr } = await supabase.storage.from('attachments').upload(path, file);
          if (uploadErr) throw uploadErr;
          await typedQuery('ph_request_attachments').insert({
            request_id: request.id, file_name: file.name, file_path: path,
            file_size: file.size, mime_type: file.type, uploaded_by: user?.id || null,
          });
        }
        queryClient.invalidateQueries({ queryKey: ['ph-attachments', request.id] });
  ...
  const isJiraSourced = !isNativeInitiative(request.id);
  ...
  const handleUpdateBudgetAllocated = async (value: string) => {
    ...
    await typedQuery('ph_requests').update({ budget_allocated: amount, updated_at: new Date().toISOString() }).eq('id', request.id);
    invalidateAllInitiatives(queryClient);
  };
  ...
  const portalContent = (
    <div data-module="product-backlog">
            ...
            <motion.div ref={panelRef} className="pb-panel"
              ...
              <div className="pb-panel-identity">
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  ...
                  {request.is_favorited && <span style={{ color: 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))', fontSize: 14 }}>★</span>}
                </div>
                <InlineEditableTitle value={request.title} onSave={(v) => handleQuickEdit('title', v, 'Title')} />
                <div style={{ marginTop: 12 }}>
                  <StatusBadge status={request.status} editable={true}
                    onChange={(s) => { onStatusChange(request.id, s); handleQuickEdit('status', s, 'Status'); }} />
                </div>
              </div>
              ...
              <div data-body className="pb-panel-body">
                {activeTab === 'Details' && (
                  <DetailsContent request={request} onQuickEdit={handleQuickEdit} onStatusChange={onStatusChange} />
                )}
                {activeTab === 'Score' && (
                  <ScoreContent request={request} scores={scores} computedScore={computedScore} priority={priority}
                    onScoreChange={setScores} onSave={handleScoreSave} />
                )}
                {activeTab === 'Budget' && (
                  <RequestBudgetTab requestId={request.id} budgetAllocated={budgetAllocated} onBudgetAllocatedChange={handleUpdateBudgetAllocated} />
                )}
                {activeTab === 'Risks' && <RequestRisksTab requestId={request.id} />}
                {activeTab === 'Milestones' && <RequestMilestonesTab requestId={request.id} />}
                {activeTab === 'Links' && <RequestLinksTab requestId={request.id} />}
                {activeTab === 'Audit' && <RequestAuditTab requestId={request.id} />}
              </div>
            </motion.div>
          </>
        )}
      ...
      {/* Link Dialog — now handled inline within RequestLinksTab */}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div>
                ...
                <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--pb-ink)' }}>{request?.initiative_key}: {request?.title}</p>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction style={{ background: 'var(--pb-danger)' }}
              onClick={async () => {
                ...
                try {
                  await typedQuery('ph_requests').update({ is_deleted: true }).eq('id', request.id);
                  invalidateAllInitiatives(queryClient);
                  catalystToast.success(`${request.initiative_key} deleted`);
                  onClose();
                } catch (err: any) { catalystToast.error('Failed: ' + err.message); }
              }}>
              <Trash2 size={14} /> Delete
...
/* ════════════════════════════════════════════════════
   ADD LINK FORM + LINKS TAB — Now uses shared component: RequestLinksTab
   ════════════════════════════════════════════════════ */

/* ════════════════════════════════════════════════════
   ROADMAP TOGGLE
   ════════════════════════════════════════════════════ */
function RoadmapToggleInline({ request }: { request: Request }) {
  const promoteMutation = usePromoteToRoadmap();
  const removeMutation = useRemoveFromRoadmap();
  const queryClient = useQueryClient();
  const [localOnRoadmap, setLocalOnRoadmap] = useState<boolean>(request.on_roadmap === true);
  const [isToggling, setIsToggling] = useState(false);
  const isPending = promoteMutation.isPending || removeMutation.isPending || isToggling;

  useEffect(() => { setLocalOnRoadmap(request.on_roadmap === true); }, [request.id, request.on_roadmap]);
...
/* ════════════════════════════════════════════════════
   COMMENTS SECTION — Real data from ph_request_comments
...
function CommentsSection({ requestId }: { requestId: string }) {
  ...
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await typedQuery('ph_request_comments')
        .select('id, body, author_id, created_at, updated_at').eq('request_id', requestId).order('created_at', { ascending: true });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      await typedQuery('ph_request_comments').insert({ request_id: requestId, body: newComment.trim(), author_id: user?.id || null });
    ...
    <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--pb-border)' }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button type="button" onClick={handleSubmit} disabled={submitting || !newComment.trim()}
          style={{ height: 50, padding: '0 16px', fontSize: 13, fontWeight: 500, background: 'var(--pb-primary)', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))', border: 'none', borderRadius: 'var(--pb-r-md)', cursor: 'pointer', opacity: submitting || !newComment.trim() ? 0.5 : 1 }}>
...
/* ════════════════════════════════════════════════════
   DETAILS TAB
   ════════════════════════════════════════════════════ */
function DetailsContent({ request, onQuickEdit, onStatusChange }: {
  request: Request;
  ...
  useEffect(() => { setSelectedTypeKey(null); }, [request.id]);
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: 20 }}>
        <div className="pb-field-label">Request Type</div>
        <div style={{ display: 'flex', gap: 8 }}>
            ...
            return (
              <button key={opt.key} onClick={async () => {
                ...
                try {
                  ...
                  if (isNativeInitiative(request.id)) {
                    await typedQuery('ph_requests').update({ initiative_type_id: typeRow.id, updated_at: now }).eq('id', request.id);
                  } else {
                    await typedQuery('ph_issue_initiative_type_overrides').upsert({ issue_key: request.initiative_key, initiative_type_id: typeRow.id, updated_at: now }, { onConflict: 'issue_key' });
                  }
                  setSelectedTypeKey(opt.key);
                  onQuickEdit('initiative_type_key', opt.key);
                  catalystToast.success('Request type updated');
      ...
      <RoadmapToggleInline request={request} />
      ...
      <div className="pb-field-grid">
        <div className="pb-field-item">
          ...
          <StatusBadge status={request.status} editable={true}
            onChange={(s) => { onStatusChange(request.id, s); onQuickEdit('status', s, 'Status'); }} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">EA Review</div>
          <EAReviewSelect value={request.ea_review ?? ''} onChange={v => onQuickEdit('ea_review', v || null, 'EA Review')} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Priority</div>
          <PrioritySelect value={request.priority ?? ''} onChange={v => onQuickEdit('priority', v || null, 'Priority')} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Target Quarter</div>
          <QuarterSelect value={request.target_quarter ?? ''} onChange={v => onQuickEdit('target_quarter', v, 'Target Quarter')} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Reporter</div>
          <PeopleSelect value={request.reporter_id ?? ''} onChange={v => onQuickEdit('reporter_id', v, 'Reporter')} profiles={profileOptions || []} placeholder="Select reporter" />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Assignee</div>
          <PeopleSelect value={request.assignee_id ?? ''} onChange={v => onQuickEdit('assignee_id', v, 'Assignee')} profiles={profileOptions || []} placeholder="Select assignee" />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Department</div>
          <DepartmentSelect value={request.department_id ?? ''} onChange={v => onQuickEdit('department_id', v, 'Department')} departments={departmentOptions || []} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Business Owner</div>
          <PeopleSelect value={request.business_owner_id ?? ''} onChange={v => onQuickEdit('business_owner_id', v, 'Business Owner')} profiles={profileOptions || []} placeholder="Select business owner" />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Business Ask Date</div>
          <InlineEditableDate value={request.business_ask_date} label="Business Ask Date"
            onSave={v => onQuickEdit('business_ask_date', v, 'Business Ask Date')} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Kickoff Date</div>
          <InlineEditableDate value={request.kickoff_date} label="Kickoff Date"
            onSave={v => onQuickEdit('kickoff_date', v, 'Kickoff Date')} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Target Complete</div>
          <InlineEditableDate value={request.target_complete} label="Target Complete"
            onSave={v => onQuickEdit('target_complete', v, 'Target Complete')} />
        </div>
        <div className="pb-field-item">
          <div className="pb-field-label">Progress</div>
          <InlineEditableProgress value={request.progress} status={request.status}
      ...
      <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--pb-border)' }}>
        ...
        <InlineEditableTextArea value={request.description} onSave={v => onQuickEdit('description', v, 'Description')} />
      </div>
...
function ScoreContent({ scores, computedScore, priority, onScoreChange, onSave }: {
  request: Request; scores: { sa: number; bi: number; tu: number; rf: number };
    ...
    <div style={{ display: 'flex', gap: 24, minHeight: 400 }}>
      <div style={{ flex: 3, display: 'flex', flexDirection: 'column' }}>
        ...
        <button type="button" onClick={onSave}
          style={{ width: '100%', height: 38, background: 'var(--pb-primary)', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))', fontSize: 13, fontWeight: 500, border: 'none', borderRadius: 'var(--pb-r-md)', cursor: 'pointer', marginTop: 'auto' }}>
          Save Score
        </button>
      </div>
      <div style={{ flex: 2, background: 'var(--pb-surface-secondary)', border: '1px solid var(--pb-border)', borderRadius: 'var(--pb-r-lg)', padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--pb-border)', width: '100%' }}>
          <p style={{ fontSize: 12, color: 'var(--pb-ink-muted)', lineHeight: 1.5, textAlign: 'center' }}>
            ...
            {priority.level === 'Unscored' && 'This request has not been scored yet.'}
          </p>
        </div>
      </div>
    </div>
