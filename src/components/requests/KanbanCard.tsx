...
export function KanbanCard({ request, density, isDragOverlay, isGhost, onClick, onFavoriteToggle }: KanbanCardProps) {
  ...
  const progressColor = isDelivered ? '#10b981' : request.progress >= 40 && request.progress < 70 ? 'var(--ds-text-warning, #f59e0b)' : 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary, #2563eb))';
