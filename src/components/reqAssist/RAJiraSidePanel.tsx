  ...
  return (
    <>
      ...
      <div style={{
        ...
        borderLeft: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.12)',
        boxShadow: isDark ? '-4px 0 24px rgba(0,0,0,0.30)' : '-4px 0 24px rgba(15,23,42,0.08)',
        animation: 'ra-slide-left 200ms ease-out',
      }}>
        {/* ── HEADER ── */}
        <div style={{ padding: '16px 20px 14px', borderBottom: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.10)', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              >
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 500, color: 'var(--cp-blue)' }}>
                  {doc.jira_ticket_key}
                </span>
          ...
          <h3 style={{
            fontSize: 18, fontWeight: 700, color: 'var(--fg-1)', margin: '10px 0 0', fontFamily: "'Sora', sans-serif",
            lineHeight: 1.3,
            display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any, overflow: 'hidden',
          }}>{doc.title}</h3>
          {/* HL-01: Parent hierarchy breadcrumb */}
          {brdData.parentJiraKey && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--fg-4)', marginTop: 2, fontFamily: "'Inter', sans-serif" }}>
              <span
        ...
        {brdData.rawTextSource === 'title_only' && (
          <div style={{
            ...
            fontSize: 12, color: isDark ? '#FBBF24' : '#FBBF24', fontFamily: "'Inter', sans-serif",
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {/* ── PIPELINE STEPPER ── */}
          <div style={{ padding: '16px 20px', borderBottom: isDark ? '0.75px solid rgba(255,255,255,0.05)' : '0.75px solid rgba(15,23,42,0.06)' }}>
            <div style={{
              background: isDark ? '#1A1A1A' : 'var(--bg-1)', border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.08)', borderRadius: 8,
              padding: '14px 16px',
            }}>
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                      <div style={{
                        ...
                        border: step.state === 'pending' ? (isDark ? '1.5px solid #292929' : '1.5px solid rgba(237,237,237,0.53)') : 'none',
                      }}>
                        {step.state === 'complete' && (
                    ...
                    {i < stepsRaw.length - 1 && (
                      <div style={{
                        ...
                        borderTop: step.state === 'complete' && stepsRaw[i + 1].state === 'complete'
                          ? '2px solid #16A34A'
                          : step.state === 'complete'
                            ? (isDark ? '2px dashed #292929' : '2px dashed rgba(237,237,237,0.53)')
          ...
          <div style={{ padding: '16px 20px', borderBottom: isDark ? '0.75px solid rgba(255,255,255,0.05)' : '0.75px solid rgba(15,23,42,0.06)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 10 }}>
              <MetaRow label="Ticket">
                <a
                  ...
                  style={{
                    ...
                    fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 500, color: 'var(--cp-blue)',
              ...
              <MetaRow label="Title">
                <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--fg-1)', fontFamily: "'Inter', sans-serif" }}>{doc.title}</span>
              </MetaRow>
              {description && (
                <MetaRow label="Description">
                  <div style={{ flex: 1 }}>
                    <div style={{
                      background: isDark ? '#1A1A1A' : 'var(--bg-1)', border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.08)', borderRadius: 6,
                    ...
                    {descTruncated && (
                      <button onClick={() => setDescExpanded(!descExpanded)} style={{
                        ...
                        fontSize: 11, fontWeight: 500, color: 'var(--cp-blue)', fontFamily: "'Inter', sans-serif",
                      }}>
              ...
              <MetaRow label="Imported">
                <span style={{ fontSize: 12, color: 'var(--fg-3)', fontFamily: "'Inter', sans-serif" }}>
                ...
                <span style={{ fontSize: 13, color: domain ? 'var(--fg-2)' : 'var(--fg-4)', fontFamily: "'Inter', sans-serif" }}>
                  {domain || (
                    <span style={{
                      ...
                      background: isDark ? '#1A1A1A' : '#1A1A1A', borderRadius: 4, fontSize: 11, color: 'var(--fg-4)',
                      fontFamily: "'Inter', sans-serif",
                    }}>Uncategorised</span>
                  )}
                </span>
                ...
                {doc.pdf_url ? (
                  ...
                  <>
                    <button onClick={() => fileInputRef.current?.click()} disabled={uploadingPdf} style={{
                      ...
                      border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(237,237,237,0.53)', borderRadius: 6, cursor: 'pointer', padding: '4px 10px',
                      fontFamily: "'Inter', sans-serif",
                      display: 'inline-flex', alignItems: 'center', gap: 4,
                    }}>
            ...
            <div style={{ marginTop: 10 }}>
              {doc.pdf_url ? (
                <div style={{
                  ...
                  background: isDark ? '#1A1A1A' : 'var(--bg-app)', border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.10)',
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--fg-4)', fontFamily: "'Inter', sans-serif" }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px 0', gap: 4 }}>
                  <Paperclip size={18} color={isDark ? '#292929' : 'rgba(237,237,237,0.53)'} />
                  <span style={{ fontSize: 13, color: 'var(--fg-4)', fontFamily: "'Inter', sans-serif" }}>No attachments found</span>
                  <span style={{ fontSize: 11, color: isDark ? '#878787' : 'rgba(237,237,237,0.53)', fontFamily: "'Inter', sans-serif" }}>Attachments are imported automatically from Jira</span>
                </div>
              )}
            </div>
          </div>
            ...
            <div style={{ marginTop: 10 }}>
              {brdData.raw_text ? (
                <>
                  ...
                  {brdData.raw_text.length > 400 && (
                    <button onClick={() => setContentExpanded(!contentExpanded)} style={{
                      ...
                      fontSize: 12, color: 'var(--cp-blue)', fontWeight: 500, padding: '4px 0', fontFamily: "'Inter', sans-serif",
                    }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 0', gap: 6 }}>
                  <FileText size={20} color={isDark ? '#292929' : 'rgba(237,237,237,0.53)'} />
                  <span style={{ fontSize: 13, color: 'var(--fg-4)', fontFamily: "'Inter', sans-serif" }}>Content not yet extracted</span>
                  <span style={{ fontSize: 12, color: isDark ? '#878787' : 'rgba(237,237,237,0.53)', fontFamily: "'Inter', sans-serif" }}>Upload a PDF or extract from Jira attachments</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ padding: '16px 20px' }}>
            ...
            <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column' }}>
              ...
              {/* Epics Row — ALL BLUE */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: isDark ? '0.75px solid rgba(255,255,255,0.05)' : '0.75px solid rgba(15,23,42,0.06)' }}>
                ...
                {epicCount > 0 ? (
                  ...
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', padding: '2px 10px', borderRadius: 12,
                    background: isDark ? '#1A1A1A' : '#1A1A1A', color: 'var(--fg-4)', border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.12)',
              ...
              <div style={{
                ...
                <div style={{ width: 28, height: 28, borderRadius: 6, background: isDark ? '#1A1A1A' : '#1A1A1A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                {uatCount > 0 ? (
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', padding: '2px 10px', borderRadius: 12,
                    background: isDark ? 'rgba(22,163,74,0.12)' : 'rgba(74,222,128,0.06)', border: isDark ? '0.75px solid rgba(22,163,74,0.25)' : '0.75px solid #BBF7D0',
                    ...
                    uatScenarios.map((s, i) => {
                      ...
                      return (
                        <div key={s.id || i} style={{
                          ...
                          <span style={{
                            fontSize: 13, color: isDark ? '#EDEDED' : 'rgba(237,237,237,0.93)', fontFamily: "'Inter', sans-serif",
        ...
        {/* ── FOOTER CTA ── */}
        <div style={{ padding: '14px 20px', borderTop: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.10)', flexShrink: 0, background: isDark ? '#0A0A0A' : 'var(--bg-app)' }}>
          {epicCount === 0 ? (
            ...
            <button onClick={() => { if (brdData.id && onViewDrafts) { onClose(); onViewDrafts(brdData.id); } }} style={{
              width: '100%', height: 40, fontSize: 14, fontWeight: 600, borderRadius: 6,
              border: isDark ? '0.75px solid rgba(255,255,255,0.08)' : '0.75px solid rgba(15,23,42,0.15)', background: isDark ? '#1A1A1A' : 'var(--bg-app)', color: 'var(--fg-2)',
...
function MetaRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start' }}>
      <span style={{ width: 100, fontSize: 12, color: 'var(--fg-4)', flexShrink: 0, fontFamily: "'Inter', sans-serif", paddingTop: 2 }}>{label}</span>
