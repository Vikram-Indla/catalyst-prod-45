...
function StatCard({ label, value, subLabel, loading, animatedBadge }: StatCardProps) {
  return (
    <div style={{
      padding: '16px 20px',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {loading ? (
          ...
          <>
            <span style={{ fontSize: 28, fontWeight: 700, color: '#111827', fontFamily: "'Sora', sans-serif" }}>{value}</span>
            {animatedBadge && (
      ...
      {subLabel && (
        <span style={{ fontSize: 11, color: '#64748B', fontFamily: "'Inter', sans-serif", marginTop: 2 }}>
          {subLabel}
        </span>
      )}
    </div>
...
export default function RAStatsBar({ totalDocuments, wikihubSynced, wikihubChunks, artifactsGenerated, processingCount, loading }: StatsBarProps) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
      ...
      <StatCard label="Indexed in WikiHub" value={`${wikihubSynced} / ${totalDocuments}`} subLabel={chunkLabel} loading={loading} />
      <StatCard label="Artifacts Generated" value={artifactsGenerated} subLabel="Epics · UAT · Initiatives" loading={loading} />
      <StatCard label="Processing Now" value={processingCount} animatedBadge={processingCount > 0} loading={loading} />
      <style>{`
