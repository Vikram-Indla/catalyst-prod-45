...
export default function RAGenerationBar({ slots, artifactCounts, isProcessing, etaMinutes, docStatus, epicCount = 0, pipelineStage }: Props) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, maxWidth: 160 }}>
      ...
      <div style={{ width: 80, height: 4, borderRadius: 4, background: 'var(--bd-default, var(--cp-border, #E2E8F0))', flexShrink: 0, overflow: 'hidden' }}>
      ...
      {labelText ? (
        ...
        <span style={{ color: 'var(--ds-text-subtlest, #94A3B8)', fontSize: 13 }}>—</span>
      )}
    </div>
  );
}
