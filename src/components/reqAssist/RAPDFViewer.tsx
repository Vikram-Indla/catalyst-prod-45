    ...
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 860, maxWidth: '95vw', height: '90vh', background: 'var(--cp-float)', borderRadius: 8, zIndex: 70, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', borderBottom: '1px solid rgba(15,23,42,0.08)', flexShrink: 0 }}>
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--fg-1)', margin: 0, fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 20px', borderBottom: '1px solid rgba(15,23,42,0.06)', flexShrink: 0, background: 'var(--bg-app)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} style={{ border: '1px solid rgba(15,23,42,0.12)', background: 'var(--bg-app)', borderRadius: 4, padding: '4px 6px', cursor: page <= 1 ? 'not-allowed' : 'pointer' }}><ChevronLeft size={14} color={page <= 1 ? 'rgba(237,237,237,0.53)' : 'var(--fg-2)'} /></button>
            <span style={{ fontSize: 12, color: 'var(--fg-2)', fontFamily: "'JetBrains Mono', monospace" }}>Page {page} of {totalPages}</span>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} style={{ border: '1px solid rgba(15,23,42,0.12)', background: 'var(--bg-app)', borderRadius: 4, padding: '4px 6px', cursor: page >= totalPages ? 'not-allowed' : 'pointer' }}><ChevronRight size={14} color={page >= totalPages ? 'rgba(237,237,237,0.53)' : 'var(--fg-2)'} /></button>
          ...
          <button onClick={onGenerateEpics} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '6px 12px', fontSize: 12, fontWeight: 500, border: 'none', borderRadius: 'var(--ra-radius-btn)', background: 'var(--cp-blue)', color: '#FFFFFF', cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}>
          ...
          <div style={{
            ...
            {doc.language === 'ar' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, padding: '6px 10px', background: '#0C66E4', borderRadius: 4, border: '1px solid #B3D4FF' }}>
                ...
                <span style={{ fontSize: 12, color: '#FFFFFF', fontFamily: "'Inter', sans-serif" }}>Original Arabic document — English translation shown</span>
              </div>
            )}
            <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--fg-1)', margin: '0 0 4px', textAlign: 'center', fontFamily: "'Sora', sans-serif" }}>{doc.title}</h2>
            <p style={{ fontSize: 13, color: 'var(--fg-3)', margin: '0 0 20px', textAlign: 'center', fontFamily: "'Inter', sans-serif" }}>Business Requirements Document · {doc.jira_project}</p>
            {/* EC-008: null content_processed → info box */}
            {doc.content_processed ? (
              <p style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--fg-2)', whiteSpace: 'pre-wrap', fontFamily: "'Inter', sans-serif" }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '14px 16px', background: 'var(--cp-primary-5)', borderRadius: 'var(--ra-radius-card)', border: '1px solid #DBEAFE' }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#7DB8FC', fontFamily: "'Inter', sans-serif" }}>Content is being extracted</div>
