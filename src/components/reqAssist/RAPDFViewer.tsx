      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 860, maxWidth: '95vw', height: '90vh', background: 'var(--cp-float)', borderRadius: 8, zIndex: 70, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 20px', borderBottom: '1px solid rgba(15,23,42,0.06)', flexShrink: 0, background: 'var(--bg-app)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} style={{ border: '1px solid rgba(15,23,42,0.12)', background: 'var(--bg-app)', borderRadius: 4, padding: '4px 6px', cursor: page <= 1 ? 'not-allowed' : 'pointer' }}><ChevronLeft size={14} color={page <= 1 ? 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))' : 'var(--fg-2)'} /></button>
            <span style={{ fontSize: 12, color: 'var(--fg-2)', fontFamily: 'var(--cp-font-mono)' }}>Page {page} of {totalPages}</span>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} style={{ border: '1px solid rgba(15,23,42,0.12)', background: 'var(--bg-app)', borderRadius: 4, padding: '4px 6px', cursor: page >= totalPages ? 'not-allowed' : 'pointer' }}><ChevronRight size={14} color={page >= totalPages ? 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))' : 'var(--fg-2)'} /></button>
        ...
        <div style={{ flex: 1, overflowY: 'auto', background: 'var(--ds-border, var(--ds-border, #E5E7EB))', display: 'flex', justifyContent: 'center', padding: '24px 0' }}>
          <div style={{
            ...
            {doc.content_processed ? (
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '14px 16px', background: 'var(--cp-primary-5)', borderRadius: 'var(--ra-radius-card)', border: '1px solid #DBEAFE' }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#1E40AF', fontFamily: 'var(--cp-font-body)' }}>Content is being extracted</div>
                  <p style={{ fontSize: 12, color: 'var(--ds-text-brand, var(--ds-text-brand, #3B82F6))', margin: '4px 0 0', lineHeight: 1.5, fontFamily: 'var(--cp-font-body)' }}>
