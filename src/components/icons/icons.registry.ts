 ...
 *
 * Legacy guardrail: src/lib/jira-issue-type-icons.tsx still ships the
 * `JiraIssueTypeIcon` component used across older surfaces. New code
 * MUST use the components in this folder. Migration of legacy consumers
 * is tracked in the icon-migration backlog.
 * ═══════════════════════════════════════════════════════════════════════
 */
...
// Dark-mode overrides — only icons that fail WCAG AA on dark surface
...
export interface WorkTypeMeta {
  ...
  color: string;
  light: string;
  dark: string; // falls back to light if no override exists
}
...
/**
 * Normalize a Jira issue type string (or any free-text type) into a
 * canonical Catalyst WorkItemType. Used when consuming `ph_issues.issue_type`
 * which may have inconsistent casing / aliasing across data sources.
 *
 * Returns null when the input cannot be confidently resolved — caller
 * should fall back to the legacy `JiraIssueTypeIcon` or the 'task' default.
 */
export function normalizeWorkItemType(raw: string | null | undefined): WorkItemType | null {
  if (!raw) return null;
  const t = raw.toLowerCase().trim();

  // Direct hits (canonical kebab-case)
  if (t in WORK_TYPE_REGISTRY) return t as WorkItemType;

  // Alias resolution — order matters (more specific first)
...
/**
 * Normalize a priority string (case-insensitive) into a canonical
 * Catalyst PriorityLevel. Returns 'none' when the input is null/empty.
 */
