  ...
  return (
    <div
      ...
      onMouseEnter={e => {
        ...
        e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.18)' : 'rgba(237,237,237,0.53)';
      }}
      onMouseLeave={e => {
        if (!isDark) e.currentTarget.style.boxShadow = '0 1px 3px rgba(0,0,0,.04)';
        e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.10)' : 'rgba(255,255,255,0.10)';
        setMoveOpen(false);
      }}
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <div style={{ position: 'relative' }}>
            ...
            >
              <Move size={10} color={isDark ? 'rgba(255,255,255,0.50)' : 'rgba(237,237,237,0.40)'} />
            </button>
            {moveOpen && (
              <div style={{
                ...
                {MOVE_OPTIONS.map(opt => (
                  <button
                    ...
                    onMouseEnter={e => (e.currentTarget.style.background = isDark ? 'rgba(255,255,255,0.06)' : '#1A1A1A')}
          ...
          <button
            ...
            style={{
              ...
              background: idea.isCommitted ? 'var(--sem-success)' : (isDark ? 'rgba(255,255,255,0.20)' : 'rgba(237,237,237,0.53)'),
      ...
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }}>
        {MILESTONE_CONFIGS.map(m => {
          ...
          const CHIP_STYLES: Record<string, { bg: string; text: string; border: string }> = {
            ...
            des:  { bg: isDark ? 'rgba(139,92,246,0.15)' : '#EDE9FE', text: isDark ? '#C4B5FD' : '#A78BFA', border: isDark ? 'rgba(139,92,246,0.25)' : '#C4B5FD' },
            dev:  { bg: isDark ? 'rgba(22,163,74,0.15)' : '#DCFCE7', text: isDark ? '#86EFAC' : '#15803D', border: isDark ? 'rgba(22,163,74,0.25)' : '#86EFAC' },
            uat:  { bg: isDark ? 'rgba(217,119,6,0.15)' : 'rgba(251,191,36,0.10)', text: isDark ? '#FCD34D' : '#FBBF24', border: isDark ? 'rgba(217,119,6,0.25)' : '#FCD34D' },
            beta: { bg: isDark ? 'rgba(13,148,136,0.15)' : '#CCFBF1', text: isDark ? '#5EEAD4' : '#0F766E', border: isDark ? 'rgba(13,148,136,0.25)' : '#5EEAD4' },
            prod: { bg: isDark ? 'rgba(22,163,74,0.15)' : '#D1FAE5', text: isDark ? '#86EFAC' : '#4ADE80', border: isDark ? 'rgba(22,163,74,0.25)' : '#6EE7B7' },
          };
          const unsetStyle = { bg: isDark ? 'rgba(255,255,255,0.04)' : '#1A1A1A', text: isDark ? 'rgba(255,255,255,0.25)' : 'rgba(237,237,237,0.53)', border: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.10)' };
      ...
      {isConverted(idea.status) ? (
        ...
        <button
          ...
          onMouseLeave={e => {
            ...
            e.currentTarget.style.color = isDark ? 'rgba(255,255,255,0.60)' : 'rgba(237,237,237,0.40)';
            e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.10)' : 'rgba(255,255,255,0.10)';
          }}
        >
          → Init
