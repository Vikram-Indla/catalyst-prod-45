...
export function DetailPanelSkeleton() {
  const dk = useIsDark();
  const shimmerBg = dk ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))';
  return (
...
function FieldRow({ label, children }: { label: string; children: React.ReactNode }) {
  const dk = useIsDark();
  return (
    <div style={{ padding: '8px 20px', borderBottom: `1px solid ${dk ? 'var(--ds-border, var(--ds-border, #292929))' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))'}` }}>
  ...
  return (
    <div>
      ...
      >
        ...
        {count !== undefined && (
          <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--fg-3)', background: dk ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))', borderRadius: 9999, padding: '1px 6px' }}>{count}</span>
        )}
      </button>
      {open && children}
    </div>
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <div style={{ padding: '8px 20px', borderBottom: '1px solid var(--divider)', display: 'flex', alignItems: 'center', gap: 8, position: 'relative' }}>
          ...
          <span style={{ color: dk ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))' }}>·</span>
                ...
                {item.assignee ? (
                  <>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--cp-blue)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))' }}>
          ...
          {item.stats.totalDescendants > 0 && (
            <FieldRow label="Progress">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ flex: 1, height: 6, background: dk ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))', borderRadius: 4, overflow: 'hidden' }}>
              ...
              {item.children.map(child => (
                <div
                  ...
                  style={{
                    ...
                    padding: '0 20px', borderBottom: `1px solid ${dk ? 'var(--ds-border, var(--ds-border, #292929))' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))'}`, cursor: 'pointer',
                    fontFamily: 'var(--cp-font-body)',
                  }}
                  onMouseEnter={e => (e.currentTarget.style.background = dk ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--bg-1, #F8FAFC)')}
                ...
                >
                  ...
                  {child.assignee && (
                    <div style={{ width: 20, height: 20, borderRadius: '50%', background: 'var(--cp-blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ fontSize: 8, fontWeight: 700, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))' }}>
        ...
        <Section title="Activity" defaultOpen={false}>
          <div style={{ padding: '8px 20px', borderBottom: '1px solid var(--divider)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: dk ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))', flexShrink: 0 }} />
              <textarea
