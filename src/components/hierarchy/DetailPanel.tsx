  ...
  return (
    <div>
      ...
      >
        ...
        {count !== undefined && (
          <span style={{ fontSize: 'var(--ds-font-size-50)', fontWeight: 600, color: 'var(--fg-3)', background: dk ? 'var(--ds-border, var(--cp-ink-1, #2E2E2E))' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))', borderRadius: 9999, padding: '1px 6px' }}>{count}</span>
        )}
      </button>
      {open && children}
    </div>
        ...
        <Section title="Key Details" defaultOpen={true}>
                ...
                {item.assignee ? (
                  <>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--cp-blue)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontSize: 'var(--ds-font-size-50)', fontWeight: 700, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))' }}>
          ...
          {item.fixVersion && (
            <FieldRow label="Fix Versions">
              <span style={{
                height: 20, padding: '0 8px', fontSize: 'var(--ds-font-size-50)', fontWeight: 600, color: 'var(--ds-chart-teal-bolder, #0f766e)',
