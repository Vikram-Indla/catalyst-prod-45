...
export function DetailPanelSkeleton() {
  const dk = useIsDark();
  const shimmerBg = dk ? 'var(--ds-border, #2E2E2E)' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))';
  return (
  ...
  return (
    <div>
      ...
      >
        ...
        {count !== undefined && (
          <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--fg-3)', background: dk ? 'var(--ds-border, #2E2E2E)' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', borderRadius: 9999, padding: '1px 6px' }}>{count}</span>
        )}
      </button>
      {open && children}
    </div>
          ...
          {item.stats.totalDescendants > 0 && (
            <FieldRow label="Progress">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ flex: 1, height: 6, background: dk ? 'var(--ds-border, #2E2E2E)' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', borderRadius: 4, overflow: 'hidden' }}>
        ...
        <Section title="Activity" defaultOpen={false}>
          <div style={{ padding: '8px 20px', borderBottom: '1px solid var(--divider)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: dk ? 'var(--ds-border, #2E2E2E)' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', flexShrink: 0 }} />
              <textarea
