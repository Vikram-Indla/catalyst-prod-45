...
const AVATAR_COLORS = ['var(--cp-teal-60, #0D9488)','var(--ds-text-brand, #2563EB)','var(--ds-text-danger, var(--cp-danger, #DC2626))','var(--ds-text-success, var(--cp-success, #16A34A))','var(--ds-text-subtlest, #64748B)','#0284C7','var(--quality-high, #059669)','#BE123C','var(--ds-background-brand-bold-hovered, #1D4ED8)','#0F766E'];
...
const TYPE_COLORS: Record<string, string> = {
  'Epic': 'var(--ds-text-brand, #2563EB)', 'Feature': 'var(--cp-teal-60, #0D9488)', 'Story': 'var(--ds-text-success, var(--cp-success, #16A34A))', 'Sub-task': 'var(--ds-text-subtlest, #64748B)',
  ...
  function renderCell(colId: string, item: WorkItem, depth: number, hasChildren: boolean, isExpanded: boolean) {
    switch (colId) {
      ...
      case 'type':
        return (
          <div style={{ padding: '0 8px' }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: TYPE_COLORS[item.issueType || ''] || 'var(--ds-text-subtlest, #64748B)' }}>
              {item.issueType || '—'}
            </span>
          </div>
        );
