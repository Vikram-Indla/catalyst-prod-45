...
export function BulkActionBar({
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))' }}>
        {selectedCount} item{selectedCount > 1 ? 's' : ''} selected
      </span>

      <button onClick={onClear} style={{
        background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px',
        display: 'flex', alignItems: 'center', gap: 4, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', fontSize: 12,
      }}>
        <X size={14} /> Clear
      </button>

      <div style={{ width: 1, height: 20, background: 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))', margin: '0 4px' }} />
      ...
      {confirmDelete ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 12, color: 'var(--ds-border-danger, var(--ds-border-danger, #FCA5A5))' }}>Delete {selectedCount} items?</span>
          <button onClick={() => { onBulkDelete(); setConfirmDelete(false); }} style={{
            fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 4,
            background: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', border: 'none', cursor: 'pointer',
          }}>Yes, delete</button>
          <button onClick={() => setConfirmDelete(false)} style={{
            fontSize: 11, padding: '4px 10px', borderRadius: 4,
            background: 'transparent', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', border: '1px solid #475569', cursor: 'pointer',
          }}>Cancel</button>
        </div>
      ) : (
...
function BulkBtn({ icon: Icon, label, onClick, danger }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      color: danger ? 'var(--ds-border-danger, var(--ds-border-danger, #FCA5A5))' : 'var(--bd-default, #E2E8F0)',
      background: 'transparent', border: '1px solid #475569', borderRadius: 4,
      cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4,
    }}
      onMouseEnter={e => (e.currentTarget.style.background = 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))')}
