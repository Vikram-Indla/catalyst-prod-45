...
export function BulkActionBar({
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))' }}>
        {selectedCount} item{selectedCount > 1 ? 's' : ''} selected
      </span>

      <button onClick={onClear} style={{
        background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px',
        display: 'flex', alignItems: 'center', gap: 4, color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', fontSize: 'var(--ds-font-size-200)',
      }}>
        <X size={14} /> Clear
      </button>
      ...
      {confirmDelete ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 'var(--ds-font-size-200)', color: 'var(--ds-border-danger, #FCA5A5)' }}>Delete {selectedCount} items?</span>
