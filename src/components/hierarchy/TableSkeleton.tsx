...
export function TableSkeleton({ rows = 10 }: { rows?: number }) {
  return (
    <div style={{ border: '1px solid #E2E8F0', borderRadius: 8, overflow: 'hidden', background: '#FFFFFF' }}>
      {/* Header */}
      <div style={{ height: 36, background: '#FAFAFA', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', gap: 16, padding: '0 16px' }}>
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} style={{
          ...
          <div style={{ width: 16, height: 16, borderRadius: 3, background: '#F1F5F9' }} className="tbl-shimmer" />
          <div style={{ width: 16, height: 16, borderRadius: 4, background: '#F1F5F9' }} className="tbl-shimmer" />
          <div style={{ width: 56, height: 12, borderRadius: 4, background: '#E2E8F0' }} className="tbl-shimmer" />
          <div style={{ flex: 1, height: 12, borderRadius: 4, background: '#F1F5F9', maxWidth: `${200 + (i % 3) * 60}px` }} className="tbl-shimmer" />
          <div style={{ width: 72, height: 22, borderRadius: 3, background: '#F1F5F9' }} className="tbl-shimmer" />
          <div style={{ width: 100, height: 12, borderRadius: 4, background: '#F1F5F9' }} className="tbl-shimmer" />
          <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#F1F5F9' }} className="tbl-shimmer" />
        </div>
      ))}
      <style>{`
