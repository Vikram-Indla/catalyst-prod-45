...
export function TableSkeleton({ rows = 10 }: { rows?: number }) {
  ...
  const subtleBorder = dk ? 'var(--ds-border, #292929)' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))';
  const shimmerStrong = dk ? 'var(--ds-border, var(--cp-ink-1, #2E2E2E))' : 'var(--bd-default, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))';
  const shimmerLight = dk ? 'var(--ds-border, #292929)' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9)))';
