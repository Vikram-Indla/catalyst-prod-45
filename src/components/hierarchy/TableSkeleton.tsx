...
export function TableSkeleton({ rows = 10 }: { rows?: number }) {
  const dk = useIsDark();
  const borderColor = dk ? 'rgba(255,255,255,0.08)' : '#E2E8F0';
  const subtleBorder = dk ? 'rgba(255,255,255,0.06)' : '#F1F5F9';
  const shimmerStrong = dk ? 'rgba(255,255,255,0.10)' : '#E2E8F0';
  ...
  return (
    <div style={{ border: `1px solid ${borderColor}`, borderRadius: 8, overflow: 'hidden', background: containerBg }}>
      ...
      <div style={{ height: 36, background: headerBg, borderBottom: `1px solid ${borderColor}`, display: 'flex', alignItems: 'center', gap: 16, padding: '0 16px' }}>
        <div style={{ width: 16, height: 16, borderRadius: 3, background: shimmerStrong }} />
      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} style={{
          ...
          <div style={{ width: 16, height: 16, borderRadius: 3, background: shimmerLight }} className="tbl-shimmer" />
          ...
          <div style={{ width: 72, height: 22, borderRadius: 3, background: shimmerLight }} className="tbl-shimmer" />
