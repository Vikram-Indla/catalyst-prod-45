...
export function TableSkeleton({ rows = 10 }: { rows?: number }) {
  const dk = useIsDark();
  const borderColor = dk ? 'var(--ds-border, #2E2E2E)' : 'var(--bd-default, #E2E8F0)';
  const subtleBorder = dk ? 'var(--ds-border, #292929)' : 'var(--ds-surface-sunken, #F1F5F9)';
  const shimmerStrong = dk ? 'var(--ds-border, #2E2E2E)' : 'var(--bd-default, #E2E8F0)';
  const shimmerLight = dk ? 'var(--ds-border, #292929)' : 'var(--ds-surface-sunken, #F1F5F9)';
