...
export function TableSkeleton({ rows = 10 }: { rows?: number }) {
  const dk = useIsDark();
  const borderColor = dk ? 'var(--ds-border, var(--cp-ink-1, #2E2E2E))' : 'var(--bd-default, var(--cp-border, #E2E8F0))';
  const subtleBorder = dk ? 'var(--ds-border, #292929)' : 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))';
  const shimmerStrong = dk ? 'var(--ds-border, var(--cp-ink-1, #2E2E2E))' : 'var(--bd-default, var(--cp-border, #E2E8F0))';
