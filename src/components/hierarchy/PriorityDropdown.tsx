...
function PriorityBarsInline({ level }: { level: number }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
      {[1, 2, 3, 4].map((i) => (
        <div key={i} style={{ width: 10, height: 4, borderRadius: 1, background: i <= level ? ('var(--cp-text-tertiary, #64748B)') : ('var(--cp-bg-sunken, var(--cp-border, #E2E8F0))') }} />
      ))}
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'var(--cp-bg-elevated, #FFFFFF)', border: isDark ? '1px solid #2E2E2E' : '1px solid var(--cp-border, #E2E8F0)', borderRadius: 6,
