 ...
 * Stage C: Wired to useCreateWorkItem hook, custom dropdowns
 */
...
export function CreateWorkItemModal({ open, onClose, projectId, parentItem }: CreateWorkItemModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setDescription('');
      if (parentItem) {
        // Auto-select the first valid child level
  ...
  const defaultStatus = (statuses as any[]).find((s: any) => s.name === 'To Do') || (statuses as any[])[0];
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      console.error('Create work item failed:', err);
    }
  };

  const priorityOptions = PRIORITY_ORDER
    .map((name) => {
      const p = (priorities as any[]).find((pr: any) => pr.name === name);
          ...
          <>
            ...
            <div style={{ padding: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {availableLevels.map((level) => (
                <div
                  key={level.id}
                  onClick={() => { setSelectedLevel(level); setStep(2); }}
                  className="hi-type-card"
                  style={{
                    padding: 16,
                    border: '1.5px solid #E2E8F0',
                    borderRadius: 8,
                ...
                <CustomSelect
                  ...
                  renderOption={(opt) => (
                    <span style={{ display: 'flex', alignItems: 'center' }}>
                      <PriorityBarsInline level={priorityToLevel(opt.label)} />
                      {opt.label}
                    </span>
                  )}
                />
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  color: '#FFFFFF',
                  background: !title.trim() ? '#94A3B8' : '#2563EB',
                  border: 'none',
                  borderRadius: 6,
