  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button type="button" onClick={() => setOpen((p) => !p)}
        style={{ width: '100%', height: 50, padding: '8px 12px', fontSize: 14, fontFamily: 'var(--cp-font-body)', color: selected ? 'var(--ds-text, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))' : 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))', background: 'var(--cp-float)', border: '1.5px solid var(--divider)', borderRadius: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', textAlign: 'left' }}>
        ...
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: 'var(--cp-float)', border: '1px solid var(--divider)', borderRadius: 6, zIndex: 100, maxHeight: 200, overflowY: 'auto' }}>
          {options.map((opt) => (
            <div key={opt.value} onClick={() => { onChange(opt.value); setOpen(false); }}
              ...
              onMouseLeave={(e) => (e.currentTarget.style.background = opt.value === value ? 'var(--ds-background-selected, #EFF6FF)' : '')}>
