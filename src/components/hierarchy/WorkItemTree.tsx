...
export function TreeSkeleton({ rows = 5 }: { rows?: number }) {
  ...
  const shimmerBg = isDark ? '#1A1A1A' : '#1A1A1A';
...
function AssigneeAvatar({ assignee }: { assignee?: WorkItem['assignee'] }) {
  const { isDark } = useTheme();
  if (!assignee) {
    return <div style={{ width: 24, height: 24, borderRadius: '50%', background: isDark ? '#1A1A1A' : '#1A1A1A', border: '1px solid var(--divider)', flexShrink: 0 }} />;
  }
...
function ProgressBar({ stats }: { stats: WorkItem['stats'] }) {
  ...
  return (
    <div style={{ width: 64, display: 'flex', flexDirection: 'column', gap: 2, justifyContent: 'center' }}>
      <div style={{ height: 4, background: isDark ? '#1A1A1A' : '#1A1A1A', borderRadius: 4, overflow: 'hidden' }}>
