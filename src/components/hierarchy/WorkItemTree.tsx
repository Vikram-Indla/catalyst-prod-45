...
export function TreeSkeleton({ rows = 5 }: { rows?: number }) {
  ...
  const shimmerBg = 'var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9))';
...
function AssigneeAvatar({ assignee }: { assignee?: WorkItem['assignee'] }) {
  const { isDark } = useTheme();
  if (!assignee) {
    return <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9))', border: '1px solid var(--divider)', flexShrink: 0 }} />;
  }
...
function ProgressBar({ stats }: { stats: WorkItem['stats'] }) {
  ...
  return (
    <div style={{ width: 64, display: 'flex', flexDirection: 'column', gap: 2, justifyContent: 'center' }}>
      <div style={{ height: 4, background: 'var(--cp-bg-sunken, var(--cp-bg-sunken, #F1F5F9))', borderRadius: 4, overflow: 'hidden' }}>
