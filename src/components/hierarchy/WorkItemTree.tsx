...
function AssigneeAvatar({ assignee }: { assignee?: WorkItem['assignee'] }) {
  ...
  return (
    <div style={{ width: 24, height: 24, borderRadius: '50%', background: bgColor, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <span style={{ fontSize: 10, fontWeight: 700, color: '#FFFFFF', fontFamily: 'var(--ds-font-family-body)' }}>{initials}</span>
    </div>
  );
}
...
function ProgressBar({ stats }: { stats: WorkItem['stats'] }) {
  ...
  return (
    <div style={{ width: 64, display: 'flex', flexDirection: 'column', gap: 2, justifyContent: 'center' }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 500, color: textColor, fontFamily: 'var(--ds-font-family-body)', fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>
        {pct}%
      </span>
    </div>
  );
      ...
      {open && (
        <div style={{
          ...
          <button
            ...
            style={{
              width: '100%', padding: '8px 12px', fontSize: 13, fontFamily: 'var(--ds-font-family-body)',
...
function NoChildrenMessage({ parentLevel, depth, onAdd, parent }: { parentLevel: number; depth: number; onAdd?: (parent: WorkItem) => void; parent: WorkItem }) {
  ...
  return (
    <div style={{
      ...
      borderBottom: '1px solid var(--divider)', fontFamily: 'var(--ds-font-family-body)',
    }}>
      ...
      {onAdd && (
        <button
          ...
          style={{
            ...
            cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)', padding: 0,
          }}
        >
          + Add {childLevel.name}
        </button>
