...
const modalBase: React.CSSProperties = {
  background: '#fff', borderRadius: 8, maxWidth: '95vw', maxHeight: '85vh',
...
const btnSecondary: React.CSSProperties = {
  ...
  cursor: 'pointer', border: 'none', background: '#F4F5F7', color: '#172B4D',
};
...
const menuItem = (hover: boolean, danger = false): React.CSSProperties => ({
  ...
  background: hover ? (danger ? '#FFEBE6' : '#F4F5F7') : 'transparent',
});
const labelStyle: React.CSSProperties = {
  fontSize: 13, fontWeight: 600, color: '#172B4D', display: 'block', marginBottom: 6,
};
      ...
      <div style={{
        ...
        <textarea
          ...
          style={{
            ...
            minHeight: 80, fontFamily: 'var(--cp-font-body)', color: '#172B4D',
            lineHeight: '1.5',
          }}
        />
        ...
        <div style={{ padding: '12px 24px', display: 'flex', gap: 8 }}>
          {STEPS.map((s, i) => (
            <div key={i} style={{
              ...
              background: i <= step ? '#0C66E4' : '#DFE1E6',
            }} />
          ))}
        </div>
        <p style={{ padding: '0 24px', fontSize: 13, color: '#6B778C', margin: '0 0 12px' }}>
          ...
          {step === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <div style={{ ...inputStyle, background: '#F4F5F7', color: '#172B4D' }}>{projectKey}</div>
              </div>
              <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {[
                  ...
                  <label key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: '#172B4D', cursor: 'pointer' }}>
            ...
            <div style={{ borderTop: '1px solid #EBECF0' }}>
              {[
                ...
                <div key={label as string} style={{ display: 'flex', padding: '10px 0', borderBottom: '1px solid #EBECF0' }}>
                  <span style={{ width: 140, fontSize: 13, fontWeight: 600, color: '#6B778C' }}>{label}</span>
                  <span style={{ fontSize: 13, color: '#172B4D' }}>{val}</span>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div style={overlayStyle} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ ...modalBase, width: 640, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '12px 24px', display: 'flex', gap: 8 }}>
          {STEPS.map((_, i) => (
            <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= step ? '#0C66E4' : '#DFE1E6' }} />
          ))}
        </div>
        <p style={{ padding: '0 24px', fontSize: 13, color: '#6B778C', margin: '0 0 12px' }}>
          Step {step + 1} of {STEPS.length}: {STEPS[step]}
        ...
        <div style={{ padding: '0 24px 20px', flex: 1, overflow: 'auto' }}>
          ...
          {step === 1 && (
            <div style={{ borderTop: '1px solid #EBECF0' }}>
              <div style={{ display: 'flex', padding: '8px 0', borderBottom: '1px solid #EBECF0', fontWeight: 700, fontSize: 12, color: '#6B778C', textTransform: 'uppercase' }}>
              ...
              {fields.map(f => (
                <div key={f.name} style={{ display: 'flex', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #EBECF0', fontSize: 13 }}>
                  <span style={{ flex: 1, fontWeight: 600, color: '#172B4D' }}>{f.name}</span>
                  <span style={{ width: 180, color: '#6B778C', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.current || '—'}</span>
            ...
            <div style={{ borderTop: '1px solid #EBECF0' }}>
              {[
                ...
                <div key={l as string} style={{ display: 'flex', padding: '10px 0', borderBottom: '1px solid #EBECF0' }}>
                  <span style={{ width: 140, fontSize: 13, fontWeight: 600, color: '#6B778C' }}>{l}</span>
                  <span style={{ fontSize: 13, color: '#172B4D' }}>{v}</span>
                </div>
              ))}
            </div>
          )}
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ ...modalBase, width: 440, padding: '24px 28px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: '#172B4D', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Archive size={20} color="#6B778C" /> Archive issue?
          </h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}><X size={18} color="#6B778C" /></button>
        </div>
        <div style={{ background: '#FFFAE6', border: '1px solid #FFE380', borderRadius: 4, padding: '12px 16px', marginBottom: 16, fontSize: 14, color: '#172B4D' }}>
  ...
  return (
    <div style={overlayStyle} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ ...modalBase, width: 440, padding: '24px 28px' }}>
        ...
        <div style={{ background: '#FFEBE6', border: '1px solid #FF8F73', borderRadius: 4, padding: '12px 16px', marginBottom: 16, fontSize: 14, color: '#172B4D' }}>
