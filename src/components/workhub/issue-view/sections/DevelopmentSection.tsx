...
const STATE_COLORS: Record<string, { bg: string; text: string; darkBg: string; darkText: string }> = {
  ...
  unknown:     { bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))', text: 'var(--ds-text-accent-gray, #505258)', darkBg: 'var(--ds-border, var(--ds-border, #292929))', darkText: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' },
};
        ...
        <div key={type}>
          ...
          {typeItems.map((devItem) => {
            ...
            return (
              ...
              >
                <span className={isDark ? 'text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))]' : 'text-[var(--ds-text-accent-gray, #505258)]'}>
                  {TYPE_ICONS[type]}
                </span>
                <div className="flex-1 min-w-0">
                  <p className={cn('font-body text-sm truncate', isDark ? 'text-[var(--ds-text,var(--ds-text, #EDEDED))]' : 'text-[var(--ds-text, #292A2E)]')}>
                    {devItem.title}
                  </p>
                  <div className={cn('flex items-center gap-2 text-xs font-body', isDark ? 'text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]' : 'text-[var(--ds-text-subtlest, #6B6E76)]')}>
                ...
                <ExternalLink className={cn(
                  ...
                  isDark ? 'text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]' : 'text-[var(--ds-text-accent-gray, #505258)]',
                )} />
              </a>
            );
          })}
