...
function IssueRow({
  ...
  return (
    ...
    >
      ...
      <span className={cn('font-mono text-xs font-medium shrink-0', isDark ? 'text-[#A1A1A1]' : 'text-[var(--ds-text-accent-gray, #505258)]')}>
        {item.key}
      </span>
      <span className={cn(
      ...
      {hasChildren && (
        <div className={hasParent ? 'mt-2' : ''}>
          ...
          {childItems.length > 0 ? (
            ...
            <p className={cn('font-body text-xs py-2', isDark ? 'text-[#878787]' : 'text-[var(--ds-text-subtlest, #6B6E76)]')}>
