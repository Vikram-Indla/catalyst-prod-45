...
export function AllWorkPagination({ currentPage, totalPages, totalCount, pageSize, onPageChange, onPageSizeChange }: Props) {
  ...
  return (
    <div
      ...
      style={{ borderTop: '1px solid var(--bd-subtle, var(--cp-ink-1))', backgroundColor: 'var(--bg-app)' }}
    >
      {/* Left: count */}
      ...
      <div className="flex items-center gap-1" role="navigation" aria-label="Pagination">
        <button
          ...
          className="p-1.5 rounded border disabled:opacity-30 hover:bg-[var(--hover)] transition-colors duration-80 focus-visible:outline-2 focus-visible:outline-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))]"
        ...
        {pages.map((p, i) => {
          ...
          return (
            <button
              ...
              className="w-8 h-8 rounded text-[12px] font-medium transition-colors duration-80 focus-visible:outline-2 focus-visible:outline-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))] focus-visible:outline-offset-2"
              style={{
      ...
      <div className="relative flex items-center gap-2" ref={sizeRef}>
        ...
        <button
          ...
          className="inline-flex items-center gap-1 px-2.5 h-8 text-[12px] rounded border hover:bg-[var(--hover)] transition-colors duration-80 focus-visible:outline-2 focus-visible:outline-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary))]"
          style={{ borderColor: 'var(--bd-default, var(--cp-ink-1))', color: 'var(--fg-1)', fontFamily: 'var(--cp-font-mono)' }}
        ...
        {sizeOpen && (
          <div
            ...
            style={{ borderColor: 'var(--bd-default, var(--cp-ink-1))', backgroundColor: 'var(--bg-app)' }}
            role="listbox"
          >
            {PAGE_SIZES.map(s => (
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-1.5 text-[12px] hover:bg-[var(--hover)] transition-colors duration-80"
                style={{ color: 'var(--fg-1)', fontFamily: 'var(--cp-font-mono)' }}
                role="option"
