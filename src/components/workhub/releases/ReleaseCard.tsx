...
function deriveStatus(r: JiraRelease): { label: string; bg: string; fg: string; dot: string } {
  if (r.totalItems > 0 && r.doneItems === r.totalItems) return { label: 'Completed', bg: 'var(--ds-background-success)', fg: 'var(--ds-background-success-bold, var(--ds-background-success-bold))', dot: 'var(--ds-text-success)' };
  if (r.blockedItems > 0) return { label: 'At Risk', bg: 'var(--ds-background-danger, var(--ds-background-danger))', fg: 'var(--ds-text-danger)', dot: 'var(--ds-text-danger)' };
  if (r.inProgressItems > 0 || r.inReviewItems > 0) return { label: 'Active', bg: 'var(--ds-background-information, var(--ds-background-information))', fg: 'var(--ds-background-brand-bold-hovered)', dot: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' };
  return { label: 'Planned', bg: 'var(--ds-surface-sunken)', fg: 'var(--ds-text-subtle)', dot: 'var(--ds-text-subtlest)' };
}

// Assign colors to project keys
const PROJECT_COLORS = ['var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', 'var(--ds-background-discovery-bold)', 'var(--ds-icon-information)', 'var(--ds-text-warning)', 'var(--ds-text-danger)', 'var(--ds-link)', 'var(--ds-text-success)', 'var(--ds-background-discovery-bold)', 'var(--ds-background-warning-bold)', 'var(--ds-text-subtle)'];
...
export function ReleaseCard({ release, onClick }: ReleaseCardProps) {
  ...
  const segments = [
    ...
    { label: 'In Review', value: release.inReviewItems, color: 'var(--ds-background-discovery-bold)' },
...
function AvatarChip({ assignee, index }: { assignee: { displayName: string; avatarUrl: string | null; roleName?: string | null }; index: number }) {
  ...
  const colors = ['var(--ds-background-discovery-bold)', 'var(--ds-chart-teal-bold)', 'var(--ds-text-warning)', 'var(--ds-text-danger)', 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', 'var(--ds-background-discovery-bold)', 'var(--ds-text-success)', 'var(--ds-link)'];
