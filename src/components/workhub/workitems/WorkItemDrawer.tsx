 ...
 * Shows: parent (key + title), description (scrollable), status, type, priority,
 * assignee, dates, comments, history. Excludes: story points, sprint, resolution, labels, components.
 */

import { X, ExternalLink, MessageSquare, Clock, User, Calendar } from 'lucide-react';
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0">
            {item.type_icon_url && (
              <img src={item.type_icon_url} alt={item.issue_type} className="w-5 h-5 shrink-0" />
            )}
            <span
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="p-5 space-y-5">
            ...
            <div>
              <DetailRow label="Type">
                <div className="flex items-center gap-1.5">
                  {item.type_icon_url && <img src={item.type_icon_url} alt="" className="w-3.5 h-3.5" />}
                  {item.issue_type}
                </div>
              </DetailRow>
              ...
              <DetailRow label="Created">{formatDate(item.jira_created_at)}</DetailRow>
              <DetailRow label="Updated">{formatDate(item.jira_updated_at)}</DetailRow>
            ...
            <div>
              ...
              {comments.length === 0 ? (
                <p className="text-[12px] italic" style={{ color: '#94a3b8' }}>No comments synced yet. Re-sync to pull latest comments.</p>
              ) : (
                <div className="space-y-2.5 max-h-[400px] overflow-y-auto">
                  {comments.map((c, i) => (
                    ...
                    >
                      ...
                      <p className="text-[12px] leading-relaxed whitespace-pre-wrap" style={{ color: '#475569' }}>
                        {c.body}
                      </p>
                    </div>
                  ))}
            ...
            <div>
              ...
              {changelog.length === 0 ? (
                <p className="text-[12px] italic" style={{ color: '#94a3b8' }}>
                  No history synced yet. Changelog sync will be available after next full sync.
                </p>
              ) : (
                <div className="space-y-1.5 max-h-[300px] overflow-y-auto">
                  {changelog.map((entry: any, i: number) => (
                    <div
                      key={i}
                      className="flex items-start gap-2 py-1.5 border-b text-[11px]"
                      style={{ borderColor: '#f1f5f9' }}
                    >
                      <span className="font-medium shrink-0" style={{ color: '#475569' }}>
                        {entry.author || 'System'}
                      </span>
