    ...
    <div ref={ref} className="relative">
      ...
      <button
        ...
        style={{
          ...
          backgroundColor: isActive ? 'var(--ds-background-selected, var(--ds-background-selected, #eff6ff))' : 'var(--wh-surface, #fff)',
      ...
      >
        ...
        {isActive && (
          <span
            ...
            style={{ backgroundColor: 'var(--wh-primary, #2563eb)', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' }}
          >
            {selected.length}
          </span>
        )}
        ...
        >
          ...
          {options.length > 0 && (
            <div className="flex items-center justify-between px-3 py-1.5 border-b" style={{ borderColor: 'var(--wh-border, #e2e8f0)' }}>
              <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))' }}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={selectNone}
                  className="text-[11px] font-medium hover:underline"
                  style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748b))' }}
                >
                  Clear
                </button>
              </div>
          ...
          <div className="overflow-y-auto flex-1" style={{ maxHeight: '260px' }}>
            {filtered.length === 0 ? (
              <div className="px-3 py-4 text-xs text-center" style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))' }}>
              ...
              filtered.map(opt => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={e => {
                      if (!isSelected) (e.currentTarget.style.backgroundColor = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #f8fafc))');
                    }}
                    onMouseLeave={e => {
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        borderColor: isSelected ? 'var(--wh-primary, #2563eb)' : 'var(--ds-text-disabled, var(--ds-text-disabled, #cbd5e1))',
                        backgroundColor: isSelected ? 'var(--wh-primary, #2563eb)' : 'transparent',
                      }}
                    >
                      {isSelected && <Check className="w-2.5 h-2.5" style={{ color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' }} />}
                    </div>
                    <span className="truncate flex-1 text-left font-medium">{opt.label}</span>
                    {opt.meta && (
                      <span className="text-[10px] shrink-0 ml-2" style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94a3b8))' }}>
                        {opt.meta}
                      </span>
                    )}
                  </button>
          ...
          {selected.length > 0 && (
            <div
              ...
              style={{ borderColor: 'var(--wh-border, #e2e8f0)', backgroundColor: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #f8fafc))' }}
            >
              <span className="text-[11px] font-medium" style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748b))' }}>
                {selected.length} selected
              </span>
              <button
                onClick={() => { onSelectionChange([]); setSearch(''); }}
                className="text-[11px] font-medium hover:underline"
                style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #dc2626))' }}
              >
                Clear all
              </button>
            </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {hasAnyFilter && (
          <button
            ...
            style={{ borderColor: '#fecaca', color: 'var(--ds-text-danger, var(--ds-text-danger, #dc2626))', backgroundColor: 'var(--ds-background-danger, var(--ds-background-danger, #fef2f2))' }}
          >
      ...
      {totalActive > 0 && (
        <div className="flex items-center gap-1.5 flex-wrap pl-7">
          {selectedProjects.map(pk => (
            <FilterChip key={`p-${pk}`} label={pk} color="var(--ds-text-brand, var(--ds-text-brand, #2563eb))" bgColor="var(--ds-background-selected, var(--ds-background-selected, #eff6ff))" borderColor="#bfdbfe" onRemove={() => removeChip('project_keys', pk)} />
          ))}
          {selectedTypes.map(t => (
            <FilterChip key={`t-${t}`} label={t} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" bgColor="var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))" borderColor="#BFDBFE" onRemove={() => removeChip('types', t)} />
          ))}
