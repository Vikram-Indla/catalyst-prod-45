  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 8 }}>
        ...
        {comments.map((c: any) => (
          <div key={c.id} style={{
            background: '#FFFFFF', border: '1px solid rgba(15,23,42,0.06)', borderRadius: 6,
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {c.comment_updated_at && (
                  <span style={{ fontSize: 12, color: '#94A3B8', fontStyle: 'italic' }}>(edited)</span>
                )}
              </div>
            </div>
            {/* Body */}
            <div style={{ fontSize: 14, color: '#334155', lineHeight: 1.5, marginTop: 8, whiteSpace: 'pre-wrap' }}>
              {c.body_text}
            </div>
          </div>
        ))}
      ...
      {/* Input */}
      <div style={{ borderTop: '1px solid rgba(15,23,42,0.08)', padding: '12px 0 0', background: '#FFFFFF' }}>
        <textarea
          ...
          style={{
            ...
            padding: '8px 12px', fontSize: 14, color: '#0F172A', fontFamily: 'Inter, sans-serif',
            resize: 'vertical', outline: 'none', minHeight: 36, maxHeight: 120, background: 'white',
        ...
        {text.trim() && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 8, marginTop: 6 }}>
            ...
            <button onClick={handleSend} disabled={addComment.isPending}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 14px', fontSize: 12, fontWeight: 600, color: 'white', background: '#2563EB', border: 'none', borderRadius: 4, cursor: 'pointer' }}>
