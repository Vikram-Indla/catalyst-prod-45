/**
 * WorkHubRow — Single table row (36px height) — Stage D: fully wired
 * Inline edit, hover actions, status dropdown, priority dropdown, delete
 */
import { useState, useRef, useEffect } from 'react';
import { MoreHorizontal, Link2, Pencil, Trash2, Copy } from 'lucide-react';
...
// Status groups for the dropdown
const STATUS_GROUPS = [
  { label: 'TO DO', statuses: ['Backlog', 'In Requirements', 'To Do', 'Open'] },
  { label: 'IN PROGRESS', statuses: ['In Progress', 'In Review', 'In Development', 'In Beta', 'In UAT', 'In QA', 'Ready for QA'] },
  { label: 'DONE', statuses: ['Done', 'Closed', 'In Production', 'Released'] },
];
...
function highlightMatch(text: string, query: string | undefined): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark style={{ background: '#FEF3C7', padding: 0 }}>{text.slice(idx, idx + query.length)}</mark>
      {text.slice(idx + query.length)}
    </>
  );
}

export default function WorkHubRow({ item, columns, selected, onSelect, onOpenPanel, onInlineEdit, onDelete, allStatuses, allPriorities }: WorkHubRowProps) {
  ...
  // Points inline edit
  const commitPoints = () => {
    const val = editValue ? parseInt(editValue, 10) : null;
    onInlineEdit(item.id, 'story_points', val);
    setEditField(null);
  };
  ...
  const renderCell = (col: ColumnConfig) => {
    switch (col.id) {
          ...
          <Popover>
            ...
            <PopoverContent align="start" style={{ width: 220, padding: '4px 0', background: '#FFFFFF', border: '1px solid rgba(15,23,42,0.12)', borderRadius: 6, zIndex: 9999, maxHeight: 320, overflowY: 'auto' }}>
              {STATUS_GROUPS.map(group => {
                const availableStatuses = group.statuses.filter(s => !allStatuses || allStatuses.length === 0 || allStatuses.includes(s) || group.statuses.includes(s));
                return (
                  <div key={group.label}>
                    ...
                    {group.statuses.map(s => (
                      <button key={s} onClick={() => {
                        onInlineEdit(item.id, 'status', s);
                        // Also update status_category
                        onInlineEdit(item.id, 'status_category', deriveStatusCategory(s));
                      }} style={{
                        ...
                        <WorkHubStatusLozenge status={s} statusCategory={group.label === 'TO DO' ? 'To Do' : group.label === 'IN PROGRESS' ? 'In Progress' : 'Done'} />
                      </button>
                    ))}
                  </div>
                );
      ...
      case 'priority':
        return (
          <Popover>
            <PopoverTrigger asChild>
              <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                <WorkHubPriorityIcon priority={item.priority} size={16} />
              </button>
            </PopoverTrigger>
      ...
      case 'points':
        if (editField === 'points') {
          return (
            <input
              ...
              style={{
                width: 50, height: 28, border: '1.5px solid #2563EB', borderRadius: 3,
                padding: '0 4px', fontSize: 13, outline: 'none', fontFamily: "'JetBrains Mono', monospace",
                color: '#0F172A', background: 'white', textAlign: 'center',
              }}
            />
          );
        }
        return (
          <span
            onClick={e => { e.stopPropagation(); startEdit('points', String(item.story_points ?? '')); }}
  ...
  return (
    <>
      ...
      >
        ...
        >
          <button onClick={e => { e.stopPropagation(); startEdit('summary', item.summary); }} title="Edit"
      ...
      {contextMenu && (
        <div style={{
          ...
          <button onClick={() => { onOpenPanel(item.id); setContextMenu(null); }}
          ...
          <button onClick={() => { navigator.clipboard.writeText(item.issue_key); setContextMenu(null); }}
          ...
          {onDelete && (
            <button onClick={() => { onDelete(item.id); setContextMenu(null); }}
