...
export default function WorkHubSidePanelActivity({ activities, onAddComment, isLoading }: WorkHubSidePanelActivityProps) {
  ...
  const renderAction = (entry: WorkHubActivityEntry) => {
    switch (entry.action) {
      case 'status_changed':
        return <span style={{ fontSize: 13, color: '#64748B' }}>Changed status from <strong style={{ fontWeight: 650, color: '#0F172A' }}>{entry.old_value}</strong> to <strong style={{ fontWeight: 650, color: '#0F172A' }}>{entry.new_value}</strong></span>;
      case 'assigned':
        return <span style={{ fontSize: 13, color: '#64748B' }}>Assigned to <strong style={{ fontWeight: 650, color: '#0F172A' }}>{entry.new_value}</strong></span>;
      case 'commented':
        return <div style={{ background: '#F8FAFC', borderRadius: 6, padding: 12, fontSize: 13, color: '#0F172A', marginTop: 4, lineHeight: 1.5 }}>{entry.comment_text}</div>;
      case 'created':
      ...
      default:
        return <span style={{ fontSize: 13, color: '#64748B' }}>Updated <strong style={{ fontWeight: 650, color: '#0F172A' }}>{entry.field_changed}</strong></span>;
    }
  };
    ...
    <div>
      <div style={{ maxHeight: 300, overflowY: 'auto' }}>
        ...
        {!isLoading && activities.length === 0 && (
          <div style={{ padding: 16, fontSize: 13, color: '#94A3B8' }}>No activity recorded</div>
        )}
      ...
      <div style={{ borderTop: '0.75px solid rgba(15,23,42,0.06)', paddingTop: 12, marginTop: 8 }}>
        <textarea
          ...
          style={{
            ...
            resize: 'vertical', outline: 'none', minHeight: 48, background: 'white',
        ...
        {comment.trim() && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 8, marginTop: 6 }}>
            <span style={{ fontSize: 11, color: '#94A3B8' }}>⌘↵ to send</span>
            <button onClick={handleSubmit} style={{ padding: '4px 14px', fontSize: 12, fontWeight: 600, color: 'white', background: '#2563EB', border: 'none', borderRadius: 4, cursor: 'pointer' }}>Send</button>
          </div>
        )}
      </div>
    </div>
