...
export function CatalystProfileCard({
  ...
  const content = (
    <div style={{ display: 'flex', gap: 12, alignItems: 'center', padding: 4 }}>
      ...
      <div>
        <div style={{ fontSize: 14, fontWeight: 600, color: token('color.text', '#172B4D') }}>{name}</div>
        {role && <div style={{ fontSize: 12, color: token('color.text.subtle', '#42526E') }}>{role}</div>}
        {email && <div style={{ fontSize: 12, color: token('color.text.subtlest', '#6B778C') }}>{email}</div>}
      </div>
    </div>
  );
