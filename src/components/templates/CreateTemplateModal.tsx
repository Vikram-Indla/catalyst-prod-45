    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto bg-white">
        ...
        <div className="flex items-center gap-1 py-2">
          {STEPS.map((step, i) => (
            <div key={step.key} className={`flex-1 h-1 rounded-full ${i <= currentIndex ? 'bg-[#2563eb]' : 'bg-slate-200'}`} />
          ))}
        </div>
        ...
        <DialogFooter className="flex justify-between">
          ...
          <div className="flex gap-2">
            ...
            {isLast ? (
              <Button onClick={handleCreate} disabled={!state.basicInfo.name || createTemplate.isPending} className="bg-[#2563eb] hover:bg-[#1d4ed8] text-white">
                {createTemplate.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Check className="w-4 h-4 mr-1" />}
                Create Template
              </Button>
            ) : (
              <Button onClick={goNext} className="bg-[#2563eb] hover:bg-[#1d4ed8] text-white">
