    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 16 }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <div
          ...
          style={{
            background: isDark ? 'rgba(255,255,255,0.04)' : 'var(--cp-bd-zone)',
...
export function GoalsStatsStrip({ goals, keyResults, themes, isDark = false }: GoalsStatsStripProps) {
  ...
  const cards = [
    ...
    { label: 'Avg Progress', value: `${avgProgress}%`, icon: Activity, iconBg: isDark ? 'rgba(217,119,6,0.12)' : 'rgba(251,191,36,0.10)', iconColor: 'var(--sem-warning)', sub: `${100 - avgProgress}% to target` },
    ...
    { label: 'Overdue KRs', value: overdueKRs, icon: AlertTriangle, iconBg: isDark ? (overdueKRs > 0 ? 'rgba(239,68,68,0.12)' : 'rgba(255,255,255,0.04)') : (overdueKRs > 0 ? 'rgba(248,113,113,0.10)' : 'var(--cp-bd-zone)'), iconColor: overdueKRs > 0 ? 'var(--sem-danger)' : 'var(--fg-4)', sub: overdueKRs > 0 ? 'needs attention' : 'all on schedule' },
  ];
