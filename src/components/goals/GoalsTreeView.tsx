...
function statusBadge(status: string, isDark = false) {
  const map: Record<string, { dot: string; bg: string; text: string; bgDark: string; textDark: string; label: string }> = {
    active:      { dot: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: 'rgba(22,163,74,0.08)',  text: '#15803D', bgDark: '#182820', textDark: '#86EFAC', label: 'Active' },
    on_track:    { dot: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: 'rgba(22,163,74,0.08)',  text: '#15803D', bgDark: '#182820', textDark: '#86EFAC', label: 'On Track' },
    ...
    at_risk:     { dot: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', bg: 'rgba(217,119,6,0.08)',  text: '#B45309', bgDark: '#2A2418', textDark: '#FBBF24', label: 'At Risk' },
    off_track:   { dot: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))', bg: 'rgba(239,68,68,0.08)',  text: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', bgDark: '#2A1C1E', textDark: 'var(--ds-border-danger, var(--ds-border-danger, #FCA5A5))', label: 'Off Track' },
    draft:       { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))',               text: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', bgDark: 'var(--ds-surface-raised, var(--ds-surface-raised, #1A1A1A))', textDark: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))', label: 'Draft' },
    not_started: { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))',               text: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', bgDark: 'var(--ds-surface-raised, var(--ds-surface-raised, #1A1A1A))', textDark: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))', label: 'Not Started' },
    cancelled:   { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))',               text: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', bgDark: 'var(--ds-surface-raised, var(--ds-surface-raised, #1A1A1A))', textDark: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))', label: 'Cancelled' },
  };
...
function progressBar(pct: number, height = 6, isDark = false) {
  ...
  return (
    <div style={{ width: 80, height, background: isDark ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--divider)', borderRadius: 4, overflow: 'hidden' }}>
      ...
      {[1, 2, 3, 4, 5].map(i => (
        <div
          ...
          style={{
            ...
            background: i <= level
              ? (level >= 4 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : level >= 3 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))')
              : (isDark ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--divider)'),
          }}
        />
      ))}
    </div>
...
function OwnerAvatar({ name, size = 28, isDark = false }: { name?: string; size?: number; isDark?: boolean }) {
  if (!name) return <span style={{ fontSize: 11, color: isDark ? DK.t4 : 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))' }}>—</span>;
...
export function GoalsEmptyState({ onCreateGoal, isDark = false }: { onCreateGoal: () => void; isDark?: boolean }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', border: `1px solid ${isDark ? DK.border : 'var(--divider)'}`, borderRadius: 12, background: isDark ? 'transparent' : 'var(--bg-app)' }}>
      <div style={{ width: 52, height: 52, borderRadius: 14, background: isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--cp-bd-zone)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
      ...
      <button onClick={onCreateGoal} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 18px', fontSize: 13, fontWeight: 600, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', background: 'var(--cp-blue)', border: 'none', borderRadius: 6, cursor: 'pointer' }}>
        + Create Goal
      </button>
    </div>
  );
...
function GoalsNoSearchResults({ query, onClear, isDark = false }: { query: string; onClear: () => void; isDark?: boolean }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 20px', border: `1px solid ${isDark ? DK.border : 'var(--divider)'}`, borderRadius: 12, background: isDark ? 'transparent' : 'var(--bg-app)' }}>
      ...
      <button onClick={onClear} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '6px 14px', fontSize: 12, fontWeight: 500, color: isDark ? DK.t2 : 'var(--fg-3)', background: isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--cp-bd-zone)', border: `1px solid ${isDark ? DK.border : 'var(--divider)'}`, borderRadius: 6, cursor: 'pointer' }}>
...
export function GoalsTreeView({
  ...
  const themeRowBg = isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--cp-bd-zone)';
  const themeRowHover = 'var(--cp-bg-sunken, #E2E8F0)';
  const goalRowBg = isDark ? 'transparent' : 'var(--bg-app)';
  const goalRowHover = isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--bg-1)';
  const krRowBg = isDark ? '#111111' : 'var(--bg-1)';
  const krRowHover = isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--cp-bd-zone)';
  const connectorColor = isDark ? 'var(--ds-border, var(--ds-border, #2E2E2E))' : 'var(--divider)';
  ...
  return (
    <div className="goals-tree-container" style={{ border: `1px solid ${tableBorder}`, borderRadius: 12, overflow: 'hidden', background: containerBg }}>
      ...
      {filteredThemes.map(theme => {
        ...
        const progressColor = avgProgress >= 60 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : avgProgress >= 40 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))';
        ...
        return (
          <div key={theme.id}>
            ...
            {themeExpanded && themeGoals.map((goal, gi) => {
              ...
              const goalProgressColor = goalPct >= 60 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : goalPct >= 40 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))';
                ...
                <div key={goal.id}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                      ...
                      />
                      <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? DK.t2 : 'var(--fg-2)', background: isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--cp-bd-zone)', padding: '2px 8px', borderRadius: 4, flexShrink: 0, fontFamily: 'ui-monospace, monospace' }}>
                        {goal.goal_key}
                      </span>
                    ...
                    <div>
                      {goal.ai_health_score != null ? (
                        ...
                        <span style={{ fontSize: 11, color: isDark ? DK.t4 : 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))' }}>—</span>
                      )}
                    </div>
                  </div>
                  ...
                  {goalExpanded && goalKRs.map((kr, ki) => {
                    ...
                    const krProgressColor = krProgress >= 60 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : krProgress >= 40 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))';
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          color: isDark ? DK.t2 : 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))',
                        }}
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                          <span style={{ fontSize: 10, fontWeight: 600, color: isDark ? DK.t3 : 'var(--fg-4)', background: isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--cp-bd-zone)', padding: '1px 5px', borderRadius: 4, flexShrink: 0, fontFamily: 'ui-monospace, monospace' }}>
                            {kr.kr_key}
                          </span>
                          ...
                          <button
                            ...
                            style={{
                              ...
                              color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', background: 'transparent',
