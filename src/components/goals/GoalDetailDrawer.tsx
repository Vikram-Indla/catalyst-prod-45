...
function statusBadge(status: string) {
  const map: Record<string, { dot: string; bg: string; text: string; label: string }> = {
    active:      { dot: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: 'rgba(22,163,74,0.08)',  text: '#15803D', label: 'Active' },
    on_track:    { dot: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: 'rgba(22,163,74,0.08)',  text: '#15803D', label: 'On Track' },
    ...
    at_risk:     { dot: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', bg: 'rgba(217,119,6,0.08)',  text: '#B45309', label: 'At Risk' },
    off_track:   { dot: 'var(--sem-danger)', bg: 'rgba(239,68,68,0.08)',  text: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', label: 'Off Track' },
    draft:       { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--cp-bd-zone)',               text: 'var(--fg-3)', label: 'Draft' },
    not_started: { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--cp-bd-zone)',               text: 'var(--fg-3)', label: 'Not Started' },
    cancelled:   { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--cp-bd-zone)',               text: 'var(--fg-3)', label: 'Cancelled' },
  };
...
function progressBar(pct: number, height = 8) {
  const color = pct >= 60 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : pct >= 40 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--sem-danger)';
  return (
...
export function GoalDetailDrawer({ goalId, isOpen, onClose, onCheckinClick }: GoalDetailDrawerProps) {
  ...
  const confColor = confPct >= 60 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : confPct >= 40 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--sem-danger)';
  ...
  const statusDotColor = goal ? ({
    active: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', on_track: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', completed: '#4F46E5',
    at_risk: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', off_track: 'var(--sem-danger)', draft: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))',
  }[goal.status] || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))') : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))';
        ...
        <div style={{
          ...
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              ...
              color: activeTab === tab ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : 'var(--fg-3)',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 8, borderTop: '1px solid var(--divider)' }}>
        ...
        <button
          ...
          disabled={isPending}
          style={{ padding: '7px 16px', fontSize: 13, fontWeight: 600, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', background: isPending ? '#93C5FD' : 'var(--cp-blue)', border: 'none', borderRadius: 6, cursor: isPending ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}
        >
...
function OverviewTab({ goal, theme, krs, confPct, confColor, daysToDeadline }: {
  ...
  const pctColor = pct >= 60 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : pct >= 40 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--sem-danger)';
    ...
    <div>
      ...
      <div style={{ background: '#DBEAFE', border: '1px solid rgba(37,99,235,0.15)', borderRadius: 12, padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
          <Sparkles size={14} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" />
  ...
  if (krs.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '48px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}><BarChart3 size={36} color="var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))" /></div>
        ...
        return (
          <div key={kr.id} className="kr-detail-card" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              {onCheckinClick && (
                <button onClick={() => onCheckinClick(kr.id)} style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 600, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', background: 'var(--cp-blue)', border: 'none', borderRadius: 6, padding: '4px 12px', cursor: 'pointer' }}>Check-in</button>
              )}
            </div>
          </div>
        );
      ...
      {links.length === 0 && !showSearch && (
        <div style={{ textAlign: 'center', padding: '48px 20px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}>
            <Rocket size={36} color="var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))" />
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {links.map(link => (
            <div key={link.id} className="init-card" style={{
              ...
              <Link2 size={16} color="var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))" />
...
function CheckinsTab({ checkins, krs }: { checkins: KRCheckin[]; krs: KeyResult[] }) {
  ...
  if (checkins.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '48px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}><Clock size={36} color="var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))" /></div>
...
function ActivityTab({ goal, krs, checkins }: { goal: Goal; krs: KeyResult[]; checkins: KRCheckin[] }) {
  const items = [
    { icon: <Activity size={12} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" />, user: 'System', text: 'Goal created', date: goal.created_at || goal.start_date },
    ...(krs.length > 0 ? [{ icon: <BarChart3 size={12} color="#0D9488" />, user: 'System', text: `${krs.length} Key Results added`, date: goal.created_at }] : []),
    ...(checkins.length > 0 ? [{ icon: <Clock size={12} color="var(--ds-text-warning, var(--ds-text-warning, #D97706))" />, user: 'Team', text: `${checkins.length} check-ins recorded`, date: checkins[0]?.created_at }] : []),
  ];
  return (
