...
export function GoalsListView({ goals, themes, onGoalClick, isDark = false }: GoalsListViewProps) {
  ...
  const rowHover = isDark ? 'rgba(248,244,240,0.03)' : 'var(--bg-1)';
    ...
    <div style={{ border: `1px solid ${tableBorder}`, borderRadius: 10, overflow: 'hidden', background: isDark ? 'transparent' : 'var(--bg-app)' }}>
      ...
      {sortedGoals.map(goal => {
        ...
        return (
          ...
          >
            <span style={{ fontSize: 12, fontWeight: 600, color: isDark ? DK.t2 : 'var(--fg-2)', background: isDark ? 'rgba(248,244,240,0.04)' : 'var(--cp-bd-zone)', padding: '2px 8px', borderRadius: 4, justifySelf: 'start', fontFamily: 'ui-monospace, monospace' }}>{goal.goal_key}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} style={{ width: 60, height: 5, background: isDark ? 'rgba(248,244,240,0.08)' : 'var(--divider)', borderRadius: 3, overflow: 'hidden' }}>
