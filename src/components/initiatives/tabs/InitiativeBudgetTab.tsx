  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-zinc-900 dark:text-[#F5F3F0]">Budget</h3>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg p-3">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-2 h-2 rounded-full bg-blue-500" /><span className="text-xs font-semibold text-zinc-700 dark:text-[#F5F3F0]">CapEx</span>
            ...
            <div className="border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg p-3">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-2 h-2 rounded-full bg-teal-500" /><span className="text-xs font-semibold text-zinc-700 dark:text-[#F5F3F0]">OpEx</span>
      ...
      {/* Inline Add Form */}
      {showAddBudget && (
        <div className="border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg p-5 bg-zinc-50 dark:bg-[#1A1714] space-y-4 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowCatDropdown(v => !v)}
                className="w-full flex items-center justify-between border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg px-3 py-2 text-sm text-zinc-700 dark:text-[#F5F3F0] bg-white dark:bg-[#232019] hover:bg-zinc-50 dark:hover:bg-[#2C2823]">
                <span className="capitalize">{budgetForm.category}</span><ChevronDown className="w-4 h-4 text-zinc-400" />
              </button>
              {showCatDropdown && (
                <div className="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-[#2C2823] border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg shadow-lg z-50 py-1 max-h-48 overflow-y-auto">
            ...
            <div>
              ...
              <div className="flex rounded-lg border border-zinc-200 overflow-hidden">
                <button type="button" onClick={() => setBudgetForm(f => ({ ...f, expense_type: 'capex' }))}
                  className={`px-4 py-2 text-xs font-medium transition-colors ${budgetForm.expense_type === 'capex' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-[#232019] text-zinc-600 dark:text-[#A09890] hover:bg-zinc-50 dark:hover:bg-[#2C2823]'}`}>CapEx</button>
                <button type="button" onClick={() => setBudgetForm(f => ({ ...f, expense_type: 'opex' }))}
                  className={`px-4 py-2 text-xs font-medium transition-colors ${budgetForm.expense_type === 'opex' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-[#232019] text-zinc-600 dark:text-[#A09890] hover:bg-zinc-50 dark:hover:bg-[#2C2823]'}`}>OpEx</button>
              </div>
            </div>
          </div>
          <div>
            ...
            <textarea value={budgetForm.description} onChange={e => setBudgetForm(f => ({ ...f, description: e.target.value }))}
              rows={2} placeholder="What does this cover..." className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-[#232019] dark:border-[rgba(255,255,255,0.08)] dark:text-[#F5F3F0] resize-y" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" value={budgetForm.planned_amount} onChange={e => setBudgetForm(f => ({ ...f, planned_amount: e.target.value }))}
                placeholder="Enter amount" className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-[#232019] dark:border-[rgba(255,255,255,0.08)] dark:text-[#F5F3F0]" />
            </div>
            <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={budgetForm.vendor} onChange={e => setBudgetForm(f => ({ ...f, vendor: e.target.value }))}
                placeholder="Vendor name" className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-[#232019] dark:border-[rgba(255,255,255,0.08)] dark:text-[#F5F3F0]" />
            </div>
            <div>
              ...
              <input value={budgetForm.po_number} onChange={e => setBudgetForm(f => ({ ...f, po_number: e.target.value }))}
                placeholder="PO-12345" className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-[#232019] dark:border-[rgba(255,255,255,0.08)] dark:text-[#F5F3F0]" />
            </div>
          </div>
      ...
      {budgetItems.length === 0 && !showAddBudget ? (
        ...
        <div className="space-y-2">
          {budgetItems.map((item: any) => {
            ...
            return (
              <div key={item.id} className="bg-white dark:bg-[#232019] border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg p-4 hover:border-zinc-300 transition-colors">
                ...
                <p className="text-sm font-medium text-zinc-900 dark:text-[#F5F3F0] mb-2">{item.description}</p>
