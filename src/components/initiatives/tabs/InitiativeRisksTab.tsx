/**
 * InitiativeRisksTab — Risk register with 5×5 matrix, severity badges, risk cards, and add drawer.
 */
...
export function InitiativeRisksTab({ initiativeId }: InitiativeRisksTabProps) {
  ...
  const { data: risks = [], refetch: refetchRisks } = useQuery({
    queryKey: ['initiative-risks', initiativeId],
    queryFn: async () => {
  ...
  const handleCreateRisk = async () => {
    ...
    const { error } = await supabase.from('ph_initiative_risks').insert({
    ...
    <div>
      ...
      {risks.length === 0 ? (
        ...
        <div className="space-y-3">
          {risks.map((risk: any) => {
            ...
            return (
              <div key={risk.id} className={`bg-white dark:bg-[#232019] border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] border-l-4 ${sev.border} rounded-lg p-4`}>
      ...
      {showAddRisk && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-[420px] max-w-[90vw] z-[301] bg-white dark:bg-[#232019] shadow-xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowCatDropdown(v => !v)}
                  className="w-full flex items-center justify-between border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg px-3 py-2.5 text-sm text-zinc-700 bg-white dark:bg-[#232019] hover:bg-zinc-50 dark:hover:bg-[#1A1714]">
                  <span className="capitalize">{riskForm.category}</span>
                  <ChevronDown className="w-4 h-4 text-zinc-400" />
                </button>
                {showCatDropdown && (
                  <div className="absolute left-0 right-0 top-full mt-1 bg-white dark:bg-[#232019] border border-zinc-200 dark:border-[rgba(255,255,255,0.08)] rounded-lg shadow-lg z-50 py-1 max-h-48 overflow-y-auto">
                    {CATEGORIES.map(cat => (
