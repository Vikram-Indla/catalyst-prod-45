...
function getSyncStatus(lastSyncAt: string | null): { color: string; label: string; tooltip: string } {
  if (!lastSyncAt) return {
    color: 'bg-slate-300 dark:bg-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]',
    label: 'Not synced',
        ...
        return (
          <div
            ...
            className={cn(
              ...
              "dark:bg-[var(--ds-surface,var(--ds-surface, #0A0A0A))] dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] dark:hover:border-[var(--ds-border-bold,var(--ds-border-bold, #454545))]"
            )}
          >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="font-semibold text-[15px] leading-tight text-slate-900 dark:text-[var(--ds-text,var(--ds-text, #EDEDED))] truncate">
                    {p.name}
                  </span>
                  <span className="font-mono text-[11px] bg-slate-100 dark:bg-[var(--ds-border,var(--ds-border, #2E2E2E))] text-slate-500 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))] px-1.5 py-0.5 rounded tracking-wide flex-shrink-0">
                    {p.project_key}
                  </span>
                </div>
                <div className="text-[12px] text-slate-500 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))] mt-0.5">
                  {p.department || '—'}
                </div>
              </div>
              <button
              ...
              >
                <Star
                  size={14}
                  fill={isFav ? 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' : 'none'}
                  color={isFav ? 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' : undefined}
                  className={isFav ? 'text-amber-400' : 'text-slate-300 hover:text-amber-300 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]'}
                />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              {p.lead_name ? (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-slate-600 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]">
                    {p.lead_name.split(' ').slice(0, 2).join(' ')}
                  </span>
                </div>
              ) : (
                <span className="text-xs text-slate-400 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))]">—</span>
              )}
            </div>
              ...
              {[
                ...
                <div
                  ...
                  className={cn(
                    ...
                    "bg-slate-50",
                    "dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] dark:border dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))]"
                  )}
                >
                  <div className="text-[22px] font-semibold text-slate-900 dark:text-[var(--ds-text,var(--ds-text, #EDEDED))]">{s.v || 0}</div>
                  <div className="text-[10px] font-medium tracking-widest uppercase text-slate-400 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))] mt-1">{s.l}</div>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100 dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))] text-xs text-slate-500 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]">
              <Tooltip content={sync.tooltip} position="bottom">
                <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 dark:bg-[var(--ds-border,var(--ds-border, #2E2E2E))] text-[11px] font-medium cursor-default">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-slate-400 dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #A1A1A1))]">
