        ...
        <div className="grid grid-cols-3 gap-3">
          {FORMATS.map(f => (
            <button
              ...
              className="flex flex-col items-center gap-2 rounded-lg transition-all hover:border-blue-300 hover:bg-blue-50 dark:hover:bg-[var(--ds-surface-overlay,#1F1F1F)] bg-[var(--ds-surface-sunken,#F8FAFC)] dark:bg-[var(--ds-surface-overlay,#1F1F1F)] border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))]"
              style={{ padding: '20px 12px', cursor: 'pointer' }}
            >
              <f.icon size={24} color="var(--cp-blue)" />
              <span className="text-[var(--cp-ink-2, var(--cp-ink-2, var(--cp-ink-2, #334155)))] dark:text-[var(--ds-text-subtlest,#A1A1A1)]" style={{ fontSize: 12, fontWeight: 600 }}>{f.label}</span>
            </button>
          ))}
        </div>
      </DialogContent>
