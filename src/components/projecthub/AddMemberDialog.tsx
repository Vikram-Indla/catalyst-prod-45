...
export function AddMemberDialog({ open, onClose, projectId, existingMemberIds }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-[520px] p-0" style={{ fontFamily: 'var(--ds-font-family-body)' }}>
        <DialogHeader className="px-5 pt-5 pb-0">
          <DialogTitle style={{ fontFamily: 'var(--ds-font-family-heading)', fontSize: 16 }}>Add Team Member</DialogTitle>
        </DialogHeader>

        <div className="px-5 pb-2">
          <p className="text-[#64748B] dark:text-[#878787]" style={{ fontSize: 12, marginBottom: 12 }}>
            Select users from the organization directory to add to this project.
          </p>

          <div className="flex items-center gap-2 rounded-lg bg-white dark:bg-transparent border border-[#CBD5E1] dark:border-[#2E2E2E]" style={{ height: 40, padding: '0 14px' }}>
            <Search size={15} className="text-[#94A3B8] dark:text-[#7D7D7D]" />
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search by name, role, or email..."
              className="flex-1 bg-transparent outline-none text-[#0F172A] dark:text-[#EDEDED] placeholder:text-[#94A3B8] dark:placeholder:text-[#7D7D7D]"
              style={{ fontSize: 13 }}
              autoFocus
            />
          </div>
        ...
        <div className="border-t border-[var(--bd-default, #E2E8F0)] dark:border-[#2E2E2E]" style={{ maxHeight: 380, overflowY: 'auto' }}>
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              <Loader2 size={20} className="animate-spin text-slate-400 dark:text-[#7D7D7D]" />
            </div>
          ) : available.length === 0 ? (
            <div className="text-center py-10 text-[#94A3B8] dark:text-[#7D7D7D]" style={{ fontSize: 13 }}>
            ...
            <div>
              {available.map(u => (
                <div
                  key={u.id}
                  className="flex items-center gap-3 px-5 py-2.5 transition-colors hover:bg-slate-50 dark:hover:bg-[#1F1F1F] cursor-pointer group border-b border-[#F1F5F9] dark:border-[#2E2E2E]"
                  onClick={() => handleAdd(u)}
                >
                  {u.avatar_url ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[#0F172A] dark:text-[#EDEDED]" style={{ fontSize: 13, fontWeight: 600, lineHeight: '18px' }}>{u.name}</div>
                    <div className="text-[#64748B] dark:text-[#878787]" style={{ fontSize: 11, lineHeight: '16px' }}>
                      {u.role_name || 'No role'}
                  ...
                  {u.email && (
                    <span className="flex-shrink-0 text-[#94A3B8] dark:text-[#7D7D7D]" style={{ fontSize: 11 }}>{u.email.split('@')[0]}</span>
                  )}
        ...
        <div className="px-5 py-3 flex items-center justify-between border-t border-[var(--bd-default, #E2E8F0)] dark:border-[#2E2E2E]">
          <span className="text-[#94A3B8] dark:text-[#7D7D7D]" style={{ fontSize: 11 }}>
            {available.length} user{available.length !== 1 ? 's' : ''} available
          </span>
          <button
            onClick={onClose}
            className="rounded-md bg-white dark:bg-transparent border border-[var(--bd-default, #E2E8F0)] dark:border-[#2E2E2E] text-[#475569] dark:text-[#A1A1A1]"
