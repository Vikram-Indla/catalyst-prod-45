...
function FilterDropdown({ label, placeholder, options, selected, onToggle }: {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <div className="text-[#64748B] dark:text-[#878787]" style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>{label}</div>
      <button
        onClick={() => setOpen(!open)}
        className="bg-white dark:bg-transparent border-[1.5px] border-[#E2E8F0] dark:border-[#2E2E2E]"
        style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          width: '100%', height: 50, padding: '8px 12px',
          fontSize: 13, color: selected.length > 0 ? (isDark ? '#EDEDED' : 'var(--fg-1)') : (isDark ? '#7D7D7D' : 'var(--fg-4)'),
          borderRadius: 6, cursor: 'pointer', fontFamily: "'Inter', sans-serif",
        }}
      >
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{display}</span>
        <ChevronDown size={14} className="text-[#94A3B8] dark:text-[#7D7D7D]" style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 150ms', flexShrink: 0 }} />
      </button>
      {open && (
        <div className="bg-white dark:bg-[#1A1A1A] border border-[#E2E8F0] dark:border-[#2E2E2E]" style={{
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontSize: 13,
                  color: active ? '#2563EB' : (isDark ? '#A1A1A1' : 'var(--fg-2)'),
                  background: active ? (isDark ? 'rgba(59,130,246,0.10)' : 'var(--cp-blue-wash)') : 'transparent',
                  fontWeight: active ? 600 : 400,
                  border: 'none', cursor: 'pointer', textAlign: 'left',
                }}
                onMouseEnter={e => { if (!active) (e.target as HTMLElement).style.background = isDark ? '#1F1F1F' : 'var(--bg-1)'; }}
                onMouseLeave={e => { if (!active) (e.target as HTMLElement).style.background = 'transparent'; }}
              >
                <span style={{
                  width: 16, height: 16, borderRadius: 4, flexShrink: 0,
                  border: `1.5px solid ${active ? 'var(--cp-blue)' : (isDark ? '#454545' : '#CBD5E1')}`,
...
export function ProjectAdvancedFilters({ filters, onChange, departments, onClose }: AdvancedFiltersProps) {
  ...
  return (
    <div className="bg-white dark:bg-[#1A1A1A] border border-[#E2E8F0] dark:border-[#2E2E2E]" style={{ borderRadius: 8, padding: 16, marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <span className="text-[#0F172A] dark:text-[#EDEDED]" style={{ fontSize: 13, fontWeight: 600 }}>Advanced Filters</span>
