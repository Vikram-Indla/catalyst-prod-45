...
function FilterDropdown({ label, placeholder, options, selected, onToggle }: {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <div className="text-[#64748B] dark:text-[rgba(235,238,245,0.55)]" style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>{label}</div>
      <button
        onClick={() => setOpen(!open)}
        className="bg-white dark:bg-transparent border-[1.5px] border-[#E2E8F0] dark:border-[rgba(235,238,245,0.10)]"
        style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          width: '100%', height: 36, padding: '0 12px',
          fontSize: 13, color: selected.length > 0 ? (isDark ? 'rgba(235,238,245,0.92)' : 'var(--fg-1)') : (isDark ? 'rgba(235,238,245,0.40)' : 'var(--fg-4)'),
          borderRadius: 6, cursor: 'pointer', fontFamily: "'Inter', sans-serif",
        }}
      >
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{display}</span>
        <ChevronDown size={14} className="text-[#94A3B8] dark:text-[rgba(235,238,245,0.40)]" style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 150ms', flexShrink: 0 }} />
      </button>
      {open && (
        <div className="bg-white dark:bg-[#1F2128] border border-[#E2E8F0] dark:border-[rgba(235,238,245,0.10)]" style={{
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontSize: 13,
                  color: active ? '#2563EB' : (isDark ? 'rgba(235,238,245,0.72)' : 'var(--fg-2)'),
                  background: active ? (isDark ? 'rgba(59,130,246,0.10)' : 'var(--cp-blue-wash)') : 'transparent',
                  fontWeight: active ? 600 : 400,
                  border: 'none', cursor: 'pointer', textAlign: 'left',
                }}
                onMouseEnter={e => { if (!active) (e.target as HTMLElement).style.background = isDark ? 'rgba(235,238,245,0.03)' : 'var(--bg-1)'; }}
                onMouseLeave={e => { if (!active) (e.target as HTMLElement).style.background = 'transparent'; }}
              >
                <span style={{
                  width: 16, height: 16, borderRadius: 4, flexShrink: 0,
                  border: `1.5px solid ${active ? 'var(--cp-blue)' : (isDark ? 'rgba(235,238,245,0.20)' : '#CBD5E1')}`,
...
export function ProjectAdvancedFilters({ filters, onChange, departments, onClose }: AdvancedFiltersProps) {
  ...
  return (
    <div className="bg-white dark:bg-[#1F2128] border border-[#E2E8F0] dark:border-[rgba(235,238,245,0.10)]" style={{ borderRadius: 8, padding: 16, marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <span className="text-[#0F172A] dark:text-[rgba(235,238,245,0.92)]" style={{ fontSize: 13, fontWeight: 600 }}>Advanced Filters</span>
