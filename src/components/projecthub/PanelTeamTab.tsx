      ...
      <div className="flex items-center gap-2 mx-4 mt-3 mb-2">
        <div className="flex items-center gap-2 flex-1 rounded-lg bg-white dark:bg-transparent border border-[var(--ds-text-disabled,#CBD5E1)] dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))]" style={{ height: 38, padding: '8px 12px' }}>
          ...
          <input
            ...
            className="flex-1 bg-transparent p-0 m-0 appearance-none text-[var(--ds-text,var(--cp-ink-1, #0F172A))] dark:text-[var(--ds-text,var(--cp-bg-neutral, #EDEDED))] placeholder:text-[var(--ds-text-subtlest,var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))] dark:placeholder:text-[#7D7D7D]"
          ...
          {searchSuggestions.map(u => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[var(--ds-text,var(--cp-ink-1, #0F172A))] dark:text-[var(--ds-text,var(--cp-bg-neutral, #EDEDED))]" style={{ fontSize: 13, fontWeight: 600 }}>{u.name}</div>
          ...
          {grouped.map(g => (
            <div key={g.category} className="mt-4">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[var(--ds-text-subtlest,var(--cp-ink-3, var(--cp-text-secondary, #64748B)))] dark:text-[var(--ds-text-subtlest,#878787)]" style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{g.category}</span>
              ...
              <div className="space-y-0.5">
                {g.members.map(m => (
                  <div key={m.user_id} className="flex items-center gap-3 rounded-lg transition-colors hover:bg-slate-50 dark:hover:bg-[var(--ds-surface-overlay,#1F1F1F)] group" style={{ padding: '10px 10px' }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[var(--ds-text,var(--cp-ink-1, #0F172A))] dark:text-[var(--ds-text,var(--cp-bg-neutral, #EDEDED))]" style={{ fontSize: 14, fontWeight: 600, lineHeight: '20px' }}>{m.full_name}</div>
