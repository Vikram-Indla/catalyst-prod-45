      ...
      <div className="space-y-3 p-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="animate-pulse flex items-center gap-3">
            <div className="bg-slate-200 dark:bg-[rgba(248,244,240,0.08)]" style={{ width: 40, height: 40, borderRadius: '50%' }} />
            <div className="flex-1 space-y-1.5">
              <div className="h-3 bg-slate-200 dark:bg-[rgba(248,244,240,0.08)] rounded w-1/2" />
              <div className="h-2.5 bg-slate-100 dark:bg-[rgba(248,244,240,0.06)] rounded w-1/3" />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mx-4 mt-3 mb-2">
        <div className="flex items-center gap-2 flex-1 rounded-lg bg-white dark:bg-transparent border border-[#CBD5E1] dark:border-[rgba(248,244,240,0.10)]" style={{ height: 38, padding: '0 12px' }}>
          <Search size={14} className="shrink-0 text-[#94A3B8] dark:text-[rgba(248,244,240,0.40)]" />
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search by name, role, or email..."
            className="flex-1 bg-transparent p-0 m-0 appearance-none text-[#0F172A] dark:text-[rgba(248,244,240,0.92)] placeholder:text-[#94A3B8] dark:placeholder:text-[rgba(248,244,240,0.40)]"
      ...
      {searchSuggestions.length > 0 && (
        <div className="mx-4 mb-3 rounded-lg overflow-hidden border border-[#E2E8F0] dark:border-[rgba(248,244,240,0.10)] bg-white dark:bg-transparent">
          <div className="text-[#64748B] dark:text-[rgba(248,244,240,0.55)] bg-[#F8FAFC] dark:bg-transparent border-b border-[#F1F5F9] dark:border-[rgba(248,244,240,0.08)]" style={{ padding: '6px 12px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Add from directory
          </div>
          {searchSuggestions.map(u => (
            <div
              key={u.id}
              className="flex items-center gap-3 px-3 py-2 transition-colors hover:bg-slate-50 dark:hover:bg-[rgba(248,244,240,0.03)] cursor-pointer group border-b border-[#F8FAFC] dark:border-[rgba(248,244,240,0.06)]"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[#0F172A] dark:text-[rgba(248,244,240,0.92)]" style={{ fontSize: 13, fontWeight: 600 }}>{u.name}</div>
                <div className="text-[#64748B] dark:text-[rgba(248,244,240,0.55)]" style={{ fontSize: 11 }}>
                  {u.role_name || 'No role'}
                  {u.department_name && <span className="text-[#CBD5E1] dark:text-[rgba(248,244,240,0.20)]"> · </span>}
                  {u.department_name && <span className="text-[#94A3B8] dark:text-[rgba(248,244,240,0.40)]">{u.department_name}</span>}
                </div>
              </div>
              <button
      ...
      {grouped.length === 0 && searchSuggestions.length === 0 ? (
        <div className="text-center py-8 text-[#94A3B8] dark:text-[rgba(248,244,240,0.40)]" style={{ fontSize: 13 }}>
          ...
          {grouped.map(g => (
            <div key={g.category} className="mt-4">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[#64748B] dark:text-[rgba(248,244,240,0.55)]" style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{g.category}</span>
                ...
                <div className="flex-1 bg-[#E2E8F0] dark:bg-[rgba(248,244,240,0.08)]" style={{ height: 1 }} />
              </div>

              <div className="space-y-0.5">
                {g.members.map(m => (
                  <div key={m.user_id} className="flex items-center gap-3 rounded-lg transition-colors hover:bg-slate-50 dark:hover:bg-[rgba(248,244,240,0.03)] group" style={{ padding: '10px 10px' }}>
                    {m.avatar_url ? (
