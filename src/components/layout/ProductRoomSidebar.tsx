...
const menuItems = [{
  ...
  title: 'Roadmaps',
  path: '/industry/roadmaps',
  icon: Map,
  exact: false
}];
const deliveryPlatformOptions = [{
  value: 'Senaei Platform',
  label: 'Senaei Platform'
}, {
...
export function ProductRoomSidebar({
  expanded,
  onToggle,
  className
}: ProductRoomSidebarProps) {
  ...
  const handleDeliveryPlatformToggle = (value: string) => {
    const newPlatforms = deliveryPlatforms.includes(value) ? deliveryPlatforms.filter(p => p !== value) : [...deliveryPlatforms, value];
    setIndustryFilters({
      ...industryFilters,
      deliveryPlatforms: newPlatforms,
      processSteps,
      quarters
    });
  };
  const handleProcessStepToggle = (value: string) => {
    const newSteps = processSteps.includes(value) ? processSteps.filter(s => s !== value) : [...processSteps, value];
    setIndustryFilters({
      ...industryFilters,
      deliveryPlatforms,
  ...
  const handleQuarterToggle = (value: string) => {
    const newQuarters = quarters.includes(value) ? quarters.filter(q => q !== value) : [...quarters, value];
    setIndustryFilters({
      ...industryFilters,
      deliveryPlatforms,
      processSteps,
  ...
  return <TooltipProvider>
      <aside className={cn('h-full border-r bg-card transition-all duration-300 flex-shrink-0 relative flex flex-col', expanded ? 'w-64' : 'w-16', className)}>
        {/* Toggle Handle */}
        <button onClick={onToggle} className="absolute -right-3 top-6 z-50 w-6 h-6 rounded-full bg-card border shadow-sm flex items-center justify-center hover:bg-accent transition-transform" aria-label={expanded ? "Collapse sidebar" : "Expand sidebar"}>
        ...
        <div className="p-4 border-b flex items-center shrink-0">
          {expanded ? <div className="flex items-center gap-3">
            ...
            </div> : <div className="w-10 h-10 rounded-lg bg-brand-gold/20 flex items-center justify-center text-brand-gold font-semibold text-sm mx-auto">
              PR
            </div>}
        </div>
          ...
          {expanded && <div className="p-4 border-b space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Popover open={deliveryPlatformOpen} onOpenChange={setDeliveryPlatformOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" aria-expanded={deliveryPlatformOpen} className="w-full justify-between bg-background font-normal">
                  ...
                  <PopoverContent className="w-56 p-0 bg-popover border shadow-lg z-50" align="start">
                    <div className="max-h-60 overflow-auto">
                      {deliveryPlatformOptions.map(platform => <div key={platform.value} className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-muted/50" onClick={() => handleDeliveryPlatformToggle(platform.value)}>
                          <div className={cn("h-4 w-4 border rounded flex items-center justify-center", deliveryPlatforms.includes(platform.value) ? "bg-brand-gold border-brand-gold" : "border-border")}>
                            {deliveryPlatforms.includes(platform.value) && <Check className="h-3 w-3 text-white" />}
                          </div>
                          <span className="text-sm">{platform.label}</span>
                        </div>)}
                    </div>
                    {deliveryPlatforms.length > 0 && <div className="border-t p-2">
                        <Button variant="ghost" size="sm" className="w-full text-xs" onClick={() => setIndustryFilters({
                    ...industryFilters,
                    deliveryPlatforms: [],
                    processSteps,
                    quarters
              ...
              <div className="space-y-1.5">
                ...
                <Popover open={processStepOpen} onOpenChange={setProcessStepOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" aria-expanded={processStepOpen} className="w-full justify-between bg-background font-normal">
                  ...
                  <PopoverContent className="w-56 p-0 bg-popover border shadow-lg z-50" align="start">
                    <div className="max-h-60 overflow-auto">
                      {PROCESS_STEPS.map(step => <div key={step.value} className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-muted/50" onClick={() => handleProcessStepToggle(step.value)}>
                          <div className={cn("h-4 w-4 border rounded flex items-center justify-center", processSteps.includes(step.value) ? "bg-brand-gold border-brand-gold" : "border-border")}>
                            {processSteps.includes(step.value) && <Check className="h-3 w-3 text-white" />}
                          </div>
                          <span className="text-sm">{step.label}</span>
                        </div>)}
                    </div>
                    {processSteps.length > 0 && <div className="border-t p-2">
                        <Button variant="ghost" size="sm" className="w-full text-xs" onClick={() => setIndustryFilters({
                    ...industryFilters,
                    deliveryPlatforms,
                    processSteps: [],
              ...
              <div className="space-y-1.5">
                ...
                <Popover open={quarterOpen} onOpenChange={setQuarterOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" aria-expanded={quarterOpen} className="w-full justify-between bg-background font-normal">
                  ...
                  <PopoverContent className="w-56 p-0 bg-popover border shadow-lg z-50" align="start">
                    <div className="max-h-60 overflow-auto">
                      {quarterOptions.map(quarter => <div key={quarter.value} className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-muted/50" onClick={() => handleQuarterToggle(quarter.value)}>
                          <div className={cn("h-4 w-4 border rounded flex items-center justify-center", quarters.includes(quarter.value) ? "bg-brand-gold border-brand-gold" : "border-border")}>
                            {quarters.includes(quarter.value) && <Check className="h-3 w-3 text-white" />}
                          </div>
                          <span className="text-sm">{quarter.label}</span>
                        </div>)}
                    </div>
                    {quarters.length > 0 && <div className="border-t p-2">
                        <Button variant="ghost" size="sm" className="w-full text-xs" onClick={() => setIndustryFilters({
                    ...industryFilters,
                    deliveryPlatforms,
                    processSteps,
