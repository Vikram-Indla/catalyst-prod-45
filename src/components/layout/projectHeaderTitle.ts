...
/** Known route segment → display word. Segments not listed are title-cased. */
const ROUTE_WORD_MAP: Record<string, string> = {
  backlog: 'Backlog',
  board: 'Board',
  boards: 'Board',
  allwork: 'Work',
...
export function deriveRouteWord(pathname: string): string | null {
  ...
  return ROUTE_WORD_MAP[seg] ?? (seg.charAt(0).toUpperCase() + seg.slice(1));
}
