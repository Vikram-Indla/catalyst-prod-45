...
function ProjectTicketCountBadge({ projectKey }: { projectKey: string }) {
  ...
  return (
    <span style={{ background: '#DFE1E6', color: '#374151', fontSize: 11, fontWeight: 600, borderRadius: 4, padding: '2px 6px', fontFamily: 'var(--ds-font-family-body)' }}>
      {count ?? '…'}
    </span>
  );
}
      ...
      <Sheet open={open} onOpenChange={handleClose} modal={false}>
        ...
        <div style={{ padding: '0 24px', height: 64, borderBottom: '0.75px solid #E5E7EB', display: 'flex', flexDirection: 'column', justifyContent: 'center', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 16, fontWeight: 650, color: '#111827', fontFamily: 'var(--ds-font-family-heading)' }}>Import from Jira</span>
            <span style={{ flex: 1 }} />
            ...
            <span style={{ fontSize: 12, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>Step {step} of 2</span>
            <button onClick={handleClose} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: 'var(--fg-3)' }}>
              <X size={16} />
            </button>
          </div>
          <span style={{ fontSize: 13, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)', marginTop: 2 }}>Connected via Catalyst integration</span>
        </div>
        ...
        <div style={{ height: 64, borderTop: '0.75px solid #E5E7EB', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          {step === 1 ? (
            <>
              <button onClick={handleClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>Cancel</button>
              <button
                ...
                style={{
                  ...
                  fontFamily: 'var(--ds-font-family-body)', opacity: selectedProject ? 1 : 0.6,
                }}
              >Next: Select Tickets →</button>
            </>
          ) : (
            <>
              <button onClick={() => setStep(1)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>← Back</button>
              <button
                ...
                style={{
                  ...
                  fontFamily: 'var(--ds-font-family-body)', opacity: selectedTickets.length > 0 ? 1 : 0.6,
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--fg-3)', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'var(--ds-font-family-body)' }}>SELECT PROJECT</span>
        {projects.length > 0 && (
          <button onClick={onSyncAll} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>
      ...
      {loadingProjects ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 0' }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: '#374151', marginTop: 12, fontFamily: 'var(--ds-font-family-body)' }}>No projects connected yet</span>
          <span style={{ fontSize: 13, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>Add your first Jira project below</span>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div style={{
                  ...
                  background: getAvatarColor(p.project_key), color: 'var(--bg-app)', fontSize: 13, fontWeight: 700, fontFamily: 'var(--ds-font-family-body)',
                }}>
                  {p.project_key}
                </div>
                <span style={{ fontSize: 14, fontWeight: 600, color: '#111827', fontFamily: 'var(--ds-font-family-body)', flex: 1 }}>
      ...
      <div style={{ marginTop: 24, position: 'relative' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--fg-4)', textTransform: 'uppercase', fontFamily: 'var(--ds-font-family-body)', background: 'var(--bg-app)', padding: '0 8px' }}>ADD PROJECT</span>
          <div style={{ flex: 1, height: '0.75px', background: '#F3F4F6' }} />
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{
              ...
              padding: '8px 12px', fontSize: 13, fontFamily: 'var(--ds-font-family-monospaced)',
          ...
          <button
            ...
            style={{
              ...
              fontFamily: 'var(--ds-font-family-body)', display: 'flex', alignItems: 'center', gap: 6,
        ...
        <div style={{ marginTop: 8 }}>
          {verifyState === 'loading' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: 13, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>Verifying with Jira...</span>
            </div>
          )}
          {verifyState === 'success' && verifyResult && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <CheckCircle size={14} style={{ color: 'var(--sem-success)' }} />
              <span style={{ fontSize: 13, color: 'var(--sem-success)', fontFamily: 'var(--ds-font-family-body)' }}>{verifyResult.project_name} found · {verifyResult.count} tickets</span>
            </div>
          )}
          {verifyState === 'not_found' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <XCircle size={14} style={{ color: 'var(--sem-danger)' }} />
              <span style={{ fontSize: 13, color: 'var(--sem-danger)', fontFamily: 'var(--ds-font-family-body)' }}>Project not found. Check the key and try again.</span>
            </div>
          )}
          {verifyState === 'not_configured' && (
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', background: '#FEF3C7', border: '0.75px solid var(--sem-warning)', borderRadius: 6, padding: '10px 12px', marginTop: 4 }}>
              ...
              <div>
                <span style={{ fontSize: 13, color: '#92400E', fontFamily: 'var(--ds-font-family-body)' }}>Jira integration not configured. Go to Settings → Integrations. </span>
                <a href="/settings/integrations" style={{ fontSize: 13, color: 'var(--cp-blue)', textDecoration: 'underline', fontFamily: 'var(--ds-font-family-body)' }}>Go to Settings →</a>
              </div>
            </div>
          )}
        </div>
...
function Step2({
  ...
  return (
    <>
      <div style={{ fontSize: 13, color: 'var(--fg-3)', marginBottom: 4, fontFamily: 'var(--ds-font-family-body)' }}>
        {projectName} · Showing tickets with attachments only
      </div>
      <div style={{ fontSize: 12, color: 'var(--fg-3)', margin: '8px 0 4px 0', fontFamily: 'var(--ds-font-family-body)' }}>
        Showing {pdfIssueCount} of {totalIssueCount} issues — PDF attachments only
      </div>
      <div style={{ fontSize: 11, color: 'var(--fg-4)', marginBottom: 16, fontFamily: 'var(--ds-font-family-monospaced)' }}>
        {syncLabel}
      </div>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <input
            ...
            style={{
              ...
              padding: '0 12px 0 32px', fontSize: 13, fontFamily: 'var(--ds-font-family-body)',
              outline: 'none', color: '#111827',
            }}
          />
        </div>
      ...
      {loading || syncing ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 0' }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: '#111827', marginTop: 12, fontFamily: 'var(--ds-font-family-body)' }}>No tickets found</span>
          <span style={{ fontSize: 13, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)', textAlign: 'center', maxWidth: 280, marginTop: 4 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 13, fontWeight: 600, fontFamily: 'var(--ds-font-family-body)',
        ...
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', height: 50, background: '#F9FAFB', borderBottom: '0.75px solid #E5E7EB', position: 'sticky', top: 0, zIndex: 1 }}>
            ...
            {[
              ...
              <div key={col.label} style={{
                ...
                textTransform: 'uppercase', fontFamily: 'var(--ds-font-family-body)',
              }}>{col.label}</div>
            ))}
          </div>
          ...
          {tickets.map(t => {
            ...
            return (
              ...
              >
                ...
                <div style={{ width: 96, padding: '8px 12px', fontSize: 12, fontWeight: 500, color: muted ? 'var(--fg-4)' : '#374151', fontFamily: 'var(--ds-font-family-monospaced)' }}>{t.ticket_key}</div>
                <div dir="auto" style={{ flex: 1, padding: '8px 12px', fontSize: 13, color: muted ? 'var(--fg-4)' : '#111827', fontFamily: 'var(--ds-font-family-body)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.ticket_summary}</div>
                ...
                <div style={{ width: 100, padding: '4px 8px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                  {reActivated ? (
                    <span style={{ fontSize: 11, padding: '2px 6px', borderRadius: 4, background: '#0C66E4', color: 'var(--bg-app)', fontFamily: 'var(--ds-font-family-body)' }}>Re-import</span>
                  ) : imported ? (
                    <>
                      <span className="group-hover:hidden" style={{ fontSize: 11, padding: '2px 6px', borderRadius: 4, background: '#F3F4F6', color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>Imported</span>
                      <button
                        className="hidden group-hover:inline-flex"
                        onClick={(e) => { e.stopPropagation(); onReImport(t.ticket_key); }}
                        style={{ fontSize: 12, fontWeight: 500, color: 'var(--cp-blue)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--ds-font-family-body)', whiteSpace: 'nowrap' }}
          ...
          {/* Footer */}
          <div style={{ padding: '8px 0', marginTop: 8, fontSize: 13, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)' }}>
