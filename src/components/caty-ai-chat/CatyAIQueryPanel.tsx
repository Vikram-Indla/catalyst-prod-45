...
const CHART_COLORS = ['var(--ds-text-brand)', 'var(--ds-text-brand)', 'var(--ds-background-success-bold)', 'var(--ds-text-warning)', 'var(--ds-text-danger)', 'var(--ds-icon-information)'];
        ...
        <div className="space-y-4">
          ...
          {result.visualization && (
            <Card><CardContent className="p-4">
              {result.visualization.type === 'bar' && <div className="h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={result.visualization.data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><Tooltip /><Bar dataKey="value" fill="var(--ds-text-brand)" /></BarChart></ResponsiveContainer></div>}
