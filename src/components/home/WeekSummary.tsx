  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{
        ...
        {/* My closures */}
        <p style={{ fontSize: 13, color: '#5E6270', lineHeight: '20px', margin: '0 0 10px', fontFamily: F.inter }}>
          {summary.myClosed.total > 0
            ? <>You closed <strong style={{ color: '#1A1D23', fontWeight: 600 }}>{summary.myClosed.total} items</strong> this week — {myClosedText}.</>
        ...
        {summary.teamClosed.total > 0 && (
          <div style={{ marginBottom: 10 }}>
            <p style={{ fontSize: 13, color: '#5E6270', lineHeight: '20px', margin: 0, fontFamily: F.inter }}>
              Your team shipped <strong style={{ color: '#1A1D23', fontWeight: 600 }}>{summary.teamClosed.total} items</strong> across {projectEntries.length} project{projectEntries.length !== 1 ? 's' : ''}:
            </p>
            <div style={{ display: 'flex', gap: 12, marginTop: 6, flexWrap: 'wrap' }}>
              {projectEntries.map(([pk, count]) => (
                <span key={pk} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 500, color: '#5E6270', fontFamily: F.inter }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: PROJECT_COLORS[pk] || '#8B8FA3' }} />
                  {pk} ({count})
                </span>
              ))}
            </div>
        ...
        {summary.agingItems.length > 0 && (
          <div style={{ borderTop: '1px solid #ECEEF2', paddingTop: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: '#AD6800', fontFamily: F.inter }}>
            ...
            {summary.agingItems.map(item => (
              ...
              >
                <span style={{ fontFamily: F.mono, fontSize: 11, fontWeight: 600, color: '#4C6EF5', minWidth: 70 }}>{item.key}</span>
                <span style={{ fontSize: 12, color: '#5E6270', fontFamily: F.inter, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</span>
                <span style={{ fontFamily: F.mono, fontSize: 11, fontWeight: 600, color: '#AD6800', flexShrink: 0 }}>{item.days}d</span>
              </button>
            ))}
          </div>
        )}
