  ...
  return (
    <div className={`inline-flex flex-col items-center ${className}`}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <circle cx={center} cy={center} r={radius} fill="none" stroke="var(--exec-border, #E2E8F0)" strokeWidth={strokeWidth} />
        <circle
        ...
        {label && (
          <text x={center} y={center + size * 0.14} textAnchor="middle" dominantBaseline="central"
            style={{ fontSize: size * 0.11, fill: 'var(--exec-text-tertiary, #64748B)' }}>
            {label}
          </text>
        )}
      </svg>
