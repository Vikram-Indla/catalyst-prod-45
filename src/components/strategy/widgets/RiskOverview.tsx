...
export function RiskOverview() {
  ...
  const criticalHighCount = useMemo(() => openRisks.filter(r => r.critical_path === 'Yes' || r.impact === 'Critical' || r.impact === 'High').length, [openRisks]);
  const mitigatedCount = useMemo(() => openRisks.filter(r => r.resolution_method === 'Mitigated').length, [openRisks]);
  const overdueCount = useMemo(() => openRisks.filter(r => r.target_resolution_date && new Date(r.target_resolution_date) < new Date()).length, [openRisks]);
  ...
  if (openRisks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-2">
        <ShieldCheck size={32} style={{ color: 'var(--exec-blue-500)' }} />
        <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--exec-blue-500)' }}>No open risks</span>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-1.5 mb-3">
        {[
          ...
          <div key={kpi.label} className="text-center relative" style={{
            ...
            {kpi.pulse && (
              <span style={{
                position: 'absolute', top: 4, right: 4, width: 6, height: 6, borderRadius: '50%', background: '#DC2626',
                animation: 'pulse 2s infinite',
              }} />
            )}
            <div style={{ fontSize: 16, fontWeight: 700, color: kpi.color }}>{kpi.value}</div>
            <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--exec-text-tertiary)' }}>{kpi.label}</div>
          </div>
        ))}
      </div>

      {/* ROAM Bar */}
      <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--exec-text-primary)', marginBottom: 4 }}>ROAM Status</div>
      <div className="flex overflow-hidden mb-1" style={{ height: 12, borderRadius: 9999 }}>
        {roamTotal > 0 && (['Resolved', 'Owned', 'Accepted', 'Mitigated'] as const).map(method => {
          ...
          return <div key={method} style={{ width: `${(count / roamTotal) * 100}%`, background: ROAM_COLORS[method], transition: 'width 600ms ease-out' }} />;
        })}
      </div>
      ...
      {/* Impact Heatstrip */}
      <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--exec-text-primary)', marginBottom: 4 }}>By Impact</div>
      ...
      {/* Top Risks */}
      <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--exec-text-primary)', marginBottom: 4 }}>Top Risks</div>
      <div>
        {topRisks.map((risk, i) => {
          ...
          return (
            <div key={risk.id} className="flex items-start gap-2" style={{
              ...
              <div className="flex-1 min-w-0">
                ...
                <div style={{ fontSize: 9, color: 'var(--exec-text-secondary)' }}>
                  {risk.occurrence || '—'} occurrence · {risk.relationship || 'Enterprise'}
                  {isOverdue && <span style={{ color: 'var(--exec-signal-red, #DC2626)', fontWeight: 600, marginLeft: 4 }}>⚠ Overdue</span>}
                </div>
              </div>
