...
export function ContractOverview() {
  ...
  /* KPI colors: red for danger, amber for warning, blue for safe */
  const kpis = [
  ...
  const timelineSegments = [
    { label: 'Expired', count: expired.length, color: '#DC2626' },
    { label: '≤3mo', count: critical.length, color: 'rgba(220,38,38,0.7)' },
    { label: '3-6mo', count: warning.length, color: '#D97706' },
    { label: '>6mo', count: safe.length, color: '#3B82F6' },
  ];
  ...
  return (
    <TooltipProvider>
      <div>
            ...
            <div key={kpi.label} className="text-center relative" style={{
              ...
              {kpi.pulse && (
                <span className="contract-pulse-dot" style={{
                  position: 'absolute', top: 3, right: 3, width: 6, height: 6, borderRadius: '50%', background: '#DC2626',
                }} />
              )}
              <div style={{ fontSize: 16, fontWeight: 700, color: kpi.color }}>{kpi.value}</div>
              <div style={{ fontSize: 8, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--exec-text-tertiary)', fontWeight: 600 }}>
                {kpi.label}
              </div>
            </div>
          ))}
            ...
            return pct > 0 ? (
              <Tooltip key={seg.label}>
                <TooltipTrigger asChild>
                  <div style={{ width: `${pct}%`, background: seg.color, transition: 'width 600ms ease-out', minWidth: pct > 0 ? 2 : 0, cursor: 'help' }} />
                </TooltipTrigger>
        ...
        <div className="flex flex-wrap gap-2 mb-3" style={{ fontSize: 9, color: 'var(--exec-text-secondary)' }}>
          {timelineSegments.map(seg => (
        ...
        {/* C) By Department — red/amber/blue stacked bars */}
        <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--exec-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>
          By Department
        </div>
        <div className="space-y-1 mb-3">
          {deptContracts.map(([dept, counts]) => {
            ...
            return (
              <Tooltip key={dept}>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-2" style={{ transition: 'background 150ms', cursor: 'pointer' }}
                    ...
                    <div className="flex-1 flex overflow-hidden" style={{ height: 8, borderRadius: 4, background: 'var(--exec-bg-hover)' }}>
                      {counts.expired > 0 && <div style={{ width: `${(counts.expired / deptTotal) * 100}%`, height: '100%', background: '#DC2626', transition: 'width 600ms ease-out' }} />}
                      {counts.critical > 0 && <div style={{ width: `${(counts.critical / deptTotal) * 100}%`, height: '100%', background: 'rgba(220,38,38,0.7)', transition: 'width 600ms ease-out' }} />}
                      {counts.warning > 0 && <div style={{ width: `${(counts.warning / deptTotal) * 100}%`, height: '100%', background: '#D97706', transition: 'width 600ms ease-out' }} />}
                      {counts.safe > 0 && <div style={{ width: `${(counts.safe / deptTotal) * 100}%`, height: '100%', background: '#3B82F6', transition: 'width 600ms ease-out' }} />}
                    </div>
                    <span style={{ width: 22, fontSize: 10, textAlign: 'right', color: 'var(--exec-text-tertiary)', flexShrink: 0 }}>{counts.total}</span>
                  </div>
                </TooltipTrigger>
        ...
        <div className="space-y-1 mb-3">
          {nextExpiring.length > 0 ? nextExpiring.map((r) => (
            <div key={r.id} className="flex items-center gap-2" style={{ fontSize: 11 }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--exec-signal-red, #DC2626)', flexShrink: 0 }} />
              <span className="flex-1 truncate" style={{ color: 'var(--exec-text-primary)', fontWeight: 500 }}>{r.name || '—'}</span>
              <span style={{ color: 'var(--exec-text-tertiary)', fontSize: 10 }}>{r.department_name || '—'}</span>
              <span style={{ color: 'var(--exec-signal-red, #DC2626)', fontWeight: 600, fontSize: 10 }}>{formatDate(r.contract_end_date!)}</span>
            </div>
          )) : (
            <div style={{ fontSize: 10, color: 'var(--exec-text-tertiary)', textAlign: 'center' }}>No imminent expirations</div>
          )}
          {critical.length > 3 && (
        ...
        {topVendor && (
          <div className="flex items-center gap-1.5" style={{ fontSize: 10, color: 'var(--exec-text-secondary)' }}>
            {vendorPct > 40 && <AlertTriangle size={10} style={{ color: vendorPct > 60 ? '#DC2626' : '#D97706' }} />}
            <span>Top vendor: <strong>{topVendor[0]}</strong> — {topVendor[1]} resources ({vendorPct}%)</span>
          </div>
        )}

        <style>{`
          .contract-pulse-dot { animation: contractPulse 2s infinite; }
          @keyframes contractPulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }
        `}</style>
      </div>
    </TooltipProvider>
  );
