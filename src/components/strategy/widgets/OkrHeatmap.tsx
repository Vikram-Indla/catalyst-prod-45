...
function getCellStyle(status: OkrStatus, isOverall = false): { bg: string; text: string; weight: number; borderLeft?: string } {
  switch (status) {
    ...
    case 'at_risk': return { bg: isOverall ? 'var(--catalyst-bg-surface-3, var(--bd-default, #E2E8F0))' : 'var(--catalyst-bg-surface-2, #F1F5F9)', text: isOverall ? 'var(--exec-signal-amber, #92400E)' : 'var(--exec-signal-amber, #B45309)', weight: 600, borderLeft: '3px solid var(--exec-signal-amber, var(--cp-warning, #D97706))' };
