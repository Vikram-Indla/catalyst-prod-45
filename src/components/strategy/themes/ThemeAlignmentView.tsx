...
const LAYER = {
  theme:      { color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', badgeBg: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', badgeText: '#1E40AF', border: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' },
  goal:       { color: '#0D9488', badgeBg: '#F0FDFA', badgeText: '#115E59', border: '#0D9488' },
  kr:         { color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', badgeBg: '#DBEAFE', badgeText: '#1E40AF', border: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' },
  initiative: { color: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', badgeBg: '#FFFBEB', badgeText: '#92400E', border: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' },
...
const STATUS_CONFIG: Record<string, { dot: string; bg: string; text: string; label: string }> = {
  active:       { dot: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: '#F0FDF4', text: '#166534', label: 'Active' },
  on_track:     { dot: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: '#F0FDF4', text: '#166534', label: 'On Track' },
  at_risk:      { dot: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', bg: '#FFFBEB', text: '#92400E', label: 'At Risk' },
  off_track:    { dot: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))', bg: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))', text: 'var(--ds-text-danger, var(--ds-text-danger, #991B1B))', label: 'Off Track' },
  draft:        { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--bg-1, #F8FAFC)', text: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', label: 'Draft' },
  planned:      { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--bg-1, #F8FAFC)', text: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', label: 'Planned' },
  completed:    { dot: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', bg: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', text: '#1E40AF', label: 'Done' },
  cancelled:    { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--bg-1, #F8FAFC)', text: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', label: 'Cancelled' },
  in_progress:  { dot: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', bg: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', text: '#1E40AF', label: 'In Progress' },
  not_started:  { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--bg-1, #F8FAFC)', text: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', label: 'Not Started' },
  approved:     { dot: '#0D9488', bg: '#F0FDFA', text: '#115E59', label: 'Approved' },
  proposed:     { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--bg-1, #F8FAFC)', text: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', label: 'Proposed' },
  analyzing:    { dot: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', bg: 'var(--bg-1, #F8FAFC)', text: 'var(--ds-text-subtle, var(--ds-text-subtle, #475569))', label: 'Analyzing' },
};

function getProgressColor(v: number) {
  if (v >= 60) return 'var(--ds-text-success, var(--ds-text-success, #16A34A))';
  if (v >= 40) return 'var(--ds-text-warning, var(--ds-text-warning, #D97706))';
...
function ChainStat({ label, value, total }: { label: string; value: number; total: number }) {
  ...
  const clr = pct >= 70 ? 'var(--ds-text-success, var(--ds-text-success, #16A34A))' : pct >= 40 ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))';
  return (
...
function GhostNode({ label }: { label: string }) {
  return (
    <div className="flex items-center justify-center gap-1.5 border border-dashed rounded-lg"
      style={{ width: 180, padding: 12, borderColor: 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))', background: 'rgba(248,250,252,0.5)' }}>
      <Unlink size={12} style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }} />
      <span style={{ fontSize: 11, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>No linked {label}</span>
    </div>
  );
}
...
function ChainPill({ color, keyLabel, title }: { color: string; keyLabel: string; title: string }) {
  return (
    <div className="flex items-center gap-1.5 max-w-[180px]">
      ...
      <span className="font-medium truncate" style={{ fontSize: 11, color: 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))' }}>{title}</span>
    </div>
  );
}
    ...
    <div className="px-8 py-8">
      <div className="flex items-center gap-3 mb-8">
        <div className="flex items-center justify-center rounded-xl" style={{ width: 40, height: 40, background: '#F3E8FF' }}>
          <Sparkles size={16} className="animate-pulse" style={{ color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} />
        </div>
        <div>
...
function getChainHealthColor(chain: LockedChainData | null): string {
  ...
  switch (label) {
    case 'Strong': return 'var(--ds-text-success, var(--ds-text-success, #16A34A))';
    case 'Moderate': return 'var(--ds-text-warning, var(--ds-text-warning, #D97706))';
    case 'At Risk': return 'var(--ds-text-warning, var(--ds-text-warning, #D97706))';
    case 'Critical': return 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))';
    case 'Broken': return 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))';
    case 'Partial': return 'var(--ds-text-warning, var(--ds-text-warning, #D97706))';
    default: return 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))';
  }
}
    ...
    <div className="flex flex-col flex-1 overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-center shrink-0 px-6 border-b border-border bg-muted/50 dark:bg-transparent" style={{ height: 40 }}>
        <div className="flex items-center gap-2 text-muted-foreground dark:text-gray-300" style={{ fontSize: 12 }}>
          ...
          <span>Each</span>
          <span className="inline-flex items-center font-semibold text-[11px] py-0 px-1.5 rounded-md border dark:bg-gray-800 dark:border-gray-700" style={{ color: '#1E40AF', background: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', borderColor: '#BFDBFE' }}>● Theme</span>
