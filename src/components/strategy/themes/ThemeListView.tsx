...
import {
  deriveHealthStatus, getStatusConfig, getBscConfig,
  formatBudget, getInitials, getAvatarColor, formatThemeId, getProgressColor, DK,
} from './theme-utils';
...
export function ThemeListView({ themes, onSelect, isDark = false }: Props) {
  ...
  const borderSubtle = isDark ? DK.borderSubtle : 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))';
  ...
  return (
    <div className="rounded-xl border overflow-hidden" style={{
      ...
      >
        <div><input type="checkbox" checked={selected.size === paged.length && paged.length > 0} onChange={toggleAll} style={{ width: 14, height: 14, accentColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} /></div>
      ...
      {paged.map(theme => {
        ...
        const sc = getStatusConfig(health, isDark);
        const bsc = theme.bsc_perspective ? getBscConfig(theme.bsc_perspective, isDark) : null;
        ...
        return (
          ...
          >
            ...
            <div onClick={e => e.stopPropagation()}>
              <input type="checkbox" checked={selected.has(theme.id)} onChange={() => toggleCheck(theme.id)} style={{ width: 14, height: 14, accentColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} />
            </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            style={{
              ...
              color: page === 1 ? (isDark ? DK.t4 : 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))') : (isDark ? DK.t2 : 'var(--fg-2)'),
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${borderColor}`,
              background: isDark ? 'transparent' : 'var(--bg-app)',
              color: page === totalPages ? (isDark ? DK.t4 : 'var(--ds-text-disabled, var(--ds-text-disabled, #CBD5E1))') : (isDark ? DK.t2 : 'var(--fg-2)'),
      ...
      {selected.size > 0 && (
        <div style={{
          ...
          <button style={{ fontSize: 11, background: 'var(--ds-border, var(--ds-border, #2E2E2E))', padding: '4px 12px', borderRadius: 4, border: 'none', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', cursor: 'pointer' }}>Change Status</button>
          <button style={{ fontSize: 11, background: 'var(--ds-border, var(--ds-border, #2E2E2E))', padding: '4px 12px', borderRadius: 4, border: 'none', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', cursor: 'pointer' }}>Assign Owner</button>
          <button style={{ fontSize: 11, background: 'rgba(239,68,68,0.7)', padding: '4px 12px', borderRadius: 4, border: 'none', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', cursor: 'pointer' }}>Delete</button>
        </div>
      )}
    </div>
  );
