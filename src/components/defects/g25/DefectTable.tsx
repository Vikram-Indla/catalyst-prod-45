...
function BugTypeIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <circle cx="8" cy="8" r="3" fill="var(--ds-surface, var(--ds-surface, #FFFFFF))" />
    </svg>
  );
}
...
const SEVERITY_MAP: Record<string, { label: string; bg: string; text: string; border: string }> = {
  ...
  medium:   { label: 'MEDIUM',   bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))', text: '#42526E', border: '#C1C7D0' },
  low:      { label: 'LOW',      bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))', text: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', border: 'var(--ds-border, var(--ds-border, #DFE1E6))' },
};
...
function PriorityCell({ priority }: { priority: string | null }) {
  if (!priority) return <span style={{ fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>—</span>;
  return <PriorityIndicator priority={priority} />;
}

// ── Avatar colours (deterministic) ──
const AVATAR_COLOURS = ['var(--ds-text-brand, var(--ds-text-brand, #2563EB))', '#0D9488', '#0284C7', 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', '#DB2777', '#7C3AED', '#059669', 'var(--ds-text-warning, var(--ds-text-warning, #D97706))'];
...
function JiraBadge() {
  return (
    <span style={{
      ...
      backgroundColor: 'var(--ds-border, var(--ds-border, #DFE1E6))', color: 'var(--ds-text, var(--ds-text, #253858))',
...
function getStatusDotColor(statusCategory: string | null): string {
  ...
  return 'var(--ds-border, var(--ds-border, #DFE1E6))';
}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected ? "bg-[#DEEBFF]" : "bg-white hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F4F5F7))]"
                )}
              >
  ...
  if (!assigneeName || assigneeName === 'Unassigned') {
    return (
      <div className="flex items-center gap-2.5">
        <div style={{ width: 28, height: 28, borderRadius: '50%', backgroundColor: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))', border: '1px solid rgba(15,23,42,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <UserRound size={14} style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }} />
        </div>
        <span style={{ fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>Unassigned</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2.5">
      {avatarUrl ? (
        ...
        <div style={{ width: 28, height: 28, borderRadius: '50%', background: clr, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
          {ini}
        </div>
      )}
...
export function DefectTable({ defects, selectedIds, onSelectionChange, onDelete, visibleColumns }: Props) {
  ...
  const renderCell = (colKey: string, d: Defect, isSelected: boolean) => {
    ...
    switch (colKey) {
        ...
        return (
          <td key={colKey} style={{ width: columnWidths.key }}>
            <div className="flex items-center gap-1">
              {attCount > 0 && <span title="Attachments"><Paperclip size={12} style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', flexShrink: 0, transform: 'rotate(-45deg)' }} /></span>}
              <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 13, fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }}>{keyText}</span>
              {isJira && <JiraBadge />}
              {isJira && d.external_url && (
                <button className="opacity-0 group-hover:opacity-100 transition-opacity duration-150" style={{ marginLeft: 2, padding: 2, borderRadius: 3, border: 'none', background: 'transparent', cursor: 'pointer' }} title="Open in Jira" onClick={e => { e.stopPropagation(); window.open(d.external_url!, '_blank'); }}>
                  <ExternalLink size={13} style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }} />
                </button>
              )}
            </div>
          </td>
      ...
      case 'age':
        return <td key={colKey} style={{ fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', width: columnWidths.age }}>{getRelativeAge(d.created_at)}</td>;
      case 'actions':
        return (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="pb-row-actions opacity-0 group-hover:opacity-100 transition-opacity duration-150" style={{ height: 28, width: 28, borderRadius: 4, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', border: 'none', background: 'transparent', cursor: 'pointer' }}>
                  <MoreHorizontal size={16} style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))' }} />
                </button>
              </DropdownMenuTrigger>
