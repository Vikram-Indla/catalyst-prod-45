    ...
    <>
      ...
      >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground font-medium">
              ...
              {showContractWarning && (
                <>
                  <span className="w-[3px] h-[3px] rounded-full bg-muted-foreground flex-shrink-0" />
                  <span className="px-1.5 py-0.5 rounded bg-[var(--ds-background-danger,var(--ds-background-danger, #fef2f2))] border border-[#fecaca] text-[var(--ds-text-danger,var(--ds-text-danger, #dc2626))] text-[10px] font-semibold">
        ...
        <div className="h-10 px-5 flex items-center gap-3 border-b border-border flex-shrink-0">
          ...
          <button 
            onClick={goToToday}
            className="ml-auto flex items-center gap-1.5 px-2 py-1 rounded bg-[var(--ds-background-danger,var(--ds-background-danger, #fef2f2))] border border-[#fecaca] text-[10px] font-semibold text-[var(--ds-text-danger,var(--ds-text-danger, #dc2626))]"
          >
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--ds-text-danger,var(--ds-text-danger, #dc2626))] animate-pulse" />
                  ...
                  return (
                    <div key={week.weekStart} className="w-14 flex-shrink-0 p-2 text-center border-l border-border" role="gridcell">
                      <div className={cn(
                        ...
                        isOver ? "text-[var(--ds-text-danger,var(--ds-text-danger, #dc2626))]" : isOverForecast ? "text-[var(--ds-text-warning,var(--ds-text-warning, #d97706))]" : "text-primary"
                      )}>
                        {totals.total}%
                      </div>
                      {isOverForecast && (
                        <div className="text-[8px] text-[var(--ds-text-warning,var(--ds-text-warning, #d97706))] font-semibold">
        ...
        <div className="h-14 px-5 flex items-center gap-3 border-t border-border bg-muted/30 flex-shrink-0">
          <div className="flex-1 text-[11px] text-muted-foreground">
            {!validation.isValid && (
              <span className="text-[var(--ds-text-danger,var(--ds-text-danger, #dc2626))]">⚠️ {validation.errors[0]}</span>
            )}
            {validation.isValid && validation.warnings.length > 0 && (
              <span className="text-[var(--ds-text-warning,var(--ds-text-warning, #d97706))]">Note: {validation.warnings[0]}</span>
            )}
...
function AllocationRow({ 
  ...
  const colors = ['var(--ds-text-brand, var(--ds-text-brand, #2563eb))', '#0d9488', '#ea580c', '#7c3aed'];
