...
export function EpicView({ document }: EpicViewProps) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)]">
      {/* Toolbar */}
      <div className="sticky top-0 z-10 bg-white dark:bg-[#1A1A1A] border-b border-[hsl(var(--border))] px-6 py-3 flex items-center gap-3">
        <button
      ...
      <div className="flex-1 grid overflow-hidden" style={{ gridTemplateColumns: '280px 1fr' }}>
        {/* Left — Epic list */}
        <div className="bg-white dark:bg-[#1A1A1A] border-r border-[hsl(var(--border))] overflow-y-auto p-4">
        ...
        <div className="bg-zinc-50 overflow-y-auto p-6">
          ...
          {selectedEpic.stories.map((story) => (
            <div key={story.id} className="bg-white dark:bg-[#1A1A1A] border border-[hsl(var(--border))] rounded-[10px] p-[18px] mb-3">
              {/* Header */}
