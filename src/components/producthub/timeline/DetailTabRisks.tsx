...
function cellBg(score: number) {
  ...
  return 'var(--ds-background-success, var(--ds-background-success, #DCFCE7))';
}
...
export const DetailTabRisks: React.FC<DetailTabRisksProps> = ({ requestId }) => {
  ...
  const handleSave = async () => {
    ...
    if (editingRisk) {
      ...
      toast.success(`${payload.risk_key} created`, { duration: 2200, style: { background: '#18181B', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' }, position: 'bottom-center' });
    }
    setShowModal(false);
    refetch();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    toast.success('Risk deleted', { duration: 2200, style: { background: '#18181B', color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #fff)))' }, position: 'bottom-center' });
    refetch();
  };
