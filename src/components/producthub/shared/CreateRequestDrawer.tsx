...
interface CreateRequestDrawerProps {
   ...
   * "+ Create request" button so a card created from a column lands
...
function useNextInitiativeKey() {
  return useQuery({
    queryKey: ['next-request-key'],
    queryFn: async () => {
      const { data } = await typedQuery('ph_requests').select('initiative_key');
  ...
  useEffect(() => {
    if (open) {
      ...
      if (conversionSource) {
        ...
        // "+ Create request" button. Unknown values fall back to
