...
export function DepartmentSelect({ value, onChange, departments, disabled }: DepartmentSelectProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute mt-1 w-full bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] border border-zinc-200 dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] rounded-lg shadow-lg overflow-hidden" style={{ zIndex: 9999 }}>
          {departments.length > 5 && (
            <div className="sticky top-0 bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] border-b border-zinc-100 dark:border-[var(--ds-border,var(--cp-ink-1, #2E2E2E))] px-3 py-2.5 flex items-center gap-2">
