...
function SectionHeader({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex items-center gap-2 pt-4 pb-3 border-t" style={{ borderColor: T.border, marginTop: 4 }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 700, color: T.inkSec, fontFamily: 'var(--ds-font-family-body)', letterSpacing: '.02em' }}>{label}</span>
    </div>
  );
}
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 24px 16px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 700, color: T.ink, fontFamily: 'var(--ds-font-family-heading)', letterSpacing: '-.02em' }}>New Initiative</div>
              {nextKey && (
                <div style={{
                  display: 'inline-block', marginTop: 6, fontSize: 12, fontWeight: 600,
                  color: T.primary, background: T.primaryBg, padding: '2px 10px',
                  borderRadius: 4, fontFamily: 'var(--ds-font-family-monospaced)', lineHeight: '20px',
                }}>{nextKey}</div>
              )}
            </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '0 24px 4px' }}>
          ...
          {conversionSource?.type === 'single' && (
            <div className="p-3 rounded-lg mb-3" style={{ background: isDark ? 'rgba(74,222,128,0.08)' : '#F0FDF4', border: `1px solid ${isDark ? 'rgba(74,222,128,0.2)' : '#BBF7D0'}` }}>
              ...
              <div className="text-[12px] mt-1" style={{ color: 'var(--fg-2)' }}>
                <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontWeight: 600, color: T.primary }}>{conversionSource.primaryIdea.key}</span>
          ...
          {conversionSource?.type === 'merge' && conversionSource.mergeIdea && (
            <div className="p-3 rounded-lg mb-3" style={{ background: isDark ? 'rgba(59,130,246,0.08)' : '#EFF6FF', border: `1px solid ${isDark ? 'rgba(59,130,246,0.2)' : '#BFDBFE'}` }}>
              ...
              <div className="text-[12px] mt-1" style={{ color: 'var(--fg-2)' }}>
                Primary: <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontWeight: 600, color: T.primary }}>{conversionSource.primaryIdea.key}</span>
                {' · '}{conversionSource.primaryIdea.title}
              </div>
              <div className="text-[12px] mt-0.5" style={{ color: 'var(--fg-2)' }}>
                Merging: <span style={{ fontFamily: 'var(--ds-font-family-monospaced)', fontWeight: 600, color: T.primary }}>{conversionSource.mergeIdea.key}</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FieldWrapper label="Kickoff Date">
              <input type="date" value={form.kickoff_date} onChange={e => updateField('kickoff_date', e.target.value)}
                className={cn(INPUT_CLS, 'appearance-none')} style={{ borderColor: T.border, fontFamily: 'var(--ds-font-family-monospaced)' }} />
            </FieldWrapper>
