...
export function SprintsTable({
  ...
  // Release resolution: batch-load the ph_releases rows referenced by release_id (S1.4).
  const releaseIds = useMemo(() => {
    const all = groups ? groups.flatMap((g) => g.rows) : rows ?? [];
    return [...new Set(all.map((r) => r.release_id).filter(Boolean))] as string[];
  }, [rows, groups]);

  const { data: linkedReleases } = useQuery({
    queryKey: ['sprint-linked-releases', releaseIds.slice().sort().join('|')],
    queryFn: async () => {
      if (releaseIds.length === 0) return new Map<string, LinkedRelease>();
      const { data, error } = await supabase
        .from('ph_releases')
        .select('id, name, release_date, target_date')
        .in('id', releaseIds);
      if (error) throw new Error(error.message);
      const m = new Map<string, LinkedRelease>();
      (data ?? []).forEach((r: any) =>
        m.set(r.id, { name: r.name, date: r.release_date ?? r.target_date ?? null }),
      );
      return m;
    },
  ...
  const columns = useMemo(
    () => [
      ...
      makeSprintStartDateCell(),
      makeSprintEndCell(),
      makeSprintReleaseCell((row) =>
        row.release_id ? linkedReleases?.get(row.release_id) ?? null : null,
      ),
    ...
    [onOpenDetail, getProgress, ownerProfiles, linkedReleases, actions],
  );
