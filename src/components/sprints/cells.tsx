...
// ── Release (linked release chip — populated by S1.4; dash until then) ──────
export function makeSprintReleaseCell(): Column<SprintRow> {
  return {
    id: 'release',
    label: 'Release',
    width: 9,
    ...
    cell: ({ row }: CellProps<SprintRow>) => {
      ...
      return (
        ...
        >
          <DropdownItemGroup>
            {active && (
              <DropdownItem onClick={() => actions.onComplete(row)}>Complete sprint</DropdownItem>
            )}
            <DropdownItem onClick={() => actions.onEdit(row)}>Edit</DropdownItem>
            {actions.onMerge && (
              <DropdownItem onClick={() => actions.onMerge!(row)}>Merge</DropdownItem>
            )}
            {!active && (
              <DropdownItem onClick={() => actions.onDelete(row)}>Delete</DropdownItem>
            )}
          </DropdownItemGroup>
          <DropdownItemGroup>
            <DropdownItem onClick={() => actions.onArchive(row)}>Archive</DropdownItem>
          </DropdownItemGroup>
        </DropdownMenu>
      );
    },
