    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 600, color: '#0F172A', margin: 0 }}>Attachments</h3>
          {attachments.length > 0 && <span style={{ fontSize: 12, fontWeight: 500, color: '#64748B', backgroundColor: '#F1F5F9', padding: '2px 8px', borderRadius: 10 }}>{attachments.length}</span>}
        </div>
        {!disabled && (
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', backgroundColor: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 8, color: '#2563EB', fontSize: 13, fontWeight: 500, cursor: isUploading ? 'not-allowed' : 'pointer', opacity: isUploading ? 0.7 : 1 }}>
      ...
      {attachments.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
          {attachments.map((att) => (
            <div key={att.id} style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 10, overflow: 'hidden' }}>
              <div onClick={() => handlePreview(att)} style={{ height: 100, backgroundColor: '#F8FAFC', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                {isImage(att.file_type) ? <Image size={32} style={{ color: '#94A3B8' }} /> : <FileText size={32} style={{ color: '#94A3B8' }} />}
              </div>
              <div style={{ padding: 10 }}>
                <p style={{ fontSize: 12, fontWeight: 500, color: '#334155', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{att.file_name}</p>
                <p style={{ fontSize: 11, color: '#94A3B8', margin: '4px 0 0' }}>{formatFileSize(att.file_size)}</p>
                <div style={{ display: 'flex', gap: 4, marginTop: 8 }}>
                  <button onClick={() => handleDownload(att)} title="Download" style={{ flex: 1, height: 28, padding: 0, border: '1px solid #E2E8F0', borderRadius: 6, backgroundColor: '#FFFFFF', color: '#64748B', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Download size={14} />
                  </button>
                  {!disabled && (
                    <button onClick={() => handleDelete(att)} title="Delete" style={{ flex: 1, height: 28, padding: 0, border: '1px solid #FECACA', borderRadius: 6, backgroundColor: '#FEF2F2', color: '#DC2626', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Trash2 size={14} />
                    </button>
                  )}
                </div>
      ...
      {previewUrl && (
        <div onClick={closePreview} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 40 }}>
          <button onClick={closePreview} style={{ position: 'absolute', top: 20, right: 20, width: 40, height: 40, border: 'none', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.1)', color: '#FFFFFF', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <X size={24} />
          </button>
