  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setIsExpanded(!isExpanded)}
        style={{ display: 'flex', alignItems: 'center', gap: 6, height: 44, padding: '0 14px', border: '1.5px solid #E2E8F0', borderRadius: 10, backgroundColor: filters.length > 0 ? '#F8FAFC' : '#FFF', color: '#64748B', fontSize: 13, cursor: 'pointer' }}>
        <Filter size={16} /> Saved ({filters.length})
      </button>

      {isExpanded && (
        <div style={{ position: 'absolute', top: '100%', left: 0, marginTop: 4, width: 280, backgroundColor: '#FFFFFF', borderRadius: 10, border: '1px solid #E2E8F0', boxShadow: '0 10px 40px rgba(0,0,0,0.1)', zIndex: 100, overflow: 'hidden' }}>
          <div style={{ padding: '12px 14px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#0F172A' }}>Saved Filters</span>
            {hasActiveFilters && (
              <button onClick={() => setShowSaveModal(true)}
                style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', backgroundColor: '#2563EB', border: 'none', borderRadius: 6, color: '#FFF', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>
          ...
          <div style={{ maxHeight: 240, overflowY: 'auto' }}>
            {filters.length === 0 ? (
              <div style={{ padding: 20, textAlign: 'center', color: '#94A3B8', fontSize: 13 }}>No saved filters yet</div>
            ) : (
              filters.map((filter) => (
                <div key={filter.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderBottom: '1px solid #F1F5F9' }}>
                  <button onClick={() => onApplyFilter(filter.filter_config)}
                    ...
                    <span style={{ fontSize: 13, fontWeight: 500, color: '#0F172A' }}>{filter.name}</span>
                  </button>
                  ...
                  <button onClick={() => deleteFilter(filter.id)}
                    style={{ width: 24, height: 24, padding: 0, border: 'none', borderRadius: 4, backgroundColor: 'transparent', color: '#94A3B8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              ))
          ...
          {hasActiveFilters && (
            <div style={{ padding: 10, borderTop: '1px solid #E2E8F0' }}>
              <button onClick={() => onApplyFilter({})}
                style={{ width: '100%', padding: '8px 12px', backgroundColor: '#F1F5F9', border: 'none', borderRadius: 6, color: '#64748B', fontSize: 12, cursor: 'pointer' }}>
                Clear All Filters
              </button>
            </div>
          )}
      ...
      {showSaveModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ width: 340, backgroundColor: '#FFF', borderRadius: 12, padding: 20, boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: '#0F172A', margin: '0 0 16px' }}>Save Current Filter</h3>
            <input type="text" placeholder="Filter name" value={newFilterName} onChange={(e) => setNewFilterName(e.target.value)} autoFocus
              style={{ width: '100%', height: 44, padding: '0 14px', border: '1.5px solid #E2E8F0', borderRadius: 10, fontSize: 14, marginBottom: 16 }} />
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => { setShowSaveModal(false); setNewFilterName(''); }}
                style={{ padding: '10px 16px', backgroundColor: '#FFF', border: '1px solid #E2E8F0', borderRadius: 8, color: '#64748B', fontSize: 13, cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={saveCurrentFilter} disabled={isSaving}
                style={{ padding: '10px 16px', backgroundColor: '#2563EB', border: 'none', borderRadius: 8, color: '#FFF', fontSize: 13, fontWeight: 500, cursor: isSaving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
