...
const PRESET_COLORS = [
  'var(--ds-text-danger, var(--ds-text-danger, #EF4444))', '#F97316', 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))', '#EAB308', '#84CC16', 'var(--ds-text-success, var(--ds-text-success, #22C55E))', '#10B981',
  '#14B8A6', '#06B6D4', '#0EA5E9', 'var(--ds-text-brand, var(--ds-text-brand, #3B82F6))', '#6366F1', '#8B5CF6', '#A855F7',
  '#D946EF', '#EC4899', '#F43F5E', '#78716C', 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', 'var(--ds-text, var(--ds-text, #0F172A))',
];
...
export function CreateTagModal({ isOpen, onClose, onSaved, editingTag }: CreateTagModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingTag) {
        const { error } = await typedQuery('tm_labels').update({ name: name.trim(), color, category: category.trim() || null, description: description.trim() || null }).eq('id', editingTag.id);
        if (error) throw error;
        catalystToast.success('Tag updated');
      } else {
        const { data: { user } } = await supabase.auth.getUser();
        const { error } = await typedQuery('tm_labels').insert({ name: name.trim(), color, category: category.trim() || null, description: description.trim() || null, created_by: user?.id });
        if (error) {
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
              {CATEGORY_SUGGESTIONS.slice(0, 4).map((cat) => (
                <button key={cat} onClick={() => setCategory(cat)}
                  style={{ padding: '4px 10px', fontSize: 12, border: '1px solid var(--divider)', borderRadius: 6, backgroundColor: category === cat ? 'var(--cp-bd-zone)' : 'var(--cp-float)', color: 'var(--fg-3)', cursor: 'pointer' }}>
                  {cat}
                </button>
              ))}
            </div>
