  ...
  return (
    <>
      <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
        <div style={{ width: 520, maxHeight: '90vh', backgroundColor: 'hsl(var(--card))', borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column' }}>
          {/* Header */}
          <div style={{ padding: '20px 24px', borderBottom: '1px solid hsl(var(--border))', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))', margin: '4px 0 0' }}>Select a reason for the failure</p>
              </div>
            </div>
            <button onClick={handleClose} style={{ width: 32, height: 32, padding: 0, border: 'none', borderRadius: 8, backgroundColor: 'transparent', color: 'hsl(var(--muted-foreground))', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <X size={20} />
            </button>
          </div>
                ...
                {FAILURE_REASONS.map((reason) => {
                  ...
                  return (
                    <button key={reason.id} onClick={() => { setSelectedReason(reason.id); setError(''); }}
                      style={{ padding: '14px 16px', border: `2px solid ${isSelected ? reason.color : 'hsl(var(--border))'}`, borderRadius: 10, backgroundColor: isSelected ? `${reason.color}10` : 'hsl(var(--card))', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 8, backgroundColor: isSelected ? `${reason.color}20` : 'hsl(var(--muted) / 0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <Icon size={18} style={{ color: isSelected ? reason.color : 'hsl(var(--muted-foreground))' }} />
                        </div>
                        <div>
                          <p style={{ fontSize: 14, fontWeight: 600, color: isSelected ? reason.color : 'hsl(var(--foreground))', margin: 0 }}>{reason.label}</p>
                          <p style={{ fontSize: 12, color: 'hsl(var(--muted-foreground))', margin: '2px 0 0' }}>{reason.description}</p>
                        </div>
                      </div>
                    </button>
                  );
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'hsl(var(--foreground))', marginBottom: 6 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Link2 size={14} style={{ color: 'hsl(var(--muted-foreground))' }} /> Defect ID <span style={{ fontWeight: 400, color: 'hsl(var(--muted-foreground))' }}>(optional)</span>
                </span>
              </label>
              <div style={{ display: 'flex', gap: 8 }}>
                <input type="text" value={defectId} onChange={(e) => setDefectId(e.target.value)} placeholder="e.g., DEF-001"
                  style={{ flex: 1, height: 40, padding: '0 12px', border: '1.5px solid hsl(var(--border))', borderRadius: 8, fontSize: 14, color: 'hsl(var(--foreground))', backgroundColor: 'hsl(var(--background))' }} />
                <button
