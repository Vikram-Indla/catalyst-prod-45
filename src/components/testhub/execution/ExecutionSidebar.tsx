...
const STEP_COLORS: Record<string, { text: string; bg: string; border: string }> = {
  ...
  failed:  { text: 'var(--ds-text-danger, #DC2626)', bg: 'var(--ds-background-danger, #FEF2F2)', border: '#FECACA' },
  ...
  not_run: { text: 'var(--ds-text-subtlest, #64748B)', bg: 'var(--ds-surface-sunken, #F1F5F9)', border: 'var(--bd-default, #E2E8F0)' },
};
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--bg-app)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        ...
        {activeTab === 'history' && previousRunData && (
          <div>
            <div style={{ marginBottom: 12, padding: '8px 12px', backgroundColor: 'var(--ds-background-selected, #EFF6FF)', borderRadius: 6, border: '1px solid #BFDBFE' }}>
              <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--ds-background-brand-bold-hovered, #1D4ED8)', margin: 0 }}>
                Previous Run #{previousRunData.execution_number}
              </p>
              <p style={{ fontSize: 10, color: 'var(--ds-text-brand, #3B82F6)', margin: '2px 0 0' }}>
