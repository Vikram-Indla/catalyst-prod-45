          ...
          {executionHistory.step_results.length === 0 ? (
            ...
            executionHistory.step_results.map((step, i) => {
              const stepColors: Record<string, { text: string; bg: string; border: string }> = isDark ? {
                ...
                not_run: { text: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', border: 'var(--ds-border, var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))' },
              };
