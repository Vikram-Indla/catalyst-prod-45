...
const STATUS_CFG: Record<string, { icon: typeof CheckCircle2; color: string; bg: string }> = {
  ...
  failed:  { icon: XCircle,       color: 'var(--sem-danger)', bg: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))' },
  blocked: { icon: AlertTriangle, color: 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))', bg: '#FFFBEB' },
  skipped: { icon: SkipForward,   color: 'var(--fg-3)', bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))' },
    ...
    <div style={{ backgroundColor: 'var(--bg-app)', border: '1px solid var(--divider)', borderRadius: 12, padding: 24, minHeight: 260 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <div style={{ width: 36, height: 50, borderRadius: 12, backgroundColor: '#DBEAFE', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Activity size={18} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" />
        </div>
        <div>
