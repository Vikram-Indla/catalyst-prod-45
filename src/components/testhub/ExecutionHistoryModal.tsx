      ...
      <div style={{ width: 600, maxHeight: '80vh', backgroundColor: 'var(--cp-float)', borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--divider)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--fg-1)', margin: 0 }}>Execution History</h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--cp-blue)', backgroundColor: 'color-mix(in srgb, var(--cp-blue) 8%, transparent)', padding: '2px 6px', borderRadius: 4 }}>{testCaseKey}</span>
                <span style={{ fontSize: 13, color: 'var(--fg-3)' }}>{testCaseTitle}</span>
              </div>
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          {isLoading ? (
            <div style={{ textAlign: 'center', padding: 40, color: 'var(--fg-3)' }}>Loading history...</div>
          ) : history.length === 0 ? (
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ position: 'absolute', left: 20, top: 24, bottom: 24, width: 2, backgroundColor: 'var(--divider)' }} />
                ...
                return (
                  <div key={entry.id} style={{ display: 'flex', gap: 16, marginBottom: index < history.length - 1 ? 24 : 0, position: 'relative' }}>
                    ...
                    <div style={{ flex: 1, backgroundColor: 'var(--bg-1)', borderRadius: 12, padding: 16 }}>
                      ...
                      {entry.notes && (
                        <div style={{ marginTop: 10, padding: 10, backgroundColor: 'var(--cp-float)', borderRadius: 6, fontSize: 13, color: 'var(--fg-2)', borderLeft: '3px solid var(--divider)' }}>
                          {entry.notes}
                        </div>
                      )}
                    </div>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--divider)', display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ height: 40, padding: '0 20px', backgroundColor: 'var(--cp-float)', border: '1.5px solid var(--divider)', borderRadius: 8, fontSize: 14, fontWeight: 500, color: 'var(--fg-2)', cursor: 'pointer' }}>Close</button>
        </div>
      </div>
    </div>
  );
