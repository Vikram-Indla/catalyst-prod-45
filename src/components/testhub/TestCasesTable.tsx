...
export function TestCasesTable({
  ...
  const headerCellStyle: React.CSSProperties = {
    ...
    color: isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : 'var(--fg-3)',
          ...
          {testCases.map((tc) => {
            ...
            return (
              <tr
                ...
                style={{
                  height: 50,
                  backgroundColor: isSelected ? 'rgba(37, 99, 235, 0.06)' : isHovered ? (isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'var(--cp-bd-zone)') : (isDark ? 'var(--cp-bg-surface, #242528)' : 'var(--cp-float)'),
                  cursor: 'pointer',
                ...
                <td className="th-table-col-type" style={bodyCellStyle}>
                  <span style={{
                    ...
                    color: tc.type === 'security' ? 'var(--sem-danger)' :
                           ...
                           tc.type === 'api' ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : 'var(--fg-2)',
                  }}>
                    {tc.type.toUpperCase()}
                  </span>
                </td>
