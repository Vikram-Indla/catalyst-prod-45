      ...
      <div style={{ width: '100%', maxWidth: 600, maxHeight: '80vh', backgroundColor: '#FFFFFF', borderRadius: 16, boxShadow: '0 25px 50px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              return (
                <div key={tc.id} onClick={() => toggleSelect(tc.id)}
                  style={{
                    ...
                    borderRadius: 10, border: `1.5px solid ${isSelected ? '#2563EB' : '#E2E8F0'}`,
                    backgroundColor: isSelected ? '#EFF6FF' : '#FFFFFF', cursor: 'pointer', transition: 'all 0.15s',
                  }}>
                  {isSelected ? <CheckSquare size={18} style={{ color: '#2563EB', flexShrink: 0 }} /> : <Square size={18} style={{ color: '#E2E8F0', flexShrink: 0 }} />}
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button onClick={onClose} style={{ height: 40, padding: '0 16px', backgroundColor: '#FFFFFF', border: '1.5px solid #E2E8F0', borderRadius: 10, fontSize: 14, color: '#64748B', cursor: 'pointer' }}>Cancel</button>
            <button onClick={handleSubmit} disabled={selectedIds.size === 0 || isSubmitting}
              style={{ height: 40, padding: '0 20px', backgroundColor: selectedIds.size === 0 ? '#E2E8F0' : '#2563EB', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, color: '#FFFFFF', cursor: selectedIds.size === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
