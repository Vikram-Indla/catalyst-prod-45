...
const PRIORITY_COLORS: Record<string, string> = {
  critical: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', high: '#EA580C', medium: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', low: '#059669',
};
      ...
      <div style={{ width: '100%', maxWidth: 600, maxHeight: '80vh', backgroundColor: 'var(--cp-bg-elevated, #FFFFFF)', borderRadius: 16, boxShadow: '0 25px 50px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: `1px solid ${'var(--cp-border, #E2E8F0)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ padding: '16px 24px', borderBottom: `1px solid ${'var(--cp-border, #E2E8F0)'}` }}>
          <div style={{ position: 'relative', marginBottom: 12 }}>
            ...
            <input type="text" placeholder="Search test cases..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              style={{ width: '100%', height: 40, padding: '0 14px 0 44px', border: `1.5px solid ${'var(--cp-border, #E2E8F0)'}`, borderRadius: 12, fontSize: 14, backgroundColor: 'var(--cp-bg-elevated, #FFFFFF)', color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined }} />
          </div>
          <button onClick={toggleAll}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', border: 'none', backgroundColor: 'transparent', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 24px' }}>
          {isLoading ? (
            ...
            filtered.map(tc => {
              ...
              const priorityColor = PRIORITY_COLORS[priorityName.toLowerCase()] || 'var(--ds-text-warning, var(--ds-text-warning, #D97706))';
              return (
                <div key={tc.id} onClick={() => toggleSelect(tc.id)}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 12, padding: 12, marginBottom: 6,
                    borderRadius: 12, border: `1.5px solid ${isSelected ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : ('var(--cp-border, #E2E8F0)')}`,
                    backgroundColor: isSelected ? ('var(--cp-primary-light, #EFF6FF)') : ('var(--cp-bg-elevated, #FFFFFF)'), cursor: 'pointer', transition: 'all 0.15s',
                  }}>
                  {isSelected ? <CheckSquare size={18} style={{ color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', flexShrink: 0 }} /> : <Square size={18} style={{ color: 'var(--cp-border, #E2E8F0)', flexShrink: 0 }} />}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', backgroundColor: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', padding: '2px 8px', borderRadius: 4 }}>{tc.case_key}</span>
        ...
        <div style={{ padding: '16px 24px', borderTop: `1px solid ${'var(--cp-border, #E2E8F0)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button onClick={handleSubmit} disabled={selectedIds.size === 0 || isSubmitting}
              style={{ height: 40, padding: '0 20px', backgroundColor: selectedIds.size === 0 ? ('var(--cp-border, #E2E8F0)') : 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 600, color: 'var(--ds-text-inverse, #FFFFFF)', cursor: selectedIds.size === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
