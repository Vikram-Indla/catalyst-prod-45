...
export function CreateRequirementModal({ isOpen, onClose, onCreated }: CreateRequirementModalProps) {
  ...
  const inputStyle = (hasError?: boolean): React.CSSProperties => ({
    ...
    border: `1.5px solid ${hasError ? 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' : 'var(--cp-border, #E2E8F0)'}`,
    borderRadius: 12, fontSize: 14,
    backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : undefined,
    color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined,
  });
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 600, maxHeight: '90vh', backgroundColor: 'var(--cp-bg-elevated, #FFFFFF)', borderRadius: 16, boxShadow: '0 25px 50px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: isDark ? '1px solid #2E2E2E' : '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={onClose} style={{ width: 36, height: 50, border: 'none', borderRadius: 8, backgroundColor: 'transparent', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <X size={20} />
          </button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          <div style={{ marginBottom: 20 }}>
            ...
            {errors.title && (
              <p style={{ fontSize: 12, color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', margin: '6px 0 0', display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              ...
              style={{ width: '100%', padding: 14, border: isDark ? '1.5px solid #2E2E2E' : '1.5px solid #E2E8F0', borderRadius: 12, fontSize: 14, resize: 'vertical', backgroundColor: isDark ? 'var(--cp-bg-surface, #242528)' : undefined, color: isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : undefined }} />
          </div>
        </div>
