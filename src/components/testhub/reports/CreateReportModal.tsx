      ...
      <div style={{ width: '100%', maxWidth: 550, maxHeight: '90vh', backgroundColor: '#FFFFFF', borderRadius: 16, boxShadow: '0 25px 50px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 10, background: 'linear-gradient(135deg, #F59E0B 0%, #D97706 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <FileBarChart size={22} style={{ color: '#FFFFFF' }} />
            </div>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0F172A', margin: 0 }}>Generate Report</h2>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#0F172A', marginBottom: 10 }}>Report Type</label>
              ...
              {REPORT_TYPES.map((type) => {
                ...
                return (
                  <div key={type.value} onClick={() => setReportType(type.value)}
                    ...
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 600, color: '#0F172A', margin: 0 }}>{type.label}</p>
                      <p style={{ fontSize: 12, color: '#64748B', margin: '2px 0 0' }}>{type.description}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {errors.name && <p style={{ fontSize: 12, color: '#DC2626', margin: '6px 0 0' }}>{errors.name}</p>}
          </div>
          ...
          {selectedTypeConfig?.requiresCycle && (
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#0F172A', marginBottom: 6 }}>
                Test Cycle <span style={{ color: '#DC2626' }}>*</span>
              </label>
              <select value={cycleId} onChange={(e) => setCycleId(e.target.value)}
                style={{ width: '100%', height: 44, padding: '0 14px', border: `1.5px solid ${errors.cycleId ? '#DC2626' : '#E2E8F0'}`, borderRadius: 10, fontSize: 14, backgroundColor: '#FFF' }}>
                <option value="">Select a cycle</option>
                {cycles.map((c) => <option key={c.id} value={c.id}>{c.cycle_key} - {c.name}</option>)}
              </select>
              {errors.cycleId && <p style={{ fontSize: 12, color: '#DC2626', margin: '6px 0 0' }}>{errors.cycleId}</p>}
            </div>
          )}
          ...
          {selectedTypeConfig?.requiresPlan && (
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#0F172A', marginBottom: 6 }}>
                Test Plan <span style={{ color: '#DC2626' }}>*</span>
              </label>
              <select value={planId} onChange={(e) => setPlanId(e.target.value)}
                style={{ width: '100%', height: 44, padding: '0 14px', border: `1.5px solid ${errors.planId ? '#DC2626' : '#E2E8F0'}`, borderRadius: 10, fontSize: 14, backgroundColor: '#FFF' }}>
                <option value="">Select a plan</option>
                {plans.map((p) => <option key={p.id} value={p.id}>{p.plan_key} - {p.name}</option>)}
              </select>
              {errors.planId && <p style={{ fontSize: 12, color: '#DC2626', margin: '6px 0 0' }}>{errors.planId}</p>}
            </div>
          )}
          ...
          {/* Description */}
          <div>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#0F172A', marginBottom: 6 }}>Description (Optional)</label>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid #E2E8F0', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
          ...
          <button onClick={handleSubmit} disabled={isSubmitting}
            style={{ height: 44, padding: '0 24px', background: 'linear-gradient(135deg, #F59E0B 0%, #D97706 100%)', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, color: '#FFFFFF', cursor: isSubmitting ? 'not-allowed' : 'pointer', opacity: isSubmitting ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 8 }}>
