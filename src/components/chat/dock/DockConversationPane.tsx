...
import { useMyBookmarks, useToggleBookmark, useConversationPins, useTogglePin } from '@/hooks/chat/usePinsBookmarks';
...
export function DockConversationPane({ conversation, onBack }: DockConversationPaneProps) {
  ...
  const { messages, isLoading, sendMessage, editMessage, deleteMessage, toggleReaction } = useMessages(conversation.id);
  ...
  const handleSaveLater = useCallback((messageId: string) => {
    if (!user?.id) return;
  ...
  return (
    <div className="cc-conv-pane cv2-chat-shell" data-cv2-theme={theme}>
      ...
      {threadParent ? (
        ...
        <>
          <MessageList
            ...
            onEdit={(messageId, markdown) => { void editMessage(messageId, markdown); }}
          ...
          <Composer
            key={conversation.id}
            placeholder={`Message ${conversation.ticketKey ?? conversation.title ?? ''}`.trim()}
            conversationId={conversation.id}
            ...
            onScheduleSend={(text, whenIso) => { void handleSend(text, whenIso); }}
          />
        </>
      )}
    </div>
