...
export default function EmptyState({ message, icon = 'info' }: Props) {
  ...
  const ICONS = {
    ...
    info: <Info size={16} color={dark ? 'rgba(255,255,255,0.40)' : 'var(--fg-4)'} />,
  ...
  return (
    <div style={{
      ...
      {ICONS[icon]}
      <span style={{ fontSize: 12, color: dark ? 'rgba(255,255,255,0.50)' : 'var(--fg-4)', fontFamily: "'Inter', sans-serif" }}>
        {message}
      </span>
    </div>
  );
