          ...
          <tbody>
            {items.map((item: any, idx: number) => (
              <tr key={item.id} style={{ height: 44, borderBottom: '1px solid var(--cp-bd-zone)', background: idx % 2 === 1 ? '#FAFBFC' : undefined }} className="ph-table-row">
                ...
                <td style={{ padding: '0 8px' }}>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 700, color: '#FFFFFF', background: item.days_on_hold > 3 ? 'var(--sem-danger)' : '#F59E0B', padding: '2px 7px', borderRadius: 9999 }}>
