...
const PATTERN_META: Record<Exclude<DwellPattern, 'none'>, { code: string; label: string; bg: string; color: string; dot: string }> = {
  ping_pong: {
    ...
    dot: 'var(--ds-text-accent-red)',
  },
  pl_gap: {
    code: 'PL',
    label: 'PL gap',
    bg: 'var(--ds-background-accent-yellow-subtler)',
    color: 'var(--ds-text-accent-yellow)',
    dot: 'var(--ds-text-accent-yellow)',
  },
  spec_rewrite: {
    code: 'SR',
    label: 'Spec rewrite',
