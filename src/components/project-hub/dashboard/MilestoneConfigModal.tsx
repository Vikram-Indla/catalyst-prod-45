  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div onClick={e => e.stopPropagation()} className="bg-[var(--cp-float)] dark:bg-[#1A1A1A]" style={{ borderRadius: 16, width: 480, maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,.15)' }}>
          ...
          <div style={{ padding: '14px 24px', borderTop: '1px solid var(--divider)', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            <button onClick={onClose} className="bg-[var(--cp-float)] dark:bg-[#1A1A1A]" style={{ padding: '8px 16px', borderRadius: 6, border: '1px solid var(--divider)', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: 'var(--fg-3)' }}>Cancel</button>
            <button onClick={save} className="bg-[var(--cp-blue)]" style={{ padding: '8px 20px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: '#FFFFFF' }}>Save</button>
          </div>
        </div>
      </div>
    </>
