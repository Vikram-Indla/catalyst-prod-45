            ...
            <tbody>{visible.map((item: any, idx: number) => (
              <tr key={item.id} className={`ph-table-row ${idx % 2 === 1 ? 'bg-[var(--bg-1)]' : ''}`} style={{ height: 44, borderBottom: '1px solid var(--cp-bd-zone)' }}>
                <td style={{ padding: '0 8px' }}>
                  <span className="bg-[var(--sem-success-bg)]" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, fontWeight: 600, color: 'var(--sem-success-fg)', padding: '2px 7px', borderRadius: 4, border: '1px solid var(--sem-success-accent)' }}>{releaseMap[item.release_id] || '—'}</span>
                </td>
                ...
                <td style={{ padding: '0 8px' }}>
                  <span className={item.days_overdue <= 3 ? 'bg-[var(--sem-warning)]' : 'bg-[var(--sem-danger)]'} style={{
