...
export function ProjectSwitcher({ projects, currentKey, isOpen, onClose, onSelect }: ProjectSwitcherProps) {
  ...
  return (
    <div
      ref={ref}
      className="absolute top-full left-0 mt-1 z-50 bg-white dark:bg-[#111111] border border-[#E2E8F0] dark:border-[rgba(235,238,245,0.10)]"
      style={{
        width: 280,
        borderRadius: 8,
    ...
    >
      {/* Search */}
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#E2E8F0] dark:border-[rgba(235,238,245,0.08)]">
        <Search size={13} className="flex-shrink-0 text-[#94A3B8] dark:text-[rgba(235,238,245,0.40)]" />
        <input
          autoFocus
          value={search}
          onChange={e => setSearch(e.target.value)}
          placeholder="Search projects..."
          className="flex-1 bg-transparent outline-none border-none text-[#0F172A] dark:text-[rgba(235,238,245,0.92)]"
          style={{ fontSize: 12 }}
        />
      </div>
      ...
      <div className="max-h-[320px] overflow-y-auto py-1">
        {pinned.length > 0 && (
          <>
            <div className="px-3 pt-2 pb-1 text-[var(--fg-3)] dark:text-[rgba(235,238,245,0.55)]" style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
        ...
        {recent.length === 0 ? (
          <div className="px-3 py-3 text-center text-[var(--fg-4)] dark:text-[rgba(235,238,245,0.40)]" style={{ fontSize: 12 }}>
            No projects found
          </div>
        ) : (
          recent.map(p => (
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start min-w-0">
        ...
        <span className="text-[var(--fg-4)] dark:text-[rgba(235,238,245,0.40)]" style={{ fontSize: 10 }}>{project.key}</span>
      </div>
    </button>
  );
}