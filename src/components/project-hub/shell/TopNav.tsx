        ...
        {HUB_TABS.map(tab => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e) => { if (!isActive) e.currentTarget.style.color = isDark ? 'rgba(248,244,240,0.92)' : 'var(--cp-blue)'; }}
              onMouseLeave={(e) => { if (!isActive) e.currentTarget.style.color = isDark ? 'rgba(248,244,240,0.72)' : 'var(--fg-2)'; }}
            >
              {tab.label}
            </button>
          );
