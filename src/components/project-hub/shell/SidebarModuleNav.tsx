...
export function SidebarModuleNav({ collapsed, onToggle }: SidebarModuleNavProps) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className="flex items-center gap-2 flex-shrink-0 border-b border-[var(--ds-border,var(--ds-border, #E2E8F0))] dark:border-[var(--ds-border,var(--ds-border, #2E2E2E))]"
        style={{ padding: '12px 10px' }}
      >
        <div
          className="flex items-center justify-center flex-shrink-0"
          style={{ width: 28, height: 28, backgroundColor: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', color: 'var(--ds-text-inverse, #FFFFFF)', fontSize: 11, fontWeight: 700, fontFamily: 'var(--cp-font-heading)', borderRadius: 6 }}
        >
          PH
        </div>
        {!collapsed && (
          <span
            className="flex-1 truncate text-[var(--ds-text,var(--ds-text, #0F172A))] dark:text-[var(--ds-text,var(--ds-text, #EDEDED))]"
        ...
        <button
          onClick={onToggle}
          className="flex items-center justify-center rounded hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F4F5F7))] dark:hover:bg-[var(--ds-surface-overlay,var(--ds-surface-overlay, #1F1F1F))] transition-colors flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]"
          style={{ width: 24, height: 24, border: 'none', background: 'transparent', cursor: 'pointer' }}
          title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
        >
          {collapsed ? (
            <ChevronsRight size={16} className="text-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] dark:text-[#4C9AFF]" />
          ) : (
            <ChevronsLeft size={16} className="text-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] dark:text-[#4C9AFF]" />
          )}
        </button>
      </div>
        ...
        {!collapsed && (
          <>
            ...
            <div className="pt-1 pb-1">
              <div className="text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #6B778C))] dark:text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #878787))]" style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' as const, padding: '0 10px 4px' }}>
                Resource 360°
              </div>
            </div>
          </>
        ...
        {!collapsed && (
          <>
            ...
            <div className="pt-1">
              ...
              <div
                className="flex items-center gap-2 mx-2 my-1 rounded-[6px] border border-dashed border-[var(--ds-border,var(--ds-border, #DFE1E6))] dark:border-[var(--ds-border-bold,var(--ds-border-bold, #454545))]"
                style={{ padding: '10px 12px' }}
              >
                <Star size={14} className="text-[#C1C7D0] dark:text-[#7D7D7D] flex-shrink-0" />
                <span className="text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #6B778C))] dark:text-[#7D7D7D]" style={{ fontSize: 12 }}>
