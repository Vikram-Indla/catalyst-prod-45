...
export function HealthBadge({ health }: { health: string | null }) {
  if (!health) return <span style={{ fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>—</span>;
...
const AVATAR_COLORS = ['var(--ds-text-brand, var(--ds-text-brand, #2563EB))', '#0D9488', 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', '#7C3AED', 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', '#059669', '#0369A1', '#BE185D'];
...
export function AvatarStack({ names }: { names: string[] }) {
  if (names.length === 0) return <span style={{ fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>—</span>;
