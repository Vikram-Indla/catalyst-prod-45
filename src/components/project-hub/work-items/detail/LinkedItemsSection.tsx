  ...
  return (
    <CollapsibleSection title="Linked Work Items" count={linkedItems.length} defaultOpen={linkedItems.length > 0}>
      ...
      {linkedItems.length > 0 && groupLinks(linkedItems).map(([type, links]) => (
        <div key={type} className="mb-2">
          ...
          {links.map(link => (
            <button
              key={link.id}
              className="flex items-center gap-2 px-3 py-1.5 rounded hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F8FAFC))] w-full text-left transition-colors"
      ...
      <button
        ...
        className="flex items-center gap-1.5 mt-2 text-[11px] font-medium hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F8FAFC))] px-2 py-1.5 rounded transition-colors"
  ...
  const { data: searchResults = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((d: any) => ({
        ...
        type_color: d.ph_work_types?.color ?? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))',
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid var(--cp-bd-zone)' }}>
          <div className="flex items-center gap-2">
            <LinkIcon size={16} className="text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))]" />
            <span className="text-[14px] font-semibold" style={{ color: 'var(--fg-1)' }}>Link Work Item</span>
          </div>
          <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F1F5F9))]">
            <X size={14} className="text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))]" />
          </button>
        </div>
        ...
        <div className="px-5 py-4 flex-1 overflow-y-auto">
          ...
          <select
            ...
            className="w-full text-[13px] px-3 py-2 rounded mb-4 focus:outline-none focus:ring-1 focus:ring-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] bg-[var(--cp-float)]"
          ...
          <div className="flex items-center gap-1.5 px-3 py-2 rounded mb-2" style={{ border: '1px solid var(--divider)' }}>
            <Search size={13} className="text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))]" />
            <input
              autoFocus
              value={search}
          ...
          <div className="max-h-[200px] overflow-y-auto">
            {searchResults.map((item: any) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F8FAFC))] text-left transition-colors ${selectedId === item.id ? 'bg-[var(--cp-blue-wash)]' : ''}`}
              >
                ...
                {selectedId === item.id && <Check size={13} className="ml-auto text-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]" />}
              </button>
            ))}
