...
export function EditStatusModal({ open, status, onClose, onSubmit, loading }: EditStatusModalProps) {
  ...
  return (
    ...
    >
      <div className="bg-[var(--cp-float)] dark:bg-[#111111]" style={{ width: 440, borderRadius: 12, padding: 24, boxShadow: '0 20px 25px -5px rgba(0,0,0,.1)', fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} className="bg-[var(--cp-float)] dark:bg-[#111111]" style={inputStyle} />
          </div>
          ...
          <div>
            ...
            <select value={category} onChange={e => setCategory(e.target.value)} className="bg-[var(--cp-float)] dark:bg-[#111111]" style={{ ...inputStyle, cursor: 'pointer' }}>
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={onClose} className="bg-[var(--cp-float)] dark:bg-[#111111]" style={{ height: 36, padding: '0 16px', fontSize: 13, fontWeight: 500, color: 'var(--fg-2)', border: '1px solid var(--divider)', borderRadius: 6, cursor: 'pointer' }}>Cancel</button>
          <button
