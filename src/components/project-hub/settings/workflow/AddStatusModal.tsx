...
const COLORS = ['var(--ds-text-brand, #2563EB)', 'var(--cp-teal-60, #0D9488)', 'var(--cp-purple-60, #7C3AED)', 'var(--ds-text-danger, var(--cp-danger, #DC2626))', '#EA580C', 'var(--ds-text-warning, var(--cp-warning, #D97706))', 'var(--ds-text-success, var(--cp-success, #16A34A))', '#0284C7'];
      ...
      <div className="bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)]" style={{ width: 440, borderRadius: 12, padding: 24, fontFamily: 'var(--cp-font-body)' }}>
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="flex items-center justify-center rounded-md hover:bg-[var(--cp-bd-zone)] dark:hover:bg-[var(--ds-surface-overlay,#1F1F1F)] transition-colors" style={{ width: 28, height: 28, border: 'none', background: 'transparent', cursor: 'pointer' }}>
            <X size={16} className="text-[var(--ds-text-subtlest,var(--cp-ink-3, #64748B))] dark:text-[var(--ds-text-subtlest,#878787)]" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. In Review"
              className="w-full bg-white dark:bg-transparent border border-[var(--bd-default,var(--cp-border, #E2E8F0))] dark:border-[var(--ds-border,#2E2E2E)] text-[var(--ds-text,var(--cp-ink-1, #0F172A))] dark:text-[var(--ds-text,#EDEDED)] placeholder:text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))] dark:placeholder:text-[#7D7D7D]"
          ...
          <div>
            ...
            <select value={category} onChange={e => setCategory(e.target.value)}
              className="w-full bg-white dark:bg-transparent border border-[var(--bd-default,var(--cp-border, #E2E8F0))] dark:border-[var(--ds-border,#2E2E2E)] text-[var(--ds-text,var(--cp-ink-1, #0F172A))] dark:text-[var(--ds-text,#EDEDED)]"
          ...
          <div>
            <label className="text-[var(--fg-2)] dark:text-[var(--ds-text-subtlest,#A1A1A1)]" style={{ fontSize: 12, fontWeight: 500, display: 'block', marginBottom: 6 }}>Color</label>
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={onClose}
            className="bg-white dark:bg-transparent border border-[var(--bd-default,var(--cp-border, #E2E8F0))] dark:border-[var(--ds-border,#2E2E2E)] text-[var(--cp-ink-2, var(--cp-ink-2, #334155))] dark:text-[var(--ds-text-subtlest,#A1A1A1)]"
