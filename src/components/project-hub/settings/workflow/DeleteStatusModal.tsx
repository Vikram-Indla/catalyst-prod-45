...
export function DeleteStatusModal({ open, statusName, itemCount, otherStatuses, onClose, onConfirm, loading }: DeleteStatusModalProps) {
  ...
  return (
    ...
    >
      <div className="bg-[var(--cp-float)] dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]" style={{ width: 440, borderRadius: 12, padding: 24, boxShadow: '0 20px 25px -5px rgba(0,0,0,.1)', fontFamily: 'var(--cp-font-body)' }}>
        ...
        {hasMigration ? (
          <>
            ...
            <div className="mb-4">
              ...
              <select value={targetId} onChange={e => setTargetId(e.target.value)} className="bg-[var(--cp-float)] dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]" style={{ ...inputStyle, cursor: 'pointer' }}>
                {otherStatuses.map(s => (
        ...
        <div className="flex justify-end gap-2">
          <button onClick={onClose} className="bg-[var(--cp-float)] dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))]" style={{ height: 50, padding: '0 16px', fontSize: 13, fontWeight: 500, color: 'var(--fg-2)', border: '1px solid var(--divider)', borderRadius: 6, cursor: 'pointer' }}>Cancel</button>
          <button
            ...
            style={{ height: 50, padding: '0 16px', fontSize: 13, fontWeight: 600, color: 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated, #ffffff)))', border: 'none', borderRadius: 6, cursor: loading ? 'default' : 'pointer' }}
          >
