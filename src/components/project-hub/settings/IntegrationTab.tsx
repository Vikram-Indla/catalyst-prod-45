...
function StatusLozenge({ status }: { status: string }) {
  const map: Record<string, { bg: string; text: string }> = {
    ...
    'Not Connected': { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', text: 'var(--ds-text, #253858)' },
  };
...
export function IntegrationTab() {
  ...
  return (
    <div className="space-y-6 dark:bg-[var(--ds-surface,#0A0A0A)]">
      {/* Section 1 — Connection Status */}
      <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #292929))] rounded-md p-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        {statCards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              className="relative bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #292929))] rounded-md p-4"
            >
      ...
      <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1, #1A1A1A))] border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #292929))] rounded-md overflow-hidden">
        ...
        {logsError ? (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #292929))]">
            ...
            <tbody>
              {syncLogs.map((log: any) => {
                ...
                return (
                  <tr
                    key={log.id}
                    className="border-b border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] dark:border-[var(--ds-border,var(--cp-ink-1, #292929))]"
