...
export function ProjectTable({ projects, starredIds, onToggleStar, onContextMenu, isDark = false }: ProjectTableProps) {
  ...
  return (
    <div
      className={`overflow-x-auto rounded-lg border ${isDark ? 'bg-transparent border-[rgba(255,255,255,0.12)]' : 'bg-white border-[rgba(255,255,255,0.10)]'}`}
    >
      <table className="w-full" style={{ borderCollapse: 'collapse', fontFamily: "'Inter', sans-serif" }}>
        <thead>
          <tr
            className={isDark ? 'bg-[rgba(255,255,255,0.04)]' : 'bg-[#1A1A1A]'}
            style={{ height: 50, position: 'sticky', top: 0, zIndex: 1 }}
          >
            {COLUMNS.map(col => (
              <th
                key={col.key}
                className={`border-b ${isDark ? 'border-[rgba(255,255,255,0.08)]' : 'border-[rgba(255,255,255,0.10)]'}`}
                style={{
                  width: col.width,
                  padding: '0 8px',
                  fontSize: 10.5,
