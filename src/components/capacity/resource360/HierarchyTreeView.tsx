...
export function HierarchyTreeView({ workItems }: HierarchyTreeViewProps) {
  ...
  const getWorkItemIcon = (type: string) => {
    const iconMap: Record<string, { icon: React.ReactNode; bg: string }> = {
      ...
      epic: { 
        ...
        bg: 'bg-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))]' 
      },
      feature: { 
      ...
      defect: { 
        ...
        bg: 'bg-[var(--ds-text-danger,var(--ds-text-danger, #dc2626))]' 
      },
      business_request: { 
        icon: <Briefcase className="w-4 h-4 text-white" />, 
        bg: 'bg-[var(--ds-text-success,var(--ds-text-success, #22c55e))]' 
      },
    };
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { bg: string; text: string; label: string }> = {
      ...
      future: { bg: 'bg-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))]/10', text: 'text-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))]', label: 'FUTURE' },
