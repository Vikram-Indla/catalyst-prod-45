 ...
 * epic-specific content in the left panel and right sidebar.
 * Data source: ph_issues (Jira-synced epics) with fallback to epics table.
 */
...
import type { CatalystViewBaseProps, PhIssue, Profile, PhComment, PhActivityLog } from '../shared/types';
import {
  IssueIcon, StatusLozenge, JiraStatusPill,
} from '@/modules/project-work-hub/components/dialogs/story-detail-modules/shared-components';
import {
  STATUS_OPTION_GROUPS, detailLabelStyle, detailValueStyle,
...
export default function CatalystViewEpic({
  ...
  const { data: issue, isLoading } = useQuery({
    queryKey: ['cv-epic-detail', itemId],
  ...
  const { data: comments = [] } = useQuery({
    queryKey: ['cv-epic-comments', itemId],
    enabled: !!itemId && isOpen,
    queryFn: async () => {
  ...
  const { data: activityLog = [] } = useQuery({
    queryKey: ['cv-epic-activity', itemId],
    enabled: !!itemId && isOpen,
    queryFn: async () => {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cv-epic-detail', itemId] }),
  });
  ...
  const deleteIssueMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Epic deleted'); onClose(); },
  });
      ...
      {showConfirmDelete && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(9,30,66,0.4)' }}>
          <div style={{ background: '#FFF', borderRadius: 8, padding: 28, width: 400, maxWidth: '95vw', animation: 'cv-confirm-in 200ms ease-out' }}>
            ...
            <p style={{ fontSize: 13, color: '#5E6C84', lineHeight: 1.6, marginBottom: 20 }}>This epic will be soft-deleted. It can be restored within 30 days.</p>
  ...
  return (
    <CatalystViewBase
      ...
      moreMenuItems={[
        ...
        { label: 'Delete epic', onClick: () => setShowConfirmDelete(true), danger: true },
      ]}
