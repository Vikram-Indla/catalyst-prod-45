 ...
 * business-request-specific content.
 * Data source: ph_issues (Jira-synced business requests).
 *
 * ── CUSTOMIZATION GUIDE ──
 * To add Business Request–specific sections, edit the leftContent or
 * rightContent JSX below. Examples:
 *   - Add "Business Justification" section in leftContent
 *   - Add "Requestor / Department" field in rightContent
 *   - Add "Linked Epics" child list in leftContent
 * No other CatalystView* file is affected by changes here.
 */
...
import { ChevronDown, MessageSquare, Clock, FileText } from 'lucide-react';
...
export default function CatalystViewBusinessRequest({
  ...
  const { data: comments = [] } = useQuery({
    queryKey: ['cv-br-comments', itemId],
    enabled: !!itemId && isOpen,
    queryFn: async () => {
  ...
  const { data: activityLog = [] } = useQuery({
    queryKey: ['cv-br-activity', itemId],
    enabled: !!itemId && isOpen,
    queryFn: async () => {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cv-br-detail', itemId] }),
  });
  ...
  const deleteIssueMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Business request deleted'); onClose(); },
  });
      ...
      {showConfirmDelete && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(9,30,66,0.4)' }}>
          <div style={{ background: '#FFF', borderRadius: 8, padding: 28, width: 400, maxWidth: '95vw', animation: 'cv-confirm-in 200ms ease-out' }}>
            ...
            <p style={{ fontSize: 13, color: '#5E6C84', lineHeight: 1.6, marginBottom: 20 }}>This business request will be soft-deleted. It can be restored within 30 days.</p>
  ...
  return (
    <CatalystViewBase
      ...
      moreMenuItems={[
        ...
        { label: 'Delete request', onClick: () => setShowConfirmDelete(true), danger: true },
      ]}
