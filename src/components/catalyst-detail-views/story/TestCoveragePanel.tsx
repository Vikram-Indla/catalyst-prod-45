...
export function TestCoveragePanel({ issueKey, statusCategory }: TestCoveragePanelProps) {
  ...
  if (isLoading) {
    return (
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 8, color: 'var(--ds-text-subtle)' }}>
        <Spinner size="small" /> <span style={{ fontSize: 13 }}>Loading test coverage…</span>
      </div>
    );
  }
  if (!data) return null;
  ...
  const isMismatch = statusCategory === 'Done' && hasFailing;

  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <Heading size="small">Test coverage</Heading>
        {data.covered
      ...
      {isMismatch && (
        <div style={{ background: 'var(--ds-background-danger)', border: '1px solid var(--ds-border-danger)', borderRadius: 6, padding: '8px 12px', marginBottom: 10, color: 'var(--ds-text-danger)', fontSize: 13 }}>
      ...
      {!data.covered ? (
        <div style={{ color: 'var(--ds-text-subtle)', fontSize: 13 }}>
          No test case is linked to this story.
        </div>
      ) : (
