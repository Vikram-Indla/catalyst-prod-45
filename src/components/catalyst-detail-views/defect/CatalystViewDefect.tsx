 ...
 * defect-specific content: severity, steps to reproduce, environment.
 * Data source: ph_issues (Jira-synced bugs/defects).
 */
...
export default function CatalystViewDefect({
  ...
  const { data: issue, isLoading } = useQuery({
    queryKey: ['cv-defect-detail', itemId],
  ...
  const { data: comments = [] } = useQuery({
    queryKey: ['cv-defect-comments', itemId],
    enabled: !!itemId && isOpen,
    queryFn: async () => {
  ...
  const { data: activityLog = [] } = useQuery({
    queryKey: ['cv-defect-activity', itemId],
    enabled: !!itemId && isOpen,
    queryFn: async () => {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cv-defect-detail', itemId] }),
  });
  ...
  const deleteIssueMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Defect deleted'); onClose(); },
  });
      ...
      {showConfirmDelete && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(9,30,66,0.4)' }}>
          <div style={{ background: '#FFF', borderRadius: 8, padding: 28, width: 400, maxWidth: '95vw', animation: 'cv-confirm-in 200ms ease-out' }}>
            ...
            <p style={{ fontSize: 13, color: '#5E6C84', lineHeight: 1.6, marginBottom: 20 }}>This defect will be soft-deleted. It can be restored within 30 days.</p>
  ...
  return (
    <CatalystViewBase
      ...
      moreMenuItems={[
        ...
        { label: 'Delete defect', onClick: () => setShowConfirmDelete(true), danger: true },
      ]}
