...
function SidebarAddTrigger({
  ...
  return (
    <button
      ...
      onMouseEnter={e => { e.currentTarget.style.borderColor = '#4C9AFF'; e.currentTarget.style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; }}
...
const PARENT_TOKENS: Record<string, { bg: string; text: string }> = {
  ...
  Task:               { bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', text: '#42526E' },
  'Production Incident': { bg: '#FFF7D6', text: '#7F5F01' },
  'Business Request': { bg: '#FFFAE6', text: '#594300' },
  default:            { bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))', text: '#42526E' },
};
    ...
    <div style={{ position: 'relative' }} ref={pickerRef}>
      ...
      {currentParent ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <button onClick={() => updateParent.mutate(null)} title="Remove parent" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', display: 'flex' }}>
            <X size={12} />
          </button>
        </div>
      ) : (
      ...
      {showPicker && (
        <div style={{
          ...
          background: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', border: '1px solid #DFE1E6', borderRadius: 6,
          ...
          <div style={{ padding: '8px 12px', borderBottom: '1px solid #F4F5F7' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, border: '2px solid #4C9AFF', borderRadius: 4, padding: '4px 8px' }}>
              ...
              {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', display: 'flex', padding: 0 }}><X size={14} /></button>}
            </div>
          </div>
          <div style={{ overflowY: 'auto', maxHeight: 340 }}>
            ...
            {filtered.length === 0 && <div style={{ padding: '16px', fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textAlign: 'center' }}>No matching business requests</div>}
          </div>
        </div>
      )}
    </div>
...
function renderBrGroup(
  ...
  return (
    <>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textTransform: 'uppercase', letterSpacing: '0.05em', padding: '8px 12px 4px' }}>{label}</div>
      {items.map(item => {
        ...
        return (
          <div key={item.id} onClick={() => item.request_key && onSelect(item.request_key)}
            ...
            onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; }}
    ...
    <div style={{ position: 'relative' }} ref={pickerRef}>
      ...
      {currentParent ? (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <button onClick={() => updateParent.mutate(null)} title="Remove parent" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', display: 'flex', marginLeft: 'auto' }}>
            <X size={12} />
          </button>
        </div>
      ) : (
      ...
      {showPicker && (
        <div style={{
          ...
          <div style={{ overflowY: 'auto', maxHeight: 340 }}>
            ...
            {filtered.length === 0 && <div style={{ padding: '16px', fontSize: 13, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textAlign: 'center' }}>No matching items</div>}
          </div>
        </div>
      )}
    </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 8 }}>
            {existingLinks.map((link: any) => (
              <div key={link.id} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <button onClick={() => removeLink.mutate(link.linkId)} title="Remove link" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', display: 'flex' }}>
                  <X size={12} />
                </button>
              </div>
            ))}
      ...
      {items.map(item => {
        ...
        return (
          <div key={item.id} onClick={() => onToggle(item)}
            ...
            onMouseEnter={e => { if (!isLinked) e.currentTarget.style.background = 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))'; }}
            onMouseLeave={e => { e.currentTarget.style.background = isLinked ? '#DEEBFF' : 'transparent'; }}
          >
            {/* Checkbox */}
            <div style={{
              width: 16, height: 16, borderRadius: 3, flexShrink: 0,
              border: `1.5px solid ${isLinked ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : '#C1C7D0'}`,
              background: isLinked ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'background 0.12s, border-color 0.12s',
            }}>
              {isLinked && <Check size={10} color="var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))" strokeWidth={3} />}
            </div>
            <IssueIcon type={item.issue_type} size={14} />
            <span style={{ fontFamily: 'var(--cp-font-mono)', fontSize: 12, color: '#5E6C84', flexShrink: 0 }}>{item.issue_key}</span>
            <span style={{ fontSize: 13, color: '#292A2E', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.summary}</span>
