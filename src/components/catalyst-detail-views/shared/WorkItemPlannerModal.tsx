            ...
            {[
              ...
              >
                <span style={{
                  ...
                  color: stepIndex >= n ? '#fff' : token('color.text.subtlest', '#6B778C'),
                }}>
                  {stepIndex > n ? '✓' : n}
                </span>
                {label}
                {n < 3 && (
                  <span style={{ color: token('color.border', '#DFE1E6'), margin: '0 2px' }}>›</span>
                )}
              </span>
            ))}
          </div>
      ...
      <ModalBody>
        ...
        {isLoading && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '32px 0' }}>
            <Spinner size="medium" />
            <span style={{ fontSize: 14, color: token('color.text.subtle', '#42526E') }}>
        ...
        {/* Error state */}
        {state.step === 'error' && (
          <div style={{ padding: '16px 0', color: token('color.text.danger', '#AE2A19') }}>
            {state.error}
          </div>
        )}
        ...
        {state.step === 'done' && (
          <div style={{ padding: '24px 0', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 500, color: token('color.text', '#172B4D'), marginBottom: 4 }}>
              Planning complete
            </div>
            <div style={{ fontSize: 13, color: token('color.text.subtle', '#42526E') }}>
