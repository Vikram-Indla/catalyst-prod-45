...
function getNumberClass(count: number): string {
  ...
  if (count <= 6) return 'text-[var(--ds-text-warning,#d97706)]';
  return 'text-[var(--ds-text-danger,#ef4444)]';
}
      ...
      {MONTHS.map((month, index) => (
        <div 
          ...
          className={cn(
            ...
            index === currentMonth && "bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))]/[0.06]"
          )}
        >
          {index === currentMonth && (
            <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))]" />
          )}
          <div className={cn(
            "text-[10px] font-semibold uppercase tracking-[0.02em] mb-1",
            index === currentMonth ? "text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))] font-bold" : "text-muted-foreground"
          )}>
            {month} 26
          </div>
          ...
          {index === currentMonth && (
            <span className="inline-block mt-1.5 px-2 py-0.5 bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))] rounded text-[8px] font-bold text-white uppercase tracking-[0.06em]">
              Now
            </span>
          )}
        </div>
