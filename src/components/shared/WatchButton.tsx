...
export default function WatchButton({ issueId, size = "sm", showCount = false, className }: WatchButtonProps) {
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    if (isWatching) {
      toast("Unwatched", {
        icon: <EyeOff size={16} color="var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))" />,
        duration: 2000,
      });
    } else {
      toast("Watching — you'll be notified of all activity", {
        icon: <Eye size={16} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))" />,
        duration: 2000,
      });
    }
  };
    ...
    >
      <button
        ...
        style={{
          ...
          color: isWatching ? "var(--ds-text-brand, var(--ds-text-brand, #2563EB))" : "var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))",
        ...
        onMouseEnter={(e) => {
          if (!isWatching) {
            ...
            e.currentTarget.style.color = "var(--ds-text-subtle, var(--ds-text-subtle, #475569))";
          }
        }}
        onMouseLeave={(e) => {
          if (!isWatching) {
