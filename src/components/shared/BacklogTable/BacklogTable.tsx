...
import type { Column, JiraTableProps, SortOrder } from "../JiraTable/types";
import { ColumnHeaderMenu, type MenuItem } from "../JiraTable/ColumnHeaderMenu";
import { ResizeColumnDialog } from "../JiraTable/ResizeColumnDialog";
...
const pageBtnStyle = (disabled: boolean): React.CSSProperties => ({
  ...
  color: disabled ? "var(--ds-text-subtlest)" : "var(--ds-text-subtle)",
...
export function BacklogTable<TRow>(props: JiraTableProps<TRow>) {
  // 2026-06-30 (user request): optional "insert work item above this row"
  // callback. Not in the shared JiraTableProps type, so read via cast — the
  // backlog page passes it; other consumers simply omit it.
  const onInsertAbove = (props as any).onInsertAbove as
    | ((row: TRow) => void)
    | undefined;
  // 2026-07-01 (user request): when the row insert-above "+" is clicked, the
  // create form must appear immediately ABOVE that specific row (on its top
  // border) — NOT at the top of the group. This renders an inline row right
  // before the matching data row. Cast-read like onInsertAbove.
  const renderInsertAboveRow = (props as any).renderInsertAboveRow as
    | ((row: TRow) => React.ReactNode)
    | undefined;
  // 2026-07-01 (user request): the drag grip is PORTALED to the border (like
  // the "+"), so it can't register the row as a pdnd drop target. This renders
  // an invisible in-cell anchor per row that does (via closest('tr')).
  const renderRowDropAnchor = (props as any).renderRowDropAnchor as
    | ((row: TRow) => React.ReactNode)
    | undefined;
  const {
    columns,
  ...
  const enableGroupCreateButton = Boolean(
    resolvedConfig.flags.enableGroupCreateButton
  );
  const enableStickyCreateFooter = Boolean(
    resolvedConfig.flags.enableStickyCreateFooter
  );
  ...
  const headerMenuTriggerRefs = useRef<Map<string, HTMLButtonElement>>(
    new Map()
  );
  // 2026-06-23 — column resize dialog (slider). Anchored under the 3-dot trigger.
  const [resizeDialogColId, setResizeDialogColId] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      ...
      if ((e.target as HTMLElement)?.closest("[data-jira-filter-trigger]"))
        return;
      setFilterMenu(null);
    };
  ...
  const visibleColumns: Column<TRow>[] = useMemo(() => {
    const base = columnVisibility
      ? columns.filter(
          (c) => !c.hidden && (c.alwaysVisible || columnVisibility.has(c.id))
        )
  ...
  const effectiveWidthFor = useCallback(
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "j": {
          e.preventDefault();
          const next =
            orderedIds[Math.min(currentIdx + 1, orderedIds.length - 1)];
  ...
  useEffect(() => {
    ...
    // 2026-06-30: reuse + UPDATE the existing style el instead of returning
    // early. The old "return if exists" guard meant edited CSS never replaced
    // the stale block on HMR — only a full page reload applied it, which made
    // styling fixes look like they "didn't work".
    const __existingStyle = document.getElementById(
      "backlog-table-focus-css"
    ...
    style.textContent = `
      ...
      /* 2026-06-30 Jira parity: row dividers are a real 1px border-bottom in
         ...
         previous inset box-shadow could be clipped by the data-row td's
         overflow:hidden, making dividers look faint/missing. A border renders
         crisply edge-to-edge regardless of overflow. */
      .jira-table-grid table tbody > tr > td {
        border-bottom: 1px solid var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral))) !important;
      }
      /* 2026-06-30 (user request): VERTICAL column dividers — 1px right border
         in --ds-border on every header + data cell. Group-header rows excluded
         (single full-width colSpan cell → no internal verticals). */
      .jira-table-grid table thead > tr > th,
      .jira-table-grid table tbody > tr:not(.jira-table-group-row) > td {
        border-right: 1px solid var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral))) !important;
      }
      /* 2026-06-30 (user request): the group-header row's single full-width
         cell is td:first-child, so the checkbox-centering first-child rules
         were centering "Epic 51". Force the header content to the LEFT
         (Jira flex-start). High specificity + !important beats those rules. */
      .jira-table-grid table tbody > tr.jira-table-group-row > td {
        text-align: left !important;
      }
      .jira-table-grid table tbody > tr.jira-table-group-row > td > .jira-group-header-row {
      ...
      .jira-table-grid .jira-table-row-focused > td:first-child {
        box-shadow: inset 3px 0 0 var(--ds-link), inset 0 -1px 0 0 var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral))) !important;
      }
      ...
      /* 2026-06-30 (user request): per-row hover affordances.
         - Drag handle: floating rounded box at the row's LEFT edge, revealed
           when the CENTER of the row is hovered (tr[data-hz="center"]).
         - Insert-above "+": top-left, revealed when the TOP strip is hovered
           (tr[data-hz="top"]). Both live in the select cell (position:relative)
           and fit inside it so they're never clipped. */
      .jira-row-drag-affordance,
      .jira-row-insert-above {
        position: absolute;
        opacity: 0;
        pointer-events: none;
      ...
      /* Let the select cell overflow so the affordances can straddle the
         table's left border (centered on it), and lift its stacking context
         on hover so the floating box paints ABOVE the table border/cells
         instead of behind them. */
      .jira-table-grid table tbody > tr > td:first-child { overflow: visible !important; position: relative; }
      .jira-table-grid table tbody > tr:hover > td:first-child { z-index: 20; }
      .jira-row-drag-affordance {
        /* Seated just inside the left border (fully visible, never clipped by
           the card's rounded/overflow edge) and INSIDE the <tr> so pdnd drag
           works. Centring it exactly ON the outer border needs a gutter, which
           adds an empty left band — deferred to a design decision. */
        left: 3px; top: 50%; transform: translateY(-50%);
        width: 18px; height: 18px; cursor: grab;
        overflow: hidden;
      }
      /* Scale the 6-dot drag glyph down to fit the small box WITHOUT squishing
         it (scale preserves the 2×3 dot grid; forcing width/height collapsed it
      ...
      .jira-table-grid table tbody > tr[data-hz="center"] .jira-row-drag-affordance {
        opacity: 1; pointer-events: auto;
      }
      .jira-table-grid table tbody > tr[data-hz="top"] .jira-row-insert-above {
        opacity: 1; pointer-events: auto;
      }
      ...
      .jira-table-viewport {
        ...
        max-height: 100%;
        width: 100%;
        /* (no extra left gutter — the row drag-handle box sits just inside the
           left border instead, so the table stays flush with no empty padding
           band.) */
        ...
        scrollbar-color: var(--ds-background-neutral-hovered) transparent;
      }
      ...
      .jira-table-grid thead th {
        ...
        /* Re-measured 2026-04-26 from Jira BAU list view header "Summary":
             ...
             - color var(--ds-text-subtlest) (color.text.subtle — re-probed 2026-04-27 vs
               Jira, was previously written as var(--ds-text-subtle) from a stale probe)
      ...
      .jira-table-grid thead th.jira-th-sortable:hover { background: var(--ds-background-neutral-hovered); }
      ...
      .jira-table-grid tbody td {
        ...
        /* Apr 27, 2026 (L60): explicit typography baseline per Jira-parity
           ...
           from legacy hardcoded var(--ds-text, var(--cp-text-primary, var(--cp-text-inverse))) (rgb 23,43,77 — old Atlassian
           ...
           td was rendering var(--ds-text, var(--cp-text-primary, var(--cp-text-inverse))) while headers + H1 used the modern
           token. Token-first ensures dark-mode + Dark mode switch flips
           too. Fallback hex updated to var(--ds-text) to match Jira's resolved
      ...
      */
      ...
      .jira-table-grid.has-select-col table tbody > tr:not(.jira-table-group-row) > td:first-child {
        text-align: center;
        padding-left: 0 !important;
        padding-right: 0 !important;
        /* 2026-06-30 (user request): vertical divider after the checkbox
           column too — matches every other column. */
        border-right: 1px solid var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral))) !important;
      }
      /* 2026-06-30: exclude the group-header row. Its single full-width colSpan
         cell is td:first-child too, and the centering below was centering the
         "Epic 51" header — Jira left-aligns it (flex-start). */
      .jira-table-grid.has-select-col table thead > tr > th:first-child > span,
      .jira-table-grid.has-select-col table tbody > tr:not(.jira-table-group-row) > td:first-child > [data-jira-table-editor],
      .jira-table-grid.has-select-col table tbody > tr:not(.jira-table-group-row) > td:first-child > div {
      ...
      /* ─────────────────────────────────────────────────────────────────────
         ...
         (var(--ds-surface-sunken, var(--cp-bg-sunken)) group-row bg, var(--ds-surface-sunken) sticky header, var(--ds-link) focus bar,
         var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral))) grid line) need explicit .dark companions per Rule 3.
         ───────────────────────────────────────────────────────────────────── */
      ...
      .dark .jira-table-grid .jira-table-row-focused > td:first-child {
        box-shadow: inset 3px 0 0 var(--ds-border-selected), inset 0 -1px 0 0 var(--ds-border) !important;
      }
    `;
    if (!__existingStyle) document.head.appendChild(style);
  }, []);
      ...
      if (
        ...
        if (anchorIdx >= 0 && curIdx >= 0) {
          ...
          for (let i = lo; i <= hi; i += 1) {
            if (next) clone.add(ordered[i]);
            else clone.delete(ordered[i]);
          }
  ...
  const rows = useMemo(() => {
    ...
    // 2026-07-01 (user request): push an inline create form as a full-width row
    // immediately ABOVE `row` when renderInsertAboveRow returns non-null for it
    // (i.e. the row whose top border the "+" was clicked). Spans all columns.
    const pushInsertAboveRow = (row: TRow) => {
      if (!renderInsertAboveRow) return;
      const node = renderInsertAboveRow(row);
      if (node == null) return;
      const rid = String(getRowId(row));
    ...
    // be toggled.
    const anyRowHasChildren =
      !!getRowHasChildren &&
      (pagedData.some((r) => getRowHasChildren(r)) ||
        (groups?.some((g) => g.rows.some((r) => getRowHasChildren(r))) ??
          false));
    ...
    const renderDataRow = (row: TRow) => {
        ...
        rowCells.push({
          ...
          content: (
            <span
              ...
              style={{ display: "inline-flex", alignItems: "center", gap: 4 }}
            >
              {/* 2026-07-01: the VISIBLE drag grip is portaled to the border
                  (centre-zone portal below) so it can straddle the border
                  fully-visible, like the "+". Here we only render an INVISIBLE
                  drop-target anchor per row (registers the <tr> as a pdnd drop
                  target via closest('tr'); the portaled grip is the drag
                  source). dragHandleNode is intentionally NOT rendered in-cell. */}
              {void dragHandleNode}
              {renderRowDropAnchor && renderRowDropAnchor(row)}
              <AkCheckbox
      ...
      return {
        ...
        onClick: (e: React.MouseEvent) => {
          ...
          if (
            target.closest(
              '[data-jira-cell-editor], [data-jira-table-editor], input, textarea, select, [contenteditable="true"]'
            )
          )
            return;
        ...
        onContextMenu:
          contextMenuActions && contextMenuActions.length > 0
            ...
            : undefined,
        cells: rowCells,
        // 2026-06-30: original row exposed so the row-hover handlers can build
        // the portaled drag handle / insert-above affordance for THIS row.
        row,
      };
    };
    ...
    if (groups && groups.length) {
      ...
      const totalCellCount = (selectable ? 1 : 0) + visibleColumns.length;

      for (const g of groups) {
        const collapsed = g.isCollapsed || !!collapsedGroups?.has(g.id);
        const groupCells: Array<{
          key: string;
          content: React.ReactNode;
          colSpan?: number;
        }> = [];
        ...
        const allSelectedInGroup =
          ...
          groupRowIds.every((rid) => selectedSet.has(rid));
        const someSelectedInGroup =
          !allSelectedInGroup &&
          groupRowIds.some((rid) => selectedSet.has(rid));

        // 2026-06-30 Jira parity: NO select-all checkbox on the group header.
        // Jira's group header is just chevron + lozenge + add — the header cell
        // spans the full row width (colSpan = totalCellCount). The per-row
        // checkboxes still exist on data rows.
        void allSelectedInGroup;
        void someSelectedInGroup;
              ...
              {onToggleGroup && (
                <span
                  ...
                  style={{
                    ...
                    // = --ds-text-subtle. Was var(--ds-text-subtlest) (--ds-text-subtlest) — too faint.
                    color: "var(--ds-text-subtle)",
  ...
  // 2026-06-30 (user request): per-row hover affordances rendered via a PORTAL
  ...
  const onRowZoneLeave = (e?: React.MouseEvent<HTMLTableRowElement>) => {
    ...
    floatHideTimer.current = setTimeout(() => setFloatAfford(null), 120);
  };
  ...
  const renderBodyRow = (r: any) => {
    ...
    return (
      ...
      >
        {r.cells.map((c: any) => {
          const isDropTarget =
            c.colId && dragOverId === c.colId && dragId && dragId !== c.colId;
          const isResizeHoverCol = c.colId && hoveredResizeColId === c.colId;
          const cellMeta = c.colId
            ? colWidthEntries.find((e) => e.id === c.colId)
            : undefined;
  ...
  return (
    ...
    >
      <div
        ...
        ref={viewportRef}
        style={
          useVirtual ? { overflowY: "auto", flex: 1, minHeight: 0 } : undefined
        }
      >
        <table
          ref={tableRef}
          ...
          <thead>
            <tr>
              {head.cells.map((cell, idx) => {
                ...
                const colMeta = meta
                  ? columns.find((c) => c.id === meta.id)
                  : undefined;
                const isLocked = !!colMeta?.lockedPosition;
                const isReorderable =
                  enableColumnReorder && !!meta && !isStructural && !isLocked;
                // Apr 27 2026 (jira-compare regression F-NEW-3 issue half):
                ...
                const firstDataColKey = head.cells.find(
                  (c) => !c.key.startsWith("__")
                )?.key;
                ...
                return (
                  <th
                    ...
                    onClick={() =>
                      meta && handleHeaderClick(meta.id, meta.sortable)
                    }
                      ...
                      isReorderable
                        ? (e) => {
                            ...
                            try {
                              ...
                              const ghostH = Math.round(
                                tableRect?.height ?? thRect.height
                              );
                              ...
                              const offsetX = Math.round(
                                e.clientX - thRect.left
                              );
                              const offsetY = Math.round(
                                e.clientY - thRect.top
                              );
                              ...
                              /* setDragImage unsupported / cross-origin guard */
                            }
                    ...
                    onDragOver={
                      isReorderable
                        ? (e) => {
                            ...
                            if (!currentDragId || currentDragId === meta!.id)
                              return;
                            e.preventDefault();
                            e.dataTransfer.dropEffect = "move";
                            if (dragOverId !== meta!.id)
                              setDragOverId(meta!.id);
                          }
                        : undefined
                    }
                    onDragLeave={
                  ...
                  >
                    ...
                    >
                      ...
                      {meta?.sortable && !isStructural && (
                        <button
                          ...
                          className={
                            isSorted
                              ? "jira-th-sort-arrow jira-th-sort-arrow-active"
                              : "jira-th-sort-arrow"
                          }
                          onClick={(e) => {
                            e.stopPropagation();
                        ...
                        (() => {
                          ...
                          return (
                            <button
                              ...
                              ref={(el) => {
                                if (el)
                                  headerMenuTriggerRefs.current.set(
                                    meta.id,
                                    el
                                  );
                                else
                                  headerMenuTriggerRefs.current.delete(meta.id);
                              }}
                              ...
                              className={
                                isMenuOpen
                                  ? "jira-th-menu-trigger jira-th-menu-trigger-active"
                                  : "jira-th-menu-trigger"
                              }
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              style={{
                                ...
                                padding: 0,
                                border: isMenuOpen
                                  ? "1px solid var(--ds-border-selected)"
                                  : "none",
                                borderRadius: 3,
                      ...
                      {(() => {
                        ...
                        return (
                          <button
                            ...
                            className={
                              col.hasActiveFilter || isFilterOpen
                                ? "jira-filter-chevron jira-filter-chevron-active"
                                : "jira-filter-chevron"
                            }
                            onClick={(e) => {
                              e.stopPropagation();
                    ...
                    {meta?.resizable && (
                      <span
                        className="jira-resize-handle"
                        data-active={
                          resizing?.id === meta.id ? "true" : "false"
                        }
                        role="separator"
                        aria-orientation="vertical"
                        aria-label="Resize column"
          ...
          {!isLoading && !virtualizer && groups && groups.length ? (
            ...
            <tbody>
              {isLoading && (
                <tr>
                  <td
                    colSpan={colWidthEntries.length}
              ...
              {!isLoading &&
                ...
                (() => {
                  ...
                  const paddingTop =
                    vItems.length > 0 ? vItems[0]?.start ?? 0 : 0;
                  const paddingBottom =
                    vItems.length > 0
                    ...
                    <>
                      ...
                      {vItems.map((vRow) => {
                        ...
                        return (
                          ...
                          >
                            {r.cells.map((c: any) => {
                              const isResizeHoverCol =
                                c.colId && hoveredResizeColId === c.colId;
                              const cellMeta = c.colId
                                ? colWidthEntries.find((e) => e.id === c.colId)
                                : undefined;
                              const cellResizable =
                                !!cellMeta?.resizable && !isGroup;
                              return (
                                <td
                                  key={c.key}
                                  colSpan={c.colSpan}
                  ...
                  >
                    ...
                    {stickyCreateFooter.onRefresh && (
                      <button
                        type="button"
                        onClick={() => {
                          void stickyCreateFooter.onRefresh?.();
                        }}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      // legacy var(--ds-text, var(--cp-text-primary, var(--cp-text-inverse))) → --ds-text fallback #292A2E to match
                      ...
                      if (!disabled)
                        (e.currentTarget as HTMLElement).style.background =
                          a.danger
                            ...
                            : "var(--ds-surface-sunken, var(--cp-bg-sunken, var(--ds-background-neutral-subtle)))";
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.background =
                        "transparent";
      ...
      {headerMenuColId &&
        (() => {
          ...
          if (!col) return null;
          const triggerRef = {
            current: headerMenuTriggerRefs.current.get(headerMenuColId) ?? null,
          };
          const reorderable = visibleColumns
            .filter((c) => !c.id.startsWith("__") && !c.lockedPosition)
            .map((c) => c.id);
          ...
          const canReorder =
            enableColumnReorder && !col.lockedPosition && idx >= 0;
          const canRemove =
            !!columnVisibility &&
            !!onColumnVisibilityChange &&
            !col.alwaysVisible;
          const widthMeta = colWidthEntries.find(
            (e) => e.id === headerMenuColId
          );
          ...
          if (col.sortable) {
            ...
            if (col.subSorts && col.subSorts.length > 0) {
              ...
              const sortablePeers = visibleColumns.filter(
                (c) => c.sortable && !c.id.startsWith("__")
              );
          ...
          if (canReorder) {
            ...
            if (!isFirst) {
              items.push({
                id: "move-first",
                label: "Move column to first position",
                onClick: () => moveColumn(headerMenuColId, "first"),
              });
              items.push({
                id: "move-left",
                label: "Move column to left",
                onClick: () => moveColumn(headerMenuColId, "left"),
              });
            }
            if (!isLast) {
              items.push({
                ...
                onClick: () => moveColumn(headerMenuColId, "right"),
              });
              items.push({
                id: "move-last",
                label: "Move column to last position",
                onClick: () => moveColumn(headerMenuColId, "last"),
              });
            }
          }
          if (canRemove) {
      ...
      {resizeDialogColId &&
        (() => {
          ...
          const triggerRef = {
            current:
              headerMenuTriggerRefs.current.get(resizeDialogColId) ?? null,
          };
          const widthMeta = colWidthEntries.find(
            (e) => e.id === resizeDialogColId
          );
          const original =
            resizeDialogOriginalWidthRef.current ?? widthMeta?.width ?? 120;
          return (
            <ResizeColumnDialog
              isOpen={true}
              ...
              onCancel={() => {
                /* preview already reverts via onPreview(original) */
              }}
            />
          );
        })()}
      {/* 2026-07-01 (user request): DRAG grip portaled to the border exactly
...
function ColumnManagerTrigger<TRow>({
  ...
  const [activeTab, setActiveTab] = useState<"my-defaults" | "system">(
    "system"
  );
  ...
  const toggleable = columns.filter(
    (c) => !c.alwaysVisible && !c.id.startsWith("__")
  );
  ...
  const reset = () => {
    ...
    const withDefault = toggleable.filter(
      (c) => c.defaultVisible !== undefined
    );
    const pool = withDefault.length
      ? withDefault.filter((c) => c.defaultVisible)
      : toggleable;
  ...
  return (
    <>
      <button
        ...
        onMouseEnter={(e) =>
          ((e.currentTarget as HTMLElement).style.background =
            "var(--ds-background-neutral, var(--ds-background-neutral))")
        }
        onMouseLeave={(e) =>
        ...
        createPortal(
          ...
          >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderBottom: `2px solid ${
                        isActive ? "var(--ds-link)" : "transparent"
                      }`,
                      padding: "4px 10px",
                      cursor: "pointer",
                      fontFamily: "inherit",
            ...
            {columns.some((c) => c.alwaysVisible) && (
              ...
              >
                {columns
                  .filter((c) => c.alwaysVisible && !c.id.startsWith("__"))
                  .map((c) => c.label || c.id)
                  .join(", ")}{" "}
                are required.
              </div>
