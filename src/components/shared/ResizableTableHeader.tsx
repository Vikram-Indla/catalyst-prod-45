...
export function ResizableTableHeader({
  ...
  return (
    <th
      ...
      style={{ width, minWidth: width, position: 'relative', color: isSorted ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' : undefined, cursor: thIsDraggable ? 'grab' : undefined }}
      className={cn(
        'select-none',
    ...
    >
      ...
      >
        ...
        {canSort && (
          isSorted
            ? (sortDirection === 'asc'
              ? <ChevronUp size={12} style={{ color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', flexShrink: 0 }} />
              : <ChevronDown size={12} style={{ color: 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))', flexShrink: 0 }} />)
            : <ChevronUp size={12} className="opacity-0 group-hover/thead:opacity-30 flex-shrink-0" style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light)))', transition: 'opacity 120ms' }} />
        )}
      </div>
