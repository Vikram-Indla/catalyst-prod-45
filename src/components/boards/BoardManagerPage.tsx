...
export default function BoardManagerPage({ projectIdOverride, basePath }: BoardManagerPageProps = {}) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-1)' }}>
      {error && (
        <div style={{ background: 'rgba(248,113,113,0.06)', color: 'var(--sem-danger)', padding: '8px 24px', fontSize: 12, fontFamily: "'Inter', sans-serif", borderBottom: '1px solid #FECACA' }}>
...
function NewBoardCard({ onClick }: { onClick: () => void }) {
  ...
  return (
    ...
    >
      <Plus size={28} color={hover ? '#2563EB' : 'rgba(237,237,237,0.40)'} strokeWidth={1.5} />
      <span style={{
