...
export default function BoardManagerPage({ projectIdOverride, basePath }: BoardManagerPageProps = {}) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-1)' }}>
      {error && (
        <div style={{ background: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))', color: 'var(--sem-danger)', padding: '8px 24px', fontSize: 12, fontFamily: 'var(--cp-font-body)', borderBottom: '1px solid #FECACA' }}>
        ...
        <div style={{ padding: '16px 24px 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '12px 0 12px' }}>
            ...
            <button onClick={() => setCreateOpen(true)} style={{
              ...
              cursor: 'pointer', fontSize: 12.5, fontWeight: 600, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))',
...
function NewBoardCard({ onClick }: { onClick: () => void }) {
  ...
  return (
    ...
    >
      <Plus size={28} color={hover ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))'} strokeWidth={1.5} />
      <span style={{
