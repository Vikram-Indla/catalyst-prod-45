...
function StatusLozenge({ status }: { status: string }) {
  ...
  let bg = 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))', color = 'var(--ds-text-subtle, #42526E)', label = status.toUpperCase();
      ...
      <div style={{ background: 'var(--bg-app)', borderBottom: '0.75px solid var(--ds-shadow-overlay, rgba(15,23,42,0.08))', flexShrink: 0, padding: '12px 24px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 0 }}>
          {boardTabs.map(tab => {
            ...
            const accent = BOARD_ACCENT[tab.name] || tab.color || 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))';
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10, paddingBottom: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button style={{
              ...
              <ChevronDown size={12} color="var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary, #64748B)))" />
            </button>
          ...
          return (
            <div key={col.id} style={{
              ...
              <span style={{
                ...
                background: col.isDone ? 'var(--ds-background-success, #DFFCF0)' : 'var(--bg-1)',
              ...
              {maxWip != null && (
                <span style={{
                  ...
                  background: atLimit ? 'var(--ds-background-danger, #FEF2F2)' : 'var(--bg-1)',
          ...
          {swimlanes.map(lane => {
            ...
            return (
              <div key={lane.id} style={{
                ...
                {lane.id === 'default' && (
                  <span style={{
                    fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text, var(--cp-ink-1, var(--cp-ink-1, #0F172A)))',
                    fontFamily: 'var(--cp-font-mono)',
                  }}>{lane.name}</span>
                )}
                <span style={{
