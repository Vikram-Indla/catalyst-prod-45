...
const BOARD_ACCENT: Record<string, string> = {
  'Demand Analysis Kanban': 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))',
  'Business Request Kanban': 'var(--ds-text-warning, var(--cp-warning))',
  'Delivery Board': 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))',
  'QA Board': 'var(--ds-text-success, var(--cp-success))',
  'Design Board': 'var(--cp-purple-60)',
  'My Planning Board': 'var(--ds-text-warning, var(--cp-warning))',
};

/* ── StatusLozenge V12 3-color guardrail ── */
function StatusLozenge({ status }: { status: string }) {
  const s = status.toLowerCase().replace(/[\s_-]+/g, '');
  let bg = 'var(--ds-border, var(--cp-lozenge-grey-bg, var(--cp-border-neutral)))', color = 'var(--ds-text-subtle)', label = status.toUpperCase();
  ...
  if (s === 'blocked') {
    leftBorder = '3px solid var(--cp-danger)';
  }
...
export default function BoardCanvasPage({ projectIdOverride, basePath }: BoardCanvasPageProps = {}) {
  ...
  if (!board) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--fg-3)', fontSize: 'var(--ds-font-size-300)', fontFamily: 'var(--cp-font-body)' }}>
        Board not found
      </div>
    );
  }
      ...
      <div style={{ background: 'var(--bg-app)', borderBottom: '0.75px solid var(--ds-shadow-overlay, rgba(15,23,42,0.08))', flexShrink: 0, padding: '12px 24px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 0 }}>
          {boardTabs.map(tab => {
            ...
            const accent = BOARD_ACCENT[tab.name] || tab.color || 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))';
            ...
            return (
              ...
              >
                {isPersonal && <User size={12} color={active ? 'var(--ds-text-warning, var(--cp-warning))' : 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light)))'} />}
                {tab.name}
              </button>
            );
          })}
        ...
        {columns.map(col => {
          ...
          return (
            <div key={col.id} style={{
              ...
              <span style={{
                fontSize: 'var(--ds-font-size-200)', fontWeight: 650, textTransform: 'uppercase' as const,
              ...
              <div key={lane.id} style={{
                ...
                {lane.id !== 'default' && (
                  <span style={{
                    ...
                    <span style={{
                      fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--fg-1)',
                      fontFamily: 'var(--cp-font-mono)',
                    }}>{lane.name}</span>
                  </span>
                )}
                {lane.id === 'default' && (
                  <span style={{
                    fontSize: 'var(--ds-font-size-200)', fontWeight: 600, color: 'var(--ds-text, var(--cp-ink-1, var(--cp-ink-1)))',
                    fontFamily: 'var(--cp-font-mono)',
                  }}>{lane.name}</span>
                )}
                <span style={{
                ...
                {lane.count > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 'auto' }}>
                    ...
                    <span style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 500, color: 'var(--fg-3)', fontFamily: 'var(--cp-font-body)' }}>
                      {pct}%
                    </span>
                  </div>
                )}
