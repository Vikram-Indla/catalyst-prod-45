...
export default function BoardCard({ board, projectId, onOpen, onSettings }: BoardCardProps) {
  ...
  const visibilityChip = () => {
    if (board.visibility === 'private') return { label: '🔒 Private', bg: 'rgba(217,119,6,0.06)', color: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' };
    if (board.visibility === 'global') return { label: '🌐 Organisation', bg: 'rgba(37,99,235,0.06)', color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' };
    ...
    <>
      <div
        ...
        style={{
          ...
          border: `0.75px solid ${isDark ? (hover ? 'var(--ds-border-bold, var(--ds-border-bold, #454545))' : 'var(--ds-border, var(--ds-border, #2E2E2E))') : (hover ? 'rgba(15,23,42,0.18)' : 'rgba(15,23,42,0.12)')}`,
      ...
      >
        ...
        <div style={{ padding: '14px 14px 12px', flex: 1 }}>
          ...
          <div ref={menuRef} style={{ position: 'absolute', top: 52, right: 42 }}>
            <button onClick={e => { e.stopPropagation(); setMenuOpen(!menuOpen); }} style={{
              ...
              <MoreHorizontal size={15} color="var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))" />
            </button>
            {menuOpen && (
              <div style={{
          ...
          <button onClick={handleStar} style={{
            ...
            <Star size={15}
              fill={board.isStarred ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'none'}
              color={board.isStarred ? 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' : 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))'}
            />
          </button>

          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 10 }}>
            {board.isPersonal && <Chip bg={'var(--cp-primary-light, #EFF6FF)'} color="var(--ds-text-brand, var(--ds-text-brand, #2563EB))">Personal</Chip>}
        ...
        <div style={{
          ...
          <button onClick={e => { e.stopPropagation(); handleOpen(); }} style={{
            ...
            fontSize: 11.5, fontWeight: 600, color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', fontFamily: 'var(--cp-font-body)',
          }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleDelete}
                ...
                style={{
                  ...
                  color: deleteConfirm === board.name ? 'var(--ds-surface, var(--ds-surface, #FFFFFF))' : 'var(--fg-4)',
...
function MenuItem({ children, onClick, danger }: { children: React.ReactNode; onClick: () => void; danger?: boolean }) {
  return (
    <button onClick={onClick} style={{
      ...
      onMouseEnter={e => (e.currentTarget.style.background = danger ? (document.documentElement.classList.contains('dark') ? 'rgba(220,38,38,0.10)' : 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))') : (document.documentElement.classList.contains('dark') ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : 'rgba(15,23,42,0.04)'))}
