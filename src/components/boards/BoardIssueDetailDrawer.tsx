...
function OverviewTab({ issue }: { issue: any }) {
  ...
  return (
    <div className="idp-overview">
      ...
      {/* Description */}
      <div>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--fg-1)', marginBottom: 8, fontFamily: 'var(--ds-font-family-heading)' }}>
          Description
        </h3>
        <div style={{
        ...
        {details.map((f, idx) => {
          ...
          return (
            <div key={f.label} className={`idp-field-cell ${isOdd ? 'idp-field-cell--odd' : ''} ${isLastRow ? 'idp-field-cell--last' : ''}`}>
              <div className="idp-field-label">{f.label}</div>
              <div className="idp-field-value">
                <span style={{ fontSize: 13, fontWeight: 500, color: f.value ? 'var(--fg-1)' : 'var(--fg-4)', fontFamily: f.label.includes('Key') ? 'var(--ds-font-family-monospaced)' : 'inherit' }}>
                  {f.value || '—'}
                </span>
              </div>
            </div>
