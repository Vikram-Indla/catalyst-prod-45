    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <h2 style={{
            fontSize: 15, fontFamily: 'var(--ds-font-family-heading)', fontWeight: 700,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
            {TEMPLATES.map(t => (
              <button key={t.id} onClick={() => setTemplate(t.id)} style={{
                ...
                <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--fg-1)', fontFamily: 'var(--ds-font-family-body)', marginBottom: 6 }}>
                  {t.name}
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {t.cols.length > 0 ? t.cols.map(c => (
                    ...
                    <span style={{ fontSize: 10.5, color: 'var(--fg-4)', fontFamily: 'var(--ds-font-family-body)' }}>
                      Start from scratch
                    </span>
                  )}
                </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 20 }}>
            {SWIMLANE_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => setSwimlane(opt.value)} style={{
                ...
                <div>
                  <div style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--fg-1)', fontFamily: 'var(--ds-font-family-body)' }}>
                    {opt.label}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--fg-3)', fontFamily: 'var(--ds-font-family-body)', marginTop: 1 }}>
                    {opt.desc}
                  </div>
                </div>
              </button>
              ...
              <button key={opt.value} onClick={() => setVisibility(opt.value)} style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--fg-1)', fontFamily: 'var(--ds-font-family-body)' }}>
                      {opt.label}
                    </span>
                    {opt.warning && (
