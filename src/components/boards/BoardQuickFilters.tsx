...
export default function BoardQuickFilters() {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingBottom: 8 }}>
      ...
      {/* Separator */}
      <span style={{ width: 0.75, height: 18, background: 'var(--ds-shadow-overlay)' }} />
      {/* Filters dropdown trigger */}
      <button style={{
        display: 'inline-flex', alignItems: 'center', gap: 4,
        height: 26, padding: '0 10px', borderRadius: 13,
        border: 'none', background: 'var(--cp-bd-zone)',
        color: 'var(--fg-2)', fontSize: 'var(--ds-font-size-200)', fontWeight: 500,
        fontFamily: 'var(--cp-font-body)', cursor: 'pointer',
      }}>
        <SlidersHorizontal size={12} /> Filters
      </button>
      {/* Sync indicator */}
      <span style={{ marginLeft: 'auto', fontSize: 'var(--ds-font-size-100)', color: 'var(--fg-4)', fontFamily: 'var(--cp-font-body)' }}>
        synced: just now
      </span>
    </div>
  );
