...
export function DiscussionsViewTab({ data }: DiscussionsViewTabProps) {
  ...
  const formatMessage = (message: string) => {
    ...
    formatted = formatted.replace(/@\[([^\]]+)\]\([^)]+\)/g, (_, name) => {
      return `<span class="text-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))] font-medium">@${name}</span>`;
    });
    return formatted;
  };
      ...
      <div className="border-t p-4 bg-muted/30 shrink-0">
        <div className="flex gap-2">
          ...
          <Button
            ...
            className="bg-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))] hover:bg-[var(--ds-background-brand-bold-hovered,#1d4ed8)] text-white self-end"
          >
