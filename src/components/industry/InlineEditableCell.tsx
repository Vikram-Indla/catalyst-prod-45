...
import { CalendarIcon, Check, X } from '@/lib/atlaskit-icons';
...
export function InlineEditableCell({
  ...
  // Fetch process steps dynamically
  ...
  const getOptions = () => {
    ...
    switch (field) {
      case 'process_step':
        return processStepOptions.map(s => ({ value: s.value, label: s.label }));
  ...
  const renderDisplay = () => (
    ...
    >
      {displayValue || <span className="text-[14px] text-[var(--ds-text,var(--cp-text-primary, var(--cp-text-inverse, #172B4D)))] truncate">{value || '-'}</span>}
    </div>
  );
          ...
          <div className="max-h-[300px] overflow-y-auto">
            {getOptions().map((opt) => (
              <div
                key={opt.value}
                className={cn(
                  "px-3 py-2 cursor-pointer hover:bg-[var(--ds-surface-sunken,var(--cp-bg-sunken, #F4F5F7))] flex items-center gap-2 text-[14px]",
