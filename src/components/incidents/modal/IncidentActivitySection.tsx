        ...
        <div className="flex-1">
          {!isComposerExpanded ? (
            ...
            <div>
              <textarea
                autoFocus
                className="w-full border-2 border-[var(--ds-text-brand,var(--cp-workstream-catalyst-primary, #2563eb))] rounded p-3 text-sm text-[var(--ds-text,var(--cp-text-primary, var(--cp-text-inverse, #172B4D)))] outline-none min-h-[80px] resize-none"
            ...
            <div key={entry.id} className="flex gap-3 py-4 border-t border-[var(--ds-border,var(--cp-lozenge-grey-bg, var(--cp-border-neutral, #DFE1E6)))] first:border-t-0">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-baseline gap-1.5 mb-0.5">
                  <span className="text-sm font-semibold text-[var(--ds-text,var(--cp-text-primary, var(--cp-text-inverse, #172B4D)))]">{entry.author_name ?? 'System'}</span>
                  {entry.type === 'history' && entry.field_name && (
                    <span className="text-sm text-[var(--ds-text,var(--cp-text-primary, var(--cp-text-inverse, #172B4D)))]">
                ...
                {entry.type === 'comment' && entry.content && (
                  <div className="text-sm text-[var(--ds-text,var(--cp-text-primary, var(--cp-text-inverse, #172B4D)))] leading-5 mt-1 whitespace-pre-wrap">
                    {entry.content}
                  </div>
                )}
              </div>
