    ...
    <div className="bg-white border border-[#E8E8E8] rounded-lg overflow-hidden">
      ...
      <div className="p-4">
        ...
        {attachments.length > 0 && (
          <div className="space-y-2 mb-4">
            {attachments.map(attachment => (
              <div 
                key={attachment.id}
                className="flex items-center justify-between p-3 bg-[var(--ds-surface-sunken,#FAFBFC)] rounded-lg border border-[#E8E8E8] group hover:border-[var(--ds-text-brand,#2563eb)]"
              >
                <div className="flex items-center gap-3">
                  {getFileIcon(attachment.name)}
                  <div>
                    <p className="text-sm font-medium text-[var(--ds-text,#172B4D)]">{attachment.name}</p>
        ...
        <div
          ...
          className="border-2 border-dashed border-[var(--ds-border,var(--cp-lozenge-grey-bg, #DFE1E6))] rounded-lg p-6 text-center cursor-pointer hover:border-[var(--ds-text-brand,#2563eb)] hover:bg-[var(--ds-text-brand,#2563eb)]/5 transition-colors"
        >
          <Upload className="w-8 h-8 text-[#8C8C8C] mx-auto mb-2" />
          <p className="text-sm text-[#5C5C5C]">
            Drop files here or <span className="text-[var(--ds-text-brand,#2563eb)] font-medium">click to upload</span>
          </p>
