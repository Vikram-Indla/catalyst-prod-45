...
export function hubColour(hub?: string | null): string {
  switch ((hub ?? '').toLowerCase()) {
    case 'projecthub': return 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))';
    case 'producthub': return 'var(--ds-text-subtle)';
    case 'taskhub': return 'var(--ds-border)';
    case 'incidenthub': return 'var(--ds-text-danger, var(--cp-danger))';
    case 'testhub': return 'var(--cp-teal-60)';
    case 'releasehub': return 'var(--cp-purple-60)';
