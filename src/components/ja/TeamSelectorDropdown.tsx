import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { Input } from '@/components/ui/input';
import { Search, Users, Star } from '@/lib/atlaskit-icons';
import { ScrollArea } from '@/components/ui/scroll-area';
import { Skeleton } from '@/components/ui/skeleton';
import { useStarredItems } from '@/hooks/useStarredItems';

interface TeamSelectorDropdownProps {
  onClose: () => void;
}

export function TeamSelectorDropdown({ onClose }: TeamSelectorDropdownProps) {
  const navigate = useNavigate();
  const [search, setSearch] = useState('');
  const { isStarred, toggleStar } = useStarredItems();

  const { data: teams, isLoading } = useQuery({
    queryKey: ['teams-header'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('teams')
        .select(`
          id,
          name,
          project_id,
          projects!project_id (
            name
          )
        `)
        .eq('is_active', true)
        .order('name');
      
      if (error) throw error;
      return data || [];
    },
  });

  const filtered = (teams || []).filter(t =>
    t.name.toLowerCase().includes(search.toLowerCase())
  );

  const handleSelect = (teamId: string) => {
    navigate(`/teams/${teamId}/room`);
    onClose();
  };

  const handleToggleStar = async (e: React.MouseEvent, team: typeof filtered[0]) => {
    e.stopPropagation();
    await toggleStar({
      room_type: 'team',
      room_id: team.id,
      room_name: team.name,
      room_subtitle: team.projects?.name || 'Team',
      room_path: `/teams/${team.id}/room`,
      pi_label: null,
    });
  };

  return (
    <div className="w-80 bg-popover border rounded-md shadow-lg">
      <div className="p-3 border-b">
        <p className="text-xs font-semibold text-muted-foreground mb-2">TEAMS</p>
        <div className="relative">
          <Input
            type="text"
            placeholder="Search teams..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="pr-8 h-9"
          />
          <Search className="absolute right-2 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        </div>
      </div>
      <ScrollArea className="max-h-80">
        <div className="p-2">
          {isLoading ? (
            <div className="space-y-2">
              <Skeleton className="h-9 w-full" />
              <Skeleton className="h-9 w-full" />
              <Skeleton className="h-9 w-full" />
            </div>
          ) : filtered.length === 0 ? (
            <div className="px-3 py-8 text-center text-sm text-muted-foreground">
              {search ? 'No teams found' : 'No teams available'}
            </div>
          ) : (
            filtered.map((team) => {
              const starred = isStarred('team', team.id);
              return (
                <button
                  key={team.id}
                  onClick={() => handleSelect(team.id)}
                  className="w-full text-left px-3 py-2 rounded hover:bg-accent text-sm group"
                >
                    <div className="flex items-center gap-2">
                    <Users className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate">{team.name}</div>
                      {team.projects && (
                        <div className="text-xs text-muted-foreground truncate">
                          {team.projects.name}
                        </div>
                      )}
                    </div>
                    <button
                      onClick={(e) => handleToggleStar(e, team)}
                      className="p-1 rounded hover:bg-muted transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-brand-primary"
                      aria-label={starred ? "Unstar team" : "Star team"}
                    >
                      <Star
                        className={`h-4 w-4 ${
                          starred
                            ? "text-brand-primary fill-brand-primary"
                            : "text-muted-foreground hover:text-brand-primary"
                        }`}
                      />
                    </button>
                  </div>
                </button>
              );
            })
          )}
        </div>
      </ScrollArea>
    </div>
  );
}