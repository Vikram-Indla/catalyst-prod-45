  ...
  return (
    <>
      <Sheet open={isOpen} onOpenChange={(open) => !open && handleAttemptClose()}>
        <SheetContent side="right" hideClose className={`executive-drawer ${drawerWidthClass} p-0 flex flex-col overflow-hidden bg-white dark:bg-[var(--ds-surface,var(--ds-surface, #0A0A0A))]`}>
          <SheetHeader className="executive-drawer-header flex-col space-y-0 shrink-0 p-0 bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))]">
            ...
            <div className="flex items-center justify-between px-4 md:px-5 pt-4 pb-3 border-b border-brand-primary/50 bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))]">
              ...
              <div className="flex items-center gap-2.5 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <span className="text-sm font-medium text-brand-primary">Risk #{risk.risk_number}</span>
                  <button
            ...
            <div className="px-4 md:px-5 py-1.5 bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] shrink-0">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
            <TabsList className="executive-tabs-list w-full justify-start rounded-none border-b border-border h-10 shrink-0 overflow-x-auto flex-nowrap bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] px-4 md:px-5">
              {TABS.map((tab) => (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
            ...
            <div className="executive-drawer-content flex-1 flex flex-col min-h-0 overflow-y-auto">
              ...
              <TabsContent value="links" className="m-0 focus-visible:outline-none flex-1">
                <RiskLinksTab 
                  riskId={risk.id} 
                  businessRequestId={risk.business_request_id}
                  relatedItemId={risk.related_item_id}
          ...
          {/* Footer */}
          <div className="border-t px-4 md:px-5 py-3 bg-white dark:bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] flex gap-2 justify-end shrink-0">
