      ...
      <Sheet open={isOpen} onOpenChange={(open) => !open && handleAttemptClose()}>
        <SheetContent side="right" hideClose className={`executive-drawer ${drawerWidthClass} p-0 flex flex-col overflow-hidden bg-white dark:bg-[#0A0A0A]`}>
          <SheetHeader className="executive-drawer-header flex-col space-y-0 shrink-0 p-0 bg-white dark:bg-[#111111]">
            ...
            <div className="px-4 md:px-5 py-1.5 bg-white dark:bg-[#111111] shrink-0">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
            <TabsList className="executive-tabs-list w-full justify-start rounded-none border-b border-border h-10 shrink-0 overflow-x-auto flex-nowrap bg-white dark:bg-[#111111] px-4 md:px-5">
              {TABS.map((tab) => (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
          ...
          {/* Footer */}
          <div className="border-t px-4 md:px-5 py-3 bg-white dark:bg-[#111111] flex gap-2 justify-end shrink-0">
