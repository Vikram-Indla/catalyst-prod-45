    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-[480px] overflow-hidden">
        <div className="bg-[var(--ds-background-danger, #FEF2F2)] border-b border-[var(--ds-border-danger, #FCA5A5)] px-6 py-4 flex items-center gap-3">
          <AlertTriangle size={20} className="text-[var(--ds-text-danger, #DC2626)]" />
          <div>
            ...
            <p className="text-[12px] text-[var(--ds-text-danger, #991B1B)]">{chgNumber} cannot advance to Beta without linked work items.</p>
          </div>
        </div>
        <div className="p-6">
          {mode === 'choose' ? (
            <div className="space-y-3">
              ...
              <button onClick={onResolved}
                className="w-full h-11 rounded-md border border-[var(--bd-default, #E2E8F0)] text-[13px] font-medium text-[var(--ds-text-subtlest, #94A3B8)] hover:bg-[#F4F7FA]">
                Skip (RM only · logged)
              </button>
            </div>
          ) : (
            <div className="space-y-3">
              <div>
                <label className="block text-[12px] font-semibold text-[var(--ds-text-subtle, #475569)] mb-1">Work Item Key *</label>
              ...
              <div className="flex gap-2">
                <button onClick={() => setMode('choose')} className="flex-1 h-9 rounded-md border border-[var(--bd-default, #E2E8F0)] text-[13px] font-medium text-[var(--ds-text-subtle, #475569)]">Back</button>
