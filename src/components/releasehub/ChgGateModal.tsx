  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-[480px] overflow-hidden">
        <div className="bg-[rgba(248,113,113,0.06)] border-b border-[#FCA5A5] px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] text-[#F87171]">{chgNumber} cannot advance to Beta without linked work items.</p>
          </div>
        </div>
        <div className="p-6">
          {mode === 'choose' ? (
            <div className="space-y-3">
              ...
              <button onClick={onResolved}
                className="w-full h-11 rounded-md border border-[var(--bd-default, rgba(255,255,255,0.10))] text-[13px] font-medium text-[rgba(237,237,237,0.40)] hover:bg-[#F4F7FA]">
                Skip (RM only · logged)
              </button>
            </div>
          ) : (
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={workItemKey} onChange={e => setWorkItemKey(e.target.value)} placeholder="e.g. BAU-4612"
                  className="w-full h-9 px-3 rounded-md border border-[var(--bd-default, rgba(255,255,255,0.10))] text-[13px] font-mono" />
              </div>
              <div>
                ...
                <input type="text" value={workItemTitle} onChange={e => setWorkItemTitle(e.target.value)} placeholder="Describe the work item"
                  className="w-full h-9 px-3 rounded-md border border-[var(--bd-default, rgba(255,255,255,0.10))] text-[13px]" />
              </div>
              <div className="flex gap-2">
                <button onClick={() => setMode('choose')} className="flex-1 h-9 rounded-md border border-[var(--bd-default, rgba(255,255,255,0.10))] text-[13px] font-medium text-[#475569]">Back</button>
