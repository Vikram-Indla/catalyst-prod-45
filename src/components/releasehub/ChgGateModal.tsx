          ...
          {mode === 'choose' ? (
            <div className="space-y-3">
              ...
              <button onClick={onResolved}
                className="w-full h-11 rounded-md border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] text-[13px] font-medium text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))] hover:bg-[#F4F7FA]">
                Skip (RM only · logged)
              </button>
            </div>
          ) : (
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={workItemKey} onChange={e => setWorkItemKey(e.target.value)} placeholder="e.g. BAU-4612"
                  className="w-full h-9 px-3 rounded-md border border-[var(--bd-default,var(--cp-border, var(--cp-bg-sunken, #E2E8F0)))] text-[13px] font-mono" />
              </div>
              <div>
