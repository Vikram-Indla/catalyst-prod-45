...
const STATUS_MINI: Record<string, { bg: string; text: string }> = {
  ...
  todo: { bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))', text: 'var(--ds-text-subtle, #475569)' },
};
...
export function WorkItemTag({ workItemKey, title, type, status, onClick }: Props) {
  ...
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1.5 h-6 px-2 rounded border border-[var(--bd-default,var(--cp-border, #E2E8F0))] bg-white hover:bg-[#F0FDFA] hover:border-[#99F6E4] transition-colors max-w-[280px] group"
