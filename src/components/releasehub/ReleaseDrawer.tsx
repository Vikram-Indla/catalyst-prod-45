  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      <div className="relative w-[700px] h-full bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300"
        ...
        {/* Header */}
        <div className="sticky top-0 bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] z-10 border-b border-[rgba(15,23,42,0.12)] dark:border-[var(--ds-border,#2E2E2E)] px-6 py-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {editingDate ? (
              ...
              <button
                ...
                className="group flex items-center gap-1 text-[12px] text-[var(--ds-text-subtlest,#64748B)] hover:text-[var(--ds-text-brand,#2563EB)] transition-colors"
        ...
        <div className="border-b border-[rgba(15,23,42,0.12)] px-6 flex gap-0">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab}
              {tab === 'Changes' && <span className="ml-1 text-[10px] font-bold bg-[var(--ds-surface-sunken,#F1F5F9)] text-[var(--ds-text-subtle,#475569)] px-1 rounded">{relChanges.length}</span>}
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-[var(--ds-surface-sunken,#F8FAFC)] rounded-lg p-3">
          <p className="text-[10px] font-bold uppercase text-[var(--ds-text-subtlest,#64748B)] mb-1">Status</p>
          <StatusLozenge status={release.status === 'todo' ? 'planning' : release.status === 'done' ? 'released' : release.status} />
        </div>
        <div className="bg-[var(--ds-surface-sunken,#F8FAFC)] rounded-lg p-3">
          <p className="text-[10px] font-bold uppercase text-[var(--ds-text-subtlest,#64748B)] mb-1">Changes</p>
          <p className="text-[20px] font-extrabold" style={{ fontFamily: RH.fontMono, color: RH.ink1 }}>{changesCount}</p>
        </div>
        <div className="bg-[var(--ds-surface-sunken,#F8FAFC)] rounded-lg p-3">
          <p className="text-[10px] font-bold uppercase text-[var(--ds-text-subtlest,#64748B)] mb-1">Quality Gates</p>
        ...
        {notesState === 'success' && generatedNotes && (
          <div>
            ...
            <div className="flex items-center gap-3 mt-3 pt-3 border-t border-[var(--ds-surface-sunken,#F1F5F9)]">
              ...
              <button
                onClick={generateReleaseNotes}
                className="text-[12px] text-[var(--ds-text-subtlest,#64748B)] hover:text-[var(--ds-text-subtle,#475569)] focus-visible:ring-2 focus-visible:ring-[var(--ds-text-brand,#2563EB)] focus-visible:ring-offset-2 outline-none rounded"
              >
                Regenerate
              </button>
            </div>
      ...
      <thead>
        <tr className="border-b border-[rgba(15,23,42,0.12)]">
          {['KEY', 'TITLE', 'STATUS', 'RISK', 'DATE'].map(h => (
            <th key={h} className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--ds-text-subtlest,#64748B)]">{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
    ...
    <div>
            ...
            return (
              <div key={tc.id} className="border border-[rgba(15,23,42,0.12)] rounded-lg p-4 group relative">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[11px] text-[var(--ds-text-subtlest,#64748B)]">{passCount}/{totalCases} cases</span>
                </div>
                <div className="w-full h-2 bg-[var(--ds-surface-sunken,#F1F5F9)] rounded-full overflow-hidden">
      ...
      <Dialog open={showLinkModal} onOpenChange={setShowLinkModal}>
        <DialogContent className="sm:max-w-[480px]">
              ...
              return (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[11px] text-[var(--ds-text-subtlest,#64748B)]">{cycle.total_cases ?? 0} cases</span>
                  </div>
                  <StatusLozenge status={cycle.status || 'not_started'} />
          ...
          <DialogFooter>
            <button onClick={() => setShowLinkModal(false)} className="h-8 px-4 rounded-md text-[13px] font-medium text-[var(--ds-text-subtlest,#64748B)] hover:bg-[var(--ds-surface-sunken,#F1F5F9)]">Close</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
      ...
      <div className="space-y-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="flex gap-3 animate-pulse">
            <div className="w-7 h-7 rounded-full bg-[var(--ds-surface-sunken,#F1F5F9)] flex-shrink-0" />
            <div className="flex-1 space-y-2">
              <div className="h-3 bg-[var(--ds-surface-sunken,#F1F5F9)] rounded w-3/4" />
              <div className="h-2.5 bg-[var(--ds-surface-sunken,#F1F5F9)] rounded w-1/2" />
            </div>
          </div>
        ))}
      </div>
