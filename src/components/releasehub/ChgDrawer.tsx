        ...
        <div className="relative w-[700px] h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300"
          ...
          <div className="sticky top-0 bg-white z-10 border-b border-[rgba(15,23,42,0.12)] px-6 py-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={onClose} aria-label="Close drawer" className="w-7 h-7 rounded flex items-center justify-center text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] hover:bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F1F5F9))]"><X size={14} /></button>
            </div>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              {c.category && <span className="text-[12px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748B))]">{c.category}</span>}
              ...
              {nextStatus && (
                <button onClick={() => handleAdvanceStatus(nextStatus)}
                  ...
                  className="ml-auto h-7 px-3 rounded-md bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] text-white text-[11px] font-bold flex items-center gap-1 hover:bg-[var(--ds-background-brand-bold-hovered,var(--ds-background-brand-bold-hovered, #1D4ED8))] disabled:opacity-50">
              ...
              {CHG_STATUS_ORDER.map((s, i) => {
                ...
                return (
                  <React.Fragment key={s}>
                    {i > 0 && <div className={`flex-1 h-0.5 ${isDone || isCurrent ? 'bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]' : 'bg-[rgba(15,23,42,0.12)]'}`} />}
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 ${
                      isDone ? 'bg-[#1B7F37] text-white' : isCurrent ? 'bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] text-white' : 'bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F1F5F9))] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] border border-[rgba(15,23,42,0.12)]'
                    }`}>
          ...
          <div className="border-b border-[rgba(15,23,42,0.12)] px-6 flex gap-0">
            {TABS.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                {tab}
                {tab === 'Work Items' && workItems.length > 0 && <span className="ml-1 text-[10px] font-bold bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F1F5F9))] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))] px-1 rounded">{workItems.length}</span>}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {c.pending_signoffs > 0 && (
        <div className="rounded-lg p-3 flex items-center gap-2" style={{ background: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))', border: '1px solid #FCA5A5' }}>
...
function WorkItemsTab({ workItems, changeId }: { workItems: any[]; changeId: string }) {
  ...
  return (
    <div className="space-y-3">
      {workItems.length === 0 && !showLink && <div className="text-center py-6 text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] text-[13px]">No work items linked</div>}
      ...
      {showLink ? (
        <div className="border border-[rgba(15,23,42,0.12)] rounded-lg p-3 space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowLink(false)} className="h-7 px-3 rounded border border-[rgba(15,23,42,0.12)] text-[11px] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">Cancel</button>
            <button onClick={handleLink} disabled={!key || !title || linkWorkItem.isPending}
              className="h-7 px-3 rounded bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] text-white text-[11px] font-bold disabled:opacity-50">
              {linkWorkItem.isPending ? 'Linking...' : 'Link'}
            </button>
          </div>
        </div>
      ) : (
        <button onClick={() => setShowLink(true)} className="h-8 px-3 rounded-md border border-[#DBEAFE] text-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] text-[12px] font-semibold hover:bg-[var(--ds-background-selected,var(--ds-background-selected, #EFF6FF))]">
          + Link Work Item
        </button>
      )}
    </div>
        ...
        return (
          <div key={s.stage} className="flex items-center gap-3 py-3 border-b border-[rgba(15,23,42,0.06)] last:border-0">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-bold shrink-0 ${
              ...
              status === 'rejected' ? 'bg-[var(--ds-background-danger,var(--ds-background-danger, #FEF2F2))] text-[var(--ds-text-danger,var(--ds-text-danger, #DC2626))]' :
            ...
            {signoff && status === 'pending' && (
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => rejectSignoff.mutate({ signoffId: signoff.id, comment: 'Rejected' }, { onSuccess: () => toast.success('Rejected') })}
                  ...
                  className="h-7 px-3 rounded border border-[var(--ds-border-danger,var(--ds-border-danger, #FCA5A5))] text-[var(--ds-text-danger,var(--ds-text-danger, #DC2626))] text-[11px] font-bold hover:bg-[var(--ds-background-danger,var(--ds-background-danger, #FEF2F2))] disabled:opacity-50">Reject</button>
              </div>
            )}
          </div>
        );
      })}
      {signoffs.length === 0 && <div className="text-center py-6 text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] text-[13px]">No sign-offs configured for this change</div>}
    </div>
  );
}
...
function ActivityTab({ changeId }: { changeId: string }) {
  ...
  if (isLoading) return <div className="text-center py-6 text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] text-[13px]">Loading...</div>;
  if (history.length === 0) return <div className="text-center py-6 text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] text-[13px]">No activity yet</div>;

  return (
    <div className="space-y-2">
      {history.map((h: any) => (
        <div key={h.id} className="flex items-start gap-3 py-2 border-b border-[rgba(15,23,42,0.06)] last:border-0">
          <div className="w-2 h-2 rounded-full bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] mt-1.5 shrink-0" />
          <div>
            <p className="text-[12px] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">
            ...
            {h.comment && <p className="text-[11px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] mt-0.5">{h.comment}</p>}
            <p className="text-[10px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))] mt-0.5">{h.changed_at ? format(new Date(h.changed_at), 'MMM d, yyyy HH:mm') : ''}</p>
          </div>
        </div>
      ))}
    </div>
