        ...
        <div className="relative w-[700px] h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300"
          ...
          <div className="sticky top-0 bg-white z-10 border-b border-[rgba(15,23,42,0.12)] px-6 py-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={onClose} aria-label="Close drawer" className="w-7 h-7 rounded flex items-center justify-center text-[rgba(237,237,237,0.40)] hover:bg-[#1A1A1A]"><X size={14} /></button>
            </div>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              {c.category && <span className="text-[12px] text-[rgba(237,237,237,0.40)]">{c.category}</span>}
                ...
                return (
                  <React.Fragment key={s}>
                    ...
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 ${
                      isDone ? 'bg-[#1B7F37] text-white' : isCurrent ? 'bg-[#2563EB] text-white' : 'bg-[#1A1A1A] text-[rgba(237,237,237,0.40)] border border-[rgba(15,23,42,0.12)]'
                    }`}>
          ...
          <div className="border-b border-[rgba(15,23,42,0.12)] px-6 flex gap-0">
            {TABS.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                {tab}
                {tab === 'Work Items' && workItems.length > 0 && <span className="ml-1 text-[10px] font-bold bg-[#1A1A1A] text-[#475569] px-1 rounded">{workItems.length}</span>}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {c.pending_signoffs > 0 && (
        <div className="rounded-lg p-3 flex items-center gap-2" style={{ background: 'rgba(248,113,113,0.06)', border: '1px solid #FCA5A5' }}>
...
function WorkItemsTab({ workItems, changeId }: { workItems: any[]; changeId: string }) {
  ...
  return (
    <div className="space-y-3">
      {workItems.length === 0 && !showLink && <div className="text-center py-6 text-[rgba(237,237,237,0.40)] text-[13px]">No work items linked</div>}
      ...
      {showLink ? (
        ...
        <button onClick={() => setShowLink(true)} className="h-8 px-3 rounded-md border border-[#DBEAFE] text-[#2563EB] text-[12px] font-semibold hover:bg-[rgba(59,130,246,0.06)]">
          + Link Work Item
        </button>
      )}
    </div>
        ...
        return (
          <div key={s.stage} className="flex items-center gap-3 py-3 border-b border-[rgba(15,23,42,0.06)] last:border-0">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-bold shrink-0 ${
              ...
              status === 'rejected' ? 'bg-[rgba(248,113,113,0.06)] text-[#DC2626]' :
            ...
            {signoff && status === 'pending' && (
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => rejectSignoff.mutate({ signoffId: signoff.id, comment: 'Rejected' }, { onSuccess: () => toast.success('Rejected') })}
                  ...
                  className="h-7 px-3 rounded border border-[#FCA5A5] text-[#DC2626] text-[11px] font-bold hover:bg-[rgba(248,113,113,0.06)] disabled:opacity-50">Reject</button>
              </div>
            )}
          </div>
        );
      })}
      {signoffs.length === 0 && <div className="text-center py-6 text-[rgba(237,237,237,0.40)] text-[13px]">No sign-offs configured for this change</div>}
    </div>
  );
}
...
function ActivityTab({ changeId }: { changeId: string }) {
  ...
  if (isLoading) return <div className="text-center py-6 text-[rgba(237,237,237,0.40)] text-[13px]">Loading...</div>;
  if (history.length === 0) return <div className="text-center py-6 text-[rgba(237,237,237,0.40)] text-[13px]">No activity yet</div>;
      ...
      {history.map((h: any) => (
        <div key={h.id} className="flex items-start gap-3 py-2 border-b border-[rgba(15,23,42,0.06)] last:border-0">
          ...
          <div>
            ...
            {h.comment && <p className="text-[11px] text-[rgba(237,237,237,0.40)] mt-0.5">{h.comment}</p>}
            <p className="text-[10px] text-[rgba(237,237,237,0.40)] mt-0.5">{h.changed_at ? format(new Date(h.changed_at), 'MMM d, yyyy HH:mm') : ''}</p>
          </div>
        </div>
      ))}
    </div>
