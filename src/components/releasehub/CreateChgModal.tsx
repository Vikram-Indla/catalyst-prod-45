...
export function CreateChgModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-[520px] max-h-[85vh] overflow-y-auto" style={{ borderRadius: 8 }} onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-6 py-4 border-b border-[rgba(15,23,42,0.12)]">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <button onClick={() => setCatOpen(!catOpen)} className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] bg-white text-[13px] text-left flex items-center justify-between hover:bg-[#F8FAFC]">
                <span className={category ? 'text-[#1E293B]' : 'text-[#94A3B8]'}>{category || 'Select category...'}</span>
                <ChevronDown size={12} className="text-[#94A3B8]" />
              </button>
              {catOpen && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white rounded-md shadow-lg border border-[rgba(15,23,42,0.12)] z-50 py-1">
                  {CATEGORIES.map(c => (
          ...
          <div>
            ...
            <div className="relative">
              <button onClick={() => setRelOpen(!relOpen)} className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] bg-white text-[13px] text-left flex items-center justify-between hover:bg-[#F8FAFC]">
                <span className={releaseId ? 'text-[#1E293B]' : 'text-[#94A3B8]'}>{releases.find((r: any) => r.id === releaseId)?.name || 'Select release...'}</span>
                <ChevronDown size={12} className="text-[#94A3B8]" />
              </button>
              {relOpen && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white rounded-md shadow-lg border border-[rgba(15,23,42,0.12)] z-50 py-1 max-h-48 overflow-y-auto">
        ...
        <div className="sticky bottom-0 bg-white border-t border-[rgba(15,23,42,0.12)] px-6 py-3 flex justify-end gap-2">
