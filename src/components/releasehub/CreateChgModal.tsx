        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <button onClick={() => setCatOpen(!catOpen)} className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] dark:border-[rgba(255,255,255,0.08)] bg-white dark:bg-[#1A1A1A] text-[13px] text-left flex items-center justify-between hover:bg-[#F8FAFC] dark:hover:bg-[#1A1A1A]">
                <span className={category ? 'text-[#1E293B]' : 'text-[var(--fg-3, #94A3B8)]'}>{category || 'Select category...'}</span>
          ...
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Describe the change..."
              className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] text-[13px] placeholder:text-[var(--fg-3, #94A3B8)] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]" />
          </div>
          <div>
            <label className="block text-[12px] font-semibold text-[#475569] mb-1">Planned Date *</label>
            <Popover>
              <PopoverTrigger asChild>
                <button className={`w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] text-[13px] text-left flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB] ${deployDate ? 'text-[#1E293B]' : 'text-[var(--fg-3, #94A3B8)]'}`}>
          ...
          <div>
            ...
            <div className="relative">
              <button onClick={() => setRelOpen(!relOpen)} className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] dark:border-[rgba(255,255,255,0.08)] bg-white dark:bg-[#1A1A1A] text-[13px] text-left flex items-center justify-between hover:bg-[#F8FAFC] dark:hover:bg-[#1A1A1A]">
                <span className={releaseId ? 'text-[#1E293B]' : 'text-[var(--fg-3, #94A3B8)]'}>{releases.find((r: any) => r.id === releaseId)?.name || 'Select release...'}</span>
                <ChevronDown size={12} className="text-[var(--fg-3, #94A3B8)]" />
              </button>
              {relOpen && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white dark:bg-[#1A1A1A] rounded-md shadow-lg border border-[rgba(15,23,42,0.12)] dark:border-[rgba(255,255,255,0.08)] z-50 py-1 max-h-48 overflow-y-auto">
                  <button onClick={() => { setReleaseId(''); setRelOpen(false); }} className="w-full px-3 h-9 text-left text-[13px] font-medium hover:bg-[#F8FAFC] text-[var(--fg-3, #94A3B8)]">None</button>
                  {releases.map((r: any) => (
