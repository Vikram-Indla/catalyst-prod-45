  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-xl shadow-2xl w-[520px] max-h-[85vh] overflow-y-auto" style={{ borderRadius: 8 }} onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] z-10 flex items-center justify-between px-6 py-4 border-b border-[rgba(15,23,42,0.12)] dark:border-[var(--ds-border,#2E2E2E)]">
          <h2 className="text-[16px] font-extrabold" style={{ fontFamily: RH.fontDisplay, color: RH.ink1 }}>New Change</h2>
          <button onClick={onClose} className="w-7 h-7 rounded flex items-center justify-center text-[var(--ds-text-subtlest,#94A3B8)] hover:bg-[var(--ds-surface-sunken,#F1F5F9)]"><X size={14} /></button>
        </div>
        <div className="p-6 space-y-4">
          {error && <div className="px-3 py-2 rounded-md bg-[var(--ds-background-danger,#FEF2F2)] text-[var(--ds-text-danger,#DC2626)] text-[12px] font-medium">{error}</div>}
          <div>
            <label className="block text-[12px] font-semibold text-[var(--ds-text-subtle,#475569)] mb-1">Category</label>
            <div className="relative">
              <button onClick={() => setCatOpen(!catOpen)} className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] dark:border-[var(--ds-border,#2E2E2E)] bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] text-[13px] text-left flex items-center justify-between hover:bg-[var(--ds-surface-sunken,#F8FAFC)] dark:hover:bg-[var(--ds-surface-raised,#1A1A1A)]">
                <span className={category ? 'text-[#1E293B]' : 'text-[var(--ds-text-subtlest,#94A3B8)]'}>{category || 'Select category...'}</span>
                <ChevronDown size={12} className="text-[var(--ds-text-subtlest,#94A3B8)]" />
              </button>
              {catOpen && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-md shadow-lg border border-[rgba(15,23,42,0.12)] dark:border-[var(--ds-border,#2E2E2E)] z-50 py-1">
                  {CATEGORIES.map(c => (
          ...
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Describe the change..."
              className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] text-[13px] placeholder:text-[var(--ds-text-subtlest,#94A3B8)] focus:outline-none focus:ring-2 focus:ring-[var(--ds-text-brand,#2563EB)]/20 focus:border-[var(--ds-text-brand,#2563EB)]" />
          </div>
          <div>
            <label className="block text-[12px] font-semibold text-[var(--ds-text-subtle,#475569)] mb-1">Planned Date *</label>
            <Popover>
              <PopoverTrigger asChild>
                <button className={`w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] text-[13px] text-left flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-[var(--ds-text-brand,#2563EB)]/20 focus:border-[var(--ds-text-brand,#2563EB)] ${deployDate ? 'text-[#1E293B]' : 'text-[var(--ds-text-subtlest,#94A3B8)]'}`}>
                  {deployDate ? format(deployDate, 'MMM d, yyyy') : 'Select date...'}
                  <CalendarIcon size={14} className="text-[var(--ds-text-subtlest,#94A3B8)]" />
                </button>
              </PopoverTrigger>
          ...
          <div>
            ...
            <div className="relative">
              <button onClick={() => setRelOpen(!relOpen)} className="w-full h-9 px-3 rounded-md border border-[rgba(15,23,42,0.12)] dark:border-[var(--ds-border,#2E2E2E)] bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] text-[13px] text-left flex items-center justify-between hover:bg-[var(--ds-surface-sunken,#F8FAFC)] dark:hover:bg-[var(--ds-surface-raised,#1A1A1A)]">
                <span className={releaseId ? 'text-[#1E293B]' : 'text-[var(--ds-text-subtlest,#94A3B8)]'}>{releases.find((r: any) => r.id === releaseId)?.name || 'Select release...'}</span>
                <ChevronDown size={12} className="text-[var(--ds-text-subtlest,#94A3B8)]" />
              </button>
              {relOpen && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white dark:bg-[var(--ds-surface-raised,#1A1A1A)] rounded-md shadow-lg border border-[rgba(15,23,42,0.12)] dark:border-[var(--ds-border,#2E2E2E)] z-50 py-1 max-h-48 overflow-y-auto">
                  <button onClick={() => { setReleaseId(''); setRelOpen(false); }} className="w-full px-3 h-9 text-left text-[13px] font-medium hover:bg-[var(--ds-surface-sunken,#F8FAFC)] text-[var(--ds-text-subtlest,#94A3B8)]">None</button>
                  {releases.map((r: any) => (
