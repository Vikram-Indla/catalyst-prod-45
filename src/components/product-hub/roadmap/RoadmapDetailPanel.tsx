...
const HEALTH_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  on_track: { label: 'On Track', color: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: '#F0FDF4' },
  at_risk: { label: 'At Risk', color: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', bg: '#FFFBEB' },
  off_track: { label: 'Off Track', color: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))', bg: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))' },
};

const STATUS_PILL: Record<string, { color: string; bg: string }> = {
  Planned: { color: 'var(--ds-background-brand-bold-hovered, var(--ds-background-brand-bold-hovered, #1D4ED8))', bg: '#DBEAFE' },
  Active: { color: '#0F766E', bg: '#F0FDFA' },
  Completed: { color: 'var(--ds-text-success, var(--ds-text-success, #16A34A))', bg: '#F0FDF4' },
  Cancelled: { color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', bg: 'var(--ds-background-danger, var(--ds-background-danger, #FEF2F2))' },
  'On Hold': { color: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))', bg: '#FFFBEB' },
  'Under Review': { color: '#7C3AED', bg: '#F5F3FF' },
  Approved: { color: '#0D9488', bg: '#F0FDFA' },
  New: { color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', bg: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))' },
};
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="text-[11px] font-semibold uppercase tracking-[0.05em] mb-1" style={{ color: 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))' }}>
        {label}
      </div>
...
export function RoadmapDetailPanel({ item, isOpen, onClose }: RoadmapDetailPanelProps) {
  ...
  const statusInfo = STATUS_PILL[item.status] || { color: 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))', bg: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F1F5F9))' };
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {activeTab === 'details' && (
            <div className="p-5 space-y-5">
              {/* Description */}
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-[0.05em] mb-1" style={{ color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))' }}>
                  Description
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-md flex items-center justify-center"
                      ...
                      <Map className="w-4 h-4" style={{ color: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' }} />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {item.ownerName !== 'Unassigned' && (
                      <div
                        ...
                        style={{ background: 'var(--ds-text-warning, var(--ds-text-warning, #D97706))' }}
                      >
