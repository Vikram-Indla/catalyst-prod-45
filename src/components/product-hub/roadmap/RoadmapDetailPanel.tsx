...
export function RoadmapDetailPanel({ item, isOpen, onClose }: RoadmapDetailPanelProps) {
  ...
  const statusInfo = STATUS_PILL[item.status] || { color: 'var(--cp-ink-2, var(--cp-ink-2, #334155))', bg: 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))' };
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {activeTab === 'details' && (
            <div className="p-5 space-y-5">
              {/* Description */}
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-[0.05em] mb-1" style={{ color: 'var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light, #94A3B8)))' }}>
                  Description
                </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {item.ownerName !== 'Unassigned' && (
                      <div
                        ...
                        style={{ background: 'var(--ds-text-warning, var(--cp-warning, #D97706))' }}
                      >
