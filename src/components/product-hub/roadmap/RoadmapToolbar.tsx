    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div className="inline-flex items-center" style={{ border: `1.5px solid ${surface.border}`, borderRadius: 6, overflow: 'hidden' }}>
          {ZOOM_OPTIONS.map(z => (
            <button
              ...
              style={{
                background: zoom === z ? 'var(--ds-text-brand, var(--cp-workstream-catalyst-primary))' : surface.card,
                color: zoom === z ? 'var(--cp-bg-elevated, var(--cp-bg-elevated, var(--cp-bg-elevated)))' : ink[2],
                fontWeight: zoom === z ? 600 : 500,
                borderRight: `1px solid ${surface.border}`,
                transition: 'background-color 0.15s ease, color 0.15s ease',
              }}
              onMouseEnter={e => { if (zoom !== z) e.currentTarget.style.background = isDark ? 'var(--ds-border, var(--cp-ink-1))' : surface.page; }}
        ...
        <button
          ...
          onMouseEnter={e => { e.currentTarget.style.background = isDark ? 'var(--ds-border, var(--cp-ink-1))' : surface.page; e.currentTarget.style.borderColor = 'var(--cp-border-strong)'; }}
        ...
        <button
          ...
          style={{
            border: '1.5px solid var(--ds-background-danger-bold)',
            borderRadius: 6,
            color: 'var(--ds-text-danger, var(--cp-danger))',
            fontWeight: 600,
            transition: 'all 0.15s ease',
          }}
          onMouseEnter={e => { e.currentTarget.style.background = 'var(--cp-danger-light)'; }}
