    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div className="inline-flex items-center" style={{ border: `1.5px solid ${surface.border}`, borderRadius: 6, overflow: 'hidden' }}>
          {ZOOM_OPTIONS.map(z => (
            <button
              ...
              style={{
                background: zoom === z ? 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))' : surface.card,
                color: zoom === z ? 'var(--ds-text-inverse, #FFFFFF)' : ink[2],
                fontWeight: zoom === z ? 600 : 500,
                borderRight: `1px solid ${surface.border}`,
                transition: 'background-color 0.15s ease, color 0.15s ease',
              }}
              onMouseEnter={e => { if (zoom !== z) e.currentTarget.style.background = isDark ? 'var(--ds-border, var(--ds-border, #292929))' : surface.page; }}
        ...
        <button
          ...
          onMouseEnter={e => { e.currentTarget.style.background = isDark ? 'var(--ds-border, var(--ds-border, #292929))' : surface.page; e.currentTarget.style.borderColor = 'var(--cp-border-strong, #CBD5E1)'; }}
