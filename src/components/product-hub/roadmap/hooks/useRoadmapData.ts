...
const OWNER_PALETTE = ['var(--ds-text-brand, #2563EB)', 'var(--cp-purple-60, #7C3AED)', 'var(--cp-teal-60, #0D9488)', '#EC4899', 'var(--ds-text-warning, var(--cp-warning, #D97706))', 'var(--quality-high, #059669)', '#E11D48', '#6366F1'];
...
function ownerColorFromName(name: string): string {
  if (!name || name === 'Unassigned') return 'var(--ds-text-subtlest, var(--cp-ink-4, #94A3B8))';
  let hash = 0;
