...
export function useRoadmapFilters(allItems: RoadmapRequest[]) {
  ...
  const groups = useMemo<RoadmapGroup[]>(() => {
    if (groupBy === 'none') return [{ key: 'all', label: 'All Business Requests', color: 'var(--ds-text-subtlest, var(--cp-ink-3, var(--cp-text-secondary)))', items: filtered, isExpanded: true }];
