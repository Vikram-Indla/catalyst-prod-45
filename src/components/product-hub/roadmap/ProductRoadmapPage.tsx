...
export function ProductRoadmapPage() {
  ...
  const { requests, stats, isLoading, error } = useRoadmapData();
  ...
  const allAsTimeline = useMemo((): TimelineRequest[] => {
  ...
  const handleToggleStar = useCallback((id: string, isCurrentlyStarred: boolean) => {
    toggleStar.mutate({ requestId: id, isCurrentlyStarred });
  }, [toggleStar]);
      ...
      <div className="flex flex-col h-full" style={{ fontFamily: FONT.body }}>
        ...
        <div className="flex flex-col items-center justify-center flex-1 gap-3">
          <div className="flex items-center gap-2 px-4 py-3 rounded-lg" style={{ background: 'var(--cp-danger-light, #FEF2F2)', border: `1px solid ${'var(--cp-danger-light, #FECACA)'}` }}>
            <AlertCircle size={18} style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #EF4444))' }} />
          ...
          <button
            ...
            onMouseEnter={e => (e.currentTarget.style.backgroundColor = isDark ? 'var(--ds-border, var(--ds-border, #292929))' : surface.page)}
      ...
      {isFullscreen && (
        <div className="flex items-center justify-between px-4 py-1.5" style={{ background: surface.page, borderBottom: `1px solid ${surface.border}` }}>
          ...
          <button
            ...
            style={{ border: `1px solid ${surface.border}`, color: ink[2], transition: 'background-color 0.15s ease' }}
            onMouseEnter={e => (e.currentTarget.style.backgroundColor = isDark ? 'var(--ds-border, var(--ds-border, #292929))' : surface.borderLight)}
            onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}
          >
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center flex-1 gap-4">
          ...
          <button
            ...
            style={{ background: 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))', color: 'var(--ds-text-inverse, #FFFFFF)', transition: 'opacity 0.15s ease' }}
