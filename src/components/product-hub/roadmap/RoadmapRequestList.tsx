  ...
  if (isUnassigned) {
    return (
      <div
        ...
        style={{ width: 22, height: 22, background: 'var(--ds-border, var(--ds-border, #E2E8F0))' }}
        title="Unassigned"
      >
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))" strokeWidth="2" strokeLinecap="round">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span style={{
            ...
            background: isDark ? 'var(--ds-border, var(--ds-border, #292929))' : SURFACE.page, border: `1px solid ${surface.border}`,
...
function RequestRow({
  ...
  const typeColor = TYPE_COLORS[item.type]?.solid || 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))';
    ...
    >
      <button
        ...
        style={{
          ...
          color: item.starred ? 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' : ink[4],
          opacity: item.starred ? 1 : undefined,
          transition: 'color 0.15s ease, opacity 0.15s ease',
        }}
        onClick={e => { e.stopPropagation(); onToggleStar(); }}
        onMouseEnter={e => { e.currentTarget.style.color = 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))'; }}
        onMouseLeave={e => { if (!item.starred) e.currentTarget.style.color = ink[4]; }}
      >
        <Star className="w-3.5 h-3.5" fill={item.starred ? 'var(--ds-text-warning, var(--ds-text-warning, #F59E0B))' : 'none'} />
      </button>
