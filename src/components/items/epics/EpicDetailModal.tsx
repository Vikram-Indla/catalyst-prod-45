...
const EPIC_STATE_DISPLAY: Record<string, { label: string; bg: string; text: string }> = {
  not_started: { label: 'NOT STARTED', bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', text: 'var(--ds-text, var(--ds-text, #253858))' },
  funnel: { label: 'FUNNEL', bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', text: 'var(--ds-text, var(--ds-text, #253858))' },
  analyzing: { label: 'ANALYZING', bg: '#DEEBFF', text: '#0747A6' },
  backlog: { label: 'BACKLOG', bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', text: 'var(--ds-text, var(--ds-text, #253858))' },
...
const DL: React.CSSProperties = { width: 110, flexShrink: 0, fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', paddingTop: 5 };
const DV: React.CSSProperties = { flex: 1, fontSize: 13, color: 'var(--ds-text, var(--ds-text, #172B4D))' };
...
const menuItemStyle: React.CSSProperties = {
  ...
  fontSize: 14, color: 'var(--ds-text, var(--ds-text, #172B4D))', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit',
};
...
export function EpicDetailModal({ isOpen, onClose, epicId, onEpicChange }: EpicDetailModalProps) {
  ...
  const stateDisplay = EPIC_STATE_DISPLAY[stateKey] ?? { label: stateKey?.toUpperCase() ?? '—', bg: 'var(--ds-border, var(--ds-border, #DFE1E6))', text: 'var(--ds-text, var(--ds-text, #253858))' };
    ...
    <>
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <div ref={dotsMenuRef} style={{ position: 'relative' }}>
                <button onClick={() => setShowDotsMenu(!showDotsMenu)} style={{
                  background: showDotsMenu ? 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))' : 'none', border: 'none', cursor: 'pointer', padding: '6px 8px',
                ...
                {showDotsMenu && (
                  <div style={{ position: 'absolute', right: 0, top: 36, background: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', border: '1px solid #DFE1E6', borderRadius: 6, boxShadow: '0 4px 16px rgba(9,30,66,0.18)', padding: '6px 0', zIndex: 50, minWidth: 200, animation: 'edm-slide-down 0.15s ease-out' }}>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                ...
                <>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--ds-text, var(--ds-text, #172B4D))' }}>Key details</span>
                    </div>
                    <div>
                      <div style={DR}>
                  ...
                  {/* DESCRIPTION */}
                  <div style={{ marginBottom: 24 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ds-text, var(--ds-text, #172B4D))', marginBottom: 10 }}>Description</div>
                    <div
                      contentEditable suppressContentEditableWarning
                      onBlur={e => {
                      ...
                      style={{
                        ...
                        borderRadius: 4, fontSize: 14, color: 'var(--ds-text, var(--ds-text, #172B4D))', lineHeight: 1.7,
                      ...
                      onBlurCapture={e => { e.currentTarget.style.borderColor = 'var(--ds-border, var(--ds-border, #DFE1E6))'; e.currentTarget.style.boxShadow = 'none'; }}
                    >
                      {epic?.description || ''}
                    </div>
                  </div>
                  ...
                  <div style={{ borderTop: '1px solid #EBECF0', paddingTop: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
                      <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--ds-text, var(--ds-text, #172B4D))' }}>Activity</span>
                      <div style={{ display: 'flex', background: 'var(--ds-surface-sunken, var(--ds-surface-sunken, #F4F5F7))', borderRadius: 4, overflow: 'hidden' }}>
                        {(['details', 'comments', 'history'] as const).map(tab => (
                          <button key={tab} onClick={() => setActiveTab(tab)} style={{
                            padding: '5px 14px', fontSize: 13, fontWeight: activeTab === tab ? 700 : 400,
                            color: activeTab === tab ? 'var(--ds-text, var(--ds-text, #172B4D))' : '#5E6C84', border: 'none', cursor: 'pointer',
                            background: activeTab === tab ? 'var(--ds-surface, var(--ds-surface, #FFFFFF))' : 'transparent',
            ...
            <div style={{
              ...
              background: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', overflowY: 'auto',
              ...
              {/* Detail rows */}
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>
                Details
              </div>
                  ...
                  {ownerProfile ? (
                    <>
                      {ownerProfile.avatar_url ? (
                        ...
                        <div style={{ width: 24, height: 24, borderRadius: '50%', background: getAvatarColor(ownerProfile.id), color: 'var(--ds-surface, var(--ds-surface, var(--ds-surface, #FFF)))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, flexShrink: 0 }}>
                          {getInitials(ownerProfile.full_name)}
                        </div>
                      )}
                      <span style={{ fontSize: 13, color: 'var(--ds-text, var(--ds-text, #172B4D))' }}>{ownerProfile.full_name}</span>
                    </>
                  ) : (
              ...
              {/* Dates */}
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12, marginTop: 8 }}>
                Dates
              </div>
              ...
              <div style={DR}>
                ...
                <span style={{ ...DV, fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))' }}>{fmtDate(epic?.created_at)}</span>
              </div>
              <div style={DR}>
                <span style={DL}>Updated</span>
                <span style={{ ...DV, fontSize: 12, color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #6B778C))' }}>{fmtDate(epic?.updated_at)}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {showConfirmDelete && (
        <div style={{
          ...
          <div style={{
            width: 480, background: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', borderRadius: 8, padding: 24,
            boxShadow: '0 8px 32px rgba(9,30,66,0.25)',
          }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--ds-text, var(--ds-text, #172B4D))', marginBottom: 8, fontFamily: 'var(--cp-font-heading)' }}>Delete epic?</h3>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => setShowConfirmDelete(false)} style={{
                ...
                background: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', fontSize: 14, fontWeight: 500, cursor: 'pointer', color: '#42526E',
              }}>Cancel</button>
              <button onClick={() => { deleteMutation.mutate(); setShowConfirmDelete(false); }} style={{
                padding: '8px 16px', borderRadius: 4, border: 'none',
                background: '#DE350B', color: 'var(--ds-surface, var(--ds-surface, #FFFFFF))', fontSize: 14, fontWeight: 600, cursor: 'pointer',
              }}>Delete</button>
            </div>
          </div>
        </div>
