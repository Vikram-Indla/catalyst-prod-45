  ...
  return (
    <>
      <Sheet open={open} onOpenChange={(isOpen) => !isOpen && handleAttemptClose()}>
        <SheetContent side="right" hideClose className={`executive-drawer ${drawerWidthClass} p-0 flex flex-col overflow-hidden bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))]`}>
          <SheetHeader className="executive-drawer-header flex-col space-y-0 shrink-0 p-0 bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))]">
            {/* Header row */}
            <div className="flex items-center justify-between px-4 md:px-5 pt-4 pb-3 border-b border-brand-primary/50 bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))]">
          ...
          {/* Status Row */}
          <div className="px-4 md:px-5 py-2 flex items-center bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] shrink-0">
            <button 
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
            <TabsList className="executive-tabs-list w-full justify-start rounded-none border-b border-border h-10 shrink-0 overflow-x-auto flex-nowrap bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] px-4 md:px-5">
              {EPIC_TABS.map((tab) => (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
