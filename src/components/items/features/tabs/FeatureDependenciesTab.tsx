/**
 * FeatureDependenciesTab - Bidirectional Dependencies View
 * 
 * Shows both Outgoing and Incoming dependencies for a Feature.
 * - Outgoing: This Feature depends on something else
 * - Incoming: Other work items depend on this Feature
 * 
...
interface FeatureDependenciesTabProps {
  feature?: Feature;
}

export function FeatureDependenciesTab({ feature }: FeatureDependenciesTabProps) {
  const [createDialogOpen, setCreateDialogOpen] = useState(false);
  const [selectedDependencyId, setSelectedDependencyId] = useState<string | null>(null);
  
  const featureId = feature?.id || '';
  const { outgoing, incoming, isLoading } = useWorkItemDependencies('feature', featureId);

  if (!feature?.id) {
    return (
      <div className="text-sm text-muted-foreground py-4 text-center">
        Save the feature first to add dependencies.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          This Feature depends on the following work items
        </p>
        {outgoing.length === 0 ? (
      ...
      <section>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          These work items depend on this Feature
        </p>
        {incoming.length === 0 ? (
          <div className="text-sm text-muted-foreground py-4 text-center border border-dashed rounded-md">
