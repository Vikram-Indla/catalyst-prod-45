...
interface DeleteFeatureStatusDialogProps {
  ...
  allStatuses: FeatureStatus[];
}

export function DeleteFeatureStatusDialog({
  open,
  onOpenChange,
  status,
  allStatuses,
}: DeleteFeatureStatusDialogProps) {
  const [newStatus, setNewStatus] = useState<string>('');
  
  const { data: linkedData, isLoading: isLoadingFeatures, refetch } = useLinkedFeatures(status?.value || null);
  const reassignMutation = useReassignFeatures();
  const deleteMutation = useDeleteFeatureStatus();
  ...
  const hasLinkedFeatures = (linkedData?.count || 0) > 0;
  const canDelete = !hasLinkedFeatures;
  ...
  const handleReassign = async () => {
    ...
    // Refresh linked features count
    refetch();
  };

  const handleDelete = async () => {
    if (!status || hasLinkedFeatures) return;
          ...
          {isLoadingFeatures ? (
            ...
            <>
              ...
              <div className="flex items-start gap-3 p-3 bg-destructive/10 border border-destructive/30 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    There are <strong>{linkedData?.count}</strong> feature(s) currently using this status. 
                  ...
                  <div className="space-y-1">
                    ...
                    {(linkedData?.count || 0) > 50 && (
                      <p className="text-xs text-muted-foreground italic pt-2">
                        ... and {(linkedData?.count || 0) - 50} more features
                      </p>
                    )}
                  </div>
                </ScrollArea>
              ...
              <div className="space-y-2 pt-2 border-t">
                ...
                <p className="text-xs text-muted-foreground">
                  This will update all {linkedData?.count} feature(s) to use the selected status.
                </p>
              </div>
            </>
          ) : (
            <div className="flex items-start gap-3 p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  No features are using this status. You can proceed with deletion.
                </p>
              </div>
            </div>
          )}
