...
const PERM_COLORS: Record<string, { bg: string; color: string; bgDark: string; colorDark: string }> = {
  ...
  none: { bg: 'var(--ds-surface-sunken, #F1F5F9)', color: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', bgDark: 'var(--ds-surface-raised, #1A1A1A)', colorDark: 'var(--ds-text-subtlest, #878787)' },
};
...
const UserDetailPanel: React.FC<Props> = ({ userId, onClose, isDark = false }) => {
  ...
  const T = isDark ? {
    ...
    text2: 'var(--ds-text-subtlest, var(--cp-ink-3, #64748B))', text3: 'var(--ds-text-subtlest, #94A3B8)', sunken: 'var(--ds-surface-sunken, #F1F5F9)',
