      ...
      {CHANNEL_DEFS.map(({ key, label, Icon }) => (
        <Tooltip key={key} delay={200} content={`${label}: ${channels[key] ? 'Enabled' : 'Disabled'}`}>
          <div className="flex items-center gap-1">
            <Icon className="h-3 w-3 text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))]" />
            <Switch
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-[var(--ds-text,var(--ds-text, #0F172A))]">Delivery Channels</span>
        ...
        {CHANNEL_DEFS.map(({ key, label, description, Icon }) => (
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-md border text-left transition-colors duration-150 ${
              channels[key]
                ? 'bg-[rgba(37,99,235,0.04)] border-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]/20'
                : 'bg-white border-[var(--bd-default, #E2E8F0)] hover:bg-[rgba(0,0,0,0.02)]'
            } ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            <Icon
              className={`h-4 w-4 flex-shrink-0 ${
                channels[key] ? 'text-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]' : 'text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))]'
              }`}
            />
            <div className="min-w-0 flex-1">
              <p className="text-xs font-medium text-[var(--ds-text,var(--ds-text, #0F172A))]">{label}</p>
              <p className="text-[10px] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))] truncate">{description}</p>
            </div>
            <Switch
              checked={channels[key]}
              onCheckedChange={(v) => onChange(key, v)}
              disabled={disabled}
              className="h-4 w-7 data-[state=checked]:bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] flex-shrink-0"
            />
          </button>
        ))}
      </div>
...
export function ChannelBadges({ channels }: ChannelBadgesProps) {
  ...
  if (active.length === 0) {
    return <span className="text-[10px] text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #94A3B8))]">No channels</span>;
  }
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      {active.map(({ key, label, Icon }) => (
        <div
          key={key}
          className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F1F5F9))] border border-[var(--bd-default, #E2E8F0)]"
        >
          <Icon className="h-2.5 w-2.5 text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]" />
          <span className="text-[9px] font-medium text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">{label}</span>
        </div>
      ))}
    </div>
  );
