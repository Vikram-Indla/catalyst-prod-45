      ...
      <DialogContent className="sm:max-w-[520px] max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold font-['Sora'] flex items-center gap-2">
            <Users className="h-5 w-5 text-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))]" />
            Recipients
          </DialogTitle>
          <DialogDescription className="text-sm text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">
            Configure who receives the{' '}
            <span className="font-medium text-[var(--ds-text,var(--ds-text, #0F172A))]">{trigger.displayName}</span>{' '}
            notification.
          </DialogDescription>
        </DialogHeader>

        {/* Trigger info bar */}
        <div className="flex items-center gap-2 px-3 py-2 bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F8FAFC))] rounded-md border border-[var(--bd-default, #E2E8F0)]">
          <Lozenge appearance="default">
            {trigger.hubSource}
          </Lozenge>
          <span className="text-xs text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))]">{trigger.description}</span>
        </div>
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-medium text-[var(--ds-text,var(--ds-text, #0F172A))]">{def.label}</p>
                  <p className="text-[11px] text-[var(--ds-text-subtle,var(--ds-text-subtle, #475569))] truncate">{def.description}</p>
                </div>
              </div>
              <Switch
                checked={config[def.key]}
                onCheckedChange={() => handleToggle(def.key)}
                className="h-4 w-7 data-[state=checked]:bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] flex-shrink-0 ml-3"
              />
            </button>
          ))}
        </div>
        ...
        <DialogFooter className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={handleSave}
              className="bg-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] hover:bg-[var(--ds-background-brand-bold-hovered,var(--ds-background-brand-bold-hovered, #1D4ED8))] text-white text-sm"
            >
              Save Recipients
            </Button>
          </div>
