...
const CreateCatalystUserModal: React.FC<Props> = ({ open, onClose, onSuccess }) => {
  ...
  const labelStyle: React.CSSProperties = { fontSize: '11px', fontWeight: 600, color: 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))', marginBottom: '4px', display: 'block' };

  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent
        className="!bg-white dark:!bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] !text-slate-900 dark:!text-[var(--ds-text,var(--ds-text, #EDEDED))] p-0 gap-0"
      ...
      >
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(15,23,42,0.06)', position: 'relative' }}>
          ...
          <p style={{ fontSize: '11px', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', marginTop: '2px' }}>
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))',
            }}
          >
            <X size={12} />
          </button>
        ...
        <div style={{ padding: '18px 20px' }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            <label style={labelStyle}>Full Name <span style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>*</span></label>
            <Input
              value={displayName}
              onChange={e => { setDisplayName(e.target.value); setErrors(p => ({ ...p, displayName: '' })); }}
              placeholder="e.g. Dr. Ahmed Al-Rashid"
              className="!bg-white dark:!bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] !text-slate-900 dark:!text-[var(--ds-text,var(--ds-text, #EDEDED))] !border-slate-200 dark:!border-[var(--ds-border,var(--ds-border, #2E2E2E))]"
              style={{ fontSize: '12px', height: '34px' }}
            />
            {errors.displayName && <span style={{ fontSize: '10px', color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', marginTop: '2px', display: 'block' }}>{errors.displayName}</span>}
          </div>

          {/* Email */}
          <div style={{ marginBottom: '12px' }}>
            <label style={labelStyle}>Email <span style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>*</span></label>
            <Input
              type="email"
              value={email}
              ...
              style={{ fontSize: '12px', height: '34px' }}
            />
            {errors.email && <span style={{ fontSize: '10px', color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', marginTop: '2px', display: 'block' }}>{errors.email}</span>}
          </div>

          {/* Password */}
          <div style={{ marginBottom: '12px' }}>
            <label style={labelStyle}>Password <span style={{ color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))' }}>*</span></label>
            <div style={{ position: 'relative' }}>
              <Input
                ...
                className="!bg-white dark:!bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] !text-slate-900 dark:!text-[var(--ds-text,var(--ds-text, #EDEDED))] !border-slate-200 dark:!border-[var(--ds-border,var(--ds-border, #2E2E2E))] pr-9"
                style={{ fontSize: '12px', height: '34px' }}
              />
              <button
                type="button"
                ...
                style={{
                  ...
                  background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #64748B))', padding: 0,
                }}
              >
                {showPassword ? <EyeOff size={14} /> : <Eye size={14} />}
              </button>
            </div>
            <span style={{ fontSize: '10px', color: 'var(--ds-text-subtlest, var(--ds-text-subtlest, #94A3B8))', marginTop: '2px', display: 'block' }}>
              Min 8 characters. Stored securely in Catalyst (bcrypt).
            </span>
            {errors.password && <span style={{ fontSize: '10px', color: 'var(--ds-text-danger, var(--ds-text-danger, #DC2626))', marginTop: '1px', display: 'block' }}>{errors.password}</span>}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
            <div>
              ...
              <Select value={roleId} onValueChange={setRoleId}>
                <SelectTrigger className="!bg-white dark:!bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] !text-slate-900 dark:!text-[var(--ds-text,var(--ds-text, #EDEDED))] !border-slate-200 dark:!border-[var(--ds-border,var(--ds-border, #2E2E2E))]" style={{ height: '34px', fontSize: '12px' }}>
                  <SelectValue placeholder="Select role" />
                </SelectTrigger>
                <SelectContent className="!bg-white dark:!bg-[var(--ds-surface-raised,var(--ds-surface-raised, #1A1A1A))] !text-slate-900 dark:!text-[var(--ds-text,var(--ds-text, #EDEDED))]">
                  {ROLES.map(r => (
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              border: '1px solid rgba(15,23,42,0.10)', background: 'var(--bg-app, #FFFFFF)', color: 'var(--ds-text-subtle, var(--ds-text-subtle, #334155))',
              cursor: 'pointer',
            }}
          >
            Cancel
