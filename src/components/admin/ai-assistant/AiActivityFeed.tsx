  ...
  return (
    <div style={{ border: `1px solid ${T.border}`, borderLeft: `3px solid ${T.link}`, borderRadius: 8, overflow: 'hidden', marginBottom: 16 }}>
      <div style={{ padding: '12px 16px', borderBottom: `1px solid ${T.borderSubtle}` }}>
        ...
        <div style={{ fontSize: 12, color: T.subtle, marginTop: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.request}</div>
        {thinking ? (
      ...
      {thinking ? (
        <div style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 9, fontSize: 13, color: T.subtle }}>
  ...
  return (
    <div style={{ border: `1px solid ${T.border}`, borderLeft: `3px solid ${T.bgSuccessBold}`, borderRadius: 8, overflow: 'hidden', marginBottom: 12 }}>
      ...
      <div style={{ padding: '12px 16px' }}>
        ...
        <div style={{ fontSize: 13, color: T.text, lineHeight: 1.5 }}>{h.summary}</div>
        {h.novel && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 8, fontSize: 12, color: T.textDiscovery, background: T.bgDiscovery, borderRadius: 6, padding: '8px 8px' }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        {empty && (
          <div style={{ textAlign: 'center', padding: '32px 16px' }}>
            ...
            <p style={{ fontSize: 13, color: T.subtle, margin: '4px auto 0', maxWidth: 400, lineHeight: 1.5 }}>Describe a change above, or pick one from the library. Each request shows live progress here, then a clear confirmation of what changed.</p>
          </div>
        )}
        {c.history.map(h =>
          h.type === 'clarify'
