...
function StatPill({ icon: Icon, label, value, color = 'var(--ds-text-brand, #2563EB)' }: {
  ...
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-[var(--ds-surface-sunken,#F8FAFC)] border border-[var(--ds-border,var(--cp-border, #E2E8F0))]">
          ...
          <div key={s.id} className="flex items-center">
            <div className="flex items-center gap-2">
              <div className={`
                ...
                ${!done && !active ? 'bg-[var(--ds-surface-sunken,#F1F5F9)] text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))]' : ''}
              `}>
                {done ? <CheckMarkIcon label="" size="small" /> : <Icon className="h-3.5 w-3.5" />}
              </div>
              <span className={`text-xs font-medium hidden sm:block ${
                active ? 'text-[var(--ds-text-brand,#2563EB)]' : done ? 'text-[var(--ds-text,#0F172A)]' : 'text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))]'
              }`}>
                {s.label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`w-8 h-px mx-2 ${done ? 'bg-[var(--ds-text-success,var(--cp-success, #16A34A))]' : 'bg-[var(--ds-border,var(--cp-border, #E2E8F0))]'}`} />
            )}
          </div>
        );
      })}
...
export function NotionImportWizard() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* ─── Header Bar ─── */}
      <div className="h-14 flex items-center justify-between border-b border-[var(--ds-border,var(--cp-border, #E2E8F0))] px-6 shrink-0 bg-white">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="bg-white border border-[var(--ds-border,var(--cp-border, #E2E8F0))] rounded-lg overflow-hidden">
                ...
                {notionRows.length > 8 && (
                  <div className="px-3 py-2 text-[11px] text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))] bg-[var(--ds-surface-sunken,#FAFBFC)] border-t border-[var(--ds-surface-sunken,#F1F5F9)]">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 text-[11px]">
                  ...
                  <span className="px-2 py-1 rounded bg-[var(--ds-surface-sunken,#F1F5F9)] text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))] font-semibold">{skippedCount} skipped</span>
                </div>
              </div>
                  ...
                  return (
                    <div key={p.id} className={`flex items-center px-4 py-2 gap-2 border-b border-[var(--ds-surface-sunken,#F1F5F9)] last:border-b-0 ${isSkip ? 'bg-[var(--ds-surface-sunken,#FAFBFC)]' : ''}`}>
                      <div className="flex-1 flex items-center gap-2 min-w-0">
                        <span className={`text-[13px] font-medium truncate ${isSkip ? 'text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))]' : 'text-[var(--ds-text,#0F172A)]'}`}>{p.name}</span>
                        <span className="shrink-0 text-[9px] uppercase tracking-wider font-bold px-1 py-px rounded bg-[var(--ds-surface-sunken,#F1F5F9)] text-[var(--ds-text-subtlest,var(--cp-ink-4, #94A3B8))]">{p.type}</span>
                      </div>
                      <ArrowRightIcon label="" size="small" />
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              {!importResult ? (
                ...
                <div className="bg-white border border-[var(--ds-border,var(--cp-border, #E2E8F0))] rounded-lg p-6 text-center space-y-4">
      ...
      {/* ─── Sticky Footer ─── */}
      <div className="shrink-0 border-t border-[var(--ds-border,var(--cp-border, #E2E8F0))] bg-white px-6 py-3">
