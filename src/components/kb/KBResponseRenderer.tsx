...
const STATUS_PATTERNS: Record<string, { bg: string; color: string }> = {
  ...
  'medium': { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', color: '#42526E' },
  'low': { bg: 'var(--ds-border, var(--cp-lozenge-grey-bg, #DFE1E6))', color: '#42526E' },
...
function renderInline(text: string): React.ReactNode[] {
  ...
  while ((match = COMBINED.exec(text)) !== null) {
    ...
    if (match[1]) {
      ...
      parts.push(<strong key={key++} style={{ color: 'var(--ds-background-brand-bold-hovered, #1D4ED8)', fontWeight: 700, fontFamily: 'system-ui' }}>{match[2]}</strong>);
    } else if (match[3]) {
                ...
                {tableRows.map((row, ri) => {
                  ...
                  <tr
                    ...
                    style={{
                      ...
                      background: ri === 0 ? 'var(--ds-surface-sunken, var(--cp-bg-sunken, #F1F5F9))' : 'transparent',
