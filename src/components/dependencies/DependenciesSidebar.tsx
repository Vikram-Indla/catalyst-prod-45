...
const menuItems = [
  { id: 'dependencies-list', label: 'Dependencies List', icon: List, path: '/dependencies' },
  { id: 'dependency-matrix', label: 'Dependency Matrix', icon: Grid3x3, path: '/dependencies?view=matrix' },
  { id: 'dependency-wheel', label: 'Dependency Wheel', icon: GitBranch, path: '/dependencies?view=wheel' },
  { id: 'dependency-maps', label: 'Dependency Maps', icon: Map, path: '/reports/dependencies/maps' },
];

export function DependenciesSidebar({ className }: DependenciesSidebarProps) {
        ...
        {expanded && (
          <div className="border-t">
            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-normal hover:bg-accent/50 transition-colors">
              ...
              <span className="text-left">Dependency Settings</span>
            </button>
          </div>
        )}
      </div>
