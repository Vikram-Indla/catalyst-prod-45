...
export function TimelineView({ releases, onReleaseClick }: TimelineViewProps) {
  ...
  return (
    <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-[#2E2E2E] shadow-sm overflow-hidden">
          ...
          <div className="flex border border-slate-200 dark:border-[#2E2E2E] rounded-md overflow-hidden">
            {(['week', 'month', 'quarter'] as ZoomLevel[]).map(level => (
              <button
                ...
                className={cn(
                  "px-3 py-1 text-xs font-medium capitalize transition-colors border-r border-slate-200 dark:border-[#2E2E2E] last:border-r-0",
                  zoom === level
                    ? "bg-white dark:bg-[#1A1A1A] text-slate-900 dark:text-[#EDEDED] shadow-sm"
                    : "text-slate-500 dark:text-[#A1A1A1] hover:text-slate-700 dark:hover:text-[#EDEDED] hover:bg-white/50 dark:hover:bg-[#1F1F1F]"
                )}
              >
                {level}
              </button>
          ...
          <div className="flex items-center gap-4 text-[11px]">
            {[
              ...
              <div key={item.label} className="flex items-center gap-1.5">
                ...
                <span className="text-slate-600 dark:text-[#A1A1A1]">{item.label}</span>
              </div>
            ))}
            {[
        ...
        {/* Header Row */}
        <div className="flex border-b border-slate-200 dark:border-[#2E2E2E] bg-slate-50 dark:bg-[#111111]">
          <div className="w-[240px] shrink-0 px-4 py-2.5 border-r border-slate-200 dark:border-[#2E2E2E] flex items-center">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Release ↕</span>
          </div>
          <div className="flex-1 flex">
            {columns.map((col, i) => (
              <div key={i} className="flex-1 px-1 py-2.5 text-center border-r border-slate-100 dark:border-[#2E2E2E] last:border-r-0">
                <span className="text-[11px] font-semibold text-slate-500 dark:text-[#A1A1A1]">{col.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {releases.map((release) => {
          ...
          return (
            <div
              key={release.id}
              className="flex border-b border-slate-100 dark:border-[#2E2E2E] last:border-b-0 hover:bg-slate-50/50 dark:hover:bg-[#1F1F1F] cursor-pointer transition-colors"
              style={{ height: '42px' }}
            ...
            >
              {/* Release Label */}
              <div className="w-[240px] shrink-0 px-3 flex items-center gap-2.5 border-r border-slate-200 dark:border-[#2E2E2E]">
                <div
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[12px] font-medium text-slate-800 dark:text-[#EDEDED] truncate leading-tight">{release.name}</div>
                  <div className="text-[10px] text-slate-400 dark:text-[#878787] leading-tight">{statusLabel}</div>
                </div>
              </div>
              ...
              <div className="flex-1 relative">
                ...
                <div className="absolute inset-0 flex pointer-events-none">
                  {columns.map((_, i) => (
                    <div key={i} className="flex-1 border-r border-dashed border-slate-100 dark:border-[#2E2E2E] last:border-r-0" />
                  ))}
                </div>
