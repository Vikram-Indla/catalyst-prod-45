...
const HEALTH_BAR_COLORS: Record<HealthLevel, string> = {
  ...
  at_risk: 'var(--ds-background-warning-bold)',
  attention: 'var(--ds-background-neutral-subtle)',
  healthy: 'var(--ds-text-success)',
};
...
const VERSION_BADGE_COLORS: Record<HealthLevel, { bg: string; text: string }> = {
  ...
  at_risk: { bg: 'var(--ds-background-warning)', text: 'var(--ds-background-warning-bold)' },
  attention: { bg: 'var(--ds-background-neutral-subtle)', text: 'var(--ds-text-warning)' },
  healthy: { bg: 'var(--ds-background-success)', text: 'var(--ds-text-success)' },
};
...
export function TimelineView({ releases, onReleaseClick }: TimelineViewProps) {
  ...
  return (
    <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] rounded-xl border border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] shadow-sm overflow-hidden">
        {/* Controls Row */}
        <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 dark:border-[var(--ds-border,var(--cp-ink-1))] bg-slate-50/50 dark:bg-[var(--ds-surface-overlay,var(--cp-ink-1))]">
          <div className="flex border border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] rounded-md overflow-hidden">
            {(['week', 'month', 'quarter'] as ZoomLevel[]).map(level => (
              <button
                ...
                className={cn(
                  "px-3 py-1 text-xs font-medium capitalize transition-colors border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] last:border-r-0",
                  zoom === level
                    ? "bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] text-slate-900 dark:text-[var(--ds-text,var(--cp-bg-neutral))] shadow-sm"
                    : "text-slate-500 dark:text-[var(--ds-text-subtlest)] hover:text-slate-700 dark:hover:text-[var(--ds-text,var(--cp-bg-neutral))] hover:bg-white/50 dark:hover:bg-[var(--ds-surface-overlay)]"
                )}
              >
                {level}
              </button>
          ...
          <div className="flex items-center gap-4 text-[11px]">
            {[
              { label: 'Critical', color: 'var(--ds-text-danger)' },
              { label: 'At Risk', color: 'var(--ds-background-warning-bold)' },
              { label: 'Attention', color: 'var(--ds-background-warning-bold)' },
              { label: 'Healthy', color: 'var(--ds-text-success)' },
            ].map(item => (
              <div key={item.label} className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-sm" style={{ background: item.color }} />
                <span className="text-slate-600 dark:text-[var(--ds-text-subtlest)]">{item.label}</span>
              </div>
            ))}
            {[
              { label: 'Code Freeze', color: 'var(--ds-background-warning-bold)' },
              { label: 'Go Live', color: 'var(--ds-text-success)' },
            ].map(item => (
              <div key={item.label} className="flex items-center gap-1.5">
        ...
        {/* Header Row */}
        <div className="flex border-b border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] bg-slate-50 dark:bg-[var(--ds-surface-overlay,var(--cp-ink-1))]">
          <div className="w-[240px] shrink-0 px-4 py-2.5 border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] flex items-center">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Release ↕</span>
          </div>
          <div className="flex-1 flex">
            {columns.map((col, i) => (
              <div key={i} className="flex-1 px-1 py-2.5 text-center border-r border-slate-100 dark:border-[var(--ds-border,var(--cp-ink-1))] last:border-r-0">
                <span className="text-[11px] font-semibold text-slate-500 dark:text-[var(--ds-text-subtlest)]">{col.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {releases.map((release) => {
          ...
          return (
            <div
              key={release.id}
              className="flex border-b border-slate-100 dark:border-[var(--ds-border,var(--cp-ink-1))] last:border-b-0 hover:bg-slate-50/50 dark:hover:bg-[var(--ds-surface-overlay)] cursor-pointer transition-colors"
              style={{ height: '42px' }}
            ...
            >
              {/* Release Label */}
              <div className="w-[240px] shrink-0 px-3 flex items-center gap-2.5 border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))]">
                <div
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[12px] font-medium text-slate-800 dark:text-[var(--ds-text,var(--cp-bg-neutral))] truncate leading-tight">{release.name}</div>
                  <div className="text-[10px] text-slate-400 dark:text-[var(--ds-text-subtlest,var(--cp-text-secondary))] leading-tight">{statusLabel}</div>
                </div>
              </div>
              ...
              <div className="flex-1 relative">
                ...
                <div className="absolute inset-0 flex pointer-events-none">
                  {columns.map((_, i) => (
                    <div key={i} className="flex-1 border-r border-dashed border-slate-100 dark:border-[var(--ds-border,var(--cp-ink-1))] last:border-r-0" />
                  ))}
                </div>
                ...
                {todayPct !== null && (
                  ...
                  >
                    <div className="w-[2px] h-full bg-red-500 opacity-60" style={{ borderLeft: '1px dashed var(--ds-background-danger-bold)' }} />
                  ...
                  <Tooltip key={i} position="top" delay={200} content={m.label}>
                    ...
                    >
                      <div
                        ...
                        style={{ background: m.type === 'code_freeze' ? 'var(--ds-background-warning-bold)' : 'var(--ds-text-success)' }}
                      />
                    </div>
                  </Tooltip>
                ))}
        ...
        {releases.length === 0 && (
          <div className="py-16 text-center text-slate-400 dark:text-[var(--ds-text-subtlest,var(--cp-text-secondary))] text-sm">No releases to display in timeline</div>
        )}
      </div>
  );
}
