...
export function AIInsightsBar({ insights, onActionClick }: AIInsightsBarProps) {
  ...
  const getIcon = (type: AIReleaseInsight['type']) => {
    switch (type) {
      ...
      case 'warning': return <AlertTriangle className="w-4 h-4 text-[var(--ds-text-subtlest,var(--ds-text-subtlest, #64748B))]" />;
  ...
  const getBgColor = (type: AIReleaseInsight['type']) => {
    switch (type) {
      ...
      case 'warning': return 'bg-[var(--ds-surface-sunken,var(--ds-surface-sunken, #F1F5F9))] border-[rgba(15,23,42,0.12)]';
      case 'positive': return 'bg-green-50 border-green-100';
    }
  };

  return (
    <div className="border rounded-xl p-4 mb-6" style={{ background: 'var(--ds-background-selected, var(--ds-background-selected, #EFF6FF))', borderColor: '#DBEAFE' }}>
      ...
      <div className="space-y-2">
        {displayedInsights.map((insight, index) => (
          <div key={`${insight.releaseId}-${index}`} className={cn("flex items-center justify-between p-3 rounded-lg border", getBgColor(insight.type))}>
            ...
            <Button variant="ghost" size="sm" onClick={() => onActionClick?.(insight)} className="text-[var(--ds-text-brand,var(--ds-text-brand, #2563EB))] hover:text-[var(--ds-background-brand-bold-hovered,var(--ds-background-brand-bold-hovered, #1D4ED8))] text-xs">
              {insight.action}
