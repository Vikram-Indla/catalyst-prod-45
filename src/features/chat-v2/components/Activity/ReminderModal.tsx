...
interface ReminderModalProps {
   ...
   *  message body text so the modal shows a preview the user can confirm. */
  attachedMessageText?: string;
  onCancel: () => void;
...
export function ReminderModal({ attachedMessageText, onCancel, onSave }: ReminderModalProps) {
  const now = new Date();
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 0 }}>
        {grid.map((cell, idx) => {
          // Slack-style: out-of-month slots render as a transparent placeholder
          // with NO border so the grid only shows real days.
