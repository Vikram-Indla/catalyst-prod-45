...
function ParentItem({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button role="menuitem" onClick={onClick}
      style={{ width: '100%', height: 36, padding: '0 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: 'none', background: 'transparent', color: token('color.text', 'var(--ds-text, #172B4D)'), fontSize: 'var(--ds-font-size-400)', fontFamily: 'inherit', cursor: 'pointer', textAlign: 'left' }}
...
function BackItem({ onClick }: { onClick: () => void }) {
  return (
    <button role="menuitem" onClick={onClick}
      style={{ width: '100%', height: 36, padding: '0 12px', display: 'flex', alignItems: 'center', gap: 6, border: 'none', background: 'transparent', color: token('color.text.subtle', 'var(--ds-icon, #44546F)'), fontSize: 'var(--ds-font-size-300)', fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>
...
function IssueRow({ issue, onClick }: { issue: BoardIssue; onClick: () => void }) {
  return (
    <button role="menuitem" onClick={onClick}
      ...
      <span style={{ fontSize: 'var(--ds-font-size-200)', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)'), flexShrink: 0 }}>{issue.issueKey}</span>
      <span style={{ fontSize: 'var(--ds-font-size-300)', color: token('color.text', 'var(--ds-text, #172B4D)'), overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{issue.summary}</span>
    </button>
  );
}
        ...
        {(close) => {
          ...
          if (view === 'parent') {
            return (<><BackItem onClick={reset} /><Divider />
              ...
              {candidates.length === 0 && <div style={{ padding: '8px 12px', fontSize: 'var(--ds-font-size-300)', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)') }}>No matches</div>}
          ...
          return (
            <>
              ...
              <button role="menuitem" onClick={() => { p.onDelete(issue); close(); }}
                style={{ width: '100%', height: 36, padding: '0 12px', display: 'flex', alignItems: 'center', border: 'none', background: 'transparent', color: token('color.text.danger', 'var(--ds-text-danger, #AE2A19)'), fontSize: 'var(--ds-font-size-400)', fontFamily: 'inherit', cursor: 'pointer', textAlign: 'left' }}
