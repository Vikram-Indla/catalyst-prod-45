...
export const SwimlaneHeader: React.FC<Props> = ({ label, count, collapsed, avatarName, avatarUrl, showAvatar, onToggle }) => (
  ...
  >
    <span style={{ display: 'inline-flex', flexShrink: 0 }}>
      {collapsed
        ? <ChevronRightIcon label="" size="medium" primaryColor={token('color.icon.subtle', '#626F86')} />
        : <ChevronDownIcon label="" size="medium" primaryColor={token('color.icon.subtle', '#626F86')} />}
    </span>
    {showAvatar && <Avatar size="small" src={avatarUrl ?? undefined} name={avatarName || label} />}
    <span style={{ fontSize: 14, fontWeight: 600, color: token('color.text', '#172B4D'), whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
      {label}
    </span>
    <span style={{ fontSize: 14, fontWeight: 400, color: token('color.text.subtlest', '#626F86'), flexShrink: 0 }}>
