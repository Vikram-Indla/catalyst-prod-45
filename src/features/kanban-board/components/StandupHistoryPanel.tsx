...
const SessionCard: React.FC<{ s: StandupSession; onOpenTicket: (key: string) => void }> = ({ s, onOpenTicket }) => {
  ...
  return (
    <div style={{ border: `1px solid ${token('color.border', 'var(--ds-border)')}`, borderRadius: 8, padding: 12, marginBottom: 8, background: token('elevation.surface', 'var(--ds-surface)') }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        {s.is_valid && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {copied && <span style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 600, color: token('color.text.success', 'var(--ds-text-success)') }}>Copied</span>}
            <button type="button" aria-label="Copy summary" title="Copy summary" onClick={onCopy}
              ...
              <CopyIcon label="" size="small" primaryColor={token('color.icon.subtle', 'var(--ds-icon)')} />
            </button>
            <button type="button" aria-label="Share to WhatsApp" title="Share to WhatsApp" onClick={onWhatsApp}
              style={{ width: 28, height: 28, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', border: 'none', background: 'transparent', borderRadius: 4, cursor: 'pointer' }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = token('color.background.neutral.subtle.hovered', '#091E420F'); }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'transparent'; }}>
              <WhatsAppGlyph color={token('color.icon.success', 'var(--ds-background-success-bold)')} />
            </button>
          </div>
        )}
      </div>
      ...
      {/* Invalid (< 5 min) */}
      {!s.is_valid ? (
        <div style={{ fontSize: 'var(--ds-font-size-200)', color: token('color.text.subtlest', 'var(--ds-icon-subtle)'), fontStyle: 'italic' }}>
        ...
        <>
          ...
          {summary && (
            <div style={{ fontSize: 'var(--ds-font-size-300)', color: token('color.text', 'var(--ds-text)'), lineHeight: '18px', marginBottom: tickets.length ? 10 : 0, whiteSpace: 'pre-wrap' }}>{summary}</div>
          )}
          {tickets.length === 0 && !summary && !genLoading && (
            <div style={{ fontSize: 'var(--ds-font-size-200)', color: token('color.text.subtlest', 'var(--ds-icon-subtle)') }}>No tracked changes by the driver.</div>
          )}
          {tickets.map((t) => (
      ...
      <aside style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 440, maxWidth: '92vw', background: token('elevation.surface', 'var(--ds-surface)'), borderLeft: `1px solid ${token('color.border', 'var(--ds-border)')}`, boxShadow: token('elevation.shadow.overlay', '-8px 0 24px var(--ds-shadow-raised)'), zIndex: 9999, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 12px', borderBottom: `1px solid ${token('color.border', '#091E4224')}` }}>
          ...
          <button type="button" aria-label="Close" onClick={onClose}
            ...
            <CrossIcon label="" size="medium" primaryColor={token('color.icon.subtle', 'var(--ds-icon)')} />
          </button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
          {isLoading ? (
            ...
            <div style={{ fontSize: 'var(--ds-font-size-300)', color: token('color.text.subtlest', 'var(--ds-icon-subtle)'), textAlign: 'center', padding: 32 }}>No standups in the last 14 days.</div>
          ) : (
            groups.map(([label, daySessions]) => (
              <div key={label} style={{ marginBottom: 16 }}>
                <button type="button" onClick={() => toggleDay(label)}
                  ...
                  {label !== 'Today' && (isCollapsed(label) ? <ChevronRightIcon label="" size="small" primaryColor={token('color.icon.subtle', 'var(--ds-icon, var(--ds-icon))')} /> : <ChevronDownIcon label="" size="small" primaryColor={token('color.icon.subtle', 'var(--ds-icon, var(--ds-icon))')} />)}
                  <span style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: token('color.text.subtlest', 'var(--ds-icon-subtle)') }}>{label}</span>
