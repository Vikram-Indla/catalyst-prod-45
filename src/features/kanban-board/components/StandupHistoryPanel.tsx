    ...
    <div style={{ border: `1px solid ${token('color.border', 'var(--ds-border, #DFE1E6)')}`, borderRadius: 8, padding: 12, marginBottom: 10, background: token('elevation.surface', 'var(--ds-surface, #FFFFFF)') }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: token('color.text', 'var(--ds-text, #172B4D)') }}>{s.driver_name ?? 'Unknown'} drove this standup</div>
          <div style={{ fontSize: 'var(--ds-font-size-100)', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)') }}>
            {fmtTime(s.started_at)}–{fmtTime(s.ended_at)}{s.duration_sec ? ` · ${fmtDur(s.duration_sec)}` : ''}
          </div>
        </div>
        {s.is_valid && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {copied && <span style={{ fontSize: 'var(--ds-font-size-100)', fontWeight: 600, color: token('color.text.success', 'var(--ds-text-success, #216E4E)') }}>Copied</span>}
      ...
      {/* Invalid (< 5 min) */}
      {!s.is_valid ? (
        <div style={{ fontSize: 'var(--ds-font-size-200)', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)'), fontStyle: 'italic' }}>
          Ran under 5 minutes — no summary captured.
        </div>
      ) : (
        <>
          {genLoading && !summary && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 'var(--ds-font-size-200)', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)'), marginBottom: 10 }}>
              <Spinner size="small" /> Generating summary…
            </div>
          )}
          {summary && (
            <div style={{ fontSize: 'var(--ds-font-size-300)', color: token('color.text', 'var(--ds-text, #172B4D)'), lineHeight: '18px', marginBottom: tickets.length ? 10 : 0, whiteSpace: 'pre-wrap' }}>{summary}</div>
          )}
          {tickets.length === 0 && !summary && !genLoading && (
            <div style={{ fontSize: 'var(--ds-font-size-200)', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)') }}>No tracked changes by the driver.</div>
          )}
          {tickets.map((t) => (
            <div key={t.key} style={{ marginBottom: 8 }}>
              <button type="button" onClick={() => onOpenTicket(t.key)}
                ...
                <span style={{ fontSize: 'var(--ds-font-size-300)', fontWeight: 600, color: token('color.link', 'var(--ds-link, #0C66E4)') }}>{t.key}</span>
              </button>
              <div style={{ paddingLeft: 22 }}>
                {t.items.map((c, i) => (
                  <div key={i} style={{ fontSize: 'var(--ds-font-size-200)', color: token('color.text.subtle', 'var(--ds-icon, #44546F)'), lineHeight: '18px' }}>{changeDesc(c)}</div>
                ))}
              </div>
            </div>
          ))}
    ...
    <>
      ...
      <aside style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 440, maxWidth: '92vw', background: token('elevation.surface', 'var(--ds-surface, #FFFFFF)'), borderLeft: `1px solid ${token('color.border', 'var(--ds-border, #DFE1E6)')}`, boxShadow: token('elevation.shadow.overlay', '-8px 0 24px var(--ds-shadow-raised, rgba(9,30,66,0.16))'), zIndex: 9999, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 12px', borderBottom: `1px solid ${token('color.border', '#091E4224')}` }}>
          <span style={{ fontSize: 'var(--ds-font-size-500)', fontWeight: 600, color: token('color.text', 'var(--ds-text, #172B4D)') }}>Standup history</span>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
          {isLoading ? (
            ...
            <div style={{ fontSize: 'var(--ds-font-size-300)', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)'), textAlign: 'center', padding: 32 }}>No standups in the last 14 days.</div>
          ) : (
            groups.map(([label, daySessions]) => (
              <div key={label} style={{ marginBottom: 16 }}>
                <button type="button" onClick={() => toggleDay(label)}
                  ...
                  <span style={{ fontSize: 'var(--ds-font-size-200)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: token('color.text.subtlest', 'var(--ds-icon-subtle, #626F86)') }}>{label}</span>
