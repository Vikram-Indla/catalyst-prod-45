...
export const StandupPanel: React.FC<Props> = ({ issues, avatars, onPersonChange, onTimerSet }) => {
  ...
  return (
    <aside style={{ width: 280, minWidth: 280, flexShrink: 0, borderRight: `1px solid ${token('color.border', '#091E4224')}`, display: 'flex', flexDirection: 'column', background: token('elevation.surface', '#FFFFFF'), overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px' }}>
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${showSettings ? token('color.border.selected', '#0C66E4') : 'transparent'}`,
            background: showSettings ? token('color.background.selected', '#E9F2FE') : 'transparent',
            borderRadius: 4, cursor: 'pointer', padding: 0,
          }}
        >
          <PreferencesIcon label="" size="medium" primaryColor={showSettings ? token('color.icon.selected', '#0C66E4') : token('color.icon.subtle', '#44546F')} />
        </button>
      </div>
      ...
      {enableTimer && (
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, padding: '0 16px 12px' }}>
          <span style={{ fontSize: 36, fontWeight: 400, fontVariantNumeric: 'tabular-nums', lineHeight: '40px', color: timerDanger ? token('color.text.danger', '#AE2A19') : token('color.text', '#172B4D') }}>
            {mm}:{ss}
          </span>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, paddingBottom: 4 }}>
            <CircleBtn label={running ? 'Pause' : 'Play'} onClick={() => { if (seconds === 0) setSeconds(timerSec); setRunning((r) => !r); }}>
              {running ? <VidPauseIcon label="" size="small" primaryColor={token('color.icon.subtle', '#44546F')} /> : <VidPlayIcon label="" size="small" primaryColor={token('color.icon.subtle', '#44546F')} />}
            </CircleBtn>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px 12px', gap: 8 }}>
        <IconGhostBtn label="Shuffle order" onClick={shuffle}>
          <IcShuffle size={16} color={token('color.icon.subtle', '#44546F')} />
        </IconGhostBtn>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: isSelected ? token('color.background.selected', '#E9F2FF') : isHovered ? token('color.background.neutral.subtle.hovered', '#091E420F') : 'transparent',
                border: `1px solid ${isSelected ? token('color.border.selected', '#0C66E4') : 'transparent'}`,
              }}
            >
              <div style={{ position: 'relative', flexShrink: 0, lineHeight: 0 }}>
                ...
                {wasVisited && (
                  <span aria-label="Standup done" title="Done" style={{
                    ...
                    background: token('color.icon.success', '#1F845A'), border: `2px solid ${token('elevation.surface', '#FFFFFF')}`,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: compact ? 13 : 14, fontWeight: isSelected ? 600 : 500, color: isSelected ? token('color.text.selected', '#0C66E4') : token('color.text', '#172B4D'), whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {b.name}
                </div>
                <div style={{ fontSize: compact ? 10 : 11, color: token('color.text.subtlest', '#626F86'), marginTop: 1 }}>
              ...
              {isHovered && (
                ...
                >
                  <CrossIcon label="" size="small" primaryColor={token('color.icon.subtle', '#44546F')} />
                </button>
              )}
            </div>
          );
  ...
  return (
    <button
      ...
      style={{
        ...
        color: disabled ? token('color.text.disabled', '#B3B9C4') : token('color.text', '#292A2E'),
      ...
      onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = token('elevation.surface', '#FFFFFF'); }}
    >
      {label}
    </button>
  );
...
function CircleBtn({ label, onClick, children }: { label: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      style={{ width: 22, height: 22, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${token('color.border', '#DFE1E6')}`, borderRadius: '50%', background: token('elevation.surface', '#FFFFFF'), cursor: 'pointer', padding: 0 }}
...
function SettingsDropdown({
  ...
  return createPortal(
    <div
      ...
      style={{ position: 'fixed', top, left, width: PANEL_W, background: token('elevation.surface.overlay', '#FFFFFF'), border: `1px solid ${token('color.border', '#DFE1E6')}`, borderRadius: 6, padding: '10px 0', zIndex: 10000, boxShadow: token('elevation.shadow.overlay', '0 8px 24px rgba(9,30,66,0.16)') }}
    >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${selected ? token('color.border.selected', '#0C66E4') : token('color.border', '#DFE1E6')}`,
                  background: selected ? token('color.background.selected', '#E9F2FE') : token('elevation.surface', '#FFFFFF'),
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 11, fontWeight: 600, color: token('color.text.subtlest', '#626F86'), textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{children}</div>;
}
...
function RadioOption({ checked, onSelect, label }: { checked: boolean; onSelect: () => void; label: string }) {
  return (
    <button type="button" role="radio" aria-checked={checked} onClick={onSelect} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '6px 0', border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
      <span style={{ width: 16, height: 16, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', border: `2px solid ${checked ? token('color.border.selected', '#0C66E4') : token('color.border', '#C1C7D0')}`, flexShrink: 0 }}>
        {checked && <span style={{ width: 8, height: 8, borderRadius: '50%', background: token('color.border.selected', '#0C66E4') }} />}
      </span>
      <span style={{ flex: 1, fontSize: 13, color: token('color.text', '#292A2E') }}>{label}</span>
    </button>
  );
}
...
function ToggleRow({ label, value, onChange }: { label: string; value: boolean; onChange: (v: boolean) => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0' }}>
      <span style={{ flex: 1, fontSize: 13, color: token('color.text', '#292A2E'), lineHeight: '18px' }}>{label}</span>
      <button
        type="button" role="switch" aria-checked={value} onClick={() => onChange(!value)}
        style={{ width: 44, height: 24, borderRadius: 12, border: 'none', padding: 2, background: value ? token('color.background.success.bold', '#22A06B') : token('color.background.neutral.bold', '#44546F'), display: 'inline-flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', flexShrink: 0 }}
      >
        {value ? (
          <>
            <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 16, height: 16, marginLeft: 2 }}><CheckIcon label="" size="small" primaryColor="#FFFFFF" /></span>
            <span style={{ width: 18, height: 18, borderRadius: '50%', background: '#FFFFFF' }} />
          </>
        ) : (
          <>
            <span style={{ width: 18, height: 18, borderRadius: '50%', background: '#FFFFFF' }} />
            <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 16, height: 16, marginRight: 2 }}><CrossIcon label="" size="small" primaryColor="#FFFFFF" /></span>
          </>
        )}
      </button>
    </div>
