  ...
  return (
    <div className="flex flex-col items-center p-5 bg-card border border-border rounded-lg">
      <div className="relative w-32 h-32">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
          ...
          <circle
            ...
            stroke="var(--ds-background-success-bold)" strokeWidth="8" strokeLinecap="round"
      ...
      <div className="grid grid-cols-4 gap-2 mt-4 w-full">
        <StatusItem icon={CheckCircle2} count={passedTests} label="Passed" color="var(--ds-background-success-bold)" />
        <StatusItem icon={XCircle} count={failedTests} label="Failed" color="var(--ds-text-danger)" />
        <StatusItem icon={Ban} count={blockedTests} label="Blocked" color="var(--ds-text-warning, var(--cp-amber))" />
        <StatusItem icon={Circle} count={notRunTests} label="Not Run" color="var(--ds-text-subtlest, var(--cp-ink-4, var(--cp-border-neutral-light)))" />
      </div>
    </div>
  );
}
