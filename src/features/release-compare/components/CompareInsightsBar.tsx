...
export function CompareInsightsBar({ insights }: CompareInsightsBarProps) {
  ...
  const getIcon = (type: ComparisonInsight['type']) => {
    switch (type) {
      case 'critical':
        return <ErrorIcon label="" size="small" primaryColor="var(--ds-surface)" />;
      case 'warning':
        return <WarningIcon label="" size="small" primaryColor="var(--ds-surface)" />;
      case 'positive':
        return <CheckCircleIcon label="" size="small" primaryColor="var(--ds-surface)" />;
      default:
        return null;
    }
  };
  ...
  return (
    <div 
      ...
      style={{
        background: 'linear-gradient(135deg, var(--ds-background-discovery-bold) 0%, var(--ds-background-discovery-bold) 100%)',
      }}
    >
