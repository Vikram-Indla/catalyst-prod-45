...
export function CalendarGrid({
  ...
  return (
    <div className="bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] rounded-xl border border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] overflow-hidden">
      {/* Column Headers */}
      <div 
        className="grid border-b border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))]"
        style={{ gridTemplateColumns: `120px repeat(${columns.length}, 1fr)` }}
      >
        <div className="p-3 bg-slate-50 dark:bg-[var(--ds-surface-overlay,var(--cp-ink-1))] border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))]">
          <span className="text-xs font-medium text-slate-500 dark:text-[var(--ds-text-subtlest)] uppercase">Release</span>
        </div>
        {columns.map((col, i) => (
          <div
            key={i}
            className={cn(
              "p-3 text-center border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] last:border-r-0",
              col.isWeekend ? "bg-[var(--ds-surface-sunken)] dark:bg-[var(--ds-surface-sunken,var(--cp-ink-1))]" : "bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))]"
            )}
          >
            <span className="text-xs font-medium text-slate-600 dark:text-[var(--ds-text-subtlest)]">{col.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="relative">
        ...
        <div 
          ...
          style={{ gridTemplateColumns: `120px repeat(${columns.length}, 1fr)` }}
        >
          <div className="border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))]" />
          {columns.map((col, i) => (
            <div
              key={i}
              className={cn(
                "border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))] last:border-r-0",
                col.isWeekend && "bg-[var(--ds-surface-sunken)] dark:bg-[var(--ds-surface-sunken,var(--cp-ink-1))]"
              )}
            />
          ))}
        </div>
        ...
        <div 
          className="absolute left-0 top-0 w-[120px] bg-white dark:bg-[var(--ds-surface-raised,var(--cp-ink-1))] border-r border-slate-200 dark:border-[var(--ds-border,var(--cp-ink-1))]"
        ...
        >
          {releasesWithPositions.map(({ release, row }) => (
            ...
            >
              <span className="text-sm font-semibold text-slate-900 dark:text-[var(--ds-text,var(--cp-bg-neutral))] truncate">
                {release.version}
              </span>
      ...
      {releases.length === 0 && (
        <div className="p-12 text-center">
          <p className="text-slate-500 dark:text-[var(--ds-text-subtlest)]">No releases in this time period</p>
        </div>
      )}
    </div>
  );
