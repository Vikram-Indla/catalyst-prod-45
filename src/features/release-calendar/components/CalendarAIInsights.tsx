...
export function CalendarAIInsights({ insights, conflicts, onInsightClick }: CalendarAIInsightsProps) {
  ...
  const getIcon = (type: 'critical' | 'warning' | 'info') => {
    switch (type) {
      case 'critical': return <AlertCircle className="w-4 h-4 text-[#ef4444]" />;
      case 'warning': return <AlertTriangle className="w-4 h-4 text-[#d97706]" />;
      case 'info': return <Clock className="w-4 h-4 text-[#2563eb]" />;
    }
  };
        ...
        {displayedItems.map((item, index) => (
          ...
          >
            ...
            <Button
              ...
              className="text-[#2563eb] hover:text-[#2563eb]/80 text-xs"
            >
              {item.action}
