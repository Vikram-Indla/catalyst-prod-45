...
export function CalendarAIInsights({ insights, conflicts, onInsightClick }: CalendarAIInsightsProps) {
  ...
  const getIcon = (type: 'critical' | 'warning' | 'info') => {
    switch (type) {
      case 'critical': return <AlertCircle className="w-4 h-4 text-[var(--ds-text-danger,var(--ds-text-danger, #ef4444))]" />;
      case 'warning': return <AlertTriangle className="w-4 h-4 text-[var(--ds-text-warning,var(--ds-text-warning, #d97706))]" />;
      case 'info': return <Clock className="w-4 h-4 text-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))]" />;
    }
  };
  ...
  const getBgColor = (type: 'critical' | 'warning' | 'info') => {
    switch (type) {
      ...
      case 'info': return 'bg-[var(--ds-background-selected,var(--ds-background-selected, #eff6ff))] border-[#bfdbfe]';
    }
  };
        ...
        {displayedItems.map((item, index) => (
          ...
          >
            ...
            <Button
              ...
              className="text-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))] hover:text-[var(--ds-text-brand,var(--ds-text-brand, #2563eb))]/80 text-xs"
            >
              {item.action}
