...
interface DashboardHeaderProps {
  ...
  onReleaseUpdate?: (data: Partial<ReleaseDetail>) => Promise<void>;
  onReleaseApprove?: () => Promise<void>;
}
              ...
              <div className="grid grid-cols-3 gap-2">
                {qualityGates.map(gate => (
                  <div
                    ...
                    style={{
                      backgroundColor: gate.status === 'pass' ? '#ccfbf1' : gate.status === 'fail' ? '#fee2e2' : 'var(--ds-surface-sunken, #f1f5f9)',
                      borderColor: gate.status === 'pass' ? '#0d9488' : gate.status === 'fail' ? 'var(--ds-text-danger, #ef4444)' : 'var(--ds-text-subtlest, #94a3b8)',
                    }}
                  >
                    <div
                      className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold"
                      style={{ backgroundColor: gate.status === 'pass' ? '#0d9488' : gate.status === 'fail' ? 'var(--ds-text-danger, #ef4444)' : 'var(--ds-text-subtlest, #94a3b8)' }}
                    >
