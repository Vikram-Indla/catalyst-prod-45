...
function LoadingState({ isDark }: { isDark: boolean }) {
  const skeletonBg = isDark ? 'var(--ds-surface-overlay, #1F1F1F)' : 'var(--ds-background-neutral, #F4F5F7)';
  return (
    <Box xcss={panelXcss}>
      {[0, 1, 2, 3].map((i) => (
        <div
  ...
  return (
    <Box xcss={emptyXcss}>
      <svg width="48" height="48" viewBox="0 0 48 48" fill="none" aria-hidden="true">
        <circle cx="24" cy="24" r="20" fill={isDark ? 'var(--ds-border, #292929)' : 'var(--ds-background-neutral, #F4F5F7)'} />
...
export default function DirectPanel({ unreadOnly, isDark, readIds: externalReadIds, onMarkRead }: DirectPanelProps) {
  ...
  const dividerColor = isDark ? 'var(--ds-border, #2E2E2E)' : 'var(--ds-border, #DFE1E6)';
