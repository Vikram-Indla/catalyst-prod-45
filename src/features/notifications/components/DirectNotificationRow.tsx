...
export default function DirectNotificationRow({ notification, isRead, onMarkRead, isDark }: Props) {
  ...
  const hoverBg  = isDark ? 'var(--ds-surface-overlay, var(--ds-surface-overlay, #1F1F1F))' : token('color.background.neutral.hovered', 'rgba(9,30,66,0.06)');
  const pressBg  = isDark ? 'var(--ds-border, var(--ds-border, #292929))' : token('color.background.neutral.pressed',  'rgba(9,30,66,0.10)');
  const rowBg    = pressed ? pressBg : hovered ? hoverBg : 'transparent';

  const text1    = isDark ? 'var(--ds-text, var(--ds-text, #EDEDED))' : token('color.text', '#292A2E');
  const text2    = isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #A1A1A1))' : token('color.text.subtle',    '#626F86');
  const text3    = isDark ? 'var(--ds-text-subtlest, var(--ds-text-subtlest, #878787))' : token('color.text.subtlest',  '#8590A2');
  const linkClr  = isDark ? '#6698FF' : token('color.link',           '#0C66E4');
  const dotColor = 'var(--ds-text-brand, var(--ds-text-brand, #2563EB))';
