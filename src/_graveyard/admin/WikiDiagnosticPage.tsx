...
function MetricCard({ label, value, subtitle, icon: Icon, status }: {
  ...
  return (
    <div className="border border-[rgba(15,23,42,0.12)] dark:border-[var(--cp-ink-1, #2E2E2E)] rounded-md p-4 bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)]">
      <div className="flex items-center gap-2 mb-2">
        <div className={`w-8 h-8 rounded flex items-center justify-center ${colors[status]}`}>
          <Icon className="w-4 h-4" />
        </div>
        <span className="text-xs font-semibold uppercase tracking-wide text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))]">{label}</span>
      </div>
      <p className="text-2xl font-bold font-[var(--cp-font-mono,monospace)] text-[var(--cp-ink-1, var(--cp-ink-1, #0F172A))]">{value}</p>
      {subtitle && <p className="text-xs text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))] mt-0.5">{subtitle}</p>}
    </div>
  );
}
...
function Section({ title, icon: Icon, children }: { title: string; icon: React.ElementType; children: React.ReactNode }) {
  return (
    <section className="border border-[rgba(15,23,42,0.12)] dark:border-[var(--cp-ink-1, #2E2E2E)] rounded-md bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)]">
      <div className="px-4 py-3 border-b border-[rgba(15,23,42,0.12)] flex items-center gap-2 bg-[#FAFBFC]">
        <Icon className="w-4 h-4 text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))]" />
        <h3 className="text-sm font-semibold uppercase tracking-wide text-[var(--cp-ink-1, var(--cp-ink-1, #0F172A))]">{title}</h3>
      </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-[#FAFBFC]">
            {headers.map((h, i) => (
              <th key={i} className="text-left px-4 py-2 text-[10px] font-semibold uppercase tracking-wider text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))] border-b border-[rgba(15,23,42,0.12)]">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((cells, ri) => (
            <tr key={ri} className="border-b border-[rgba(15,23,42,0.06)] h-9 hover:bg-[rgba(15,23,42,0.02)]">
              {cells.map((cell, ci) => (
                <td key={ci} className="px-4 py-1.5 font-[var(--cp-font-mono,monospace)] text-xs text-[var(--cp-ink-1, var(--cp-ink-1, #0F172A))]">{cell}</td>
              ))}
            </tr>
          ))}
          {rows.length === 0 && (
            <tr><td colSpan={headers.length} className="px-4 py-6 text-center text-xs text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))]">No data</td></tr>
          )}
        </tbody>
      </table>
    </div>
...
export default function WikiDiagnosticPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFBFC]">
      {/* Header */}
      <div className="bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] border-b border-[rgba(15,23,42,0.12)] dark:border-[var(--cp-ink-1, #2E2E2E)] px-7 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Stethoscope className="w-6 h-6 text-[var(--cp-purple-60, #7C3AED)]" />
          <div>
            <h1 className="text-lg font-bold text-[var(--cp-ink-1, var(--cp-ink-1, #0F172A))]">Wiki Pipeline Diagnostic</h1>
            <p className="text-xs text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))]">Live health checks against all wiki-related tables</p>
          </div>
        </div>
        <button
      ...
      <div className="px-7 py-6 space-y-6">
        ...
        <Section title="Sample Source Keys (Top 30)" icon={Layers}>
          <DiagTable
            ...
            rows={(data?.sampleKeys || []).map((r: any) => [
              ...
              r.chunks,
              <span className="max-w-[300px] truncate block text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))]">{(r.preview || '').substring(0, 80)}</span>,
            ])}
          />
        </Section>
        ...
        <Section title="Wiki Pages" icon={FileText}>
          {(data?.pages || []).length === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-[var(--cp-text-secondary, var(--cp-text-secondary, #44546F))]">No wiki articles generated yet.</div>
          ) : (
            <DiagTable
