...
/* ═══════════════════════════════════════════
   SHARED STYLES
   ═══════════════════════════════════════════ */
const card = 'bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] border border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] rounded-lg p-5 shadow-sm';
    ...
    <div className="space-y-5">
      ...
      <div className={`${overallBg} border rounded-xl p-6 flex items-center justify-between`}>
        ...
        <button
          onClick={runChecks}
          disabled={loading}
          className="flex items-center gap-1.5 px-4 py-2 bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] border border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] rounded-lg text-xs font-semibold text-zinc-700 dark:text-[#A1A1A1] hover:bg-zinc-50 dark:hover:bg-[var(--cp-ink-1, #1A1A1A)] transition-colors disabled:opacity-50"
        >
                    ...
                    <tr>
                      <td colSpan={12} className="px-0 py-0">
                        <div className="bg-blue-50/60 border-t border-b border-blue-100 px-6 py-4 space-y-3">
                          ...
                          <div className="bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] rounded-lg border border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] p-3">
          ...
          <tbody className="divide-y divide-zinc-100">
            {roles.map((role) => (
              <tr key={role} className="hover:bg-zinc-50/50">
                <td className="px-3 py-2 font-medium text-zinc-700 dark:text-[var(--cp-bg-neutral, #EDEDED)] sticky left-0 bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] z-10 whitespace-nowrap">{role}</td>
                {modules.map((mod) => {
      ...
      <div className={card}>
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <input
            ...
            className="flex-1 min-w-[200px] px-3 py-1.5 text-xs border border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] rounded bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] dark:text-[var(--cp-bg-neutral, #EDEDED)] focus:outline-none focus:ring-1 focus:ring-blue-400"
          />
          <select value={catFilter} onChange={(e) => setCatFilter(e.target.value)} className="px-3 py-1.5 text-xs border border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] rounded bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] dark:text-[var(--cp-bg-neutral, #EDEDED)]">
            <option value="all">All Categories ({questions.length})</option>
            {categories.map(c => (
              <option key={c.category} value={c.category}>{c.category} ({c.count})</option>
            ))}
          </select>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="px-3 py-1.5 text-xs border border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] rounded bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] dark:text-[var(--cp-bg-neutral, #EDEDED)]">
            <option value="all">All Status</option>
...
export default function KBAdminPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50" style={{ paddingTop: 48 }}>
      {/* Page header */}
      <div className="bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] border-b border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] px-6 py-4">
      ...
      {/* Tabs */}
      <div className="bg-white dark:bg-[var(--cp-ink-1, #1A1A1A)] border-b border-zinc-200 dark:border-[var(--cp-ink-1, #2E2E2E)] px-6">
