...
import("./App")
  ...
  .catch((err) => {
    ...
    el.innerHTML = `
      <div style="padding:40px;font-family:monospace;color:var(--ds-background-danger-bold)">
        <h1>Boot Error</h1>
        <pre style="white-space:pre-wrap;word-break:break-all">${String(err?.message || err)}</pre>
        <pre style="white-space:pre-wrap;word-break:break-all;font-size:12px;color:var(--ds-text-subtlest)">${String(err?.stack || '')}</pre>
      </div>
    `;
  });
