...
When this skill fires, emit this block in chat BEFORE probing:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔬 jira-compare · PARITY PROBE
Surface: {surface} · Route: {route}
Lanes: Chrome MCP DOM probe · Atlassian MCP schema · Computed-style diff
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

When probe is complete:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔬 jira-compare · PROBE COMPLETE
{N} drift items found · Cycle {X}/5
Red arrows injected on Catalyst surface. Screenshot follows.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
resized. Visible in screenshots: label at wrong element after resize. **Fix:** violations now use
`{selector, label, side}`. Live `render()` re-queries `getBoundingClientRect()` on every
resize/scroll via `requestAnimationFrame`. Arrows now stick to their elements at any viewport size.
...
(function injectArrows(violations) {
  ...
  const ID = '__jc_overlay';
  const BTN_ID = '__jc_overlay_btn';
  ...
  [['jc-red', RED], ['jc-green', GREEN]].forEach(function(pair) {
  ...
  function render() {
    ...
    violations.forEach(function(v) {
      ...
      const markerRef = v.fixed ? 'jc-green' : 'jc-red';
  ...
  // { selector: '[data-testid="for-you-row"]:first-child', label: 'JC-1: lozenge uppercase', side: 'right', fixed: false },
  // { selector: '.status-pill',                            label: 'JC-2: wrong color token', side: 'left',  fixed: false },
  // After fixing JC-1: change its fixed to true and re-run this block.
]);
```

Rules:
- Screenshot immediately after injection.
- Caption: `🔴 JC VIOLATIONS — {surface} — {N} drift items · Cycle {X}/5 · resize-stable`
- Resize the window and confirm arrows track. Take a second screenshot if resized.
...
4. Screenshot → caption: `🟡 JC PROGRESS — {N} open · {M} fixed · Cycle {X}/5`
5. When all `fixed: true` → caption: `✅ JC FIXED — {surface} — {N} resolved · re-probe follows`

Raw screenshots with no arrows are REJECTED.

---

## 0. 500-IQ Foundation Council Discovery (mandatory pre-probe layer)

Before any DOM probe, run the design-intelligence v2 Foundation Council scan on the surface being audited. This is NOT the same as jira-compare's DOM probe — it is a separate lens that feeds INTENT to the probe, so the probe knows what to look for beyond visual pixel drift.

Emit this block when this layer activates:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔬 jira-compare · 500-IQ PRE-PROBE COUNCIL SCAN
Surface: {surface} · Route: {route}
Council: Saffer micro-interaction · Tufte data-ink · Rams 10 principles
         Norman affordances · Ive reduction · Raskin cognitive · Cooper goals
Generating intent map for DOM probe…
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

For each council lens, generate a **probe target list** — what specifically to look for in the Catalyst DOM and Jira DOM based on that lens's concerns:

| Council lens | Probe targets for this surface |
|---|---|
| Saffer | micro-interaction triggers, feedback delay, mode traps, missing error states |
| Tufte | chartjunk (decorative borders, redundant labels), data-ink ratio of list rows |
| Rams | error/empty states designed?, every element serving a user goal? |
| Norman | false affordances, hidden affordances (hover-only), missing async feedback |
| Ive | decorative shadows/borders, transition timing (ease-out expand, ease-in collapse) |
| Raskin | n > 7 choices in any menu, targets < 24px, mode without visual indicator |
| Cooper | task-centric labels vs goal-centric, empty states without CTA, workflow dead ends |

The probe target list is injected into the Lane A (Chrome MCP) prompt so the DOM interrogation is guided by 500-IQ intent, not pixel-matching alone.

---

## 0.5 Warmup (mandatory, every invocation)
...
## 0.5 — Proactive ADS Sensor (fires before Lane A, every invocation)

Before ANY lane probe, run both sensors on the Catalyst source for this surface. No prompt required.

```bash
# Static scan — catches isBold, raw hex, hardcoded spacing, hand-rolled components
invoke anthropic-skills:ads-validator on [surface source files]

# Live DOM probe — catches unwanted elements, premature errors, uppercase labels
invoke anthropic-skills:design-intelligence on localhost:8080/[route]
```

| Catch | Sensor | Action |
|---|---|---|
| `isBold` + `appearance="default"` on Lozenge (BLACK pill) | ads-validator | P0 — file BAU bug immediately, add to evidence |
| Raw hex literal not in allowed-hardcoded.md whitelist | ads-validator | P0 — file BAU bug immediately |
| Hardcoded px in inline style | ads-validator | P1 — add to evidence, plan row |
| Unwanted HelperMessage / "Learn about" link | design-intelligence | P1 — add to evidence |
| Premature validation error on load | design-intelligence | P1 — add to evidence |
| `text-transform: uppercase` on label | design-intelligence | P1 — add to evidence |
| Hand-rolled menu/dropdown/modal | ads-validator | P0 — file BAU bug |

Sensor output is prepended to Lane A observation. P0s from sensor are filed as BAU bugs immediately (do not wait for full audit cycle). Evidence envelope receives `ads_violations` array.
...
1. **Take maximum visual screenshots** of every state the surface owns (default, empty, sidebar expanded/collapsed, dark mode, any modals or flyouts changed).
...
3. **Display each annotated screenshot inline in the chat** — the conversation IS the artefact. No file export, no folder, no disk storage. Caption each image directly in chat: route · view · what changed · what it replaced.
4. Update `HANDOVER.md` under `## Closure Evidence` with the **caption list only** — visuals live in the chat session above.
5. Commit before declaring done.
