...
An audit of this repo's skill stack (`.claude/skills/*`) surfaced four repeat gaps that let "looks finished" ship instead of "matches the objective" (the "bound spots" this skill closes):
...
- Classify the objective: **product/UI/data change** (full CLAUDE.md ceremony applies — Feature Work ID, feature folder, Plan Lock, screenshot signoff, commit gate) vs. **tooling/meta work** (e.g. authoring a skill, a script, docs — lighter-weight version of Phases 0-4, no feature-folder ceremony forced onto it). State which, and why, explicitly.
...
- Objective is product/UI/data work → full CLAUDE.md ceremony applies. Objective is tooling/meta work with no product surface → say so and use the lighter-weight path.
