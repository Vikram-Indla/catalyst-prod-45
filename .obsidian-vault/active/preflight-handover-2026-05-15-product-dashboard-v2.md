...
## Copy-paste block — for Claude Code (paste as first message in a new session)

```
/preflight resume — Product Dashboard v2 rebuild

═══════════════════════════════════════════════════════════════════
REQUIRED FIRST ACTIONS (do these in this order, in parallel where possible)
═══════════════════════════════════════════════════════════════════
...
2. Read the project gates (look for any new lessons since 2026-05-15):
...
4. Verify Group 1 SQL is applied (run via Supabase MCP if connected, OR ask me to paste verification result):
   The verification query is at the bottom of the handover under "Post-paste verification".
   Expect: 3 / 1 / 2 / 2 row counts.

═══════════════════════════════════════════════════════════════════
DECISIONS LOCKED (do NOT re-deliberate — straight to execution)
═══════════════════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════════════════
CURRENT STATE — fill in before starting Group 2
═══════════════════════════════════════════════════════════════════

Group 1 SQL status: [REPLACE WITH ONE: "all 4 blocks applied, verification returned 3/1/2/2"
                     OR  "block N failed with error: <paste error>"
                     OR  "not yet pasted in Lovable — please pause Group 2 until I confirm"]

═══════════════════════════════════════════════════════════════════
NEXT WORK — Group 2 (Hooks layer · 5 rows · 1 session)
═══════════════════════════════════════════════════════════════════

Files to create (TDD — failing test first, then minimal impl, then ask before next row):

1. src/hooks/useWidgetSettings.ts
   - TanStack Query
   - Reads user_widget_settings (per-user override) → falls back to dashboard_widget_defaults (tenant default)
...
3. src/hooks/useBrCycleTime.ts
   ...
   - Used 3 ways: Business leg (startStep=funnel, endStep=ready_for_implementation),
...
• Do NOT regenerate the plan, council, or Phase 2.5 evidence — they're locked in the handover.
