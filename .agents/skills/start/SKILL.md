...
# /start — Proceed-Gated Codex Orchestrator
...
**For UI-related tasks (Feature replication / Implementation / Design audit / UI-DOM analysis):** also state in the widget: "Mockup-first workflow required per `.Codex/mockup-contract.md`".
...
**FOR UI-RELATED TASKS ONLY** (Feature replication / Implementation / Design audit / UI-DOM analysis):
   ...
   - **Full contract:** Reference `.Codex/mockup-contract.md` for all rules (canonical components, hand-rolled blocking, handoff at 80%, etc.)
...
- `.Codex/start-orchestrator/registry/repos.json`
- `.Codex/start-orchestrator/registry/how-Codex-gets-skills.md`
- `.Codex/vendor/<repo-name>/`
...
design-critique:
- Path: `.Codex/skills/design-critique/SKILL.md` (both repo-local and global `~/.Codex/skills/`)
...
- Path: `.Codex/skills/systematic-debugging/SKILL.md` (repo-local, distributed via git)
- Availability: Native local skill
...
- If active as a Codex plugin but not on PATH, do not mark it missing only because CLI check fails.

Codex Context:
- Repo: https://github.com/zilliztech/Codex-context.git
