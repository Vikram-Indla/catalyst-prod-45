...
version: 1.3.0
author: preflight × Vikram × Codex, 2026-06-19 (v1.3: 80/20 output contract — one-liners + widget, no commentary; v1.2: mandatory visual widget consent gate; v1.1: clone-parity gate + 10 anti-shipping-fast guards)
metadata:
...
G5 [icon type registry] Every work-item-type icon must use the
   `JiraIssueTypeIcon type=` string from the locked AGENTS.md icon
...
AGENTS.md icon registry (Story, Epic, Business Request, ...). Field values
...
explicitly banned by AGENTS.md §19. The only legal sources are
...
### L6 — Read AGENTS.md before EVERY edit on the target file
A lesson logged earlier in the same session is worthless if it isn't
re-read before the next commit touches the relevant surface. AGENTS.md
is loaded into context — re-read the relevant section by `grep -n`
before editing.
**Halt if you cannot quote the relevant AGENTS.md rule for the change.**
...
- `AGENTS.md` — gates, banned columns, ADS-token rules, jira-compare lessons
