...
author: Vikram × Codex, 2026-05-11
metadata:
...
0. **Code archaeology FIRST.** Before using ANY Jira API endpoint, check `wh-jira-bulk-sync` and similar functions for the proven endpoint. Replicate the working pattern exactly. Only debug if replication fails. See AGENTS.md 2026-05-16 lesson. Cost: ~30 seconds. Benefit: eliminates wrong alternatives.
1. **DOM probe, not assumption.** Never assert Jira renders something without measuring it via `getComputedStyle` in Chrome MCP. Prior measurements in AGENTS.md are starting points — re-probe at audit time.
...
3. **CRUD gate is the acceptance test.** Visual match alone is NOT parity. Create → Read → Update → Delete on both sides must pass before the surface is declared done. Use Supabase MCP (`execute_sql`, `list_tables` on project `cyijbdeuehohvhnsywig`) to verify backend state for CRUD-R.
...
|---|---|---|---|
| **C** (Create) | Create a new work item / comment / sub-task via Catalyst UI | Row appears in Catalyst UI + persists in Supabase backend | Use Supabase MCP: `execute_sql(project_id="cyijbdeuehohvhnsywig", query="SELECT * FROM {table} WHERE id='{new_id}'")` |
...
```
list_tables(project_id="cyijbdeuehohvhnsywig", schemas=["public"], verbose=true)
execute_sql(project_id="cyijbdeuehohvhnsywig", query="SELECT * FROM {table} WHERE {condition} LIMIT 10")
```
...
See AGENTS.md `# jira-compare — compounding lessons` section. These are authoritative.
...
- `AGENTS.md` `# jira-compare — compounding lessons` — all prior audit lessons
