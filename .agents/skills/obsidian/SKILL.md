...
## ⚠️ CRITICAL REQUIREMENT — AGENTS.md MUST BE READ FIRST

**Every single invocation of `/obsidian` commands MUST follow this sequence:**

1. **READ AGENTS.md** (`.Codex/AGENTS.md`) — Load enforcement rules, branch naming, guidelines
2. **VERIFY RULES** — Confirm branch validation regex, constraints, project context
3. **RETRIEVE HANDOVER** — Load transcript from `.Codex/obsidian-handovers/`
...
**Exceptions:** None. AGENTS.md is always read first, before any handover retrieval.

**Why:** Handovers are context snapshots, but AGENTS.md is the living source of truth for rules, constraints, and current project state. Without reading AGENTS.md first, enforced patterns (branch naming, design system, banned fields, etc.) from when the handover was saved may be stale or conflicting with updated rules.

---

## Overview

The `/obsidian` skill manages detailed handovers and branch context in the centralized Obsidian vault (`.Codex/obsidian-handovers/`). It captures full conversation state, progress, and metadata when transitioning between branches or conversations, and retrieves that context when resuming work.

**Centralization Model:** All handovers are stored in `.Codex/obsidian-handovers/` (git version-controlled), making them immediately accessible from any local machine working on the GitHub repo. No Obsidian vault setup required — just `git pull origin main`.
...
- Stores metadata in `.Codex/obsidian-handovers/metadata.json`

**Examples:**
```
# You're on: BAU-backlog-table-01
You: /obsidian save
Codex: ✓ Saved handover for BAU-backlog-table-01 (ID: 01)
        ✓ Progress: 50% (default)
...
1. **Read AGENTS.md first** — Load all enforcement rules, constraints, and guidelines from `AGENTS.md`
2. **Verify branch rules** — Confirm branch naming convention and validation rules
3. **Retrieve handover** — Load the handover transcript from `.Codex/obsidian-handovers/`
...
**What it restores:**
- AGENTS.md rules and enforcement context (read first)
...
- **ALWAYS reads AGENTS.md before retrieving handover** (non-negotiable)
...
Codex:
  ...
  ✓ AGENTS.md constraints and rules loaded
...
Previous work (verified against AGENTS.md rules):
...
Remaining work (per AGENTS.md constraints):
...
Handovers are stored in `.Codex/obsidian-handovers/`:

```
.Codex/obsidian-handovers/
...
- [AGENTS.md § 2026-05-18](../../AGENTS.md) — Branch management rules
...
```
You: /obsidian branch 01
Codex: [Retrieves and hydrates context automatically]
```
...
```
You: /obsidian save
Codex: [Captures current state, commits pending work, saves handover]
```
...
You: /obsidian branch 01
Codex: [Full context restored immediately]
```
...
All handovers are stored in **`.Codex/obsidian-handovers/`** which is the **centralized, authoritative source for all branch context** across the entire project:
...
/obsidian branch 01
# Reads AGENTS.md first, then displays full context
...
git pull origin main
/obsidian branch 01
# Output: [Full handover loaded, AGENTS.md rules verified, context ready]
```
...
- `/obsidian save` — Explicit save command (reads AGENTS.md first for rules context)
- `/obsidian branch {NN}` — Explicit context retrieval (ALWAYS reads AGENTS.md first before handover)
- `/obsidian list` — List all handovers (reads AGENTS.md first for validation rules)
- `/obsidian delete {NN}` — Archive a handover (reads AGENTS.md first)
- Auto-detection when starting a conversation with a branch ID in the context (reads AGENTS.md first)

**Execution Order (MANDATORY for all triggers):**
```
1. Read .Codex/AGENTS.md (full file, § 2026-05-18 section minimum)
...
5. Display result with AGENTS.md context embedded in response
```
