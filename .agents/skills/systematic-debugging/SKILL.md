...
description: Run a disciplined multi-phase debugging loop for Codex tasks. Use when failures are ambiguous, regressions are hard to localize, logs are noisy, or a bug fix must be proven rather than guessed.
version: 0.1.0
author: OpenAI Codex
license: MIT
metadata:
