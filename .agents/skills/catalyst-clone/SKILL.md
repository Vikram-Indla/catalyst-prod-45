...
Run logged to feature folder. (AGENTS.md touched only if a new guard was promoted.)
Commit: {sha}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
| In-scope regions | …, … |
| Out-of-scope regions | …, … (banned columns, banned fields, banned interactions per AGENTS.md) |
...
| Ability | Axis | Rationale | AGENTS.md citation |
|---|---|---|---|
...
## Phase 7 — Guardrails Re-Scan (L1-L10 + permanent AGENTS.md bans)

Re-read AGENTS.md before this phase. Quote each guard + verdict:
...
| L6 | "Read AGENTS.md before EVERY edit" | PASS | Re-read at Phase 0 + Phase 7 |
...
**Permanent AGENTS.md bans (always re-checked):**
...
- Branch name: `{TargetHub}-{Surface}-clone-{NN}` per AGENTS.md branch convention (or push to main if `main` is the convention and the user authorized).
...
## Phase 10 — Lesson Log (mandatory, every run — AGENTS.md only on a NEW guard)
...
**Only append to project AGENTS.md** when this run discovered a genuinely NEW failure mode worth promoting to a permanent guard (new L-number or G-number below). Routine "gates passed" logs stay in the feature folder — AGENTS.md is the law, not a run log, and appending on every run silently bloats the contract every clone touches.
...
- Upgrade the entry to a permanent AGENTS.md guard (add L11, L12, …)
...
- `AGENTS.md` — the contract this skill enforces (re-read at Phase 0 + 7)
- `.Codex/skills/design-critique/SKILL.md` — Clone-Parity Gate G1-G10 + L1-L10 + closure evidence protocol
- `.Codex/skills/jira-compare/SKILL.md` — 3-lane Jira parity protocol (used in Phase 1 Lane B for Jira→Catalyst mode)
- `.Codex/skills/catalyst-agent/SKILL.md` — probe-first router with TABLE gap report pattern
- `design-governance/cli/index.js` — ADS validator
- `design-governance/rules/audit.js` — audit rules
- Atlassian Design System: https://atlassian.design/
- Locked icon registry: AGENTS.md "🔒 WORK ITEM TYPE ICONS — LOCKED REGISTRY"
- Banned columns / fields / components: AGENTS.md permanent ban table (Comments, Story Points, MDT Ref, Service Now#, Assessment Feature, Catalyst Intelligence button, Development section, Automation section, …)
...
4. The AGENTS.md icon registry or banned list grows → mirror in Phase 7.
...
The inaugural use of this skill MUST be: re-clone the broken `/product-hub/INV/backlog` table from `/project-hub/BAU/backlog` end-to-end, including the inline edits that the rushed Phase B commit missed. This run will populate the first lesson entry under "## 2026-06-01 — Catalyst Clone (inaugural)" in AGENTS.md.
