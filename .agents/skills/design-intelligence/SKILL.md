...
description: >-
  ...
  Lessons-applied scan: checks all AGENTS.md lesson anchors and
...
Lessons-applied: {N} AGENTS.md anchors checked · {M} active warnings
...
> Before any council lens fires, scan the surface against all known AGENTS.md lessons and JIRA_ARCHITECT.md patterns. This prevents the council from recommending something that is already known-broken or permanently banned.

### Pre-scan checklist (run top-to-bottom against the surface)

| Check | Source | What to look for |
|---|---|---|
| JiraIssueTypeIcon | AGENTS.md 2026-05-09 | Any colored dot, square, or color map for work item type? → P0 halt |
| Colored lozenge appearance | AGENTS.md 2026-05-08 | Any lozenge `appearance` not from live DOM probe? → P0 |
| @atlaskit/popup inside overflow:hidden | AGENTS.md 2026-05-08 | Any popup in an overflow:hidden parent without createPortal? → P0 |
| Section header font | AGENTS.md 2026-05-08 | Any `@atlaskit/heading size="small"` used as a section header? → P1 |
| Assignee "Assign to me" link | AGENTS.md 2026-05-06 | Right rail Assignee field without idle "Assign to me" link (11px, blue)? → P1 |
| Watcher Escape propagation | AGENTS.md 2026-05-08 | WatchersChip popover without capture-phase Escape handler? → P1 |
| Row click navigation | AGENTS.md 2026-05-10 | navigate('/issues/:key') used in any For You / home panel? → P0 |
| Projects route | AGENTS.md 2026-05-10 | navigate('/projects') used for "View all projects"? → P0 |
| Section count badges | AGENTS.md 2026-05-05 | Any pill/badge on section header counts? → P2 |
| Right rail idle borders | AGENTS.md 2026-05-05 | Any visible border on idle select fields in right rail? → P2 |
| InlineCreateWithAI empty guard | AGENTS.md 2026-05-08 | InlineCreate only inside rows.length > 0? → P1 |
| Focused-state border span | AGENTS.md 2026-05-08 | Key cell focus border uses inline-block instead of block + width:100%? → P1 |
| Comment badge dot | AGENTS.md 2026-05-08 | Comments cell missing 6px blue dot on rows with comments? → P2 |
...
| Priority in wrong location | AGENTS.md 2026-05-06 | Priority in right rail for non-Epic types? (Must be in Key details left). Priority in left for Epic? (Must be in right rail). → P1 |
| Star button dead interaction | AGENTS.md 2026-05-10 | AgeingPanel star button rendered but no-op? → P1 |
| Motion not ADS-spec | AGENTS.md 2026-04-28 | Any linear/ease/custom-bezier animation not from ADS motion spec? → P2 |
...
```
📋 Lessons-applied pre-scan: {N} AGENTS.md anchors checked
  ✅ {M} clear
  🔴 {X} active violations (listed below with AGENTS.md anchor)
...
**AGENTS.md anchor:** 2026-05-09 — "carry the full violation list across sessions (arrow continuity)."
...
| Popup/popover | Self-rolled `useRef` + mousedown (AGENTS.md 2026-05-05 — `@atlaskit/popup` v4 has empty-portal bug) | — | — | `@atlaskit/popup` v4, shadcn Popover |
...
Each council finding MUST reference an ADS URL or AGENTS.md ban. Lens 2h (AtlasKit Architect) is mandatory alongside 2a–2g.
...
## Permanent Bans (AGENTS.md aligned)

These are banned at AGENTS.md level AND at ADS level:
...
| `@atlaskit/popup` v4 | Empty-portal bug (AGENTS.md 2026-05-05) | Self-rolled `useRef` + mousedown |
...
| Coloured dots for work item types | AGENTS.md 2026-05-09 — non-discoverable colour-recall | `JiraIssueTypeIcon` |
| `WorkItemIcon` (deprecated shim) | AGENTS.md 2026-05-09 | `JiraIssueTypeIcon` from `@/lib/jira-issue-type-icons` |
| MDT Ref field | AGENTS.md permanent ban | Never rendered |
| Service Now# field | AGENTS.md permanent ban | Never rendered |
| Assessment Feature field | AGENTS.md permanent ban | Never rendered |
| Story Points field | AGENTS.md permanent ban | Never rendered |
| AI Sparkles inline button | AGENTS.md permanent ban | AI improve only via right-rail ImproveIssueDropdown |
| Development section | AGENTS.md permanent ban | Never implemented |
| Automation section | AGENTS.md permanent ban | Never implemented |
| Notion in Projects module | AGENTS.md 2026-05-09 | Out of scope entirely |
...
```
Phase 0   — Bootstrap (Obsidian-primary, AGENTS.md gates, port 8080 lock)
Phase 0.5 — Jira Architect Scan (28 patterns, always runs)
Phase 1   — THIS SKILL fires here (all 8 lenses, ADS ring-fenced, 1000-IQ)
            → Lessons-applied pre-scan (AGENTS.md anchors + JIRA_ARCHITECT.md patterns)
...
### Skills
- `AGENTS.md` (worktree root) — ban list, canonical rules
