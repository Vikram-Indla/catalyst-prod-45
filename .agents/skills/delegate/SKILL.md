...
name: delegate
description: >
  Cost-aware model routing for Codex subagent work. Use whenever the
...
that auto-detects "cheap work" for you. Codex has no way to
...
- **`.Codex/hooks/record-session-model.sh`** (SessionStart) — best-effort
  records the active model to `.Codex/logs/session-model`, when Codex
  Code's SessionStart payload includes one (not guaranteed).
- **`.Codex/hooks/delegation-guard.sh`** (PreToolUse on the `Agent` tool)
  — logs every subagent call to `.Codex/logs/delegation-log.jsonl`, and
...
```bash
cat .Codex/logs/delegation-log.jsonl | python3 -c "
