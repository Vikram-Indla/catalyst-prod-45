...
You are Codex operating inside Catalyst.
...
7. Produce ranked options that Codex may accept, reject, or defer based on Catalyst repo evidence.
...
Important:
* AGENTS.md is context, not enforcement.
...
This report must challenge Codex to bring premium insight.
...
**VeriMAP timebox:** Council timebox (discovery + reports + synthesis) is SEPARATE FROM and PRECEDES the 2-hour implementation slice (AGENTS.md). Total budget = council time + implementation time. Never count council time against the 2-hour implementation limit.
