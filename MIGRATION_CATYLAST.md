...
## 0. CURRENT STATE — Phase 1 done, awaiting commit (as of 2026-05-11)

**TL;DR for a fresh Claude session in `catalyst-prod-45`:** Phase 1
setup is **complete and verified**. The seven `@catylast/*` packages
are vendored as workspace packages, npm install resolves cleanly, all
seven packages build, and Storybook 10 boots on port 6007 alongside
prod's existing Storybook 8 on port 6006. The user has NOT committed
yet — they're testing first. **Your immediate job is to diagnose
prod-app console errors and stage the commit.**
...
- ⏳ **User reports "a lot of errors in the console" on the prod app** —
  these were not captured. **First action of the next session:** start
  `npm run dev`, open the app at `localhost:8080`, copy the console
  output and classify each error:
...
- ⏳ Phase 1 commit (user is waiting until console errors are explained)

### Recommended commit message for Phase 1

```
feat: vendor @catylast component library as workspace packages

Adds 7 packages under packages/* (tokens, theme, icons, primitives,
card, rich-editor, dynamic-table) and a parallel Storybook 10 docs
site under apps/storybook-catylast. Wires npm workspaces in
package.json, path mappings in tsconfig.app.json, and top-of-file CSS
+ font imports in src/main.tsx. No src/ imports changed — this PR is
pure infrastructure.

Two Atlaskit packages promoted from `overrides`-only to direct deps
so they install at the top level of node_modules (npm on Windows does
not hoist overrides-only packages, but vite.config.ts aliases expect
them at the top):
- @atlaskit/adf-schema 52.6.6 (was overrides-only)
- @atlaskit/editor-plugins 13.0.120 (was overrides-only)

Versions match existing overrides — behavior identical on Bun/macOS.

See MIGRATION_CATYLAST.md for the full migration plan.
```
