-- ============================================================================
-- STRATA Execution — manual Excel import RPC.
-- CAT-STRATA-20260705-001 · Session 007 (Execution Manual Excel Import)
--
-- One workbook, three sheets (Project Cards, Milestones, Delivery Dependencies),
-- through the corrected Project Card-centric model (Execution Reconciliation
-- §K). Project Reference ID is the stable business key; Milestones and
-- Dependencies link to Project Cards via Project Reference ID. Strategic Theme
-- must match an existing configured theme — never auto-created.
--
-- Reuses the existing single-row write RPCs (strata_create/update_project_card,
-- strata_create/update_milestone, strata_create/update_dependency) for all
-- field-setting logic — this RPC only orchestrates: resolves cross-sheet
-- Project Reference ID links, resolves Strategic Theme / owner names, detects
-- create-vs-update, and patches the two columns those RPCs don't expose
-- (reference_id, dependency name) via a direct UPDATE immediately after
-- create (D-015 — no existing RPC signatures are widened, zero regression
-- surface on the manual "New project card" / "New dependency" UI forms).
--
-- p_dry_run=true (preview/validate): computes every row's outcome and errors,
-- writes NOTHING. p_dry_run=false (confirm): creates a strata_upload_runs row
-- (channel='excel'), performs the writes, and records strata_lineage_records
-- (source_system='upload', import_method='manual_excel') + strata_audit_events
-- for every written entity — the existing create/update RPCs already write
-- their own audit events; this function adds the lineage record and, for
-- milestones/dependencies (which have no audit call of their own for the
-- import-specific "batch" context), relies on the same underlying RPCs' audit
-- inserts.
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.strata_safe_date(p_text text)
...
END;
$$;
COMMENT ON FUNCTION public.strata_safe_date(text) IS
  'Parses text to date, returning NULL (never raising) on blank or unparseable input — callers distinguish blank-vs-malformed for validation messages.';

CREATE OR REPLACE FUNCTION public.strata_safe_numeric(p_text text)
...
CREATE OR REPLACE FUNCTION public.strata_resolve_profile_by_name(p_name text)
RETURNS TABLE(matched_id uuid, match_count int) LANGUAGE sql STABLE AS $$
  WITH matches AS (
    SELECT id FROM public.profiles
     WHERE btrim(coalesce(p_name, '')) <> ''
       AND (
         (p_name ~ '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$' AND lower(email) = lower(btrim(p_name)))
         OR (NOT (p_name ~ '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$') AND lower(full_name) = lower(btrim(p_name)))
       )
  )
  SELECT (array_agg(id))[1], count(*)::int FROM matches;
$$;
COMMENT ON FUNCTION public.strata_resolve_profile_by_name(text) IS
  'Resolves an Excel owner-name cell to a profiles.id by exact (case-insensitive) match on email (if the cell looks like one) or full_name otherwise. match_count=0 or >1 means the caller must reject the row rather than guess (zero-assumption rule).';

CREATE OR REPLACE FUNCTION public.strata_resolve_picklist_value(p_key text, p_input text)
RETURNS text LANGUAGE sql STABLE AS $$
  SELECT value FROM public.strata_project_card_picklists
   WHERE picklist_key = p_key AND is_active
     AND (lower(value) = lower(btrim(p_input)) OR lower(label) = lower(btrim(p_input)))
   LIMIT 1;
$$;
COMMENT ON FUNCTION public.strata_resolve_picklist_value(text, text) IS
  'Resolves an Excel status/enum cell against a governed strata_project_card_picklists entry (matches value or label, case-insensitive). Returns NULL if the cell does not match any configured value.';
...
BEGIN
  ...
  FOR v_row, v_idx IN SELECT value, ordinality::int FROM jsonb_array_elements(coalesce(p_project_cards, '[]'::jsonb)) WITH ORDINALITY LOOP
    ...
    IF v_theme_name IS NULL THEN v_errors := array_append(v_errors, 'Strategic Theme is required'); END IF;
    IF v_business_owner IS NULL THEN v_errors := array_append(v_errors, 'Business Owner is required'); END IF;
    IF v_pm IS NULL THEN v_errors := array_append(v_errors, 'Project Manager is required'); END IF;
    IF v_lbu IS NULL THEN v_errors := array_append(v_errors, 'Lead Business Unit is required'); END IF;
    IF v_dteam IS NULL THEN v_errors := array_append(v_errors, 'Delivery Team is required'); END IF;
    IF v_status_raw IS NULL THEN v_errors := array_append(v_errors, 'Delivery Status is required'); END IF;
    IF v_scope IS NULL THEN v_errors := array_append(v_errors, 'Scope Description is required'); END IF;
    IF v_outcomes IS NULL THEN v_errors := array_append(v_errors, 'Target Outcomes is required'); END IF;
    IF v_success IS NULL THEN v_errors := array_append(v_errors, 'Success Criteria is required'); END IF;
    ...
    IF v_business_owner IS NOT NULL THEN
      ...
      IF v_owner_match.match_count = 0 THEN
        v_errors := array_append(v_errors, format('Business Owner "%s" does not match any user', v_business_owner));
    ...
    IF v_pm IS NOT NULL THEN
      ...
      IF v_pm_match.match_count = 0 THEN
        v_errors := array_append(v_errors, format('Project Manager "%s" does not match any user', v_pm));
    ...
    IF array_length(v_errors, 1) > 0 THEN
      ...
      v_card_results := v_card_results || jsonb_build_object(
        ...
        'status', 'error', 'action', NULL, 'errors', to_jsonb(v_errors));
      CONTINUE;
    END IF;
    ...
    IF p_dry_run THEN
      ...
      v_card_results := v_card_results || jsonb_build_object(
        'row_number', v_idx, 'reference_id', v_ref, 'name', v_name,
        'status', 'valid', 'action', v_action, 'errors', '[]'::jsonb);
      CONTINUE;
    END IF;
    ...
    BEGIN
      ...
      v_card_results := v_card_results || jsonb_build_object(
        'row_number', v_idx, 'reference_id', v_ref, 'name', v_name,
        'status', 'valid', 'action', v_action, 'id', v_card_id, 'errors', '[]'::jsonb);
    EXCEPTION WHEN OTHERS THEN
      v_cards_rejected := v_cards_rejected + 1;
      v_card_results := v_card_results || jsonb_build_object(
        'row_number', v_idx, 'reference_id', v_ref, 'name', v_name,
        'status', 'error', 'action', NULL, 'errors', to_jsonb(ARRAY[SQLERRM]));
    END;
  END LOOP;
  ...
  FOR v_row, v_idx IN SELECT value, ordinality::int FROM jsonb_array_elements(coalesce(p_milestones, '[]'::jsonb)) WITH ORDINALITY LOOP
    ...
    IF v_ms_owner IS NULL THEN v_errors := array_append(v_errors, 'Milestone Owner is required'); END IF;
    ...
    IF v_ms_owner IS NOT NULL THEN
      ...
      IF v_owner_match.match_count = 0 THEN
        v_errors := array_append(v_errors, format('Milestone Owner "%s" does not match any user', v_ms_owner));
      ELSIF v_owner_match.match_count > 1 THEN
    ...
    BEGIN
      ...
      v_milestone_results := v_milestone_results || jsonb_build_object(
        'row_number', v_idx, 'project_reference_id', v_ref, 'name', v_name,
        'status', 'valid', 'action', v_action, 'id', v_ms_id, 'errors', '[]'::jsonb);
    EXCEPTION WHEN OTHERS THEN
      v_ms_rejected := v_ms_rejected + 1;
      v_milestone_results := v_milestone_results || jsonb_build_object(
        'row_number', v_idx, 'project_reference_id', v_ref, 'name', v_name,
  ...
  FOR v_row, v_idx IN SELECT value, ordinality::int FROM jsonb_array_elements(coalesce(p_dependencies, '[]'::jsonb)) WITH ORDINALITY LOOP
    ...
    IF v_dep_desc IS NULL THEN v_errors := array_append(v_errors, 'Dependency Description is required'); END IF;
    IF v_dep_req IS NULL THEN v_errors := array_append(v_errors, 'Requesting Project / Team is required'); END IF;
    IF v_dep_serving IS NULL THEN v_errors := array_append(v_errors, 'Serving Department / Team is required'); END IF;
    IF v_status_raw IS NULL THEN v_errors := array_append(v_errors, 'Status is required'); END IF;
    IF v_dep_owner IS NULL THEN v_errors := array_append(v_errors, 'Owner is required'); END IF;
    IF v_dep_impact IS NULL THEN v_errors := array_append(v_errors, 'Impact Note is required'); END IF;
    ...
    v_dep_blocker := lower(btrim(coalesce(v_row->>'blocker', ''))) IN ('true','yes','y','1');
    IF nullif(btrim(coalesce(v_row->>'blocker','')),'') IS NULL THEN
      v_errors := array_append(v_errors, 'Blocker is required');
    ...
    IF v_dep_owner IS NOT NULL THEN
      ...
      IF v_owner_match.match_count = 0 THEN
        v_errors := array_append(v_errors, format('Owner "%s" does not match any user', v_dep_owner));
      ELSIF v_owner_match.match_count > 1 THEN
    ...
    BEGIN
      ...
      IF v_existing_dep IS NULL THEN
        v_dep_id := public.strata_create_dependency(
          ...
          p_serving_type := 'external', p_serving_label := v_dep_serving,
      ...
      v_dependency_results := v_dependency_results || jsonb_build_object(
        'row_number', v_idx, 'project_reference_id', v_ref, 'name', v_name,
        'status', 'valid', 'action', v_action, 'id', v_dep_id, 'errors', '[]'::jsonb);
    EXCEPTION WHEN OTHERS THEN
      v_dep_rejected := v_dep_rejected + 1;
      v_dependency_results := v_dependency_results || jsonb_build_object(
        'row_number', v_idx, 'project_reference_id', v_ref, 'name', v_name,
