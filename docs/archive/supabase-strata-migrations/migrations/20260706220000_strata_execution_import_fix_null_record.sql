-- ============================================================================
-- STRATA Execution import — fix "record not assigned" crash on blank owners.
-- CAT-STRATA-20260705-001 · Session 009 (Execution Excel Import v2)
--
-- Real bug found via live testing with the "Jira (13).xls" fixture: setting
-- `v_owner_match := NULL;` gives the record variable no known row-type shape.
-- Dereferencing `v_owner_match.matched_id` afterwards raises
-- "record ... is not assigned yet" — but ONLY on the actual write path
-- (dry-run never reaches the create/update calls that dereference it), so
-- every dry-run preview showed "All rows pass validation" while the real
-- confirm rejected every single row. Root cause: the 20260706210000 migration
-- (relaxed required fields) made Business Owner/Project Manager/Milestone
-- Owner/Dependency Owner blank-able — and this file's real data has ALL of
-- them blank, so the owner-lookup block is skipped entirely and the record
-- is left in its just-reset NULL state.
--
-- Fix: track the resolved profile id in a plain `uuid` scalar per owner
-- field, defaulted to NULL and only set when exactly one profile matches —
-- never dereference the lookup record when the lookup was never attempted.
-- ============================================================================
...
DECLARE
  ...
  v_owner_match record; v_business_owner_id uuid; v_pm_id uuid;
  v_existing_card uuid; v_card_id uuid; v_action text;
  v_ms_owner text; v_ms_owner_id uuid; v_ms_fe date; v_ms_ae date; v_ms_progress numeric; v_ms_weight numeric;
  v_ms_id uuid; v_existing_ms uuid;
  v_dep_desc text; v_dep_req text; v_dep_serving text; v_dep_owner text; v_dep_owner_id uuid; v_dep_blocker boolean;
...
BEGIN
  ...
  FOR v_row, v_idx IN SELECT value, ordinality::int FROM jsonb_array_elements(coalesce(p_project_cards, '[]'::jsonb)) WITH ORDINALITY LOOP
    ...
    IF v_business_owner IS NOT NULL THEN
      ...
      ELSE
        v_business_owner_id := v_owner_match.matched_id;
      END IF;
    END IF;
    IF v_pm IS NOT NULL THEN
      SELECT * INTO v_owner_match FROM public.strata_resolve_profile_by_name(v_pm);
      IF v_owner_match.match_count = 0 THEN
        v_warnings := array_append(v_warnings, format('Project Manager "%s" did not match any user — left blank', v_pm));
      ELSIF v_owner_match.match_count > 1 THEN
        v_errors := array_append(v_errors, format('Project Manager "%s" matches %s users — ambiguous', v_pm, v_owner_match.match_count));
      ELSE
        v_pm_id := v_owner_match.matched_id;
      END IF;
    END IF;
    ...
    BEGIN
      IF v_existing_card IS NULL THEN
        v_card_id := public.strata_create_project_card(
          ...
          p_pm := v_pm_id, p_stage := v_status, p_theme := v_theme_id,
          p_business_owner := v_business_owner_id, p_lead_business_unit := v_lbu, p_delivery_team := v_dteam,
      ...
      ELSE
        PERFORM public.strata_update_project_card(
          p_project := v_existing_card, p_name := v_name, p_pm := v_pm_id,
  ...
  FOR v_row, v_idx IN SELECT value, ordinality::int FROM jsonb_array_elements(coalesce(p_milestones, '[]'::jsonb)) WITH ORDINALITY LOOP
    ...
    IF v_ms_owner IS NOT NULL THEN
      ...
      ELSE
        v_ms_owner_id := v_owner_match.matched_id;
      END IF;
    END IF;
    ...
    BEGIN
      ...
      IF v_existing_ms IS NULL THEN
        v_ms_id := public.strata_create_milestone(
          p_project := v_project_card_id, p_name := v_name, p_owner := v_ms_owner_id,
      ...
      ELSE
        PERFORM public.strata_update_milestone(
          p_milestone := v_existing_ms, p_name := v_name, p_owner := v_ms_owner_id,
          p_baseline_start := v_bs, p_baseline_end := v_be, p_forecast_date := v_ms_fe, p_actual_date := v_ms_ae,
          p_status := v_status, p_progress := v_ms_progress, p_weight := v_ms_weight,
          p_source_system := 'upload');
  ...
  FOR v_row, v_idx IN SELECT value, ordinality::int FROM jsonb_array_elements(coalesce(p_dependencies, '[]'::jsonb)) WITH ORDINALITY LOOP
    ...
    IF v_dep_owner IS NOT NULL THEN
      ...
      ELSE
        v_dep_owner_id := v_owner_match.matched_id;
      END IF;
    END IF;
    ...
    BEGIN
      ...
      IF v_existing_dep IS NULL THEN
        v_dep_id := public.strata_create_dependency(
          ...
          p_impact := v_dep_impact, p_description := v_dep_desc, p_owner := v_dep_owner_id,
      ...
      ELSE
        PERFORM public.strata_update_dependency(
          ...
          p_owner := v_dep_owner_id, p_baseline_start := v_bs, p_baseline_end := v_be,
          p_source_system := 'upload');
