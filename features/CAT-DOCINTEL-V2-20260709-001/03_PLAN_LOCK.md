...
**Execution status:** Slices 1–8A COMPLETE; Slice 8B next.
...
**Status:** COMPLETE — 2026-07-11; readable source, safe evidence states and focus return proven.
...
6. `src/modules/docintel/DocintelRoutes.tsx` — mount the completed hub in place of the pending page.
7. `src/modules/docintel/pages/__tests__/DocintelRoutes.test.tsx` — prove the namespaced route mounts
   the real hub and still wins over the legacy slug route.
...
5. `supabase/migrations/20260712000726_docintel_promotion_recovery.sql` — ADDITIVE durable recovery
   ledger with project-member RLS; generated by the Supabase CLI.
6. `src/modules/docintel/types.ts` — typed persisted recovery result only.
7. `src/modules/docintel/domain/index.ts` — read/upsert/complete the recovery ledger without
   recreating or deleting work.

**Binary acceptance:** draft/verified cannot promote; approved can; created-work/link-failed remains
visible with retry across reloads; retry never recreates or deletes work; full success is shown only
after artifact status and every provenance link persist.
