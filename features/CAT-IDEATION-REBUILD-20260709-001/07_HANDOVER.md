...
- (uncommitted as of this handover — see below) Fixes from the live verification pass: `VoteControl`'s `@atlaskit/popup` → manual-position rewrite, error-surfacing fix in `useIdeationMerge.ts`/`useIdeationConvert.ts` (was throwing raw Postgrest objects, not `Error` instances — masked the real RLS error behind a generic message), compensating-cleanup fix attempt in Conversion (confirmed still non-functional due to the second RLS gap, documented not hidden).
