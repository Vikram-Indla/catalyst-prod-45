...
- **Phase 4** (AI Copilot): **explicitly OUT OF SCOPE for module acceptance** (D14, 2026-07-11). `03_PLAN_LOCK_PHASE4_S1_CLASSIFICATION_DRAFT.md` remains a standing, concrete first-slice plan (classification suggestion: one edge function call to the already-existing `_shared/llm.ts` gateway, one `idn_ai_suggestions` insert, one review card) for whenever this becomes its own future feature — not a blocker on this feature's completion. If picked up later: check whether `idn_ai_suggestions` needs its own UPDATE RLS policy for the accept/reject path first, given the self-referential bug class `idn_ideas_update` had.
- **Deferred, non-blocking gaps** (module-accepted without these; see `04_ELITE_DESIGN_BLUEPRINT.md`'s P0/P1 table for the full list): Merge's vote/evidence/watcher transfer (needs a `SECURITY DEFINER` RPC — client RLS structurally can't move another user's rows, see `11_KARPATHY_LOOP_LOG.md` #15), evidence `document`/`voice_transcript`/`image` kinds (need docintel/voice-flow/attachment infra), remaining notification events (`idea_submitted`/`idea_triage_assigned`/`idea_mentioned`/`idea_vote_milestone`/`idea_merged`/`idea_converted`/`idea_delivered`/`idea_ai_suggestions_ready` — see Phase 6 S1's Plan Lock non-scope for why each is deferred), scoring-model editing/publish flow (Admin page is read-only), role-gating the workflow action buttons (pre-existing gap across the *whole* canonical workflow system, not ideation-specific — `ph_wf_transition_roles` role groups aren't mapped to `product_roles.code` anywhere), Homepage widgets/ForYou section (deliberately not touched — shared universal infrastructure, real regression risk), AR/RTL pass, `VITE_ENABLE_IDEATION` still defaults false (dark-launch flag, `src/lib/featureFlags.ts:110`) — exposing the module to real users is a separate deploy decision, not made in this session. Also: `HubSwitcher.tsx:73`'s `href: '/ideation/backlog'` doesn't match any mounted route (actual root is `/ideation`) — a one-line link fix needed whenever the flag flips on.
...
**Prod** (`lmqwtldpfacrrlvdnmld`): explicitly out of scope per D13 — this feature's acceptance is staging-only. (Separately noted: prod is currently paused; irrelevant to this feature's scope, recorded here only in case it's relevant to unrelated future work.)
