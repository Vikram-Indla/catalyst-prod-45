...
- **Phase 4b — partial (2026-06-27):** timer DONE (duration_seconds wired+proven). evidence-attach NOT built (no upload UI; deferred). defect-from-exec works (auto-trigger → tm_defects).
- **Phase 5 (Defects) — DONE & PROVEN (2026-06-27), model REVISED (D14):** D1 (defects=ph_issues) was incompatible with the DEMO seed (0 ph_issues for DEMO); the engine works on tm_defects. **D14 keeps tm_defects** as the testhub defect store (ph_issues deferred to real-project integration). DefectsPage = canonical BacklogPage/JiraTable on tm_defects; shows DEF-00001 (auto-created from the Phase 4a failed run, source_test_run_id links back). Fixed defectsDataSource status options (were UPPERCASE + invalid FIXED/VERIFIED/etc; real tm_defect_status enum = open/in_progress/resolved/closed/reopened) → status update now persists (PATCH 200). tm_defect_links table available for manual links.
- **Active slice:** none (sign-off gate; Phases 1, 2, 3a, 4a, 5 done).
- **Execution authorization:** Phases 1/2/3a/4a/5 + enum-fixes done this session. Next: Phase 4b evidence-attach (new build), 4c multi-run, Phase 3b (assignees), Phase 6 (Traceability), Phase 7 (Admin), on sign-off.
