...
Return JSON: {"description": "...", "acceptance_criteria": "..."}`;
    ...
    // ─────────────────────────────────────────────────────────────
    // Comment-summary branches (`summarize_comments_v2` streaming +
    // legacy `summarize_comments`) were extracted into the dedicated
    // `summarize-comments` edge function on 2026-06-14 so this file
    // stops being a multi-purpose handler. Clients now POST to
    // `summarize-comments` directly. The Phase 7d standup-status-changes
    // payload + grouped-by-standup prompt live in that function.
    // ─────────────────────────────────────────────────────────────
