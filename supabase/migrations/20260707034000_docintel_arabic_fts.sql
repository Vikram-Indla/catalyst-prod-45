-- ============================================================================
-- DOCINTEL — upgrade Arabic FTS from 'simple' to 'arabic' config (probed on
-- staging 2026-07-06: pg_ts_config contains 'arabic'). Idempotent re-create.
-- CAT-DOCINTEL-ARABIC-RAG-20260706-001
-- ============================================================================
-- DOCINTEL — hybrid search + fact match RPCs
-- CAT-DOCINTEL-ARABIC-RAG-20260706-001
-- Cloned from kb_hybrid_search (RRF fusion of vector + runtime FTS), adapted:
--   * project-scoped (membership enforced inside the function)
--   * per-lang FTS config: 'simple' for Arabic, 'english' for English
--   * DocIntel filters: document_ids, langs, content_kinds, page range, confidence
-- FTS computed at query time (no stored tsvector column), mirroring kb_hybrid_search.
-- SECURITY DEFINER + SET search_path = public per house style.
...
BEGIN
  ...
  keyword_results AS (
    ...
    SELECT
      ...
      ce.page_numbers, ce.block_ids,
      ts_rank_cd(
        to_tsvector(CASE WHEN ce.lang = 'ar' THEN 'arabic' ELSE 'english' END, ce.content),
        websearch_to_tsquery(CASE WHEN ce.lang = 'ar' THEN 'arabic' ELSE 'english' END, query_text)
      ) AS kw_score,
      ROW_NUMBER() OVER (
        ORDER BY ts_rank_cd(
          to_tsvector(CASE WHEN ce.lang = 'ar' THEN 'arabic' ELSE 'english' END, ce.content),
    ...
    FROM candidate_embeddings ce
    WHERE to_tsvector(CASE WHEN ce.lang = 'ar' THEN 'arabic' ELSE 'english' END, ce.content)
          @@ websearch_to_tsquery(CASE WHEN ce.lang = 'ar' THEN 'arabic' ELSE 'english' END, query_text)
    LIMIT match_count * 2
  ),
  fused AS (
...
-- ---------------------------------------------------------------------------
-- docintel_match_facts — vector match over requirement facts (via their chunk
-- embeddings). Project-scoped; returns confirmed/unreviewed facts by similarity.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    rf.id, rf.document_id, rf.kind, rf.statement_en, rf.statement_ar, rf.review_status,
    1 - (e.embedding <=> query_embedding) AS similarity
  FROM public.ai_requirement_facts rf
  JOIN public.ai_document_embeddings e
    ON e.document_id = rf.document_id
