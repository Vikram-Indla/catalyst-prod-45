...
DROP FUNCTION IF EXISTS public.kb_hybrid_search(vector, text, integer, double precision, double precision, text, text[], integer);
...
DROP FUNCTION IF EXISTS public.kb_match_embeddings(vector, double precision, integer, text);

CREATE FUNCTION public.kb_match_embeddings(
  query_embedding vector(1536),
...
BEGIN
  ...
  FROM public.kb_embeddings e
  WHERE (filter_source IS NULL OR e.source_type = filter_source)
    AND (1 - (e.embedding::vector(1536) <=> query_embedding::vector(1536))) > match_threshold
  ORDER BY e.embedding::vector(1536) <=> query_embedding::vector(1536)
  LIMIT match_count;
END;
$$;