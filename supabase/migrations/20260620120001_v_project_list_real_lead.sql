-- v_project_list: surface the real project lead instead of hardcoded NULL.
--
-- RCA 2026-06-20: the view selected `NULL::uuid AS lead_id` (+ NULL name/avatar)
-- in BOTH unions, discarding the real projects.lead_id. The UI then fell back to
-- the current authenticated user, so the Lead column showed whoever was logged in
-- on every project. Fix: join projects.lead_id -> profiles in the `projects`
-- branch and select the real lead. The ph_projects branch has no lead column and
-- stays NULL (genuinely unassigned).
CREATE OR REPLACE VIEW v_project_list AS
 SELECT p.id,
    ...
    p.department,
    p.description,
    COALESCE(p.display_status, p.status) AS status,
    p.health_status,
    p.status_category,
 ...
 SELECT ph.id,
   ...
   FROM ph_projects ph
     ...
     LEFT JOIN LATERAL ( SELECT count(*)::integer AS total_count,
            ...
            count(*) FILTER (WHERE i.status = ANY (ARRAY['done'::text, 'Done'::text, 'closed'::text, 'Closed'::text, 'resolved'::text, 'Resolved'::text]))::integer AS done_count
           FROM ph_issues i
          WHERE i.project_key = ph.key::text) pic2 ON true
