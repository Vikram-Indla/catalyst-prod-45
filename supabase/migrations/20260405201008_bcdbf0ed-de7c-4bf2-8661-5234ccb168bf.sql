...
-- Attach triggers to the 4 new tables
DROP TRIGGER IF EXISTS trg_notify_features ON features;
...
DROP TRIGGER IF EXISTS trg_notify_products ON products;
CREATE TRIGGER trg_notify_products
  AFTER INSERT OR UPDATE OF owner_id
  ON products
  FOR EACH ROW EXECUTE FUNCTION catalyst_notify_trigger();

DROP TRIGGER IF EXISTS trg_notify_objectives ON objectives;
CREATE TRIGGER trg_notify_objectives
  AFTER INSERT OR UPDATE OF owner_id, status
  ON objectives
  FOR EACH ROW EXECUTE FUNCTION catalyst_notify_trigger();

DROP TRIGGER IF EXISTS trg_notify_initiatives ON initiatives;
CREATE TRIGGER trg_notify_initiatives
  AFTER INSERT OR UPDATE OF owner_id
  ON initiatives
  FOR EACH ROW EXECUTE FUNCTION catalyst_notify_trigger();
