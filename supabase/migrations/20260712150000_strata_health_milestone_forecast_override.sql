-- CAT-STRATA-V6QA-20260712-001 — Wave 3 · V6-OPEN-034
--
-- A milestone whose Forecast End Date is >30 days beyond its baseline end did not
-- force Major Delay. Root cause: strata_calc_execution_progress derived
-- final_forecast ONLY from the earned-schedule system forecast and the
-- PROJECT-CARD forecast_end column (v_submitted_forecast). It never read
-- strata_milestones.forecast_date — the exact field the milestone "Forecast End
-- Date" control writes. So a milestone forecast of e.g. 30 Sep (31 days past a
-- 30 Aug milestone baseline end) was invisible to the >30-day override, which
-- evaluated the wrong forecast and stayed On Track.
--
-- Fix (RPC-only, no client change): add a milestone-forecast aggregate
-- max(m.forecast_date) and fold it into final_forecast alongside the system and
-- project forecasts. Postgres greatest() ignores NULLs, so final_forecast becomes
-- the latest of the three available forecasts (or NULL if none), and
-- forecast_variance_days + the >30-day override now see the milestone forecast.
--
-- Same function signature — CREATE OR REPLACE only. Every other rule is
-- byte-identical to 20260712110000 (which also carries the V5-OPEN-023
-- not_started precedence — unchanged here). Backfill recomputes all cards so a
-- stored On Track that should be Major Delay is corrected on apply.
--
-- Unapplied (D-1) — Vikram/release applies. Rollback = re-apply 20260712110000.
...
DECLARE
  ...
  delay_days numeric;
  sys_forecast date;
  m_forecast date;           -- V6-OPEN-034: latest milestone-level forecast date
  final_forecast date;
  fvar_days int;
  health text;
  reason text;
BEGIN
  ...
  -- V6-OPEN-034: also carry the latest milestone forecast_date so a milestone
  -- schedule slip feeds the forecast override below.
  SELECT min(m.baseline_start) FILTER (WHERE m.baseline_start IS NOT NULL),
         ...
         sum(m.baseline_end - m.baseline_start)
           FILTER (WHERE m.baseline_start IS NOT NULL AND m.baseline_end IS NOT NULL AND m.baseline_end >= m.baseline_start),
         max(m.forecast_date) FILTER (WHERE m.forecast_date IS NOT NULL)
    INTO b_start, b_end, a_num, a_den, m_forecast
  ...
  -- Rule 8: final forecast = the latest of the system forecast, the project
  -- submitted forecast_end, and the latest milestone forecast_date (V6-OPEN-034).
  -- greatest() ignores NULLs, so this is the latest available forecast, or NULL
  -- when none exists.
  final_forecast := greatest(sys_forecast, v_submitted_forecast, m_forecast);
  ...
  VALUES ('project_card', p_project, 'execution_progress', a_progress, variance, health,
          ...
          jsonb_build_object(
            ...
            'submitted_forecast_end', v_submitted_forecast, 'milestone_forecast_end', m_forecast,
            'final_forecast_end', final_forecast,
...
-- Backfill: recompute health for every project card so a stored On Track that
-- should be Major Delay (milestone forecast >30 days late) is corrected on apply.
DO $$
DECLARE r record;
BEGIN
