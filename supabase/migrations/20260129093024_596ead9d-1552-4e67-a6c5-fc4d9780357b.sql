-- Drop existing functions with various signatures
DROP FUNCTION IF EXISTS get_module_matrix();
DROP FUNCTION IF EXISTS get_module_matrix(TEXT);
DROP FUNCTION IF EXISTS get_module_matrix(TEXT, TEXT);
DROP FUNCTION IF EXISTS get_module_matrix(TEXT, TEXT, TEXT);
...
DROP FUNCTION IF EXISTS check_module_access(UUID, TEXT, TEXT);

-- ============================================
-- RPC 1: Get grouped matrix data (new signature)
-- ============================================
CREATE OR REPLACE FUNCTION get_module_matrix_v2(
...
-- ============================================
CREATE OR REPLACE FUNCTION update_module_permission_v2(
  p_role_code TEXT,
  p_module_key TEXT,
  p_access_level TEXT
)
...
-- ============================================
CREATE OR REPLACE FUNCTION get_permission_stats_v2(p_role_code TEXT DEFAULT NULL)
RETURNS JSONB AS $$
BEGIN
  RETURN (
...
-- ============================================
CREATE OR REPLACE FUNCTION get_module_groups_v2()
...
-- RPC 5: Bulk update
-- ============================================
CREATE OR REPLACE FUNCTION bulk_update_permissions_v2(
  p_module_keys TEXT[],
  p_role_codes TEXT[],
  p_access_level TEXT
)
  ...
  FOREACH v_module IN ARRAY p_module_keys LOOP
    FOREACH v_role IN ARRAY p_role_codes LOOP
      IF v_role != 'super_admin' THEN
        PERFORM update_module_permission_v2(v_role, v_module, p_access_level);
        v_count := v_count + 1;
      END IF;
    END LOOP;
  END LOOP;
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION get_module_matrix_v2 TO authenticated;
GRANT EXECUTE ON FUNCTION update_module_permission_v2 TO authenticated;
GRANT EXECUTE ON FUNCTION get_permission_stats_v2 TO authenticated;
GRANT EXECUTE ON FUNCTION get_module_groups_v2 TO authenticated;
GRANT EXECUTE ON FUNCTION bulk_update_permissions_v2 TO authenticated;