-- G20: Auto-versioning trigger for tm_test_cases

-- Ensure version column exists
ALTER TABLE public.tm_test_cases
ADD COLUMN IF NOT EXISTS current_version integer DEFAULT 1;

-- Ensure th_test_case_versions has steps and changed_fields
ALTER TABLE public.th_test_case_versions
ADD COLUMN IF NOT EXISTS steps jsonb DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS changed_fields text[];

-- Create the auto-versioning trigger function
...
BEGIN
  ...
  IF (
    ...
    FROM th_test_case_versions WHERE test_case_id = NEW.id;
    ...
    -- Insert version record (snapshot of OLD state before change)
