    ...
    FROM projects p
        ...
        LEFT JOIN LATERAL (
            SELECT count(*)::integer AS total_count,
                   count(*) FILTER (WHERE i.issue_type = 'Epic')::integer AS epic_count,
                   count(*) FILTER (WHERE i.issue_type = 'Feature')::integer AS feature_count,
                   count(*) FILTER (WHERE i.issue_type = 'Story')::integer AS story_count,
                   count(*) FILTER (WHERE i.issue_type IN ('Task', 'Sub-task', 'Bug'))::integer AS task_count,
                   count(*) FILTER (WHERE i.status IN ('to_do', 'To Do', 'backlog', 'Backlog', 'open', 'Open'))::integer AS todo_count,
                   count(*) FILTER (WHERE i.status IN ('in_progress', 'In Progress', 'in_review', 'In Review'))::integer AS in_progress_count,
                   count(*) FILTER (WHERE i.status IN ('done', 'Done', 'closed', 'Closed', 'resolved', 'Resolved'))::integer AS done_count
            FROM ph_issues i
            WHERE i.project_key = p.key
        ) pic ON true
    ...
    FROM ph_projects ph
        ...
        LEFT JOIN LATERAL (
            SELECT count(*)::integer AS total_count,
                   ...
                   count(*) FILTER (WHERE i.status IN ('done', 'Done', 'closed', 'Closed', 'resolved', 'Resolved'))::integer AS done_count
            FROM ph_issues i
            WHERE i.project_key = ph.key::text
        ) pic2 ON true
    WHERE ph.is_archived = false AND NOT (EXISTS (
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Skipping v_project_list: %', SQLERRM;
END $outer$;
