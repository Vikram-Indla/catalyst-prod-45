...
-- aqd_lists is created in a later migration (20260202063921); skip view creation if table absent.
-- The view will be created when aqd_lists is set up.
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='aqd_lists') THEN
    RAISE NOTICE 'aqd_lists not yet created, skipping aqd_lists_summary view';
    RETURN;
  END IF;
  EXECUTE $view$
