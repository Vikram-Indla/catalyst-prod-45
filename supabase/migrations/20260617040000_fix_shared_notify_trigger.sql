-- Repoint the shared catalyst_notify_trigger() from the non-existent
-- `catalyst_notifications` table to the live `public.notifications` table.
--
-- The function is fired by triggers on ~15 entity tables (stories, epics,
-- catalyst_issues, defects, incidents, releases, subtasks, t10_items,
-- features, products, objectives, initiatives, tm_test_cases, tm_cycle_scope).
-- It inserted into `catalyst_notifications`, which DOES NOT EXIST — a stale
-- landmine from a prior schema rename. Every code path that reached the
-- INSERT would ERROR (and abort the underlying assignee/status UPDATE). Most
-- of these tables are currently empty (legacy Lovable tables; live project
-- work is in ph_issues), which is why it never surfaced.
--
-- Every WHEN branch is preserved verbatim; only the 3 INSERTs are remapped to
-- the `notifications` schema:
--   user_id→recipient_user_id, actor_id→actor_user_id, type→notification_type,
--   icon_type→entity_icon_type, title→entity_title (entity name; the old
--   composed "X assigned to you" string is dropped — the UI composes display
--   text from notification_type + entity_title + entity_key), plus the
--   status / status_type / tab columns. SET search_path=public added.
--
-- The Tasks Hub keeps its own catalyst_notify_tasks_trigger (see
-- 20260617030000_tasks_notifications.sql) — untouched here.
...
BEGIN
  CASE TG_TABLE_NAME
    ...
    WHEN 'products' THEN
      ...
      v_hub_source   := 'ProductHub';
      v_icon_type    := 'product';
      v_entity_key   := COALESCE(NEW.code, NEW.id::TEXT);
  ...
  IF TG_OP = 'INSERT' AND v_new_assignee IS NOT NULL AND v_new_assignee IS DISTINCT FROM v_actor THEN
    INSERT INTO notifications (
      ...
      v_new_assignee, v_actor, 'assignment', v_entity_type, NEW.id,
      v_entity_title, v_entity_key, v_icon_type, v_hub_source,
      COALESCE(v_new_status, 'To Do'), v_status_color, 'direct', '{}'::jsonb
    );
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND v_new_status IS DISTINCT FROM v_old_status AND v_new_assignee IS NOT NULL AND v_new_assignee IS DISTINCT FROM v_actor THEN
    INSERT INTO notifications (
      ...
      v_new_assignee, v_actor, 'status_change', v_entity_type, NEW.id,
      v_entity_title, v_entity_key, v_icon_type, v_hub_source,
      COALESCE(v_new_status, 'unknown'), v_status_color, 'direct',
      jsonb_build_object('old_status', v_old_status, 'new_status', v_new_status, 'status_color', v_status_color)
    );
  END IF;
