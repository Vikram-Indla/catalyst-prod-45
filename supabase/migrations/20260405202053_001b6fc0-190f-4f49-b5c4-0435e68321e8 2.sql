...
-- STEP 1: DROP 8 legacy duplicate notification triggers
DROP TRIGGER IF EXISTS trg_catalyst_issue_change       ON catalyst_issues;
DROP TRIGGER IF EXISTS notify_on_story_assignment      ON stories;
DROP TRIGGER IF EXISTS notify_on_story_status_change   ON stories;
DROP TRIGGER IF EXISTS notify_on_subtask_assignment    ON subtasks;
DROP TRIGGER IF EXISTS notify_on_subtask_status_change ON subtasks;
...
-- STEP 2: FIX stories trigger — add owner_id to watch list
DROP TRIGGER IF EXISTS catalyst_notify_stories ON stories;
CREATE TRIGGER catalyst_notify_stories
  AFTER INSERT OR UPDATE OF assignee_id, owner_id, status
  ON stories FOR EACH ROW
  EXECUTE FUNCTION catalyst_notify_trigger();

-- STEP 3: FIX tm_cycle_scope trigger — add current_status to watch list
DROP TRIGGER IF EXISTS catalyst_notify_tm_cycle_scope ON tm_cycle_scope;
CREATE TRIGGER catalyst_notify_tm_cycle_scope
  AFTER INSERT OR UPDATE OF assigned_to, current_status
  ON tm_cycle_scope FOR EACH ROW
  EXECUTE FUNCTION catalyst_notify_trigger();

-- STEP 4: ADD trigger to planner_tasks (uses status_id, not status)
DROP TRIGGER IF EXISTS catalyst_notify_planner_tasks ON planner_tasks;
CREATE TRIGGER catalyst_notify_planner_tasks
  AFTER INSERT OR UPDATE OF assignee_id
  ON planner_tasks FOR EACH ROW
  EXECUTE FUNCTION catalyst_notify_trigger();

-- STEP 5: ADD trigger to subtasks
DROP TRIGGER IF EXISTS catalyst_notify_subtasks ON subtasks;
CREATE TRIGGER catalyst_notify_subtasks
  AFTER INSERT OR UPDATE OF assignee_id, status
  ON subtasks FOR EACH ROW
  EXECUTE FUNCTION catalyst_notify_trigger();

-- STEP 6: ADD trigger to t10_items
DROP TRIGGER IF EXISTS catalyst_notify_t10_items ON t10_items;
CREATE TRIGGER catalyst_notify_t10_items
  AFTER INSERT OR UPDATE OF assignee_id, status
  ON t10_items FOR EACH ROW
  EXECUTE FUNCTION catalyst_notify_trigger();

-- STEP 7: Rebuild catalyst_notify_trigger() with defect_key fix + planner_tasks status_id fix
