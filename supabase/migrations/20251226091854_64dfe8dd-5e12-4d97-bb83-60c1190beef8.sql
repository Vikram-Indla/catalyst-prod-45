...
-- C) Ensure epics.program_id is NOT NULL with proper default (skip if column absent)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='epics' AND column_name='program_id') THEN
    UPDATE epics
...
-- D) For features, inherit program_id from epic or use first program (skip if either column absent)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='features' AND column_name='program_id')
  AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='epics' AND column_name='program_id') THEN
    UPDATE features f
...
-- E) Migrate existing feature.project_id to feature_projects join table (skip if column absent)
DO $$ BEGIN
...
-- G) Create index for faster program-scoped queries (conditional on column existence)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='features' AND column_name='program_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_features_program_id ON features(program_id)';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='epics' AND column_name='program_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_epics_program_id ON epics(program_id)';
  END IF;
END $$;
