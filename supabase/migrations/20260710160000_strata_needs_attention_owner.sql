-- CAT-STRATA-CLOSEOUT-20260710-001 · W4 — "needs YOUR attention" scoping
--
-- The Needs-attention rule engine already surfaces the org's outstanding items
-- (attestations, validations, blockers, overdue actions/gates, assumptions,
-- missing actuals, upload rejections, governance drift, project delays) but is
-- period-scoped, not user-scoped. W4 adds a trailing owner_id column so the
-- Command Center can offer a "Mine" filter (items the current user must act on)
-- without a parallel query. owner_id is NULL where no single personal owner
-- exists (attestation, overdue gate, upload rejections) — those show only in "All".
--
-- Verbatim copy of the 20260709170000 definition with ONE added column per branch.
-- Adding a column changes the return type, so the old signature is dropped first
-- (no DB dependents — the only consumer is the client-side domain reader).

DROP FUNCTION IF EXISTS public.strata_needs_attention(uuid);
...
RETURNS TABLE (
  ...
  -- 1) KPI actuals awaiting attestation (validator unknown — not user-scoped)
  ...
  UNION ALL
  -- 2) Benefit values awaiting validation → the benefit's validator
  ...
  UNION ALL
  -- 3) Blocked dependencies → the dependency owner
  ...
  UNION ALL
  -- 4) Overdue actions → the action owner
  ...
  UNION ALL
  -- 5) Overdue gates (no single decider field — not user-scoped)
  ...
  UNION ALL
  -- 6) Broken assumptions → the assumption owner
  ...
  UNION ALL
  -- 7) Approved KPIs missing an actual for the (open) period → the KPI creator
  ...
  UNION ALL
  -- 8) Upload runs with rejected rows (no owner field — not user-scoped)
  ...
  UNION ALL
  -- 9) Active Themes with incomplete charters → the element owner
  ...
  UNION ALL
  -- 10) Project Card major delay → the project PM
  ...
  UNION ALL
  -- 11) Project Card health Not Available → the project PM
  ...
  UNION ALL
  -- 12) Project Card has an open blocker dependency → the project PM
...
COMMENT ON FUNCTION public.strata_needs_attention(uuid) IS
  'Rule-driven Needs Attention feed (F-REP-004 + Execution Health rule 15 + CLOSEOUT W4 owner_id). Trailing owner_id enables the Command Center "Mine" filter; NULL where no single personal owner exists (attestation, overdue gate, upload rejections).';
