-- ═══════════════════════════════════════════════════════════
-- FIX: Add notification trigger to tm_defects table
-- ═══════════════════════════════════════════════════════════
-- The catalyst_notify_trigger() only covers the legacy "defects" table.
-- TestHub operates on "tm_defects" — assigning a defect there produces
-- no notification because the trigger was never attached.
-- This migration:
--   1. Adds a WHEN clause for tm_defects in catalyst_notify_trigger()
--   2. Creates the trigger on tm_defects
-- ═══════════════════════════════════════════════════════════

-- STEP 1: Rebuild catalyst_notify_trigger() to include tm_defects
...
BEGIN
  CASE TG_TABLE_NAME
    ...
    -- NEW: TestHub tm_defects table (canonical defects table)
...
-- STEP 2: Attach trigger to tm_defects
DROP TRIGGER IF EXISTS trg_catalyst_notify_tm_defects ON tm_defects;
CREATE TRIGGER trg_catalyst_notify_tm_defects
