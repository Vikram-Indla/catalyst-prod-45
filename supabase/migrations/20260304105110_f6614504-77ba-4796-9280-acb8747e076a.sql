-- Fix: Drop and recreate t10 views to resolve dependency chain for deploy
-- Drop child view first, then parent, then recreate both

DROP VIEW IF EXISTS t10_completed_summary_stats;
DROP VIEW IF EXISTS t10_completed_weeks_summary;

CREATE OR REPLACE VIEW t10_completed_weeks_summary AS
SELECT 
  w.id AS week_id,
  w.list_id,
  w.week_start,
...
CREATE OR REPLACE VIEW t10_completed_summary_stats AS
SELECT 
