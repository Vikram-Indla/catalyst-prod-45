...
-- es_themes_list_view requires columns added later to es_strategic_themes
-- (owner_id, vision_statement, priority, etc.) and tables es_theme_groups, es_theme_milestones
-- that may not yet exist. Skip gracefully; the view will be (re)created once schema is complete.
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'es_strategic_themes' AND column_name = 'owner_id'
  ) THEN
    RAISE NOTICE 'es_strategic_themes.owner_id not found, skipping es_themes_list_view';
    RETURN;
  END IF;
  EXECUTE $view$
