-- Extend catalyst_notify_trigger() (builds on 20260617040000):
--   * Fix `initiatives` branch — the table has no status / title / initiative_key
--     columns (it has `name` + `id`); the branch referenced non-existent columns
--     and would error if ever hit. Now: entity_key = id, entity_title = name,
--     status = NULL.
--   * Add `tm_defects` branch — its trigger existed but the function had no
--     matching WHEN branch (silent no-op). Verified live (rolled back).
--   * Add `tm_plan_approvals` branch — INSERT-only trigger notifies the approver.
-- All other branches and the 3 notifications INSERTs are unchanged.
...
BEGIN
  CASE TG_TABLE_NAME
    ...
    WHEN 'tm_plan_approvals' THEN
      ...
      v_hub_source   := 'TestHub';
      v_icon_type    := 'bug';
      v_entity_key   := NEW.plan_id::TEXT;
      v_entity_title := 'Test plan approval';
    ...
    WHEN 'initiatives' THEN
      ...
      v_entity_key   := NEW.id::TEXT;                       -- no initiative_key column
      v_entity_title := COALESCE(NEW.name, 'Initiative');   -- uses name, not title
      v_new_assignee := NEW.owner_id;
      v_old_assignee := CASE WHEN TG_OP='UPDATE' THEN OLD.owner_id ELSE NULL END;
      v_new_status   := NULL;                               -- no status column
      v_old_status   := NULL;
      v_actor        := auth.uid();
