...
The Catalyst local dev server always runs on **http://localhost:8080**. Never use 8081. When navigating with agent-browser (the localhost browser tool — see 2026-05-11 lesson), always use port 8080.

---

## 2026-05-11 — Browser tooling is surface-based, not primary/fallback
...
**Pattern:** Browser tools are split by surface, not by primary/fallback. `agent-browser` (Vercel Labs, native Rust CLI + Chrome for Testing via CDP, accessibility-tree snapshots with `@eN` element refs) handles localhost and unauthenticated probes. Claude for Chrome handles authenticated Atlassian-hosted surfaces where the user's real SSO Chrome session is required. Install: `npx skills add vercel-labs/agent-browser` (skill stub at `.claude/skills/agent-browser → ../../.agents/skills/agent-browser/SKILL.md`) + `npm i -g agent-browser && agent-browser install`. Discovery: `Skill agent-browser`; runtime: `agent-browser …` shell commands. Read `agent-browser skills get core` before issuing commands — the CLI serves the canonical workflow doc for the installed version.
**Rule:** **agent-browser** owns: Catalyst `http://localhost:8080` DOM probes, accessibility-tree snapshots, screenshots, click/fill, network capture, CRUD test loops, and any public/unauthenticated page. **Claude for Chrome** owns: live Jira at `digital-transformation.atlassian.net`, Confluence, Bitbucket Cloud, and any Atlassian-authenticated surface that needs the user's SSO Chrome session. Each tool owns its surface — do not try agent-browser on authenticated Jira and then "fall back," and do not reach for Claude for Chrome on localhost. Operational notes: (i) Chrome for Testing must be installed via `agent-browser install` before agent-browser is usable on a given machine — if that step never ran (common in sandboxed Claude Code sessions where `googlechromelabs.github.io` is unreachable), say so explicitly in the user-facing update and skip the localhost probe for the session rather than silently routing to Claude for Chrome; (ii) all prior CLAUDE.md references to "Chrome MCP" map to agent-browser under this rule.
